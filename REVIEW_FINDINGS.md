...
### [ ] C1 — MDI interpreter errors never abort queued motion
...
### [ ] C2 — Bare S-word enables a stopped spindle and releases its brake
...
### [ ] C3 — ESTOP command while already estopped/off permanently kills the sequencer
...
### [ ] C4 — RigidTap velocity is clamped by the F word; tap cannot follow the spindle
...
### [ ] C5 — M66 wait types "rise"/"fall" implemented as levels, not edges
...
### [ ] C6 — M66 timeout contract broken: #5399 never gets −1
...
### [ ] C7 — G96 CSS in inch mode is 25.4× too slow
...
### [ ] C8 — finishMDI has no ownership token: stale-completion race + ExecError clobber
...
### [ ] C9 — A failed dequeued MDI strands the mdiQueue and causes out-of-order execution
...
### [ ] C10 — Pause/resume with no program running wedges the task
...
### [ ] C12 — IDL `@min(0)` on spindle_num rejects the −1 all-spindles broadcast this branch implements
...
### [ ] C13 — Constraint codegen reuses loop variable `i` at every nesting level (latent)
...
### [ ] T1 — PlasmaC launch-guard positive path left untested
...
### [ ] D2 — The machine-shutdown sequence is hand-rolled four times
...
### [ ] D3 — "Aborter owns the terminal state" enforced by convention at six sites
...
### [ ] D4 — autoSignal and autoCommand duplicate the pause/resume/step bodies
...
### [ ] D5 — The spindle "program owns the spindle" guard is written five times
...
### [ ] D6 — The idempotent channel-close idiom is copy-pasted 14 times
...
### [ ] D7 — Motion-drain barriers are sprinkled op-by-op instead of declared per command
...
### [ ] D8 — Validation block generated twice per API (REST + WS) with a regex-prefix scheme
...
### [ ] D9 — Client-side validation walker mirrors the server-side walker
...
### [ ] D10 — Named-type lookups implemented three times
...
### [ ] E1 — Every motion-drain barrier pays a ≥50-60 ms floor, even on an empty queue
...
### [ ] E2 — BuildStat does triple lookups and throwaway allocations per status cycle
...
### [ ] E3 — M66 wait copies the full MotionStatus every 10 ms to read one bit
...
### [ ] E4 — setSeqInflight costs two mutex round-trips per dequeued command
...
### [ ] E5 — finishMDI's o-word continuation pays a full drain round-trip even when nothing was queued
...
### [ ] E6 — Every (MSG,)/(DEBUG,) is logged twice, once misleadingly at read-ahead
...
### [ ] S1 — waitIdle hand-rolls a poll loop next to the generic waitForCond helper
...
### [ ] S2 — Blend test fixture (axis limits) defined twice
...
### [ ] S3 — moves.sh duplicates normalize.sh's float-rounding pipeline
...
### [ ] S4 — Nine near-identical link_test.go files
