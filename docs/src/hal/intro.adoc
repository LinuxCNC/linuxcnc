...
[[sub:hal-external-programs-hooks]]
=== External Programs with HAL hooks(((HAL External Programs,HAL Hooks)))
...
iocontrol:: (((HAL `iocontrol`)))
  A user space module that accepts NML I/O commands and
  interacts with HAL.

classicladder:: (((HAL `classicladder`)))
...
[[sub:hal-internal-components]]
=== Internal Components(((HAL Internal Components)))
...
blocks:: (((HAL `blocks`,HAL `mux`,HAL `demux`,HAL `or`,HAL `and`,HAL `integ`,HAL `ddt`,HAL `limit`,HAL `wcomp`)))
  Un assortiment de composants (mux, demux, or, and, integ,
  ddt, limit, wcomp, etc.).

[[sub:hal-hardware-drivers]]
=== Hardware Drivers(((HAL Hardware drivers)))

hal_ax5214h:: (((HAL `hal-ax5214h`)))
...
hal_m5i20:: (((HAL `hal-m5i20`)))
  Mesa Electronics 5i20 board.

hal_motenc:: (((HAL `hal-motenc`)))
  Vital Systems MOTENC-100 board.

hal_parport:: (((HAL `hal-parport`)))
  PC parallel port.

hal_ppmc:: (((HAL `hal-ppmc`)))
