...
These pins can carry numerical and logical values, boolean, floating point and signed and unsigned integers. There is also a pin type named hal_port intended for byte streams, and a framework for exchanging more complex data called hal_stream (which uses a private shared memory area, rather than a HAL pin).
...
+
- bool - a single TRUE/FALSE or ON/OFF value
- real - a double precision floating point value, with approximately 53 bits of
  resolution and over 1000 bits of dynamic range.
- uint - a 64 bit unsigned integer, legal values are 0 to 18,446,744,073,709,551,615
- sint - a 64 bit signed integer, legal values are -9,223,372,036,854,775,808 to
