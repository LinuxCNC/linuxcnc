...
`halshow` can also be started from a terminal command line and specify formats for integer and real items (pins or signals) and identify a saved watchlist file to use:
...
For more information regarding the format, please refer to the https://www.tcl.tk/man/tcl/TclCmd/format.html[Tcl format man page].
...
Because the pins and parameters of a component cannot share the same name, they are listed together in a single _Pins & Parameters_ branch, with parameters shown in brown.
To list pins and parameters in separate branches, enable _Separate parameters from pins in tree_ in the <<sec:halshow-settings,Settings>> tab.
...
06     bool   -W   TRUE  parport.0.pin-10-in
06     bool   -W  FALSE  parport.0.pin-10-in-not
----
...
_Watch_ displays bool type (binary) values using colored circles representing LEDs.
They show as dark red when a bool signal or pin is false, and as light yellow whenever that signal is true.
If you select a pin or signal that is not a bool type (binary) signal, _watch_ will show it as a numerical value.
...
Sets the format of the displayed values, for more information refer to the https://www.tcl.tk/man/tcl/TclCmd/format.html[Tcl format man page].
...
Separate parameters from pins in tree:: Lists pins and parameters in separate _Pins_ and _Parameters_ branches of the tree, instead of the combined _Pins & Parameters_ branch.
The setting takes effect when you press _Apply_ and is remembered between sessions.
...
Component Pins:
Owner Type  Dir Value       Name
08    real  R-  0.00000e+00 ddt.0.in
08    real  -W  0.00000e+00 ddt.0.out
----
...
Component Pins:
Owner Type  Dir Value       Name
03    real  -W  0.00000e+00 joint.0.motor-pos-cmd ==> Xpos-cmd
----
