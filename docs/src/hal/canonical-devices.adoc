...
Note that only the `_<io-type>_` and `_<specific-name>_` fields are
defined for a canonical device. The `_<device-name>`, `_<device-num>_`,
and `_<chan-num>_` fields are set based on the characteristics of the
real device.
