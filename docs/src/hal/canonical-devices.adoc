...
(((HAL Digital Input Pins)))
(bool) *in*:: State of the hardware input.
(bool) *in-not*:: Inverted state of the input.

[[sub:hal-cdi:di:parameters]]
=== Parameters

(((HAL Digital Input Parameters)))
None

[[sub:hal-cdi:di:functions]]
=== Functions

(((HAL Digital Input Functions)))
...
(((HAL Digital Output Pins)))
(bool) *out*:: Value to be written (possibly inverted) to the hardware output.

[[sub:hal-cdi:do:parameters]]
=== Parameters

(((HAL Digital Output Parameters)))
(bool) *invert*:: If TRUE, *out* is inverted before writing to the hardware.

[[sub:hal-cdi:do:functions]]
=== Functions

(((HAL Digital Output Functions)))
...
(((HAL Analog Input Pins)))
(real) *value*:: The hardware reading, scaled according to the *scale* and *offset* parameters. +
  *value* = ((input reading, in hardware-dependent units) * *scale*) - *offset*

[[sub:hal-cdi:ai:parameters]]
=== Parameters

(((HAL Analog Input Parameters)))
(real) *scale*:: The input voltage (or current) will be multiplied by *scale* before being output to *value*.
(real) *offset*:: This will be subtracted from the hardware input voltage (or current) after the scale multiplier has been applied.
(real) *bit_weight*:: The value of one least significant bit (LSB). This is effectively the granularity of the input reading.
(real) *hw_offset*:: The value present on the input when 0 Volts is applied to the input pin(s).

[[sub:hal-cdi:ai:functions]]
=== Functions

(((HAL Analog Input Functions)))
...
(((HAL Analog Output Pins)))
(real) *value*:: The value to be written. The actual value output to the hardware will depend on the scale and offset parameters.
(bool) *enable*:: If false, then output 0 to the hardware, regardless of the *value* pin.

[[sub:hal-cdi:ao:parameters]]
=== Parameters

(((HAL Analog Output Parameters)))
(real) *offset*:: This will be added to the *value* before the hardware is updated.
(real) *scale*:: This should be set so that an input of 1 on the *value* pin will cause the analog output pin to read 1 volt.
(real) *high_limit* (optional):: When calculating the value to output to the hardware,
  if *value* + *offset* is greater than *high_limit*, then *high_limit* will be used instead.
(real) *low_limit* (optional):: When calculating the value to output to the hardware,
  if *value* + *offset* is less than *low_limit*, then *low_limit* will be used instead.
(real) *bit_weight* (optional):: The value of one least significant bit (LSB), in volts (or mA, for current outputs).
(real) *hw_offset*  (optional):: The actual voltage (or current) that will be output if 0 is written to the hardware.

[[sub:hal-cdi:ao:functions]]
=== Functions

(((HAL Analog Output Functions)))
(funct) *write*::
