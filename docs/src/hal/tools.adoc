...
Note:  LinuxCNC (or a standalone HAL application) must be running
        ...
        HAL item types bool,sint,uint,real are supported.

        When a bool item is specified, a pushbutton is created
        ...
        The bool pushbutton mode can be specified on the command
...
.Notes:
 ...
 4. Pintypes real, sint, uint, bool are supported.
