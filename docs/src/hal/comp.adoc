...
* 'option extra_link_args "..."' - (default: "")
   ...
   --extra_link_args="-L.....". This alternative provides a way to set
   extra flahgs in cases where the input file is a .c rather than a
   .comp.
...
* 'option extra_compile_args "..."' - (default: "")
   ...
   command-line with --extra_compile_args="-I.....". This alternative
...
stateents in the .comp file. For .c files this is not possible so the
--extra_compile_args and --extra_link_args parameters can be used instead.
As an example, this command line can be used to compile the vfdb_vfd.c
component out-of-tree.
----
halcompile --userspace --install --extra_compile_args="-I/usr/include/modbus" --extra_link_args="-lm -lmodbus -llinuxcncini" vfdb_vfd.c
----
