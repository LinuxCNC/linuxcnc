...
`loadrt` with "HAL: ERROR: duplicate pin".
An array claims one identifier per element, so 'x_#' with '[4]' and 'x_0'
collide, and a function claims '<name>.time', '<name>.tmax' and
'<name>.tmax-increased' as well, since `hal_export_funct` creates those.
Declarations carrying DIFFERENT 'if' conditions are not compared, because they
need not ever exist at the same time; the same condition on both sides is still
a collision.
...
* 'option count_function yes' - (default: no) +
  ...
  In userspace components, 'names=' is not supported with 'count_function'.
