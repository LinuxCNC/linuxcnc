...
Next `pin-11` of the parallel port is connected to the `in0` bool of the and gate.
Next `pin-12` is connected to the `in1` bool of the and gate.
Last we connect the `and2` out bool to the parallel port `pin-14`.
...
The weighted sum converts a group of bools into an integer.
The conversion is the sum of the 'weights' of the bools present plus any offset.
It's similar to 'binary coded decimal' but with more options.
The 'hold' bool interrupts the input processing, so that the 'sum' value no longer changes.
...
Creates groups of ``weighted_sum``s, each with the given number of input bools (size).
