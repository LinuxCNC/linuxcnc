...
Owner   CodeAddr  Arg       FP   Users  Name
 00004  f8bc5000  f8f950c8  YES      0   or2.0
----
...
[NOTE]
====
The `FP` column always shows 'YES' and will be removed in the future.
All threads unconditionally save and restore floating point state.
The old `fp`/`nofp` distinction has been removed.
====
...
Type must be 'bool', 'sint', 'uint' or 'real'. Error if _<signame>_ already exists.
...
(((HAL bool,bool)))
A bool value is an on or off.

- bool values = true or 1 and false or 0 (True, TRUE, true are all valid)

[[sub:hal-real]]
=== real

(((HAL real,real)))
A 'real' is a floating point number.
In other words the decimal point can move as needed.

- real values = a double precision floating point value, with approximately 53 bits of resolution and over 2^10^ (~ 1000) bits of dynamic range.
...
(((HAL sint,sint)))
An 'sint' number is a whole number that can have a negative or positive value.

- sint values = integer numbers from -9223372036854775808 to 9223372036854775807

[[sub:hal-uint]]
=== uint

(((HAL uint,uint)))
A 'uint' number is a whole number that is positive only.

- uint values = integer numbers from 0 to 18446744073709551615
...
(((HAL not,not)))
The `not` component is a boolean inverter.
...
Next `pin-11` of the parallel port is connected to the `in0` input of the and gate.
Next `pin-12` is connected to the `in1` output of the and gate.
Last we connect the `and2` output to the parallel port `pin-14`.
...
The weighted sum converts a group of booleans into an integer.
The conversion is the sum of the 'weights' of the booleans present plus any offset.
It's similar to 'binary coded decimal' but with more options.
The 'hold' input interrupts the input processing, so that the 'sum' value no longer changes.
...
Creates groups of ``weighted_sum``s, each with the given number of input booleans (size).
...
In the following example, a copy of the AXIS HAL configuration window, bools '0' and '2' are TRUE, they have no offset.
The weight ('weight') of bool 0 is 1, that of bool 2 is 4, so the sum is 5.

.Component pins of `weighted_sum`
[width="90%",options="header"]
|===
|Owner |Type |Dir  |Value  |Name
|10    |bool |In   |TRUE  m|wsum.0.bit.0.in
|10    |sint |I/O  |1     m|wsum.0.bit.0.weight
|10    |bool |In   |FALSE m|wsum.0.bit.1.in
|10    |sint |I/O  |2     m|wsum.0.bit.1.weight
|10    |bool |In   |TRUE  m|wsum.0.bit.2.in
|10    |sint |I/O  |4     m|wsum.0.bit.2.weight
|10    |bool |In   |FALSE m|wsum.0.bit.3.in
|10    |sint |I/O  |8     m|wsum.0.bit.3.weight
|10    |bool |In   |FALSE m|wsum.0.hold
|10    |sint |I/O  |0     m|wsum.0.offset
|10    |sint |Out  |5     m|wsum.0.sum
|===
