...
Nothing. If a module registers no Jacobian, the shared code computes one by
...
iteration's convergence tolerance divided by the step. Modules built on `switchkins.c` answer this way for every type that
registers nothing; an identity type answers exactly.
...
under two for the geometric Jacobian it registers instead.

So a module whose inverse iterates should register a closed-form Jacobian,
as below. Nothing enforces this. A module that registers nothing still gets
