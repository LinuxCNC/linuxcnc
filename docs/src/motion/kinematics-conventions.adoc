...
Kinematics chapter). A type may also name the two axis letters that orient
the tool, first rotation then second, in `orient`: "AC" for instance. Left
out, they are the letters the joint map gives the joints that turn the frames,
which is right for a module whose forward puts each joint in its own letter;
a module that writes an orienting joint into another letter declares them.
`G43.5` writes its solution into these letters, and the interpreter reads them
back as `#<_kins_orient_1>` and `#<_kins_orient_2>`. A module with
