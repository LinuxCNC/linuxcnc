...
The machine frame is the frame with the orientation left out: XYZ is the
pivot, the point the rotary joints do not move, in machine coordinates, and
the rotary letters are the rotary joints as they are. On a machine whose
slides line up with its frame that is the identity type, and it is what
`G13.1` and `G49` select and `G53.5` moves in. A machine whose carriage does
not line up, a slanted slide, a composite Y made by two slides, an offset
pivot, declares a machine frame type of its own next to its identity type,
and its identity type then keeps the plain meaning, joints are axes, which a
consumer uses to skip the maths. The zero-pose rule holds for the machine
frame type as it does for the identity: the working transform and the machine
frame type agree with the rotaries at zero.
...
identity type is that frame (see the Switchable Kinematics chapter). A module with
