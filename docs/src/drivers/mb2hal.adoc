...
l|INIT_DEBUG | Integer | No | Debug level of init and INI file parsing. +
0 = silent +
1 = error messages (default) +
2 = OK confirmation messages +
3 = debugging messages +
4 = maximum debugging messages (only in transactions) +
l|HAL_MODULE_NAME | String | No | HAL module (component) name. Defaults
to "mb2hal".
l|SLOWDOWN | Float | No | Insert a delay of "FLOAT seconds" between
...
l|TOTAL_TRANSACTIONS | Integer | Yes | The number of total Modbus transactions. There is no maximum.
|===
...
|===
|Value            | Type | Required | Description
l|LINK_TYPE       | String | Yes | You must specify either a "serial" or
"tcp" link for the first transaction.
Later transactions will use the previous transaction link if not
specified.
l|TCP_IP          | IP address | If `LINK_TYPE=tcp` | The Modbus slave
device IP address. Ignored if `LINK_TYPE=serial`.
l|TCP_PORT        | Integer | No | The Modbus slave device TCP port.
Defaults to 502.  Ignored if `LINK_TYPE=serial`.
l|SERIAL_PORT     | String | If `LINK_TYPE=serial` | The serial port.
For example "/dev/ttyS0". Ignored if `LINK_TYPE=tcp`.
l|SERIAL_BAUD     | Integer | If `LINK_TYPE=serial` | The baud rate.
Ignored if `LINK_TYPE=tcp`.
l|SERIAL_BITS     | Integer | If `LINK_TYPE=serial` | Data bits.
One of 5, 6, 7, 8. Ignored if `LINK_TYPE=tcp`.
l|SERIAL_PARITY   | String | If `LINK_TYPE=serial` | Data parity.
One of: even, odd, none. Ignored if `LINK_TYPE=tcp`.
l|SERIAL_STOP     | Integer | If `LINK_TYPE=serial` | Stop bits.
One of 1, 2. Ignored if `LINK_TYPE=tcp`.
l|SERIAL_DELAY_MS | Integer | If `LINK_TYPE=serial` | Serial port delay
between transactions of this section only.
In ms. Defaults to 0. Ignored if `LINK_TYPE=tcp`.
l|MB_SLAVE_ID     | Integer | Yes | Modbus slave number.
l|FIRST_ELEMENT   | Integer | Yes | The first element address.
l|NELEMENTS       | Integer | Unless `PIN_NAMES` is specified | The
number of elements. It is an error to specify both `NELEMENTS` and
`PIN_NAMES`. The pin names will be sequential numbers
e.g. `mb2hal.plcin.01`
l|PIN_NAMES       | List | Unless `NELEMENTS` is specified | A list of
element names.
...
l|MB_RESPONSE_TIMEOUT_MS | Integer | No | 
...
l|MB_BYTE_TIMEOUT_MS     | Integer | No | Byte timeout for this
transaction. In ms. Defaults to 500 ms. This is how much to wait from
byte to byte before raise an error.
l|HAL_TX_NAME            | String | No | Instead of giving the transaction
...
l|MAX_UPDATE_RATE        | Float | No | Maximum update rate in Hz. Defaults to
...
l|DEBUG                  | String | No | Debug level for this transaction only.
