...
* 'pdm_frequency' - (u32, RW) This specifies the PDM frequency, in Hz,
  ...
  For example, setting the pdm_frequency to 2*10^6^&thinsp;Hz (2&thinsp;MHz) and the duty
  cycle to 50% results in a 1&thinsp;MHz square wave, identical to a 1&thinsp;MHz
  PWM signal with 50% duty cycle. The effective range of this parameter
  is from about 1525&thinsp;Hz up to just under 100&thinsp;MHz. Note that the max
  frequency is determined by the ClockHigh frequency of the Anything I/O
  board; the 5I20 and 7I43 both have a 100&thinsp;MHz clock, resulting in a
  100&thinsp;MHz max PDM frequency. Other boards may have different clocks,
...
* 'pwm_frequency' - (u32, RW) This specifies the PWM frequency, in Hz,
  ...
  1&thinsp;Hz up to 193&thinsp;kHz. Note that the max frequency is determined by the
  ClockHigh frequency of the Anything I/O board; the 5i20 and 7i43 both
  have a 100&thinsp;MHz clock, resulting in a 193&thinsp;kHz max PWM frequency. Other
  ...
  Frequencies below about 5&thinsp;Hz are not terribly accurate,
  but above 5&thinsp;Hz they are pretty close.
