...
* '<n>.acceleration-pattern' (bool, in) when true, set acceleration and deceleration times as defined in registers F500 and F501 respectively.
   Used in PID loops to choose shorter ramp times to avoid oscillation.
* '<n>.alarm-code' (sint, out) non-zero if drive is in alarmed state.
...
* '<n>.current-load-percentage' (real, out)
...
* '<n>.status' (sint, out)
  ...
  A bitmap.
* '<n>.trip-code' (sint, out)
  trip code if VF-S11 is in tripped state.
* '<n>.error-count' (sint, out)
