...
This is a non-realtime HAL program to control the S11 series of VFD's from Toshiba.
...
* '<n>.max-rpm' (float, R)
  ...
  nameplate-RPM_ is 1410, but the VFD may generate up to 80 Hz, then max-
...
* '<n>.jog-mode' (bit, in)
  ...
  (preset to 5 Hz). This might be useful for spindle orientation. In
  normal mode, the VFD shuts off if the frequency drops below 12 Hz.
* '<n>.status' (s32, out)
