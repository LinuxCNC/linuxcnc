...
== Create or Edit
# insert attached image here image::images/pncconf-create.png["PnCconf Create or Edit"]
...
over written (such as custom.hal, custom_gvcp.hal, and custom_postgui.hal) and PnCconf places a note in those files. It also allows you to
...
You can also select 'Use INI substitution for Mesa board names.' and 'Show Advanced option Pages', which make additional settings and pages available as you proceed through the configuration process. These include making PID settings available in the motor/encoder section, and aditional configuration pages such as 'External Controls', 'Options', and 'Real Time'.
...
https://wiki.linuxcnc.org/cgi-bin/wiki.pl?Latency-Test
...
See https://wiki.linuxcnc.org/cgi-bin/wiki.pl?Stepper_Drive_Timing for
