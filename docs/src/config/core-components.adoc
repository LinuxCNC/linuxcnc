...
* 'motion.enable' - (bool, in) If this bit is driven FALSE, motion stops, the machine is
...
* 'motion.feed-inhibit' - (bool, in) When this bit is TRUE, the feed rate is set to 0.
...
* 'spindle.__N__.inhibit' - (bool, in) When this bit is TRUE, the spindle speed is set to 0.
