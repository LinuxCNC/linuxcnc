...
* 'iocontrol.0.toolchanger-fault' (bit, in) Should be driven TRUE while the tool changer logic reports a fault.
* 'iocontrol.0.toolchanger-reason' (s32, in) Fault reason code; tracked by the interpreter as #5601.
+
While a fault is active the pending tool change never completes (the
task keeps waiting for tool-prepared/tool-changed), so the program pauses
until the fault is cleared and the change finishes; the operator can
