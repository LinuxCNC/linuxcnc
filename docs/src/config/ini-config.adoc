...
If LinuxCNC is started with the `linuxcnc` script using the "`-H dirname`" option, the specified dirname is prepended to the search described above so that "dirname" is searched first.
The "`-H dirname`" option may be specified more than once, directories are prepended in order.
...
--
* `HALFILE =` _texample.tcl_ [_arg1_ [_arg2_] ...] - Execute the tcl file 'texample.tcl' at start up with arg1, arg2, etc. as argv list.
...
* ``HALFILE = LIB:``_sys_example.hal_ - Execute the system library file _sys_example.hal_ at start up.
  Explicit use of the LIB: prefix causes use of the system library HALFILE without searching the INI file directory.
* ``HALFILE = LIB:``_sys_texample.tcl_ [_arg1_ [_arg2_ ...]] - Execute the system library file _sys_texample.tcl_ at start up.
  Explicit use of the LIB: prefix causes use of the system library HALFILE without searching the INI file directory.
...
* `HALCMD =` _command_ - Execute 'command' as a single HAL command.
...
* `DELAY =` _value_ - seconds to wait beore starting other applications.
...
* `ARC_BLEND_ENABLE = 1` - Turn on new TP.
  If set to 0 TP uses parabolic blending (1 segment look ahead).  Default value 1.
