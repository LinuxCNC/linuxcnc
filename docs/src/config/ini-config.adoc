...
* `TOOL_CHANGE_SECONDS = 20` - (real) How long one M6 takes on this machine, for the program time estimate.
  The estimate cannot know what a tool change costs - on a manual changer it is however long the operator takes - so it counts zero unless this says otherwise.
