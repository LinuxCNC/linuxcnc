...
* `LATHE_TXXXX = 0` (Default: 0) +
  ...
  the tool change (no `M6` needed) and applies geometry plus wear of
  offset 1. `T1` is treated as `T0101`. `T0100` changes to tool 1 and
  cancels the offset (`G49`). `M6` is a no-op. Wear is stored on the
  tool table as `WX`/`WZ`/`WD` and set with `G10 L12`. Off by default so
  mill `T101` remains tool 101.
