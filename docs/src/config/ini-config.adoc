...
* `ENABLE_TWIN = 0` - (bool) Serve the browser digital-twin viewer at `/twin`.
  Off by default.  Its 3D rendering uses the distribution's `libjs-three`
  package, served by the agent from `/three/`.
...
The embedded HTTP agent serves `/probe`, `/current`, `/sample` and `/assets`, an
optional digital-twin viewer at `/twin`, and geometry under `/models/`.  The spindle speed
