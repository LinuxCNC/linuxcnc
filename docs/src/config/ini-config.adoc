...
* `ROTARY_MODULO = 1` - (bool) Continuous-modulo rotary mode.
  ...
  If the axis `MIN_LIMIT`/`MAX_LIMIT` range is not approximately 360 deg a startup warning is printed (typical of limited-range tilt axes where modulo is unusual).
  Mutually exclusive with `WRAPPED_ROTARY` (only one of the two can be set in a configuration).
