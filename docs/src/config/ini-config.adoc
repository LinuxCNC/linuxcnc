...
* `USE_USERSPACE_KINEMATICS = 0` - Set to 1 to enable userspace kinematics computation.
  When enabled, joint-space trajectories are computed in userspace rather than real-time,
  reducing RT CPU load for complex kinematics.
...
# Optional: Enable userspace kinematics for complex machines
# USE_USERSPACE_KINEMATICS = 1
