...
----
+
So, a machine with a maximum axis velocity of 10&thinsp;IPS, a max acceleration
of 100&thinsp;IPS^2^, and a servo period of 0.001&thinsp;s would need:
+
...
----
+
For the aforementioned machine, the ripple for a 20&thinsp;Hz cutoff frequency
is 100 / (4 * 20) = 1.25&thinsp;IPS. This seems high, but keep in mind that it
...
discontinuities. A value in the range 20&thinsp;Hz to 200&thinsp;Hz should be reasonable
to start.
...
* 'ANGULAR_UNITS = <units>' - (((ANGULAR UNITS))) Specifies the 'machine units' for rotational axes.
  ...
  (2*&pi; per circle), 'grad', or 'gon' (400 per circle).
...
1&thinsp;V results in 250&thinsp;mm/s velocity.
