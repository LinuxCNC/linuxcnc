...
. Rotations applied after translations may use the "*!*" provision to to act with respect to the current machine offset.
  Example: GEOMETRY=!CXYZ translates the control point to the X,Y,Z position and then performs
...
* 'MAX_VELOCITY = 1.2' - Maximum velocity for this joint in <<sec:traj-section,machine units>> per second.
...
For more information on custom INI file entries see the <<sub:custom-variables,Custom Sections and Variables>> subsection.
