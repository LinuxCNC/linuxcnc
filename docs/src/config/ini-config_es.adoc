...
[[sec:emc-section]]
=== Sección [EMC](((Archivo INI, Sección EMC)))
...
[[sec:display-section]]
=== Sección [DISPLAY](((Archivo INI, Sección DISPLAY)))
 ...
 * 'DEFAULT_LINEAR_VELOCITY = .25' - La velocidad predeterminada para los movimientos lineales, en  <<sec:traj-section,unidades máquina>> por segundo.
 ...
 * 'PYVCP = /filename.xml' - el archivo de descripción del panel PyVCP.
    Ver el capítulo PyVCP para más información. //<<cha:pyvcp,Capítulo PyVCP>>
 * 'PYVCP_POSITION = BOTTOM' - la ubicación del panel PyVCP en la interfaz de usuario AXIS.
  ...
    es BOTTOM. Vea el capítulo PyVCP para más información. //<<cha:pyvcp,Capítulo PyVCP>>
...
. Las máquinas de corte de espuma (FOAM = 1) deben especificar "XY;UV" o dejar el valor en blanco aunque este valor se ignore actualmente en el modo de cortador de espuma.
  UNA   versión futura puede definir qué significa ";", pero si lo hace "XY;UV" significará lo mismo que el cortador de espuma actual por defecto.
...
[[sec:filter-section]]
=== Sección [FILTER](((Archivo INI, Sección FILTER)))
...
[[sec:emcmot-section]]
=== Sección [EMCMOT](((Archivo INI, Sección EMCMOT)))
...
[[sec:task-section]]
=== Sección [TASK](((Archivo INI, Sección TAREA)))
...
[[sec:hal-section]]
=== Sección [HAL](((Archivo INI, Sección HAL)))
...
[[sec:halui-section]]
=== Sección [HALUI](((Archivo INI, Sección HALUI)))
...
=== Sección [APPLICATIONS](((Archivo INI, Sección de APLICACIONES)))
...
[[sec:traj-section]]
=== [TRAJ] Sección(((Archivo INI,Sección TRAJ)))
...
[[sec:kins-section]]
=== Sección [KINS](((Archivo INI, Sección KINS)))
...
* 'JOINTS = 3' - especifica el número de articulaciones (motores) en el sistema.
  ...
  (Esta variable de configuración puede ser utilizada por una interfaz
  gráfica de usuario para establecer el número de articulaciones
...
[[sec:axis-section]]
=== Sección [AXIS_<letter>](((Archivo INI, Sección AXIS)))
...
Consulte el capítulo: <<cha:external-offsets,'Offsets Externos de Ejes'>> para información de su uso.
...
* 'MAX_VELOCITY = 1.2' - Velocidad máxima para esta articulación en <<sec:traj-section,unidades máquina>> por segundo.
...
Para más información sobre entradas de archivo INI personalizadas ver la subsección <<sub:custom-variables,Secciones y Variables personalizadas>> .
...
* 'DEADBAND = 0.000015' - qué tan cerca es "suficientemente cerca" como para considerar el motor en posición,
  en <<sec:traj-section,unidades máquina>>. Esto a menudo se establece en una distancia equivalente a 1, 1.5, 2, o 3
  recuentos de codificador, pero no hay reglas estrictas.
...
[[sec:spindle-section]]
=== Sección [SPINDLE_<num>] Section(((Archivo INI, Sección SPINDLE_num)))
