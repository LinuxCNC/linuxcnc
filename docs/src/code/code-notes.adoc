...
S-curve planner ([TRAJ]PLANNER_TYPE = 1 and MAX_JERK at 1.0 or above; a
smaller jerk forces the planner back to trapezoidal). Both default to off,
so a stock machine decelerates on a trapezoidal profile.
...
Velocity-synchronized segments (G95 feed per revolution) can be paused
normally.
