...
*-B*
Add 5&thinsp;mm and 10&thinsp;mm to Step feedrate output
...
`xhc-whb04b-6 -HsfB`::
Start in HAL mode + Spindle Override + Feedrate Override + Big step 5/10&thinsp;mm).
...
`whb.axis._<N>_.jog-enable` (bit,out)::
 connect to `axis._<N>_.jog-enable`. If true (and in manual mode),
...
Note:
As a consequence of 3 modes from manufacturer, switching the feed rotary button back from Lead revert to Mpg mode,
...
`whb.button.safe-z` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on Safe-Z button down, false
 otherwise. Requests MDI mode before button pin is set. See also
 `whb.halui.mode.mdi`.
`whb.button.w-home` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on W-Home button down, false
 otherwise. Requests MDI mode before button pin is set. See also
 `whb.halui.mode.mdi`.
...
`whb.button.probe-z` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on Probe-Z button down, false
 otherwise.Requests MDI mode before button pin is set. See also
 `whb.halui.mode.mdi`.
`whb.button.macro-1` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on Macro-1 button (Fn + Feed^+^)
 down, false otherwise.
`whb.button.macro-2` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on Macro-2 button (Fn + Feed^-^)
 down, false otherwise.
...
`whb.button.macro-5` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on Macro-5 button down
 (Fn + M-HOME), false otherwise.
`whb.button.macro-6` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on Macro-6 button down
 (Fn + Safe-Z), false otherwise.
`whb.button.macro-7` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on Macro-7 button down
...
`whb.button.macro-9` (bit,out)::
 connect to ``halui.mdi-command-``_<M>_ True on Macro-9 button down
