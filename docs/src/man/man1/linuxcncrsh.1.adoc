...
and PORT is the port it's listening on (5007 if you did not give
linuxcncrsh the --port option).
____
...
____
The set_wait setting controls the wait after receiving a command. It can
be "received" (after the command was sent and received) or "done" (after
the command was done). With get, any parameter is ignored and the
...
direction. If TELEOP_ENABLE is NO, use joint_number; If TELEOP_ENABLE is
YES, use axis_letter.
____
