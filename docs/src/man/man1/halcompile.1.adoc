...
mangled HAL identifier: underscores become dashes, and a trailing dash or period
is removed. *pin in float my_input* is reached from HAL as
*component.N.my-input*, and *loadrt my_comp* exports its pins under
*my-comp.N.*. Two declarations that claim the same HAL name are rejected,
counting every element an array can create and the *.time*, *.tmax* and
*.tmax-increased* names a function brings with it. Declarations under
different *if* conditions are exempt, since they need not ever coexist.
