...
*iocontrol.0.emc-abort-ack* (Bit, In)::
  Acknowledge line from toolchanger for previous signal,
  or jumpered to abort-tool-change if not used in toolchanger.
...
*iocontrol.0.toolchanger-fault* (Bit, In)::
  Toolchanger signals fault. This line is contionuously monitored.
...
*iocontrol.0.toolchanger-fault-ack* (Bit, Out)::
  ...
  above fault line True is recognized and deasserted when toolchanger-fault drops.
  Toolchanger is free to interpret the ack;
