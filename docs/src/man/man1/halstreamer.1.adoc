...
then each line of input must consist of two reals, a bool, and a signed integer,
in that order and separated by whitespace. Reals must be formatted as required by *strtod*(3),
signed and unsigned integers must be formatted as required by *strtol*(3) and *strtoul*(3),
and bools must be either '0' or '1'.
