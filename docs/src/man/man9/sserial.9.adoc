...
Any other parameters than the ones above are created by the card istelf
...
(float in) angle::
  ...
  poitive voltage applied to the A (or U) phase and both B and C (or V
...
output from the spindle control pin. For example with a 10 V drive
voltage and a 10000 RPM scalemax a value of 10,000 RPM on the spinout
pin would produce 10 V output. However, if spinout-maxlim were set to
...
positive inputs relative to a common field ground. Input impedance is 10
kΩ and input voltage can range from 5 VDC to 32 VDC. All inputs have LED
