...
*loadrt pid [num_chan=*_num_* | names=*_name1_*[,*_name2..._*]]
[debug=*_dbg_]
...
'pid.N.', where 'N' is the channel number. Channel numbers start at
zero.
...
*pid.*_N_*.do-pid-calcs* (uses floating-point) Does the PID calculations
for control loop _N_.
...
*pid.*_N_*.maxerrorD* float in::
  ...
  *Dgain* term will be limited to this value, unless the value is zero.
  ...
  spikes due to steps on the command and/or feedback. Not normally
  needed.
...
*pid.*_N_*.tune-mode* bit in::
  When true, enables auto tune mode. When false, normal PID calculations
  are performed.
*pid.*_N_*.tune-start* bit io::
  When set to true, starts auto tuning. Cleared when the auto tuning
  completes.
*pid.*_N_*.tune-type* u32 rw::
  When set to 0, *Pgain/Igain/Dgain* are calculated. When set to 1,
...
*pid.*_N_*.tune-effort* float rw::
  Determines the effort used in setting up the limit cycle in the
  process. *tune-effort* should be set to a positive value less than
  *maxoutput*. Start with something small and work up to a value that
  results in a good portion of the maximum motor current being used. The
  smaller the value, the smaller the amplitude of the limit cycle.
