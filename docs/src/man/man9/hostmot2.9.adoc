...
hostmot2 - LinuxCNC HAL driver for the Mesa Electronics HostMot2
firmware.
...
*loadrt hostmot2 [debug_idrom=__N__*]*
[debug_module_descriptors=__N__*]* [debug_pin_descriptors=__N__*]*
[debug_modules=__N__*]**
...
to the LinuxCNC HAL. This driver by itself does nothing, the boards that
actually run the firmware require their own drivers before anything can
...
instance (encoder, pwmgen, stepgen etc), Either as a particular I/O pin
of a particular Module instance (encoder, pwmgen, stepgen etc), or as a
...
* [firmware=__F__*]** [num_dplls=__N__*]* [num_encoders=__N__*]*
[ssi_chan__N_*=*_abc%nq_*]* [biss_chan__N_*=*_abc%nq_*]*
[fanuc_chan__N_*=*_abc%nq_*]* [num_inmuxs=__N__*]* [num_inms=__N__*]*
[num_resolvers=__N__*]* [num_pwmgens=__N__*]* [num_3pwmgens=__N__*]*
[num_oneshots=__N__*]* [num_rcpwmgens=__N__*]* [num_stepgens=__N__*]*
[stepgen_width=__N__*]* [sserial_port__0_*=*_00000000_*]*
[num_bspis=__N__*]* [num_leds=__N__*]* [num_ssrs=__N__*]*
[num_outms=__N__*]* [num_xy2mods=__N__*]* [enable_raw]
...
*num_dplls* [optional, default: -1]::
  ...
  parameter can be used to disable the hm2dpll by setting the number to
  0. There is only ever one module of this type, with 4 timer channels,
...
the problem dpend on whether the encoder read is being triggered by the
...
(bit, in) joint-pos-fb::
  ...
  matches the current reolver position. It will then pre-load the driver
  output with this offset. This should only be used on systems where
  axis movement in the unpowered state is unlikely. This feature will
  only work properly if the machine is initially homed to "index" and if
  the axis home positions are exactly zero.
(bit, in/out) index-enable::
...
(bit in) out-NN::
  ...
  invert-nn HAL pin.
(bit in) invert-NN::
  Inverts the state of the this OutM instance's NNth output, defaults to
  0. When invert-NN is set to 1, OutM output NN is high when the out-NN
