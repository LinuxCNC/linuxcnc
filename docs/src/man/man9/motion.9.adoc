...
*motion.switchkins-type* IN float::
  ...
  the pin value.
  The G-code words *G12.1 P-* and *G13.1* write this pin and synchronize
  task and motion themselves, so a program that uses them needs no such
  extra commands.
*motion.kins-type* OUT float::
  The kinematics currently selected, echoing the value that was last
  applied from *motion.switchkins-type*.
