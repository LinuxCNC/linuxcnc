...
*no_iptables* [default: 0]::
  ...
  hm2_eth probes iptables at load time with a read-only listing and
  silently skips rule installation if the probe fails (for example
  when rtapi_app is running unprivileged via file capabilities rather
  than setuid root, since Linux capabilities are not inherited across
  exec() into the iptables binary).  Set *no_iptables=1* when iptables
  is reachable but you prefer to manage the firewall externally
  (nftables, firewalld, systemd units).  In both cases the
  interface-isolation rules must be provided by the administrator; see
