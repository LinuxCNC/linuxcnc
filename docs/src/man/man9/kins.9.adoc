...
*maxkins.tool-length*::
  ...
  only until motion has sent anything. To avoid a joint jump, change the
  tool offset only when B is 0.
...
*5axiskins.tool-length*::
  ...
  offset in effect (G43, G49), so the pin needs no connection; it is read
  only until motion has sent anything. To avoid a joint jump, change the
  tool offset only when B is 0. A tool length in the W column of the tool
