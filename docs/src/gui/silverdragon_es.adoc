...
QtDragon y QtDragon_hd estan construido con el marco QTVCP.
...
[NOTE]
QtDragon y QtDragon_hd y QtVcp son nuevos programas agregados a linuxcnc. +
...
Si su configuración no está configurada actualmente para usar QtDragon,
...
Para realizar un seguimiento de las preferencias, qtdragon busca un archivo
de texto de preferencias. Agregue la siguiente entrada bajo el encabezado
'[DISPLAY]'. (son posibles otras opciones, consulte los documentos del widget
screenoption del qtvcp).
