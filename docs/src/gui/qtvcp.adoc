...
[NOTE]
All `_<options>_` must appear before `_<screen_name>_`.
...
._<screen_name>_
`_<screen_name>_` is the _base name of the .ui and _handler.py files_. +
If `_<screen_name>_` is missing, the default screen will be loaded.
...
using the screen basename and rc, ie: *`CONFIGDIRECTORY/.__<screen_name>__rc`*
...
//FIXME Add cam_align VCP capture
*`sim_panel`*::
...
`_<screen_name>_` is the _base name_ of the `.ui` and `_handler.py` files.
...
*`POSTGUI_HALFILE=_<filename>_`*::
  Typically `_<filename>_` would be `__<screen\_name>___postgui.hal`, but can
...
*`POSTGUI_HALCMD=<command>`*::
  `_<command>_` would be _any valid HAL command_. +
