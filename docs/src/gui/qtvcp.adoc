...
class UserHandlerClass(HandlerClass):
    ...
    def init_pins(self):
        ...
        # add jog pins X axis
        pin = QHAL.newpin("jog.axis.jog-x-plus", QHAL.HAL_BOOL, QHAL.HAL_IN)
        pin.value_changed.connect(lambda s: self.kb_jog(s, 0, 1, fast = False, linear = True))

        pin = QHAL.newpin("jog.axis.jog-x-minus", QHAL.HAL_BOOL, QHAL.HAL_IN)
        pin.value_changed.connect(lambda s: self.kb_jog(s, 0, -1, fast = False, linear = True))
----
...
# Here we used the function to add a bool input pin with a callback.
def after_override__(self):
    try:
        pin = hdlr.QHAL.newpin("new_pin", hdlr.QHAL.HAL_BOOL, hdlr.QHAL.HAL_IN)
...
# connect pins
net bool-input1    test_panel.checkbox_1        classicladder.0.in-00
net bool-hide      test_panel.checkbox_4        classicladder.0.hide_gui

net bool-output1   test_panel.led_1             classicladder.0.out-00

net sint-in1       test_panel.doublescale_1-s   classicladder.0.s32in-00
