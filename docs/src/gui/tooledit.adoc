...
Valid column names are: x y z a b c u v w diam front back orien wx wy wz wa wb wc wu wv ww wd
----

The native lathe wear words (`wx` `wy` `wz` `wa` `wb` `wc` `wu` `wv`
`ww` `wd`) are always preserved in the tool table, even when they are
not selected as columns.  Select them by name to display and edit them.
