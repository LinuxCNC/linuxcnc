...
https://wiki.linuxcnc.org/cgi-bin/wiki.pl?Installing_LinuxCNC[LinuxCNC] open
...
. It reads some of the INI file to initialize the units, and the number/type
  of axes.
. Initializes Python's binding to HAL to build a non-realtime component with the
  Gscreen name.
