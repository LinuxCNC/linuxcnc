...
This widget is a *spin button entry* widget used for _setting a sint and real HAL pin_.

It has an internal _scale factor_, set to a default of 1, that can be set programmatically or using a QtSignal.

The `setInput` slot can be connected to an integer, or a real signal.
...
This widget is used to indicate level or value, usually of a HAL sint/real pin. +
...
** `SINT` A sint integer pin will be added
** `REAL` A real pin will be added
...
Each button has an selectable type (BOOL, SINT or REAL) output HAL pin.

The LED center ring has selectable colors for off and on and is controlled by a bool HAL pin.
...
* _HAL pins_ +
  The input pin can be selected as BOOL, SINT, REAL or no pin selected
* _Programmatically_
* _A `QtSignal`_
...
The `setDisplay` slot can be connected to an integer, a real or a bool signal.
...
This widget _displays HAL real/sint/bool values in a LCD looking way_.
...
*`bit_pin_type`*::
  Selects the input pin as type `BOOL`.
*`s32_pin_type`*::
  Selects the input pin as type `SINT`.
*`float_pin_type`*::
  Select the input pin as type `REAL`.
*`floatTemplate`*::
  A string that will be used as a Python3 format template to tailor the LCD display. +
  Only used when a `REAL` pin is selected, e.g., `{:.2f}` will display a float rounded to 2 numbers after the decimal. +
...
*`halpin_option`*::
  Sets option to make a HAL real pin that reflects current value.
*`rapid_rate`*::
...
from qtvcp.widgets.hal_selectionbox import HALSelectionBox
    ...
    def buildComboBox(self):
        ...
        combobox.setPinTypes([combobox.HAL_BOOL], direction = [combobox.HAL_IN])
        combobox.setSignalTypes([combobox.HAL_BOOL], driven = [False,True])
        combobox.hal_init()
...
# set the pin types to show: HAL_BOOL,HAL_REAL,HAL_SINT,HAL_UINT
# and a list of directions: HAL_IN HAL_OUT
combobox.setPinTypes(types=[combobox.HAL_BOOL], direction = [HAL_IN])

# set the signal types to show: HAL_BOOL,HAL_REAL,HAL_SINT,HAL_UINT
# and a list of driven/undriven (by a connected pin) to show
combobox.setSignalTypes( types=[combobox.HAL_BOOL], driven = [True,True])
----
