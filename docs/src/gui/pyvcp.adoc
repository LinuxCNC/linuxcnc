...
displaying the state of a HAL BOOL signal. It will also create a HAL
...
HAL signals come in two variants, bools and numbers.
Bools are off/on signals. Numbers can be 'real', 'sint' or 'uint'.
...
* Widgets for indicating 'bool' signals: `led`, `rectled`.
* Widgets for controlling 'bool' signals: `button`, `checkbutton`, `radiobutton`.
* Widgets for indicating 'number' signals: `number`, `sint`, `uint`, `bar`, `meter`.
...
Selectable text label, can display up to 6 label legends when associated 'bool' pin is activated.
...
A LED is used to indicate the status of a 'bool' halpin.
...
A button is used to control a BOOL pin.
...
A text button controls a 'bool' halpin.
...
A checkbutton controls a 'bool' halpin.
...
The number widget displays the value of a real signal.
...
.sint Number
The 'sint' number widget displays the value of a `sint` number.
The syntax is the same as 'number' except the name which is `<sint>`.
Make sure the width is wide enough to cover the largest number you expect to use.
This was formerly known as `<s32>`, which is now an alias for `<sint>`.
...
.Simple sint Number Example
image::images/pyvcp_s32.png[]

.uint Number
The 'uint' number widget displays the value of a `uint` number.
The syntax is the same as 'number' except the name which is `<uint>`.
This was formerly known as `<u32>`, which is now an alias for `<uint>`.

.Bar
A bar widget displays the value of a 'real' signal both graphically using a bar display and numerically.
...
.Meter
Meter displays the value of a 'real' signal using a traditional dial indicator.
...
.Scale
A scale controls a `real` or a `sint` pin.
...
The 'halpin' will have both '-f' and '-i' added to it to form the `real` and `sint` pins.
...
.Dial
The Dial outputs a HAL real and reacts to both mouse wheel and dragging.
...
Jogwheel mimics a real jogwheel by outputting a 'real' pin which counts
...
* '<scale_pin>1</scale_pin>' creates scale text and a `REAL.scale` pin to display jog scale
* '<clear_pin>1</clear_pin>' creates `DRO` and a `BOOL.reset` pin to reset DRO.
...
The 'image_bool' toggles between two images by setting the halpin to true or false.
This was formerly known as 'image_bit', which is now an alias for 'image_bool'.
...
<vbox>
    <image_bool halpin='selectimage' images='fwd rev'/>
</vbox>
----
...
The 'image_uint' is the same as 'image_bool',
...
This was formerly known as 'image_u32', which is now an alias for 'image_uint'.
...
<image name='rev' file='rev.gif'/>
<vbox>
    <image_uint halpin='selectimage' images='stb fwd rev'/>
</vbox>
----

The above code produced the following example by adding the stb.gif image.

.Simple image_uint Example with halpin=0
image::images/pyvcp_image_u32_01.png[]

.Simple image_uint Example withhalpin=1
image::images/pyvcp_image01.png[]

.Simple image_uint Example withhalpin=2
image::images/pyvcp_image02.png[]
