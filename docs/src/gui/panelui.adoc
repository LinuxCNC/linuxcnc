...
* OUTPUT= This sets the Button's output type, eg SINT, UINT, REAL, BOOL, NONE, COMMAND, ZMQ.
...
[RADIO_BUTTONS]
  ...
  [[group1_name]]
    ...
    # It has a SINT output of its own, will be 20 on true and 0 on false.
...
[TOGGLE_BUTTONS]
  ...
  # It has a BOOL output, will output 1 on true state and 0 on false state.
  # It also has a status pin which will follow its current state.
