...
----
+
*`_register_messages()`* function connects the audio player to the
`STATUS` library so sounds can be played with the `STATUS` message
system.

=== Example


To play sounds uppon `STATUS` messages, use these general syntaxes:
...
<<cha:qtvcp-vismach,QtVCP Vismach>> chapter.
