...
You can select between the two options in the functions that take these entries:
  ...
  `hal_pin`;; The _name of the BOOL HAL IN pin_ that will change the color. +
...
=== HALColorFlip
Sets the _display color of the part based on a designated HAL BOOL pin state_. +
...
=== HALColorRGB
Sets the _display color of the part based on a designated HAL UINT pin value_. +
The color is decoded from the UINT value. each color is a 0-255 decimal value (shown here in hex) +
red = 0xXXXXXXRR +
green = 0xXXXXGGXX +
blue = 0xXXBBXXXX +
...
*`part = HALColorRGB([_part_], hal_comp, hal_pin, alpha=1.0)`*::
  ...
  `hal_comp`;; The _HAL component Object_ or None. +
    In QtVCP if you are reading _system pins_ directly, then the component argument is set to `None`. +
  `hal_pin`;; The _name of the UINT HAL IN pin_ that will change the color. +
    if hal_comp is 'None' then this must be the full name of a system pin other wise this is the pin name excluding the component name. +
