...
* A status bar which shows the state of the machine. In this screen
  ...
  (showing feedback position). Its last field is the loaded program's
  estimated 'Run time', which while the program runs becomes the time so far
  beside the run it is heading for; it is empty when the program could not be
  timed. See 'G-code properties' below for what the estimate does and does not
  model.
...
* 'G-code properties' - The program's size, its rapid and feed path lengths,
  its bounds, and its run time. The run time is an estimate made while the
  program was parsed: it models the machine's velocity and acceleration
  limits, the corners between moves, the feed modes (G93, G95, G96), dwells
  and tool changes, and it assumes 100% feed, rapid and spindle override.
  It is not a trajectory planner, so expect a few percent either way; and it
  does not model overrides, adaptive feed, M0/M1/M66 waits or spindle
  spin-up, so a run that waits takes longer than it says. It reads
  `[DISPLAY]TOOL_CHANGE_SECONDS` for what one M6 costs, and shows 'not
  available' on a machine whose INI states no velocity limits at all.
