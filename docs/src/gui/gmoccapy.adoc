...
GMOCCAPY 3 is included in the standard distribution of LinuxCNC since release 2.8.
So the easiest way to get GMOCCAPY on your controlling PC, is just to download the
http://www.linuxcnc.org/downloads/[ISO] and install it
...
on http://www.linuxcnc.org/docs/html/gui/gladevcp.html[Glade VCP].
...
.Configuration of five macros in the INI file for the MDI button list
[source,{ini}]
----
[MACROS]
MACRO = i_am_lost
...
In <<cha:halshow,Halshow>> you will see the pins for the right (vertical) buttons are:
...
influences, the hardware buttons will activate the corresponding functions. So you
...
- *gmoccapy.jog.jog-inc-0* _(bit IN)_
- *gmoccapy.jog.jog-inc-1* _(bit IN)_
...
- *gmoccapy.unlock-settings* _(bit IN)_
...
- *gmoccapy.error* _(bit OUT)_
- *gmoccapy.delete-message* _(bit IN)_
...
- *gmoccapy.spindle_feedback_bar* _(float IN)_
- *gmoccapy.spindle_at_speed_led* _(bit IN)_
...
- *gmoccapy.program.length* _(s32 OUT)_ - showing the total number of lines of the program
- *gmoccapy.program.current-line* _(s32 OUT)_ - indicating the current working line of the program
- *gmoccapy.program.progress* _(float OUT)_ - giving the program progress in percentage
