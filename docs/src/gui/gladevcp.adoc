...
HAL signals come in two variants, booleans and numbers.
...
- Widgets for indicating "BOOL" signals: <<gladevcp:hal-led,HAL_LED>>
- Widgets for controlling "BOOL" signals: <<gladevcp:hal-buttons,HAL_Button HAL_RadioButton HAL_CheckButton>>
...
- 'HAL_Spinbutton' and 'HAL_ComboBox', which have two pins: a +<widgetname>-f+ (REAL) and a +<widgetname>-s+ (SINT) pin
...
hal_pin_s, hal_pin_f::
  The SINT and REAL pins of the 'HAL_Spinbutton' and 'HAL_ComboBox' widgets - note these widgets do not have a 'hal_pin' attribute!
hal_pin_scale::
  The REAL input pin of 'HAL_ProgressBar' widget representing the maximum absolute value of input.
...
All of them have a single output BOOL pin named identical to the widget.
...
count_type_shown::
  ...
  default = 1
  * count is based on the CPR selected - it will count positive and negative. It is available as a SINT pin.
  * Scaled-count is CPR count times the scale - it can be positive and negative. +
    If you change the scale the output will immediately reflect the change. It is available as a REAL pin.
...
* `[widget name].get_value()` +
  Will return the counts value as an integer
...
label_pin_type::
  The pin's HAL type  (0:SINT, 1:REAL, 2:UINT), see also the tooltip on 'General→HAL pin type' (note this is different from PyVCP which has three label widgets, one for each type).
text_template::
  ...
  Example: +Distance: %.03f+ will display the text and the pin value with 3 fractional digits padded with zeros for a REAL pin.
...
*HAL_HideTable*:: Has one HAL BOOL input pin which controls if its child widgets are hidden or not.
+
--
*Pin:*::
`<Panel_basename>.<widgetname>`:: in BOOL pin +
If the pin is low then child widgets are visible which is the default state.
--
*HAL_Table* and *HAL_Hbox*::
Have one HAL BOOL input pin which controls if their child widgets are sensitive or not.
+
--
*Pin:*::
`<Panel_basename>.<widgetname>`:: in BOOL pin +
...
It has a single input BOOL pin which controls its state: ON or OFF.
...
The `HAL_ProgressBar` is derived from gtk.ProgressBar and has two REAL
HAL input pins:
...
<widgetname>-f::
  Current value, type REAL
<widgetname>-s::
  Current value, type SINT
...
`HAL_Bar` and `HAL_VBar` widgets for horizontal and vertical bars representing floating point values.

==== Pins

`HAL_Bar` and `HAL_VBar` each have one input REAL HAL pin.
...
`HAL_Meter` is a widget similar to PyVCP meter - it represents a floating point value.

==== Pins

`HAL_Meter` has one input REAL HAL pin.
...
- The HAL SpinBox will have a REAL-type HAL pin named speed-f (see HalWidgets description).
