...
It's possible to load HTML (.html ending) or PDF files with setup notes. +
HTML docs will be displayed in the setup tab and PDF will launch the system
PDF Viewer.+
Some program, such as Fusion and Aspire will create this files for you. +
If you load a G-code program and there is an HTML/PDF file with the same name,
