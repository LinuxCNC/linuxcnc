...
Multimessages use an sint HAL pin to pop multiple defined messages.
...
Owner   Type  Dir                 Value  Name
    29  bool  OUT                 FALSE  bridge.axis-x-is-selected
    29  bool  IN                  FALSE  bridge.axis-x-select
    29  bool  OUT                 FALSE  bridge.axis-y-is-selected
    29  bool  IN                  FALSE  bridge.axis-y-select
    29  bool  OUT                 FALSE  bridge.axis-z-is-selected
    29  bool  IN                  FALSE  bridge.axis-z-select
    29  bool  IN                  FALSE  bridge.cancel-in
    29  bool  IN                  FALSE  bridge.cycle-pause-in
    29  bool  IN                  FALSE  bridge.cycle-start-in
    29  bool  IN                  FALSE  bridge.ini-macro-cmd-MACRO2
    29  bool  IN                  FALSE  bridge.ini-macro-cmd-MACRO3
    29  bool  IN                  FALSE  bridge.ini-mdi-cmd-MACRO0
    29  bool  IN                  FALSE  bridge.ini-mdi-cmd-MACRO1
    29  real  OUT                     0  bridge.jog-increment
    29  real  OUT                     0  bridge.jog-increment-angular
    29  real  OUT                  3000  bridge.jog-rate
    29  real  OUT                   360  bridge.jog-rate-angular
    29  real  OUT                     0  bridge.jog-rate-angular-in
    29  real  IN                      0  bridge.jog-rate-in
    29  sint  OUT                    -1  bridge.joint-selected
    29  bool  IN                  FALSE  bridge.ok-in
    29  bool  IN                  FALSE  bridge.reload-display-in
    29  bool  IN                  FALSE  bridge.shutdown-in
----
...
This bool pin is an output to the spindle control to pause it. +
...
This bool output pin can be connected to turn on a laser:
...
This real output pin indicates the camera rotation in degrees:
...
These bool/sint/real pins are related to external offsets if they are used:
...
These real output pins reflect the current slider jograte (in machine units):
...
These real output pins reflect the current slider override rates:
...
loadrt scaled_sint_sums
addf scaled-sint-sums.0 servo-thread
...
net eoffset-spindle-count   scaled-sint-sums.0.in0
net eoffset-zlevel-count    scaled-sint-sums.0.in1
setp scaled-sint-sums.0.scale0 1000
net eoffset-counts          scaled-sint-sums.0.out-s
