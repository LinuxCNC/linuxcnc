...
`on_abort` receives a single parameter indicating 
the cause for calling the abort procedure,
...
`"a message text"`::
  ...
  abort the program'. Works like `(abort, msg)`.
...
<<sub:argspec-parameter,`argspec=<words>` parameter>>. While this is
