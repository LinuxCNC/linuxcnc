...
``argspec=``_<argspec>_::
  See <<sub:argspec-parameter, description of the argspec
  parameter options>>. Optional.

``ngc=``_<ngc_basename>_::
...
``python=``_<Python function name>_::
...
``prolog=``_<Python function name>_::
...
``epilog=``_<Python function name>_::
...
``argspec=``_<words>_ ``ngc=``_<procname>_ ``modalgroup=``_<group>_::
...
``prolog=``_<pythonprolog>_ ``ngc=``_<procname>_ ``epilog=``_<pythonepilog>_ ``modalgroup=``_<group>_::
...
``python=``_<pythonfunction>_ ``modalgroup=``_<group>_::
...
``argspec=``_<words>_ ``python=``_<pythonfunction>_ ``modalgroup=``_<group>_::
...
Words FST will have the normal functions but will be available as 
...
Now edit ``/home/user/``_xxx_``/``_xxx_``.ini`` and add the following:
...
all in Python, in which case you would just have a `python=_<function>_`
...
---------------------------------------------------------------------
If you for some reason want to remap `T__x__` (prepare), the corresponding
...
- `T` (prepare_prolog): `#__<tool>__` , `#__<pocket>__`
- `M6` (change_prolog): `#__<tool_in_spindle>__`, `#__<selected_tool>__`, `#__<current_pocket>__`, `#__<selected_pocket>__`
- `M61` (settool_prolog): `#__<tool>__` , `#__<pocket>__`
- `S` (setspeed_prolog):  `#__<speed>__`
- `F` (setfeed_prolog):  `#__<feed>__`
...
with a program abort (e.g., by hitting escape in AXIS during a change).
...
good enough, use the ``(abort, ``_<message>_``)`` feature to terminate the handler with an
...
Note that both ``(abort, ``_msg_``)`` and returning INTERP_ERROR from an
...
subroutines. They are expected to either return
...
`words`::
  ...
  If an argspec was present, words are collected from the current block accordingly and
...
`"a message text"`::
  ...
  abort the program'. Works like ``(abort, ``_msg_``)``.
...
arbitrary predicates may be defined this way.
...
working prolog and epilog routines in a single Python module.
These can currently be found in
...
- The NGC procedure is expected to return a positive value, otherwise
  and error message containing the return value is given and the interpreter aborts.
- In case the NGC procedure executed the T command (which then refers
  to the built in T behavior), no further action is taken.
  This can be used for instance to minimally adjust the built in behavior be
  preceding or following it with some other statements.
- Otherwise, the `#<tool>` and `#<pocket>` parameters are extracted
  from the subroutine's parameter space.
...
motion code is retained after finishing execution.
If the next line just contains parameter words (e.g. new X,Y values),
the code is executed again with the new parameter words merged
into the set of the parameters given in the first invocation.

These routines are designed to work in conjunction with an
<<sub:argspec-parameter,`argspec=<words>` parameter>>.
...
Therefore, the calling convention has been extended to use
...
remapped code whose NGC procedure refers to some other remapped code will execute properly.
...
Python scripts, and only a limited supply of debuggers exists.
A working open-source based solution is to use the
...
likely want to see what the maximum depth of the probe is,
...
3. _Modal state and settings_ -
   Refers to state which is 'carried over' between executing
...
4. _Interpreter execution state_ -
   Holds information about the block currently executed, whether we
...
continue. Aagain, this 'wait' is not strictly necessary IMO.
