...
Metric example:  given a Z axis MAX_ACCELERATION of 600&thinsp;mm/s^2^ and MAX_VELOCITY of 60&thinsp;mm/s, the overrun would be 3&thinsp;mm.

Imperial example: given a Z axis MAX_ACCELERATION of 24&thinsp;in/s^2^ and MAX_VELOCITY of 2.4&thinsp;in/s, the overrun would be 0.12&thinsp;in.
...
0&thinsp;V starts the delay when the torch on signal is activated.
...
* This speed defaults to 0.001&thinsp;mm (0.000039") per servo cycle.
...
A user periodic file must be named user_periodic.py and must be loated in the machines config directory. This file is processed every cycle (usually 100mS) and is used for functions that require regular updating.
...
Pressing the button again during the countdown will cause the torch to be turned off, as will pressing Esc if keyboard shortcuts are enabled in the <<qt_settings-tab, SETTINGS Tab>>.
