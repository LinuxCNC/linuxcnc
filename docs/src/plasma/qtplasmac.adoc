...
. Press the *LASER* button and hold for longer than 750 mSec, the laser will turn on and the dry run will start.
. Release the *LASER* button.
...
The angle is measured such that positive increases in value move the leadin counterclockwise: +
...
If a file of a different units sytem has been opened then the copy loaded into the CONVERSATIONAL Tab will be converted to the machines native units when the CONVERSATIONAL Tab becomes active.
