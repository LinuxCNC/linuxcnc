...
QtPlasmaC is a GUI for plasma cutting which utilises the http://linuxcnc.org/docs/devel/html/man/man9/plasmac.9.html[plasmac component] for controlling a plasma table from LinuxCNC v2.9 or later using the Debian Buster or similar distribution.
...
* *[AXIS_Z] MIN_LIMIT* should be just below top of the slats with
  ...
  if the user's float switch takes 4&thinsp;mm (0.157") to activate then set the Z
  minimum to 5&thinsp;mm (0.2") plus an allowance for overrun (either calculated
  using the equation below or allow 5&thinsp;mm (0.2") below the lowest slat).
* *[AXIS_Z] MAX_LIMIT* should be the highest the user wants the Z axis
  to travel (it must not be lower than Z HOME_OFFSET).
* *[AXIS_Z] HOME* should be set to be approximately 5&thinsp;mm to 10&thinsp;mm (0.2"-0.4")
...
Metric example:  given a Z axis MAX_ACCELERATION of 600mm/s^2^ and MAX_VELOCITY of 60mm/s, the overrun would be 3&thinsp;mm.
...
It will read the existing __<machine_name>__\_config.cfg, __<machine_name>___run.cfg, __<machine_name>___wizards.cfg, and plasmac_stats.var files and write them to an existing _<machine_name>_.prefs file. It will also copy the __<machine_name>___material.cfg file to the existing QtPlasmaC configuration.
...
Each increment of delay adds one servo thread cycle to the debounce time. For example: given a servo thread period of 1000000 (measured in nano seconds), a debounce delay of 5 would equate to 5000000&thinsp;ns, or 5&thinsp;ms.

For the Float and Ohmic switches this equates to a 0.001&thinsp;mm (0.00004") increase in the probed height result.
...
. A 0.1&thinsp;μF film capacitor placed across the contacts.
. A 1200&thinsp;Ω 1&thinsp;W resistor across the load (see calculations below).
...
|===
|Filename                    |Function
|__<machine_name>___material.cfg |A file for storing the material settings from the MATERIAL section of the <<qt_parameters-tab,PARAMETERS Tab>>.
...
A shutdown warning can be displayed on every shutdown by checking the *Exit Warning* checkbox on the <<qt_parameters-tab,PARAMETERS Tab>>.
...
|RS485 |0, 1, 2 |This box will enable or disable the communications to a PowerMax. This button is only visible if a PM_PORT is configured in the *[POWERMAX]* section of the _<machine_name>_.prefs file.
...
The **SAVE** button will save the current material set to the _<machine_name>__material.cfg file.

The **RELOAD** button will reload the material set from the _<machine_name>__material.cfg file.
...
It is possible to add a custom message to the warning by adding the message to the *Exit warning text* option in the *[GUI_OPTIONS]* section of the _<machine_name>_.prefs file. +
...
The file name will be __<machine_name>______<version>______<date>_____<time>__.tar.gz where _<machine_name>_ is the machine name entered in the configuration wizard, <version> is the current QtPlasmaC version the user is on, <date> is the current date (YY-MM-DD), and <time> is the current time (HH-MM-SS).
...
. Press the *LASER* button and hold for longer than 750&thinsp;ms.
...
The provided G-code filter program will test for the existence of a `G64 P__n__` command prior to the first motion command. If no G64 command is found it will insert a `G64 P0.1` command which sets the path tolerance to 0.1&thinsp;mm. For a imperial config the command will be `G64 P0.004`.
...
The probe could be a mechanically deployed probe, a permanently mounted proximity sensor or even simply a stiff piece of wire extending about 0.5&thinsp;mm (0.2") below the torch tip.
...
It is also recommended that holes with a diameter of less than 32&thinsp;mm (1.26") are cut at 60% of the feed rate used for profile cuts. This should also lock out THC due to velocity constraints.
...
`M67 E3 Q__nn__` where `__nn__` is the percentage of the velocity desired.
...
After the torch is turned off the hole path will be followed for a default length of 4&thinsp;mm (0.157"). This distance may be specified by adding `#<oclength> = _n_` to the G-code file.
...
* *M62 and M63* (Synchronized with Motion) - The actual change of the specified output (P2 (THC) for example) will happen at the beginning of the next motion command.
  If there is no subsequent motion command, the output changes will not occur. It is best practice to program a motion code (`G0` or `G1` for example) right after an `M62` or `M63`.
...
* `#<holes> = 1` - Causes QtPlasmaC to reduce the speed of holes less than 32&thinsp;mm (1.26") to 60% of *CutFeedRate*.
* `#<holes> = 2` - Causes QtPlasmaC to <<qt_overcut,Over cut>> the hole in addition to the velocity changes in setting 1.
* `#<holes> = 3` - Causes QtPlasmaC to reduce the speed of holes less than 32&thinsp;mm (1.26") and arcs less than 16&thinsp;mm (0.63") to 60% of *CutFeedRate*.
* `#<holes> = 4` - Causes QtPlasmaC to <<qt_overcut,Over cut>> the hole in addition to the velocity change in setting 3.

The default hole size for QtPlasmaC hole sensing is 32&thinsp;mm (1.26"). It is possible to change this value with the following command in a G-code file:
...
The default over cut length for QtPlasmaC hole sensing is 4&thinsp;mm (0.157"). It is possible to change this value with the following command in a G-code file:
...
Once the user has selected the starting place, the *CYCLE START* button will blink *"SELECTED _nn_"* where _nn_ is the corresponding line number selected. Clicking this button will bring up the following Run From Line dialog box:

It is not possible to use Run From Line from within a subroutine. If the user selects a line within a subroutine and clicks *"SELECTED _nn_"* then an error message will be displayed that includes the O-code name of the subroutine.

It is not possible to use Run From Line if previous G-code has set cutter compensation active. If the user selects a line while cutter compensation is active and clicks *"SELECTED _nn_"* then an error message will be displayed.
...
Examples are: Feed Rate, Pierce Height, Pierce Delay, Cut Height, and Kerf Width (for Conversational only).
Cut Amps will be shown if PowerMax communications are enabled.
...
The QtPlasmaC GUI offers user buttons that can be customized by adding commands in the _<machine_name>_.prefs file.
...
_<machine_name>_.ini file variables can be entered by using {} (a space must be placed after the })
...
Run a single unidirectional cut.
This utilises the automatic <<qt_single-cut,Single Cut>> feature.
...
Upon starting a Framing cycle, it is important to note that by default the Z axis will be moved to a height of [AXIS_Z]MAX_LIMIT - 5&thinsp;mm (0.2") before X/Y motion begins.
...
- *OK High Volts* which is the upper value of the voltage range. The default is 250&thinsp;V.

- *OK Low Volts* which is the lower value of the voltage range. The default is 60&thinsp;V.
...
After all contributing factors have been mitigated, if a small fluctuation still exists it is possible to eliminate it by widening the voltage window for which QtPlasmaC will display 0&thinsp;V.
...
The following example would set the voltage window to be displayed as 0&thinsp;V from -5&thinsp;V to +5&thinsp;V:
...
To change this value, add the pin and the required value to the custom.hal file.
It is not recommended to use values less than 5&thinsp;mm as offset overrun may cause unforeseen issues.

The following example would set the distance from Z MAX_LIMIT to 10&thinsp;mm:
...
|Wait for <<qt_material-handling,material>> change confirmation |`M66 P3 L3 Q__n__` +
...
`M65 P3` (immediate)
|Set <<qt_velocity_thc,velocity>> to a percentage of feed rate |`M67 E3 Q__n__` (synchronized with motion) +
`M68 E3 Q__n__` (immediate) +
...
`G0 X0 Y0` +
`M2` (end job) +
|Cut a hole with 6.5&thinsp;mm over cut using the #<holes> command |`G21` (metric) +
`G64 P0.05` +
`M52 P1` (allow paused motion) +
`\#<holes> = 2` (over cut for holes) +
...
_z_ = 0&thinsp;V value from calibration sticker.
...
A common ratio used is 20:1

If connecting to the plasma machines full arc voltage then a common setup for a THCAD-10 is to use a 1&thinsp;M&ohm; resistor from arc negative to THCAD negative and a 1&thinsp;M&ohm; resistor from arc positive to THCAD positive.
