...
It is possible to extend this filter with custom python code ececuted from a file in the configuration directory to aid in converting different flavours of G-code to a format suitable for QtPlasmac.
...
For example to remove any code begining with 'G71' and change 'M2' to 'M5 $0' and 'M2':
...
This reqires defining the same number of arguments as the existing method, noting that 'self' in the original does not constitute an argument.
