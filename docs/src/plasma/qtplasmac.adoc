...
As all the elements combine to drive the shape of the ramping pierce it is key that all these elements are carefully considered when designing the ramp pierce and the parameters that set it. Inevitably there will need to be experimentation to build recipes that work for your plasma source and the materials you cut.
...
Then the the torch will begin descending to 2.5mm for the remaining 0.5 seconds of pierce delay. The creep distance takes roughly 0.7 seconds to complete. This means creep speed continues slightly past the point pierce end height is achieved by about 0.2 seconds.
...
From the above text the user should be able to see that quite a bit of configuration and subtlety can be achieved through careful use of different combination of parameters.
