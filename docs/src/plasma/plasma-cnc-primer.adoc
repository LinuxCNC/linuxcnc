...
This graph was prepared from a sample of about 16,000 readings at varying cut height and the regression analysis shows 7.53&thinsp;V/mm with 99.4% confidence. In this particular instance this sample was taken from an Everlast 50&thinsp;A machine being controlled by LinuxCNC.

Torch voltage then becomes an ideal process control variable to use to adjust the cut height. Let's assume for simplicity that voltage changes by 10&thinsp;V/mm. This can be restated to be 1 Volt per 0.1&thinsp;mm (0.004").
Major plasma machine manufacturers (eg Hypertherm, Thermal Dynamics and ESAB), produce cut charts that specify the recommended cut height and estimated arc voltage at this height as well as some additional data. So if the arc voltage is 1&thinsp;V higher than the manufacturers specification, the controller simply needs to lower the torch by 0.1&thinsp;mm (0.004") to move back to the desired cut height. A torch height control unit (THC) is traditionally used to manage this process.
...
We used a 15&thinsp;W Mean Well HDR-15 Ultra Slim DIN Rail Supply 24&thinsp;V DIN rail based isolated power supply.. This is a double insulated  Isolation Class II device that will withstand any arc voltage that might be applied to the terminals.
...
For further information about external offsets (for V 2.8 and higher) please read the <<sub:ini:sec:axis-letter,[AXIS_<letter>] Section>> of the INI file document and <<cha:external-offsets,External Axis Offsets>> in the LinuxCNC documentation.
...
Stepper motors suffer from resonance and a direct drive pinion is likely to mean that the motor is operating under unfavourable conditions. Ideally, for plasma machines a distance of around 15-25&thinsp;mm per motor revolution is considered ideal but even around 30&thinsp;mm per revolutions is still acceptable. A 5&thinsp;mm pitch ball screw with a 3:1 or 5:1 reduction drive is ideal for the Z axis.
