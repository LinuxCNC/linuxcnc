...
[[sec:config-launcher]]
== Configuration Launcher(((Configuracion Launcher)))
...
** 'apps' - Applications that do not require starting linuxcnc but may
   be useful for testing or trying applications like <<cha:pyvcp,PyVCP>>
   or <<cha:glade-vcp,GladeVCP>>.
