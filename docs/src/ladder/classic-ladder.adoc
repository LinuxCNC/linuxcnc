...
|Number of HAL inputs bool pins   | (numPhysInputs)  | 15
|Number of HAL output bool pins   | (numPhysOutputs) | 15
|Number of arithmetic expressions | (numArithmExpr)  | 50
|Number of Sections               | (numSections)    | 10
|Number of Symbols                | (numSymbols)     | Auto
|Number of Sint inputs            | (numS32in)       | 10
|Number of Sint outputs           | (numS32out)      | 10
|Number of Real inputs            | (numFloatIn)     | 10
|Number of Real outputs           | (numFloatOut)    | 10
|===

Objects of most interest are numPhysInputs, numPhysOutputs, numS32in, and numS32out.

Changing these numbers will change the number of HAL bool pins available.
numPhysInputs and numPhysOutputs control how many HAL bool (on/off) pins are available.
...
Notice all variables start with the % sign. The %I variables represent HAL input bool pins.
The %Q represents the relay coil and HAL output bool pins.
...
If the variable can have a HAL signal connected to it (%I, %Q, and %W-if you have loaded sint pin with the real time module)
...
* `%IW`__xxx__ - Word memory _xxx_ (Sint in pin)
* `%QW`__xxx__ - Word memory _xxx_ (Sint out pin)
* `%IF`__xx__ - Word memory _xx_ (Real in pin) (*converted to Sint in ClassicLadder*)
* `%QF`__xx__ - Word memory _xx_ (Real out pin) (*converted to Sint in ClassicLadder*)
...
* `%I`__xxx__ - Physical input _xxx_ (Boolean) (HAL input bool)
* `%Q`__xxx__ - Physical output _xxx_ (Boolean) (HAL output bool)
