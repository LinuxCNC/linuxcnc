...
baffled many users. They should be seen as
...
offsets that are in effect when the 'G92' command is invoked.

By default, 'G92' offsets values are stored in parameters [5211..5219] 
on 'M2'/'M30' and are saved to the 'PARAMETER_FILE' on shutdowm. These
offset values can be restored with 'G92.3' in another program or after 
a restart.
Programmers that wish for Fanuc behavior, where 'G92' offsets are
cleared at machine start, after 'M2'/'M30' may disable 'G92' persistence 
by setting 'DISABLE_G92_PERSISTENCE = 1' in the '[RS274NGC]' section
of the INI file.

[NOTE]
Beware that a 'G92' offset in effect during a program abort may result 
in unintended offsets when the next program is run.
It is good practice to handle program aborts using the 'ON_ABORT_COMMAND'
entry in the [RS274NGC] section of the INI file. 
...
'G52' offset values will also persist in parameters [5211..5219] after 
'M2'/'M30', machine reset or config restart.

[NOTE]
Beware that a 'G52' offset in effect during a program abort may result 
in unintended offsets when the next program is run.
It is good practice to handle program aborts using the 'ON_ABORT_COMMAND'
entry in the [RS274NGC] section of the INI file. 
