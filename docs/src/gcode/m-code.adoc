...
* 'M26' - on rotary axes flagged with `[AXIS_<L>] ROTARY_MODULO = 1`, absolute
  ...
  180 degrees in the shorter direction. This is the default when `ROTARY_MODULO`
  is enabled.
...
* 'M27' - on the same axes, absolute moves take the modulo-with-sign-direction
  ...
  `A-180`. Use this when sign convention must select rotation direction
  explicitly, similar to a wrapped rotary axis or to Heidenhain M127 / Siemens
  ACP/ACN behavior on a modulo axis.
...
(`#5423`/`#5424`/`#5425` continue to report the wrapped value).
...
G1 A-45     (backward to 45 degrees, taking the long way)
