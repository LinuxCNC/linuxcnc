...
WARNING: G5.2, G5.3 are experimental and not fully tested.
...
used for subsequent G41/G42 cutter radius compensation
...
spindle.
For instance, if starting at 'Z=0', `G33 Z-1 K.0625` produces a 1&thinsp;inch
motion in Z over 16 revolutions of the spindle.
This command might be part of a program to produce a 16&thinsp;TPI thread.
Another example in metric, `G33 Z-15 K1.5` produces a movement of 15&thinsp;mm
while the spindle rotates 10 times for a thread of 1.5&thinsp;mm.
...
synchrony with the `spindle._N_.revs` HAL pin value.
...
.HAL Connections
The pin `spindle._N_.at-speed` must be set or driven true for the motion to
start. Additionally `spindle._N_.revs` must increase by 1 for each revolution
of the spindle and the `spindle._N_.index-enable` pin must be connected to
...
* 'G64 P- <Q- >' blending with tolerance.
  ...
  In addition, when you activate G64 P- Q- it turns on the 'Naive CAM
...
.HAL Connections
The pins `spindle._N_.at-speed` and the `encoder._n_.phase-Z` for the
...
[WARNING]
Only use 'G92' after your machine has been positioned to the desired
point.
...
* 'G92.1' - turn off G92 offsets and reset
  <<sub:numbered-parameters,parameters>> 5211 - 5219 to zero.
* 'G92.2' - turn off G92 offsets but keep
  <<sub:numbered-parameters,parameters>> 5211 - 5219 available.

[NOTE]
G92.1 only clears G92 offsets, to change G53-G59.3 coordinate system
offsets in G-code use either <<gcode:g10-l2,G10 L2>> or
<<gcode:g10-l20,G10 L20>>.

[[gcode:g92.3]]
== G92.3 Restore G92 Offsets(((G92.3 Restore G92 Offsets)))

* 'G92.3' - set the G92 offset to the values saved in parameters 5211 to
  5219
...
* 'G95' - is Units per Revolution Mode
  ...
  G95 requires that `spindle._N_.speed-in` to be connected. The actual
  spindle to which the feed is synchronised is chosen by the $ parameter
