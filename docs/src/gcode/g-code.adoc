...
kinematics number, the same number that the `motion.switchkins-type` pin
takes, so 'G13.1' and `G12.1 P0` do the same thing. A config may select
the kinematics from G-code, from that pin, or from both: each is acted on
when it changes, so the most recent request is the one in force.
