...
|<<gcode:g2-g3,G2 G3>>              |Coordinated Helical Motion at Feed
                                     Rate
|<<gcode:g4,G4>>                    |Dwell
...
|<<gcode:g43,G43>>                  |Use Tool Length Offset from Tool
                                     Table
...
|<<gcode:g64,G64>>                  |Path Control Mode with Optional
                                     Tolerance
...
|<<gcode:g87,G87>>                  |Back-borring Cycle'(not yet
                                     implemented)'
|<<gcode:g88,G88>>                  |Boring Cycle, Stop, Manual Out
                                     '(not yet implemented)'
...
|<<gcode:g96-g97,G96 G97>>          |Spindle Control Mode, Constant
                                     Surface vs Rotation Speed (IPM or
                                     m/min vs RPM)
...
* 'G95' - is Units per Revolution Mode
  ...
  G95 requires that spindle.N.speed-in to be connected. The actual
