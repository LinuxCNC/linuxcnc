...
G-code preview draws it as a wireframe outline around the toolpath, and, where
the GUI's workpiece opacity is set above zero, as a translucent solid with that
outline over it. It has no effect on machine behaviour, on the program extents,
or on zoom-to-fit.
...
(WORKPIECE,CYLINDER,AXIS=Z,X=0,Y=0,ZMIN=-40,ZMAX=0,OD=80)
(WORKPIECE,TUBE,AXIS=Z,X=0,Y=0,ZMIN=-40,ZMAX=0,OD=80,ID=40)
----
...
|'OD' |CYLINDER, TUBE |The outside diameter; required, and always a
 diameter, never a radius, regardless of lathe diameter mode (G7).
|'ID' |TUBE |The bore diameter; required, and must be greater than zero and
 less than 'OD'.
