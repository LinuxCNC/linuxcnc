...
(((Tool IO)))
The non-realtime program specified by `[EMCIO]EMCIO = io` is conventionally used for tool changer management
(and other io functions for enabling LinuxCNC and the control of coolant/lube hardware).
...
Manual toolchanges can be aided by a HAL configuration that employs the *manualtoolchange* component and is typically specified in an INI file with INI statements:
