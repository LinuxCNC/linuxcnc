...
- WX, WY, WZ, WA..WW, WD - lathe wear offsets (optional).  The pose
  words (`WX`/`WY`/`WZ`/`WA`...`WW`) are added to the matching geometry
  offsets when `[RS274NGC]LATHE_TXXXX` applies `T` or by `G43`.
  `WD` (wear diameter) is stored and shown by the tool editors but no
  diameter compensation is applied to motion.  Do not confuse `WX` with
  the W-axis geometry word `W`. With `LATHE_TXXXX`, the tooledit Wear tab
  edits `WX`/`WZ` on the same tool row (not fake tools `T10001`).
