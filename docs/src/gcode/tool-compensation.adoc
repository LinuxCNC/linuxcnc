...
. For `RANDOM_TOOLCHANGER = 1`, idx is the *current* pocket number
  for the tool number specified by the G-code select tool command *T__n__*.
...
.. `iocontrol.0.tool-prep-index`  = tooldata index (_idx_) for tool _n_
.. `iocontrol.0.tool-prep-number` = _n_
...
`[EMCIO]TOOL_TABLE` or programmatically when the `[EMCIO]DB_PROGRAM` is
used.  LinuxCNC pushes updates to the data source (`[EMCIO]TOOL_TABLE`
or `[EMCIO]DB_PROGRAM`) for G-codes G10L1, G10L10, G10L11, M61.  LinuxCNC
