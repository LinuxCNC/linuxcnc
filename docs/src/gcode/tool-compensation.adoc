...
- WX, WZ, WD - lathe wear offsets (optional). `WX`/`WZ` are added to the
  X/Z geometry when `[RS274NGC]LATHE_TXXXX` applies `T0101`. `WD` is
  diameter wear (stored, for `G10 L12 R`). Do not confuse `WX` with the
  W-axis geometry word `W`. With `LATHE_TXXXX`, the tooledit Wear tab
