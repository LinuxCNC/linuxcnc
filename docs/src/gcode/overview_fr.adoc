...
[[sub:block-delete]]
== Caractère d'effacement de bloc(((Block Delete)))
...
<<gcode:binary-operators,les opérateurs booléens>> ('AND', 'OR', 'XOR' et les
...
Utilisation prévue::
 ...
 . <<sub:subroutine-parameters,Paramètres de sous-programme>> - Ils sont
 ...
 . <<gcode:predefined-named-parameters,paramètres nommés prédéfinis>> - utilisés pour
...
visibles dans le tableau <<cap:modal-groups, ci-dessous>>.
...
.Groupes modaux des M-codes(((M-codes modaux)))
...
Ignorer le contenu de la section <<gcode:order-of-execution,ordre d'exécution>> et
