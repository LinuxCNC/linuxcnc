...
# An empty (first, last, offsets) span: no row is in 0..-1, so the first
# tool_offset_at of a program always looks its answer up.
_NO_TOOL_OFFSET_SPAN = (0, -1, (0.0,) * 9)
...
class ProgramGeometry:
    ...
    **Events are vertices.** A coordinate jump, a dwell and a tool change each
    ...
    def clear(self) -> None:
        ...
        #: ``(row it takes effect at, the nine xo..wo)`` per tool offset;
        #: rows increase, each holds until the next. See
        #: :meth:`tool_offset_at`.
        self.tool_offsets: list[tuple[int, tuple[float, ...]]] = []
        #: Those rows alone, to bisect.
        self._tool_offset_rows: list[int] = []
        #: The last answer and the inclusive row range it covers, as
        #: ``(first, last, offsets)``. Empty here, so the first read bisects.
        self._tool_offset_span = _NO_TOOL_OFFSET_SPAN
    ...
    def tool_offset_at(self, index: int) -> tuple[float, ...]:
        ...
        An answer covers every row up to the next offset, so it is kept with
        that range: a row inside it is the same answer again, and only a row
        outside it costs a bisect.
        """
        if index < 0:
            index += self._n
        if not 0 <= index < self._n:
            raise IndexError("no vertex %d in a %d-vertex record"
                             % (index, self._n))
        first, last, offsets = self._tool_offset_span
        if not first <= index <= last:
            first, last, offsets = self._tool_offset_span_at(index)
            self._tool_offset_span = (first, last, offsets)
        return offsets

    def _tool_offset_span_at(
            self, index: int) -> tuple[int, int, tuple[float, ...]]:
        """``(first, last, offsets)`` for the record covering row ``index``.

        ``k < 0`` is the state before the first offset, which is no offset
        and runs from row 0; the last record runs to the end of the program.
        """
        rows = self._tool_offset_rows
        k = bisect.bisect_right(rows, index) - 1
        first = rows[k] if k >= 0 else 0
        last = rows[k + 1] - 1 if k + 1 < len(rows) else self._n - 1
        offsets = self.tool_offsets[k][1] if k >= 0 else (0.0,) * 9
        return first, last, offsets
    ...
    def adopt(self, pg: Any, colors: dict[str, Any]) -> None:
        ...
        self.tool_offsets = [(index, offsets)
                             for index, offsets in pg.tool_offsets()]
        self._tool_offset_rows = [index for index, _ in self.tool_offsets]
        self._tool_offset_span = _NO_TOOL_OFFSET_SPAN
        self._index = None
