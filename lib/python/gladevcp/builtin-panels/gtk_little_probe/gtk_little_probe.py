...
class ProbeScreenClass:
    ...
    def get_preference_file_path(self):
        ...
        if not temp:
            ...
            if not machinename:
                temp = os.path.join(CONFIGPATH1, "gtk_little_probe.pref")
            else:
...
#        print("Key %s (%d) was pressed" % (keyname, data.keyval))
    ...
    def on_down_released(self, gtkbutton, data = None):
        ...
        # Switch to the "result" page in ntb_probe
        self.show_ntb_probe_result_page()
...
#        to_new_xc=self.stat.position[0]-self.stat.g5x_offset[0] - self.stat.g92_offset[0] - self.stat.tool_offset[0] - xcres
    ...
    def show_ntb_probe_result_page(self):
        """
        Switch GtkNotebook 'ntb_probe' to the page whose child widget
        has the ID 'box_result'.
        """
        # Get notebook widget from the builder
        notebook = self.builder.get_object("ntb_probe")
        # Get page container widget from the builder
        page_widget = self.builder.get_object("box_result")
        # Find the page index of this widget inside the notebook
        page_num = notebook.page_num(page_widget)
        # Switch notebook to the desired page
