...
def widgets(P, W):
    ...
    W.liEntry = QLineEdit()
    W.liEntry.textChanged.connect(lambda:P.conv_entry_changed('liEntry'))
    W.entries.addWidget(W.liEntry, 5, 1)
    W.loEntry = QLineEdit()
    W.loEntry.textChanged.connect(lambda:P.conv_entry_changed('loEntry'))
    ...
    W.hdEntry = QLineEdit()
    W.hdEntry.textChanged.connect(lambda:P.conv_entry_changed('hdEntry'))
    W.entries.addWidget(W.hdEntry, 7, 1)
    W.hsEntry = QLineEdit()
    W.hsEntry.textChanged.connect(lambda:P.conv_entry_changed('hsEntry'))
    ...
    W.gsEntry = QLineEdit()
    W.gsEntry.textChanged.connect(lambda:P.conv_entry_changed('gsEntry'))
