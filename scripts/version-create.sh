...
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    echo "Not a Git repository: $VERSION"
...
if [ -n "$GIT_VERSION" ]; then
    ...
    if [ "$GIT_VERSION" = "$SRC_BASE_VERSION" ]; then
        echo "Tagged and version match, using VERSION_BASE as final version: $(cat VERSION_BASE)"
    ...
    if [ -n "$GIT_DEV_VERSION" ]; then
        ...
        echo "No tag matching $GIT_VERSION_TAG, version counter based on last change in VERSION_BASE: $VERSION"
        echo "$VERSION" > VERSION
    fi
fi
