...
functions = {}
class LuiBase:
    def __init__(self, fname, cname, ctype, dotname, doc1, doc2="", deprecated=False):
        functions[fname] = self
        self.fname = fname
...
objects/liblinuxcnc-ui/lui_%(fname)s.cc: $(LUIGEN)
objects/liblinuxcnc-ui/lui_%(fname)s.cc: $(LUIGEN)
\t$(LUIGEN) cfile $@ %(fname)s

LIBLINUXCNC_UI_SRC += objects/liblinuxcnc-ui/lui_%(fname)s.cc""" % self.__dict__

class LuiGetter(LuiBase):
    def __init__(self, cname, *args, **kw):
        LuiBase.__init__(self, 'get_' + cname, cname, *args, **kw)
...
class LuiSetter(LuiBase):
    def __init__(self, cname, struct, varargs, constargs, doc1, doc2='', deprecated=False):
        LuiBase.__init__(self, cname, cname, None, None, doc1, doc2, deprecated=deprecated)
        self.struct = struct
    ...
    def make_signature(self):
        if not self.varargs: return ''
        return ', ' + ", ".join("%s %s" for it in self.varargs)

    def do_hfile(self, fd):
        print >>fd, "int lui_%(fname)s(lui_t *lui%(signature)s);" % self.__dict__

    def do_manfile(self, fd):
        print >>fd, "sorry, not yet"
...
int lui_%(fname)s(lui_t *lui%(signature)s) {
    %(struct)s command;
""" % self.__dict__
        for t, a in self.varargs:
            print >>fd, "    command.%s = %s;" % (a, a)
        for n, v in self.constargs:
            print >>fd, "    command.%s = %s;" % (n, v)
        print >>fd
        print >>fd, """\
    return lui_send_nml_command_and_wait(lui, command);
}"""
