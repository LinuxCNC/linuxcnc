...
pid_t hal_systemv_nowait(char *const argv[], connectionRecType *context) {
    ...
    if (spawn_ret != 0) {
        snprintf(errorStr, sizeof(errorStr), "posix_spawnp(%s) failed: %s", argv[0], strerror(spawn_ret));
        sockWriteError(nakStr, context);
...
static int doLoadUsr(char *args[])
{
   ...
   if ( prog_path[0] == '\0' ) {
    ...
    /* now we need to spawn the program.... */
    /* build argv array first (in parent context) */
    argv[0] = prog_name;
    /* loop thru remaining arguments */
    n = 0;
    m = 1;
...

    {
        ...
        /* use posix_spawn with the resolved path */
        spawn_ret = posix_spawn(&pid, prog_path, &file_actions, &attr,
                                (char *const *)argv, environ);
        ...
        if (spawn_ret != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                "HAL:%d: ERROR: posix_spawn(%s) failed: %s\n", linenumber, prog_name,
                strerror(spawn_ret));
            return -1;
        }
    }
