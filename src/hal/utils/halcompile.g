...
def parse(filename):
    ...
    if '\r' in f:
        if require_unix_line_endings:
            ...
            sys.stderr.write("%s:0: Warning: File contains DOS-style or Mac-style line endings.\n" % filename)
...
def process(filename, mode, outfilename):
    ...
    try:
        ...
        if options.get("userinit") and not options.get("userspace"):
            sys.stderr.write("Warning: comp '%s' sets 'userinit' without 'userspace', ignoring\n" % filename)
    ...
    for f in args:
        try:
            ...
            try:
                ...
                pass
            raise Exception(ex_type, ex_value, exc_tb)
