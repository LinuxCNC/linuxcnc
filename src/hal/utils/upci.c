...
int upci_scan_bus(void)
{
    ...
    while (1) {
	   ...
	   look deeper to find it */
        snprintf(
            dev_fname, 255, DEVICE_FILE, dev->p.bus, dev->p.dev, dev->p.func);
...
int upci_open_region(int devnum, int region_num)
{
    ...
    if (reg->size == 0) {
        errmsg(
            __func__, "region %d not found in device %d", region_num, devnum);
        return -1;
    }
...
__u8 upci_read_u8(int rd, __u32 offset)
{
    ...
    if ((rd < 0) || (rd >= MAX_REGIONS) || ((reg = regions[rd]) == NULL) ||
        (offset > reg->size))
        return 0;
    /* get the data */
    if (reg->type == UPCI_REG_IO) {
