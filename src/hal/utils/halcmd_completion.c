...
static const char *command_table[] = {
    "loadrt", "loadusr", "unload", "lock",    "unlock", "linkps", "linksp",
    "linkpp", "unlinkp", "net",    "newsig",  "delsig", "getp",   "gets",
    "setp",   "sets",    "ptype",  "stype",   "addf",   "delf",   "show",
    "list",   "status",  "save",   "source",  "start",  "stop",   "quit",
    "exit",   "help",    "alias",  "unalias", NULL,
};
...
static const char *list_table[] = {
    "comp", "alias", "pin", "sig", "param", "funct", "thread", NULL};
...
static char **completion_matches_table(const char *text,
                                       const char **table,
...
char **halcmd_completer(
    const char *text, int start, int end, hal_completer_func func, char *buffer)
{
    (void)end;
    int i;
    char **result = NULL, *n;
...
{
    return halcmd_completer(
        text, start, end, rl_completion_matches, rl_line_buffer);
}
