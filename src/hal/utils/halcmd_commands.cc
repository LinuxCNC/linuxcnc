...
int do_newinst_cmd(char *comp_name, char *inst_name) {
    ...
    // FIXME:
    // If this code is ever going to be used, the it should be moved into
    // hal_lib. It is also wrong to use a flag in the comp_id field. It should
    // capture the new instance in a separate field pointing to the component
    // ID of the next component instance of this module.
...
int do_unloadrt_cmd(const char *mod_name)
{
    if(strcmp(mod_name, "all")) {
        ...
        if(HAL_COMP_TYPE_REALTIME != q.comp.type) {
            halcmd_error("component '%s' is not a realtime module\n", mod_name);
            return -EEXIST;
        }
...
static void print_comp_info_print(const hal_query_t *q, const char **patterns)
{
    ...
    case HAL_COMP_TYPE_OTHER: {
        ...
        // FIXME: This should acquire the /next/ component in the list of
        // components. However, this is not possible. The construct is only
        // used if the 'newinst' method is made available and that should not
        // fiddle with the comp_id field, but add a field to point to the next
        // component ID. See also comment there.
...
static std::string querydata_refstr_20(hal_type_t type, hal_refs_u u)
{
    ...
    case HAL_PORT:
        return fmt::format("{:20d}", 0 != hal_get_port(u.p) ? hal_port_buffer_size(u.p) : 0);
    case HAL_UINT:
...
static std::string querydata_refstr(hal_type_t type, hal_refs_u u)
{
    ...
    case HAL_PORT:
        return fmt::format("{}", 0 != hal_get_port(u.p) ? hal_port_buffer_size(u.p) : 0);
    case HAL_UINT:
