...
static int preflight_net_cmd(char *signal, hal_sig_t *sig, char *pins[])
{
    ...
    for (i = 0; pins[i] && *pins[i]; i++) {
        ...
        if (pin->dir == HAL_OUT) {
            if (writers || bidirs) {
            dir_error:
                halcmd_error("Signal '%s' can not add %s pin '%s', "
                             "it already has %s pin '%s'\n",
                             signal,
                             pin_data_dir(pin->dir),
                             pin->name,
                             bidir_name ? pin_data_dir(HAL_IO)
                                        : pin_data_dir(HAL_OUT),
    ...
    case HAL_PORT:
        ...
        if ((*cp != '\0') && (!isspace(*cp))) {
            ...
            if ((*((hal_port_t *)d_ptr) != 0) &&
                (hal_port_buffer_size(((hal_port_t *)d_ptr)) > 0)) {
...
static int get_type(char ***patterns)
{
    ...
    if (strcmp(typestr, "u32") == 0)
        return HAL_U32;
    if (strcmp(typestr, "s64") == 0)
        return HAL_S64;
    if (strcmp(typestr, "u64") == 0)
        return HAL_U64;
    ...
    /* make full module name '<path>/<name>.o' */
    {
        ...
        if (r < 0) {
            ...
            halcmd_error("module path too long (max %lu) for %s/%s%s\n",
                         (unsigned long)sizeof(mod_path) - 1,
                         rtmod_dir,
                         mod_name,
                         MODULE_EXT);
            return -1;
...
int do_unload_cmd(char *mod_name)
{
    if (strcmp(mod_name, "all") == 0) {
        ...
        if (type == COMPONENT_TYPE_REALTIME)
            return do_unloadrt_cmd(mod_name);
        else
...
static void warn_newly_loaded_comps(std::set<std::string> &names,
                                    const char *newname)
{
    ...
    for (const auto &name : new_names) {
        ...
        if (names.find(name) == names.end()) {
            fprintf(stderr,
                    ...
                    "specify the correct name via 'loadusr -Wn'?",
                    newname,
                    name.c_str());
        }
    }
    ...
    while (next != 0) {
        ...
        if (match(patterns, comp->name)) {
            if (comp->type == COMPONENT_TYPE_OTHER) {
                hal_comp_t *comp1 =
                    halpr_find_comp_by_id(comp->comp_id & 0xffff);
                ...
                halcmd_output(" %5d  %-4s  %-*s",
                              comp->comp_id,
                              (comp->type == COMPONENT_TYPE_REALTIME) ? "RT"
                                                                      : "User",
                              HAL_NAME_LEN,
                              comp->name);
                if (comp->type == COMPONENT_TYPE_USER) {
                    halcmd_output(" %5d %s",
                                  comp->pid,
                                  comp->ready > 0 ? "ready" : "initializing");
                } else {
                    halcmd_output(" %5s %s",
                                  "",
                                  comp->ready > 0 ? "ready" : "initializing");
        ...
        if (match(patterns, tptr->name)) {
            ...
            if (ret >= sizeof(name)) {
                rtapi_print_msg(RTAPI_MSG_ERR,
                                "unexpected: pin name too long for buffer %s",
                                tptr->name);
            } else {
...
template <class T> int count_list(SHMFIELD(T) list_root)
{
    int n;
    SHMFIELD(T) next;
...
static void print_mem_status()
{
    ...
    halcmd_output("  used/total shared memory:   %ld/%d\n",
                  (long)(HAL_SIZE - hal_data->shmem_avail),
                  HAL_SIZE);
    // count components
...
static const char *data_type(int type)
{
    ...
    case HAL_S64: type_str = "s64  "; break;
    case HAL_U64: type_str = "u64  "; break;
    case HAL_PORT: type_str = "port "; break;
    default:
        /* Shouldn't get here, but just in case... */
        type_str = "undef";
    }
...
int do_save_cmd(const char *type, char *filename)
{
    ...
    if ((strcmp(type, "all") == 0) || (strcmp(type, "allu") == 0)) {
        /* save everything */
        save_comps(dst);
        save_aliases(dst);
        save_signals(dst, 1);
...
int do_setexact_cmd()
{
    ...
    if (hal_data->base_period) {
        halcmd_error("HAL_LIB: Cannot run 'setexact'"
...
int do_help_cmd(char *command)
{
	...
	printf("  'modname', naming it 'instname'.\n");
        ...
        printf("  Unloads HAL module 'compname', whether user space or "
               "realtime.\n");
        ...
        printf(
            "Creates 'signame' with the type of 'pinname' if it does not yet "
            "exist\n");
        ...
        printf(
            "  Sets a non-port signal 'signame' to 'value' (if signal has no "
            "writers).\n");
        printf("  If 'signame' is a port signal (that has not previously been "
               "allocated),\n");
        printf(
            "  then 'value' is the new maximum size in bytes of that port.\n");
        ...
        printf(
            "  If signal 'signame' is a port, returns the buffer size of that "
            "port.\n");
        ...
        printf("debug [level]\n");
        printf("   set the messaging level for the realtime API (calls "
               "rtapi_set_msg_level)\n");
        printf("   levels are \n");
        printf("   0 = None\n");
        ...
        printf(
            "  Type can be 'comp', 'alias', 'sig[u]', 'signal', 'link[a]'\n");
        printf("  'net[a]', 'netl', 'netla', netal', 'param', 'parameter,\n");
        printf("  'unconnectedinpins', 'all, 'allu', or 'thread'.\n");
        printf(
            "  (A final 'a' character (like 'neta' means show arrows for pin "
            "direction.)\n");
        printf("  If 'type' is 'allu'), does the equivalent of:\n");
        printf(
            "  'comp', 'alias', 'sigu', 'netla', 'param', 'unconnectedinpins' "
            "and 'thread'.\n\n");
        printf("  If 'type' is omitted (or type is 'all'), does the equivalent "
               "of:\n");
        printf(
            "  'comp', 'alias', 'sigu', 'netla', 'param', and 'thread'.\n\n");
        ...
        printf("  name that can be used to refer to a pin or parameter, both "
               "the\n");
        printf("  original name and the alias will work.\n");
        printf("  \"type\" must be pin or param\n");
        printf("  \"name\" must be an existing name or alias of the specified "
               "type.\n");
        ...
        printf(
            "Useful for debugging scripted commands from a running program\n");
        ...
        printf(
            "This will print to the terminal the filename, line number and\n");
        printf(
            "an (optional) message. If the message has spaces, put it inside "
            "quotes.\n");
        printf(
            "Useful for debugging ie. confirming a HAL file is being run.\n");
    } else {
...
static void print_help_commands(void)
{
    ...
    printf(
        "  unload              Unload realtime module or terminate userspace "
        "component\n");
    ...
    printf("  print               print filename, line number and optional "
           "message to terminal\n");
