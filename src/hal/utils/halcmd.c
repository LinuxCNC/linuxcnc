...
static int parse_cmd1(char **argv)
{
    ...
    if (!command) {
        ...
        int is_optional = command->type & A_OPTIONAL,
            is_plus = command->type & A_PLUS, nargs = command->type & 0xff,
            posargs;

        if (command->type & A_REMOVE_ARROWS) {
            int s, d;
            for (s = d = 0; argv[s] && argv[s][0]; s++) {
                if (!strcmp(argv[s], "<=") || !strcmp(argv[s], "=>") ||
                    !strcmp(argv[s], "<=>")) {
                    continue;
                } else {
                    argv[d++] = argv[s];
        ...
        case A_ZERO: result = command->func.v(); break;

        case A_PLUS: result = command->func.cpp(REST(1)); break;

        case A_ONE: result = command->func.cp(ARG(1)); break;

        case A_ONE | A_PLUS:
            result = command->func.cp_cpp(ARG(1), REST(2));
            break;

        case A_TWO: result = command->func.cp_cp(ARG(1), ARG(2)); break;
        ...
        case A_THREE | A_PLUS:
            result =
                command->func.cp_cp_cp_cpp(ARG(1), ARG(2), ARG(3), REST(4));
            break;
...
static int tokenize(char *cmd_buf, char **tokens)
{
    ...
    while (m < MAX_TOK) {
        ...
        case DOUBLE_QUOTE:
            ...
            break;
        case END_OF_LINE: tokens[m++] = cp1; break;
        default:
            /* should never get here */
            state = BETWEEN_TOKENS;
        }
...
*/
static int
replace_vars(char *source_str, char *dest_str, int max_chars, char **detail)
{
    int retval = 0;
    ...
    *dest_str = '\0';          /* return null string if input is null string */
    ...
    while ((remaining = strlen(sp)) > 0) {
        ...
        if (next_delim < remaining) { /* a new delimiter was found */
            switch (
                *sp++) { /* this is the found delimiter, since sp was incremented */
            case '$':
                varP = sp;
                if (*sp == '(') { /* look for a parenthesized var */
                    ...
                    if (next_delim >=
                        (int)strlen(varP)) /* error - no matching parens */
                        return -1;
                    sp++;
                } else
                ...
                if (strlimcpy(
                        &dp, replacement, strlen(replacement), &buf_space) < 0)
                    return -6;
                sp += next_delim;
                break;
            case '[':
                ...
                if (next_delim >=
                    (int)strlen(secP)) /* error - no matching square bracket */
                    return -3;
...
int halcmd_preprocess_line(char *line, char **tokens)
{
    ...
    if (retval != 0) {
        if ((retval < 0) &&
            (retval >= -7)) { /* print better replacement errors */
            if (detail) {
