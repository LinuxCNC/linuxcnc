...
static int
handle_click(GtkWidget *widget, GdkEventButton *event, gpointer data);
static int
handle_release(GtkWidget *widget, GdkEventButton *event, gpointer data);
static int
handle_motion(GtkWidget *widget, GdkEventButton *event, gpointer data);
static int
handle_scroll(GtkWidget *widget, GdkEventScroll *event, gpointer data);
...
static void
update_drawing_size(GtkWidget *widget, GdkEventConfigure *event, gpointer data);
...
static void calculate_offset(int chan_num)
{
    ...
    for (n = 0; n < ctrl_usr->samples; n++) {
        ...
        case HAL_BIT:
            ...
            break;
        case HAL_FLOAT: value = dptr->d_real; break;
        case HAL_S32: value = dptr->d_s32; break;
        case HAL_U32: value = dptr->d_u32; break;
...
static void alloc_color(GdkRGBA *color, int red, int green, int blue)
{
    ...
    if (!gdk_rgba_parse(color, buf)) {
        fprintf(stderr,
                "%s: Failed to parse color, RGB(%d, %d, %d)\n",
                __func__,
                red,
                green,
                blue);
...
static void init_display_window(void)
{
    ...
    gtk_box_pack_start(
        GTK_BOX(ctrl_usr->waveform_win), disp->drawing, TRUE, TRUE, 0);
    /* hook up a function to handle expose events */
    g_signal_connect(
        disp->drawing, "size_allocate", G_CALLBACK(update_drawing_size), NULL);
    g_signal_connect(
        disp->drawing, "realize", G_CALLBACK(create_context), NULL);
    g_signal_connect(disp->drawing, "draw", G_CALLBACK(refresh_display), NULL);
    g_signal_connect(disp->drawing,
                     "button_release_event",
                     G_CALLBACK(handle_release),
                     NULL);
    g_signal_connect(
        disp->drawing, "button_press_event", G_CALLBACK(handle_click), NULL);
    g_signal_connect(
        disp->drawing, "motion_notify_event", G_CALLBACK(handle_motion), NULL);
    g_signal_connect(
        disp->drawing, "scroll_event", G_CALLBACK(handle_scroll), NULL);
    ...
    for (i = 0; i < 16; i++) {
        alloc_color(&(disp->color_normal[i]),
                    normal_colors[i][0],
                    normal_colors[i][1],
                    normal_colors[i][2]);
        alloc_color(&(disp->color_selected[i]),
                    selected_colors[i][0],
                    selected_colors[i][1],
                    selected_colors[i][2]);
    }
...
static void change_zoom(int dir, int x)
{
    ...
    double old_pixels_per_sample, pixels_per_div, pixels_per_sec,
        new_pixels_per_sample, old_fraction, new_fraction;
    ...
    // pixel?
    old_fraction =
        (x - disp->width / 2) / old_pixels_per_sample / ctrl_shm->rec_len;
    // and new?
    new_fraction =
        (x - disp->width / 2) / new_pixels_per_sample / ctrl_shm->rec_len;
...
handle_motion(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
    ...
    gdk_window_get_device_position(
        gtk_widget_get_window(disp->drawing), event->device, &x, &y, &mod);
...
#define COORDINATE_CLIP(coord)                                                 \
    ((coord < -32768) ? -32768 : (coord > 32767) ? 32767 : coord)
...
static void draw_triggerline(int chan_num, int highlight)
{
    ...
    if (highlight) {
        gdk_cairo_set_source_rgba(disp->context,
                                  &disp->color_selected[chan_num - 1]);
    } else {
        gdk_cairo_set_source_rgba(disp->context,
                                  &disp->color_normal[chan_num - 1]);
    }
...
void draw_waveform(int chan_num, int highlight)
{
    ...
    while (n <= end) {
        ...
        case HAL_BIT:
            ...
            break;
        case HAL_FLOAT: fy = dptr->d_real; break;
        case HAL_S32: fy = dptr->d_s32; break;
        case HAL_U32: fy = dptr->d_u32; break;
        default: fy = 0.0; break;
        }
...
static void create_context(void)
{
    ...
    disp->surface =
        gdk_window_create_similar_surface(gtk_widget_get_window(disp->drawing),
                                          CAIRO_CONTENT_COLOR,
                                          disp->width,
                                          disp->height);
