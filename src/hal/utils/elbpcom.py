...
parser.add_option(
    ...
    "--port",
    dest="sport",
    help="UDP port of board (default: 27181)",
    type="int",
    default=27181,
)
parser.add_option(
    ...
    "--timeout",
    dest="timeout",
    help="Response timeout in seconds (default: 0.2)",
    type="float",
    default=0.2,
)
parser.add_option(
...
if options.space:
    options.space = int(options.space)
...
def transact(sdata: bytes, quiet=False, response=True):
    ...
    try:
        ...
        if not quiet:
            print(
                "     ", re.sub("[^ -~]", ".", data.decode("utf-7", errors="replace"))
            )
        return data
...
def make_read_request(
    space: int, info: bool, size: int, increment: bool, address: int, nbytes: int
) -> bytes:
    return struct.pack(
        "<HH",
        (1 << 14)
...
def make_write_request(
    space: int, info: bool, size: int, increment: bool, address: int, data: bytes
) -> bytes:
    return (
        struct.pack(
            "<HH",
...
def optimal_size(space: int, info: bool, address: int, nbytes: int):
    ...
    for i in (3, 2, 1, 0):
        b = 1 << i
        if address % b or nbytes % b or not memsizes & b:
            continue
        return b
    raise ValueError(
        "Access size incompatible with address or length (address=%d nbytes=%d memsizes=%d)"
        % (address, nbytes, memsizes)
    )
...
if options.read:
    if options.address is None:
        raise SystemExit("--read must specify --address")
    if options.size == 0:
        options.size = optimal_size(
            options.space,
            options.info,
            options.address,
            options.read if options.increment else 0,
        )
    command = make_read_request(
        options.space,
        options.info,
        options.size,
        options.increment,
    ...
    if options.address is None:
        raise SystemExit("--write must specify --address")
    ...
    if options.size == 0:
        options.size = optimal_size(
            ...
            options.info,
            options.address,
            len(write) if options.increment else 0,
        )
    command = make_write_request(
        options.space,
        options.info,
        options.size,
        options.increment,
    ...
    command = make_read_request(
        options.space, True, options.size, options.increment, 0x00, 0x10
    )
    print(">", command.hex())
