...
int main(int argc, gchar *argv[])
{
    ...
    if (!halpr_find_funct_by_name("scope.sample")) {
        ...
        snprintf(buf,
                 sizeof(buf),
                 EMC2_BIN_DIR "/halcmd loadrt scope_rt num_samples=%d",
                 num_samples);
        if (system(buf) != 0) {
    ...
    shm_id =
        rtapi_shmem_new(SCOPE_SHM_KEY, comp_id, sizeof(scope_shm_control_t));
    if (shm_id < 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
    ...
    g_signal_connect(
        ctrl_usr->main_win, "destroy", G_CALLBACK(main_window_closed), NULL);
    g_signal_connect(
        ctrl_usr->main_win, "focus-in-event", G_CALLBACK(set_focus), NULL);
    /* define menu windows */
...
void capture_complete(void)
{
    ...
    case STOP: break;
    case NORMAL:
    case ROLL: start_capture(); break;
    case SINGLE:
        /* 'push' the stop button */
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(ctrl_usr->rm_stop_button), TRUE);
        break;
    default: break;
    }
...
static void init_usr_control_struct(void *shmem)
{
    ...
    if (ctrl_shm->shm_size == 0) {
        ...
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "Realtime component not loaded? ctrl_shm->size == 0");
        hal_exit(comp_id);
        exit(1);
    }
...
static void define_menubar(GtkWidget *vboxtop)
{
    ...
    GtkWidget *menubar, *filemenu, *fileopenconfiguration,
        *filesaveconfiguration, *fileopendatafile, *filesavedatafile, *filequit,
        *sep1, *sep2;
    ...
    fileopenconfiguration =
        gtk_menu_item_new_with_mnemonic(_("_Open Configuration..."));
    gtk_menu_shell_append(GTK_MENU_SHELL(filemenu), fileopenconfiguration);
    g_signal_connect_swapped(
        fileopenconfiguration, "activate", G_CALLBACK(open_configuration), 0);
    gtk_widget_show(fileopenconfiguration);

    filesaveconfiguration =
        gtk_menu_item_new_with_mnemonic(_("_Save Configuration..."));
    gtk_menu_shell_append(GTK_MENU_SHELL(filemenu), filesaveconfiguration);
    g_signal_connect_swapped(
        filesaveconfiguration, "activate", G_CALLBACK(save_configuration), 0);
    ...
    g_signal_connect_swapped(fileopendatafile,
                             "activate",
    ...
    g_signal_connect_swapped(
        filesavedatafile, "activate", G_CALLBACK(save_log_cb), 0);
...
static void define_scope_windows(void)
{
    GtkWidget *vbox, *hbox, *vboxtop, *vboxbottom, *vboxleft, *vboxright,
        *hboxright;
    ...
    /* left side */
    ctrl_usr->horiz_info_win = gtk_vbox_framed_new_in_box(
        _("Horizontal"), FALSE, 0, 0, vboxleft, FALSE, FALSE, 1);
    ...
    ctrl_usr->chan_info_win = gtk_hbox_framed_new_in_box(
        _("Selected Channel"), FALSE, 0, 0, vboxleft, FALSE, FALSE, 0);
    /* right side */
    ...
    ctrl_usr->run_mode_win = gtk_vbox_framed_new_in_box(
        _("Run Mode"), TRUE, 0, 0, vboxleft, FALSE, FALSE, 0);
    ctrl_usr->trig_info_win = gtk_vbox_framed_new_in_box(
        _("Trigger"), FALSE, 0, 0, vboxright, TRUE, TRUE, 0);
    ctrl_usr->trig_mode_win = gtk_vbox_new_in_box(
        TRUE, 0, 0, ctrl_usr->trig_info_win, FALSE, FALSE, 0);
    ctrl_usr->vert_info_win = gtk_vbox_framed_new_in_box(
        _("Vertical"), FALSE, 0, 0, vboxleft, TRUE, TRUE, 0);
...
static void init_run_mode_window(void)
{
    ...
    gtk_box_pack_start(GTK_BOX(ctrl_usr->run_mode_win),
                       ...
                       0);
    gtk_box_pack_start(GTK_BOX(ctrl_usr->run_mode_win),
                       ctrl_usr->rm_stop_button,
                       FALSE,
                       FALSE,
                       0);
    /* hook callbacks to buttons */
    g_signal_connect(ctrl_usr->rm_normal_button,
                     "clicked",
                     G_CALLBACK(rm_normal_button_clicked),
                     NULL);
    g_signal_connect(ctrl_usr->rm_single_button,
                     "clicked",
                     G_CALLBACK(rm_single_button_clicked),
                     NULL);
    g_signal_connect(ctrl_usr->rm_roll_button,
                     "clicked",
                     G_CALLBACK(rm_roll_button_clicked),
                     NULL);
    g_signal_connect(ctrl_usr->rm_stop_button,
                     "clicked",
                     G_CALLBACK(rm_stop_button_clicked),
                     NULL);
