...
int hal_pin_bit_new(const char *name,
                    hal_pin_dir_t dir,
...
int hal_pin_float_new(const char *name,
                      hal_pin_dir_t dir,
...
int hal_pin_u32_new(const char *name,
                    hal_pin_dir_t dir,
...
int hal_pin_s32_new(const char *name,
                    hal_pin_dir_t dir,
...
int hal_pin_u64_new(const char *name,
                    hal_pin_dir_t dir,
...
int hal_pin_s64_new(const char *name,
                    hal_pin_dir_t dir,
...
int hal_pin_port_new(const char *name,
                     hal_pin_dir_t dir,
...
static int hal_pin_newfv(hal_type_t type,
                         hal_pin_dir_t dir,
                         void **data_ptr_addr,
                         int comp_id,
                         const char *fmt,
                       ...
                       ...)
{
    ...
    va_start(ap, fmt);
    ret =
        hal_pin_newfv(HAL_FLOAT, dir, (void **)data_ptr_addr, comp_id, fmt, ap);
    va_end(ap);
    return ret;
}
                      ...
                      ...)
{
    ...
    va_start(ap, fmt);
    ret =
        hal_pin_newfv(HAL_PORT, dir, (void **)data_ptr_addr, comp_id, fmt, ap);
    va_end(ap);
    return ret;
}
                ...
                int comp_id)
{
    ...
    if (dir != HAL_IN && dir != HAL_OUT && dir != HAL_IO) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "HAL: ERROR: pin direction not one of HAL_IN, HAL_OUT, "
                        "or HAL_IO\n");
        return -EINVAL;
    }
    if (type == HAL_PORT && dir == HAL_IO) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "HAL: ERROR: pin direction must be one of HAL_IN or HAL_OUT for "
            "hal port\n");
        return -EINVAL;
    }
    if (strlen(name) > HAL_NAME_LEN) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "HAL: ERROR: pin name '%s' is too long\n", name);
        return -EINVAL;
    }
...
int hal_pin_alias(const char *pin_name, const char *alias)
{
    ...
    while (1) {
        if (next == 0) {
            ...
            rtapi_mutex_give(&(hal_data->mutex));
            rtapi_print_msg(
                RTAPI_MSG_ERR, "HAL: ERROR: pin '%s' not found\n", pin_name);
            return -EINVAL;
        }
        pin = SHMPTR(next);
    ...
    if (alias != NULL) {
        ...
        if (pin->oldname == 0) {
            ...
            rtapi_snprintf(
                oldname->name, sizeof(oldname->name), "%s", pin->name);
        }
...
int hal_signal_new(const char *name, hal_type_t type)
{
    ...
    if (strlen(name) > HAL_NAME_LEN) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "HAL: ERROR: signal name '%s' is too long\n", name);
        return -EINVAL;
    }
    if (hal_data->lock & HAL_LOCK_CONFIG) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    if (halpr_find_sig_by_name(name) != 0) {
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(
            RTAPI_MSG_ERR, "HAL: ERROR: duplicate signal '%s'\n", name);
        return -EINVAL;
    }
...
*/
    ...
    case HAL_U64:
    case HAL_FLOAT:
    case HAL_PORT: data_addr = shmalloc_up(sizeof(hal_data_u)); break;
    default:
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(
    ...
    switch (type) {
    case HAL_BIT: *((hal_bit_t *)data_addr) = 0; break;
    case HAL_S32: *((hal_s32_t *)data_addr) = 0; break;
    case HAL_U32: *((hal_u32_t *)data_addr) = 0; break;
    case HAL_S64: *((hal_s64_t *)data_addr) = 0; break;
    case HAL_U64: *((hal_u64_t *)data_addr) = 0; break;
    case HAL_FLOAT: *((hal_float_t *)data_addr) = 0.0; break;
    case HAL_PORT: *((int *)data_addr) = 0; break;
    default: break;
    }
    /* initialize the structure */
...
int hal_link(const char *pin_name, const char *sig_name)
{
    ...
    rtapi_print_msg(
        RTAPI_MSG_DBG, "HAL: linking pin '%s' to '%s'\n", pin_name, sig_name);
    ...
    *data_ptr_addr = data_addr;
    ...
    if (drive_pin_default_value_onto_signal) {
        ...
        switch (pin->type) {
        case HAL_BIT: *((hal_bit_t *)data_addr) = pin->dummysig.b; break;
        case HAL_S32: *((hal_s32_t *)data_addr) = pin->dummysig.s; break;
        case HAL_U32: *((hal_u32_t *)data_addr) = pin->dummysig.u; break;
        case HAL_S64: *((hal_s64_t *)data_addr) = pin->dummysig.s; break;
        case HAL_U64: *((hal_u64_t *)data_addr) = pin->dummysig.u; break;
        case HAL_FLOAT: *((hal_float_t *)data_addr) = pin->dummysig.f; break;
        default:
            rtapi_print_msg(RTAPI_MSG_ERR,
...
int hal_param_bit_new(const char *name,
                      hal_param_dir_t dir,
                      hal_bit_t *data_addr,
                      int comp_id)
{
...
int hal_param_float_new(const char *name,
                        hal_param_dir_t dir,
...
int hal_param_u32_new(const char *name,
                      hal_param_dir_t dir,
                      hal_u32_t *data_addr,
                      int comp_id)
{
...
int hal_param_s32_new(const char *name,
                      hal_param_dir_t dir,
                      hal_s32_t *data_addr,
                      int comp_id)
{
...
int hal_param_u64_new(const char *name,
                      hal_param_dir_t dir,
                      hal_u64_t *data_addr,
                      int comp_id)
{
...
int hal_param_s64_new(const char *name,
                      hal_param_dir_t dir,
                      hal_s64_t *data_addr,
                      int comp_id)
{
...
int hal_param_bit_newf(hal_param_dir_t dir,
                       hal_bit_t *data_addr,
                       int comp_id,
                       const char *fmt,
                       ...)
...
int hal_param_float_newf(hal_param_dir_t dir,
                         hal_float_t *data_addr,
                         int comp_id,
                         const char *fmt,
                         ...)
...
int hal_param_u32_newf(hal_param_dir_t dir,
                       hal_u32_t *data_addr,
                       int comp_id,
                       const char *fmt,
                       ...)
...
int hal_param_s32_newf(hal_param_dir_t dir,
                       hal_s32_t *data_addr,
                       int comp_id,
                       const char *fmt,
                       ...)
...
int hal_param_u64_newf(hal_param_dir_t dir,
                       hal_u64_t *data_addr,
                       int comp_id,
                       const char *fmt,
                       ...)
...
int hal_param_s64_newf(hal_param_dir_t dir,
                       hal_s64_t *data_addr,
                       int comp_id,
                       const char *fmt,
                       ...)
...
int hal_param_set(const char *name, hal_type_t type, void *value_addr)
{
    ...
    if (param->dir == HAL_RO) {
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(
            RTAPI_MSG_ERR, "HAL: ERROR: param '%s' is not writable\n", name);
        return -EINVAL;
    }
...
int hal_param_alias(const char *param_name, const char *alias)
{
    ...
    if (alias != NULL) {
        ...
        if (param->oldname == 0) {
            ...
            rtapi_snprintf(
                oldname->name, sizeof(oldname->name), "%s", param->name);
        }
        ...
        if (param->oldname != 0) {
            ...
            oldname = SHMPTR(param->oldname);
            rtapi_snprintf(
                param->name, sizeof(param->name), "%s", oldname->name);
            param->oldname = 0;
            free_oldname_struct(oldname);
        }
    }
...
int hal_get_pin_value_by_name(const char *hal_name,
                              hal_type_t *type,
                              hal_data_u **data,
                              bool *connected)
{
...
int hal_get_signal_value_by_name(const char *hal_name,
                                 hal_type_t *type,
                                 hal_data_u **data,
                                 bool *has_writers)
{
...
int hal_get_param_value_by_name(const char *hal_name,
                                hal_type_t *type,
                                hal_data_u **data)
{
    hal_param_t *param;
...
int hal_export_funct(const char *name,
                     ...
                     int comp_id)
{
    ...
    while (1) {
        ...
        if (cmp == 0) {
            ...
            rtapi_mutex_give(&(hal_data->mutex));
            rtapi_print_msg(
                RTAPI_MSG_ERR, "HAL: ERROR: duplicate function '%s'\n", name);
            return -EINVAL;
        }
        /* didn't find it yet, look at next one */
        prev = &(fptr->next_ptr);
    ...
    if (hal_pin_s32_newf(HAL_OUT, &(new->runtime), comp_id, "%s.time", name)) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "HAL: ERROR: fail to create pin '%s.time'\n", name);
        return -EINVAL;
    }
    *(new->runtime) = 0;
...
int hal_create_thread(const char *name, unsigned long period_nsec, int uses_fp)
{
    ...
    if (strlen(name) > HAL_NAME_LEN) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "HAL: ERROR: thread name '%s' is too long\n", name);
        return -EINVAL;
    }
    if (hal_data->lock & HAL_LOCK_CONFIG) {
        rtapi_print_msg(
    ...
    if ((long)period_nsec < hal_data->base_period) {
        ...
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "HAL_LIB: ERROR: new thread period %ld is less than clock period "
            "%ld\n",
            period_nsec,
            hal_data->base_period);
        return -EINVAL;
    }
    ...
    if (new->period < prev_period) {
        ...
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "HAL_LIB: ERROR: new thread period %ld is less than existing "
            "thread period %ld\n",
            period_nsec,
            prev_period);
        return -EINVAL;
    ...
    retval = rtapi_task_new(
        thread_task, new, new->priority, lib_module_id, HAL_STACKSIZE, uses_fp);
    if (retval < 0) {
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    rtapi_snprintf(buf,
                   sizeof(buf),
                   HAL_PSEUDO_COMP_PREFIX "%s",
                   new->name); // pseudo prefix
    ...
    if (hal_pin_s32_newf(
            HAL_OUT, &(new->runtime), new->comp_id, "%s.time", new->name)) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                            ...
                            int position)
{
    ...
    if (funct == 0) {
        ...
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(
            RTAPI_MSG_ERR, "HAL: ERROR: function '%s' not found\n", funct_name);
        return -EINVAL;
    }
    ...
    if ((funct->uses_fp) && (!thread->uses_fp)) {
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(
            RTAPI_MSG_ERR, "HAL: ERROR: function '%s' needs FP\n", funct_name);
        return -EINVAL;
    }
...
static void unlink_pin(hal_pin_t *pin)
{
    ...
    if (pin->signal != 0) {
        ...
        comp = SHMPTR(pin->owner_ptr);
        dummy_addr =
            (void *)((char *)comp->shmem_base + SHMOFF(&(pin->dummysig)));
        *data_ptr_addr = dummy_addr;
        ...
        switch (pin->type) {
        case HAL_BIT: dummy_addr->b = sig_data_addr->b; break;
        case HAL_S32: dummy_addr->s = sig_data_addr->s; break;
        case HAL_U32: dummy_addr->u = sig_data_addr->u; break;
        case HAL_S64: dummy_addr->s = sig_data_addr->s; break;
        case HAL_U64: dummy_addr->u = sig_data_addr->u; break;
        case HAL_FLOAT: dummy_addr->f = sig_data_addr->f; break;
        case HAL_PORT:
...
static char *halpr_type_string(int type, char *buf, size_t nbuf)
{
    ...
    default: rtapi_snprintf(buf, nbuf, "UNK#%d", type); return buf;
    }
}
...
static void
hal_port_atomic_load(hal_port_shm_t *port_shm, unsigned *read, unsigned *write)
{
...
static void hal_port_atomic_store_write(hal_port_shm_t *port_shm,
                                        unsigned write)
{
...
static unsigned
hal_port_bytes_readable(unsigned read, unsigned write, unsigned size)
{
    if (size == 0) {
        return 0;
    } else if (read <= write) {
...
static unsigned
hal_port_bytes_writable(unsigned read, unsigned write, unsigned size)
{
    if (size == 0) {
        return 0;
    } else if (write < read) {
...
{
    unsigned read, write, bytes_avail, end_bytes_avail, end_bytes_to_write,
        beg_bytes_to_write, final_pos;
    ...
    if (!port || !*port || !count) {
        ...
        if (count > bytes_avail) {
            ...
            memcpy(
                port_shm->buff, src + end_bytes_to_write, beg_bytes_to_write);
...
unsigned hal_port_readable(const hal_port_t *port)
{
    if (!port || !*port) {
        ...
        hal_port_shm_t *port_shm = SHMPTR(*port);
        return hal_port_bytes_readable(
            port_shm->read, port_shm->write, port_shm->size);
    }
}
...
unsigned hal_port_writable(const hal_port_t *port)
{
    if (!port || !*port) {
        ...
        hal_port_shm_t *port_shm = SHMPTR(*port);
        return hal_port_bytes_writable(
            port_shm->read, port_shm->write, port_shm->size);
    }
}
...
#ifdef ULAPI
void hal_port_wait_readable(hal_port_t **port,
                            unsigned count,
                            sig_atomic_t *stop)
{
...
void hal_port_wait_writable(hal_port_t **port,
                            unsigned count,
                            sig_atomic_t *stop)
{
...
int hal_stream_create(hal_stream_t *stream,
                      int comp,
                      int key,
                      unsigned depth,
                      const char *typestring)
{
    ...
    size_t size = sizeof(struct hal_stream_shm) +
                  sizeof(union hal_stream_data) * depth * (1 + pin_count);
...
int hal_stream_read(hal_stream_t *stream,
...
int hal_stream_attach(hal_stream_t *stream,
                      int comp_id,
                      int key,
                      const char *typestring)
{
        ...
        for (i = 0; i < result; i++) {
            if (type[i] != fifo->type[i]) {
                ...
                rtapi_print_msg(
                    ...
                    i,
                    halpr_type_string(
                        fifo->type[i], typename0, sizeof(typename0)),
