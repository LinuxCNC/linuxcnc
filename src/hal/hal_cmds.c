...
/** This file, 'hal_cmds.c', is a HAL component that provides processing
...
* tokenize_string
*
* This function takes a command contained in cmd_buf and creates
* a set of command tokens for use in run_command.
*
* cmd_buf is modified by the process, and is assumed to be a null
* terminated string containing the hal command
*
* tokens is an array of maximum_tokens+1 pointers to strings,
* which will be filled with the tokenized commands from cmd_buf
*
* 0 is returned on success, and -1 is returned on rare catastrophic error.
*
************************************************************************/
int tokenize_string(char *cmd_buf, char *tokens[], int maximum_tokens)
{
    int m;
    char *cp1= NULL;
    enum { BETWEEN_TOKENS,
...
* run_command
*
* This function takes a hal command contained in command and executes it.
*
* command is assumed to be a null terminated string containing the hal command
*
...
int run_command(const char *command)
{
	...
	/* The tokenizer is going to modify the command buffer that we
	   give to is, so we make a copy of the command here to preserve
	   the caller's command buffer. */

	cmd_buf=strdup(command);

	/* Tokenize command */

	tokenize_string(cmd_buf, tokens, MAX_TOK);
...
* run_script_file
*
* This function reads each line in the open file handle executes
* it using the run_command function above.
*
* The number of errors encountered is returned (0 for success)
*
************************************************************************/
...
* run_script
*
* This function takes a hal command script name and executes every
* line in the file using the run_script_file function above.
*
* -1 is returned on file access error, 0 is returned on success,
* and a number >0 is representing the number of lines which failed
