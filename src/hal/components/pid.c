...
static int export_pid(hal_pid_t * addr, char * prefix)
{
	...
	// Ensure we have target memory allocated. It doesn't matter that these
	// are fake parameters when they otherwise are pins. This is just
	// setting the correct reference and allocate the appropriate amount of
	// memory so we don't crash.
        hal_param_new_fake(comp_id, (hal_refs_u *)&addr->error_i);
        hal_param_new_fake(comp_id, (hal_refs_u *)&addr->error_d);
        hal_param_new_fake(comp_id, (hal_refs_u *)&addr->cmd_d);
        hal_param_new_fake(comp_id, (hal_refs_u *)&addr->cmd_dd);
        hal_param_new_fake(comp_id, (hal_refs_u *)&addr->cmd_ddd);
#ifdef AUTO_TUNER
        hal_param_new_fake(comp_id, (hal_refs_u *)&addr->ultimateGain);
        hal_param_new_fake(comp_id, (hal_refs_u *)&addr->ultimatePeriod);
#endif /* AUTO_TUNER */
    }
