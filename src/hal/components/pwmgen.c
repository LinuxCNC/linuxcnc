...
static void update(void *arg, long period)
{
	...
	rtapi_real max_dc = hal_get_real(pwmgen->max_dc);
	rtapi_real min_dc = hal_get_real(pwmgen->min_dc);
	if ( max_dc > 1.0 ) {
	    max_dc = hal_set_real(pwmgen->max_dc, 1.0);
	}
	if ( min_dc > max_dc ) {
	    min_dc = hal_set_real(pwmgen->min_dc, max_dc);
	}
	if ( min_dc < 0.0 ) {
	    min_dc = hal_set_real(pwmgen->min_dc, 0.0);
	}
	if ( max_dc < min_dc ) {
	    max_dc = hal_set_real(pwmgen->max_dc, min_dc);
	}
