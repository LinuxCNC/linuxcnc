...
*/
static const unsigned char lut[16] = {0x00,
                                      0x44,
                                      0x88,
                                      0x0C,
                                      0x80,
...
int rtapi_app_main(void)
{
    ...
    if (comp_id < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "ENCODER_RATIO: ERROR: hal_init() failed\n");
        return -1;
    }
    ...
    for (n = 0; n < howmany; n++) {
        ...
        if (num_chan) {
            ...
            retval =
                export_encoder_pair(n, &(encoder_pair_array[n]), names[i++]);
        }
    ...
    retval = hal_export_funct(
        "encoder-ratio.sample", sample, encoder_pair_array, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_export_funct(
        "encoder-ratio.update", update, encoder_pair_array, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
...
static int export_encoder_pair(int num, encoder_pair_t *addr, char *prefix)
{
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->master_A), comp_id, "%s.master-A", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->master_B), comp_id, "%s.master-B", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->slave_A), comp_id, "%s.slave-A", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->slave_B), comp_id, "%s.slave-B", prefix);
    if (retval != 0) {
        return retval;
    }
    ...
    retval = hal_pin_u32_newf(
        HAL_IO, &(addr->master_ppr), comp_id, "%s.master-ppr", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_u32_newf(
        HAL_IO, &(addr->slave_ppr), comp_id, "%s.slave-ppr", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_u32_newf(
        HAL_IO, &(addr->master_teeth), comp_id, "%s.master-teeth", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_u32_newf(
        HAL_IO, &(addr->slave_teeth), comp_id, "%s.slave-teeth", prefix);
    if (retval != 0) {
        return retval;
    }
