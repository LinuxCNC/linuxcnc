...
int rtapi_app_main(void)
{
    ...
    for (n = 0; n < MAX_STREAMERS; n++) {
        ...
        retval = hal_stream_create(&streams[n].fifo,
                                   comp_id,
                                   STREAMER_SHMEM_KEY + n,
                                   depth[n],
                                   cfg[n]);
        if (retval < 0) {
            goto fail;
...
static void update(void *arg, long period)
{
    ...
    int myclockedge = str->myclockedge =
    ...
    *(str->empty) = depth == 0;
    ...
    for (n = 0; n < num_pins; n++) {
        ...
        case HAL_FLOAT: *(pptr->hfloat) = dptr->f; break;
        case HAL_BIT:
            if (dptr->b) {
                *(pptr->hbit) = 1;
            } else {
            ...
            break;
        case HAL_U32: *(pptr->hu32) = dptr->u; break;
        case HAL_S32: *(pptr->hs32) = dptr->s; break;
        default: break;
        }
        dptr++;
        pptr++;
...
static int init_streamer(int num, streamer_t *str)
{
    ...
    retval = hal_pin_bit_newf(
        HAL_OUT, &(str->empty), comp_id, "streamer.%d.empty", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, &(str->enable), comp_id, "streamer.%d.enable", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_s32_newf(
        HAL_OUT, &(str->curr_depth), comp_id, "streamer.%d.curr-depth", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_s32_newf(
        HAL_IO, &(str->underruns), comp_id, "streamer.%d.underruns", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, &(str->clock), comp_id, "streamer.%d.clock", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_s32_newf(
        HAL_IN, &(str->clock_mode), comp_id, "streamer.%d.clock-mode", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    *(str->clock_mode) = 0;
    ...
    for (n = 0; n < hal_stream_element_count(&str->fifo); n++) {
        ...
        case HAL_FLOAT:
            *(pptr->hfloat) = 0.0;
            usefp = 1;
            break;
        case HAL_BIT: *(pptr->hbit) = 0; break;
        case HAL_U32: *(pptr->hu32) = 0; break;
        case HAL_S32: *(pptr->hs32) = 0; break;
        default: break;
        }
        pptr++;
    }
    ...
    retval =
        hal_export_functf(update, str, usefp, 0, comp_id, "streamer.%d", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
