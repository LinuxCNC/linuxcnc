...
# Conversion from real to integer has the problem that the real has too few
# bits to represent the integer value correctly. That means we need to test for
# >= and <= or the bounds are not met properly.
# EU - Test equality in upper band
# EL - Test equality in lower band
EU=""
EL=""
test "$1" = 'real' -a "$2" = 'sint' && EL="="
test "$1" = 'real' -a \( "$2" = 'sint' -o "$2" = 'uint' \) && EU="="
...
exec sed -e "$IN; $OUT; $CC; $MIN; $MAX; $TYPI; $TYPO; $MINEN; $MAXEN; $XIN; $XOUT; $FEU; $FEL;"
