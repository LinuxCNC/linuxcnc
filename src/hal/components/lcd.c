...
int rtapi_app_main(void)
{
    ...
    if (!fmt_strings[0]) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "The LCD component requires at least one valid format string");
        return -EINVAL;
    }
    ...
    for (i = 0; i < lcd->num_insts; i++) {
        ...
        for (f = 0; fmt_strings[i][f]; f++) {
            ...
            if (fmt_strings[i][f + 1] == '|' || fmt_strings[i][f + 1] == 0) {
                ...
                retval = snprintf(
                    inst->pages[p].fmt, f - f1 + 2, "%s", fmt_strings[i] + f1);
                inst->pages[p].length = f - f1 + 2;
                inst->pages[p].args =
                    hal_malloc(inst->pages[p].num_args * sizeof(hal_float_t));
                f1 = f + 2;
                {
                    int a = -1, s = -1;
                        ...
                        if (page.fmt[s] == '%') {
                            ...
                            case 'f':
                                retval = hal_pin_float_newf(
                                    HAL_IN,
                                    (hal_float_t **)&(inst->pages[p].args[a]),
                                    comp_id,
    ...
    for (i = 0; i < lcd->num_insts; i++) {
        retval = hal_pin_u32_newf(
            HAL_IN, &(lcd->insts[i].page_num), comp_id, "lcd.%02i.page_num", i);
        if (retval != 0) {
            return retval;
        }
        lcd->insts[i].last_page = -1; // force screen refresh
        retval = hal_pin_u32_newf(
            HAL_OUT, &(lcd->insts[i].out), comp_id, "lcd.%02i.out", i);
        if (retval != 0) {
            return retval;
        }
        retval = hal_pin_float_newf(
            HAL_IN, &(lcd->insts[i].contrast), comp_id, "lcd.%02i.contrast", i);
        if (retval != 0) {
            return retval;
        }
...
static void write_one(lcd_inst_t *inst)
{
    ...
    case '%':
        ...
        /* Fallthrough */
    default: *inst->out = inst->pages[*inst->page_num].fmt[inst->f_ptr++];
    }
}
...
static int parse_fmt(char *in, int *ptr, char *out, void *val, char dp)
{
    ...
    const double pow10[] = {1,    10,   1e2,  1e3,  1e4,  1e5,  1e6,
