...
/** This file, 'stepgen.c', is a HAL component that provides software
    ...
    NOTE: The following comments apply to the new behavior as implemented
    in CVS trunk.  In the 2.1 branch, the timing parameters are faked to
    remain consistent with existing behavior.  The parameters themselves
    work as described, but they can only be set by the user for step
    type zero.  For all other step types, they are set to their minimum
    legal values (either 0 or 1, see the notes under each step type) 
    ...
    exported to HAL.  The values of these parameters are in periods of
    the "make-pulses" thread.  In the timing diagrams that follow, they
    are identfied by the following numbers:
...
typedef struct {
    ...
    hal_u32_t step_len_ns;	/* step pulse length */
    hal_u32_t dir_hold_dly_ns;	/* direction hold time or delay */
    hal_u32_t dir_setup_ns;	/* direction setup time */
    ...
    hal_u32_t step_space_ns;	/* parameter: min step pulse spacing */
...
static void make_pulses(void *arg, long period)
{
	...
	if ( step_now ) {
	    ...
	    stepgen->timer1 = stepgen->step_len_ns;
	    /* timer 2 = time till allowed to change dir pin */
	    stepgen->timer2 = stepgen->timer1 + stepgen->dir_hold_dly_ns;
	    /* timer 3 = time till allowed to step the other way */
	    stepgen->timer3 = stepgen->timer2 + stepgen->dir_setup_ns;
...
static void update_freq(void *arg, long period)
{
	...
	/* process timing parameters if they changed */
	if ( stepgen->step_len != stepgen->old_step_len ) {
	    /* must be non-zero */
	    if ( stepgen->step_len == 0 ) {
		stepgen->step_len = 1;
	    }
	    /* get ready to detect future changes */
	    stepgen->old_step_len = stepgen->step_len;
	    /* convert to nanoseconds */
	    stepgen->step_len_ns = stepgen->step_len * periodns;
	}
	if ( stepgen->step_space != stepgen->old_step_space ) {
	    ...
	    /* convert to nanoseconds */
	    stepgen->step_space_ns = stepgen->step_space * periodns;
	}
	if ( stepgen->dir_setup != stepgen->old_dir_setup ) {
	    /* get ready to detect future changes */
	    stepgen->old_dir_setup = stepgen->dir_setup;
	    /* convert to nanoseconds */
	    stepgen->dir_setup_ns = stepgen->dir_setup * periodns;
	}
	if ( stepgen->dir_hold_dly != stepgen->old_dir_hold_dly ) {
	    ...
	    /* get ready to detect future changes */
	    stepgen->old_dir_hold_dly = stepgen->dir_hold_dly;
	    /* convert to nanoseconds */
	    stepgen->dir_hold_dly_ns = stepgen->dir_hold_dly * periodns;
	}
	...
	min_step_period = stepgen->step_len_ns + stepgen->step_space_ns;
...
static int export_stepgen(int num, stepgen_t * addr, int step_type)
{
	...
	if (retval != 0) { return retval; }
	retval = hal_param_u32_newf(HAL_RW, &(addr->dir_setup), comp_id,
	    "stepgen.%d.dirsetup", num);
	if (retval != 0) { return retval; }
	retval = hal_param_u32_newf(HAL_RW, &(addr->dir_hold_dly), comp_id,
	    "stepgen.%d.dirhold", num);
	if (retval != 0) { return retval; }
    }
    /* export output pins */
    if ( step_type == 0 ) {
	...
	*(addr->phase[DOWN_PIN]) = 0;
    ...
    /* these will get recalculated, but set some initial value */
    addr->step_len_ns = 0;
    addr->step_space_ns = 0;
    addr->dir_setup_ns = 0;
    addr->dir_hold_dly_ns = 0;
    /* set 'old' values to make update_freq recalculate the timing params */
    addr->old_step_len = ~0;
