...
typedef struct {
    ...
    hal_bit_t *
        wrap; /* input/output pin: set true if the array is circular, false if linear */
} mod_dir_t;
...
int rtapi_app_main(void)
{
    ...
    if (mod_dir > 0) {
        ...
        rtapi_print_msg(
            RTAPI_MSG_INFO, "MODMATH: installed %d mod-dirs\n", mod_dir);
    }
    hal_ready(comp_id);
    return 0;
}
...
static int export_mod_dir(int num)
{
    ...
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "MODMATH: ERROR: '%s' pin export failed\n", buf);
        return retval;
    }
    ...
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "MODMATH: ERROR: '%s' param export failed\n", buf);
        return retval;
    }
    /* export function */
    ...
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "MODMATH: ERROR: '%s' funct export failed\n", buf);
        return -1;
    }
