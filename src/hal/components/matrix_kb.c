        ...
        if (scan == inst->now[inst->row] && scan != inst->then[inst->row]) {
            ...
            for (c = 0; c < inst->ncols; c++) {
                ...
                if ((inst->then[inst->row] & mask) && !(scan & mask)) { //keyup
                    *inst->hal.keycode =
                        inst->keyup + (inst->row << inst->rowshift) + c;
                    keyup(inst);
                } else if (!(inst->then[inst->row] & mask) &&
                           (scan & mask)) { //keydown
                    *inst->hal.keycode =
                        inst->keydown + (inst->row << inst->rowshift) + c;
...
int rtapi_app_main(void)
{
    ...
    if (n && n != kb->num_insts) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "matrix_kb: Number of sizes and number"
                        " of names must match\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    for (i = 0; i < kb->num_insts; i++) {
        ...
        for (inst->rowshift = 1; inst->ncols > (1u << inst->rowshift);
             inst->rowshift++)
            ;
        ...
        inst->hal.key = (hal_bit_t **)hal_malloc(inst->nrows * inst->ncols *
                                                 sizeof(hal_bit_t *));
        ...
        if (inst->scan) { //internally generated scanning
            inst->hal.rows =
                (hal_bit_t **)hal_malloc(inst->nrows * sizeof(hal_bit_t *));
            inst->hal.cols =
                (hal_bit_t **)hal_malloc(inst->ncols * sizeof(hal_bit_t *));
        ...
        retval =
            hal_export_funct(inst->name, loop, inst, 1, 0, comp_id); //needs fp?
        if (retval < 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "matrix_kb: ERROR: function export failed\n");
            return -1;
        }
    }
    hal_ready(comp_id);
