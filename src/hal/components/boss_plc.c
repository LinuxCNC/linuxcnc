    ...
    for (i = 0; i < count; i++) {
        ...
        if (pComp == NULL) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "BOSS_PLC: ERROR: hal_malloc() failed\n");
...
static int Plc_Export(Plc *this, int compId, int id)
{
    ...
    if (!error) {
        error = hal_export_functf(
            Plc_Refresh, this, 1, 0, compId, "boss_plc.%d.refresh", id);
    }
...
static int Plc_ExportFeed(Plc *this, int compId, int id)
{
    ...
    error = hal_pin_bit_newf(
        HAL_IN, &this->pCycleStartIn, compId, "boss_plc.%d.cycle-start-in", id);

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN,
                                 &this->pCycleHoldIn,
                                 compId,
                                 "boss_plc.%d.cycle-hold-in",
                                 id);
    }
    ...
    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT,
                                 &this->pFeedHoldOut,
                                 compId,
                                 "boss_plc.%d.feed-hold-out",
                                 id);
    }

    if (!error) {
        error = hal_pin_float_newf(HAL_IN,
                                   &this->pAdaptiveFeedIn,
                                   compId,
                                   "boss_plc.%d.adaptive-feed-in",
                                   id);
    }
    ...
    if (!error) {
        *this->pAdaptiveFeedIn = 1.0;
        error = hal_pin_float_newf(HAL_OUT,
                                   &this->pAdaptiveFeedOut,
                                   compId,
                                   "boss_plc.%d.adaptive-feed-out",
                                   id);
    }
    ...
    if (!error) {
        error = hal_pin_bit_newf(HAL_IN,
                                 &this->pToolChangeIn,
                                 compId,
                                 "boss_plc.%d.tool-change-in",
                                 id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT,
                                 &this->pToolChangedOut,
                                 compId,
                                 "boss_plc.%d.tool-changed-out",
                                 id);
    }
    ...
    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT,
                                 &this->pWaitUserOut,
                                 compId,
                                 "boss_plc.%d.wait-user-out",
                                 id);
    }

    if (!error) {
        error = hal_pin_bit_newf(
            HAL_IN, &this->pMistOnIn, compId, "boss_plc.%d.mist-on-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(
            HAL_OUT, &this->pMistOnOut, compId, "boss_plc.%d.mist-on-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(
            HAL_IN, &this->pFloodOnIn, compId, "boss_plc.%d.flood-on-in", id);
    }
...
static int Plc_ExportLimits(Plc *this, int compId, int id)
{
    ...
    // Export pins.
    error = hal_pin_bit_newf(HAL_IN,
                             &this->pLimitOverrideIn,
                             compId,
                             "boss_plc.%d.limit-override-in",
                             id);

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT,
                                 &this->pLimitActiveOut,
                                 compId,
                                 "boss_plc.%d.limit-active-out",
                                 id);
    }
    ...
    if (debug > 0) {
        if (!error) {
            error = hal_param_u32_newf(HAL_RO,
                                       &this->xLimit.state,
                                       compId,
                                       "boss_plc.%d.%c-limit-state",
                                       id,
                                       axisNames[0]);
        }
...
static int Plc_ExportAmps(Plc *this, int compId, int id)
{
    ...
    error = hal_pin_u32_newf(HAL_IO,
                             &this->ampReadyDelay,
                             compId,
                             "boss_plc.%d.amp-ready-delay",
                             id);
...
static int Plc_ExportSpindle(Plc *this, int compId, int id)
{
    ...
    // Export parameters.
    error = hal_pin_u32_newf(
        HAL_IO, &this->brakeOnDelay, compId, "boss_plc.%d.brake-on-delay", id);

    if (!error) {
        error = hal_pin_u32_newf(HAL_IO,
                                 &this->brakeOffDelay,
                                 compId,
                                 "boss_plc.%d.brake-off-delay",
                                 id);
    }

    if (!error) {
        error = hal_pin_float_newf(HAL_IO,
                                   &this->spindleLoToHi,
                                   compId,
                                   "boss_plc.%d.spindle-lo-to-hi",
                                   id);
    }
    ...
    if (!error) {
        error = hal_pin_float_newf(HAL_IN,
                                   &this->pSpindleSpeedIn,
                                   compId,
                                   "boss_plc.%d.spindle-speed-in",
                                   id);
    }
    ...
    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT,
                                 &this->pSpindleDecOut,
                                 compId,
                                 "boss_plc.%d.spindle-dec-out",
                                 id);
    }

    if (!error) {
        error = hal_pin_bit_newf(
            HAL_IN, &this->pBrakeEnIn, compId, "boss_plc.%d.brake-en-in", id);
    }
...
static void Plc_RefreshFeed(Plc *this, long period)
{
    ...
    *this->pFeedHoldOut = *this->pCycleHoldIn ||
                          ...
                          (*this->pSpindleIsOnIn && (this->lastSpindleSpeed !=
                                                     *this->pSpindleSpeedIn)) ||
...
static void Plc_RefreshLimits(Plc *this, long period)
{
    ...
    *this->pZLimitPosOut =
        *this->pZLimitPosIn && !(*this->pZJogEnIn && *this->pLimitOverrideIn);
    *this->pZLimitNegOut =
        *this->pZLimitNegIn && !(*this->pZJogEnIn && *this->pLimitOverrideIn);
...
static void Plc_RefreshSpindle(Plc *this, long period)
{
    ...
    *this->pSpindleIncOut =
        *this->pSpindleIncIn && !*this->pSpindleDecIn && *this->pSpindleIsOnIn;
    *this->pSpindleDecOut =
        *this->pSpindleDecIn && !*this->pSpindleIncIn && *this->pSpindleIsOnIn;
}
...
static int Limit_Export(Limit *this, int compId, int id, char axis)
{
    ...
    if (!error) {
        error = hal_pin_bit_newf(
            HAL_IN, &this->pIn, compId, "boss_plc.%d.%c-limit-in", id, axis);
    }
...
static void Amp_Refresh(Amp *this, long period, hal_u32_t readyDelay)
{
    ...
    *this->pFaultOut =
        *this->pEnableIn && !*this->pReadyIn && !Timer_IsEnabled(&this->timer);
