...
RTAPI_MP_ARRAY_INT(wsum_sizes,
                   MAX_SUMMERS,
                   "Sizes of up to 8 weighted summers");
...
static int
export_wsum(int num, int num_bits, wsum_t *addr, wsum_bit_t *bitaddr);
...
int rtapi_app_main(void)
{
    ...
    for (n = 0; n < MAX_SUMMERS && wsum_sizes[n] != -1; n++) {
        if (wsum_sizes[n] < 1) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
                "WEIGHTED_SUM: ERROR: Invalid number of bits '%i' for summer "
                "%i\n",
                wsum_sizes[n],
                n);
            return -1;
    ...
    for (n = 0; n < num_summers; n++) {
        /* export all vars */
        retval = export_wsum(
            n, wsum_sizes[n], &(wsum_array[n]), &(wsum_bit_array[total_bits]));
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_export_funct(
        "process_wsums", process_wsums, wsum_array, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
