...
int rtapi_app_main(void)
{
    ...
    if (!enums[0]) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "The enum_decode component requires at least"
    ...
    for (i = 0; i < MAX_CHAN; i++) {
        if (!enums[i] && !names[i]) {
            e.num_insts = i;
            rtapi_print_msg(
                RTAPI_MSG_ERR, "making %i instances\n", e.num_insts);
            break;
        }
        if ((!enums[i] && names[i]) || (!names[i] && names[0] && enums[i])) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "Inconsistent number of names and enums\n");
            return -EINVAL;
        }
    }
    ...
    e.insts = (enum_inst_t *)rtapi_kmalloc(e.num_insts * sizeof(enum_inst_t),
                                           RTAPI_GFP_KERNEL);
    for (i = 0; i < e.num_insts; i++) {
        ...
        inst->hal =
            (enum_hal_t *)hal_malloc((inst->num_pins + 1) * sizeof(enum_hal_t));
        token = enums[i];
        switch (*token) {
        case 'E':
        case 'e': // encode
            ...
            break;
        case 'D':
        case 'd': inst->dir = HAL_OUT; break;
        default:
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "Each enum string must start"
        ...
        if (inst->dir == HAL_OUT) {
            retval = hal_pin_u32_newf(
                HAL_IN, &(inst->hal[0].en), comp_id, "%s.input", this);
        } else {
            retval = hal_pin_u32_newf(
                HAL_OUT, &(inst->hal[0].en), comp_id, "%s.output", this);
        }
        v = 0;
        for (j = 1; j <= inst->num_pins; j++) { // 1-based indexing
            ...
            retval = hal_pin_bit_newf(inst->dir,
                                      &(inst->hal[j].bit),
                                      comp_id,
                                      "%s.%s-%s",
                                      this,
            ...
            retval += hal_pin_u32_newf(
                HAL_IN, &(inst->hal[j].en), comp_id, "%s.%s-val", this, token);
            *(inst->hal[j].en) = v++;
