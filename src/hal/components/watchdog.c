...
typedef struct {
    ...
    hal_float_t
        oldtimeout; /* internal:  used to determine whether the timeout has changed */
    hal_s32_t c_secs, c_nsecs; /* internal:  elapsed seconds and nanoseconds */
    hal_s32_t t_secs,
        t_nsecs;    /* internal:  seconds and nanoseconds for timeout */
...
typedef struct {
    hal_bit_t *
        output; /* output pin: high if all inputs are toggling, low otherwise */
    hal_bit_t *
        enable; /* pin: only runs while this is high (kind of like an enable) */
} watchdog_data_t;
...
int rtapi_app_main(void)
{
    ...
    if (num_inputs < 1) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "WATCHDOG: ERROR: must specify at least one input\n");
        return -1;
    }
    ...
    if (num_inputs > MAX_INPUTS) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "WATCHDOG: ERROR: too many inputs requested (%d > %d)\n",
            num_inputs,
            MAX_INPUTS);
        return -1;
    }
    ...
    for (n = 0; n < num_inputs; n++) {
        retval = hal_pin_bit_newf(
            HAL_IN, &(inputs[n].input), comp_id, "watchdog.input-%d", n);
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "WATCHDOG: ERROR: couldn't create input pin "
                            "watchdog.input-%d\n",
                            n);
            goto err;
        }
        retval = hal_param_float_newf(
            HAL_RW, &(inputs[n].timeout), comp_id, "watchdog.timeout-%d", n);
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "WATCHDOG: ERROR: couldn't create input parameter "
                            "watchdog.timeout-%d\n",
                            n);
            goto err;
        }
    ...
    /* export "global" pins */
    retval =
        hal_pin_bit_newf(HAL_OUT, &(data->output), comp_id, "watchdog.ok-out");
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, &(data->enable), comp_id, "watchdog.enable-in");
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
    ...
    /* export functions */
    retval =
        hal_export_funct("watchdog.process", process, inputs, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_export_funct(
        "watchdog.set-timeouts", set_timeouts, inputs, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
