...
int rtapi_app_main(void)
{
    ...
    for (n = 0; n < MAX_SAMPLERS; n++) {
        ...
        retval = hal_stream_create(&samplers[n].fifo,
                                   comp_id,
                                   SAMPLER_SHMEM_KEY + n,
                                   depth[n],
                                   cfg[n]);
...
static void sample(void *arg, long period)
{
    ...
    for (n = 0; n < num_pins; n++) {
        ...
        case HAL_FLOAT: dptr->f = *(pptr->hfloat); break;
        case HAL_BIT:
            if (*(pptr->hbit)) {
                dptr->b = 1;
            } else {
            ...
            break;
        case HAL_U32: dptr->u = *(pptr->hu32); break;
        case HAL_S32: dptr->s = *(pptr->hs32); break;
        default: break;
        }
        dptr++;
        pptr++;
...
static int init_sampler(int num, sampler_t *str)
{
    ...
    retval = hal_pin_bit_newf(
        HAL_OUT, &(str->full), comp_id, "sampler.%d.full", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, &(str->enable), comp_id, "sampler.%d.enable", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_s32_newf(
        HAL_OUT, &(str->curr_depth), comp_id, "sampler.%d.curr-depth", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_s32_newf(
        HAL_IO, &(str->overruns), comp_id, "sampler.%d.overruns", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_pin_s32_newf(
        HAL_IO, &(str->sample_num), comp_id, "sampler.%d.sample-num", num);
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
    ...
    *(str->sample_num) = 0;
    ...
    for (n = 0; n < hal_stream_element_count(&str->fifo); n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "SAMPLER: ERROR: pin '%s' export failed\n", buf);
            return -EIO;
        }
        /* init the pin value */
    ...
    retval =
        hal_export_functf(sample, str, usefp, 0, comp_id, "sampler.%d", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
