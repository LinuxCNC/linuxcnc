...
*/
static const unsigned char lut_x4[16] = {0x00,
                                         0x44,
                                         0x88,
                                         0x0C,
                                         0x80,
...
static const unsigned char lut_x1[16] = {0x00,
                                         0x44,
                                         0x08,
                                         0x0C,
                                         0x80,
...
int rtapi_app_main(void)
{
    ...
    retval = hal_export_funct(
        "encoder.update-counters", update, counter_array, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_export_funct(
        "encoder.capture-position", capture, counter_array, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "ENCODER: installed %d encoder counters\n", howmany);
    hal_ready(comp_id);
    return 0;
}
...
static void capture(void *arg, long period)
{
    ...
    for (n = 0; n < howmany; n++) {
        ...
        if (buf->count_detected) {
            ...
            if (delta_counts != 0) {
                cntr->limit_dt *= 0.9;
                cntr->limit_dt += 0.1 * ((*(cntr->missing_teeth) + 0.5) *
                                         (delta_time / delta_counts));
            }
            // correct counts for tooth gap
            *cntr->raw_counts += *(cntr->missing_teeth) * cntr->gaps;
            cntr->raw_count =
                buf->raw_count + *(cntr->missing_teeth) * cntr->gaps;
            ...
            if (cntr->counts_since_timeout) {
                ...
                if (*(cntr->missing_teeth) &&
                    delta_time > 1.5 * cntr->limit_dt) {
                    // dont update the velocity in the tooth gap
                } else if (delta_time <
                           1e9 / (*(cntr->min_speed) * cntr->scale)) {
...
static int export_encoder(counter_t *addr, char *prefix)
{
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->phaseA), comp_id, "%s.phase-A", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->phaseB), comp_id, "%s.phase-B", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for the index input */
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->phaseZ), comp_id, "%s.phase-Z", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for the index enable input */
    retval = hal_pin_bit_newf(
        HAL_IO, &(addr->index_ena), comp_id, "%s.index-enable", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for the reset input */
    retval =
        hal_pin_bit_newf(HAL_IN, &(addr->reset), comp_id, "%s.reset", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pins for position latching */
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->latch_in), comp_id, "%s.latch-input", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->latch_rising), comp_id, "%s.latch-rising", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->latch_falling), comp_id, "%s.latch-falling", prefix);
    if (retval != 0) {
        return retval;
    }
    ...
    retval = hal_pin_s32_newf(
        HAL_OUT, &(addr->raw_counts), comp_id, "%s.rawcounts", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for counts captured by capture() */
    retval =
        hal_pin_s32_newf(HAL_OUT, &(addr->count), comp_id, "%s.counts", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for counts latched by capture() */
    retval = hal_pin_s32_newf(
        HAL_OUT, &(addr->count_latch), comp_id, "%s.counts-latched", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for minimum speed estimated by capture() */
    retval = hal_pin_float_newf(
        HAL_IN, &(addr->min_speed), comp_id, "%s.min-speed-estimate", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled position captured by capture() */
    retval = hal_pin_float_newf(
        HAL_OUT, &(addr->pos), comp_id, "%s.position", prefix);
    if (retval != 0) {
        return retval;
    }
    ...
    retval = hal_pin_float_newf(
        HAL_OUT, &(addr->pos_latch), comp_id, "%s.position-latched", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled velocity captured by capture() */
    retval = hal_pin_float_newf(
        HAL_OUT, &(addr->vel), comp_id, "%s.velocity", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled velocity captured by capture() */
    retval = hal_pin_float_newf(
        HAL_OUT, &(addr->vel_rpm), comp_id, "%s.velocity-rpm", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaling */
    retval = hal_pin_float_newf(
        HAL_IO, &(addr->pos_scale), comp_id, "%s.position-scale", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for x4 mode */
    retval = hal_pin_bit_newf(
        HAL_IO, &(addr->x4_mode), comp_id, "%s.x4-mode", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for counter mode */
    retval = hal_pin_bit_newf(
        HAL_IO, &(addr->counter_mode), comp_id, "%s.counter-mode", prefix);
    if (retval != 0) {
        return retval;
    }
    /* export pin for missing-tooth index */
    retval = hal_pin_s32_newf(
        HAL_IN, &(addr->missing_teeth), comp_id, "%s.missing-teeth", prefix);
    if (retval != 0) {
        return retval;
    }
