...
/** This file, 'freqgen.c', is a HAL component that generates step
    ...
    thread, which is called 'freqgen.thread'.  The 'fp_period'
    parameter sets the period of the floating point thread, which
    is called 'freqgen.threadFP'
...
int rtapi_app_main(void)
{
	...
	if ((period < 1000) || (period > 500000)) {
	    rtapi_print_msg(RTAPI_MSG_ERR,
		"FREQGEN: ERROR: invalid period: %d\n", period);
	    return -1;
	}
    }
    /* test for 'fp_period' */
	...
	if ((fp_period < (2 * period)) || (period > 50000000)) {
	    rtapi_print_msg(RTAPI_MSG_ERR,
		"FREQGEN: ERROR: invalid fp_period: %d\n", period);
	    return -1;
	}
    }
    ...
    if (period > 0) {
	/* create a thread */
	retval = hal_create_thread("freqgen.thread", period, 0);
	if (retval < 0) {
	    rtapi_print_msg(RTAPI_MSG_ERR,
		"FREQGEN: ERROR: could not create non-FP thread\n");
	    hal_exit(comp_id);
	    return -1;
	} else {
	    rtapi_print_msg(RTAPI_MSG_INFO,
		"FREQGEN: created %d uS non-FP thread\n", period / 1000);
	}
    }
    if (fp_period > 0) {
	/* create a floating point thread */
	retval = hal_create_thread("freqgen.threadFP", fp_period, 1);
	if (retval < 0) {
	    rtapi_print_msg(RTAPI_MSG_ERR,
		"FREQGEN: ERROR: could not create FP thread\n");
	    hal_exit(comp_id);
	    return -1;
	} else {
	    rtapi_print_msg(RTAPI_MSG_INFO,
		"FREQGEN: created %d uS FP thread\n", fp_period / 1000);
	}
    }
    return 0;
}
