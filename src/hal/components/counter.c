...
int rtapi_app_main(void)
{
    ...
    if ((num_chan <= 0) || (num_chan > MAX_CHAN)) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "COUNTER: ERROR: invalid num_chan: %d\n", num_chan);
        return -EINVAL;
    }
    ...
    retval = hal_export_funct(
        "counter.update-counters", update, counter_array, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_export_funct(
        "counter.capture-position", capture, counter_array, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "COUNTER: installed %d counter counters\n", num_chan);
    hal_ready(comp_id);
    return 0;
}
...
static int export_counter(int num, counter_t *addr)
{
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->phaseA), comp_id, "counter.%d.phase-A", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for the index input */
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->phaseZ), comp_id, "counter.%d.phase-Z", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for the index enable input */
    retval = hal_pin_bit_newf(
        HAL_IO, &(addr->index_ena), comp_id, "counter.%d.index-enable", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for the reset input */
    retval = hal_pin_bit_newf(
        HAL_IN, &(addr->reset), comp_id, "counter.%d.reset", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for raw counts */
    retval = hal_pin_s32_newf(
        HAL_OUT, &(addr->raw_count), comp_id, "counter.%d.rawcounts", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for counts captured by capture() */
    retval = hal_pin_s32_newf(
        HAL_OUT, &(addr->count), comp_id, "counter.%d.counts", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled position captured by capture() */
    retval = hal_pin_float_newf(
        HAL_OUT, &(addr->pos), comp_id, "counter.%d.position", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled velocity captured by capture() */
    retval = hal_pin_float_newf(
        HAL_OUT, &(addr->vel), comp_id, "counter.%d.velocity", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for scaling */
    retval = hal_pin_float_newf(
        HAL_IO, &(addr->pos_scale), comp_id, "counter.%d.position-scale", num);
    if (retval != 0) {
        return retval;
    }
