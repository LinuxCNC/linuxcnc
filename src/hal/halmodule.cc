...
#define EXCEPTION_IF_NOT_LIVE(retval)                                          \
    do {                                                                       \
        if (self->hal_id <= 0) {                                               \
...
bool from_python(PyObject *o, double *d)
{
    ...
    if (!tmp) {
        PyErr_Format(
            PyExc_TypeError, "Number expected, not %s", Py_TYPE(o)->tp_name);
        return false;
    }
...
static int pyhal_init(PyObject *_self, PyObject *args, PyObject *kw)
{
    ...
    if (!PyArg_ParseTuple(args, "s|s:hal.component", &name, &prefix))
        return -1;
...
static int pyhal_write_common(halitem *pin, PyObject *value)
{
    ...
    if (pin->is_pin) {
        switch (pin->type) {
        case HAL_BIT: *pin->u->pin.b = PyObject_IsTrue(value); break;
        case HAL_FLOAT: {
            double tmp;
            if (!from_python(value, &tmp))
                return -1;
            *pin->u->pin.f = tmp;
            break;
        }
        ...
        switch (pin->type) {
        case HAL_BIT: pin->u->param.b = PyObject_IsTrue(value); break;
        case HAL_FLOAT: {
            double tmp;
            if (!from_python(value, &tmp))
                return -1;
...
static PyObject *pyhal_create_param(halobject *self,
                                    char *name,
                                    hal_type_t type,
                                    hal_param_dir_t dir)
{
...
static PyObject *pyhal_create_pin(halobject *self,
                                  char *name,
                                  hal_type_t type,
                                  hal_pin_dir_t dir)
{
...
static PyObject *pyhal_new_param(PyObject *_self, PyObject *o)
{
    ...
    return pyhal_create_param(
        self, name, (hal_type_t)type, (hal_param_dir_t)dir);
}
...
static PyObject *pyhal_get_pins(PyObject *_self, PyObject * /*o*/)
{
    ...
    for (itemmap::iterator i = self->items->begin(); i != self->items->end();
         ++i) {
...
static PyObject *pyhal_repr(PyObject *_self)
{
    ...
    return PyUnicode_FromFormat(
        "<hal component %s(%d) with %d pins and params>",
        self->name,
        self->hal_id,
        (int)self->items->size());
}
...
static PyObject *pyhal_set_prefix(PyObject *_self, PyObject *args)
{
    char *newprefix;
    halobject *self = (halobject *)_self;
    if (!PyArg_ParseTuple(args, "s", &newprefix))
        return NULL;
    EXCEPTION_IF_NOT_LIVE(NULL);
...
static PyMethodDef hal_methods[] = {
    {"setprefix",
     pyhal_set_prefix, METH_VARARGS,
     "Set the prefix for newly created pins and parameters"},
    {"getprefix",
     pyhal_get_prefix, METH_VARARGS,
     "Get the prefix for newly created pins and parameters"},
    {"newparam", pyhal_new_param, METH_VARARGS, "Create a new parameter"},
    {"newpin", pyhal_new_pin, METH_VARARGS, "Create a new pin"},
    {"getitem", pyhal_get_pin, METH_VARARGS, "Get existing pin object"},
    {"getpins",
     pyhal_get_pins, METH_VARARGS,
     "Get all pins and values of component"},
    {"exit", pyhal_exit, METH_NOARGS, "Call hal_exit"},
    {"ready", pyhal_ready, METH_NOARGS, "Call hal_ready"},
    {"unready", pyhal_unready, METH_NOARGS, "Call hal_unready"},
    {},
};

static PyMappingMethods halobject_map = {
    pyhal_len, pyhal_getattro, pyhal_setattro};

static PyTypeObject halobject_type = {
    PyVarObject_HEAD_INIT(NULL, 0) "hal.component", /*tp_name*/
...
static PyObject *pyhalpin_repr(PyObject *_self)
{
    ...
    if (!self->is_pin)
        return PyUnicode_FromFormat("<hal param \"%s\" %s-%s>",
                                    name,
                                    pin_type2name(self->type),
                                    param_dir2name(self->dir.paramdir));
    return PyUnicode_FromFormat("<hal pin \"%s\" %s-%s>",
                                name,
                                pin_type2name(self->type),
                                pin_dir2name(self->dir.pindir));
}
...
static PyTypeObject halpin_type = {
    PyVarObject_HEAD_INIT(NULL, 0) "hal.item", /*tp_name*/
...
PyObject *new_sig(PyObject * /*self*/, PyObject *args)
{
    ...
    switch (type) {
    case HAL_BIT: retval = hal_signal_new(name, HAL_BIT); break;
    case HAL_S32: retval = hal_signal_new(name, HAL_S32); break;
    case HAL_U32: retval = hal_signal_new(name, HAL_U32); break;
    case HAL_S64: retval = hal_signal_new(name, HAL_S64); break;
    case HAL_U64: retval = hal_signal_new(name, HAL_U64); break;
    case HAL_FLOAT: retval = hal_signal_new(name, HAL_FLOAT); break;
    default: {
...
PyObject *connect(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ss", &pinname, &signame))
        return NULL;
    if (!hal_shmem_base) {
        PyErr_Format(PyExc_RuntimeError,
                     "Cannot call before creating component");
...
static int set_common(hal_type_t type, void *d_ptr, char *value)
{
    ...
    struct scoped_locale {
        scoped_locale(int category_, const char *locale_)
            : category(category_), oldlocale(setlocale(category, NULL))
        {
            setlocale(category, locale_);
        }
...
PyObject *set_s(PyObject * /*self*/, PyObject *args)
{
    ...
    if (sig == 0) {
        rtapi_mutex_give(&(hal_data->mutex));
        PyErr_Format(PyExc_RuntimeError, "signal not found");
        return NULL;
    } else {
...
PyObject *get_value(PyObject * /*self*/, PyObject *args)
{
    ...
    if (param) {
        ...
        case HAL_U32:
            return Py_BuildValue("l", (unsigned long)*(hal_u32_t *)d_ptr);
        case HAL_S32: return Py_BuildValue("l", (long)*(hal_s32_t *)d_ptr);
        case HAL_U64:
            return Py_BuildValue("l", (unsigned long long)*(hal_u64_t *)d_ptr);
        case HAL_S64: return Py_BuildValue("l", (long long)*(hal_s64_t *)d_ptr);
        case HAL_FLOAT:
            return Py_BuildValue("f", (double)*(hal_float_t *)d_ptr);
        case HAL_PORT: // HAL_PORT is currently not supported
        default: break;
        }
    }
    ...
    rtapi_mutex_give(&(hal_data->mutex));
    PyErr_Format(
        PyExc_RuntimeError, "Can't set value: pin / param %s not found", name);
    return NULL;
}
    ...
    while (next != 0) {
        ...
        case HAL_BIT:
            obj = Py_BuildValue("{s:s,s:N,s:N,s:N}",
                                ...
                                str_d,
                                PyLong_FromLong(pin->dir),
                                str_t,
                                PyLong_FromLong(HAL_BIT));
            break;
        case HAL_U32:
        ...
        case HAL_S32:
            obj = Py_BuildValue("{s:s,s:l,s:N,s:N}",
                                ...
                                pin->name,
                                str_v,
                                (long)*(hal_s32_t *)d_ptr,
                                str_d,
                                PyLong_FromLong(pin->dir),
                                str_t,
                                PyLong_FromLong(HAL_S32));
            break;
        case HAL_FLOAT:
...
PyObject *get_info_params(PyObject * /*self*/, PyObject * /*args*/)
{
    ...
    while (next != 0) {
        ...
        case HAL_TYPE_UNINITIALIZED: /* fallthrough */;
        default:
            obj = Py_BuildValue("{s:s,s:s}", str_n, param->name, str_v, NULL);
            break;
        }
...
static PyObject *pyshm_repr(PyObject *_self)
{
    shmobject *self = (shmobject *)_self;
    return PyUnicode_FromFormat(
        "<shared memory buffer key=%08x id=%d size=%ld>",
        self->key,
        self->shm_id,
...
static PyBufferProcs shmbuffer_procs = {
    ...
    (releasebufferproc)
        NULL, //(releasebufferproc)shm_buffer_releasebuffer, /* bf_releasebuffer */
};

static PyMethodDef shm_methods[] = {
    {"getbuffer",
     shm_getbuffer, METH_NOARGS,
     "Get a writable buffer object for the shared memory segment"},
    {"setsize",
     shm_setsize, METH_VARARGS,
     "Set the size of the shared memory segment"},
    {},
};

static PyTypeObject shm_type = {
    PyVarObject_HEAD_INIT(NULL, 0) "hal.shm", /*tp_name*/
...
struct streamobj {
    PyObject_HEAD hal_stream_t stream;
    PyObject *pyelt;
    halobject *comp;
    int key;
    bool creator;
...
static int pystream_init(PyObject *_self, PyObject *args, PyObject * /*kw*/)
{
    ...
    if (depth) {
        self->creator = 1;
        r = hal_stream_create(
            &self->stream, self->comp->hal_id, self->key, depth, typestring);
    } else {
        self->creator = 0;
        r = hal_stream_attach(
            &self->stream, self->comp->hal_id, self->key, typestring);
    }
    if (r < 0) {
        errno = -r;
...
PyObject *stream_write(PyObject *_self, PyObject *args)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        case 'f':
            if (!from_python(o, &buf[i].f))
                return NULL;
            break;
        case 's':
            if (!from_python(o, &buf[i].s))
                return NULL;
            break;
        case 'u':
            if (!from_python(o, &buf[i].u))
                return NULL;
            break;
...
template <class T> PyObject *stream_getter(PyObject *_self, void *vfp)
{
...
static PyMemberDef stream_members[] = {
    {"sampleno",
     T_UINT, offsetof(streamobj, sampleno),
     READONLY, "The number of the last successfully read sample"},
    {}
};
...
static PyGetSetDef stream_getset[] = {
    ...
    {"num_underruns",
     stream_getter<int>,
     NULL, NULL,
     VFC(hal_stream_num_underruns)},
    {"num_overruns",
     stream_getter<int>,
     NULL, NULL,
     VFC(hal_stream_num_overruns)},
    {}
};
...
static PyObject *pystream_repr(PyObject *_self)
{
    streamobj *self = reinterpret_cast<streamobj *>(_self);
    return PyUnicode_FromFormat(
        "<stream 0x%x%s>", self->key, self->creator ? " creator" : "");
}

static PyTypeObject stream_type = {
    PyVarObject_HEAD_INIT(NULL, 0) "hal.stream", /*tp_name*/
...
PyMethodDef module_methods[] = {
    {"pin_has_writer",
     pin_has_writer, METH_VARARGS,
     ".pin_has_writer('pin_name'): Return a FALSE value if a pin has no "
     "writers and TRUE if it does"},
    {"component_exists",
     component_exists, METH_VARARGS,
     ".component_exists('component_name'): Return a TRUE value if the named "
     "component exists"},
    {"component_is_ready",
     component_is_ready, METH_VARARGS,
     ".component_is_ready('component_name'): Return a TRUE value if the named "
     "component is ready"},
    {"set_msg_level",
     set_msg_level, METH_VARARGS,
     ".set_msg_level(level): Set the RTAPI message level"},
    {"get_msg_level",
     get_msg_level, METH_NOARGS,
     ".get_msg_level(): Get the RTAPI message level"},
    {"new_sig",
     new_sig, METH_VARARGS,
     ".new_sig('signal_name', type): Create a new signal with the specified "
     "name.  'type' is one of HAL_BIT, HAL_FLOAT, HAL_S32, or HAL_U32."},
    {"connect",
     connect, METH_VARARGS,
     ".connect('pin_name', 'signal_name'): Connect the named pin to the named "
     "signal."},
    {"disconnect",
    ...
    {"get_info_pins",
     get_info_pins, METH_VARARGS,
     ".get_info_pins(): Get a list of dicts for all the pins; {NAME:, VALUE:, "
     "DIRECTION:}"},
    {"get_info_signals",
     get_info_signals, METH_VARARGS,
     ".get_info_signals(): Get a list of dicts for all the signals; {NAME:, "
     "VALUE:}"},
    {"get_info_params",
     get_info_params, METH_VARARGS,
     ".get_info_params(): Get a list of dicts for all the parameters; {NAME:, "
     "VALUE:}"},
    {},
};
...
PyMODINIT_FUNC PyInit__hal(void)
{
    ...
    PyRun_SimpleString("(lambda s=__import__('signal'):"
