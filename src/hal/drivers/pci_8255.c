...
int rtapi_app_main(void) {
	...
	r = hal_param_new_ui32(comp_id, HAL_RO, &(inst[i].ioaddr), io[i],
	    "pci8255.%d.io-addr", i);
	for(j=0; j<3; j++) {
...
static void write(struct port *inst, long period) {
    int p = hal_get_ui32(dir_);
    rtapi_u32 xioaddr = hal_get_ui32(ioaddr);
    static int first=1;
...
static void read(struct port *inst, long period) {
    ...
    int i;
    if((p & 5) == 5) {
        int byte = pci_8255_inb(xioaddr, 2);
        for(i=0; i<8; i++) {
            int t = (byte & (1<<i)) != 0;
            c_set(i, t);
            ci_not_set(i, !t);
    ...
    if(p & 2) {
        int byte = pci_8255_inb(xioaddr, 1);
        for(i=0; i<8; i++) {
            int t = (byte & (1<<i)) != 0;
            b_set(i, t);
            bi_not_set(i, !t);
    ...
    if(p & 8) {
        int byte = pci_8255_inb(xioaddr, 0);
        for(i=0; i<8; i++) {
            int t = (byte & (1<<i)) != 0;
            a_set(i, t);
            ai_not_set(i, !t);
