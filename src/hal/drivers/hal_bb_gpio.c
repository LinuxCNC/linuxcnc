...
typedef struct {
    ...
    hal_bit_t *
        input_pins[1 + PINS_PER_HEADER * HEADERS]; // array of pointers to bivts
    hal_bit_t *
        output_pins[1 + PINS_PER_HEADER * HEADERS]; // array of pointers to bits
    hal_bit_t *led_inv[4];
...
RTAPI_MP_STRING(
    input_pins,
    "input pins, comma separated.  P8 pins add 800, P9 pins add 900");

static char *output_pins;
RTAPI_MP_STRING(
    output_pins,
    "output pins, comma separated.  P8 pins add 800, P9 pins add 900");

int configure_control_module()
{
    int fd = rtapi_open_as_root("/dev/mem", O_RDWR);

    control_module = mmap(0,
                          CONTROL_MODULE_SIZE,
...
int configure_gpio_port(int n)
{
    ...
    if (n > 0) {
        cm_per = mmap(
            0, CM_PER_LEN, PROT_READ | PROT_WRITE, MAP_SHARED, fd, CM_PER_ADDR);
        if (cm_per == MAP_FAILED) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "%s: ERROR: Unable to map Clock Module: %s\n",
                            modname,
                            strerror(errno));
            return -errno;
        }
        // point at CM_PER_GPIOn_CLKCTRL register for port n
        regptr = (void *)((char *)cm_per + CM_PER_GPIO1_CLKCTRL_OFFSET +
                          4 * (n - 1));
        regvalue = *regptr;
    ...
    gpio_ports[n]->gpio_addr = mmap(0,
                                    GPIO_SIZE,
                                    PROT_READ | PROT_WRITE,
                                    MAP_SHARED,
                                    fd,
                                    start_addr_for_port(n));

    if (gpio_ports[n]->gpio_addr == MAP_FAILED) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "%s: ERROR: Unable to map GPIO: %s\n",
                        modname,
                        strerror(errno));
        return -errno;
    }
    ...
    gpio_ports[n]->oe_reg =
        (void *)((char *)gpio_ports[n]->gpio_addr + GPIO_OE);
    gpio_ports[n]->setdataout_reg =
        (void *)((char *)gpio_ports[n]->gpio_addr + GPIO_SETDATAOUT);
    gpio_ports[n]->clrdataout_reg =
        (void *)((char *)gpio_ports[n]->gpio_addr + GPIO_CLEARDATAOUT);
    gpio_ports[n]->datain_reg =
        (void *)((char *)gpio_ports[n]->gpio_addr + GPIO_DATAIN);


    rtapi_print("memmapped gpio port %d to %p, oe: %p, set: %p, clr: %p\n",
                n,
                gpio_ports[n]->gpio_addr,
                gpio_ports[n]->oe_reg,
                gpio_ports[n]->setdataout_reg,
                gpio_ports[n]->clrdataout_reg);
...
int rtapi_app_main(void)
{
    ...
    if (user_leds != NULL) {
        ...
        while ((token = strtok(data, ",")) != NULL) {
            ...
            // Add HAL pin
            retval = hal_pin_bit_newf(HAL_IN,
                                      &(port_data->led_inv[led]),
                                      comp_id,
                                      "bb_gpio.userled%d-invert",
                                      led);
    ...
    if (input_pins != NULL) {
        ...
        while ((token = strtok(data, ",")) != NULL) {
            ...
            if (pin >= 801 && pin <= 846) {
                ...
                rtapi_print_msg(
                    RTAPI_MSG_ERR,
                    "%s: ERROR: invalid pin number '%d'.  Valid pins are "
                    "801-846 for P8 pins, 901-946 for P9 pins.\n",
                    modname,
                    pin);
                hal_exit(comp_id);
                return -1;
            ...
            data =
                NULL; // after the first call, subsequent calls to strtok need to be on NULL

            // Add HAL pin
            retval = hal_pin_bit_newf(
                HAL_OUT,
                &(port_data->input_pins[pin + (header - 8) * PINS_PER_HEADER]),
                comp_id,
                "bb_gpio.p%d.in-%02d",
                header,
                pin);
    ...
    if (output_pins != NULL) {
        ...
        while ((token = strtok(data, ",")) != NULL) {
            ...
            retval = hal_pin_bit_newf(
                HAL_IN,
                &(port_data->output_pins[pin + (header - 8) * PINS_PER_HEADER]),
                comp_id,
                "bb_gpio.p%d.out-%02d",
                header,
                pin);
            ...
            retval = hal_pin_bit_newf(
                HAL_IN,
                &(port_data->output_inv[pin + (header - 8) * PINS_PER_HEADER]),
                comp_id,
                "bb_gpio.p%d.out-%02d-invert",
                header,
                pin);
    ...
    // export functions
    retval =
        hal_export_funct("bb_gpio.write", write_port, port_data, 0, 0, comp_id);
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval =
        hal_export_funct("bb_gpio.read", read_port, port_data, 0, 0, comp_id);
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "%s: ERROR: port %d read funct export failed\n",
                        modname,
                        n);
        hal_exit(comp_id);
        return -1;
...
{
    switch (port) {
    case 0: return GPIO0_START_ADDR; break;
    case 1: return GPIO1_START_ADDR; break;
    case 2: return GPIO2_START_ADDR; break;
    case 3: return GPIO3_START_ADDR; break;
...
void configure_pin(bb_gpio_pin *pin, char mode)
{
    volatile unsigned int *control_reg =
        (void *)((char *)control_module + pin->control_offset);
    pin->claimed = mode;
    switch (mode) {
    case 'O':
        *(pin->port->oe_reg) &=
            ~(1 << pin->pin_num); // 0 in OE is output enable
    ...
    case 'U':
        *(pin->port->oe_reg) |= (1 << pin->pin_num); // 1 in OE is input
        *control_reg =
            PIN_MODE7 | PIN_PULLUD_ENABLED | PIN_PULLUP | PIN_RX_ENABLED;
        break;
    case 'D':
        *(pin->port->oe_reg) |= (1 << pin->pin_num); // 1 in OE is input
        *control_reg =
            PIN_MODE7 | PIN_PULLUD_ENABLED | PIN_PULLDOWN | PIN_RX_ENABLED;
        break;
    default: break;
    }
}
