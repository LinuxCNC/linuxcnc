...
int hm2_watchdog_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    hm2->watchdog.instance = (hm2_watchdog_instance_t *)hal_malloc(
        hm2->watchdog.num_instances * sizeof(hm2_watchdog_instance_t));
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->watchdog.num_instances * sizeof(rtapi_u32)),
        &hm2->watchdog.status_reg);
    if (r < 0) {
    ...
    hm2->watchdog.timer_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->watchdog.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    *hm2->watchdog.instance[0].hal.pin.has_bit = 0;
    hm2->watchdog.instance[0].hal.param.timeout_ns =
        5 * 1000 * 1000; // default timeout is 5 milliseconds
    hm2->watchdog.instance[0].enable =
        0; // the first pet_watchdog will turn it on
...
void hm2_watchdog_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              ...
              hm2_hz_to_mhz(hm2->watchdog.clock_frequency));
...
void hm2_watchdog_force_write(hostmot2_t *hm2)
{
    ...
    if (hm2->watchdog.instance[0].enable == 0) {
        ...
        tmp = (hm2->watchdog.instance[0].hal.param.timeout_ns *
               ((double)hm2->watchdog.clock_frequency /
        ...
        if (tmp < 0x80000000) {
            ...
            hm2->watchdog.timer_reg[0] = tmp;
            hm2->watchdog.instance[0].hal.param.timeout_ns =
                (tmp + 1) / ((double)hm2->watchdog.clock_frequency /
                             (double)(1000 * 1000 * 1000));
            HM2_ERR("requested watchdog timeout is out of range, setting it to "
                    "max: %u ns\n",
                    hm2->watchdog.instance[0].hal.param.timeout_ns);
        }
    }
    ...
    hm2->llio->write(hm2->llio,
                     hm2->watchdog.timer_addr,
                     hm2->watchdog.timer_reg,
                     (hm2->watchdog.num_instances * sizeof(rtapi_u32)));
    hm2->watchdog.instance[0].written_timeout_ns =
        hm2->watchdog.instance[0].hal.param.timeout_ns;
...
void hm2_watchdog_write(hostmot2_t *hm2, long period_ns)
{
    ...
    if ((hm2->watchdog.instance[0].hal.param.timeout_ns ==
         hm2->watchdog.instance[0].written_timeout_ns) &&
        (hm2->watchdog.instance[0].enable ==
         hm2->watchdog.instance[0].written_enable)) {
        return;
    }
    ...
    if (hm2->watchdog.instance[0].hal.param.timeout_ns < (1.5 * period_ns)) {
        HM2_PRINT("Watchdog timeout (%u ns) is dangerously short compared to "
                  "hm2_write() period (%ld ns)\n",
                  hm2->watchdog.instance[0].hal.param.timeout_ns,
                  period_ns);
    }
