...
void hm2_periodm_update_limit(hostmot2_t *hm2, int i)
{
    hm2->periodm.limit_reg[i] =
        ((double)hm2->periodm.clock_frequency /
         (double)*hm2->periodm.instance[i].hal.pin.minfreq) *
        *hm2->periodm.instance[i].hal.pin.averages;
    if ((((double)hm2->periodm.clock_frequency /
          (double)*hm2->periodm.instance[i].hal.pin.minfreq) *
         *hm2->periodm.instance[i].hal.pin.averages) > 0xFFFFFFFF) {
        HM2_ERR("periodm %d has invalid min freq time, resetting to min\n", i);
        *hm2->periodm.instance[i].hal.pin.minfreq =
            (0.025 * *hm2->periodm.instance[i].hal.pin.averages);
        hm2->periodm.limit_reg[i] =
            ((double)hm2->periodm.clock_frequency /
             (double)*hm2->periodm.instance[i].hal.pin.minfreq) *
            *hm2->periodm.instance[i].hal.pin.averages;
...
void hm2_periodm_update_mode(hostmot2_t *hm2, int i)
{
    ...
    rtapi_u32 averages;
    filterclocks = (double)*hm2->periodm.instance[i].hal.pin.filtertc *
                   ((double)hm2->periodm.clock_frequency / (double)1e6);
    if (filterclocks > 0xFFFF) {
        HM2_ERR(
            "periodm %d has invalid filter time constant, resetting to max\n",
            i);
        filterclocks = 0xFFFF;
        *hm2->periodm.instance[i].hal.pin.filtertc =
            (double)filterclocks *
            ((double)1e6 / (double)hm2->periodm.clock_frequency);
    }
...
void hm2_periodm_force_write(hostmot2_t *hm2)
{
    ...
    hm2->llio->write(hm2->llio,
                     hm2->periodm.mode_write_addr,
                     hm2->periodm.mode_write_reg,
                     (hm2->periodm.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
                     hm2->periodm.limit_addr,
                     hm2->periodm.limit_reg,
                     (hm2->periodm.num_instances * sizeof(rtapi_u32)));
...
int hm2_periodm_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->config.num_periodms > md->instances) {
        HM2_ERR(
            "config.num_periodms=%d, but only %d are available, not loading "
            "driver\n",
    ...
    hm2->periodm.instance = (hm2_periodm_instance_t *)hal_malloc(
        hm2->periodm.num_instances * sizeof(hm2_periodm_instance_t));
    ...
    // FIXME: r hides the r in enclosing function, and it returns the wrong thing
    {
        ...
        for (i = 0; i < hm2->periodm.num_instances; i++) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.period_us",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_OUT,
                                  &(hm2->periodm.instance[i].hal.pin.period),
                                  hm2->llio->comp_id);
            if (r < 0) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.width_us",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_OUT,
                                  &(hm2->periodm.instance[i].hal.pin.width),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.duty_cycle",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_OUT,
                                  &(hm2->periodm.instance[i].hal.pin.dutycycle),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.duty_cycle_scale",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->periodm.instance[i].hal.pin.dutyscale),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.duty_cycle_offset",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(
                name,
                HAL_IN,
                &(hm2->periodm.instance[i].hal.pin.dutyoffset),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.frequency",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_OUT,
                                  &(hm2->periodm.instance[i].hal.pin.frequency),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.minimum_frequency",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.invert",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IO,
                                &(hm2->periodm.instance[i].hal.pin.polarity),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            r = hal_pin_bit_new(name,
                                HAL_OUT,
                                &(hm2->periodm.instance[i].hal.pin.valid),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.periodm.%02d.input_status",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_OUT,
                                &(hm2->periodm.instance[i].hal.pin.input),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
...
void hm2_periodm_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              hm2->periodm.clock_frequency,
              hm2_hz_to_mhz(hm2->periodm.clock_frequency));
    HM2_PRINT("    version: %d\n", hm2->periodm.version);
...
void hm2_periodm_process_tram_read(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->periodm.num_instances; i++) {
        ...
        *hm2->periodm.instance[i].hal.pin.period =
            (hm2->periodm.period_reg[i] /
             ((double)hm2->periodm.clock_frequency)) *
            (double)1e6 / *hm2->periodm.instance[i].hal.pin.averages;
        *hm2->periodm.instance[i].hal.pin.width =
            (hm2->periodm.width_reg[i] /
             ((double)hm2->periodm.clock_frequency)) *
            (double)1e6 / *hm2->periodm.instance[i].hal.pin.averages;
        if (*hm2->periodm.instance[i].hal.pin.period != 0) {
            *hm2->periodm.instance[i].hal.pin.dutycycle =
                (((double)*hm2->periodm.instance[i].hal.pin.width /
                  (double)*hm2->periodm.instance[i].hal.pin.period) *
                 (double)*hm2->periodm.instance[i].hal.pin.dutyscale) +
                *hm2->periodm.instance[i].hal.pin.dutyoffset;
            *hm2->periodm.instance[i].hal.pin.frequency =
                (1.0 / (hm2->periodm.period_reg[i] /
                        (double)hm2->periodm.clock_frequency)) *
                *hm2->periodm.instance[i].hal.pin.averages;
        }
    }
