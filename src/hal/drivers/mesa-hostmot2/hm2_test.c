...
MODULE_DESCRIPTION(
    "Test pattern for the hostmot2 driver, does not talk to any hardware");
...
static int
hm2_test_read(hm2_lowlevel_io_t *this, rtapi_u32 addr, void *buffer, int size)
{
...
static int hm2_test_write(hm2_lowlevel_io_t *this,
                          rtapi_u32 addr,
                          const void *buffer,
                          int size)
{
...
int rtapi_app_main(void)
{
    ...
    case 4: {
        ...
        set8(me, HM2_ADDR_CONFIGNAME + 7, '2');
        set32(me,
              HM2_ADDR_IDROM_OFFSET,
              0x400);        // put the IDROM at 0x400, where it usually lives
    ...
    case 12: {
        ...
        for (pd_index = 0; pd_index < num_io_pins; pd_index++) {
            set8(
                me,
                0x600 + (pd_index * 4) + 0,
                0); // SecPin (byte) = Which pin of secondary function connects here eg: A,B,IDX.  Output pins have bit 7 = '1'
            set8(
                me,
                0x600 + (pd_index * 4) + 1,
                0); // SecTag (byte) = Secondary function type (PWM,QCTR etc).  Same as module GTag
            set8(
                me,
                0x600 + (pd_index * 4) + 2,
                0); // SecUnit (byte) = Which secondary unit or channel connects here
            set8(
                me,
                0x600 + (pd_index * 4) + 3,
                HM2_GTAG_IOPORT); // PrimaryTag (byte) = Primary function tag (normally I/O port)
        }
