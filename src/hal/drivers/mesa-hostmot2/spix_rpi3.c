...
static int
spi0_transfer(const spix_port_t *sp, uint32_t *wptr, size_t txlen, int rw);
static int
spi1_transfer(const spix_port_t *sp, uint32_t *wptr, size_t txlen, int rw);
...
static int
    has_spi_module; // Set to non-zero when the kernel module spi_bcm2835 is loaded
static int
    driver_enabled; // Set to non-zero when rpi3_setup() is successfully called
...
static uint32_t spiclk_base =
    F_PERI; // The base clock (sys_clk) for the SPI port dividers
...
spi1_transfer(const spix_port_t *sp, uint32_t *wptr, size_t txlen, int rw)
{
    ...
    size_t tx16len =
        txlen *
        2; // There are twice as many 16-bit words as there are 32-bit words
    // cppcheck-suppress duplicateAssignExpression
    ...
    reg_wr(&aux->spi0_cntl0,
           AUX_SPI_CNTL0_SPEED(rw ? rp->clkdivr : rp->clkdivw) |
               AUX_SPI_CNTL0_ENABLE | AUX_SPI_CNTL0_MSB_OUT |
               AUX_SPI_CNTL0_IN_RISING | AUX_SPI_CNTL0_VAR_WIDTH | rp->cemask);
    reg_wr(&aux->spi0_cntl1, AUX_SPI_CNTL1_MSB_IN);

    // Send data to the fifo
    while (tx16len > 0 && pending < SPI1_FIFO_MAXDEPTH &&
           !(reg_rd(&aux->spi0_stat) & AUX_SPI_STAT_TX_FULL)) {
        if (tx16len > 1)
            reg_wr(&aux->spi0_hold,
                   ((uint32_t)*w16ptr << 8) |
                       (16 << 24)); // More data to follow
        else
            reg_wr(&aux->spi0_io,
                   ((uint32_t)*w16ptr << 8) | (16 << 24)); // Final write
        ++w16ptr;
        --tx16len;
        ++pending;
    }
    ...
    while (rx16len > 0) {
        ...
        if (tx16len > 0 && pending < SPI1_FIFO_MAXDEPTH &&
            !(stat & AUX_SPI_STAT_TX_FULL)) {
            if (tx16len > 1)
                reg_wr(&aux->spi0_hold,
                       ((uint32_t)*w16ptr << 8) |
                           (16 << 24)); // More data to follow
...
static int peripheral_map(uintptr_t membase, size_t memsize)
{
    ...
    LL_INFO(
        "Mapped peripherals from 0x%p (size 0x%08x) to gpio:0x%p, spi:0x%p, "
        "aux:0x%p\n",
        (void *)membase,
...
static uint32_t read_spiclkbase(void)
{
    ...
    if ((err = spix_read_file(sysclkref, buf, sizeof(buf))) < 0) {
        ...
        LL_INFO("No VPU clock at '%s' (errno=%d), trying core clock as "
                "alternative.\n",
                sysclkref,
                errno);
    ...
    if (err >= (int)sizeof(buf) - 1) {
        ...
        LL_ERR("Read full buffer '%s' from '%s', number probably wrong or too "
               "large, using %d Hz\n",
               buf,
               sysclkref,
               spiclk_base);
...
static int read_membase(uintptr_t *pmembase, size_t *pmemsize)
{
    ...
    *pmemsize = 0;
    ...
    if (len / sizeof(uint32_t) < 3) {
        LL_ERR("Insufficient data read from '%s' for RPi3/RPi4 IO base address "
               "and size.\n",
               DTC_SOC_RANGES);
        return -ENXIO;
    }
    ...
    *pmemsize = be32toh(buf[2]);
    ...
    if (!*pmembase) {
        ...
        if (len / sizeof(uint32_t) < 4) {
            LL_ERR(
                "Insufficient data read from '%s' for RPi4 IO base address and "
                "size.\n",
                DTC_SOC_RANGES);
            return -ENXIO;
        }
...
static int rpi3_detect(const char *dtcs[])
{
    ...
    if (spix_read_file("/proc/device-tree/model",
                       spix_driver_rpi3.model,
                       sizeof(spix_driver_rpi3.model)) < 0)
        strncpy(spix_driver_rpi3.model,
                "??? Unknown board ???",
                sizeof(spix_driver_rpi3.model) - 1);
...
static int rpi3_setup(int probemask)
{
    ...
    // detected
    if ((has_spi_module =
             (0 == shell("/usr/bin/grep -qw ^spi_bcm2835 /proc/modules")))) {
