...
int hm2_rcpwmgen_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->config.num_rcpwmgens > md->instances) {
        HM2_ERR(
            "config.num_rcpwmgens=%d, but only %d are available, not loading "
            "driver\n",
    ...
    hm2->rcpwmgen.instance = (hm2_rcpwmgen_instance_t *)hal_malloc(
        hm2->rcpwmgen.num_instances * sizeof(hm2_rcpwmgen_instance_t));
    ...
    hm2->rcpwmgen.hal = (hm2_rcpwmgen_module_global_t *)hal_malloc(
        sizeof(hm2_rcpwmgen_module_global_t));
    ...
    hm2->rcpwmgen.width_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->rcpwmgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    r = hm2_register_tram_write_region(
        ...
        (hm2->rcpwmgen.num_instances * sizeof(rtapi_u32)),
        &hm2->rcpwmgen.width_reg);
    if (r < 0) {
        HM2_ERR(
            "error registering tram write region for rcpwmgen width register "
            "(%d)\n",
            r);
        goto fail1;
    }
...

    {
        ...
        for (i = 0; i < hm2->rcpwmgen.num_instances; i++) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.rcpwmgen.%02d.width",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->rcpwmgen.instance[i].hal.pin.width),
                                  hm2->llio->comp_id);
            if (r < 0) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.rcpwmgen.%02d.scale",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->rcpwmgen.instance[i].hal.pin.scale),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.rcpwmgen.%02d.offset",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->rcpwmgen.instance[i].hal.pin.offset),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
        ...
        r = hal_pin_float_new(
            name, HAL_IN, &(hm2->rcpwmgen.hal->pin.rate), hm2->llio->comp_id);
        if (r < 0) {
            HM2_ERR("error adding pin '%s', aborting\n", name);
            goto fail1;
        }
...
void hm2_rcpwmgen_update_regs(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->rcpwmgen.num_instances; i++) {
        ...
        double width = (*hm2->rcpwmgen.instance[i].hal.pin.width) /
                           (*hm2->rcpwmgen.instance[i].hal.pin.scale) +
                       (*hm2->rcpwmgen.instance[i].hal.pin.offset);
        ...
        if ((reg + 1) > 65535) {
            *hm2->rcpwmgen.instance[i].hal.pin.width =
                65535 / (hm2->rcpwmgen.clock_frequency / (16.0 * 1000.0));
            reg = 65535;
            if (hm2->rcpwmgen.error_throttle == 0) {
...
void hm2_rcpwmgen_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              hm2->rcpwmgen.clock_frequency,
              hm2_hz_to_mhz(hm2->rcpwmgen.clock_frequency));
