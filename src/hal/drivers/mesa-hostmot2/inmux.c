...
int hm2_inmux_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->config.num_inmuxs > md->instances) {
        HM2_ERR("config.num_inmuxs=%d, but only %d are available, not loading "
                "driver\n",
    ...
    hm2->inmux.instance = (hm2_inmux_instance_t *)hal_malloc(
        hm2->inmux.num_instances * sizeof(hm2_inmux_instance_t));
    ...
    hm2->inmux.control_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->inmux.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    if (hm2->inmux.control_reg == NULL) {
        HM2_ERR("out of memory!\n");
        r = -ENOMEM;
        goto fail0;
    }
    hm2->inmux.mpg_mode_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->inmux.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->inmux.num_instances * sizeof(rtapi_u32)),
        &hm2->inmux.filt_data_reg);
    if (r < 0) {
        HM2_ERR("error registering tram read region for InMux Filtered Data "
                "register (%d)\n",
                r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->inmux.num_instances * sizeof(rtapi_u32)),
        &hm2->inmux.raw_data_reg);
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for InMux Raw Data register "
            "(%d)\n",
            r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->inmux.num_instances * sizeof(rtapi_u32)),
        &hm2->inmux.mpg_read_reg);
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for InMux MPG register (%d)\n",
            r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_write_region(
        ...
        (hm2->inmux.num_instances * sizeof(rtapi_u32)),
        &hm2->inmux.filter_reg);
    if (r < 0) {
        HM2_ERR("error registering tram write region for InMux Filter register "
                "(%d)\n",
                r);
        goto fail1;
    }
...

    {
        ...
        for (i = 0; i < hm2->inmux.num_instances; i++) {
            ...
            hm2->llio->read(hm2->llio,
                            hm2->inmux.control_addr + (i * md->instance_stride),
                            &temp,
                            sizeof(rtapi_u32));
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inmux.%02d.scan_rate",
                           hm2->llio->name,
                           i);
            r = hal_param_u32_new(name,
                                  HAL_RW,
                                  &(hm2->inmux.instance[i].hal.param.scan_rate),
                                  hm2->llio->comp_id);
            if (r < 0) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inmux.%02d.slow_scans",
                           hm2->llio->name,
                           i);
            r = hal_param_u32_new(
                name,
                HAL_RW,
                &(hm2->inmux.instance[i].hal.param.slow_scans),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inmux.%02d.fast_scans",
                           hm2->llio->name,
                           i);
            r = hal_param_u32_new(
                name,
                HAL_RW,
                &(hm2->inmux.instance[i].hal.param.fast_scans),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inmux.%02d.enc0_4xmode",
                           hm2->llio->name,
                           i);
            r = hal_param_bit_new(name,
                                  HAL_RW,
                                  &(hm2->inmux.instance[i].hal.param.enc0_mode),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inmux.%02d.enc1_4xmode",
                           hm2->llio->name,
                           i);
            r = hal_param_bit_new(name,
                                  HAL_RW,
                                  &(hm2->inmux.instance[i].hal.param.enc1_mode),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inmux.%02d.enc2_4xmode",
                           hm2->llio->name,
                           i);
            r = hal_param_bit_new(name,
                                  HAL_RW,
                                  &(hm2->inmux.instance[i].hal.param.enc2_mode),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inmux.%02d.enc3_4xmode",
                           hm2->llio->name,
                           i);
            r = hal_param_bit_new(name,
                                  HAL_RW,
                                  &(hm2->inmux.instance[i].hal.param.enc3_mode),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inmux.%02d.scan_width",
                           hm2->llio->name,
                           i);
            r = hal_param_u32_new(
                name,
                HAL_RO,
                &(hm2->inmux.instance[i].hal.param.scan_width),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
...

            {
                ...
                for (j = 0; j < hm2->inmux.instance[i].scanwidth; j++) {
                    ...
                    r = hal_pin_bit_new(
                        name,
                        HAL_OUT,
                        &(hm2->inmux.instance[i].hal.pin.filt_data[j]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                    ...
                    r = hal_pin_bit_new(
                        name,
                        HAL_OUT,
                        &(hm2->inmux.instance[i].hal.pin.raw_data[j]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                    ...
                    r = hal_pin_bit_new(
                        name,
                        HAL_OUT,
                        &(hm2->inmux.instance[i].hal.pin.filt_data_not[j]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                    ...
                    r = hal_pin_bit_new(
                        name,
                        HAL_OUT,
                        &(hm2->inmux.instance[i].hal.pin.raw_data_not[j]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                    ...
                    r = hal_pin_bit_new(
                        name,
                        HAL_IN,
                        &(hm2->inmux.instance[i].hal.pin.slow[j]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inmux.%02d.enc0-count",
                               hm2->llio->name,
                               i);
                r = hal_pin_s32_new(
                    name,
                    HAL_OUT,
                    &(hm2->inmux.instance[i].hal.pin.enc0_count),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inmux.%02d.enc1-count",
                               hm2->llio->name,
                               i);
                r = hal_pin_s32_new(
                    name,
                    HAL_OUT,
                    &(hm2->inmux.instance[i].hal.pin.enc1_count),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inmux.%02d.enc2-count",
                               hm2->llio->name,
                               i);
                r = hal_pin_s32_new(
                    name,
                    HAL_OUT,
                    &(hm2->inmux.instance[i].hal.pin.enc2_count),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inmux.%02d.enc3-count",
                               hm2->llio->name,
                               i);
                r = hal_pin_s32_new(
                    name,
                    HAL_OUT,
                    &(hm2->inmux.instance[i].hal.pin.enc3_count),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inmux.%02d.enc0-reset",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(
                    name,
                    HAL_IN,
                    &(hm2->inmux.instance[i].hal.pin.enc0_reset),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inmux.%02d.enc1-reset",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(
                    name,
                    HAL_IN,
                    &(hm2->inmux.instance[i].hal.pin.enc1_reset),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inmux.%02d.enc2-reset",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(
                    name,
                    HAL_IN,
                    &(hm2->inmux.instance[i].hal.pin.enc2_reset),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inmux.%02d.enc3-reset",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(
                    name,
                    HAL_IN,
                    &(hm2->inmux.instance[i].hal.pin.enc3_reset),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
...

    {
        ...
        for (i = 0; i < hm2->inmux.num_instances; i++) {
            ...
            hm2->llio->read(hm2->llio,
                            ...
                            &rawmpgs,
                            sizeof(rtapi_u32));
            hm2->inmux.instance[i].prev_enc0_count =
                (rtapi_s32)((rawmpgs >> 0) & 0x000000FF);
            hm2->inmux.instance[i].prev_enc1_count =
                (rtapi_s32)((rawmpgs >> 8) & 0x000000FF);
            hm2->inmux.instance[i].prev_enc2_count =
                (rtapi_s32)((rawmpgs >> 16) & 0x000000FF);
            hm2->inmux.instance[i].prev_enc3_count =
...
void hm2_inmux_force_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->inmux.num_instances; i++) {
        muxrate = hm2->inmux.instance[i].scanwidth *
                  hm2->inmux.instance[i].hal.param.scan_rate;
        if (muxrate > 5000000) {
            muxrate = 5000000;
            hm2->inmux.instance[i].hal.param.scan_rate =
    ...
    hm2->llio->write(
        hm2->llio, hm2->inmux.control_addr, hm2->inmux.control_reg, size);
    hm2->llio->write(
        hm2->llio, hm2->inmux.mpg_mode_addr, hm2->inmux.mpg_mode_reg, size);


    // Cache written-out register values.
    for (i = 0; i < hm2->inmux.num_instances; i++) {
        hm2->inmux.instance[i].written_control_reg = hm2->inmux.control_reg[i];
        hm2->inmux.instance[i].written_mpg_mode_reg =
...
void hm2_inmux_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->inmux.num_instances; i++) {
        ...
        if (hm2->inmux.instance[i].hal.param.fast_scans > 63) {
            hm2->inmux.instance[i].hal.param.fast_scans = 63;
            HM2_ERR(
                "InMux %d fastscans must be less than 63, resetting to %d \n",
                i,
                63);
        }
        if (hm2->inmux.instance[i].hal.param.slow_scans > 1023) {
            hm2->inmux.instance[i].hal.param.slow_scans = 1023;
            HM2_ERR(
                "InMux %d slowscans  must be less than 1023, resetting to %d "
                "\n",
                i,
                1023);
        }
        if (hm2->inmux.instance[i].hal.param.fast_scans < 1) {
            ...
            HM2_ERR(
                "InMux %d fastscans must be greater than 0, resetting to %d \n",
                i,
                1);
        }
        if (hm2->inmux.instance[i].hal.param.slow_scans < 1) {
            ...
            HM2_ERR(
                "InMux %d slowscans must be greater than 0, resetting to %d \n",
                i,
                1);
        }
        ...
        hm2->inmux.control_reg[i] =
            ...
            (hm2->inmux.instance[i].hal.param.fast_scans << 16) +
            (hm2->inmux.instance[i].hal.param.slow_scans << 22);
        if (hm2->inmux.control_reg[i] !=
            hm2->inmux.instance[i].written_control_reg) {
            hm2->llio->write(hm2->llio,
                             hm2->inmux.control_addr,
                             hm2->inmux.control_reg,
                             size);
            hm2->inmux.instance[i].written_control_reg =
                hm2->inmux.control_reg[i];
        ...
        for (unsigned j = 0; j < hm2->inmux.instance[i].scanwidth; j++) {
            hm2->inmux.filter_reg[i] |=
                (*hm2->inmux.instance[i].hal.pin.slow[j] << j);
        }
        if (hm2->inmux.filter_reg[i] !=
            hm2->inmux.instance[i].written_filter_reg) {
            hm2->llio->write(
                hm2->llio, hm2->inmux.filter_addr, hm2->inmux.filter_reg, size);
            hm2->inmux.instance[i].written_filter_reg =
                hm2->inmux.filter_reg[i];
        ...
        if (hm2->inmux.mpg_mode_reg[i] !=
            hm2->inmux.instance[i].written_mpg_mode_reg) {
            hm2->llio->write(hm2->llio,
                             hm2->inmux.mpg_mode_addr,
                             hm2->inmux.mpg_mode_reg,
                             size);
            hm2->inmux.instance[i].written_mpg_mode_reg =
                hm2->inmux.mpg_mode_reg[i];
...
void hm2_inmux_process_tram_read(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->inmux.num_instances; i++) {
        for (unsigned j = 0; j < hm2->inmux.instance[i].scanwidth; j++) {
            *hm2->inmux.instance[i].hal.pin.filt_data[j] =
                (hm2->inmux.filt_data_reg[i] >> j) & 1;
            *hm2->inmux.instance[i].hal.pin.raw_data[j] =
                (hm2->inmux.raw_data_reg[i] >> j) & 1;
            *hm2->inmux.instance[i].hal.pin.filt_data_not[j] =
                !((hm2->inmux.filt_data_reg[i] >> j) & 1);
            *hm2->inmux.instance[i].hal.pin.raw_data_not[j] =
                !((hm2->inmux.raw_data_reg[i] >> j) & 1);
        ...
        count_diff =
            (rtapi_s32)raw_count - hm2->inmux.instance[i].prev_enc0_count;
        hm2->inmux.instance[i].prev_enc0_count =
            hm2->inmux.instance[i].prev_enc0_count + count_diff;
        if (count_diff > 128)
            count_diff -= 256;
        if (count_diff < -128)
            count_diff += 256;
        if (*hm2->inmux.instance[i].hal.pin.enc0_reset == 0) {
            *hm2->inmux.instance[i].hal.pin.enc0_count =
                *hm2->inmux.instance[i].hal.pin.enc0_count + count_diff;
        } else {
            *hm2->inmux.instance[i].hal.pin.enc0_count = 0;
        }
        raw_count = (hm2->inmux.mpg_read_reg[i] >> 8) & 0x000000FF;
        count_diff =
            (rtapi_s32)raw_count - hm2->inmux.instance[i].prev_enc1_count;
        hm2->inmux.instance[i].prev_enc1_count =
            hm2->inmux.instance[i].prev_enc1_count + count_diff;
        if (count_diff > 128)
            count_diff -= 256;
        if (count_diff < -128)
            count_diff += 256;
        if (*hm2->inmux.instance[i].hal.pin.enc1_reset == 0) {
            *hm2->inmux.instance[i].hal.pin.enc1_count =
                *hm2->inmux.instance[i].hal.pin.enc1_count + count_diff;
        } else {
            *hm2->inmux.instance[i].hal.pin.enc1_count = 0;
        }
        raw_count = (hm2->inmux.mpg_read_reg[i] >> 16) & 0x000000FF;
        count_diff =
            (rtapi_s32)raw_count - hm2->inmux.instance[i].prev_enc2_count;
        hm2->inmux.instance[i].prev_enc2_count =
            hm2->inmux.instance[i].prev_enc2_count + count_diff;
        if (count_diff > 128)
            count_diff -= 256;
        if (count_diff < -128)
            count_diff += 256;
        if (*hm2->inmux.instance[i].hal.pin.enc2_reset == 0) {
            *hm2->inmux.instance[i].hal.pin.enc2_count =
                *hm2->inmux.instance[i].hal.pin.enc2_count + count_diff;
        } else {
            *hm2->inmux.instance[i].hal.pin.enc2_count = 0;
        }
        raw_count = (hm2->inmux.mpg_read_reg[i] >> 24) & 0x000000FF;
        count_diff =
            (rtapi_s32)raw_count - hm2->inmux.instance[i].prev_enc3_count;
        hm2->inmux.instance[i].prev_enc3_count =
            hm2->inmux.instance[i].prev_enc3_count + count_diff;
        if (count_diff > 128)
            count_diff -= 256;
        if (count_diff < -128)
            count_diff += 256;
        if (*hm2->inmux.instance[i].hal.pin.enc3_reset == 0) {
            *hm2->inmux.instance[i].hal.pin.enc3_count =
                *hm2->inmux.instance[i].hal.pin.enc3_count + count_diff;
        } else {
...
void hm2_inmux_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              ...
              hm2_hz_to_mhz(hm2->inmux.clock_frequency));
    HM2_PRINT("    version: %d\n", hm2->inmux.version);
