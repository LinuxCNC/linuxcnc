...
MODULE_DESCRIPTION(
    "Driver for HostMot2 devices connected via SPI to RaspberryPi");
MODULE_SUPPORTED_DEVICE("Mesa-AnythingIO-7i90");

#define RPSPI_MAX_BOARDS 5
#define RPSPI_MAX_MSG                                                          \
    (127 +                                                                     \
     1) // The 7i90 docs say that the max. burstlen == 127 words (i.e. cmd+message <= 1+127)
...
static uint32_t *peripheralmem =
    (uint32_t *)MAP_FAILED;   // mmap'ed peripheral memory
static size_t peripheralsize; // Size of the mmap'ed block
static bcm2835_gpio_t
    *gpio;                 // GPIO peripheral structure in mmap'ed address space
...
RTAPI_MP_INT(spiclk_rate,
             "SPI clock rate in kHz (default 31250 kHz, slowest 3 kHz)");
RTAPI_MP_INT(spiclk_rate_rd,
             "SPI clock rate for reading in kHz (default same as spiclk_rate)");
...
RTAPI_MP_INT(
    spi_pull_miso,
    "Enable/disable pull-{up,down} on SPI MISO (default pulldown, 0=off, "
    "1=pulldown, 2=pullup)");
RTAPI_MP_INT(
    spi_pull_mosi,
    "Enable/disable pull-{up,down} on SPI MOSI (default pulldown, 0=off, "
    "1=pulldown, 2=pullup)");
RTAPI_MP_INT(
    spi_pull_sclk,
    "Enable/disable pull-{up,down} on SPI SCLK (default pulldown, 0=off, "
    "1=pulldown, 2=pullup)");
...
RTAPI_MP_INT(
    spi_probe,
    "Bit-field to select which SPI/CE combinations to probe (default 1 "
    "(SPI0/CE0))");
...
{
    return (addr << 16) | CMD_7I90_READ | (aib ? CMD_7I90_ADDRINC : 0) |
           (msglen << 4);
}

static inline uint32_t mk_write_cmd(uint32_t addr, uint32_t msglen, bool aib)
{
    return (addr << 16) | CMD_7I90_WRITE | (aib ? CMD_7I90_ADDRINC : 0) |
           (msglen << 4);
}
...
static inline void spi1_xfer_setup(hm2_rpspi_t *hm2, bool rd)
{
    ...
    reg_wr(&aux->spi0_cntl0,
           AUX_SPI_CNTL0_SPEED(spi1_clkdiv_calc(
               hm2->spiclkbase, rd ? hm2->spiclkrater : hm2->spiclkratew)) |
               AUX_SPI_CNTL0_ENABLE | AUX_SPI_CNTL0_MSB_OUT |
               AUX_SPI_CNTL0_OUT_RISING | AUX_SPI_CNTL0_SHIFT_LENGTH(16) |
               hm2->spice);
 ...
 */
static int hm2_rpspi_write_spi1(hm2_lowlevel_io_t *llio,
                                uint32_t addr,
                                const void *buffer,
                                int size)
{
    ...
    while (txlen) {
        ...
        if (txlen && !(stat & AUX_SPI_STAT_TX_FULL)) {
            reg_wr(&aux->spi0_hold,
                   (uint32_t)htobe16(*bptr++) << 15); // More data to follow
            --txlen;
        }
    }
    ...
    while (rxlen) {
        ...
        if (txlen && !(stat & AUX_SPI_STAT_TX_FULL)) {
            ...
            if (--txlen)
                reg_wr(&aux->spi0_hold,
                       (uint32_t)htobe16(*bptr++)
                           << 15); // Write while still more data to follow
            else
                reg_wr(&aux->spi0_io,
                       (uint32_t)htobe16(*bptr++) << 15); // Final write
        }
 ...
 */
static int hm2_rpspi_read_spi1(hm2_lowlevel_io_t *llio,
                               uint32_t addr,
                               void *buffer,
                               int size)
{
    ...
    while (txlen) {
        ...
        if (txlen && !(stat & AUX_SPI_STAT_TX_FULL)) {
            reg_wr(&aux->spi0_hold,
                   (uint32_t)be16toh(*bptr++) << 15); // More data to follow
            --txlen;
        }
    }
    ...
    while (rxlen) {
        ...
        if (txlen && !(stat & AUX_SPI_STAT_TX_FULL)) {
            if (--txlen)
                reg_wr(&aux->spi0_hold,
                       0); // Write while still more data to follow
            else
                reg_wr(&aux->spi0_io,
                       0); // Final write (maybe reached on very short reads)
        }
...
static inline int32_t spi0_clkdiv_calc(uint32_t base, uint32_t rate)
{
    ...
    if (clkdiv > 65534)
        ...
        clkdiv +=
            clkdiv & 1; // Must be multiple of 2 (round to lower frequency)
    return clkdiv;
}
...
static inline void spi0_xfer_setup(hm2_rpspi_t *hm2, bool rd)
{
    ...
    cs &= ~(SPI_CS_CS_10 | SPI_CS_CS_01 |
            SPI_CS_REN);          // Reset CE and disable 3-wire mode
    cs |= hm2->spice | SPI_CS_TA; // Set proper CE_x and activate transfer
    reg_wr(&spi->clk,
           spi0_clkdiv_calc(hm2->spiclkbase,
                            rd ? hm2->spiclkrater
                               : hm2->spiclkratew)); // Set clock divider
    reg_wr(&spi->cs, cs);                            // Go!
}
 ...
 */
static int hm2_rpspi_write_spi0(hm2_lowlevel_io_t *llio,
                                uint32_t addr,
                                const void *buffer,
                                int size)
{
 ...
 */
static int hm2_rpspi_read_spi0(hm2_lowlevel_io_t *llio,
                               uint32_t addr,
                               void *buffer,
                               int size)
{
...
static inline unsigned count_ones(uint32_t val)
{
    ...
    static const unsigned nibble_table[16] = {
...
static int32_t check_cookie(hm2_rpspi_t *board)
{
    ...
    if ((!co && spi_pull_miso == SPI_PULL_DOWN) ||
        (ca == 0xffffffff && spi_pull_miso == SPI_PULL_UP)) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: SPI%d/CE%d No drive seen on MISO line (kept at pull-%s "
            "level)."
            " No board connected or bad connection?\n",
            board->spidevid,
            board->spiceid,
            spi_pull_miso == SPI_PULL_DOWN ? "down" : "up");
        ...
        if (!ones) {
            ...
            rtapi_print_msg(
                RPSPI_ERR,
                "hm2_rpspi: SPI%d/CE%d MISO input does not match any expected "
                "bit-pattern (>= %u bit difference)."
                " Maybe SPI read clock frequency too high or noise on the "
                "input?\n",
                board->spidevid,
                board->spiceid,
                ones);
...
/*************************************************/
#define RPSPI_SYS_CLKVPU                                                       \
    "/sys/kernel/debug/clk/vpu/clk_rate" // Newer kernels (4.8+, I think) have detailed info
#define RPSPI_SYS_CLKCORE                                                      \
    "/sys/kernel/debug/clk/core/clk_rate" // Older kernels only have core-clock info
...
static uint32_t read_spiclkbase(void)
{
    ...
    if ((fd = rtapi_open_as_root(sysclkref, O_RDONLY)) < 0) {
        ...
        rtapi_print_msg(
            RPSPI_INFO,
            "hm2_rpspi: No VPU clock at '%s' (errno=%d), trying core clock as "
            "alternative.\n",
            sysclkref,
            errno);
        ...
        if ((fd = rtapi_open_as_root(sysclkref, O_RDONLY)) < 0) {
            ...
            rtapi_print_msg(
                RPSPI_ERR,
                "hm2_rpspi: Cannot open clock setting '%s' (errno=%d), using "
                "%d Hz\n",
                sysclkref,
                errno,
                spiclk_base);
    ...
    if ((err = read(fd, buf, sizeof(buf) - 1)) < 0) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: Cannot read clock setting '%s' (errno=%d), using %d "
            "Hz\n",
            sysclkref,
            errno,
            spiclk_base);
    ...
    if (err >= (int)sizeof(buf) - 1) {
        ...
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: Read full buffer '%s' from '%s', number probably wrong "
            "or too large, using %d Hz\n",
            buf,
            sysclkref,
            spiclk_base);
        return spiclk_base;
    ...
    if (1 != sscanf(buf, "%u", &rate)) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: Cannot interpret clock setting '%s' from '%s', using "
            "%d Hz\n",
            buf,
            sysclkref,
            spiclk_base);
...
static int probe_board(hm2_rpspi_t *board)
{
    ...
    rtapi_print_msg(RPSPI_INFO,
                    "hm2_rpspi: SPI%d/CE%d Valid cookie matched\n",
                    board->spidevid,
                    board->spiceid);
    ...
    // the cookie check.
    if (board->llio.read(
            &board->llio, (uint32_t)ret, &idrom, sizeof(hm2_idrom_t)) <= 0) {
        rtapi_print_msg(RPSPI_ERR,
                        "hm2_rpspi: SPI%d/CE%d Board ident read failed\n",
                        board->spidevid,
                        board->spiceid);
    ...
    rtapi_print_msg(RPSPI_INFO,
                    "hm2_rpspi: SPI%d/CE%d Base: %s.%d\n",
                    board->spidevid,
                    board->spiceid,
                    base,
                    board->nr);
    rtapi_snprintf(
        board->llio.name, sizeof(board->llio.name), "%s.%d", base, board->nr);
...
static int peripheral_map(uint32_t membase, uint32_t memsize)
{
    ...
    if (peripheralmem == MAP_FAILED) {
        rtapi_print_msg(
            RPSPI_ERR, "hm2_rpspi: Can't map peripherals: %s\n", strerror(err));
        if (err == EPERM) {
            rtapi_print_msg(
                RPSPI_ERR,
                "hm2_rpspi: Try adding 'iomem=relaxed' to your kernel "
                "command-line.\n");
        }
        return -err;
    }
    ...
    gpio = (bcm2835_gpio_t *)(peripheralmem +
                              (BCM2835_GPIO_OFFSET / sizeof(*peripheralmem)));
    spi = (bcm2835_spi_t *)(peripheralmem +
                            (BCM2835_SPI_OFFSET / sizeof(*peripheralmem)));
    aux = (bcm2835_aux_t *)(peripheralmem +
                            (BCM2835_AUX_OFFSET / sizeof(*peripheralmem)));

    rtapi_print_msg(
        RPSPI_INFO,
        "hm2_rpspi: Mapped peripherals from 0x%08x (size 0x%08x) to gpio:0x%p, "
        "spi:0x%p, aux:0x%p\n",
        membase,
...
static void waste_150_cycles(void)
{
    ...
    for (i = 0; i < 40; i++)
        x = reg_rd(
            &gpio->gplev0); // Just read the pin register, nothing interesting to do here
}
...
static inline void gpio_fsel(uint32_t pin, uint32_t func)
{
    if (pin <= 53) { /* There are 54 GPIOs */
        uint32_t bits =
            pin * 3; /* Three bits per fsel field and 10 gpio per uint32_t */
        reg_wr(&gpio->gpfsel[bits / 30],
               (reg_rd(&gpio->gpfsel[bits / 30]) & ~(7 << (bits % 30))) |
                   ((func & 7) << (bits % 30)));
    }
}
...
static inline void gpio_pull(unsigned pin, uint32_t pud)
{
    ...
    reg_wr(&gpio->gppudclk0,
           0); // We are not sure about the previous state, make sure
...
static void peripheral_setup(void)
{
    ...
    if (spi_probe & SPI1_PROBE_MASK) {
        ...
        if (!(aux_enables & AUX_ENABLES_SPI1))
            reg_wr(&aux->enables,
                   aux_enables | AUX_ENABLES_SPI1); // Enable SPI1
...
static uint8_t *read_file(const char *fname, size_t maxsize, size_t minsize)
{
    ...
    if ((size_t)sb.st_size < minsize) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: Target file '%s' stat's less than minimum size of %zu "
            "bytes (st_size=%zu)\n",
            fname,
            minsize,
            (size_t)sb.st_size);
    ...
    if (!(fp = fopen(fname, "r"))) {
        rtapi_print_msg(
            RPSPI_ERR, "hm2_rpspi: Cannot open '%s' for read\n", fname);
        rtapi_kfree(buf);
        return NULL;
    }
    ...
    if (!fsize) {
        rtapi_print_msg(RPSPI_ERR,
                        "hm2_rpspi: Nothing read from '%s' (errno=%d)\n",
                        fname,
                        errno);
        rtapi_kfree(buf);
        return NULL;
    ...
    if (fsize < nn) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: Short read from '%s'; read=%zu required>=%zu\n",
            fname,
            fsize,
            nn);
        rtapi_kfree(buf);
...
static int hm2_rpspi_setup(void)
{
    ...
    rtapi_print_msg(RPSPI_INFO,
                    "hm2_rpspi: Platform: %s\n",
                    *buf ? (const char *)buf
                         : "<no data from /proc/device-tree/model>");
    rtapi_kfree(buf);
    ...
    if (!(buf = read_file("/proc/device-tree/soc/ranges", 4095, 12))) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: Cannot determine IO base address and size.\n");
        return -1;
    }

    pmembase =
        be32toh(((uint32_t *)buf)[1]); // This should do the trick for RPi3
    ...
    if (!pmembase || !pmemsize) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: IO base address (0x%08x) or size (0x%08x) are zero.\n",
            pmembase,
            pmemsize);
        return -1;
    }
    rtapi_print_msg(RPSPI_INFO,
                    "hm2_rpspi: Base address 0x%08x size 0x%08x\n",
                    pmembase,
                    pmemsize);
    ...
    if (spiclk_rate < 30 || spiclk_rate > 63000) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: SPI clock rate '%d' too slow/fast. Must be >= 30 kHz "
            "and <= 63000 kHz\n",
            spiclk_rate);
        return -EINVAL;
    }
    ...
    if (spiclk_rate_rd < 30 || spiclk_rate_rd > 63000) {
        rtapi_print_msg(
            RPSPI_ERR,
            "hm2_rpspi: SPI clock rate for reading '%d' too slow/fast. Must be "
            ">= 30 kHz and <= 63000 kHz\n",
            spiclk_rate_rd);
        return -EINVAL;
    }
    ...
    for (j = i = 0; i < RPSPI_MAX_BOARDS; i++) {
        ...
        rtapi_print_msg(
            RPSPI_INFO,
            "hm2_rpspi: SPI%d/CE%d clock rate: %d/%d Hz, VPU clock rate: %u "
            "Hz\n",
            iddev,
            idce,
            clkratew,
        ...
        if (!iddev) {
            ...
            rtapi_print_msg(
                RPSPI_INFO,
                "hm2_rpspi: SPI%d/CE%d write clock rate calculated: %d Hz "
                "(clkdiv=%u)\n",
                iddev,
                idce,
                clkbase / clkdiv,
            ...
            rtapi_print_msg(
                RPSPI_INFO,
                "hm2_rpspi: SPI%d/CE%d read clock rate calculated: %d Hz "
                "(clkdiv=%u)\n",
                iddev,
                idce,
                clkbase / clkdiv,
        ...
        if ((retval = hm2_register(&boards[j].llio, config[j])) < 0) {
            rtapi_print_msg(
                RPSPI_ERR,
                "hm2_rpspi: hm2_register() failed for SPI%d/CE%d.\n",
                iddev,
                idce);
            return retval;
        }
