...
RTAPI_MP_ARRAY_INT(spiclk_rate,
                   SPIX_MAX_BOARDS,
                   "SPI clock rates in kHz (default 25000 kHz)");
RTAPI_MP_ARRAY_INT(
    spiclk_rate_rd,
    SPIX_MAX_BOARDS,
    "SPI clock rates for reading in kHz (default same as spiclk_rate)");
...
RTAPI_MP_STRING(
    force_driver,
    "Force one specific hardware driver (default empty, auto detecting "
    "hardware))");
...
RTAPI_MP_INT(
    spi_noqueue,
    "Disable queued SPI requests, use for debugging only (default 0 (off))");
...
RTAPI_MP_ARRAY_STRING(
    spidev_path,
    SPIX_MAX_BOARDS,
    "The device node path override(s) for the spidev driver (default "
    "/dev/spidev{0.[01],1.[012]})");
 ...
 */
static int hm2_spix_write(hm2_lowlevel_io_t *llio,
                          rtapi_u32 addr,
                          const void *buffer,
                          int size)
{
 ...
 */
static int
hm2_spix_read(hm2_lowlevel_io_t *llio, rtapi_u32 addr, void *buffer, int size)
{
    spix_board_t *brd = (spix_board_t *)llio;
 ...
 */
static int hm2_spix_queue_read(hm2_lowlevel_io_t *llio,
                               rtapi_u32 addr,
                               void *buffer,
                               int size)
{
    ...
    memset(&rbptr[brd->rbuf.n + 1],
           0,
           rxlen * sizeof(uint32_t)); // Fill zeros as data
    brd->rbuf.n += rxlen + 1;
...
static int hm2_spix_send_queued_reads(hm2_lowlevel_io_t *llio)
{
    ...
    if (rv > 0) {
        ...
        for (i = 0; i < brd->rref.n; i++) {
            ...
            memcpy(rxref->ptr,
                   &((uint32_t *)brd->rbuf.ptr)[rxref->idx],
                   rxref->size);
        }
 ...
 */
static int hm2_spix_queue_write(hm2_lowlevel_io_t *llio,
                                rtapi_u32 addr,
                                const void *buffer,
                                int size)
{
    ...
    memcpy(
        &wbptr[brd->wbuf.n + 1], buffer, txlen * sizeof(uint32_t)); // The data
    brd->wbuf.n += txlen + 1;
    return 1;
}
...
static int32_t check_cookie(spix_board_t *board)
{
    ...
    if (!board->llio.read(
            &board->llio, HM2_ADDR_IOCOOKIE, cookie, sizeof(cookie)))
        return -ENODEV;
    ...
    if ((!co && port->miso_pull == SPIX_PULL_DOWN) ||
        (ca == 0xffffffff && port->miso_pull == SPIX_PULL_UP)) {
        LL_ERR("%s: No drive seen on MISO line (kept at pull-%s level)."
               " No board connected or bad connection?\n",
               port->name,
               port->miso_pull == SPIX_PULL_DOWN ? "down" : "up");
        ...
        if (!ones) {
            ...
            LL_ERR(
                "%s: MISO input does not match any expected bit-pattern (>= %u "
                "bit difference)."
...
static int probe_board(spix_board_t *board)
{
    ...
    // the cookie check.
    if (!board->llio.read(
            &board->llio, (uint32_t)ret, &idrom, sizeof(hm2_idrom_t))) {
...
static int spix_setup(void)
{
    ...
    if (spi_pull_miso != -1 || spi_pull_mosi != -1 || spi_pull_sclk != -1) {
        LL_WARN(
            "Setting spi_pull_{miso,mosi,sclk} has no effect in the hm2_spix "
            "driver.\n");
    }

    if (spi_probe > (1 << SPIX_MAX_BOARDS) - 1) {
        LL_WARN("Probing with spi_probe must not have flags larger than %d "
                "included; truncating.\n",
                1 << (SPIX_MAX_BOARDS - 1));
    ...
    if (force_driver) {
        ...
        if (i >= NELEM(drivers)) {
            LL_ERR("Unsupported hardware driver '%s' passed to force_driver "
                   "option\n",
                   force_driver);
            return -ENODEV;
        }
    ...
    int i, j;
    if ((i = hwdriver->setup(spi_probe)) <
        0) { // Let the hardware driver do its thing
