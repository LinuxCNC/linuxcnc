...
def add_deps(fname):
	...
	# what file provides that entity?
...
def usage ():
    print "\nUsage: spec2bit foo.spec\n"
    print "  Reads foo.spec, which is a HostMot2 FPGA spec, and executes"
    print "  the entire process to build foo.bit, a bitfile which can be"
    print "  loaded into an FPGA to implement the spec.\n"
...
prefix = os.path.splitext(spec_fname)[0]
...
if os.path.exists(spec_fname):
    ...
    print "spec file '%s' not found" % spec_fname
    sys.exit(1)

# make sure it provided the requred fields
retval = 0
names = ["template", "device", "constraints", "module_id", "pin_desc"]
for v in names:
    if not vars().has_key(v):
        print "Spec file missing '%s'" % v
	retval = 1
if retval == 1:
    sys.exit(2)
...
# spec file seems legit, remove old generated files
print "removing old generated files"
generated = [".vhd", ".prj", ".scr", ".ngc", ".log.syn", ".ngd", ".log.ngd",
             ".nad", ".nbd", ".ncd", ".bit" ]
...
try:
    ...
    time.sleep(1)
    print "Synthesis interrupted by user"
    retcode = -1

if not retcode == 0:
    print "Synthesis Failed - see above for error messages"
    sys.exit(1)
