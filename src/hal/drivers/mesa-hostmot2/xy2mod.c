...
void hm2_xy2mod_process_tram_read(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->xy2mod.num_instances; i++) {
        ...
        if (fabs(*s->hal.pin.posx_scale) < 1e-6) {
            if (*s->hal.pin.posx_scale >= 0.0) {
                *s->hal.pin.posx_scale = 1.0;
                HM2_ERR(
                    "xy2mod %d position_scalex is too close to 0, resetting to "
                    "1.0\n",
                    i);
            } else {
                *s->hal.pin.posx_scale = -1.0;
                HM2_ERR(
                    "xy2mod %d position_scalxe is too close to 0, resetting to "
                    "-1.0\n",
                    i);
            }
        }
        ...
        if (fabs(*s->hal.pin.posy_scale) < 1e-6) {
            if (*s->hal.pin.posy_scale >= 0.0) {
                *s->hal.pin.posy_scale = 1.0;
                HM2_ERR(
                    "xy2mod %d position_scaley is too close to 0, resetting to "
                    "1.0\n",
                    i);
            } else {
                *s->hal.pin.posy_scale = -1.0;
        ...
        *(s->hal.pin.posx_fb) =
            ((double)(int32_t)(posx) / 2147483647.0) / *s->hal.pin.posx_scale;
        *(s->hal.pin.posy_fb) =
            ((double)(int32_t)(posy) / 2147483647.0) / *s->hal.pin.posy_scale;

        // need to "unscale" the velocity registers to return to machine units

        *(s->hal.pin.velx_fb) =
            ((double)(int32_t)velx) /
            (*s->hal.pin.posx_scale *
             (256 * 2147483647.0 / (double)hm2->xy2mod.clock_frequency));
        *(s->hal.pin.vely_fb) =
            ((double)(int32_t)vely) /
            (*s->hal.pin.posy_scale *
             (256 * 2147483647.0 / (double)hm2->xy2mod.clock_frequency));
...
static void hm2_xy2mod_instance_write(hostmot2_t *hm2, int i)
{
    ...
    if (*s->hal.pin.enable != 0) {
        // position setting
        stepsx_cmd =
            (*s->hal.pin.posx_cmd * 2147483647.0) / *s->hal.pin.posx_scale;
        stepsy_cmd =
            (*s->hal.pin.posy_cmd * 2147483647.0) / *s->hal.pin.posy_scale;
        ...
        if (*s->hal.pin.velx_cmd != s->prev_velx_cmd) {
            lvelx_reg =
                (uint32_t)(int32_t)(steps_per_secx_cmd *
                                    (256 * 2147483648.0 /
                                     (double)hm2->xy2mod.clock_frequency));
            hm2->xy2mod.velx_reg[i] = lvelx_reg;
            //      HM2_PRINT("VELX register set to:0x%08X\n",lvelx_reg);
            hm2->llio->write(hm2->llio,
                             hm2->xy2mod.velx_addr + istride,
        ...
        if (*s->hal.pin.accx_cmd != s->prev_accx_cmd) {
            laccx_reg =
                (uint32_t)(int32_t)(steps_per_sec2x_cmd *
                                    (double)(4096.0 * 4294967296.0 * 32768.0) /
                                    ((double)(hm2->xy2mod.clock_frequency *
                                              (double)
                                                  hm2->xy2mod.clock_frequency *
                                              256)));
            hm2->xy2mod.accx_reg[i] = laccx_reg;
            //    HM2_PRINT("ACCX register set to:0x%08X\n",laccx_reg);
            hm2->llio->write(hm2->llio,
                             hm2->xy2mod.accx_addr + istride,
        ...
        if (*s->hal.pin.accy_cmd != s->prev_accy_cmd) {
            laccy_reg =
                (uint32_t)(int32_t)(steps_per_sec2y_cmd *
                                    (double)(4096.0 * 4294967296.0 * 32768.0) /
                                    ((double)(hm2->xy2mod.clock_frequency *
                                              (double)
                                                  hm2->xy2mod.clock_frequency *
        ...
        *s->hal.pin.controly = (*s->hal.pin.controly & 7);
        ...
        hm2->xy2mod.mode_reg[i] =
            *s->hal.pin.controlx << 0 | *s->hal.pin.controly << 3 |
            *s->hal.pin.mode18bitx << 10 | *s->hal.pin.mode18bity << 11 |
            *s->hal.pin.commandmodex << 12 | *s->hal.pin.commandmodey << 13;

        hm2->llio->write(hm2->llio,
                         hm2->xy2mod.mode_addr + istride,
                         &hm2->xy2mod.mode_reg[i],
                         sizeof(rtapi_u32));
        ...
        *s->hal.pin.commandy = (*s->hal.pin.commandy & 0xFFFF);
        ...
        hm2->xy2mod.command_reg[i] =
            *s->hal.pin.commandx << 0 | *s->hal.pin.commandy << 16;

        hm2->llio->write(hm2->llio,
                         hm2->xy2mod.command_addr + istride,
                         &hm2->xy2mod.command_reg[i],
                         sizeof(rtapi_u32));
        ...
        *s->hal.pin.controly = 1;
        hm2->xy2mod.mode_reg[i] =
            0x000003C9; // reset X and Y overflow bits when disabled
        hm2->llio->write(hm2->llio,
                         hm2->xy2mod.mode_addr + istride,
                         &hm2->xy2mod.mode_reg[i],
                         sizeof(rtapi_u32));
...
static void hm2_xy2mod_set_dpll_rtimer(hostmot2_t *hm2)
{
    ...
    if ((*hm2->xy2mod.hal->pin.dpll_rtimer_num < -1) ||
        (*hm2->xy2mod.hal->pin.dpll_rtimer_num > 4)) {
    ...
    hm2->llio->write(
        hm2->llio, hm2->xy2mod.dpll_rtimer_num_addr, &data, sizeof(rtapi_u32));
...
static void hm2_xy2mod_set_dpll_wtimer(hostmot2_t *hm2)
{
    ...
    if ((*hm2->xy2mod.hal->pin.dpll_wtimer_num < -1) ||
        (*hm2->xy2mod.hal->pin.dpll_wtimer_num > 4)) {
    ...
    hm2->llio->write(
        hm2->llio, hm2->xy2mod.dpll_wtimer_num_addr, &data, sizeof(rtapi_u32));
...
void hm2_xy2mod_write(hostmot2_t *hm2)
{
    ...
    if (hm2->xy2mod.num_instances > 0 && hm2->dpll_module_present) {
        if (*hm2->xy2mod.hal->pin.dpll_rtimer_num !=
            (int)hm2->xy2mod.written_dpll_rtimer_num) {
            hm2_xy2mod_set_dpll_rtimer(hm2);
        }
        if (*hm2->xy2mod.hal->pin.dpll_wtimer_num !=
            (int)hm2->xy2mod.written_dpll_wtimer_num) {
...
void hm2_xy2mod_allocate_pins(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        if ((hm2->pin[i].sec_tag != HM2_GTAG_XY2MOD) ||
...
int hm2_xy2mod_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->config.num_xy2mods > md->instances) {
        HM2_ERR("config.num_xy2mods=%d, but only %d are available, not loading "
                "driver\n",
    ...
    hm2->xy2mod.hal = (hm2_xy2mod_module_global_t *)hal_malloc(
        sizeof(hm2_xy2mod_module_global_t));
    ...
    hm2->xy2mod.instance = (hm2_xy2mod_instance_t *)hal_malloc(
        hm2->xy2mod.num_instances * sizeof(hm2_xy2mod_instance_t));
    ...
    hm2->xy2mod.dpll_rtimer_num_addr =
        md->base_address + (9 * md->register_stride);
    hm2->xy2mod.dpll_wtimer_num_addr =
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->xy2mod.num_instances * sizeof(rtapi_u32)),
        &hm2->xy2mod.posx_reg);
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for xy2mod X Position register "
            "(%d)\n",
            r);
        goto fail0;
    }
    ...
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for xy2mod Y Position register "
            "(%d)\n",
            r);
        goto fail0;
    }
    ...
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for xy2mod X Velocity register "
            "(%d)\n",
            r);
        goto fail0;
    }
    ...
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for xy2mod Y Velocity register "
            "(%d)\n",
            r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        &hm2->xy2mod.accx_reg);
    if (r < 0) {
        HM2_ERR("error registering tram read region for xy2mod X Acceleration "
                "register (%d)\n",
                r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        &hm2->xy2mod.accy_reg);
    if (r < 0) {
        HM2_ERR("error registering tram read region for xy2mod Y Acceleration "
                "register (%d)\n",
                r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        &hm2->xy2mod.mode_reg);
    if (r < 0) {
        HM2_ERR("error registering tram read region for xy2mod mode register "
                "(%d)\n",
                r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->xy2mod.num_instances * sizeof(rtapi_u32)),
        &hm2->xy2mod.status_reg);
    if (r < 0) {
        HM2_ERR("error registering tram read region for xy2mod status register "
                "(%d)\n",
                r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->xy2mod.num_instances * sizeof(rtapi_u32)),
        &hm2->xy2mod.command_reg);
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for xy2mod command register "
            "(%d)\n",
            r);
        goto fail0;
    }

    hm2->xy2mod.command_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->xy2mod.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
...

    {
        ...
        if (hm2->dpll_module_present) {
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.read-timer-number",
                           hm2->llio->name);
            r = hal_pin_s32_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.hal->pin.dpll_rtimer_num),
                                hm2->llio->comp_id);
            if (r < 0) {
        ...
        if (hm2->dpll_module_present) {
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.write-timer-number",
                           hm2->llio->name);
            r = hal_pin_s32_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.hal->pin.dpll_wtimer_num),
                                hm2->llio->comp_id);
            if (r < 0) {
        ...
        for (i = 0; i < hm2->xy2mod.num_instances; i++) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.posx-cmd",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->xy2mod.instance[i].hal.pin.posx_cmd),
                                  hm2->llio->comp_id);
            if (r < 0) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.posy-cmd",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->xy2mod.instance[i].hal.pin.posy_cmd),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.velx-cmd",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->xy2mod.instance[i].hal.pin.velx_cmd),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.vely-cmd",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->xy2mod.instance[i].hal.pin.vely_cmd),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.accx-cmd",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->xy2mod.instance[i].hal.pin.accx_cmd),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.accy-cmd",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->xy2mod.instance[i].hal.pin.accy_cmd),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.velx-fb",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_OUT,
                                  &(hm2->xy2mod.instance[i].hal.pin.velx_fb),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.vely-fb",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_OUT,
                                  &(hm2->xy2mod.instance[i].hal.pin.vely_fb),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.posx-fb",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_OUT,
                                  &(hm2->xy2mod.instance[i].hal.pin.posx_fb),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.posy-fb",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_OUT,
                                  &(hm2->xy2mod.instance[i].hal.pin.posy_fb),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.posx-scale",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->xy2mod.instance[i].hal.pin.posx_scale),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.posy-scale",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->xy2mod.instance[i].hal.pin.posy_scale),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.controlx",
                           hm2->llio->name,
                           i);
            r = hal_pin_u32_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.instance[i].hal.pin.controlx),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.controly",
                           hm2->llio->name,
                           i);
            r = hal_pin_u32_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.instance[i].hal.pin.controly),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.commandx",
                           hm2->llio->name,
                           i);
            r = hal_pin_u32_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.instance[i].hal.pin.commandx),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.commandy",
                           hm2->llio->name,
                           i);
            r = hal_pin_u32_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.instance[i].hal.pin.commandy),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.18bitmodex",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.instance[i].hal.pin.mode18bitx),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.18bitmodey",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.instance[i].hal.pin.mode18bity),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.commandmodex",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.instance[i].hal.pin.commandmodex),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.commandmodey",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->xy2mod.instance[i].hal.pin.commandmodey),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.posx-overflow",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(
                name,
                HAL_OUT,
                &(hm2->xy2mod.instance[i].hal.pin.posx_overflow),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.posy-overflow",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(
                name,
                HAL_OUT,
                &(hm2->xy2mod.instance[i].hal.pin.posy_overflow),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.velx-overflow",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(
                name,
                HAL_OUT,
                &(hm2->xy2mod.instance[i].hal.pin.velx_overflow),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.xy2mod.%02d.vely-overflow",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(
                name,
                HAL_OUT,
                &(hm2->xy2mod.instance[i].hal.pin.vely_overflow),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            r = hal_pin_u32_new(name,
                                HAL_OUT,
                                &(hm2->xy2mod.instance[i].hal.pin.status),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
...
void hm2_xy2mod_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              hm2->xy2mod.clock_frequency,
              hm2_hz_to_mhz(hm2->xy2mod.clock_frequency));
    HM2_PRINT("    version: %d\n", hm2->xy2mod.version);
