...
void hm2_oneshot_update_width1(hostmot2_t *hm2, int i)
{
    ...
    hm2->oneshot.width1_reg[i] =
        (double)*hm2->oneshot.instance[i].hal.pin.width1 *
        ((double)hm2->oneshot.clock_frequency / (double)1e3);
    if (hm2->oneshot.width1_reg[i] > 0x7FFFFFFF) {
        ...
        *hm2->oneshot.instance[i].hal.pin.width1 =
            (double)hm2->oneshot.width1_reg[i] *
            ((double)1e3 / (double)hm2->oneshot.clock_frequency);
    }
}
...
void hm2_oneshot_update_width2(hostmot2_t *hm2, int i)
{
    ...
    hm2->oneshot.width2_reg[i] =
        (double)*hm2->oneshot.instance[i].hal.pin.width2 *
        ((double)hm2->oneshot.clock_frequency / (double)1e3);
    if (hm2->oneshot.width2_reg[i] > 0x7FFFFFFF) {
        HM2_ERR("oneshot %d has invalid width1, resetting to max\n", i);
        hm2->oneshot.width2_reg[i] = 0x7FFFFFFF;
        *hm2->oneshot.instance[i].hal.pin.width2 =
            (double)hm2->oneshot.width2_reg[i] *
            ((double)1e3 / (double)hm2->oneshot.clock_frequency);
    }
}
...
void hm2_oneshot_update_filter1(hostmot2_t *hm2, int i)
{
    ...
    hm2->oneshot.filter1_reg[i] =
        (double)*hm2->oneshot.instance[i].hal.pin.filter1 *
        ((double)hm2->oneshot.clock_frequency / (double)1e3);
    if (hm2->oneshot.filter1_reg[i] > 0xFFFFFF) {
        ...
        *hm2->oneshot.instance[i].hal.pin.filter1 =
            (double)hm2->oneshot.filter1_reg[i] *
            ((double)1e3 / (double)hm2->oneshot.clock_frequency);
    }
}
...
void hm2_oneshot_update_filter2(hostmot2_t *hm2, int i)
{
    ...
    hm2->oneshot.filter2_reg[i] =
        (double)*hm2->oneshot.instance[i].hal.pin.filter2 *
        ((double)hm2->oneshot.clock_frequency / (double)1e3);
    if (hm2->oneshot.filter2_reg[i] > 0xFFFFFF) {
        ...
        *hm2->oneshot.instance[i].hal.pin.filter2 =
            (double)hm2->oneshot.filter2_reg[i] *
            ((double)1e3 / (double)hm2->oneshot.clock_frequency);
    }
}
...
void hm2_oneshot_update_rate(hostmot2_t *hm2, int i)
{
    hm2->oneshot.rate_reg[i] =
        (uint32_t)(*hm2->oneshot.instance[i].hal.pin.rate *
                   (4294967296.0 / (double)hm2->oneshot.clock_frequency));
}
...
void hm2_oneshot_update_control(hostmot2_t *hm2, int i)
{
    ...
    if (*hm2->oneshot.instance[i].hal.pin.trigselect1 > 7) {
        HM2_ERR(
            "oneshot %d has invalid trigger 1 select value , resetting to 0\n",
            i);
        *hm2->oneshot.instance[i].hal.pin.trigselect1 = 0;
    }
    if (*hm2->oneshot.instance[i].hal.pin.trigselect2 > 7) {
        HM2_ERR(
            "oneshot %d has invalid trigger 2 select value , resetting to 0\n",
            i);
    ...
    controlbuff |=
        ((*hm2->oneshot.instance[i].hal.pin.dpll_timer_num & 7) << 12);
...
void hm2_oneshot_force_write(hostmot2_t *hm2)
{
    ...
    hm2->llio->write(hm2->llio,
                     ...
                     hm2->oneshot.width1_reg,
                     (hm2->oneshot.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
                     hm2->oneshot.width2_addr,
                     hm2->oneshot.width2_reg,
                     (hm2->oneshot.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
                     hm2->oneshot.filter1_addr,
                     hm2->oneshot.filter1_reg,
                     (hm2->oneshot.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
                     hm2->oneshot.filter2_addr,
                     hm2->oneshot.filter2_reg,
                     (hm2->oneshot.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
                     hm2->oneshot.rate_addr,
                     hm2->oneshot.rate_reg,
                     (hm2->oneshot.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
                     hm2->oneshot.control_addr,
                     hm2->oneshot.control_reg,
                     (hm2->oneshot.num_instances * sizeof(rtapi_u32)));
...
int hm2_oneshot_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->config.num_oneshots > md->instances) {
        HM2_ERR(
            "config.num_oneshots=%d, but only %d are available, not loading "
            "driver\n",
    ...
    hm2->oneshot.instance = (hm2_oneshot_instance_t *)hal_malloc(
        hm2->oneshot.num_instances * sizeof(hm2_oneshot_instance_t));
    ...
    if (r < 0) {
        HM2_ERR(
            "error registering tram write region for Width1 register (%d)\n",
            r);
        goto fail0;
    }
    ...
    if (r < 0) {
        HM2_ERR(
            "error registering tram write region for Width2 register (%d)\n",
            r);
        goto fail0;
    }
    ...
    if (r < 0) {
        HM2_ERR(
            "error registering tram write region for Filter1 register (%d)\n",
            r);
        goto fail0;
    }
    ...
    if (r < 0) {
        HM2_ERR(
            "error registering tram write region for Filter2 register (%d)\n",
            r);
        goto fail0;
    }
    ...
    if (r < 0) {
        HM2_ERR(
            "error registering tram write region for Control register (%d)\n",
            r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        hm2->oneshot.control_read_addr,
        (hm2->oneshot.num_instances * sizeof(rtapi_u32)),
        &hm2->oneshot.control_read_reg);
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for Control register (%d)\n",
            r);
        goto fail0;
    }
    ...
    // FIXME: r hides the r in enclosing function, and it returns the wrong thing
    {
        ...
        for (i = 0; i < hm2->oneshot.num_instances; i++) {
            ...
            rtapi_snprintf(
                name, sizeof(name), "%s.oneshot.%02d.rate", hm2->llio->name, i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->oneshot.instance[i].hal.pin.rate),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.trigger_select1",
                           hm2->llio->name,
                           i);
            r = hal_pin_u32_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.trigselect1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.trigger_select2",
                           hm2->llio->name,
                           i);
            r = hal_pin_u32_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.trigselect2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.dpll_timer_number",
                           hm2->llio->name,
                           i);
            r = hal_pin_s32_new(
                name,
                HAL_IN,
                &(hm2->oneshot.instance[i].hal.pin.dpll_timer_num),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.trigger_on_rise1",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.trigrise1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.trigger_on_rise2",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.trigrise2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.trigger_on_fall1",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.trigfall1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.trigger_on_fall2",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.trigfall2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.retriggerable1",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.retrig1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.retriggerable2",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.retrig2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.enable1",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.enable1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.enable2",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.enable2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.reset1",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.reset1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.reset2",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.reset2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.swtrigger1",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.swtrig1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.swtrigger2",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->oneshot.instance[i].hal.pin.swtrig2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.exttrigger1",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_OUT,
                                &(hm2->oneshot.instance[i].hal.pin.exttrig1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.oneshot.%02d.exttrigger2",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_OUT,
                                &(hm2->oneshot.instance[i].hal.pin.exttrig2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(
                name, sizeof(name), "%s.oneshot.%02d.out1", hm2->llio->name, i);
            r = hal_pin_bit_new(name,
                                HAL_OUT,
                                &(hm2->oneshot.instance[i].hal.pin.out1),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(
                name, sizeof(name), "%s.oneshot.%02d.out2", hm2->llio->name, i);
            r = hal_pin_bit_new(name,
                                HAL_OUT,
                                &(hm2->oneshot.instance[i].hal.pin.out2),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
...
void hm2_oneshot_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              hm2->oneshot.clock_frequency,
              hm2_hz_to_mhz(hm2->oneshot.clock_frequency));
    HM2_PRINT("    version: %d\n", hm2->oneshot.version);
