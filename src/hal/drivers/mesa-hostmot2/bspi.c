...
int hm2_bspi_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    hm2->bspi.instance = (hm2_bspi_instance_t *)hal_malloc(
        hm2->bspi.num_instances * sizeof(hm2_bspi_instance_t));
    ...
    for (i = 0; i < hm2->bspi.num_instances; i++) {
        ...
        r = snprintf(
            chan->name, sizeof(chan->name), "%s.bspi.%01d", hm2->llio->name, i);
        ...
        chan->cd_addr =
            md->base_address + md->register_stride + i * sizeof(rtapi_u32);
        chan->count_addr =
            md->base_address + 2 * md->register_stride + i * sizeof(rtapi_u32);
        for (j = 0; j < 16; j++) {
...
void hm2_bspi_force_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->bspi.num_instances; i++) {
        ...
        for (j = 15; j >= 0; j--) {
            hm2->llio->write(
                hm2->llio, chan.cd_addr, &(chan.cd[j]), sizeof(rtapi_u32));
        }
    }
}
                            ...
                            rtapi_u32 **rbuff)
{
    ...
    if (wbuff != NULL) {
        r = hm2_register_tram_write_region(
            hm2, hm2->bspi.instance[i].addr[chan], sizeof(rtapi_u32), wbuff);
        if (r < 0) {
    ...
    if (rbuff != NULL) {
        ...
        r = hm2_register_tram_read_region(
            hm2, hm2->bspi.instance[i].addr[0], sizeof(rtapi_u32), rbuff);
        if (r < 0) {
...
int hm2_bspi_clear_fifo(char *name)
{
    ...
    rtapi_u32 zero = 0;
    r = hm2->llio->write(
        hm2->llio, hm2->bspi.instance[i].count_addr, &zero, sizeof(rtapi_u32));
    if (r < 0) {
...
int hm2_bspi_write_chan(char *name, int chan, rtapi_u32 val)
{
    ...
    r = hm2->llio->write(
        hm2->llio, hm2->bspi.instance[i].addr[chan], &buff, sizeof(rtapi_u32));
    if (r < 0) {
        HM2_ERR("BSPI: hm2->llio->write failure %s\n", name);
    }
                        ...
                        int samplelate)
{
    ...
    buff =
        (noecho != 0) << 31 | (noclear != 0) << 30 | (samplelate != 0) << 29 |
        ((delay <= 0) ? 0x10
                      : (rtapi_u32)((delay * board_mhz / 1000.0) - 1) & 0x1f)
            << 24 |
        (cs & 0xF) << 16 |
        (((rtapi_u16)(board_mhz / (mhz * 2) - 1) & 0xFF)) << 8 |
        (cpha != 0) << 7 | (cpol != 0) << 6 | (((rtapi_u16)(bits - 1)) & 0x3F);
...
void hm2_bspi_process_tram_read(hostmot2_t *hm2, long period)
{
    ...
    for (i = 0; i < hm2->bspi.num_instances; i++) {
        ...
        if (func != NULL) {
            ...
            if (r < 0)
                HM2_ERR(
                    "BSPI read function @%p failed (returned %d)\n", func, r);
        }
    }
}
