...
static int
spi_transfer(const spix_port_t *sp, uint32_t *wptr, size_t txlen, int rw);
...
static int
    has_spi_module; // Set to non-zero when the kernel modules dw_spi and dw_spi_mmio are loaded
static int
    driver_enabled; // Set to non-zero when rpi5_setup() is successfully called
...
static rp1_io_bank0_t
    *iobank0; // GPIO pin config structure in mmap'ed address space
static rp1_pads_bank0_t
    *padsbank0; // GPIO pin pads structure in mmap'ed address space
...
spi_transfer(const spix_port_t *sp, uint32_t *wptr, size_t txlen, int rw)
{
    ...
    reg_wr(&port->ctrlr0,
           DW_SSI_CTRLR0_DFS_32(32 - 1) |
               DW_SSI_CTRLR0_TMOD(DW_SSI_CTRLR0_TMOD_TXRX));
...
static int peripheral_map(uintptr_t membase, size_t memsize)
{
    ...
    spi_ports[0].port = spi_ports[1].port =
        (dw_ssi_t *)((void *)((uintptr_t)peripheralmem + RP1_SPI0_OFFSET));
    spi_ports[2].port = spi_ports[3].port = spi_ports[4].port =
        (dw_ssi_t *)((void *)((uintptr_t)peripheralmem + RP1_SPI1_OFFSET));
    iobank0 = (rp1_io_bank0_t *)((void *)((uintptr_t)peripheralmem +
                                          RP1_IO_BANK0_OFFSET));
    padsbank0 = (rp1_pads_bank0_t *)((void *)((uintptr_t)peripheralmem +
                                              RP1_PADS_BANK0_OFFSET));
    rio0 =
        (rp1_rio_t *)((void *)((uintptr_t)peripheralmem + RP1_SYS_RIO0_OFFSET));
...
static void peripheral_setup(void)
{
    ...
    static const int spi_pull_miso[SPI_MAX_SPI] = {SPI_PULL_MISO_DEF,
                                                   SPI_PULL_MISO_DEF};
    static const int spi_pull_mosi[SPI_MAX_SPI] = {SPI_PULL_MOSI_DEF,
                                                   SPI_PULL_MOSI_DEF};
    static const int spi_pull_sclk[SPI_MAX_SPI] = {SPI_PULL_SCLK_DEF,
                                                   SPI_PULL_SCLK_DEF};
    ...
    if (port_probe_mask & SPIX_PROBE_SPI0_MASK) {
        ...
        reg_wr(&iobank0->gpio[SPI0_PIN_SCLK].ctrl,
               RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
        reg_wr(&padsbank0->gpio[SPI0_PIN_SCLK],
               RP1_PADS_IE | DRIVE_LEVEL | pud_sclk);
        reg_wr(&iobank0->gpio[SPI0_PIN_MOSI].ctrl,
               RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
        reg_wr(&padsbank0->gpio[SPI0_PIN_MOSI],
               RP1_PADS_IE | DRIVE_LEVEL | pud_mosi);
        reg_wr(&iobank0->gpio[SPI0_PIN_MISO].ctrl,
               RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
        reg_wr(&padsbank0->gpio[SPI0_PIN_MISO],
               RP1_PADS_IE | DRIVE_LEVEL | pud_miso);
        if (port_probe_mask & SPIX_PROBE_SPI0_CE0) {
            reg_wr(&iobank0->gpio[SPI0_PIN_CE_0].ctrl,
                   RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
            reg_wr(&padsbank0->gpio[SPI0_PIN_CE_0],
                   RP1_PADS_IE | DRIVE_LEVEL | RP1_PADS_PUE);
        }
        if (port_probe_mask & SPIX_PROBE_SPI0_CE1) {
            reg_wr(&iobank0->gpio[SPI0_PIN_CE_1].ctrl,
                   RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
            reg_wr(&padsbank0->gpio[SPI0_PIN_CE_1],
                   RP1_PADS_IE | DRIVE_LEVEL | RP1_PADS_PUE);
        }
    ...
    if (port_probe_mask & SPIX_PROBE_SPI1_MASK) {
        ...
        reg_wr(&iobank0->gpio[SPI1_PIN_SCLK].ctrl,
               RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
        reg_wr(&padsbank0->gpio[SPI1_PIN_SCLK],
               RP1_PADS_IE | DRIVE_LEVEL | pud_sclk);
        reg_wr(&iobank0->gpio[SPI1_PIN_MOSI].ctrl,
               RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
        reg_wr(&padsbank0->gpio[SPI1_PIN_MOSI],
               RP1_PADS_IE | DRIVE_LEVEL | pud_mosi);
        reg_wr(&iobank0->gpio[SPI1_PIN_MISO].ctrl,
               RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
        reg_wr(&padsbank0->gpio[SPI1_PIN_MISO],
               RP1_PADS_IE | DRIVE_LEVEL | pud_miso);
        if (port_probe_mask & SPIX_PROBE_SPI1_CE0) {
            reg_wr(&iobank0->gpio[SPI1_PIN_CE_0].ctrl,
                   RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
            reg_wr(&padsbank0->gpio[SPI1_PIN_CE_0],
                   RP1_PADS_IE | DRIVE_LEVEL | RP1_PADS_PUE);
        }
        if (port_probe_mask & SPIX_PROBE_SPI1_CE1) {
            reg_wr(&iobank0->gpio[SPI1_PIN_CE_1].ctrl,
                   RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
            reg_wr(&padsbank0->gpio[SPI1_PIN_CE_1],
                   RP1_PADS_IE | DRIVE_LEVEL | RP1_PADS_PUE);
        }
        if (port_probe_mask & SPIX_PROBE_SPI1_CE2) {
            reg_wr(&iobank0->gpio[SPI1_PIN_CE_2].ctrl,
                   RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_ALT0));
            reg_wr(&padsbank0->gpio[SPI1_PIN_CE_2],
                   RP1_PADS_IE | DRIVE_LEVEL | RP1_PADS_PUE);
        }
    ...
    reg_wr(&padsbank0->gpio[RPSPI_DEBUG_PIN],
           RP1_PADS_IE | DRIVE_LEVEL | RP1_PADS_PUE);
    reg_wr(&iobank0->gpio[RPSPI_DEBUG_PIN].ctrl,
           RP1_GPIO_CTRL_FUNCSEL(RP1_GPIO_CTRL_FUNCSEL_SYS_RIO));
#endif
}
...
static int rpi5_detect(const char *dtcs[])
{
    ...
    for (i = 0; dtcs[i] != NULL; i++) {
        if (!strcmp(dtcs[i], DTC_RPI_MODEL_5B) ||
            !strcmp(dtcs[i], DTC_RPI_MODEL_5CM)) {
    ...
    if (spix_read_file("/proc/device-tree/model",
                       spix_driver_rpi5.model,
                       sizeof(spix_driver_rpi5.model)) < 0)
        strncpy(spix_driver_rpi5.model,
                "??? Unknown board ???",
                sizeof(spix_driver_rpi5.model) - 1);
...
static int rpi5_setup(int probemask)
{
    ...
    port_probe_mask =
        probemask; // For peripheral_setup() and peripheral_restore()

    // Now we know what platform we are running, remove kernel SPI module if
    // detected
    if ((has_spi_module =
             (0 == shell("/usr/bin/grep -qw ^dw_spi_mmio /proc/modules")))) {
        if (shell("/sbin/rmmod dw_spi_mmio dw_spi"))
            LL_ERR(
                "Unable to remove kernel SPI modules dw_spi_mmio and dw_spi. "
                "Your system may become unstable using LinuxCNC with "
                "the " HM2_LLIO_NAME " driver.\n");
    }
...
static const spix_port_t *rpi5_open(int port, const spix_args_t *args)
{
    ...
    if (args->clkw < SCLK_FREQ_MIN || args->clkw > SCLK_FREQ_MAX) {
        LL_ERR("%s: SPI write clock frequency outside acceptable range (%d..%d "
               "kHz).\n",
               rpp->spix.name,
               SCLK_FREQ_MIN,
               SCLK_FREQ_MAX);
