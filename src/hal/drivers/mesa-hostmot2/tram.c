...
int hm2_allocate_tram_regions(hostmot2_t *hm2)
{
    ...
    {
        hm2_tram_entry_t *tram_entry =
            rtapi_list_entry(ptr, hm2_tram_entry_t, list);
    ...
    HM2_DBG("allocating Translation RAM buffers (reading %d bytes, writing %d "
            "bytes)\n",
            hm2->tram_read_size,
            hm2->tram_write_size);

    hm2->tram_read_buffer = (rtapi_u32 *)rtapi_krealloc(
        hm2->tram_read_buffer, hm2->tram_read_size, RTAPI_GFP_KERNEL);
    ...
    if (hm2->tram_read_size > old_tram_read_size)
        memset((char *)hm2->tram_read_buffer + old_tram_read_size,
               0,
               hm2->tram_read_size - old_tram_read_size);

    hm2->tram_write_buffer = (rtapi_u32 *)rtapi_krealloc(
        hm2->tram_write_buffer, hm2->tram_write_size, RTAPI_GFP_KERNEL);
    ...
    if (hm2->tram_write_size > old_tram_write_size)
        memset((char *)hm2->tram_write_buffer + old_tram_write_size,
               0,
               hm2->tram_write_size - old_tram_write_size);
    ...
    rtapi_list_for_each(ptr, &hm2->tram_read_entries)
    {
        hm2_tram_entry_t *tram_entry =
            rtapi_list_entry(ptr, hm2_tram_entry_t, list);
        *tram_entry->buffer =
            (rtapi_u32 *)((rtapi_u8 *)hm2->tram_read_buffer + offset);
    ...
    rtapi_list_for_each(ptr, &hm2->tram_write_entries)
    {
        hm2_tram_entry_t *tram_entry =
            rtapi_list_entry(ptr, hm2_tram_entry_t, list);
        *tram_entry->buffer =
            (rtapi_u32 *)((rtapi_u8 *)hm2->tram_write_buffer + offset);
        offset += tram_entry->size;
        HM2_DBG("    addr=0x%04x, size=%d, buffer=%p\n",
                tram_entry->addr,
                tram_entry->size,
...
int hm2_tram_read(hostmot2_t *hm2)
{
    ...
    rtapi_list_for_each(ptr, &hm2->tram_read_entries)
    {
        ...
        if (!hm2->llio->queue_read(hm2->llio,
                                   tram_entry->addr,
                                   *tram_entry->buffer,
                                   tram_entry->size)) {
...
int hm2_tram_write(hostmot2_t *hm2)
{
    ...
    rtapi_list_for_each(ptr, &hm2->tram_write_entries)
    {
        ...
        if (!hm2->llio->queue_write(hm2->llio,
                                    tram_entry->addr,
                                    *tram_entry->buffer,
                                    tram_entry->size)) {
