...
void hm2_tp_pwmgen_handle_pwm_frequency(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->tp_pwmgen.num_instances; i++) {
        ...
        deadtime = (hm2->tp_pwmgen.instance[i].hal.param.deadzone *
                    hm2->tp_pwmgen.clock_frequency * dds) /
                   (2 * 65536 * 1e9);
        ...
        hm2->tp_pwmgen.setup_reg[i] =
            (((int)(hm2->tp_pwmgen.instance[i].hal.param.sampletime * 1023)
              << 16) +
...
void hm2_tp_pwmgen_force_write(hostmot2_t *hm2)
{
    ...
    hm2->llio->write(hm2->llio,
                     hm2->tp_pwmgen.setup_addr,
                     hm2->tp_pwmgen.setup_reg,
                     (hm2->tp_pwmgen.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
                     hm2->tp_pwmgen.enable_addr,
                     hm2->tp_pwmgen.enable_reg,
                     (hm2->tp_pwmgen.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
                     hm2->tp_pwmgen.pwmgen_master_rate_dds_addr,
                     &hm2->tp_pwmgen.pwmgen_master_rate_dds_reg,
                     sizeof(rtapi_u32));
    ...
    for (i = 0; i < hm2->tp_pwmgen.num_instances; i++) {
        hm2->tp_pwmgen.instance[i].written_faultpolarity =
            hm2->tp_pwmgen.instance[i].hal.param.faultpolarity;
        hm2->tp_pwmgen.instance[i].written_deadzone =
            hm2->tp_pwmgen.instance[i].hal.param.deadzone;
        hm2->tp_pwmgen.instance[i].written_sampletime =
            hm2->tp_pwmgen.instance[i].hal.param.sampletime;
    }

    hm2->tp_pwmgen.written_pwm_frequency =
        hm2->tp_pwmgen.hal->param.pwm_frequency;
}
...
void hm2_tp_pwmgen_write(hostmot2_t *hm2)
{
    ...
    // check pwm frequency
    if (hm2->tp_pwmgen.hal->param.pwm_frequency !=
        hm2->tp_pwmgen.written_pwm_frequency)
        goto force_write;

    // update enable register?
    for (i = 0; i < hm2->tp_pwmgen.num_instances; i++) {
        if ((hm2->tp_pwmgen.instance[i].hal.param.deadzone !=
             hm2->tp_pwmgen.instance[i].written_deadzone) ||
            (hm2->tp_pwmgen.instance[i].hal.param.sampletime !=
             hm2->tp_pwmgen.instance[i].written_sampletime) ||
            (hm2->tp_pwmgen.instance[i].hal.param.faultpolarity !=
...
int hm2_tp_pwmgen_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->config.num_tp_pwmgens > md->instances) {
        HM2_ERR(
            "config.num_3pwmgens=%d, but only %d are available, not loading "
            "driver\n",
    ...
    hm2->tp_pwmgen.instance = (hm2_tp_pwmgen_instance_t *)hal_malloc(
        hm2->tp_pwmgen.num_instances * sizeof(hm2_tp_pwmgen_instance_t));
    ...
    hm2->tp_pwmgen.hal = (hm2_tp_pwmgen_global_hal_t *)hal_malloc(
        sizeof(hm2_tp_pwmgen_global_hal_t));
    if (hm2->tp_pwmgen.instance == NULL) {
        HM2_ERR("out of memory!\n");
        r = -ENOMEM;
        goto fail0;
    ...
    hm2->tp_pwmgen.pwm_value_addr =
        md->base_address + (0 * md->register_stride);
    hm2->tp_pwmgen.pwmgen_master_rate_dds_addr =
    ...
    hm2->tp_pwmgen.setup_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->tp_pwmgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    if (hm2->tp_pwmgen.setup_reg == NULL) {
        HM2_ERR("out of memory!\n");
        r = -ENOMEM;
        goto fail1;
    }
    hm2->tp_pwmgen.enable_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->tp_pwmgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    r = hm2_register_tram_write_region(
        ...
        hm2->tp_pwmgen.pwm_value_addr,
        (hm2->tp_pwmgen.num_instances * sizeof(rtapi_u32)),
        &hm2->tp_pwmgen.pwm_value_reg);
    if (r < 0) {
        HM2_ERR("error registering tram write region for 3PWM Value register "
                "(%d)\n",
                r);
        goto fail2;
    }
    ...
    // FIXME: r hides the r in enclosing function, and it returns the wrong thing
    {
        ...
        for (i = 0; i < hm2->tp_pwmgen.num_instances; i++) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.3pwmgen.%02d.A-value",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->tp_pwmgen.instance[i].hal.pin.Avalue),
                                  hm2->llio->comp_id);
            if (r < 0) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.3pwmgen.%02d.B-value",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->tp_pwmgen.instance[i].hal.pin.Bvalue),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail2;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.3pwmgen.%02d.C-value",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->tp_pwmgen.instance[i].hal.pin.Cvalue),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail2;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.3pwmgen.%02d.enable",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(name,
                                HAL_IN,
                                &(hm2->tp_pwmgen.instance[i].hal.pin.enable),
                                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail2;
            ...
            r = hal_param_float_new(
                name,
                HAL_RW,
                &(hm2->tp_pwmgen.instance[i].hal.param.scale),
                hm2->llio->comp_id);
            if (r < 0) {
            ...
            *(hm2->tp_pwmgen.instance[i].hal.pin.Cvalue) = 0.0;
            ...
            hm2->tp_pwmgen.instance[i].hal.param.faultpolarity =
                0.0; //active low
            hm2->tp_pwmgen.instance[i].hal.param.deadzone =
                5000; // 5000nS conservative deadzone for safety
...
void hm2_tp_pwmgen_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              hm2->tp_pwmgen.clock_frequency,
              hm2_hz_to_mhz(hm2->tp_pwmgen.clock_frequency));
...
void hm2_tp_pwmgen_prepare_tram_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->tp_pwmgen.num_instances; i++) {
        ...
        if (hm2->tp_pwmgen.instance[i].hal.param.scale == 0) {
            ...
            HM2_ERR("3pwmgen scale must be greater than zero. Scale set to %i",
                    (int)hm2->tp_pwmgen.instance[i].hal.param.scale);
        }

        scaled_Avalue = (*hm2->tp_pwmgen.instance[i].hal.pin.Avalue /
                         hm2->tp_pwmgen.instance[i].hal.param.scale);
        scaled_Bvalue = (*hm2->tp_pwmgen.instance[i].hal.pin.Bvalue /
                         hm2->tp_pwmgen.instance[i].hal.param.scale);
        scaled_Cvalue = (*hm2->tp_pwmgen.instance[i].hal.pin.Cvalue /
                         hm2->tp_pwmgen.instance[i].hal.param.scale);
