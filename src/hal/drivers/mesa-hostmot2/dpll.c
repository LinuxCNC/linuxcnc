...
int hm2_dpll_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    // export to HAL
    hm2->dpll.pins = hal_malloc(sizeof(*hm2->dpll.pins));

    r = hal_pin_new_real(hm2->llio->comp_id, HAL_IN, &(hm2->dpll.pins->time1_us),
            100.0, "%s.dpll.01.timer-us", hm2->llio->name);
    r += hal_pin_new_real(hm2->llio->comp_id, HAL_IN, &(hm2->dpll.pins->time2_us),
            100.0, "%s.dpll.02.timer-us", hm2->llio->name);
    r += hal_pin_new_real(hm2->llio->comp_id, HAL_IN, &(hm2->dpll.pins->time3_us),
            100.0, "%s.dpll.03.timer-us", hm2->llio->name);
    r += hal_pin_new_real(hm2->llio->comp_id, HAL_IN, &(hm2->dpll.pins->time4_us),
            100.0, "%s.dpll.04.timer-us", hm2->llio->name);
    // init=-1 is an indication it needs init
    r += hal_pin_new_real(hm2->llio->comp_id, HAL_IN, &(hm2->dpll.pins->base_freq),
            -1.0, "%s.dpll.base-freq-khz", hm2->llio->name);
    r += hal_pin_new_real(hm2->llio->comp_id, HAL_OUT, &(hm2->dpll.pins->phase_error),
            0.0, "%s.dpll.phase-error-us", hm2->llio->name);
    /* The init value is an empirical compromise between insensitivity to
     ...
     * and ntpdate.
     */
    r += hal_pin_new_ui32(hm2->llio->comp_id, HAL_IN, &(hm2->dpll.pins->time_const),
            2000, "%s.dpll.time-const", hm2->llio->name);
    r += hal_pin_new_ui32(hm2->llio->comp_id, HAL_IN, &(hm2->dpll.pins->plimit),
            0x400000, "%s.dpll.plimit", hm2->llio->name);
    r += hal_pin_new_ui32(hm2->llio->comp_id, HAL_OUT, &(hm2->dpll.pins->ddssize),
            0, "%s.dpll.ddsize", hm2->llio->name);
    r += hal_pin_new_ui32(hm2->llio->comp_id, HAL_OUT, &(hm2->dpll.pins->prescale),
            1, "%s.dpll.prescale", hm2->llio->name);
    if (r < 0) {
...
void hm2_dpll_process_tram_read(hostmot2_t *hm2, long period){
    ...
    hal_set_real(pins->phase_error, (rtapi_s32)*hm2->dpll.hm2_dpll_sync_reg
            * (period / 4294967296000.00));
    hal_set_ui32(pins->ddssize, *hm2->dpll.control_reg1_read & 0xFF);
}
...
void hm2_dpll_write(hostmot2_t *hm2, long period) {
    ...
    rtapi_real base_freq = hal_get_real(pins->base_freq);
    if (base_freq < 0 ) {
        base_freq = hal_set_real(pins->base_freq, 1000.0/period_us);
    }

    rtapi_u32 ddssize = hal_get_ui32(pins->ddssize);
    rtapi_u32 prescale = (0x40000000LL * hm2->dpll.clock_frequency)
                    / ((1LL << ddssize) * base_freq * 1000.0);

    if (prescale < 1) prescale = 1;
    hal_set_ui32(pins->prescale, prescale);
    
    buff = (rtapi_u32)((base_freq * 1000.0
            * (1LL << ddssize) 
            * prescale)
    ...
    buff = (rtapi_u32)(prescale << 24
                | hal_get_ui32(pins->plimit));
    ...
    buff = (rtapi_u32)(hal_get_ui32(pins->time_const) << 16);
    ...
    buff = (rtapi_u32)((-hal_get_real(hm2->dpll.pins->time2_us) / period_us) * 0x10000) << 16
         | ((rtapi_u32)((-hal_get_real(hm2->dpll.pins->time1_us) / period_us) * 0x10000) & 0xFFFF);
    ...
    buff = (rtapi_u32)((-hal_get_real(hm2->dpll.pins->time4_us) / period_us) * 0x10000) << 16
         | ((rtapi_u32)((-hal_get_real(hm2->dpll.pins->time3_us) / period_us) * 0x10000) & 0xFFFF);
