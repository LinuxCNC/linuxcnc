...
int setlocal32(hostmot2_t *hm2, hm2_sserial_instance_t *inst, int addr, int val)
{
    ...
    for (; bytes < 4; bytes++) {
        ...
        if (hm2_sserial_waitfor(hm2, inst->command_reg_addr, 0xFFFFFFFF, 22) <
            0) {
                                ...
                                int length)
{
    ...
    hm2->llio->write(
        hm2->llio, chan->command_reg_addr, &buff, sizeof(rtapi_u32));
    if (0 >
        hm2_sserial_waitfor(hm2, chan->command_reg_addr, 0xFFFFFFFF, 1012)) {
        HM2_ERR("Timeout in sserial_read_nvram_word(2)\n");
        goto fail0;
    }
    switch (length) {
    case 1: buff = READ_REM_BYTE_CMD + addr; break;
    case 2: buff = READ_REM_WORD_CMD + addr; break;
    case 4: buff = READ_REM_LONG_CMD + addr; break;
    case 8: buff = READ_REM_DOUBLE_CMD + addr; break;
    default:
    ...
    hm2->llio->write(
        hm2->llio, chan->command_reg_addr, &buff, sizeof(rtapi_u32));
    if (0 >
        hm2_sserial_waitfor(hm2, chan->command_reg_addr, 0xFFFFFFFF, 1013)) {
...
fail0: // attempt to set back to normal access
    ...
    hm2->llio->write(
        hm2->llio, chan->command_reg_addr, &buff, sizeof(rtapi_u32));
    if (0 >
        hm2_sserial_waitfor(hm2, chan->command_reg_addr, 0xFFFFFFFF, 1014)) {
...
int check_set_baudrate(hostmot2_t *hm2, hm2_sserial_instance_t *inst)
{
    ...
    if (hm2->sserial.version < 34) {
        HM2_ERR("Setting baudrate is not supported in the current firmware "
                "version\n"
    ...
    for (c = 0; c < inst->num_channels; c++) {
        baudaddr =
            getlocal8(hm2, inst, SSLBPCHANNELSTARTLOC) + (c * lbpstride) + 42;
        ...
        if (baudrate != (unsigned)hm2->sserial.baudrate) {
            if (setlocal32(hm2, inst, baudaddr, hm2->sserial.baudrate) < 0) {
                HM2_ERR("Problem setting new baudrate, power-off reset may be "
                        "needed to"
                        " recover from this.\n");
                return -EINVAL;
            }
...
int hm2_sserial_stopstart(hostmot2_t *hm2,
                          hm2_module_descriptor_t *md,
                          hm2_sserial_instance_t *inst,
                          rtapi_u32 start_mode)
{
    rtapi_u32 buff, addr;
    int i = inst->index;
    int c;
    ...
    buff = 0x800; //Stop All
    hm2->llio->write(
        hm2->llio, inst->command_reg_addr, &buff, sizeof(rtapi_u32));
    ...
    hm2->llio->write(
        hm2->llio, inst->command_reg_addr, &start_mode, sizeof(rtapi_u32));
    if (hm2_sserial_waitfor(hm2, inst->command_reg_addr, 0xFFFFFFFF, 8000) <
        0) {
        return -EINVAL;
    }
    return 0;
                          ...
                          int size)
{
    ...
    while (0 != size) {
        ...
        hm2->llio->write(
            hm2->llio, chan->reg_cs_addr, &data, sizeof(rtapi_u32));

        if (0 > hm2_sserial_waitfor(hm2, chan->reg_cs_addr, 0x0000FF00, 50)) {
            HM2_ERR(
                "Timeout trying to read config data in sserial_get_bytes\n");
            return -EINVAL;
        }
        data = 0x1000 | (1 << chan->index);
        hm2->llio->write(
            hm2->llio, chan->command_reg_addr, &data, sizeof(rtapi_u32));

        if (0 >
            hm2_sserial_waitfor(hm2, chan->command_reg_addr, 0xFFFFFFFF, 51)) {
                                ...
                                int set_hal)
{
    ...
    case LBP_UNSIGNED:
        ...
        if ((strcmp(g->NameString, "swrevision") == 0) && (p->u32_param < 14)) {
            HM2_ERR(
                "Warning: sserial remote device %s channel %d has old firmware "
    ...
    case LBP_NONVOL_SIGNED:
        ...
        if (set_hal)
            p->s32_param = p->s32_written;
    case LBP_STREAM: break; // Have not seen a stream type yet
    case LBP_BOOLEAN: break;
    case LBP_ENCODER: break; // Hard to imagine an encoder not in Process data
    case LBP_FLOAT: {
        char buf[HM2_SSERIAL_MAX_DATALENGTH / 8];
        r = hm2_sserial_get_bytes(
            hm2, chan, &buf[0], g->ParmAddr, g->DataLength / 8);
        if (g->DataLength == sizeof(float) * 8) {
            ...
            HM2_ERR("sserial get param value: LBP_FLOAT of bit-length %i not "
                    "handled\n",
                    g->DataLength);
        }
    }
        ...
        break;
    case LBP_ENCODER_H:
    case LBP_ENCODER_L: break; // Hard to imagine an encoder not in Process data
    default:
...
int hm2_sserial_get_globals_list(hostmot2_t *hm2, hm2_sserial_remote_t *chan)
{
    ...
    if (gtoc == 0) {
        if (hm2->sserial.baudrate == 115200) {
            HM2_DBG(
                "Setup mode, creating no pins for smart-serial channel %s\n",
                chan->name);
            chan->num_confs = 0;
            chan->num_globals = 0;
            return 0;
        ...
        do {
            ...
            if (((addr &= 0xFFFF) <= 0) || (gtoc < 0))
                break;
            if ((addr = hm2_sserial_get_bytes(hm2, chan, &data, addr, 14)) <
                0) {
                return -EINVAL;
            }
            ...
            if (data.RecordType == LBP_DATA && i < 1000) {
                addr = hm2_sserial_get_bytes(
                    hm2, chan, &(data.UnitString), addr, -1);
                if (addr < 0) {
                    return -EINVAL;
                }
                addr = hm2_sserial_get_bytes(
                    hm2, chan, &(data.NameString), addr, -1);
                if (addr < 0) {
                    return -EINVAL;
                }
                HM2_DBG("Global: %s  RecordType: %02X Datatype: %02X Dir: %02X "
                        "Addr: %04X Length: %i\n",
                        data.NameString,
                        data.RecordType,
                        data.DataType,
                        ...
                        data.DataLength);
                chan->num_globals++;
                chan->globals = (hm2_sserial_data_t *)rtapi_krealloc(
                    chan->globals,
                ...
                addr = hm2_sserial_get_bytes(
                    hm2, chan, &mode.NameString, addr, -1);
...
int hm2_sserial_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    case HM2_GTAG_SMARTSERIAL:
        ...
        if (!hm2_md_is_consistent_or_complain(
                hm2, md_index, 0, 6, 0x40, 0x003C)) {
    ...
    hm2->sserial.instance = (hm2_sserial_instance_t *)hal_malloc(
        hm2->sserial.num_instances * sizeof(hm2_sserial_instance_t));
    ...
    for (port = 0; port < hm2->ioport.num_instances; port++) {
        ddr_reg = 0;
        src_reg = 0;
        for (unsigned port_pin = 0; port_pin < hm2->idrom.port_width;
             port_pin++) {
            pin++;
            if (hm2->pin[pin].sec_tag == HM2_GTAG_SMARTSERIAL ||
                ...
                if ((hm2->pin[pin].sec_pin & 0x0F) >
                    chan_counts[hm2->pin[pin].sec_unit]) {
                    chan_counts[hm2->pin[pin].sec_unit] =
                        (hm2->pin[pin].sec_pin & 0x0F);
                }
                // check if the channel is enabled
                HM2_DBG("port %i sec unit = %i, sec pin = %i mode=%c\n",
                        port,
                        hm2->pin[pin].sec_unit,
                        hm2->pin[pin].sec_pin & 0x0F,
                        hm2->config
                            .sserial_modes[hm2->pin[pin].sec_unit]
                                          [(hm2->pin[pin].sec_pin & 0x0F) - 1]);
                if (hm2->config
                        .sserial_modes[hm2->pin[pin].sec_unit]
                                      [(hm2->pin[pin].sec_pin & 0x0F) - 1] !=
                    'x') {
        ...
        hm2->llio->write(hm2->llio,
                         hm2->ioport.ddr_addr + 4 * port,
                         &ddr_reg,
                         sizeof(rtapi_u32));
        hm2->llio->write(hm2->llio,
                         hm2->ioport.alt_source_addr + 4 * port,
                         &src_reg,
                         sizeof(rtapi_u32));
    }
    ...
    for (i = 0; i < md->instances; i++) {
        ...
        buff = 0x0001; //Clear
        HM2WRITE(inst->command_reg_addr, buff);
        if (hm2_sserial_waitfor(hm2, inst->command_reg_addr, 0xFFFFFFFF, 1007) <
            0) {
            r = -EINVAL;
            goto fail0;
        }
        ...
        for (c = 0; c < inst->num_channels; c++) {
            ...
            if (hm2->sserial.baudrate == 115200 &&
                ...
                if ((user1 & 0xFF00) == 0x4900) { //XiXXboard
                    ...
                    if (getlocal8(hm2, inst, SSLBPMINORREVISIONLOC) < 37) {
                        HM2_PRINT(
                            "Unable to check for 8i20s with firmware < 37 "
                    ...
                    } else if ((r = hm2_sserial_stopstart(
                                    hm2, md, inst, 0xF00)) < 0) {
                        goto fail0;
                    } else {
                        HM2READ(addr1, user1);
            ...
            else if (user1 == 0 || (inst->tag & 1 << c) == 0 ||
        ...
        if (inst->num_remotes > 0) {
            ...
            if ((r = hm2_register_tram_write_region(hm2,
    ...
    for (i = 0; i < hm2->sserial.num_instances; i++) {
        ...
        hm2->llio->write(
            hm2->llio, inst->command_reg_addr, &buff, sizeof(rtapi_u32));
        if (hm2_sserial_waitfor(hm2, inst->command_reg_addr, 0xFFFFFFFF, 51) <
            0) {
            return -EINVAL;
        }
    }
...
int hm2_sserial_setup_channel(hostmot2_t *hm2,
                              hm2_sserial_instance_t *inst,
                              int index)
{
    int r;

    r = hal_pin_s32_newf(HAL_OUT,
                         &(inst->debug),
                         hm2->llio->comp_id,
                         "%s.%i.debug",
                         hm2->llio->name,
    ...
    *inst->run = true;
    ...
    r = hm2_register_tram_read_region(
        hm2, inst->data_reg_addr, sizeof(rtapi_u32), &inst->data_reg_read);
    if (r < 0) {
        HM2_ERR("error registering tram read region for sserial "
                "command register (%d)\n",
                index);
                              ...
                              hm2_module_descriptor_t *md)
{
    ...
    inst->remotes = (hm2_sserial_remote_t *)rtapi_kzalloc(
        inst->num_remotes * sizeof(hm2_sserial_remote_t), RTAPI_GFP_KERNEL);
    ...
    for (c = 0; c < inst->num_channels; c++) {
        if (inst->tag & (1 << c)) {
            ...
            hm2->llio->read(
                hm2->llio, chan->rw_addr[0], &buff, sizeof(rtapi_u32));
            chan->serialnumber = buff;
            HM2_DBG("BoardSerial %08x\n", chan->serialnumber);
            hm2->llio->read(
                hm2->llio, chan->rw_addr[1], chan->raw_name, sizeof(rtapi_u32));
            chan->raw_name[1] |= 0x20; ///lower case
            if (hm2->use_serial_numbers) {
                rtapi_snprintf(chan->name,
                               sizeof(chan->name),
                               "hm2_%2s.%04x",
                               chan->raw_name,
...
int hm2_sserial_read_configs(hostmot2_t *hm2, hm2_sserial_remote_t *chan)
{
    ...
    do {
        ...
        if (rectype == LBP_DATA) {
            c = chan->num_confs++;
            chan->confs = (hm2_sserial_data_t *)rtapi_krealloc(
                chan->confs,
            ...
            addr = hm2_sserial_get_bytes(
                hm2, chan, &(chan->confs[c].UnitString), addr, -1);
            if (addr < 0) {
                return -EINVAL;
            }
            addr = hm2_sserial_get_bytes(
                hm2, chan, &(chan->confs[c].NameString), addr, -1);
            if (addr < 0) {
                return -EINVAL;
            }
            ...
            HM2_DBG(
                "Process: %s  RecordType: %02X Datatype: %02X Dir: %02X Addr: "
                "%04X Length: %i\n",
                chan->confs[c].NameString,
                chan->confs[c].RecordType,
                chan->confs[c].DataType,
                chan->confs[c].DataDir,
                chan->confs[c].ParmAddr,
                chan->confs[c].DataLength);
        } else if (rectype == LBP_MODE) {
            chan->num_modes++;
            m = chan->num_modes - 1;
            chan->modes = (hm2_sserial_mode_t *)rtapi_krealloc(
                chan->modes,
            ...
            addr = hm2_sserial_get_bytes(
                hm2, chan, &(chan->modes[m].NameString), addr, -1);
            if (addr < 0) {
                return -EINVAL;
            }
        }
...
int hm2_sserial_create_pins(hostmot2_t *hm2, hm2_sserial_remote_t *chan)
{
    ...
    for (i = 0; i < chan->num_confs; i++) {
        ...
        case LBP_PAD: break;
        case LBP_BITS:
            chan->pins[i].bit_pins = (hal_bit_t **)hal_malloc(
                chan->confs[i].DataLength * sizeof(hal_bit_t *));
            chan->pins[i].bit_pins_not = (hal_bit_t **)hal_malloc(
                chan->confs[i].DataLength * sizeof(hal_bit_t *));
            chan->pins[i].invert = (hal_bit_t *)hal_malloc(
                chan->confs[i].DataLength * sizeof(hal_bit_t));
        ...
        case LBP_SIGNED:
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_pin_float_new(
                name, data_dir, &(chan->pins[i].float_pin), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return r;
            }
            ...
            r = hal_param_float_new(
                name, HAL_RW, &(chan->pins[i].fullscale), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return r;
            }
            ...
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_param_float_new(
                name, HAL_RW, &(chan->pins[i].maxlim), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return r;
            }
            ...
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_param_float_new(
                name, HAL_RW, &(chan->pins[i].minlim), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return r;
            }
        ...
        case LBP_STREAM:
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_pin_u32_new(
                name, data_dir, &(chan->pins[i].u32_pin), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return r;
            }
        ...
        case LBP_BOOLEAN:
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_pin_bit_new(
                name, data_dir, &(chan->pins[i].boolean), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return r;
            }
            ...
            if (data_dir == HAL_IN) {
                ...
                rtapi_snprintf(name,
                               ...
                               chan->confs[i].NameString);
                r = hal_param_bit_new(
                    name, HAL_RW, chan->pins[i].invert, hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return r;
                }
        ...
        case LBP_ENCODER_H:
            ...
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_pin_s32_new(
                name, HAL_OUT, &(chan->pins[i].s32_pin), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return -EINVAL;
            }
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_pin_s32_new(
                name, HAL_OUT, &(chan->pins[i].s32_pin2), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return -EINVAL;
            }
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_pin_float_new(
                name, HAL_OUT, &(chan->pins[i].float_pin), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return -EINVAL;
            }
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_pin_bit_new(
                name, HAL_IO, &(chan->pins[i].boolean), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return -EINVAL;
            }
            ...
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_pin_bit_new(
                name, HAL_IO, &(chan->pins[i].boolean2), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return -EINVAL;
            }
            ...
            rtapi_snprintf(name,
                           ...
                           chan->confs[i].NameString);
            r = hal_param_u32_new(
                name, HAL_RW, &(chan->pins[i].u32_param), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                return -EINVAL;
            }
...
int hm2_sserial_register_tram(hostmot2_t *hm2, hm2_sserial_remote_t *chan)
{
    ...
    r = hm2_register_tram_read_region(
        hm2, chan->reg_cs_addr, sizeof(rtapi_u32), &chan->reg_cs_read);
    if (r < 0) {
    ...
    for (i = 0; i < chan->num_read_regs; i++) {
        r = hm2_register_tram_read_region(
            hm2, chan->rw_addr[i], sizeof(rtapi_u32), &chan->read[i]);
        if (r < 0) {
            HM2_ERR("error registering tram read region for sserial "
    ...
    r = hm2_register_tram_write_region(
        hm2, chan->reg_cs_addr, sizeof(rtapi_u32), &(chan->reg_cs_write));
    if (r < 0) {
    ...
    for (i = 0; i < chan->num_write_regs; i++) {
        r = hm2_register_tram_write_region(
            hm2, chan->rw_addr[i], sizeof(rtapi_u32), &chan->write[i]);
        if (r < 0) {
            HM2_ERR("error registering tram read region for sserial "
                    "interface %i register (%d)\n",
                    i,
...
int hm2_sserial_update_params(hostmot2_t *hm2,
                              hm2_sserial_instance_t *inst,
                              long period)
{
    ...
    case 1:
        ...
        switch (*inst->state3) {
            ...
            int ret;
        default: HM2_ERR("Unhandled state %i", *inst->state3); return 1;
        case 0:
            HM2_DBG(
                "Checking Param %s datatype %02X\n", g->NameString, p->type);
            switch (p->type) {
            case LBP_SIGNED:
            case LBP_NONVOL_SIGNED:
            ...
            case LBP_NONVOL_FLOAT:
                ...
                case 16: shift = (52 - 10); break;
                case 32: shift = (52 - 23); break;
                case 64: shift = 0;
                }
                if (xlabs((p->s64_param - p->s64_written) >> shift) > 2)
                    break;
                *inst->state2 = 2; // increment indices
                return *inst->state2;
            default:
            ...
            HM2_WARN(
                "Writing value of %s datatype %02X\n", g->NameString, p->type);
            *inst->state3 = 1;
            inst->timer = 20000000;
        ...
        case 1:
            ...
            *inst->state3 = 2;
            inst->timer = 20000000;
            *inst->command_reg_write =
                0xF00 | (1 << r->index); // channel in setup mode
            break;
        case 2: // Unlock Nonvol access
            ...
            if (ret < 0)
                *inst->state3 = 100; // quit and tidy up
            if (p->type == LBP_NONVOL_FLOAT || p->type == LBP_NONVOL_UNSIGNED ||
                p->type == LBP_NONVOL_SIGNED) {
                *r->reg_cs_write = LBPNONVOL_flag + LBPWRITE;
                *r->write[0] = LBPNONVOLEEPROM;
                *inst->command_reg_write =
                    0x1000 | (1 << r->index); // doit command
                inst->timer = 20000000;
                *inst->state3 = 3;
                HM2_PRINT(
                    "A non-volatile smart-serial parameter has been changed\n"
                    "A full power-cycle will be needed before the effect is "
                    "seen\n");
            } else {
                *inst->state3 = 4;
            }
        ...
        case 4: // Now send the data
            ...
            case LBP_NONVOL_SIGNED:
                *r->write[0] = (rtapi_u32)p->s32_param;
                break;
            case LBP_UNSIGNED:
            case LBP_NONVOL_UNSIGNED: *r->write[0] = p->u32_param; break;
            case LBP_FLOAT:
            case LBP_NONVOL_FLOAT:
                if (g->DataLength == sizeof(float) * 8) {
                    ...
                    HM2_ERR("sserial write: LBP_FLOAT of bit-length %i not "
                            "handled\n",
                            g->DataLength);
            ...
            case 8:
                *r->reg_cs_write =
                    WRITE_REM_BYTE_CMD | g->ParmAddr; // Data Address
                break;
            case 16:
                *r->reg_cs_write =
                    WRITE_REM_WORD_CMD | g->ParmAddr; // Data Address
                break;
            case 32:
                *r->reg_cs_write =
                    WRITE_REM_LONG_CMD | g->ParmAddr; // Data Address
                break;
            case 64:
                *r->reg_cs_write =
                    WRITE_REM_DOUBLE_CMD | g->ParmAddr; // Data Address
                break;
            }
        ...
        case 5: // wait for doit clear
            ...
            switch (p->type) {
            case LBP_SIGNED:
            case LBP_NONVOL_SIGNED: p->s32_written = p->s32_param; break;
            case LBP_UNSIGNED:
            case LBP_NONVOL_UNSIGNED: p->u32_written = p->u32_param; break;
            case LBP_FLOAT:
            case LBP_NONVOL_FLOAT: p->float_written = p->float_param; break;
            default: break;
            }
        ...
        for (p = 0; p < chan->num_confs; p++) {
            ...
            if (conf->DataDir & 0xC0) {
                ...
                case LBP_BOOLEAN:
                    buff = 0;
                    if (*pin->boolean ^
                        ((conf->DataDir == LBP_OUT) ? (*pin->invert) : 0)) {
                ...
                case LBP_FLOAT:
                    if (conf->DataLength == sizeof(float) * 8) {
                        ...
                        HM2_ERR_NO_LL(
                            "sserial write: LBP_FLOAT of bit-length %i not "
                            "handled\n",
                            conf->DataLength);
    ...
    for (p = 0; p < chan->num_confs; p++) {
        ...
        if (!(conf->DataDir & 0x80)) {
            ...
            case LBP_UNSIGNED:
                ...
                *pin->float_pin =
                    (buff * pin->fullscale) / ((1 << conf->DataLength) - 1);
                break;
            case LBP_SIGNED:
            ...
            case LBP_ENCODER: {
                ...
                *pin->float_pin =
                    (double)(pin->accum - pin->offset) / pin->fullscale;
                break;
            case LBP_FLOAT:
                if (conf->DataLength == sizeof(float) * 8) {
                    float temp;
                    ...
                    *pin->float_pin = temp;
                    ...
                    HM2_ERR_NO_LL(
                        "sserial read: LBP_FLOAT of bit-length %i not "
                        "handled\n",
                        conf->DataLength);
                    conf->DataType = 0; // Only warn once, then ignore
                }
    ...
    for (i = 0; i < hm2->sserial.num_instances; i++) {
        ...
        for (r = 0; r < hm2->sserial.instance[i].num_remotes; r++) {
            HM2_PRINT("        port %i device %s\n",
                      r,
                      hm2->sserial.instance[i].remotes[r].name);
            HM2_PRINT("             Parameters:\n");
            for (c = 0; c < hm2->sserial.instance[i].remotes[r].num_confs;
                 c++) {
                hm2_sserial_data_t conf =
                    hm2->sserial.instance[i].remotes[r].confs[c];
            ...
            HM2_PRINT("             Globals:\n");
            for (g = 0; g < hm2->sserial.instance[i].remotes[r].num_globals;
                 g++) {
                hm2_sserial_data_t conf =
                    hm2->sserial.instance[i].remotes[r].globals[g];
                HM2_PRINT("                   RecordType = 0x%02x\n",
                          conf.RecordType);
                HM2_PRINT("                   DataLength = 0x%02x\n",
                          conf.DataLength);
            ...
            HM2_PRINT("             Modes:\n");
            for (m = 0; m < hm2->sserial.instance[i].remotes[r].num_modes;
                 m++) {
                hm2_sserial_mode_t mode =
                    hm2->sserial.instance[i].remotes[r].modes[m];
                HM2_PRINT("               RecordType = 0x%02x\n",
                          mode.RecordType);
...
void hm2_sserial_setmode(hostmot2_t *hm2, hm2_sserial_instance_t *inst)
{
    ...
    for (c = 0; c < inst->num_remotes; c++) {
        ...
        if (hm2->config.sserial_modes[i][n] != 'x') {
            // CS - write card mode
            *inst->remotes[c].reg_cs_write =
                (hm2->config.sserial_modes[i][n] - '0') << 24;
        }
    }
}
...
int hm2_sserial_check_local_errors(hostmot2_t *hm2,
                                   hm2_sserial_instance_t *inst)
{
    rtapi_u32 buff;
    int i, r;
...
int hm2_sserial_check_remote_errors(hostmot2_t *hm2,
                                    hm2_sserial_instance_t *inst)
{
    rtapi_u32 buff;
    int i, r;
