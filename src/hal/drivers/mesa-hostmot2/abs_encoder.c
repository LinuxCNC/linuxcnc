...
int hm2_absenc_register_tram(hostmot2_t *hm2)
{
    ...
    if (hm2->absenc.ssi_global_start_addr) {
        if (hm2_register_tram_read_region(hm2,
                                          hm2->absenc.ssi_global_start_addr,
                                          sizeof(rtapi_u32),
    ...
    for (i = 0; i < hm2->absenc.num_chans; i++) {
        ...
        case HM2_GTAG_FABS:
            r += hm2_register_tram_read_region(
                hm2, chan->rw_addr[2], sizeof(rtapi_u32), &chan->read[2]);
            /* Fallthrough */
        case HM2_GTAG_SSI:
            r += hm2_register_tram_read_region(
                hm2, chan->rw_addr[1], sizeof(rtapi_u32), &chan->read[1]);
            r += hm2_register_tram_read_region(
                hm2, chan->rw_addr[0], sizeof(rtapi_u32), &chan->read[0]);
            break;
        case HM2_GTAG_BISS:
            ...
            if (chan->num_read_bits > 32) {
                r += hm2_register_tram_read_region(
                    hm2, chan->rw_addr[0], sizeof(rtapi_u32), &chan->read[1]);
            }
            if (chan->num_read_bits > 64) {
                r += hm2_register_tram_read_region(
                    hm2, chan->rw_addr[0], sizeof(rtapi_u32), &chan->read[2]);
            }
        ...
        if (r < 0) {
            HM2_ERR(
                "error registering tram read region for Absolute encoder\n");
            return -EINVAL;
        }
    }
...
int hm2_absenc_setup_ssi(hostmot2_t *hm2,
                         ...
                         hm2_module_descriptor_t *md)
{
    ...
    chan->rw_addr[0] = md->base_address + (0 * md->register_stride) +
                       chan->index * md->instance_stride;
    chan->rw_addr[1] = md->base_address + (1 * md->register_stride) +
                       chan->index * md->instance_stride;
...
int hm2_absenc_setup_biss(hostmot2_t *hm2,
                          hm2_sserial_remote_t *chan,
                          hm2_module_descriptor_t *md)
{
    ...
    chan->rw_addr[1] = md->base_address + (1 * md->register_stride) +
                       chan->index * md->instance_stride;
    chan->rw_addr[2] = md->base_address + (2 * md->register_stride) +
                       chan->index * md->instance_stride;
    hm2->absenc.biss_global_start_addr =
        md->base_address + (3 * md->register_stride);
    chan->data_written[0] = 0;
...
int hm2_absenc_setup_fabs(hostmot2_t *hm2,
                          hm2_sserial_remote_t *chan,
                          hm2_module_descriptor_t *md)
{
    ...
    chan->rw_addr[2] = md->base_address + (2 * md->register_stride) +
                       chan->index * md->instance_stride;
    chan->reg_cs_addr = md->base_address + (3 * md->register_stride) +
                        chan->index * md->instance_stride;
    chan->data_reg_addr = md->base_address + (4 * md->register_stride) +
                          chan->index * md->instance_stride;
    hm2->absenc.fabs_global_start_addr =
        md->base_address + (5 * md->register_stride);
    chan->data_written[0] = 0;

    if (hal_param_u32_newf(HAL_RW,
                           &(chan->params->u32_param),
                           hm2->llio->comp_id,
                           "%s.filter",
                           chan->name)) {
...
int hm2_absenc_parse_format(hm2_sserial_remote_t *chan,
                            hm2_absenc_format_t *def)
{
    char *AA64 = "%5pbatt_fail%1b%2ppos_invalid%1b%9plow%16l%2pencoder%16h%"
                 "2pcomm%10u%7pcrc%5u";
...
int hm2_absenc_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->absenc.num_chans == 0) { // first time though
        hm2->absenc.clock_frequency = md->clock_freq;
        hm2->absenc.ssi_busy_flags =
            rtapi_kmalloc(sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
        *hm2->absenc.ssi_busy_flags = 0;
        hm2->absenc.biss_busy_flags =
            rtapi_kmalloc(sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
        *hm2->absenc.biss_busy_flags = 0;
        hm2->absenc.fabs_busy_flags =
            rtapi_kmalloc(sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
        *hm2->absenc.fabs_busy_flags = 0;
    }
    ...
    for (index = 0; index < md->instances; index++) {
        ...
        rtapi_list_for_each(ptr, &hm2->config.absenc_formats)
        {
            ...
            if (index == def->index && md->gtag == def->gtag) {
                ...
                // And Params
                if (hal_param_float_newf(HAL_RW,
    ...
    for (i = 0; i < hm2->absenc.num_chans; i++) {
        ...
        case HM2_GTAG_SSI:
            ...
            buff = ((rtapi_u32)(0x10000 * (chan->params->float_param * 1000 /
                   ...
                   (chan->params->timer_num != 0) << 9 | chan->num_read_bits;
            if (buff != chan->data_written[0]) {
                hm2->llio->write(
                    hm2->llio, chan->reg_cs_addr, &buff, sizeof(rtapi_u32));
        ...
        case HM2_GTAG_BISS:
            ...
            if (buff != chan->data_written[0]) {
                ...
                hm2->llio->write(
                    hm2->llio, chan->rw_addr[1], &buff, sizeof(rtapi_u32));
                chan->data_written[0] = buff;
            }
            ...
            if (buff2 != chan->data_written[1]) {
                hm2->llio->write(
                    hm2->llio, chan->rw_addr[2], &buff2, sizeof(rtapi_u32));
        ...
        case HM2_GTAG_FABS:
            ...
            buff = chan->params->timer_num << 12 |
                   (chan->params->timer_num == 0) << 8 |
                   (chan->params->timer_num != 0) << 9 |
                   (buff3 != chan->data_written[2] ||
                    buff2 != chan->data_written[1])
                       << 7;
            if (buff != chan->data_written[0]) {
            ...
            if (buff2 != chan->data_written[1]) {
                hm2->llio->write(
                    hm2->llio, chan->data_reg_addr, &buff, sizeof(rtapi_u32));
                chan->data_written[1] = buff2;
            }
            if (buff3 != chan->data_written[2]) {
                hm2->llio->write(
                    hm2->llio, chan->rw_addr[2], &buff, sizeof(rtapi_u32));
