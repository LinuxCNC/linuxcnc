...
static spidev_port_t spi_ports[PORT_MAX] = {
    {.spix = {.width = 8, .name = "/dev/spidev0.0", .transfer = spi_transfer},
     .fd = -1},
    {.spix = {.width = 8, .name = "/dev/spidev0.1", .transfer = spi_transfer},
     .fd = -1},
    {.spix = {.width = 8, .name = "/dev/spidev1.0", .transfer = spi_transfer},
     .fd = -1},
    {.spix = {.width = 8, .name = "/dev/spidev1.1", .transfer = spi_transfer},
     .fd = -1},
    {.spix = {.width = 8, .name = "/dev/spidev1.2", .transfer = spi_transfer},
     .fd = -1},
};
...
static int
    driver_enabled; // Set to non-zero when spidev_setup() is successfully called
...
static int spidev_detect(const char *dtcs[])
{
    ...
    if (dtcs[0])
        strncpy(spix_driver_spidev.dtc,
                dtcs[0],
                sizeof(spix_driver_spidev.dtc) - 1);
    else
        strncpy(spix_driver_spidev.dtc,
                "spix_spidev-unknown-dtc",
                sizeof(spix_driver_spidev.dtc) - 1);
    if (spix_read_file("/proc/device-tree/model",
                       spix_driver_spidev.model,
                       sizeof(spix_driver_spidev.model)) < 0)
        strncpy(spix_driver_spidev.model,
                "??? Unknown board ???",
                sizeof(spix_driver_spidev.model) - 1);
...
static const spix_port_t *spidev_open(int port, const spix_args_t *args)
{
    ...
    LL_INFO(
        "%s: write clock rate calculated: %u Hz\n", sdp->spix.name, sdp->clkw);
    LL_INFO(
        "%s: read clock rate calculated: %u Hz\n", sdp->spix.name, sdp->clkr);
