...
void hm2_pwmgen_handle_pwm_frequency(hostmot2_t *hm2)
{
    ...
    dds = ((double)hm2->pwmgen.hal->param.pwm_frequency * 65536.0 * 4096.0) /
          (double)hm2->pwmgen.clock_frequency;
    if (dds < 65536) {
    ...
    // try 11 bits
    dds = ((double)hm2->pwmgen.hal->param.pwm_frequency * 65536.0 * 2048.0) /
          (double)hm2->pwmgen.clock_frequency;
    if (dds < 65536) {
        hm2->pwmgen.pwmgen_master_rate_dds_reg = dds;
    ...
    // try 10 bits
    dds = ((double)hm2->pwmgen.hal->param.pwm_frequency * 65536.0 * 1024.0) /
          (double)hm2->pwmgen.clock_frequency;
    if (dds < 65536) {
        hm2->pwmgen.pwmgen_master_rate_dds_reg = dds;
    ...
    // try 9 bits
    dds = ((double)hm2->pwmgen.hal->param.pwm_frequency * 65536.0 * 512.0) /
          (double)hm2->pwmgen.clock_frequency;
    if (dds < 65536) {
        hm2->pwmgen.pwmgen_master_rate_dds_reg = dds;
    ...
    //     PWMFreq = (ClockHigh * DDS) / (65536 * 2^PWMBits)
    hm2->pwmgen.hal->param.pwm_frequency =
        ((double)hm2->pwmgen.clock_frequency * 65535.0) / (65536.0 * 512.0);
...
void hm2_pwmgen_handle_pdm_frequency(hostmot2_t *hm2)
{
    ...
    // can we do it with 12 bits?
    dds = ((double)hm2->pwmgen.hal->param.pdm_frequency * 65536.0) /
          (double)hm2->pwmgen.clock_frequency;
    if (dds == 0) {
        ...
        dds = 1;
        hm2->pwmgen.hal->param.pdm_frequency =
            ((double)hm2->pwmgen.clock_frequency * (double)dds) / 65536.0;
    ...
    //     PulseFreq = (ClockHigh * DDS) / 65536
    hm2->pwmgen.hal->param.pdm_frequency =
        ((double)hm2->pwmgen.clock_frequency * 65535.0) / 65536.0;
...
void hm2_pwmgen_force_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->pwmgen.num_instances; i++) {
        ...
        default: { // unknown pwm mode!  complain and switch to pwm/dir
            ...
            HM2_ERR(
                "supported .output-type values are: %d (PWM & Dir), %d (Up & "
                "Down), %d (PDM & Dir), and %d (Dir & PWM)\n",
    ...
    hm2->llio->write(hm2->llio,
                     ...
                     (hm2->pwmgen.num_instances * sizeof(rtapi_u32)));
    hm2->llio->write(hm2->llio,
    ...
    hm2->llio->write(hm2->llio,
                     ...
                     &hm2->pwmgen.pwmgen_master_rate_dds_reg,
                     sizeof(rtapi_u32));
    hm2->llio->write(hm2->llio,
                     hm2->pwmgen.pdmgen_master_rate_dds_addr,
                     &hm2->pwmgen.pdmgen_master_rate_dds_reg,
                     sizeof(rtapi_u32));
    ...
    for (i = 0; i < hm2->pwmgen.num_instances; i++) {
        hm2->pwmgen.instance[i].written_output_type =
            hm2->pwmgen.instance[i].hal.param.output_type;
        hm2->pwmgen.instance[i].written_offset_mode =
            hm2->pwmgen.instance[i].hal.param.offset_mode;
        hm2->pwmgen.instance[i].written_dither =
            hm2->pwmgen.instance[i].hal.param.dither;
        hm2->pwmgen.instance[i].written_enable =
            *hm2->pwmgen.instance[i].hal.pin.enable;
    }
...
void hm2_pwmgen_write(hostmot2_t *hm2)
{
    ...
    // check output type
    for (i = 0; i < hm2->pwmgen.num_instances; i++) {
        if (hm2->pwmgen.instance[i].hal.param.output_type !=
            hm2->pwmgen.instance[i].written_output_type) {
            goto force_write;
        }
    }
    // check offset mode
    for (i = 0; i < hm2->pwmgen.num_instances; i++) {
        if (hm2->pwmgen.instance[i].hal.param.offset_mode !=
            hm2->pwmgen.instance[i].written_offset_mode) {
            goto force_write;
        }
    }
    ...
    for (i = 0; i < hm2->pwmgen.num_instances; i++) {
        ...
        if (hm2->pwmgen.hal->param.pwm_frequency !=
            hm2->pwmgen.written_pwm_frequency)
            goto force_write;
        if (hm2->pwmgen.hal->param.pdm_frequency !=
            hm2->pwmgen.written_pdm_frequency)
            goto force_write;
...
int hm2_pwmgen_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->config.num_pwmgens > md->instances) {
        HM2_ERR("config.num_pwmgens=%d, but only %d are available, not loading "
                "driver\n",
    ...
    hm2->pwmgen.hal = (hm2_pwmgen_module_global_t *)hal_malloc(
        sizeof(hm2_pwmgen_module_global_t));
    ...
    hm2->pwmgen.instance = (hm2_pwmgen_instance_t *)hal_malloc(
        hm2->pwmgen.num_instances * sizeof(hm2_pwmgen_instance_t));
    ...
    hm2->pwmgen.pwm_mode_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->pwmgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    // FIXME: r hides the r in enclosing function, and it returns the wrong thing
    {
        ...
        for (i = 0; i < hm2->pwmgen.num_instances; i++) {
            // pins
            rtapi_snprintf(
                name, sizeof(name), "%s.pwmgen.%02d.value", hm2->llio->name, i);
            r = hal_pin_float_new(name,
                                  HAL_IN,
                                  &(hm2->pwmgen.instance[i].hal.pin.value),
                                  hm2->llio->comp_id);
            if (r < 0) {
            ...
            rtapi_snprintf(
                name, sizeof(name), "%s.pwmgen.%02d.scale", hm2->llio->name, i);
            r = hal_param_float_new(name,
                                    HAL_RW,
                                    &(hm2->pwmgen.instance[i].hal.param.scale),
                                    hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            *(hm2->pwmgen.instance[i].hal.pin.value) = 0.0;
            ...
            hm2->pwmgen.instance[i].hal.param.output_type =
                HM2_PWMGEN_OUTPUT_TYPE_PWM;
            hm2->pwmgen.instance[i].written_output_type =
                -666; // force an update at the start
            hm2->pwmgen.instance[i].written_enable =
                -666; // force an update at the start
            hm2->pwmgen.instance[i].written_dither =
                -666; // force an update at the start
...
void hm2_pwmgen_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              hm2->pwmgen.clock_frequency,
              hm2_hz_to_mhz(hm2->pwmgen.clock_frequency));
    HM2_PRINT("    version: %d\n", hm2->pwmgen.version);
...
void hm2_pwmgen_prepare_tram_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->pwmgen.num_instances; i++) {
        ...
        scaled_value = *hm2->pwmgen.instance[i].hal.pin.value /
                       hm2->pwmgen.instance[i].hal.param.scale;
        if (scaled_value > 1.0)
            scaled_value = 1.0;
        if (scaled_value < -1.0)
        ...
        if (hm2->pwmgen.instance[i].hal.param.offset_mode == 0) {
            ...
            register_value =
                scaled_value * (double)(((1 << bits) - topdrop)) + (1 << bits);
        }
