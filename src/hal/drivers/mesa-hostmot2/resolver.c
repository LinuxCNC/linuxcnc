...
int hm2_resolver_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    resolvers_per_instance =
        hm2_resolver_get_param(2); // just returns 6 at the moment

    if (hm2->config.num_resolvers > (md->instances * resolvers_per_instance)) {
        HM2_ERR(
            "config.num_resolvers=%d, but only %d are available, not loading "
            "driver\n",
    ...
    hm2->resolver.hal =
        (hm2_resolver_global_t *)hal_malloc(sizeof(hm2_resolver_global_t));
...

    {
        ...
        rtapi_snprintf(
            name, sizeof(name), "%s.resolver.excitation-khz", hm2->llio->name);
        ret = hal_param_float_new(name,
                                  HAL_RW,
        ...
        for (i = 0; i < hm2->resolver.num_resolvers; i++) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.position",
                           hm2->llio->name,
                           i);
            ret =
                hal_pin_float_new(name,
                                  HAL_OUT,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.angle",
                           hm2->llio->name,
                           i);
            ret = hal_pin_float_new(name,
                                    HAL_OUT,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.velocity-rpm",
                           hm2->llio->name,
                           i);
            ret = hal_pin_float_new(
                name,
                HAL_OUT,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.count",
                           hm2->llio->name,
                           i);
            ret = hal_pin_s32_new(name,
                                  HAL_OUT,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.rawcounts",
                           hm2->llio->name,
                           i);
            ret =
                hal_pin_s32_new(name,
                                HAL_OUT,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.reset",
                           hm2->llio->name,
                           i);
            ret = hal_pin_bit_new(name,
                                  HAL_IN,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.scale",
                           hm2->llio->name,
                           i);
            ret = hal_param_float_new(
                name,
                HAL_RW,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.index-divisor",
                           hm2->llio->name,
                           i);
            ret = hal_param_u32_new(
                name,
                HAL_RW,
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.resolver.%02d.use-position-file",
                           hm2->llio->name,
                           i);
            ret = hal_param_bit_new(
                name,
                HAL_RW,
...
void hm2_resolver_process_tram_read(hostmot2_t *hm2, long period)
{
    ...
    for (i = 0; i < hm2->resolver.num_resolvers; i++) {
        ...
        if (res->hal.param.vel_scale == 0.0) {
            HM2_ERR(
                "resolver.%02d.velocity-scale == 0.0, bogus, setting to 1.0\n",
                i);
        ...
        if (res->hal.param
                .use_abs) { // pseudo-absolute behaviour enabled but not initialised
            double new_pos;
            int turns;
            ...
            if (old_pos == 0 &&
                cycle_count++ <
                    5000) { // position.txt not updated yet. Or (small probability) position.txt = 0
            ...
            if (fabs((turns * scale + new_pos) - old_pos) >
                fabs(((turns + 1) * scale + new_pos) - old_pos)) {
                turns += 1;
            } else if (fabs((turns * scale + new_pos) - old_pos) >
                       fabs(((turns - 1) * scale + new_pos) - old_pos)) {
                turns -= 1;
            }
            ...
            res->old_reg =
                hm2->resolver
                    .position_reg[i]; // prevent wrap detection at init.
            res->accum =
                hm2->resolver.position_reg
                    [i]; //necessary to allow rawcounts to still work for commutation
        ...
        if ((res->old_reg > hm2->resolver.position_reg[i]) &&
            (res->old_reg - hm2->resolver.position_reg[i] > 0x80000000)) {
            res->index_cnts++;
        ...
        } else if ((res->old_reg < hm2->resolver.position_reg[i]) &&
                   (hm2->resolver.position_reg[i] - res->old_reg >
                    0x80000000)) {
            res->index_cnts--;
            if (*res->hal.pin.index_enable &&
                (res->index_cnts % res->hal.param.index_div == 0)) {
                res->offset = (res->accum - hm2->resolver.position_reg[i] +
                               0x100000000LL);
                *res->hal.pin.index_enable = 0;
            }
        ...
        *res->hal.pin.position =
            (res->accum - res->offset) / 0x1P32 * res->hal.param.scale;
...
void hm2_resolver_write(hostmot2_t *hm2, long period)
{
    ...
    case 0: // Idle/waiting
        ...
        if (hm2->resolver.hal->param.excitation_khz !=
            hm2->resolver.written_khz) {
    ...
    case 10: // wait for command register clear before setting params
        hm2->llio->read(
            hm2->llio, hm2->resolver.command_addr, &buff, sizeof(rtapi_u32));
        if (buff) {
            timer += period;
            if (timer > 1e9) {
        ...
        hm2->llio->write(
            hm2->llio, hm2->resolver.data_addr, &data_val, sizeof(rtapi_u32));
        hm2->llio->write(
            hm2->llio, hm2->resolver.command_addr, &cmd_val, sizeof(rtapi_u32));
        state = 20;
        timer = 0;
        return;
