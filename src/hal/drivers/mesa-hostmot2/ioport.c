...
int hm2_ioport_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->idrom.io_ports != md->instances) {
        HM2_ERR("IDROM IOPorts is %d but MD IOPort NumInstances is %d, "
                "inconsistent firmware, aborting driver load\n",
                hm2->idrom.io_ports,
                md->instances);
        return -EINVAL;
    }
    ...
    if (r < 0) {
        HM2_ERR("error registering tram read region for IOPort Data register "
                "(%d)\n",
                r);
        goto fail0;
    }
    ...
    if (r < 0) {
        HM2_ERR("error registering tram write region for IOPort Data register "
                "(%d)\n",
                r);
        goto fail0;
    }

    hm2->ioport.ddr_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->ioport.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    hm2->ioport.written_ddr = (rtapi_u32 *)rtapi_kmalloc(
        hm2->ioport.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    hm2->ioport.alt_source_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->ioport.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    hm2->ioport.open_drain_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->ioport.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    // this one's not a real register
    hm2->ioport.written_open_drain = (rtapi_u32 *)rtapi_kmalloc(
        hm2->ioport.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    hm2->ioport.output_invert_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->ioport.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    // this one's not a real register
    hm2->ioport.written_output_invert = (rtapi_u32 *)rtapi_kmalloc(
        hm2->ioport.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
...
static int do_alias(const char *orig_base,
                    ...
                    int (*funct)(const char *, const char *))
{
    ...
    if (ob + sf >= HAL_NAME_LEN) {
        HM2_ERR_NO_LL(
            "ioport: do_alias(): HAL name too long '%s%s' (orig_base(%zu) + "
            "suffix(%zu) >= %u\n",
            orig_base,
            suffix,
            ob,
    ...
    if (ab + sf >= HAL_NAME_LEN) {
        HM2_ERR_NO_LL(
            "ioport: do_alias(): HAL name too long '%s%s' (alias_base(%zu) + "
            "suffix(%zu) >= %u\n",
            alias_base,
            suffix,
            ab,
...
static void count_instances(hostmot2_t *hm2,
                            int pin,
                            int *this_instance,
                            int *total_instances)
{
...
int hm2_ioport_gpio_export_hal(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        ...
        hm2->pin[i].instance =
            (hm2_gpio_instance_t *)hal_malloc(sizeof(hm2_gpio_instance_t));
        ...
        if (hm2->pin[i].gtag != HM2_GTAG_IOPORT &&
            ...
            if (total_instances == 0) {
                ...
                size_t ret = snprintf(alias_base,
                                      sizeof(alias_base),
                                      "%s.%s.%02d.%s",
                                      hm2->llio->name,
                                      gtag_name,
            ...
            r = do_alias(
                orig_base, alias_base, ".invert_output", hal_param_alias);
            if (r < 0) {
                HM2_ERR("Failed to add %s.invert_output alias, continuing\n",
                        orig_base);
            }
            r = do_alias(
                orig_base, alias_base, ".is_opendrain", hal_param_alias);
            if (r < 0) {
...
void hm2_ioport_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              hm2->ioport.clock_frequency,
              hm2_hz_to_mhz(hm2->ioport.clock_frequency));
    HM2_PRINT("    version: %d\n", hm2->ioport.version);
...
static void hm2_ioport_force_write_ddr(hostmot2_t *hm2)
{
    ...
    hm2->llio->write(
        hm2->llio, hm2->ioport.ddr_addr, hm2->ioport.ddr_reg, size);
...
static void hm2_ioport_force_write_output_invert(hostmot2_t *hm2)
{
    ...
    hm2->llio->write(hm2->llio,
                     hm2->ioport.output_invert_addr,
                     hm2->ioport.output_invert_reg,
                     size);
    memcpy(
        hm2->ioport.written_output_invert, hm2->ioport.output_invert_reg, size);
}
    ...
    for (port = 0; port < hm2->ioport.num_instances; port++) {
        for (unsigned port_pin = 0; port_pin < hm2->idrom.port_width;
            ...
            if (hm2->pin[io_pin].direction == HM2_PIN_DIR_IS_OUTPUT) {
                hm2->ioport.ddr_reg[port] |=
                    (1 << port_pin); // set the bit in the ddr register

                // Open Drain Register
                if (hm2->pin[io_pin].instance->hal.param.is_opendrain) {
                    hm2->ioport.open_drain_reg[port] |=
                        (1
                         << port_pin); // set the bit in the open drain register
                } else {
                    hm2->ioport.open_drain_reg[port] &= ~(
                        1
                        << port_pin); // clear the bit in the open drain register
                }

                // Invert Output Register
                if (hm2->pin[io_pin].instance->hal.param.invert_output) {
                    hm2->ioport.output_invert_reg[port] |=
                        (1
                         << port_pin); // set the bit in the output invert register
                } else {
                    hm2->ioport.output_invert_reg[port] &= ~(
                        1
                        << port_pin); // clear the bit in the output invert register
                }
            } else {
                hm2->ioport.open_drain_reg[port] &= ~(
                    1 << port_pin); // clear the bit in the open drain register
                hm2->ioport.ddr_reg[port] &=
                    ~(1 << port_pin); // clear the bit in the ddr register
...
void hm2_ioport_write(hostmot2_t *hm2)
{
    ...
    for (port = 0; port < hm2->ioport.num_instances; port++) {
        if (hm2->ioport.written_open_drain[port] !=
            hm2->ioport.open_drain_reg[port]) {
            hm2_ioport_force_write_open_drain(hm2);
            break;
        }
    ...
    for (port = 0; port < hm2->ioport.num_instances; port++) {
        if (hm2->ioport.written_output_invert[port] !=
            hm2->ioport.output_invert_reg[port]) {
            hm2_ioport_force_write_output_invert(hm2);
            break;
        }
...
void hm2_ioport_gpio_prepare_tram_write(hostmot2_t *hm2)
{
    ...
    for (port = 0; port < hm2->ioport.num_instances; port++) {
        for (unsigned port_pin = 0; port_pin < hm2->idrom.port_width;
            ...
            hm2->ioport.data_write_reg[port] &=
                ~(1 << port_pin); // zero the bit
            if (*(hm2->pin[io_pin].instance->hal.pin.out))
                hm2->ioport.data_write_reg[port] |=
                    (1 << port_pin); // and set if appropriate
        }
    }
}
    ...
    for (port = 0; port < hm2->ioport.num_instances; port++) {
        for (unsigned port_pin = 0; port_pin < hm2->idrom.port_width;
            ...
            hm2->ioport.data_write_reg[port] &=
                ~(1 << port_pin); // zero the bit
            hm2->ioport.data_write_reg[port] |=
                (*(hm2->pin[io_pin].instance->hal.pin.out)
                 << port_pin); // and set it as appropriate
        }
    }
