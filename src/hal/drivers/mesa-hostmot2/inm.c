...
int hm2_inm_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->config.num_inms > md->instances) {
        HM2_ERR("config.num_inms=%d, but only %d are available, not loading "
                "driver\n",
                hm2->config.num_inms,
                md->instances);
        return -EINVAL;
    ...
    hm2->inm.instance = (hm2_inm_instance_t *)hal_malloc(
        hm2->inm.num_instances * sizeof(hm2_inm_instance_t));
    ...
    hm2->inm.control_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->inm.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    if (hm2->inm.control_reg == NULL) {
        HM2_ERR("out of memory!\n");
        r = -ENOMEM;
        goto fail0;
    }
    hm2->inm.mpg_mode_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->inm.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->inm.num_instances * sizeof(rtapi_u32)),
        &hm2->inm.filt_data_reg);
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for inm Filtered Data register "
            "(%d)\n",
            r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->inm.num_instances * sizeof(rtapi_u32)),
        &hm2->inm.raw_data_reg);
    if (r < 0) {
        HM2_ERR("error registering tram read region for inm Raw Data register "
                "(%d)\n",
                r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        (hm2->inm.num_instances * sizeof(rtapi_u32)),
        &hm2->inm.mpg_read_reg);
    if (r < 0) {
        HM2_ERR(
            "error registering tram read region for inm MPG register (%d)\n",
            r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_write_region(
        ...
        &hm2->inm.filter_reg);
    if (r < 0) {
        HM2_ERR("error registering tram write region for inm Filter register "
                "(%d)\n",
                r);
        goto fail1;
    }
...

    {
        ...
        for (i = 0; i < hm2->inm.num_instances; i++) {
            ...
            for (j = 0; j < hm2->num_pins; j++) {
                if (hm2->pin[j].sec_tag == HM2_GTAG_INM &&
            ...
            hm2->llio->read(hm2->llio,
                            hm2->inm.control_addr + (i * md->instance_stride),
                            &temp,
                            sizeof(rtapi_u32));
            ...
            r = hal_param_u32_new(name,
                                  HAL_RW,
                                  &(hm2->inm.instance[i].hal.param.scan_rate),
                                  hm2->llio->comp_id);
            if (r < 0) {
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inm.%02d.slow_scans",
                           hm2->llio->name,
                           i);
            r = hal_param_u32_new(name,
                                  HAL_RW,
                                  &(hm2->inm.instance[i].hal.param.slow_scans),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inm.%02d.fast_scans",
                           hm2->llio->name,
                           i);
            r = hal_param_u32_new(name,
                                  HAL_RW,
                                  &(hm2->inm.instance[i].hal.param.fast_scans),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            if (hm2->inm.instance[i].enc0_present) {
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inm.%02d.enc0_4xmode",
                               hm2->llio->name,
                               i);
                r = hal_param_bit_new(
                    name,
                    HAL_RW,
                    &(hm2->inm.instance[i].hal.param.enc0_mode),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    goto fail1;
            ...
            if (hm2->inm.instance[i].enc1_present) {
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inm.%02d.enc1_4xmode",
                               hm2->llio->name,
                               i);
                r = hal_param_bit_new(
                    name,
                    HAL_RW,
                    &(hm2->inm.instance[i].hal.param.enc1_mode),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    goto fail1;
            ...
            if (hm2->inm.instance[i].enc2_present) {
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inm.%02d.enc2_4xmode",
                               hm2->llio->name,
                               i);
                r = hal_param_bit_new(
                    name,
                    HAL_RW,
                    &(hm2->inm.instance[i].hal.param.enc2_mode),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    goto fail1;
            ...
            if (hm2->inm.instance[i].enc3_present) {
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inm.%02d.enc3_4xmode",
                               hm2->llio->name,
                               i);
                r = hal_param_bit_new(
                    name,
                    HAL_RW,
                    &(hm2->inm.instance[i].hal.param.enc3_mode),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    goto fail1;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.inm.%02d.scan_width",
                           hm2->llio->name,
                           i);
            r = hal_param_u32_new(name,
                                  HAL_RO,
                                  &(hm2->inm.instance[i].hal.param.scan_width),
                                  hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            ...
            for (j = 0; j < hm2->num_pins; j++) {
                if (hm2->pin[j].sec_tag == HM2_GTAG_INM &&
                    ...
                    if ((hm2->pin[j].sec_pin & 0x80) != 00) {
                        HM2_ERR(
                            "Pin Descriptor %d has an inm pin that's not an "
                            "input!\n",
                            j);
                        r = -EINVAL;
                        goto fail0;
                    ...
                    r = hal_pin_bit_new(
                        name,
                        HAL_OUT,
                        &(hm2->inm.instance[i].hal.pin.filt_data[inm_number]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                    ...
                    r = hal_pin_bit_new(
                        name,
                        HAL_OUT,
                        &(hm2->inm.instance[i].hal.pin.raw_data[inm_number]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                    ...
                    r = hal_pin_bit_new(
                        ...
                        HAL_OUT,
                        &(hm2->inm.instance[i]
                              .hal.pin.filt_data_not[inm_number]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                    ...
                    r = hal_pin_bit_new(
                        ...
                        HAL_OUT,
                        &(hm2->inm.instance[i]
                              .hal.pin.raw_data_not[inm_number]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                    ...
                    r = hal_pin_bit_new(
                        name,
                        HAL_IN,
                        &(hm2->inm.instance[i].hal.pin.slow[inm_number]),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
            ...
            if (hm2->inm.instance[i].enc0_present) {
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inm.%02d.enc0-count",
                               hm2->llio->name,
                               i);
                r = hal_pin_s32_new(name,
                                    HAL_OUT,
                                    &(hm2->inm.instance[i].hal.pin.enc0_count),
                                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inm.%02d.enc0-reset",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(name,
                                    HAL_IN,
                                    &(hm2->inm.instance[i].hal.pin.enc0_reset),
                                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
            ...
            if (hm2->inm.instance[i].enc1_present) {
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inm.%02d.enc1-count",
                               hm2->llio->name,
                               i);
                r = hal_pin_s32_new(name,
                                    HAL_OUT,
                                    &(hm2->inm.instance[i].hal.pin.enc1_count),
                                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.inm.%02d.enc1-reset",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(name,
                                    HAL_IN,
                                    &(hm2->inm.instance[i].hal.pin.enc1_reset),
                                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                if (hm2->inm.instance[i].enc2_present) {
                    rtapi_snprintf(name,
                                   sizeof(name),
                                   "%s.inm.%02d.enc2-count",
                                   hm2->llio->name,
                                   i);
                    r = hal_pin_s32_new(
                        name,
                        HAL_OUT,
                        &(hm2->inm.instance[i].hal.pin.enc2_count),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                    ...
                    rtapi_snprintf(name,
                                   sizeof(name),
                                   "%s.inm.%02d.enc2-reset",
                                   hm2->llio->name,
                                   i);
                    r = hal_pin_bit_new(
                        name,
                        HAL_IN,
                        &(hm2->inm.instance[i].hal.pin.enc2_reset),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                ...
                if (hm2->inm.instance[i].enc3_present) {
                    rtapi_snprintf(name,
                                   sizeof(name),
                                   "%s.inm.%02d.enc3-count",
                                   hm2->llio->name,
                                   i);
                    r = hal_pin_s32_new(
                        name,
                        HAL_OUT,
                        &(hm2->inm.instance[i].hal.pin.enc3_count),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                    ...
                    rtapi_snprintf(name,
                                   sizeof(name),
                                   "%s.inm.%02d.enc3-reset",
                                   hm2->llio->name,
                                   i);
                    r = hal_pin_bit_new(
                        name,
                        HAL_IN,
                        &(hm2->inm.instance[i].hal.pin.enc3_reset),
                        hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
...

    {
        ...
        for (i = 0; i < hm2->inm.num_instances; i++) {
            hm2->inm.instance[i].hal.param.scan_rate =
                20000; // 20 KHz = 50 usec/scan
            hm2->inm.instance[i].hal.param.slow_scans =
                500; // 500*50 usec = 25 ms
            hm2->inm.instance[i].hal.param.fast_scans =
                5; //  5*50 usec = 250 usec
            hm2->llio->read(hm2->llio,
                            hm2->inm.mpg_read_addr + (i * md->instance_stride),
                            &rawmpgs,
                            sizeof(rtapi_u32));
            if (hm2->inm.instance[i].enc0_present) {
                hm2->inm.instance[i].prev_enc0_count =
                    (rtapi_s32)((rawmpgs >> 0) & 0x000000FF);
            }
            if (hm2->inm.instance[i].enc1_present) {
                hm2->inm.instance[i].prev_enc1_count =
                    (rtapi_s32)((rawmpgs >> 8) & 0x000000FF);
            }
            if (hm2->inm.instance[i].enc2_present) {
                hm2->inm.instance[i].prev_enc2_count =
                    (rtapi_s32)((rawmpgs >> 16) & 0x000000FF);
            }
            if (hm2->inm.instance[i].enc3_present) {
                hm2->inm.instance[i].prev_enc3_count =
                    (rtapi_s32)((rawmpgs >> 24) & 0x000000FF);
            }
        }
    }
...
void hm2_inm_force_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->inm.num_instances; i++) {
        scanrate = hm2->inm.instance[i].scanwidth *
                   hm2->inm.instance[i].hal.param.scan_rate;
        ...
        hm2->inm.control_reg[i] =
            ...
            (divisor << 6) + (hm2->inm.instance[i].hal.param.fast_scans << 16) +
    ...
    hm2->llio->write(
        hm2->llio, hm2->inm.control_addr, hm2->inm.control_reg, size);
    hm2->llio->write(
        hm2->llio, hm2->inm.mpg_mode_addr, hm2->inm.mpg_mode_reg, size);
...
void hm2_inm_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->inm.num_instances; i++) {
        ...
        if (hm2->inm.instance[i].hal.param.slow_scans > 1023) {
            hm2->inm.instance[i].hal.param.slow_scans = 1023;
            HM2_ERR(
                "inm %d slowscans  must be less than 1023, resetting to %d \n",
                i,
                1023);
        }
        if (hm2->inm.instance[i].hal.param.fast_scans < 1) {
            ...
            HM2_ERR(
                "inm %d fastscans must be greater than 0, resetting to %d \n",
                i,
                1);
        }
        if (hm2->inm.instance[i].hal.param.slow_scans < 1) {
            ...
            HM2_ERR(
                "inm %d slowscans must be greater than 0, resetting to %d \n",
                i,
                1);
        }
        ...
        hm2->inm.control_reg[i] =
            ...
            (divisor << 6) + (hm2->inm.instance[i].hal.param.fast_scans << 16) +
            (hm2->inm.instance[i].hal.param.slow_scans << 22);
        if (hm2->inm.control_reg[i] !=
            hm2->inm.instance[i].written_control_reg) {
            hm2->llio->write(
                hm2->llio, hm2->inm.control_addr, hm2->inm.control_reg, size);
            hm2->inm.instance[i].written_control_reg = hm2->inm.control_reg[i];
        ...
        if (hm2->inm.filter_reg[i] != hm2->inm.instance[i].written_filter_reg) {
            hm2->llio->write(
                hm2->llio, hm2->inm.filter_addr, hm2->inm.filter_reg, size);
        ...
        if (hm2->inm.instance[i].enc0_present) {
            hm2->inm.mpg_mode_reg[i] = hm2->inm.instance[i].hal.param.enc0_mode
                                       << 0;
        }
        if (hm2->inm.instance[i].enc1_present) {
            hm2->inm.mpg_mode_reg[i] |= hm2->inm.instance[i].hal.param.enc1_mode
                                        << 8;
        }
        if (hm2->inm.instance[i].enc2_present) {
            hm2->inm.mpg_mode_reg[i] |= hm2->inm.instance[i].hal.param.enc2_mode
                                        << 16;
        }
        if (hm2->inm.instance[i].enc3_present) {
            hm2->inm.mpg_mode_reg[i] |= hm2->inm.instance[i].hal.param.enc3_mode
                                        << 24;
        }
        if (hm2->inm.mpg_mode_reg[i] !=
            hm2->inm.instance[i].written_mpg_mode_reg) {
            hm2->llio->write(
                hm2->llio, hm2->inm.mpg_mode_addr, hm2->inm.mpg_mode_reg, size);
            hm2->inm.instance[i].written_mpg_mode_reg =
    ...
    for (i = 0; i < hm2->inm.num_instances; i++) {
        for (j = 0; j < hm2->num_pins; j++) {
            if (hm2->pin[j].sec_tag == HM2_GTAG_INM &&
                ...
                inm_number = (hm2->pin[j].sec_pin & 0x7f) - 1;
                *hm2->inm.instance[i].hal.pin.filt_data[inm_number] =
                    (hm2->inm.filt_data_reg[i] >> inm_number) & 1;
                *hm2->inm.instance[i].hal.pin.raw_data[inm_number] =
                    (hm2->inm.raw_data_reg[i] >> inm_number) & 1;
                *hm2->inm.instance[i].hal.pin.filt_data_not[inm_number] =
                    !((hm2->inm.filt_data_reg[i] >> inm_number) & 1);
                *hm2->inm.instance[i].hal.pin.raw_data_not[inm_number] =
                    !((hm2->inm.raw_data_reg[i] >> inm_number) & 1);
            }
        }
        ...
        if (hm2->inm.instance[i].enc0_present) {
            ...
            count_diff =
                (rtapi_s32)raw_count - hm2->inm.instance[i].prev_enc0_count;
            hm2->inm.instance[i].prev_enc0_count =
                hm2->inm.instance[i].prev_enc0_count + count_diff;
            if (count_diff > 128)
                count_diff -= 256;
            if (count_diff < -128)
                count_diff += 256;
            if (*hm2->inm.instance[i].hal.pin.enc0_reset == 0) {
                *hm2->inm.instance[i].hal.pin.enc0_count =
                    *hm2->inm.instance[i].hal.pin.enc0_count + count_diff;
            } else {
        ...
        if (hm2->inm.instance[i].enc1_present) {
            ...
            count_diff =
                (rtapi_s32)raw_count - hm2->inm.instance[i].prev_enc1_count;
            hm2->inm.instance[i].prev_enc1_count =
                hm2->inm.instance[i].prev_enc1_count + count_diff;
            if (count_diff > 128)
                count_diff -= 256;
            if (count_diff < -128)
                count_diff += 256;
            if (*hm2->inm.instance[i].hal.pin.enc1_reset == 0) {
                *hm2->inm.instance[i].hal.pin.enc1_count =
                    *hm2->inm.instance[i].hal.pin.enc1_count + count_diff;
            } else {
        ...
        if (hm2->inm.instance[i].enc2_present) {
            ...
            count_diff =
                (rtapi_s32)raw_count - hm2->inm.instance[i].prev_enc2_count;
            hm2->inm.instance[i].prev_enc2_count =
                hm2->inm.instance[i].prev_enc2_count + count_diff;
            if (count_diff > 128)
                count_diff -= 256;
            if (count_diff < -128)
                count_diff += 256;
            if (*hm2->inm.instance[i].hal.pin.enc2_reset == 0) {
                *hm2->inm.instance[i].hal.pin.enc2_count =
                    *hm2->inm.instance[i].hal.pin.enc2_count + count_diff;
            } else {
        ...
        if (hm2->inm.instance[i].enc3_present) {
            ...
            count_diff =
                (rtapi_s32)raw_count - hm2->inm.instance[i].prev_enc3_count;
            hm2->inm.instance[i].prev_enc3_count =
                hm2->inm.instance[i].prev_enc3_count + count_diff;
            if (count_diff > 128)
                count_diff -= 256;
            if (count_diff < -128)
                count_diff += 256;
            if (*hm2->inm.instance[i].hal.pin.enc3_reset == 0) {
                *hm2->inm.instance[i].hal.pin.enc3_count =
                    *hm2->inm.instance[i].hal.pin.enc3_count + count_diff;
            } else {
...
void hm2_inm_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              ...
              hm2_hz_to_mhz(hm2->inm.clock_frequency));
    HM2_PRINT("    version: %d\n", hm2->inm.version);
