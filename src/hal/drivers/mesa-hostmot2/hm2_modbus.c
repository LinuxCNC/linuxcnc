...
#include <sys/stat.h>
#include <endian.h>
static inline rtapi_u32 be32_to_cpu(rtapi_u32 v)
{
    return be32toh(v);
}
static inline rtapi_u16 be16_to_cpu(rtapi_u16 v)
{
    return be16toh(v);
}
#else
...
static inline bool mtypeiscompound(unsigned mtype)
{
    return mtype >= MBT_ABCD && mtype <= MBT_HGFEDCBA;
}
static inline unsigned mtypeformat(unsigned mtype)
{
    return mtype & MBT_X_MASK;
}
static inline unsigned mtypetype(unsigned mtype)
{
...
static inline bool mtypeisvalid(unsigned mtype)
{
    ...
    return mtypetype(mtype) <= MBT_F &&
           !((mtypeformat(mtype) == MBT_A || mtypeformat(mtype) == MBT_B) &&
             mtypetype(mtype) == MBT_F);
}
static inline unsigned mtypesize(unsigned mtype)
{
    static const rtapi_u8 s[16] = {
        1, 1, 2, 2, 2, 2, 4, 4, 4, 4, 4, 4, 4, 4, 1, 1};
...
#define MSG_DBG(fmt...)                                                        \
    do {                                                                       \
        rtapi_print_msg(RTAPI_MSG_DBG, fmt);                                   \
    } while (0)
#endif
#define MSG_INFO(fmt...)                                                       \
    do {                                                                       \
        rtapi_print_msg(RTAPI_MSG_INFO, fmt);                                  \
    } while (0)
#define MSG_ERR(fmt...)                                                        \
    do {                                                                       \
...
#define MSG_WARN(fmt...)                                                       \
    do {                                                                       \
        rtapi_print_msg(RTAPI_MSG_WARN, fmt);                                  \
    } while (0)
...
typedef struct {
    ...
    hal_u32_t
        rxdelay; // RO Inter frame delay for packet end detection in receive
...
typedef struct {
    ...
    rtapi_u8 data
        [MAX_PKT_LEN]; // PDU: 2-byte header, MAX_MSG_LEN payload, 2-byte CRC
} hm2_modbus_cmd_t;

static inline bool hastimesout(const hm2_modbus_cmd_t *cc)
{
    return 0 != (cc->cmd.flags & MBCCB_CMDF_TIMESOUT);
}
static inline bool hasbcanswer(const hm2_modbus_cmd_t *cc)
{
    return 0 != (cc->cmd.flags & MBCCB_CMDF_BCANSWER);
}
static inline bool hasnoanswer(const hm2_modbus_cmd_t *cc)
{
    return 0 != (cc->cmd.flags & MBCCB_CMDF_NOANSWER);
}
static inline bool hasresend(const hm2_modbus_cmd_t *cc)
{
    return 0 != (cc->cmd.flags & MBCCB_CMDF_RESEND);
}
static inline bool haswflush(const hm2_modbus_cmd_t *cc)
{
    return 0 != (cc->cmd.flags & MBCCB_CMDF_WFLUSH);
}
static inline bool hasdisabled(const hm2_modbus_cmd_t *cc)
{
    return 0 != (cc->cmd.flags & MBCCB_CMDF_DISABLED);
}
static inline bool haspinscale(const hm2_modbus_mbccb_type_t *t)
{
    return 0 != (t->flags & MBCCB_PINF_SCALE);
}
static inline bool haspinclamp(const hm2_modbus_mbccb_type_t *t)
{
    return 0 != (t->flags & MBCCB_PINF_CLAMP);
}

typedef struct {
    char name[HAL_NAME_LEN]; // What we call ourselves (hm2_modbus.X)
    char uart
        [HAL_NAME_LEN]; // The PktUART we attached to (like hm2_5i25.Y.pktuart.Z)
    ...
    hm2_modbus_cmd_t
        *cmds;       // List of commands sent in loop *or* inits at startup
    ...
    unsigned
        maxicharbits; // The max allowed inter-character delay (in bit-times)
...
RTAPI_MP_ARRAY_STRING(mbccbs,
                      MAX_PORTS,
                      "Binary Modbus configuration and command sequence "
...
RTAPI_MP_INT(
    debug,
    "Set message level for debugging purpose [0...5] where 0=none and 5=all "
    "(default: -1; upstream defined)");
...
static unsigned calc_icdelay(unsigned baudrate,
                             unsigned parity,
                             unsigned stopbits,
                             unsigned icdelay)
{
...
//
static void setup_icdelay(hm2_modbus_inst_t *inst,
                          unsigned baudrate,
                          unsigned parity,
                          unsigned stopbits,
                          unsigned icdelay)
...
//
static unsigned calc_ifdelay(hm2_modbus_inst_t *inst,
                             unsigned baudrate,
                             unsigned parity,
                             unsigned stopbits)
{
    if (baudrate > 582000) {
        MSG_WARN("%s: warning: Baudrate > 582000 will make inter-frame timer "
                 "overflow. Setting to maximum.\n",
                 inst->name);
        return 1020;
    }
...
static int send_comms_change(hm2_modbus_inst_t *inst)
{
    ...
    inst->cfg_rx.flags &=
        ~(HM2_PKTUART_CONFIG_PARITYEN | HM2_PKTUART_CONFIG_PARITYODD |
          HM2_PKTUART_CONFIG_STOPBITS2);
    inst->cfg_tx.flags &=
        ~(HM2_PKTUART_CONFIG_PARITYEN | HM2_PKTUART_CONFIG_PARITYODD |
          HM2_PKTUART_CONFIG_STOPBITS2);
    // Set the new flags
    ...
    if (cc->cmd.irxdelay)
        ...
        inst->cfg_rx.ifdelay =
            calc_ifdelay(inst, baudrate, parity, stopbits) - 1;
    if (cc->cmd.itxdelay)
    ...
    if ((r = hm2_pktuart_config(inst->uart, &inst->cfg_rx, &inst->cfg_tx, 1)) <
        0) {
...
static int send_modbus_pkt(hm2_modbus_inst_t *inst)
{
    ...
    if (cc->datalen >= MAX_MSG_LEN + 2) {
        MSG_ERR("%s: error: Data buffer overflow in send_modbus_pkt(), channel "
                "%d\n",
                inst->name,
                inst->cmdidx);
        return -EMSGSIZE;
...
#ifdef DEBUG
    MSG_DBG("Sending cmd=%u to '%s' %i bytes",
            inst->cmdidx,
            inst->uart,
            cc->datalen);
    for (int i = 0; i < cc->datalen; i++)
        MSG_DBG(" 0x%02x", cc->data[i]);
    MSG_DBG("\n");
#endif
...
static void write_flush_cmd(hm2_modbus_inst_t *inst, unsigned idx)
{
    ...
    if (idx >= inst->ncmds) {
        MSG_ERR("%s: error: Command index out of range (%u >= %u) in "
                "write_flush_cmd()\n",
                inst->name,
                idx,
                inst->ncmds);
    ...
    case MBCMD_W_REGISTERS: break; // Only write functions shall be built
    default: return;
    }
    ...
    if (r < 0) {
        ...
        if (!cc->disabled) {
            MSG_ERR(
                "%s: error: Build data frame failed (%d) in write_flush_cmd "
                "for command %d, disabling\n",
                inst->name,
                r,
                idx);
            cc->disabled = 1;
...
{
#ifdef DEBUG_STATE
    if (newstate < 0 || newstate >= STATE_LAST || inst->state < 0 ||
        inst->state > STATE_LAST) {
        MSG_ERR("%s: error: Invalid state detected in set_state() state=%d, "
                "newstate=%d\n",
                inst->name,
                inst->state,
                newstate);
...
static void process(void *arg, long period)
{
...
#ifdef DEBUG_STATE
    {
        if (oldst != inst->state || oldrx != rxstatus || oldtx != txstatus) {
            MSG_DBG(
                "START txstatus=0x%08x rxstatus=0x%08x\n", txstatus, rxstatus);
            oldrx = rxstatus;
            oldtx = txstatus;
            oldst = inst->state;
        }
        ...
        if (rxstatus & HM2_PKTUART_RXMODE_HASDATA) {
            ...
            MSG_WARN(
                "%s: warning: Out-of-band data received. Might not belong to "
                "current command %d\n",
                inst->name,
                inst->cmdidx);
        ...
        retry_next_init:
            ...
            if (0 == cc->cmd.func) {
                ...
                if (0 == cc->cmd.imetacmd) { // This is a delay command
                    ...
                    next_command(
                        inst); // Advance because we stay in STATE_START
                } else {
                    MSG_ERR(
                        "%s: error: init command %u uses unknown meta-command "
                        "%u\n",
                        inst->name,
                        inst->cmdidx,
                        cc->cmd.imetacmd);
        ...
        do {
            ...
            if ((r = build_data_frame(inst)) < 0) {
                ...
                MSG_ERR("%s: error: Build data frame failed command %d, "
                        "disabling\n",
                        inst->name,
                        inst->cmdidx);
                cc->disabled = 1;
            ...
            if (r || hasresend(cc)) { // if data has changed or forced
                if ((r = send_modbus_pkt(inst)) < 0) {
                    ...
                    MSG_ERR("%s: error: Send PDU failed (error %d) command %d, "
                            "resetting\n",
                            inst->name,
                            r,
                            inst->cmdidx);
...
#ifdef DEBUG_STATE
    {
        if (oldst != inst->state || oldrx != rxstatus || oldtx != txstatus) {
            MSG_DBG(
                "WAIT_FOR_TIMEOUT RX 0x%08x TX 0x%08x\n", txstatus, rxstatus);
            oldrx = rxstatus;
            oldtx = txstatus;
            oldst = inst->state;
        }
        ...
        if (rxstatus & HM2_PKTUART_RXMODE_HASDATA) {
            ...
            set_state(inst, STATE_WAIT_FOR_DATA_FRAME);
            MSG_WARN(
                "%s: warning: Out-of-band data received in WAIT_FOR_TIMEOUT in "
                "command %d\n",
                inst->name,
                inst->cmdidx);
            // Jump to the next state. There is no point in delaying queueing
...
#ifdef DEBUG_STATE
    {
        ...
        if (!(txstatus & HM2_PKTUART_TXMODE_TXBUSY)) {
            ...
            if ((!cc->cmd.mbid && !hasbcanswer(cc)) ||
                (cc->cmd.mbid && hasnoanswer(cc))) {
...
#ifdef DEBUG_STATE
    {
        ...
        if ((r = hm2_pktuart_queue_get_frame_sizes(inst->uart, inst->fsizes)) <
            0)
            MSG_ERR(
                "%s: error: hm2_pktuart_queue_get_frame_sizes() returned an "
                "error: %d\n",
                inst->name,
                r);
    ...
    fetch_more_data:
#ifdef DEBUG_STATE
        MSG_DBG(
            "FETCH_(MORE_)DATA Index %u Frames 0x%04x 0x%04x 0x%04x 0x%04x\n",
            inst->frameidx,
            inst->fsizes[0],
            inst->fsizes[1],
            inst->fsizes[2],
        ...
        if (frsize &
            (HM2_PKTUART_RCR_ERROROVERRUN | HM2_PKTUART_RCR_ERRORSTARTBIT |
             HM2_PKTUART_RCR_ERRORPARITY)) {
            const char *eor =
                (frsize & HM2_PKTUART_RCR_ERROROVERRUN) ? " OVERRUN" : "";
            const char *esb =
                (frsize & HM2_PKTUART_RCR_ERRORSTARTBIT) ? " STARTBIT" : "";
            const char *epa =
                (frsize & HM2_PKTUART_RCR_ERRORPARITY) ? " PARITY" : "";
            MSG_WARN(
                "%s: warning: reply to command %u had an error%s%s%s set in "
                "fsize (0x%04x), dropping\n",
                inst->name,
                inst->cmdidx,
                eor,
                esb,
                epa,
                frsize & (HM2_PKTUART_RCR_ERROROVERRUN |
                          HM2_PKTUART_RCR_ERRORSTARTBIT |
                          HM2_PKTUART_RCR_ERRORPARITY));
            set_error(inst, EIO);
            force_resend(inst);
            queue_reset(inst);
        }
        if (HM2_PKTUART_RCR_NBYTES_VAL(frsize) >
            MAX_PKT_LEN) { // indicates an overrun
            MSG_WARN(
                "%s: warning: reply to command %u had packet size overrun (%u "
                "> %u), dropping\n",
                inst->name,
                inst->cmdidx,
                HM2_PKTUART_RCR_NBYTES_VAL(frsize),
                MAX_PKT_LEN);
        ...
        if (inst->maxicharbits &&
            HM2_PKTUART_RCR_ICHARBITS_VAL(frsize) > inst->maxicharbits) {
            MSG_WARN(
                "%s: warning: reply to command %u had too long inter-character "
                "delay (%u > %u), dropping\n",
                inst->name,
                inst->cmdidx,
                HM2_PKTUART_RCR_ICHARBITS_VAL(frsize),
                inst->maxicharbits);
        ...
        inst->rxdata[0] =
            0; // This will fail the parse packet if the read did not resolve
        r = hm2_pktuart_queue_read_data(
            inst->uart, inst->rxdata, HM2_PKTUART_RCR_NBYTES_VAL(frsize));
        if (r < 0)
            MSG_ERR(
                "%s: error: hm2_pktuart_queue_read_data() returned an error: "
                "%d\n",
                inst->name,
                r); // What to do...
    ...
    case STATE_HANDLE_DATA:
        ...
        if (inst->frameidx < 16 - 1 &&
            HM2_PKTUART_RCR_NBYTES_VAL(inst->fsizes[++(inst->frameidx)]) > 0) {
            // FIXME: We should never get multiple replies. Only one
            // outstanding packet should be waiting for a reply.
            MSG_WARN("%s: warning: Multiple data packets as reply? Maybe "
                     "out-of-band? Command %d\n",
                     inst->name,
                     inst->cmdidx);
            inst->ignoredata = 1;
    ...
    default:
        MSG_ERR(
            "%s: error: Unknown state (%d) in process(), setting START state\n",
            inst->name,
            inst->state);
        *(inst->hal->fault) = 1;
...
static inline int ch_append8(hm2_modbus_cmd_t *cc, rtapi_u8 v)
{
    ...
    if (cc->datalen >= MAX_MSG_LEN + 2)
        return -EMSGSIZE;
    if (cc->data[cc->datalen] != v)
        r = 1; // flag data changed
...
static inline int
ch_append16_sw(hm2_modbus_cmd_t *cc, rtapi_u16 v, bool reverse)
{
    int r = 0;
    if (reverse) {
...
static inline int
ch_append32(hm2_modbus_cmd_t *cc, const mb_types32_u *v, unsigned tidx)
{
    int r = 0;
...
static inline int
ch_append64(hm2_modbus_cmd_t *cc, const mb_types64_u *v, unsigned tidx)
{
    int r = 0;
    unsigned idx = mtypeformat(cc->typeptr[tidx].mtype);
...
static int map_u(hm2_modbus_cmd_t *cc, rtapi_u64 v, unsigned tidx)
{
    ...
    case MBT_A:
    case MBT_B:
        if (haspinclamp(&cc->typeptr[tidx]) && v > RTAPI_UINT8_MAX)
            v = RTAPI_UINT8_MAX;
    ...
    case MBT_AB:
    case MBT_BA:
        if (haspinclamp(&cc->typeptr[tidx]) && v > RTAPI_UINT16_MAX)
            v = RTAPI_UINT16_MAX;
...
static int map_s(hm2_modbus_cmd_t *cc, rtapi_s64 v, unsigned tidx)
{
    ...
    case MBT_A:
    case MBT_B:
        if (haspinclamp(&cc->typeptr[tidx]) && v > RTAPI_INT8_MAX)
            v = RTAPI_INT8_MAX;
        if (haspinclamp(&cc->typeptr[tidx]) && v < RTAPI_INT8_MIN)
            v = RTAPI_INT8_MIN;
        CHK_RV(ch_append16_sw(cc, (rtapi_u16)v & 0xff, fmt == MBT_B));
        break;
    ...
    case MBT_AB:
    case MBT_BA:
        if (haspinclamp(&cc->typeptr[tidx]) && v > RTAPI_INT16_MAX)
            v = RTAPI_INT16_MAX;
        if (haspinclamp(&cc->typeptr[tidx]) && v < RTAPI_INT16_MIN)
            v = RTAPI_INT16_MIN;
    ...
    case MBT_CDAB:
    case MBT_DCBA:
        if (haspinclamp(&cc->typeptr[tidx]) && v > RTAPI_INT32_MAX)
            v = RTAPI_INT32_MAX;
        if (haspinclamp(&cc->typeptr[tidx]) && v < RTAPI_INT32_MIN)
            v = RTAPI_INT32_MIN;
        v32.s = (rtapi_s32)v;
        CHK_RV(ch_append32(cc, &v32, tidx));
        break;
...
static int map_f(hm2_modbus_cmd_t *cc, double v, unsigned tidx)
{
    ...
    case MBT_BA:
        ...
        if (!(v64.u & 0x7ff0000000000000ul)) {
            ...
            if (v > +65504.0)
                v = +65504.0; // Clamp instead of +/-inf
            if (v < -65504.0)
                v = -65504.0;
            w |= ((((v64.u >> 52) & 0x7ff) - 1023 + 15) << 10) &
                 0x7c00;                        // exponent (b14..10)
    ...
    case MBT_CDAB:
    case MBT_DCBA:
        if (haspinclamp(&cc->typeptr[tidx]) && v > +FLT_MAX)
            v = +FLT_MAX;
        if (haspinclamp(&cc->typeptr[tidx]) && v < -FLT_MAX)
            v = -FLT_MAX;
        v32.f = (float)v;
...
{
    if (v > (double)RTAPI_INT64_MAX)
        return RTAPI_INT64_MAX;
    if (v < (double)RTAPI_INT64_MIN)
...
static int build_data_frame(hm2_modbus_inst_t *inst)
{
    ...
    case MBCMD_W_REGISTER: // Write single register
        ...
        case HAL_BIT: CHK_RV(map_u(cc, hal->pins[p].pin->b ? 1 : 0, 0)); break;
        case HAL_U32:
            switch (mtypetype(cc->typeptr[0].mtype)) {
            case MBT_U: CHK_RV(map_u(cc, hal->pins[p].pin->u, 0)); break;
            case MBT_S:
                CHK_RV(map_s(cc, map_us(hal->pins[p].pin->u), 0));
                break;
            case MBT_F:
                CHK_RV(map_f(cc, map_uf(hal->pins[p].pin->u), 0));
                break;
            }
            break;
        case HAL_U64:
            ...
            case MBT_S:
                CHK_RV(map_s(cc, map_us(hal->pins[p].pin->lu), 0));
                break;
            case MBT_F:
                CHK_RV(map_f(cc, map_uf(hal->pins[p].pin->lu), 0));
                break;
            }
            break;
        case HAL_S32:
            if (!haspinscale(&cc->typeptr[0])) {
                switch (mtypetype(cc->typeptr[0].mtype)) {
                case MBT_U:
                    CHK_RV(map_u(cc, map_su(hal->pins[p].pin->s), 0));
                    break;
                case MBT_S: CHK_RV(map_s(cc, hal->pins[p].pin->s, 0)); break;
                case MBT_F:
                    CHK_RV(map_f(cc, map_sf(hal->pins[p].pin->s), 0));
                    break;
                }
            } else {
                val64.f = (real_t)((rtapi_s64)hal->pins[p].pin->s -
                                   hal->pins[p].offset->s) *
                          *(hal->pins[p].scale);
                switch (mtypetype(cc->typeptr[0].mtype)) {
        ...
        case HAL_S64:
            if (!haspinscale(&cc->typeptr[0])) {
                switch (mtypetype(cc->typeptr[0].mtype)) {
                case MBT_U:
                    CHK_RV(map_u(cc, map_su(hal->pins[p].pin->ls), 0));
                    break;
                case MBT_S: CHK_RV(map_s(cc, hal->pins[p].pin->ls, 0)); break;
                case MBT_F:
                    CHK_RV(map_f(cc, map_sf(hal->pins[p].pin->ls), 0));
                    break;
                }
            } else {
                val64.f =
                    (real_t)(hal->pins[p].pin->ls - hal->pins[p].offset->ls) *
                    *(hal->pins[p].scale);
                switch (mtypetype(cc->typeptr[0].mtype)) {
                case MBT_U: CHK_RV(map_u(cc, map_fu(val64.f), 0)); break;
                case MBT_S: CHK_RV(map_s(cc, map_fs(val64.f), 0)); break;
        ...
        case HAL_FLOAT:
            if (!haspinscale(&cc->typeptr[0])) {
                switch (mtypetype(cc->typeptr[0].mtype)) {
                case MBT_U:
                    CHK_RV(map_u(cc, map_fu(hal->pins[p].pin->f), 0));
                    break;
                case MBT_S:
                    CHK_RV(map_s(cc, map_fs(hal->pins[p].pin->f), 0));
                    break;
                case MBT_F: CHK_RV(map_f(cc, hal->pins[p].pin->f, 0)); break;
                }
            } else {
                val64.f = (hal->pins[p].pin->f - hal->pins[p].offset->f) *
                          *(hal->pins[p].scale);
                switch (mtypetype(cc->typeptr[0].mtype)) {
                case MBT_U: CHK_RV(map_u(cc, map_fu(val64.f), 0)); break;
                case MBT_S: CHK_RV(map_s(cc, map_fs(val64.f), 0)); break;
    ...
    case MBCMD_W_COILS: // Write multiple coils
        ...
        for (unsigned i = 0; i < cc->cmd.cpincnt; i++) {
            if (hal->pins[p++].pin->b)
                acc |= 1u << (i % 8);
            if ((i % 8) == 7 ||
                i == cc->cmd.cpincnt - 1u) { // time for the next byte
    ...
    case MBCMD_W_REGISTERS: // write multiple holding registers
        ...
        for (unsigned i = 0; i < cc->cmd.cpincnt; i++) {
            ...
            case HAL_U32:
                ...
                case MBT_S:
                    CHK_RV(map_s(cc, map_us(hal->pins[p].pin->u), i));
                    break;
                case MBT_F:
                    CHK_RV(map_f(cc, map_uf(hal->pins[p].pin->u), i));
                    break;
                }
                break;
            case HAL_U64:
                ...
                case MBT_S:
                    CHK_RV(map_s(cc, map_us(hal->pins[p].pin->lu), i));
                    break;
                case MBT_F:
                    CHK_RV(map_f(cc, map_uf(hal->pins[p].pin->lu), i));
                    break;
                }
                break;
            case HAL_S32:
                if (!haspinscale(&cc->typeptr[i])) {
                    switch (mtypetype(cc->typeptr[i].mtype)) {
                    case MBT_U:
                        CHK_RV(map_u(cc, map_su(hal->pins[p].pin->s), i));
                        break;
                    case MBT_S:
                        CHK_RV(map_s(cc, hal->pins[p].pin->s, i));
                        break;
                    case MBT_F:
                        CHK_RV(map_f(cc, map_sf(hal->pins[p].pin->s), i));
                        break;
                    }
                } else {
                    val64.f = (real_t)((rtapi_s64)hal->pins[p].pin->s -
            ...
            case HAL_S64:
                if (!haspinscale(&cc->typeptr[i])) {
                    switch (mtypetype(cc->typeptr[i].mtype)) {
                    case MBT_U:
                        CHK_RV(map_u(cc, map_su(hal->pins[p].pin->ls), i));
                        break;
                    case MBT_S:
                        CHK_RV(map_s(cc, hal->pins[p].pin->ls, i));
                        break;
                    case MBT_F:
                        CHK_RV(map_f(cc, map_sf(hal->pins[p].pin->ls), i));
                        break;
                    }
                } else {
                    val64.f = (real_t)(hal->pins[p].pin->ls -
                                       hal->pins[p].offset->ls) *
                              *(hal->pins[p].scale);
                    switch (mtypetype(cc->typeptr[i].mtype)) {
                    case MBT_U: CHK_RV(map_u(cc, map_fu(val64.f), i)); break;
                    case MBT_S: CHK_RV(map_s(cc, map_fs(val64.f), i)); break;
            ...
            case HAL_FLOAT:
                if (!haspinscale(&cc->typeptr[i])) {
                    switch (mtypetype(cc->typeptr[i].mtype)) {
                    case MBT_U:
                        CHK_RV(map_u(cc, map_fu(hal->pins[p].pin->f), i));
                        break;
                    case MBT_S:
                        CHK_RV(map_s(cc, map_fs(hal->pins[p].pin->f), i));
                        break;
                    case MBT_F:
                        CHK_RV(map_f(cc, hal->pins[p].pin->f, i));
                        break;
                    }
                } else {
                    val64.f = (hal->pins[p].pin->f - hal->pins[p].offset->f) *
...
static int test_bytecount(const hm2_modbus_inst_t *inst,
                          const rtapi_u8 *bytes,
                          unsigned pkt_len,
                          unsigned mini,
                          unsigned maxi)
{
    if (bytes[2] < mini || bytes[2] > maxi) {
        MSG_ERR(
            "%s: error: Invalid byte count %u in received PDU not in [%u, %u], "
            "cmd %u\n",
            inst->name,
            bytes[2],
            mini,
    ...
    if (mini > 1 && 0 != bytes[2] % mini) {
        MSG_ERR(
            "%s: error: Invalid odd byte count %u in received PDU, expected "
            "mod %u, cmd %u\n",
            inst->name,
            bytes[2],
            mini,
    ...
    if (bytes[2] > pkt_len - 5) {
        MSG_ERR(
            "%s: error: Byte count in received PDU too large (%u > %u), cmd "
            "%u\n",
            inst->name,
            bytes[2],
            pkt_len - 4,
    ...
    if (bytes[2] < pkt_len - 5) {
        MSG_ERR(
            "%s: error: Byte count in received PDU too small (%u < %u), cmd "
            "%u\n",
            inst->name,
            bytes[2],
            pkt_len - 4,
...
static inline rtapi_u64 mask_mbtsize(unsigned mtype, rtapi_u64 v)
{
    ...
    case MBT_A:
    case MBT_B: return v & 0xff;
    case MBT_AB:
    case MBT_BA: return v & 0xffff;
    case MBT_ABCD:
    case MBT_BADC:
    case MBT_CDAB:
    case MBT_DCBA: return v & 0xffffffff;
    default: return v;
    }
}

static inline rtapi_u32
unmap32_uu(const hm2_modbus_cmd_t *cc, rtapi_u64 v, unsigned tidx)
{
    if (haspinclamp(&cc->typeptr[tidx]) && v > RTAPI_UINT32_MAX)
...
static inline rtapi_u32
unmap32_us(const hm2_modbus_cmd_t *cc, rtapi_s64 v, unsigned tidx)
{
...
static inline rtapi_u32
unmap32_uf(const hm2_modbus_cmd_t *cc, double v, unsigned tidx)
{
    if (haspinclamp(&cc->typeptr[tidx])) {
        if (v > (double)RTAPI_INT32_MAX)
            return RTAPI_INT32_MAX;
        if (v < 0.0)
            return 0;
    }
...
static inline rtapi_s32
unmap32_su(const hm2_modbus_cmd_t *cc, rtapi_u64 v, unsigned tidx)
{
...
static inline rtapi_s32
unmap32_ss(const hm2_modbus_cmd_t *cc, rtapi_s64 v, unsigned tidx)
{
    if (haspinclamp(&cc->typeptr[tidx])) {
        if (v > (rtapi_s64)RTAPI_INT32_MAX)
            return RTAPI_INT32_MAX;
        if (v < (rtapi_s64)RTAPI_INT32_MIN)
...
static inline rtapi_s32
unmap32_sf(const hm2_modbus_cmd_t *cc, double v, unsigned tidx)
{
    if (haspinclamp(&cc->typeptr[tidx])) {
        if (v > (double)RTAPI_INT32_MAX)
            return RTAPI_INT32_MAX;
        if (v < (double)RTAPI_INT32_MIN)
            return RTAPI_INT32_MIN;
    }
    return (rtapi_s32)v;
}
...
unmap64_us(const hm2_modbus_cmd_t *cc, rtapi_s64 v, unsigned tidx)
{
...
static inline rtapi_u64
unmap64_uf(const hm2_modbus_cmd_t *cc, double v, unsigned tidx)
{
    if (haspinclamp(&cc->typeptr[tidx])) {
        if (v > (double)RTAPI_INT64_MAX)
            return RTAPI_INT64_MAX;
...
unmap64_su(const hm2_modbus_cmd_t *cc, rtapi_u64 v, unsigned tidx)
{
...
static inline rtapi_s64
unmap64_sf(const hm2_modbus_cmd_t *cc, double v, unsigned tidx)
{
    if (haspinclamp(&cc->typeptr[tidx])) {
        if (v > (double)RTAPI_INT64_MAX)
            return RTAPI_INT64_MAX;
...
static int parse_data_frame(hm2_modbus_inst_t *inst)
{
    ...
    if (rxcount > sizeof(bytes)) {
        MSG_ERR("%s: error: Received PDU larger than buffer (%u > %zu), "
                "truncating\n",
                inst->name,
                rxcount,
                sizeof(bytes));
    ...
    if (bytes[0] != cc->cmd.mbid) {
        MSG_ERR(
            "%s: error: Modbus device address mismatch: got 0x%02x, expected "
            "0x%02x\n",
            inst->name,
            bytes[0],
            cc->cmd.mbid);
    ...
    if ((bytes[1] & 0x7f) != cc->cmd.func) {
        MSG_ERR(
            "%s: error: Call/response function number mismatch: got 0x%02x, "
            "expected 0x%02x)\n",
            inst->name,
            cc->cmd.func,
            bytes[1]);
    ...
    rtapi_u16 retcrc =
        ((rtapi_u16)bytes[rxcount - 1] << 8) | bytes[rxcount - 2];
    if (retcrc != checksum) {
        MSG_ERR(
            "%s: error: Modbus checksum error: got 0x%04x, expected 0x%04x\n",
            inst->name,
            retcrc,
            checksum);
    ...
    case MBCMD_R_REGISTERS: // read holding registers
        ...
        if (bytes[2] != cc->cmd.cregcnt * 2) {
            MSG_ERR("%s: error: Cmd response data length %u was expected to be "
                    "%u\n",
                    inst->name,
                    (unsigned)bytes[2],
                    cc->cmd.cregcnt * 2);
        ...
        for (int i = 0; i < cc->cmd.cpincnt; i++) {
            ...
            case MBT_AB: // Always sign-extended
                val64.s =
                    256 * (rtapi_s64)(rtapi_s8)bytes[pos] + bytes[pos + 1];
                break;
            case MBT_BA: // Always sign-extended
                val64.s =
                    256 * (rtapi_s64)(rtapi_s8)bytes[pos + 1] + bytes[pos];
                break;
            ...
            default:
                ...
                MSG_ERR("%s: error: Invalid mtype %u for command %d\n",
                        inst->name,
                        mtypeformat(cc->typeptr[i].mtype),
                        inst->cmdidx);
                cc->disabled = 1;
            ...
            if (MBT_F == mtypetype(cc->typeptr[i].mtype)) {
                ...
                case 1:
                    ...
                    if (!(val64.u & 0x7c00)) {
                        ...
                        u |=
                            (rtapi_u64)(val64.u &
                                        0x3ff); // Mantissa --> NaN in lower bits
                    } else {
                        u |= (rtapi_u64)((((val64.u & 0x7c00) >> 10) - 15 +
                                          1023) &
                                         0x7ff)
                             << 52; // Exponent
                        u |= (rtapi_u64)(val64.u & 0x3ff)
                             << (52 - 10); // Mantissa
                    }
            ...
            case HAL_BIT:
                hal->pins[p].pin->b =
                    0 != val64.u; // Zero maps to false, anything else to true
                break;
            case HAL_U32:
                switch (mtypetype(cc->typeptr[i].mtype)) {
                case MBT_U:
                    hal->pins[p].pin->u = unmap32_uu(cc, val64.u, i);
                    break;
                case MBT_S:
                    hal->pins[p].pin->u = unmap32_us(cc, val64.s, i);
                    break;
                case MBT_F:
                    hal->pins[p].pin->u = unmap32_uf(cc, val64.f, i);
                    break;
                }
                break;
            case HAL_S32:
                switch (mtypetype(cc->typeptr[i].mtype)) {
                case MBT_U:
                    hal->pins[p].pin->s = unmap32_su(cc, val64.u, i);
                    break;
                case MBT_S:
                    hal->pins[p].pin->s = unmap32_ss(cc, val64.s, i);
                    break;
                case MBT_F:
                    hal->pins[p].pin->s = unmap32_sf(cc, val64.f, i);
                    break;
                }
                if (haspinscale(&cc->typeptr[i])) {
                    switch (mtypetype(cc->typeptr[i].mtype)) {
                    case MBT_U:
                        *(hal->pins[p].scaled) =
                            (real_t)(rtapi_s64)(val64.u -
                                                hal->pins[p].offset->lu) *
                            *(hal->pins[p].scale);
                        break;
                    case MBT_S:
                        *(hal->pins[p].scaled) =
                            (real_t)(val64.s - hal->pins[p].offset->ls) *
                            *(hal->pins[p].scale);
                        break;
                    case MBT_F:
                        *(hal->pins[p].scaled) =
                            (val64.f - hal->pins[p].offset->f) *
                            *(hal->pins[p].scale);
                        break;
                    }
                }
            ...
            case HAL_U64:
                ...
                case MBT_S:
                    hal->pins[p].pin->lu = unmap64_us(cc, val64.s, i);
                    break;
                case MBT_F:
                    hal->pins[p].pin->lu = unmap64_uf(cc, val64.f, i);
                    break;
                }
                break;
            case HAL_S64:
                switch (mtypetype(cc->typeptr[i].mtype)) {
                case MBT_U:
                    hal->pins[p].pin->ls = unmap64_su(cc, val64.u, i);
                    break;
                case MBT_S: hal->pins[p].pin->ls = val64.s; break;
                case MBT_F:
                    hal->pins[p].pin->ls = unmap64_sf(cc, val64.f, i);
                    break;
                }
                if (haspinscale(&cc->typeptr[i])) {
                    switch (mtypetype(cc->typeptr[i].mtype)) {
    ...
    case (128 + MBCMD_W_REGISTERS): // 16 + error bit
        ...
        if (bytes[2] >= (sizeof(error_codes) / sizeof(*error_codes))) {
            MSG_ERR("%s: error: Modbus error response cmd %u function %u with "
                    "invalid/unknown error code %u\n",
                    inst->name,
                    inst->cmdidx,
                    bytes[1] & 0x7f,
                    bytes[2]);
        } else {
            MSG_ERR("%s: error: Modbus error response cmd %u function %u error "
                    "'%s' (%u)\n",
                    inst->name,
                    inst->cmdidx,
                    bytes[1] & 0x7f,
    ...
    default:
        MSG_ERR(
            "%s: error: Unknown or unsupported Modbus function code: 0x%02x "
            "(mbid=%u, cmd=%u)\n",
            inst->name,
            bytes[1],
            bytes[0],
...
static rtapi_u16 crc_modbus(const rtapi_u8 *buffer, size_t len)
{
    static const rtapi_u8 crctabhi[256] = {
    ...
    static const rtapi_u8 crctablo[256] = {
...
#if !defined(__KERNEL__)
// Userspace file read
static ssize_t read_mbccb(const hm2_modbus_inst_t *inst,
                          const char *fname,
                          hm2_modbus_mbccb_header_t **pmbccb)
{
    if (!pmbccb)
        return -EINVAL;
    ...
    *pmbccb = rtapi_kzalloc(sb.st_size, RTAPI_GFP_KERNEL);
    if (!*pmbccb) {
        MSG_ERR(
            "%s: error: Failed to allocate %zd bytes memory for mbccb buffer\n",
            inst->name,
            (ssize_t)sb.st_size);
        close(fd);
        return -ENOMEM;
...
retry_read:
    ...
    if (err != (ssize_t)sb.st_size) {
        MSG_ERR("%s: error: Read %zd bytes instead of %zd bytes from '%s', "
                "aborting\n",
                inst->name,
                err,
                (ssize_t)sb.st_size,
...
static int load_mbccb(hm2_modbus_inst_t *inst, const char *fname)
{
    ...
    static const rtapi_u8 signature[8] = {
        'M', 'e', 's', 'a', 'M', 'B', '0', '1'};
    if (memcmp(signature, mbccb->sig, sizeof(signature))) {
        ...
        MSG_ERR("%s: error: Invalid signature in mbccb file: '%s' (expected "
                "'MesaMB01')\n",
                inst->name,
                buf);
        MSG_ERR(
            "%s: error: Have you compiled the mbccs source into a binary mbccb "
            "file using mesambccc?\n",
            inst->name);
        goto errout;
    }
    ...
    if (mbccb->baudrate < 1200) {
        MSG_WARN(
            "%s: warning: Mbccb baudrate %u slower than 1200 baud. May fail to "
            "setup\n",
            inst->name,
            mbccb->baudrate);
    } else if (mbccb->baudrate > 460800) {
        MSG_WARN(
            "%s: warning: Mbccb baudrate %u faster than 460800 baud. May fail "
            "to setup\n",
            inst->name,
            mbccb->baudrate);
    }

    if ((mbccb->format & MBCCB_FORMAT_PARITYODD) &&
        !(mbccb->format & MBCCB_FORMAT_PARITYEN)) {
    ...
    if (0 != mbccb->cmdslen % sizeof(hm2_modbus_mbccb_cmds_t)) {
        MSG_ERR("%s: error: Mbccb commands section has invalid size %zu\n",
                inst->name,
                (size_t)mbccb->cmdslen);
        goto errout;
    }
    // Must have data in the data segment
    if (!mbccb->datalen) {
        MSG_ERR(
            "%s: error: Mbccb datalen is zero, expected at least some pin name "
            "data\n",
            inst->name);
        goto errout;
    }
    ...
    ssize_t len =
        sizeof(*mbccb) + mbccb->initlen + mbccb->cmdslen + mbccb->datalen;
    if (mbccblen != len) {
    ...
    hm2_modbus_mbccb_cmds_t *initptr =
        (hm2_modbus_mbccb_cmds_t *)((rtapi_u8 *)mbccb + ofs);
    ofs += mbccb->initlen;
    hm2_modbus_mbccb_cmds_t *cmdsptr =
        (hm2_modbus_mbccb_cmds_t *)((rtapi_u8 *)mbccb + ofs);
    ofs += mbccb->cmdslen;
    ...
    rtapi_u32 dl = 0;
    for (const rtapi_u8 *dptr = dataptr; dptr < dataptr + mbccb->datalen;
         dptr += *dptr + 1) {
        dl += *dptr + 1; // Also count the length byte
    }
    if (dl != mbccb->datalen) {
        MSG_ERR("%s: error: Mbccb data segment size mismatch. Read %u and "
                "calculated %u\n",
                inst->name,
                mbccb->datalen,
                dl);
    ...
    for (unsigned i = 0; i < ninit; i++) {
        ...
        if (initptr[i].flags & ~MBCCB_CMDF_INITMASK) {
            MSG_ERR(
                "%s: error: Mbccb init %u has invalid flags set (flags=0x%04x, "
                "allowed=0x%04x)\n",
                inst->name,
                i,
                initptr[i].flags,
        ...
        if (initptr[i].cdataptr >= mbccb->datalen) {
            MSG_ERR("%s: error: Mbccb init %u data size mismatch. Read %u is "
                    "beyond segment size %u\n",
                    inst->name,
                    i,
                    initptr[i].cdataptr,
                    mbccb->datalen);
        ...
        if (initptr[i].cdataptr) {
            ...
            if (*dp <
                6) { // Packet: mbid(1), func(1), addr(2), value_or_count(2), ...
                MSG_ERR(
                    "%s: error: Mbccb init %u data packet size less than 6\n",
                    inst->name,
                    i);
                goto errout;
            }
        } else if (initptr[i].func || initptr[i].caddr > 1) {
            MSG_ERR("%s: error: Mbccb init %u has no data packet and invalid "
                    "meta-command\n",
                    inst->name,
                    i);
            goto errout;
        ...
        if (initptr[i].ctimeout > MAXDELAY) {
            MSG_ERR("%s: error: Mbccb init %u timeout larger than %u "
                    "microseconds\n",
                    inst->name,
                    i,
                    (unsigned)MAXDELAY);
        ...
        case 0: // The delay command
            if (0 != initptr[i].mbid) {
                MSG_ERR("%s: error: Mbccb init %u mbid not zero for delay "
                        "command\n",
                        inst->name,
                        i);
                goto errout;
        ...
        case MBCMD_W_REGISTER:
        case MBCMD_W_COILS:
        case MBCMD_W_REGISTERS: break;
        default:
            MSG_ERR("%s: error: Mbccb init %u has unsupported Modbus command "
                    "function %u\n",
                    inst->name,
                    i,
                    initptr[i].func);
    ...
    for (unsigned c = 0; c < ncmds; c++) {
        ...
        if (cmdsptr[c].cdataptr >= mbccb->datalen) {
            MSG_ERR(
                "%s: error: Mbccb cmds %u cmds' datasize mismatch. Read %u is "
                "beyond segment size %u\n",
                inst->name,
                c,
                cmdsptr[c].cdataptr,
        ...
        if (cmdsptr[c].ctypeptr >= mbccb->datalen) {
            MSG_ERR(
                "%s: error: Mbccb cmds %u types' datasize mismatch. Read %u is "
                "beyond segment size %u\n",
                inst->name,
                c,
                cmdsptr[c].ctypeptr,
        ...
        if (cmdsptr[c].ctypeptr && 0 != (cmdsptr[c].ctypeptr + 1) % 4) {
            MSG_ERR(
                "%s: error: Mbccb cmds %u types' data alignment mismatch %u %% "
                "4 != 0\n",
                inst->name,
                c,
                cmdsptr[c].ctypeptr + 1);
            goto errout;
        }
        ...
        if (cmdsptr[c].flags & ~MBCCB_CMDF_MASK) {
            MSG_WARN(
                "%s: warning: Mbccb cmds %u has extra flags set (flags=0x%04x, "
                "allowed=0x%04x)\n",
                inst->name,
                c,
                cmdsptr[c].flags,
        ...
        if (cmdsptr[c].ctimeout > MAXDELAY) {
            MSG_ERR("%s: error: Mbccb cmds %u timeout larger than %u "
                    "microseconds\n",
                    inst->name,
                    c,
                    (unsigned)MAXDELAY);
        ...
        case 0: // The delay command
            if (cmdsptr[c].mbid || cmdsptr[c].cpincnt || cmdsptr[c].caddr) {
                MSG_ERR(
                    "%s: error: Mbccb cmds %u mbid, pin count or address not "
                    "zero for delay command\n",
                    inst->name,
                    c);
                goto errout;
            }
        ...
        case MBCMD_R_INPUTS:
            if (cmdsptr[c].cpincnt < 1 || cmdsptr[c].cpincnt > 2000) {
                MSG_ERR(
                    "%s: error: Mbccb cmds %u bit function %u with invalid %u "
                    "pins\n",
                    inst->name,
                    c,
                    cmdsptr[c].func,
        ...
        case MBCMD_W_REGISTER:
            ...
            if (!cmdsptr[c].ctypeptr) {
                MSG_ERR(
                    "%s: error: Mbccb cmds %u missing type info, typeptr is "
                    "zero\n",
                    inst->name,
                    c);
                goto errout;
            ...
            xtype =
                (hm2_modbus_mbccb_type_t *)(dataptr + cmdsptr[c].ctypeptr + 1);
            if (xtypelen % sizeof(*xtype)) {
                MSG_ERR(
                    "%s: error: Mbccb cmds %u has invalid types modulo length "
                    "(%u %% %zu != 0)\n",
                    inst->name,
                    c,
                    xtypelen,
            ...
            if (cmdsptr[c].func == MBCMD_W_REGISTER) {
                ...
                if (cmdsptr[c].cregcnt != 1) {
                    MSG_ERR(
                        "%s: error: Mbccb cmds %u has register count %u != 1\n",
                        inst->name,
                        c,
                        cmdsptr[c].cregcnt);
                    goto errout;
                ...
                if (mtypesize(xtype->mtype) != 1) { // One word
                    MSG_ERR(
                        "%s: error: Mbccb cmds %u has too large Modbus type "
                        "for function W_REGISTER\n",
                        inst->name,
                        c);
                    goto errout;
                }
                ...
                if (cmdsptr[c].cpincnt < 1 || cmdsptr[c].cpincnt > 125) {
                    MSG_ERR("%s: error: Mbccb cmds %u has pin count %u out of "
                            "range [1..125]\n",
                            inst->name,
                            c,
                            cmdsptr[c].cpincnt);
                ...
                if (cmdsptr[c].cregcnt < 1 || cmdsptr[c].cregcnt > 125) {
                    MSG_ERR(
                        "%s: error: Mbccb cmds %u has register count %u out of "
                        "range [1..125]\n",
                        inst->name,
                        c,
                        cmdsptr[c].cregcnt);
            ...
            if (0 != xtype[0].regofs) {
                MSG_ERR("%s: error: Mbccb cmds %u has invalid first xtype reg "
                        "offset %u\n",
                        inst->name,
                        c,
                        xtype[0].regofs);
            ...
            for (unsigned x = 0; x < cmdsptr[c].cpincnt; x++) {
                if (!mtypeisvalid(xtype[x].mtype)) {
                    MSG_ERR("%s: error: Mbccb cmds %u, pin %u has invalid "
                            "modbustype %u\n",
                            inst->name,
                            c,
                            x,
                ...
                if (check_htype(xtype[x].htype) < 0) {
                    MSG_ERR(
                        "%s: error: Mbccb cmds %u, pin %u has invalid haltype "
                        "%u\n",
                        inst->name,
                        c,
                        x,
                ...
                if (xtype[x].flags & ~MBCCB_PINF_MASK) {
                    MSG_ERR(
                        "%s: error: Mbccb cmds %u, pin %u has invalid flags "
                        "0x%02x\n",
                        inst->name,
                        c,
                        x,
                ...
                if (xtype[x].regofs < xregofs || xtype[x].regofs > 125 - mbs) {
                    MSG_ERR(
                        "%s: error: Mbccb cmds %u, pin %u has invalid register "
                        "offset 0x%02x\n",
                        inst->name,
                        c,
                        x,
            ...
            if (xregofs >
                125 +
                    1) { // Plus one because this would point to the /next/ offset
                MSG_ERR("%s: error: Mbccb cmds %u xtypes adds up to too many "
                        "registers %u out of range [1..125]\n",
                        inst->name,
                        c,
                        xregofs);
                goto errout;
        ...
        default:
            MSG_ERR("%s: error: Mbccb cmds %u has unsupported Modbus command "
                    "function %u\n",
                    inst->name,
                    c,
                    cmdsptr[c].func);
        ...
        for (unsigned p = 0; p < cmdsptr[c].cpincnt; p++) {
            ...
            if (*dp <
                2) { // Should contain at least one char and the terminating zero
                MSG_ERR(
                    "%s: error: Mbccb pin %u:%u is empty\n", inst->name, c, p);
                goto errout;
            }
            if (*dp > MAXPINNAME) {
            ...
            for (unsigned ch = *dp - 1; ch > 0; ch--) {
                if (!((dp[ch] >= 'a' && dp[ch] <= 'z') ||
                      (dp[ch] >= '0' && dp[ch] <= '9') || dp[ch] == '.' ||
                      dp[ch] == '-')) {
                    MSG_ERR("%s: error: Mbccb Invalid character(s) '%c' in pin "
                            "%u:%u\n",
                            inst->name,
                            isprint(dp[ch]) ? dp[ch] : '?',
                            c,
...
int rtapi_app_main(void)
{
    ...
    if (!ports[0]) {
        MSG_ERR(COMP_NAME ": The component requires at least one valid pktuart "
                          "port, eg ports=\"hm2_5i25.0.pktuart.7\"\n");
        return -EINVAL;
    }
    ...
    // Count the instances.
    for (mb.ninsts = 0; mb.ninsts < MAX_PORTS && ports[mb.ninsts];
         mb.ninsts++) {
    }
    // Allocate memory for the instances
    if (!(mb.insts = (hm2_modbus_inst_t *)rtapi_kzalloc(
              mb.ninsts * sizeof(*mb.insts), RTAPI_GFP_KERNEL))) {
    ...
    for (int i = 0; i < mb.ninsts; i++) {
        ...
        if (!mbccbs[i]) {
            MSG_ERR("%s: error: Missing mbccb file path for instance %d in "
                    "'mbccbs' argument\n",
                    inst->name,
                    i);
            retval = -EINVAL;
        ...
        if ('/' != mbccbs[i][0]) {
            MSG_WARN(
                "%s: warning: The 'mbccb' file path '%s' for instance %d in "
                "'mbccbs' argument is not absolute\n",
                inst->name,
                mbccbs[i],
                i);
        }
        ...
        if (!(inst->hal->pins = (mbt_pin_hal_t *)hal_malloc(
                  inst->npins * sizeof(*inst->hal->pins)))) {
            MSG_ERR("%s: error: Failed to allocate HAL pins memory\n",
                    inst->name);
            retval = -ENOMEM;
            goto errout;
        }
        if (!(inst->hal->cmds = (mbt_cmd_hal_t *)hal_malloc(
                  inst->ncmds * sizeof(*inst->hal->cmds)))) {
        ...
        if (inst->ninit > 0) {
            ...
            if (!(inst->_init = rtapi_kzalloc(
                      inst->ninit * sizeof(*inst->_init), RTAPI_GFP_KERNEL))) {
        ...
        if (!(inst->_cmds = rtapi_kzalloc(inst->ncmds * sizeof(*inst->_cmds),
                                          RTAPI_GFP_KERNEL))) {
        ...
        for (unsigned i = 0; i < inst->ncmds; i++) {
            ...
            if (inst->cmdsptr[i].ctypeptr)
                inst->_cmds[i].typeptr =
                    (hm2_modbus_mbccb_type_t *)(inst->dataptr +
                                                inst->cmdsptr[i].ctypeptr + 1);
        }
        ...
        if ((retval = hal_export_functf(
                 process, inst, 1, 0, comp_id, COMP_NAME ".%d.process", i)) <
            0) {
...
#define CHECK(x)                                                               \
    do {                                                                       \
        ...
        CHECK(hal_param_u32_newf(HAL_RO,
                                 &(inst->hal->baudrate),
                                 comp_id,
                                 "%s.baudrate",
                                 inst->name));
        CHECK(hal_param_u32_newf(
            HAL_RO, &(inst->hal->parity), comp_id, "%s.parity", inst->name));
        CHECK(hal_param_u32_newf(HAL_RO,
                                 &(inst->hal->stopbits),
                                 comp_id,
                                 "%s.stopbits",
                                 inst->name));
        CHECK(hal_param_u32_newf(
            HAL_RO, &(inst->hal->icdelay), comp_id, "%s.icdelay", inst->name));
        CHECK(hal_param_u32_newf(
            HAL_RO, &(inst->hal->txdelay), comp_id, "%s.txdelay", inst->name));
        CHECK(hal_param_u32_newf(
            HAL_RO, &(inst->hal->rxdelay), comp_id, "%s.rxdelay", inst->name));
        CHECK(hal_param_u32_newf(HAL_RO,
                                 &(inst->hal->drvdelay),
                                 comp_id,
                                 "%s.drivedelay",
        ...
        CHECK(hal_pin_bit_newf(
            HAL_IN, &(inst->hal->suspend), comp_id, "%s.suspend", inst->name));
        CHECK(hal_pin_bit_newf(
            HAL_IN, &(inst->hal->reset), comp_id, "%s.reset", inst->name));
        CHECK(hal_pin_bit_newf(
            HAL_OUT, &(inst->hal->fault), comp_id, "%s.fault", inst->name));
        CHECK(hal_pin_u32_newf(HAL_OUT,
                               &(inst->hal->faultcmd),
                               comp_id,
                               "%s.fault-command",
                               inst->name));
        ...
        inst->hal->baudrate = inst->cfg_rx.baudrate = inst->cfg_tx.baudrate =
            inst->mbccb->baudrate;
        unsigned parity = 0;
        ...
        if (!inst->mbccb->rxdelay) // Auto
            inst->hal->rxdelay = inst->cfg_rx.ifdelay =
                calc_ifdelay(inst, inst->mbccb->baudrate, parity, stopbits) - 1;
        else // Manual
        ...
        if (!(inst->mbccb->format & MBCCB_FORMAT_DUPLEX))
            inst->cfg_rx.flags |=
                HM2_PKTUART_CONFIG_RXMASKEN; // Set rx masking if half-duplex
        ...
        inst->cfg_tx.flags |=
            HM2_PKTUART_CONFIG_DRIVEEN | HM2_PKTUART_CONFIG_DRIVEAUTO;
        ...
        if (inst->rxversion < 3 || inst->txversion < 3) {
            if (inst->rxversion < 2 || inst->txversion < 2) {
                MSG_ERR(
                    "%s: error: The driver does not support PktUART versions "
                    "before 2 (Rx=%u Tx=%u), aborting.\n",
                    inst->name,
                    inst->rxversion,
                    inst->txversion);
                goto errout;
            ...
            MSG_WARN(
                "%s: warning: PktUART version is less than 3 (Rx=%u Tx=%u). "
                "Please consider upgrading.\n",
                inst->name,
                inst->rxversion,
                inst->txversion);
            if (stopbits > 1) {
                MSG_WARN("%s: warning: Old PktUART cannot set two stop-bits. "
                         "Setting to one.\n",
                         inst->name);
                stopbits = 1;
            }
            if (inst->txversion < 3 && inst->cfg_tx.ifdelay > 0xff) {
                MSG_WARN(
                    "%s: warning: Old PktUART cannot set txdelay to 0x%04x. "
                    "Clamping to 0xff.\n",
                    inst->name,
                    inst->cfg_tx.ifdelay);
            ...
            if (inst->rxversion < 3 && inst->cfg_rx.ifdelay > 0xff) {
                MSG_WARN(
                    "%s: warning: Old PktUART cannot set rxdelay to 0x%04x. "
                    "Clamping to 0xff.\n",
                    inst->name,
                    inst->cfg_rx.ifdelay);
            ...
            if (inst->rxversion < 3 && inst->mbccb->icdelay != 0) {
                MSG_WARN(
                    "%s: warning: Old PktUART cannot set icdelay, disabling.\n",
                    inst->name);
        ...
        *(inst->hal->suspend) =
            0 != (inst->mbccb->format & MBCCB_FORMAT_SUSPEND);
...
#define CPTR(x) ((const char *)((x) + 1))
        for (unsigned c = 0; c < inst->ncmds; c++) {
            ...
            CHECK(hal_pin_bit_newf(HAL_IN,
                                   &(inst->hal->cmds[c].disable),
                                   comp_id,
                                   "%s.command.%02d.disable",
                                   inst->name,
                                   c));
            CHECK(hal_pin_bit_newf(HAL_OUT,
                                   &(inst->hal->cmds[c].disabled),
                                   comp_id,
                                   "%s.command.%02d.disabled",
                                   inst->name,
                                   c));
            CHECK(hal_pin_u32_newf(HAL_OUT,
                                   &(inst->hal->cmds[c].error),
                                   comp_id,
                                   "%s.command.%02d.errors",
                                   inst->name,
                                   c));
            CHECK(hal_pin_u32_newf(HAL_OUT,
                                   &(inst->hal->cmds[c].errorcode),
                                   comp_id,
                                   "%s.command.%02d.error-code",
                                   inst->name,
                                   c));
            CHECK(hal_pin_bit_newf(HAL_IN,
                                   &(inst->hal->cmds[c].reset),
                                   comp_id,
                                   "%s.command.%02d.reset",
                                   inst->name,
                                   c));
            ...
            for (int j = 0; j < cc->cmd.cpincnt; j++) {
                ...
                case MBCMD_W_REGISTERS:
                    ...
                    case HAL_S32:
                        ...
                        if (haspinscale(&cc->typeptr[j])) {
                            CHECK(
                                hal_pin_float_newf(HAL_IN,
                                                   &(inst->hal->pins[p].scale),
                                                   comp_id,
                                                   "%s.%s.scale",
                                                   inst->name,
                                                   CPTR(dptr)));
                            *(inst->hal->pins[p].scale) = 1.0;
                            if (HAL_OUT == dir) {
                                CHECK(hal_pin_float_newf(
                                    HAL_OUT,
                                    &(inst->hal->pins[p].scaled),
                                    comp_id,
                                    "%s.%s.scaled",
                                    inst->name,
                                    CPTR(dptr)));
                                ...
                                CHECK(hal_pin_s32_newf(
                                    HAL_IN,
                                    (hal_s32_t **)&(inst->hal->pins[p].offset),
                                    comp_id,
                                    "%s.%s.offset",
                                    inst->name,
                                    CPTR(dptr)));
                        ...
                        if (haspinscale(&cc->typeptr[j])) {
                            ...
                            *(inst->hal->pins[p].scale) = 1.0;
                            if (HAL_OUT == dir) {
                                ...
                                CHECK(hal_pin_s64_newf(
                                    HAL_IN,
                                    (hal_s64_t **)&(inst->hal->pins[p].offset),
                                    comp_id,
                                    "%s.%s.offset",
                                    inst->name,
                                    CPTR(dptr)));
                ...
                dptr +=
                    *dptr +
                    1; // Add length byte to get to next data record/pin name
            }
        }
#undef CPTR
#undef CHECK
        ...
        if ((retval = hm2_pktuart_config(
                 inst->uart, &inst->cfg_rx, &inst->cfg_tx, 0)) < 0) {
            MSG_ERR("%s: error: PktUART setup problem: error=%d\n",
                    inst->name,
                    retval);
            goto errout;
