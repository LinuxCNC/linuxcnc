...
int hm2_read_pin_descriptors(hostmot2_t *hm2)
{
    ...
    const rtapi_u8 DB25[] = {
        1, 14, 2, 15, 3, 16, 4, 17, 5, 6, 7, 8, 9, 10, 11, 12, 13};

    hm2->num_pins = hm2->idrom.io_width;
    hm2->pin =
        rtapi_kmalloc(sizeof(hm2_pin_t) * hm2->num_pins, RTAPI_GFP_KERNEL);
    if (hm2->pin == NULL) {
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        ...
        case 19: /* 7C81 */ pin->port_pin = i + 1; break;
        case 21: /* 7I94/4I74 punt on this for now */
            pin->port_pin = i + 1;
            break;
        case 27: /* 7C80 punt on this for now */ pin->port_pin = i + 1; break;
        case 29: /* 7I95 punt on this for now */ pin->port_pin = i + 1; break;
        case 30: /* 8cSS , MC04 boards */ pin->port_pin = i + 1; break;
        default:
...
void hm2_set_pin_source(hostmot2_t *hm2, int pin_number, int source)
{
    ...
    if ((pin_number < 0) || (pin_number >= hm2->num_pins) ||
                                     ...
                                     int direction)
{
    ...
    if ((direction != HM2_PIN_DIR_IS_INPUT) &&
        (direction != HM2_PIN_DIR_IS_OUTPUT)) {
        HM2_ERR(
            "hm2_set_pin_direction_immediate: invalid pin direction 0x%08X\n",
            direction);
        return;
    }
                                    ...
                                    int direction)
{
    ...
    if ((direction != HM2_PIN_DIR_IS_INPUT) &&
        (direction != HM2_PIN_DIR_IS_OUTPUT)) {
        HM2_ERR(
            "hm2_set_pin_direction_at_start: invalid pin direction 0x%08X\n",
            direction);
        return;
    }
...
void hm2_print_pin_usage(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        ...
        if (hm2->llio->io_connector_pin_names == NULL) {
            snprintf(connector_pin_name,
                     sizeof(connector_pin_name),
                     "%s-%02d",
        ...
        if (pin->gtag == pin->sec_tag) {
            if (pin->sec_unit &
                0x80) // global pins have a 0x80 secondary unit #
...
static void hm2_pins_allocate_all(hostmot2_t *hm2,
                                  int gtag,
                                  int num_instances,
                                  bool immediate)
{
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        if ((hm2->pin[i].sec_tag == gtag) &&
            ...
            if (hm2->pin[i].sec_pin & 0x80) {
                if (immediate) {
                    hm2_set_pin_direction_immediate(
                        hm2, i, HM2_PIN_DIR_IS_OUTPUT);
                } else {
                    hm2_set_pin_direction_at_start(
                        hm2, i, HM2_PIN_DIR_IS_OUTPUT);
                }
            }
        }
...
void hm2_configure_pins(hostmot2_t *hm2)
{
    ...
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_ENCODER, hm2->encoder.num_instances, false);
    ...
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_PKTUART_RX, hm2->pktuart.num_instances, true);
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_PKTUART_TX, hm2->pktuart.num_instances, true);
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_PWMGEN, hm2->pwmgen.num_instances, false);
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_RCPWMGEN, hm2->rcpwmgen.num_instances, false);
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_RESOLVER, hm2->resolver.num_instances, true);
    hm2_pins_allocate_all(hm2, HM2_GTAG_SSI, MAX_ABSENCS, true);
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_TPPWM, hm2->tp_pwmgen.num_instances, false);
    ...
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_XY2MOD, hm2->xy2mod.num_instances, false);
    // smart-serial might also not be contiguous
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_SMARTSERIAL, HM2_SSERIAL_MAX_PORTS, true);
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_SMARTSERIALB, HM2_SSERIAL_MAX_PORTS, true);
    // muxed encoder gets the sel pins
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_MUXED_ENCODER_SEL, hm2->encoder.num_instances, true);
    // and about half as many I/Os as you'd expect
    hm2_pins_allocate_all(hm2,
                          HM2_GTAG_MUXED_ENCODER,
                          (hm2->encoder.num_instances + 1) / 2,
                          false);
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_HM2DPLL, hm2->dpll.num_instances, false);
    ...
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_ONESHOT, hm2->oneshot.num_instances, false);
    hm2_pins_allocate_all(
        hm2, HM2_GTAG_PERIODM, hm2->periodm.num_instances, false);
}
