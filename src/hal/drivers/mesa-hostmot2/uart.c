...
int hm2_uart_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->uart.num_instances == 0) {
        ...
        hm2->uart.instance = (hm2_uart_instance_t *)hal_malloc(
            hm2->uart.num_instances * sizeof(hm2_uart_instance_t));
                   ...
                   rtapi_s32 rx_mode)
{
    ...
    buff = (rtapi_u32)((bitrate * 1048576.0) /
                       inst->clock_freq); //20 bits in this version
    r = 0;
    if (buff != inst->bitrate) {
        inst->bitrate = buff;
        r += hm2->llio->write(
            hm2->llio, inst->rx_bitrate_addr, &buff, sizeof(rtapi_u32));
        r += hm2->llio->write(
            hm2->llio, inst->tx_bitrate_addr, &buff, sizeof(rtapi_u32));
        buff = 0;
        r += hm2->llio->write(hm2->llio,
                              inst->rx_mode_addr,
                              &buff,
                              sizeof(rtapi_u32)); // clear faults
        r += hm2->llio->write(hm2->llio,
                              inst->rx_fifo_count_addr,
                              &buff,
                              sizeof(rtapi_u32)); // clear fifo
        r += hm2->llio->write(hm2->llio,
                              inst->tx_fifo_count_addr,
                              &buff,
                              sizeof(rtapi_u32)); // clear fifo
    }
...
int hm2_uart_send(char *name, unsigned char data[], int count)
{
    ...
    while (c < count - 3) {
        buff = (data[c] + (data[c + 1] << 8) + (data[c + 2] << 16) +
                (data[c + 3] << 24));
        r = hm2->llio->write(hm2->llio,
                             hm2->uart.instance[inst].tx4_addr,
                             &buff,
                             sizeof(rtapi_u32));
        if (r < 0) {
    ...
    case 1:
        buff = data[c];
        r = hm2->llio->write(hm2->llio,
                             hm2->uart.instance[inst].tx1_addr,
                             &buff,
                             sizeof(rtapi_u32));
        if (r < 0) {
            HM2_ERR("UART WRITE: hm2->llio->write failure %s\n", name);
    ...
    case 2:
        ...
        r = hm2->llio->write(hm2->llio,
                             hm2->uart.instance[inst].tx2_addr,
                             &buff,
                             sizeof(rtapi_u32));
        if (r < 0) {
    ...
    case 3:
        buff = (data[c] + (data[c + 1] << 8) + (data[c + 2] << 16));
        r = hm2->llio->write(hm2->llio,
                             hm2->uart.instance[inst].tx3_addr,
                             &buff,
                             sizeof(rtapi_u32));
        if (r < 0) {
            HM2_ERR("UART WRITE: hm2->llio->write failure %s\n", name);
...
int hm2_uart_read(char *name, unsigned char data[])
{
    ...
    r = hm2->llio->read(hm2->llio,
                        hm2->uart.instance[inst].rx_fifo_count_addr,
                        &buff,
                        sizeof(rtapi_u32));
    ...
    while (c < count - 3 && c < 16) {
        r = hm2->llio->read(hm2->llio,
                            hm2->uart.instance[inst].rx4_addr,
                            &buff,
                            sizeof(rtapi_u32));
    ...
    case 1:
        r = hm2->llio->read(hm2->llio,
                            hm2->uart.instance[inst].rx1_addr,
                            &buff,
                            sizeof(rtapi_u32));
        if (r < 0) {
    ...
    case 2:
        r = hm2->llio->read(hm2->llio,
                            hm2->uart.instance[inst].rx2_addr,
                            &buff,
                            sizeof(rtapi_u32));
        if (r < 0) {
            HM2_ERR("UART READ: hm2->llio->read failure %s\n", name);
    ...
    case 3:
        r = hm2->llio->read(hm2->llio,
                            hm2->uart.instance[inst].rx3_addr,
                            &buff,
                            sizeof(rtapi_u32));
        if (r < 0) {
            HM2_ERR("UART READ: hm2->llio->read failure %s\n", name);
        ...
        data[c + 2] = (buff & 0x00FF0000) >> 16;
        return c + 3;
    default: HM2_ERR("UART READ: Error in buffer parsing.\n"); return -EINVAL;
    }
}
