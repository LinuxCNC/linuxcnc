...
#define MAX_TX_FRAMES                                                          \
    (16) // Send counts are written to 16 deep FIFO, burst mode
...
int hm2_pktuart_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2->pktuart.num_instances == 0) {
        ...
        hm2->pktuart.instance = (hm2_pktuart_instance_t *)hal_malloc(
            hm2->pktuart.num_instances * sizeof(hm2_pktuart_instance_t));
    ...
    if (md->gtag == HM2_GTAG_PKTUART_RX) {
        ...
        if (r < 0) {
            HM2_ERR("error registering tram read region for PktUART Rx "
                    "status(%d)\n",
                    r);
            goto fail0;
        }
        ...
        if (r < 0) {
            HM2_ERR("error registering tram read region for PktUART Tx "
                    "status(%d)\n",
                    r);
            goto fail0;
        }
    ...
    for (i = 0; i < hm2->pktuart.num_instances; i++) {
        ...
        if (md->gtag == HM2_GTAG_PKTUART_TX) {
            ...
            inst->tx_fifo_count_addr = md->base_address + md->register_stride +
                                       i * md->instance_stride;
            inst->tx_bitrate_addr = md->base_address + 2 * md->register_stride +
                                    i * md->instance_stride;
            inst->tx_mode_addr = md->base_address + 3 * md->register_stride +
                                 i * md->instance_stride;
        } else if (md->gtag == HM2_GTAG_PKTUART_RX) {
            inst->rx_addr = md->base_address + i * md->instance_stride;
            inst->rx_fifo_count_addr = md->base_address + md->register_stride +
                                       i * md->instance_stride;
            inst->rx_bitrate_addr = md->base_address + 2 * md->register_stride +
                                    i * md->instance_stride;
            inst->rx_mode_addr = md->base_address + 3 * md->register_stride +
                                 i * md->instance_stride;
        } else {
            HM2_ERR(
                "hm2_pktuart_parse_md(): md->gtag changed? Memory corruption "
                "or wrong function call.\n");
            r = -ENODEV;
            goto fail0;
        }
    }
...
static void llio_noqueue_warn(const hostmot2_t *hm2)
{
    ...
    if (!hm2->llio->queue_write)
        HM2_ERR(
            "Hostmot2 low-level driver is missing the queue_write() function. "
            "PktUART will crash.\n");
    if (!hm2->llio->queue_read)
        HM2_ERR(
            "Hostmot2 low-level driver is missing the queue_read() function. "
            "PktUART will crash.\n");
    warned = 1;
}
...
//
int hm2_pktuart_setup_tx(const char *name,
                         ...
                         int enable_delay)
{
    hm2_pktuart_config_t cfg = {.baudrate = bitrate,
                                .filterrate = 0,
                                .drivedelay = enable_delay,
                                .ifdelay = frame_delay,
                                .flags = 0};
    if (parity != 0)
        cfg.flags |= HM2_PKTUART_CONFIG_PARITYEN;
    if (parity == 1)
        cfg.flags |= HM2_PKTUART_CONFIG_PARITYODD;
    if (drive_enable)
        cfg.flags |= HM2_PKTUART_CONFIG_DRIVEEN;
    if (drive_auto)
        cfg.flags |= HM2_PKTUART_CONFIG_DRIVEAUTO;
...
// Instead use hm2_pktuart_config().
//
int hm2_pktuart_setup_rx(const char *name,
                         unsigned int bitrate,
                         unsigned int filter_hz,
                         unsigned int parity,
                         int frame_delay,
                         bool rx_enable,
                         bool rx_mask)
{
    ...
    if (parity == 1)
        cfg.flags |= HM2_PKTUART_CONFIG_PARITYODD;
    if (rx_enable)
        cfg.flags |= HM2_PKTUART_CONFIG_RXEN;
    if (rx_mask)
        cfg.flags |= HM2_PKTUART_CONFIG_RXMASKEN;
...
//
static int config_tx(const char *name,
                     const hostmot2_t *hm2,
                     hm2_pktuart_instance_t *inst,
                     const hm2_pktuart_config_t *cfg,
                     int queue)
{
    rtapi_u32 bitrate;
    rtapi_u32 mode = 0;
    ...
    if (hm2->pktuart.tx_version >= 2) {
        bitrate = (rtapi_u64)cfg->baudrate * 16777216ul /
                  inst->clock_freq; // 24 bits in v2+
        if (!bitrate)
            bitrate = 1;
        if (bitrate > 0x00ffffff)
            bitrate = 0x00ffffff;
    } else {
        bitrate = (rtapi_u64)cfg->baudrate * 1048576ul /
                  inst->clock_freq; // 20 bits in v0 & v1
    }
    ...
    if (cfg->ifdelay > 0xff) {
        if (hm2->pktuart.tx_version >= 3) {
            ...
            if (cfg->ifdelay > 0x3fc)
                ...
                mode |=
                    HM2_PKTUART_TXMODE_INTERFRAMEDLY((cfg->ifdelay + 3) >> 2);
        } else {
            mode |= HM2_PKTUART_TXMODE_INTERFRAMEDLY(0xff);
        }
    } else {
    ...
    if (cfg->flags & HM2_PKTUART_CONFIG_DRIVEAUTO)
        mode |= HM2_PKTUART_TXMODE_DRIVEAUTO;
    if (cfg->flags & HM2_PKTUART_CONFIG_DRIVEEN)
        mode |= HM2_PKTUART_TXMODE_DRIVEEN;
    if (cfg->flags & HM2_PKTUART_CONFIG_PARITYEN)
        mode |= HM2_PKTUART_TXMODE_PARITYEN;
    if (cfg->flags & HM2_PKTUART_CONFIG_PARITYODD)
        mode |= HM2_PKTUART_TXMODE_PARITYODD;
    if (cfg->flags & HM2_PKTUART_CONFIG_STOPBITS2 &&
        hm2->pktuart.tx_version >= 3)
    ...
    if ((cfg->flags & HM2_PKTUART_CONFIG_FORCECONFIG) ||
        bitrate != inst->tx_bitrate) {
        inst->tx_bitrate = bitrate;
        if ((r = writefn(
                 hm2->llio, inst->tx_bitrate_addr, &bitrate, sizeof(bitrate))) <
            0) {
            HM2_ERR(
                "Configure TX baudrate: hm2->llio->%s failure %s (error %d)\n",
                writenm,
                name,
                r);
            return r;
    ...
    if ((cfg->flags & HM2_PKTUART_CONFIG_FORCECONFIG) ||
        mode != inst->tx_mode) {
        inst->tx_mode = mode;
        if ((r = writefn(hm2->llio, inst->tx_mode_addr, &mode, sizeof(mode))) <
            0) {
    ...
    if (cfg->flags & HM2_PKTUART_CONFIG_FLUSH) {
        rtapi_u32 buff =
            HM2_PKTUART_CLEAR; // clear data FIFO and count register
        if ((r = writefn(hm2->llio, inst->tx_mode_addr, &buff, sizeof(buff))) <
            0) {
...
//
static int config_rx(const char *name,
                     const hostmot2_t *hm2,
                     hm2_pktuart_instance_t *inst,
                     const hm2_pktuart_config_t *cfg,
                     int queue)
{
    ...
    if (cfg->flags & HM2_PKTUART_CONFIG_RXMASKEN)
        mode |= HM2_PKTUART_RXMODE_RXMASKEN;
    if (cfg->flags & HM2_PKTUART_CONFIG_RXEN)
        mode |= HM2_PKTUART_RXMODE_RXEN;
    if (cfg->flags & HM2_PKTUART_CONFIG_PARITYEN)
        mode |= HM2_PKTUART_RXMODE_PARITYEN;
    if (cfg->flags & HM2_PKTUART_CONFIG_PARITYODD)
        mode |= HM2_PKTUART_RXMODE_PARITYODD;
    if (cfg->flags & HM2_PKTUART_CONFIG_STOPBITS2 &&
        hm2->pktuart.rx_version >= 3)
    ...
    if ((cfg->flags & HM2_PKTUART_CONFIG_FORCECONFIG) ||
        bitrate != inst->rx_bitrate) {
        inst->rx_bitrate = bitrate;
        if ((r = writefn(
                 hm2->llio, inst->rx_bitrate_addr, &bitrate, sizeof(bitrate))) <
            0) {
            HM2_ERR(
                "Configure RX baudrate: hm2->llio->%s failure %s (error %d)\n",
                writenm,
                name,
                r);
            return r;
    ...
    if ((cfg->flags & HM2_PKTUART_CONFIG_FORCECONFIG) ||
        mode != inst->rx_mode) {
        inst->rx_mode = mode;
        if ((r = writefn(hm2->llio, inst->rx_mode_addr, &mode, sizeof(mode))) <
            0) {
    ...
    if (cfg->flags & HM2_PKTUART_CONFIG_FLUSH) {
        rtapi_u32 buff =
            HM2_PKTUART_CLEAR; // clear data FIFO and count register
        if ((r = writefn(hm2->llio, inst->rx_mode_addr, &buff, sizeof(buff))) <
            0) {
...
//
int hm2_pktuart_config(const char *name,
                       const hm2_pktuart_config_t *rxcfg,
                       const hm2_pktuart_config_t *txcfg,
                       int queue)
{
    hostmot2_t *hm2;
    int r;
    ...
    if (i < 0) {
        HM2_ERR_NO_LL(
            "Can not find PktUART instance %s (error %d).\n", name, i);
        return -ENODEV;
    }
...
//
int hm2_pktuart_setup(const char *name,
                      unsigned bitrate,
                      rtapi_s32 tx_mode,
                      rtapi_s32 rx_mode,
                      int txclear,
                      int rxclear)
{
    ...
    if (bitrate > 0) {
        if (hm2->pktuart.tx_version >= 2) {
            buff = (rtapi_u32)((bitrate * 16777216.0) /
                               inst->clock_freq); // 24 bits in v2+
        } else {
            buff = (rtapi_u32)((bitrate * 1048576.0) /
                               inst->clock_freq); // 20 bits in v0 & v1
        }
        if (buff != inst->tx_bitrate) {
            inst->tx_bitrate = buff;
            if ((r = hm2->llio->write(hm2->llio,
                                      inst->tx_bitrate_addr,
                                      &buff,
                                      sizeof(rtapi_u32))) < 0) {
                HM2_ERR(
                    "PktUART setup: hm2->llio->write failure %s on tx bitrate "
                    "(error %d)\n",
                    name,
                    r);
                return r;
        ...
        if (buff != inst->rx_bitrate) {
            ...
            if ((r = hm2->llio->write(hm2->llio,
                                      inst->rx_bitrate_addr,
                                      &buff,
                                      sizeof(rtapi_u32))) < 0) {
                HM2_ERR(
                    "PktUART setup: hm2->llio->write failure %s on rx bitrate "
                    "(error %d)\n",
                    name,
                    r);
                return r;
    ...
    if (tx_mode >= 0) {
        ...
        if ((r = hm2->llio->write(
                 hm2->llio, inst->tx_mode_addr, &buff, sizeof(rtapi_u32))) <
            0) {
            HM2_ERR(
                "PktUART setup: hm2->llio->write failure %s on tx_mode (error "
                "%d)\n",
                name,
                r);
            return r;
    ...
    if (rx_mode >= 0) {
        ...
        if ((r = hm2->llio->write(
                 hm2->llio, inst->rx_mode_addr, &buff, sizeof(rtapi_u32))) <
            0) {
            HM2_ERR(
                "PktUART setup: hm2->llio->write failure %s on rx_mode (error "
                "%d)\n",
                name,
                r);
            return r;
    ...
    if (txclear == 1) {
        ...
        if ((r = hm2->llio->write(
            ...
            0) {
            HM2_ERR(
                "PktUART setup: hm2->llio->write failure %s on tx clear (error "
                "%d)\n",
                name,
                r);
            return r;
    ...
    if (rxclear == 1) {
        ...
        if ((r = hm2->llio->write(
            ...
            0) {
            HM2_ERR(
                "PktUART setup: hm2->llio->write failure %s on rx clear (error "
                "%d)\n",
                name,
                r);
            return r;
...
//
int hm2_pktuart_send(const char *name,
                     const unsigned char data[],
                     rtapi_u8 *num_frames,
                     const rtapi_u16 frame_sizes[])
{
    hostmot2_t *hm2;
    int c = 0;
    int r = 0;
    ...
    *num_frames = 0;
    ...
    for (unsigned i = 0; i < nframes; i++) {
        count += frame_sizes[i];
        while (c < count - 3) {
            rtapi_u32 buff = data[c] + (data[c + 1] << 8) +
                             (data[c + 2] << 16) + (data[c + 3] << 24);
            r = hm2->llio->queue_write(hm2->llio,
                                       hm2->pktuart.instance[inst].tx_addr,
                                       &buff,
                                       sizeof(buff));
            if (r < 0) {
    ...
    for (unsigned i = 0; i < nframes; i++) {
        ...
        r = hm2->llio->queue_write(
            hm2->llio,
            hm2->pktuart.instance[inst].tx_fifo_count_addr,
            &buff,
            sizeof(buff));
...
//
int hm2_pktuart_read(const char *name,
                     unsigned char data[],
                     rtapi_u8 *num_frames,
                     ...
                     rtapi_u16 frame_sizes[])
{
    ...
    r = hm2->llio->read(hm2->llio,
                        hm2->pktuart.instance[inst].rx_mode_addr,
                        &buff,
                        sizeof(rtapi_u32));
    if (r < 0) {
    ...
    *num_frames = 0;
	...
	has be determined that there are packets available to read. */
        r = hm2->llio->read(hm2->llio,
                            hm2->pktuart.instance[inst].rx_fifo_count_addr,
                            &buff,
                            sizeof(buff));

        countb =
            buff &
            0x3ff; // PktUARTr  receive count register Bits 9..0 : bytes in receive packet
        ...
        if ((bytes_total + countb) > data_size) {
            HM2_ERR_NO_LL(
                "%s: bytes available %d are more than data array size %d\n",
                name,
                bytes_total + countb,
                data_size);
        ...
        while (c < countb - 3) {
            r = hm2->llio->read(hm2->llio,
                                hm2->pktuart.instance[inst].rx_addr,
                                &buff,
                                sizeof(buff));
        ...
        if (countb - c) {
            ...
            switch (countb - c) {
            case 1: data[bytes_total + c + 0] = (buff & 0x000000FF); break;
            case 2:
...
int hm2_pktuart_queue_read_data(const char *name, rtapi_u32 data[], int bytes)
{
    ...
    for (i = 0; i < nrx; i++) {
        r = hm2->llio->queue_read(hm2->llio,
                                  hm2->pktuart.instance[inst].rx_addr,
                                  &data[i],
                                  sizeof(rtapi_u32));
        if (r < 0) {
