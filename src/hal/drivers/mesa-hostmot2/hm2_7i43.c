...
MODULE_INFO(
    linuxcnc,
    "component:hm2_7i43:LinuxCNC HAL driver for the Mesa Electronics 7i43 EPP "
...
static int ioaddr[HM2_7I43_MAX_BOARDS] = {
    0, [1 ...(HM2_7I43_MAX_BOARDS - 1)] = -1};
RTAPI_MP_ARRAY_INT(
    ioaddr,
    HM2_7I43_MAX_BOARDS,
    "base address of the parallel port(s) (see hm2_7i43(9) manpage)");

static int ioaddr_hi[HM2_7I43_MAX_BOARDS] = {[0 ...(HM2_7I43_MAX_BOARDS - 1)] =
                                                 0};
RTAPI_MP_ARRAY_INT(
    ioaddr_hi,
    HM2_7I43_MAX_BOARDS,
    "secondary address of the parallel port(s) (see hm2_7i43(9) manpage)");

static int epp_wide[HM2_7I43_MAX_BOARDS] = {[0 ...(HM2_7I43_MAX_BOARDS - 1)] =
                                                1};
RTAPI_MP_ARRAY_INT(
    epp_wide,
    HM2_7I43_MAX_BOARDS,
    "set to 0 to disable wide EPP mode (see (hm2_7i43(9) manpage)");
...
RTAPI_MP_ARRAY_STRING(
    config,
    HM2_7I43_MAX_BOARDS,
    "config string(s) for the 7i43 board(s) (see hostmot2(9) manpage)");
...
static inline void hm2_7i43_epp_write_status(uint8_t status_byte,
                                             hm2_7i43_t *board)
{
...
static inline void hm2_7i43_epp_write_control(uint8_t control_byte,
                                              hm2_7i43_t *board)
{
...
int hm2_7i43_read(hm2_lowlevel_io_t *this,
                  rtapi_u32 addr,
                  void *buffer,
                  int size)
{
...
int hm2_7i43_write(hm2_lowlevel_io_t *this,
                   rtapi_u32 addr,
                   const void *buffer,
                   int size)
{
...
{
    int orig_debug_epp =
        debug_epp; // we turn off EPP debugging for this part...
    hm2_7i43_t *board = this->private;
...

    {
        ...
        if (duration_ns != 0) {
            THIS_INFO("%zu bytes of firmware sent (%u KB/s)\n",
                      bitfile->e.size,
                      (uint32_t)(((double)bitfile->e.size /
                                  ((double)duration_ns /
    ...
    if (board->epp_wide) {
        ...
        if (cookie != HM2_IOCOOKIE) {
            THIS_ERR(
                "Reading cookie with epp_wide failed. (read 0x%08x) Falling "
                "back to byte transfers\n",
                cookie);
            board->epp_wide = 0;
            hm2_7i43_epp_clear_timeout(board);
            hm2_7i43_read(this, HM2_ADDR_IOCOOKIE, &cookie, sizeof(cookie));
            if (cookie == HM2_IOCOOKIE) {
                THIS_ERR("Successfully read cookie after selecting byte "
                         "transfers\n");
            } else {
                THIS_ERR("Reading cookie still failed without epp_wide. (read "
                         "0x%08x)\n",
                         cookie);
            }
        }
...
static int hm2_7i43_setup(void)
{
    ...
    for (i = 0; i < HM2_7I43_MAX_BOARDS; i++) {
        ...
        r = hal_parport_get(
            comp_id, &board[i].port, ioaddr[i], ioaddr_hi[i], PARPORT_MODE_EPP);
        if (r < 0)
            return r;
        ...
        if (board[i].port.base_hi) {
            rtapi_outb(
                ...
                board[i].port.base_hi +
                    HM2_7I43_ECP_CONTROL_HIGH_OFFSET); // select EPP mode in ECR
        }
        ...
        hm2_7i43_epp_write_control(
            0x04, &board[i]); // set control lines and input mode
        hm2_7i43_epp_clear_timeout(&board[i]);

        rtapi_snprintf(board[i].llio.name,
                       sizeof(board[i].llio.name),
                       "%s.%d",
                       HM2_LLIO_NAME,
                       i);
        ...
        if (r != 0) {
            hal_parport_release(&board[i].port);
            THIS_ERR(
                "board at (ioaddr=0x%04X, ioaddr_hi=0x%04X, epp_wide %s) not "
                "found!\n",
                board[i].port.base,
                board[i].port.base_hi,
