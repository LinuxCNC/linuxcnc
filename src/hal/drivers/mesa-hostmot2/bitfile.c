...
static int bitfile_do_small_chunk(const struct rtapi_firmware *fw,
                                  bitfile_chunk_t *chunk,
                                  size_t *i)
{
    if (*i + 2 > fw->size) {
...
static int bitfile_do_big_chunk(const struct rtapi_firmware *fw,
                                bitfile_chunk_t *chunk,
                                size_t *i)
{
    if (*i + 4 > fw->size) {
    ...
    chunk->size = ((uint32_t)fw->data[*i] << 24) +
                  ((uint32_t)fw->data[*i + 1] << 16) +
                  ((uint32_t)fw->data[*i + 2] << 8) + fw->data[*i + 3];
    (*i) += 4;
...
static int bitfile_parse_and_verify_chunk(const struct rtapi_firmware *fw,
                                          bitfile_t *bitfile,
                                          size_t *i)
{
    char tag;
    ...
    case 'e': return bitfile_do_big_chunk(fw, &bitfile->e, i);
...
int bitfile_parse_and_verify(const struct rtapi_firmware *fw,
                             bitfile_t *bitfile)
{
    size_t i;
    int r;

    const unsigned char bitfile_header[BITFILE_HEADERLEN] = {0x00,
                                                             0x09,
                                                             0x0f,
                                                             0xf0,
                                                             0x0f,
