...
static int bitfile_do_small_chunk(const struct firmware *fw, bitfile_chunk_t *chunk, int *i) {
    if (*i + 2 > fw->size) {
        PRINT_NO_LL(RTAPI_MSG_ERR, "bitfile chunk extends past end of firmware\n");
        return -ENODATA;
    }
    ...
    if (chunk->data[chunk->size - 1] != '\0') {
        PRINT_NO_LL(RTAPI_MSG_ERR, "bitfile small chunk is not NULL terminated\n");
        return -EINVAL;
    }
...
static int bitfile_parse_and_verify_chunk(const struct firmware *fw, bitfile_t *bitfile, int *i) {
    ...
    switch (tag) {
        ...
        default: {
            PRINT_NO_LL(RTAPI_MSG_ERR, "bitfile has unknown chunk '%c'\n", tag);
            break;
        }
    }
...
int bitfile_parse_and_verify(const struct firmware *fw, bitfile_t *bitfile) {
    ...
    if (fw->size < BITFILE_HEADERLEN) {
        PRINT_NO_LL(RTAPI_MSG_ERR, "bitfile is too short\n");
        return -ENODATA;
    }

    for (i = 0; i < BITFILE_HEADERLEN; i ++) {
        if (fw->data[i] != bitfile_header[i]) {
            PRINT_NO_LL(RTAPI_MSG_ERR, "bitfile has invalid header\n");
            return -EINVAL;
        }
    }
    ...
    if (bitfile->b.data == NULL) {
        PRINT_NO_LL(RTAPI_MSG_ERR, "bitfile lacks Part Name (chunk 'b')!\n");
        return -EINVAL;
    }

    if (bitfile->e.data == NULL) {
        PRINT_NO_LL(RTAPI_MSG_ERR, "bitfile lacks FPGA Config (part 'e')!\n");
        return -EINVAL;
    }
