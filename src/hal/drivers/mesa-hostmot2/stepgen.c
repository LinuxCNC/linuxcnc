    ...
    for (i = 0; i < hm2->stepgen.num_instances; i++) {
        ...
        if (fabs(hm2->stepgen.instance[i].hal.param.position_scale) < 1e-6) {
            if (hm2->stepgen.instance[i].hal.param.position_scale >= 0.0) {
                ...
                HM2_ERR(
                    "stepgen %d position_scale is too close to 0, resetting to "
                    "1.0\n",
                    i);
            } else {
                hm2->stepgen.instance[i].hal.param.position_scale = -1.0;
        ...
        acc_delta = (rtapi_s64)acc -
                    (rtapi_s64)hm2->stepgen.instance[i].prev_accumulator;
        ...
        if (hm2->stepgen.firmware_supports_index) {
            if (hm2->stepgen.instance[i]
                    .written_index_enable) { // searching for index
                if ((mode & HM2_STEPGEN_LATCH_ON_INDEX) ==
                    0) { // hardware index detected and position at index latched
                    latch_delta =
                        (rtapi_s64)latch -
                        (rtapi_s64)hm2->stepgen.instance[i].prev_accumulator;
                    ...
                    *hm2->stepgen.instance[i].hal.pin.position_latch =
                        ((double)(hm2->stepgen.instance[i].subcounts +
                                  latch_delta) /
                         65536.0) /
        ...
        *(hm2->stepgen.instance[i].hal.pin.counts) =
            hm2->stepgen.instance[i].subcounts >> 16;
...
static void hm2_stepgen_instance_position_control(hostmot2_t *hm2,
                                                  long l_period_ns,
                                                  int i,
                                                  double *new_vel)
{
    ...
    (*s->hal.pin.dbg_pos_minus_prev_cmd) =
        (*s->hal.pin.position_fb) - s->old_position_cmd;
    ...
    *s->hal.pin.dbg_s_to_match = seconds_to_vel_match;
    ...
    // Note: this is "feedback position at the beginning of the servo period after we attain velocity match"
    {
        ...
        position_at_match = *s->hal.pin.position_fb +
                            (avg_v * (seconds_to_vel_match + f_period_s));
    }

    // Note: this assumes that position-cmd keeps the current velocity
    position_cmd_at_match =
        *s->hal.pin.position_cmd + (ff_vel * seconds_to_vel_match);
    ...
    *s->hal.pin.dbg_err_at_match = error_at_match;
    ...
    if (seconds_to_vel_match < f_period_s) {
        ...
        // apply accel limits?
        if (s->hal.param.maxaccel > 0) {
            if (velocity_cmd > (*s->hal.pin.velocity_fb +
                                (s->hal.param.maxaccel * f_period_s))) {
                velocity_cmd = *s->hal.pin.velocity_fb +
                               (s->hal.param.maxaccel * f_period_s);
            } else if (velocity_cmd < (*s->hal.pin.velocity_fb -
                                       (s->hal.param.maxaccel * f_period_s))) {
                velocity_cmd = *s->hal.pin.velocity_fb -
                               (s->hal.param.maxaccel * f_period_s);
            }
...
static void hm2_stepgen_instance_prepare_tram_write(hostmot2_t *hm2,
                                                    long l_period_ns,
                                                    int i)
{
    double new_vel;
    ...
    double
        physical_maxvel; // max vel supported by current step timings & position-scale
    ...
    // maxvel must be >= 0.0, and may not be faster than 1 step per (steplen+stepspace) seconds
    {
        ...
        if (s->hal.param.maxvel > physical_maxvel) {
            HM2_ERR("stepgen.%02d.maxvel is too big for current step timings & "
    ...
    if (*s->hal.pin.control_type == 0) {
        ...
        if (s->hal.param.maxaccel > 0.0) {
            if (((new_vel - *s->hal.pin.velocity_fb) / f_period_s) >
                s->hal.param.maxaccel) {
                new_vel = (*s->hal.pin.velocity_fb) +
                          (s->hal.param.maxaccel * f_period_s);
            } else if (((new_vel - *s->hal.pin.velocity_fb) / f_period_s) <
                       -s->hal.param.maxaccel) {
                new_vel = (*s->hal.pin.velocity_fb) -
                          (s->hal.param.maxaccel * f_period_s);
            }
    ...
    *s->hal.pin.velocity_fb = (hal_float_t)new_vel;
    ...
    hm2->stepgen.step_rate_reg[i] =
        (uint32_t)(int32_t)(steps_per_sec_cmd *
                            (4294967296.0 /
                             (double)hm2->stepgen.clock_frequency));
...
static void hm2_stepgen_update_dir_setup_time(hostmot2_t *hm2, int i)
{
    hm2->stepgen.dir_setup_time_reg[i] =
        (double)hm2->stepgen.instance[i].hal.param.dirsetup *
    ...
    if (hm2->stepgen.dir_setup_time_reg[i] > 0x3FFF) {
        ...
        hm2->stepgen.instance[i].hal.param.dirsetup =
            (double)hm2->stepgen.dir_setup_time_reg[i] *
            ((double)1e9 / (double)hm2->stepgen.clock_frequency);
    }
    hm2->stepgen.instance[i].written_dirsetup =
        hm2->stepgen.instance[i].hal.param.dirsetup;
}
...
static void hm2_stepgen_update_dir_hold_time(hostmot2_t *hm2, int i)
{
    hm2->stepgen.dir_hold_time_reg[i] =
        (double)hm2->stepgen.instance[i].hal.param.dirhold *
        ((double)hm2->stepgen.clock_frequency / (double)1e9);
    if (hm2->stepgen.dir_hold_time_reg[i] > 0x3FFF) {
        ...
        hm2->stepgen.instance[i].hal.param.dirhold =
            (double)hm2->stepgen.dir_hold_time_reg[i] *
            ((double)1e9 / (double)hm2->stepgen.clock_frequency);
    }
    hm2->stepgen.instance[i].written_dirhold =
        hm2->stepgen.instance[i].hal.param.dirhold;
}
...
static void hm2_stepgen_update_pulse_idle_width(hostmot2_t *hm2, int i)
{
    hm2->stepgen.pulse_idle_width_reg[i] =
        (double)hm2->stepgen.instance[i].hal.param.stepspace *
        ((double)hm2->stepgen.clock_frequency / (double)1e9);
    if (hm2->stepgen.pulse_idle_width_reg[i] > 0x3FFF) {
        ...
        hm2->stepgen.instance[i].hal.param.stepspace =
            (double)hm2->stepgen.pulse_idle_width_reg[i] *
            ((double)1e9 / (double)hm2->stepgen.clock_frequency);
    }
    hm2->stepgen.instance[i].written_stepspace =
        hm2->stepgen.instance[i].hal.param.stepspace;
}
...
static void hm2_stepgen_update_pulse_width(hostmot2_t *hm2, int i)
{
    hm2->stepgen.pulse_width_reg[i] =
        (double)hm2->stepgen.instance[i].hal.param.steplen *
        ((double)hm2->stepgen.clock_frequency / (double)1e9);
    if (hm2->stepgen.pulse_width_reg[i] > 0x3FFF) {
        ...
        hm2->stepgen.instance[i].hal.param.steplen =
            (double)hm2->stepgen.pulse_width_reg[i] *
            ((double)1e9 / (double)hm2->stepgen.clock_frequency);
    }
    hm2->stepgen.instance[i].written_steplen =
        hm2->stepgen.instance[i].hal.param.steplen;
}
...
static void hm2_stepgen_update_mode(hostmot2_t *hm2, int i)
{
    ...
    inst->hal.param.table[4] =
        (((inst->hal.param.table[0] ^ inst->hal.param.table[1]) ^
          inst->hal.param.table[2]) ^
         inst->hal.param.table[3]);

    modebuff = inst->hal.param.step_type; // start with the step mode


    if (hm2->stepgen
            .firmware_supports_index) { // or in the index/probe control bits if needed
    ...
    if (inst->table_width < inst->hal.param.step_type) {
        HM2_ERR(
            "the firmware only supports %i pins in the step pattern for "
    ...
    hm2->stepgen.mode_reg[i] =
        3 |
        modebuff; // force 2 LSbs to 1,1 to select table mode in stepgen hardware
...
static void hm2_stepgen_set_dpll_timer(hostmot2_t *hm2)
{
    ...
    if ((*hm2->stepgen.hal->pin.dpll_timer_num < -1) ||
        (*hm2->stepgen.hal->pin.dpll_timer_num > 4)) {
    ...
    hm2->llio->write(
        hm2->llio, hm2->stepgen.dpll_timer_num_addr, &data, sizeof(rtapi_u32));
...
void hm2_stepgen_write(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->stepgen.num_instances; i++) {
        ...
        if ((inst->hal.param.step_type != inst->written_step_type) ||
            (((inst->hal.param.table[0] ^ inst->hal.param.table[1]) ^
              inst->hal.param.table[2]) ^
             inst->hal.param.table[3]) != inst->hal.param.table[4]) {
            need_mode_update = 1;
        }
        ...
        if (hm2->stepgen.firmware_supports_index) {
            if ((*inst->hal.pin.index_enable != inst->written_index_enable) ||
                ...
                (*inst->hal.pin.index_polarity !=
                 inst->written_index_polarity) ||
                (*inst->hal.pin.latch_polarity !=
                 inst->written_probe_polarity)) {
                need_mode_update |= 1;
            }
        }
        if (need_mode_update) {
            ...
            hm2->llio->write(hm2->llio,
                             hm2->stepgen.mode_addr + (i * sizeof(rtapi_u32)),
                             &hm2->stepgen.mode_reg[i],
                             sizeof(rtapi_u32));
            if (hm2->stepgen.firmware_supports_index) {
                inst->written_index_enable =
                    *inst->hal.pin
                         .index_enable; // we need to update these only after the write has occurred
                inst->written_probe_enable =
                    *inst->hal.pin
                         .latch_enable; // to avoid a race condition (index detected before index enable has been set)
            }
        }
    }
    ...
    if (hm2->stepgen.num_instances > 0 && hm2->dpll_module_present) {
        if (*hm2->stepgen.hal->pin.dpll_timer_num !=
            (int)hm2->stepgen.written_dpll_timer_num) {
...
static void hm2_stepgen_force_write_master_dds(hostmot2_t *hm2)
{
    ...
    hm2->llio->write(
        hm2->llio, hm2->stepgen.master_dds_addr, &val, sizeof(rtapi_u32));
}
...
void hm2_stepgen_tram_init(hostmot2_t *hm2)
{
    ...
    for (i = 0; i < hm2->stepgen.num_instances; i++) {
        hm2->stepgen.instance[i].prev_accumulator =
...
int hm2_stepgen_parse_md(hostmot2_t *hm2, int md_index)
{
    ...
    if (hm2_md_is_consistent(hm2, md_index, 0, 10, 4, 0x01FF)) {
        ...
        HM2_PRINT("WARNING: this version is susceptible to waveform timing "
                  "violations in certain configurations!\n");
        HM2_PRINT("WARNING: upgrade your firmware!\n");
    ...
    if (hm2->config.num_stepgens > md->instances) {
        HM2_ERR(
            "config.num_stepgens=%d, but only %d are available, not loading "
            "driver\n",
    ...
    hm2->stepgen.hal = (hm2_stepgen_module_global_t *)hal_malloc(
        sizeof(hm2_stepgen_module_global_t));
    ...
    hm2->stepgen.instance = (hm2_stepgen_instance_t *)hal_malloc(
        hm2->stepgen.num_instances * sizeof(hm2_stepgen_instance_t));
    ...
    hm2->stepgen.pulse_width_addr =
        md->base_address + (5 * md->register_stride);
    hm2->stepgen.pulse_idle_width_addr =
        md->base_address + (6 * md->register_stride);
    hm2->stepgen.table_sequence_data_setup_addr =
        md->base_address + (7 * md->register_stride);
    hm2->stepgen.table_sequence_length_addr =
        md->base_address + (8 * md->register_stride);
    hm2->stepgen.master_dds_addr = md->base_address + (9 * md->register_stride);
    hm2->stepgen.dpll_timer_num_addr =
        md->base_address + (10 * md->register_stride);

    r = hm2_register_tram_write_region(
        hm2,
        hm2->stepgen.step_rate_addr,
        (hm2->stepgen.num_instances * sizeof(rtapi_u32)),
        &hm2->stepgen.step_rate_reg);
    if (r < 0) {
        HM2_ERR("error registering tram write region for StepGen Step Rate "
                "register (%d)\n",
                r);
        goto fail0;
    }
    ...
    r = hm2_register_tram_read_region(
        ...
        hm2->stepgen.accumulator_addr,
        (hm2->stepgen.num_instances * sizeof(rtapi_u32)),
        &hm2->stepgen.accumulator_reg);
    if (r < 0) {
        HM2_ERR("error registering tram read region for StepGen Accumulator "
                "register (%d)\n",
                r);
        goto fail0;
    }
    ...
    if (hm2->stepgen.firmware_supports_index) {
        ...
        if (r < 0) {
            HM2_ERR(
                "error registering tram read region for StepGen mode register "
                "(%d)\n",
                r);
            goto fail0;
        }
    ...
    hm2->stepgen.mode_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->stepgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    hm2->stepgen.dir_setup_time_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->stepgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    hm2->stepgen.dir_hold_time_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->stepgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    hm2->stepgen.pulse_width_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->stepgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
    ...
    hm2->stepgen.pulse_idle_width_reg = (rtapi_u32 *)rtapi_kmalloc(
        hm2->stepgen.num_instances * sizeof(rtapi_u32), RTAPI_GFP_KERNEL);
...

    {
        ...
        if (hm2->dpll_module_present) {
            rtapi_snprintf(
                name, sizeof(name), "%s.stepgen.timer-number", hm2->llio->name);
            r = hal_pin_s32_new(name,
                                HAL_IN,
                                &(hm2->stepgen.hal->pin.dpll_timer_num),
                                hm2->llio->comp_id);
            if (r < 0) {
        ...
        for (i = 0; i < hm2->stepgen.num_instances; i++) {
            ...
            // Work out if table setup registers are needed.
            {
                ...
                for (j = 0; j < hm2->num_pins; j++) {
                    if (hm2->pin[j].sec_tag == HM2_GTAG_STEPGEN &&
                        hm2->pin[j].sec_unit == i) {
                        if (hm2->pin[j].sec_pin >
                            hm2->stepgen.instance[i].table_width) {
                            hm2->stepgen.instance[i].table_width =
                                hm2->pin[j].sec_pin;
                        }
                    }
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.velocity-cmd",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(
                name,
                HAL_IN,
                &(hm2->stepgen.instance[i].hal.pin.velocity_cmd),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.velocity-fb",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(
                name,
                HAL_OUT,
                &(hm2->stepgen.instance[i].hal.pin.velocity_fb),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.control-type",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(
                name,
                HAL_IN,
                &(hm2->stepgen.instance[i].hal.pin.control_type),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.position-reset",
                           hm2->llio->name,
                           i);
            r = hal_pin_bit_new(
                name,
                HAL_IN,
                &(hm2->stepgen.instance[i].hal.pin.position_reset),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            if (hm2->stepgen.firmware_supports_index) {
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.index-enable",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(
                    name,
                    HAL_IO,
                    &(hm2->stepgen.instance[i].hal.pin.index_enable),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.probe-enable",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(
                    name,
                    HAL_IO,
                    &(hm2->stepgen.instance[i].hal.pin.latch_enable),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.index-invert",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(
                    name,
                    HAL_IN,
                    &(hm2->stepgen.instance[i].hal.pin.index_polarity),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.probe-invert",
                               hm2->llio->name,
                               i);
                r = hal_pin_bit_new(
                    name,
                    HAL_IN,
                    &(hm2->stepgen.instance[i].hal.pin.latch_polarity),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.dbg_pos_minus_prev_cmd",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(
                name,
                HAL_OUT,
                &(hm2->stepgen.instance[i].hal.pin.dbg_pos_minus_prev_cmd),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.dbg_ff_vel",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(
                name,
                HAL_OUT,
                &(hm2->stepgen.instance[i].hal.pin.dbg_ff_vel),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.dbg_s_to_match",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(
                name,
                HAL_OUT,
                &(hm2->stepgen.instance[i].hal.pin.dbg_s_to_match),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.dbg_vel_error",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(
                name,
                HAL_OUT,
                &(hm2->stepgen.instance[i].hal.pin.dbg_vel_error),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.dbg_err_at_match",
                           hm2->llio->name,
                           i);
            r = hal_pin_float_new(
                name,
                HAL_OUT,
                &(hm2->stepgen.instance[i].hal.pin.dbg_err_at_match),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.dbg_step_rate",
                           hm2->llio->name,
                           i);
            r = hal_pin_s32_new(
                name,
                HAL_OUT,
                &(hm2->stepgen.instance[i].hal.pin.dbg_step_rate),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.maxvel",
                           hm2->llio->name,
                           i);
            r = hal_param_float_new(
                name,
                HAL_RW,
                &(hm2->stepgen.instance[i].hal.param.maxvel),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            rtapi_snprintf(name,
                           sizeof(name),
                           "%s.stepgen.%02d.maxaccel",
                           hm2->llio->name,
                           i);
            r = hal_param_float_new(
                name,
                HAL_RW,
                &(hm2->stepgen.instance[i].hal.param.maxaccel),
                hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                r = -ENOMEM;
            ...
            if (hm2->stepgen.firmware_supports_swap) {
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.swap_step_dir",
                               hm2->llio->name,
                               i);
                r = hal_param_bit_new(
                    name,
                    HAL_RW,
                    &(hm2->stepgen.instance[i].hal.param.swap_step_dir),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    r = -ENOMEM;
            ...
            if (hm2->stepgen.instance[i].table_width > 2) {
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.table-data-0",
                               hm2->llio->name,
                               i);
                r = hal_param_u32_new(
                    name,
                    HAL_RW,
                    &(hm2->stepgen.instance[i].hal.param.table[0]),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.table-data-1",
                               hm2->llio->name,
                               i);
                r = hal_param_u32_new(
                    name,
                    HAL_RW,
                    &(hm2->stepgen.instance[i].hal.param.table[1]),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.table-data-2",
                               hm2->llio->name,
                               i);
                r = hal_param_u32_new(
                    name,
                    HAL_RW,
                    &(hm2->stepgen.instance[i].hal.param.table[2]),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    r = -ENOMEM;
                ...
                rtapi_snprintf(name,
                               sizeof(name),
                               "%s.stepgen.%02d.table-data-3",
                               hm2->llio->name,
                               i);
                r = hal_param_u32_new(
                    name,
                    HAL_RW,
                    &(hm2->stepgen.instance[i].hal.param.table[3]),
                    hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding param '%s', aborting\n", name);
                    r = -ENOMEM;
...
void hm2_stepgen_print_module(hostmot2_t *hm2)
{
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n",
              ...
              hm2_hz_to_mhz(hm2->stepgen.clock_frequency));
    HM2_PRINT("    version: %d\n", hm2->stepgen.version);
