...
#define RP1_PADS_BANK0_OFFSET                                                  \
    0xf0000 /* Pad control, drive strength, pull-up/down, etc. */
...
typedef struct __rp1_rio_t {
    rp1_rio_regs_t rw;
    __I uint32_t
        reserved1[((RP1_XOR_OFFSET - RP1_RW_OFFSET - sizeof(rp1_rio_regs_t)) /
                   sizeof(uint32_t))];
    rp1_rio_regs_t xor ;
    __I uint32_t
        reserved2[((RP1_SET_OFFSET - RP1_XOR_OFFSET - sizeof(rp1_rio_regs_t)) /
                   sizeof(uint32_t))];
    rp1_rio_regs_t set;
    __I uint32_t
        reserved3[((RP1_CLR_OFFSET - RP1_SET_OFFSET - sizeof(rp1_rio_regs_t)) /
                   sizeof(uint32_t))];
    rp1_rio_regs_t clr;
...
typedef struct __dw_ssi_t {
    ...
    union {
        ...
        __IO uint32_t
            drx[36]; /* All these act the same according to the docs, use dr0 */
    };
...
#define DW_SSI_RISR_MSTIR_BIT                                                  \
    5 /* multi-master contention interrupt raw status */
...
#define DW_SSI_RISR_RXUIR_BIT                                                  \
    2 /* receive fifo underflow interrupt raw status */
#define DW_SSI_RISR_TXOIR_BIT                                                  \
    1 /* transmit fifo overflow interrupt raw status */
