...
int doit(void)
{
    ...
    if (buff & (1 << remote->index)) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "Error flag set after CMD Clear %08x\n", buff);
        return -1;
    }
    return 0;
}
            ...
            if (t < erase_sz) { // found a non-zero byte
                ...
                for (i = 0; i < erase_sz; i += write_sz) {
                    ...
                    for (j = 0; j < write_sz; j += 8) {
                        ...
                        m = block_start + i + j;
                        data0 =
                            (fw->data[m] + (fw->data[m + 1] << 8) +
                             (fw->data[m + 2] << 16) + (fw->data[m + 3] << 24));
                        data1 =
                            (fw->data[m + 4] + (fw->data[m + 5] << 8) +
                             (fw->data[m + 6] << 16) + (fw->data[m + 7] << 24));
