...
int allocate_lines(char **names, hal_gpio_bulk_t **bulk)
{
    ...
    if (n == -1) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "No valid gpio devices recognized by gpiod in /dev");
        return -1;
    }

    while (n--) {
        rtapi_print_msg(RTAPI_MSG_DBG,
                        "hal_gpio processing chip %s\n",
                        namelist[n]->d_name);
        int l = 0;

        *bulk = rtapi_krealloc(
            *bulk, sizeof(hal_gpio_bulk_t) * (b + 1), RTAPI_GFP_KERNEL);
        (*bulk)[b].offsets = NULL;
        (*bulk)[b].flags = NULL;
        (*bulk)[b].vals = NULL;
        ...
        for (int i = 0; names[i]; i++) {
            offset =
                gpiod_chip_get_line_offset_from_name((*bulk)[b].chip, names[i]);
            if (offset >= 0) {
                rtapi_print_msg(
                    RTAPI_MSG_DBG,
                    "hal_gpio processing line %s found at offset %i\n",
                    names[i],
                    offset);
                (*bulk)[b].offsets = rtapi_krealloc((*bulk)[b].offsets,
                                                    (l + 1) * sizeof(offset),
                                                    RTAPI_GFP_KERNEL);
                (*bulk)[b].offsets[l] = offset;
                (*bulk)[b].flags = rtapi_krealloc((*bulk)[b].flags,
                                                  (l + 1) * sizeof(offset),
                                                  RTAPI_GFP_KERNEL);
                (*bulk)[b].flags[l] = flags(names[i]);
                (*bulk)[b].vals = rtapi_krealloc(
                    (*bulk)[b].vals, (l + 1) * sizeof(int), RTAPI_GFP_KERNEL);
                (*bulk)[b].num_lines = ++l;
            }
            for (int o = 0; o < l; o++)
                rtapi_print_msg(RTAPI_MSG_DBG,
...
int setup_lines(hal_gpio_bulk_t *bulk, enum gpiod_line_direction direction)
{
    ...
    gpiod_line_config_add_line_settings(
        line_cfg, bulk->offsets, bulk->num_lines, settings);
    ...
    for (int i = 0; i < bulk->num_lines; i++) {
        ...
        if (ret) {
            ...
            rtapi_print_msg(RTAPI_MSG_DBG,
                            "reconfigure returned %d errno=%d (%s)\n",
                            ret,
                            errno,
                            strerror(errno));
            if (ret) {
...
int build_chips_collection(char **names, hal_gpio_bulk_t **ptr)
{
    ...
    for (i = 0; names[i] && strlen(names[i]); i++) {
        ...
        if (!temp_line) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "The GPIO line %s can not be found\n", name);
            return -EINVAL;
        }
        temp_chip = gpiod_line_get_chip(temp_line);
        for (c = 0;
             c < count &&
             (strcmp(gpiod_chip_name((*ptr)[c].chip),
        ...
        if (c >= count) {
            (count)++;
            *ptr = rtapi_krealloc(
                *ptr, sizeof(hal_gpio_bulk_t) * count, RTAPI_GFP_KERNEL);
            (*ptr)[c].chip = NULL;
            (*ptr)[c].flags = NULL;
            (*ptr)[c].vals = NULL;
            (*ptr)[c].num_lines = 0;
            ...
            (*ptr)[c].lines =
                rtapi_kmalloc(sizeof(*(*ptr)[c].lines), RTAPI_GFP_KERNEL);
            gpiod_line_bulk_init((*ptr)[c].lines);
            rtapi_print_msg(RTAPI_MSG_INFO,
                            "hal_gpio: added chip %s index %i\n",
                            gpiod_chip_name((*ptr)[c].chip),
                            c);
        }
        rtapi_print_msg(RTAPI_MSG_INFO,
                        "hal_gpio: adding IO line %s to chip %i\n",
                        name,
                        c);
        temp_line = gpiod_chip_find_line((*ptr)[c].chip, name);
        (*ptr)[c].num_lines++;
        (*ptr)[c].flags = rtapi_krealloc((*ptr)[c].flags,
                                         (*ptr)[c].num_lines * sizeof(int),
                                         RTAPI_GFP_KERNEL);
        (*ptr)[c].flags[(*ptr)[c].num_lines - 1] = flags(name);
#if LIBGPIOD_VER >= 105
        gpiod_line_set_flags(temp_line,
                             (*ptr)[c].flags[(*ptr)[c].num_lines - 1]);
#endif
        (*ptr)[c].vals = rtapi_krealloc((*ptr)[c].vals,
                                        (*ptr)[c].num_lines * sizeof(int),
                                        RTAPI_GFP_KERNEL);
...
int rtapi_app_main(void)
{
    ...
    gpio->in_chips =
        NULL; // so that realloc knows that they need malloc first time throigh
    gpio->out_chips = NULL;
    ...
    if (gpio->num_in_chips < 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "hal_gpio: Failed to identify all specified input pins\n");
        goto fail0;
    }
    for (c = 0; c < gpio->num_in_chips; c++) {
...
#else
        retval =
            gpiod_line_request_bulk_input(gpio->in_chips[c].lines, "linuxcnc");
#endif
        if (retval < 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
        ...
        gpio->in_chips[c].hal =
            hal_malloc(gpio->in_chips[c].num_lines * sizeof(hal_gpio_hal_t));
        for (i = 0; i < gpio->in_chips[c].num_lines; i++) {
            line_name = get_line_name(&gpio->in_chips[c], i);
            retval += hal_pin_bit_newf(HAL_OUT,
                                       &(gpio->in_chips[c].hal[i].value),
                                       comp_id,
                                       "hal_gpio.%s-in",
                                       line_name);
            retval += hal_pin_bit_newf(HAL_OUT,
                                       &(gpio->in_chips[c].hal[i].value_not),
                                       comp_id,
                                       "hal_gpio.%s-in-not",
                                       line_name);
        }
        if (retval < 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
                "hal_gpio: Failed to allocate GPIO input HAL pins\n");
            goto fail0;
        }
    }
    ...
    if (gpio->num_in_chips < 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "hal_gpio: Failed to identify all specified output pins\n");
        goto fail0;
    }
    for (c = 0; c < gpio->num_out_chips; c++) {
...
#else
        retval = gpiod_line_request_bulk_output(
            gpio->out_chips[c].lines, "linuxcnc", gpio->out_chips[c].vals);
#endif
        if (retval < 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
        ...
        if (retval < 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
                "hal_gpio: Failed to allocate GPIO output HAL pins\n");
            goto fail0;
        }
    }
    ...
    if (reset_active) {
        ...
        retval += hal_param_u32_newf(
            HAL_RW, gpio->reset_ns, comp_id, "hal_gpio.reset_ns");
        retval +=
            hal_export_funct(hal_name, hal_gpio_reset, gpio, 0, 0, comp_id);
    }
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "hal_gpio: failed to export functions\n");
        goto fail0;
    }
    hal_ready(comp_id);
    return 0;
...
static void hal_gpio_read(void *arg, __attribute__((unused)) long period)
{
    ...
    for (c = 0; c < gpio->num_in_chips; c++) {
#if LIBGPIOD_VER >= 200
        gpiod_line_request_get_values(gpio->in_chips[c].lines,
                                      gpio->in_chips[c].vals);
#else
        gpiod_line_get_value_bulk(gpio->in_chips[c].lines,
                                  gpio->in_chips[c].vals);
#endif
        for (i = 0; i < gpio->in_chips[c].num_lines; i++) {
...
#if LIBGPIOD_VER > 200
        gpiod_line_request_set_values(gpio->out_chips[c].lines,
                                      gpio->out_chips[c].vals);
#else
        gpiod_line_set_value_bulk(gpio->out_chips[c].lines,
                                  gpio->out_chips[c].vals);
#endif
    }
...
static void hal_gpio_reset(void *arg, long period)
{
    ...
    for (c = 0; c < gpio->num_out_chips; c++) {
        for (i = 0; i < gpio->out_chips[c].num_lines; i++) {
            if ((gpio->out_chips[c].flags[i] & 0x60) ==
                0x60) { // inverted and reset
                gpio->out_chips[c].vals[i] = 1;
            } else if ((gpio->out_chips[c].flags[i] & 0x60) ==
                       0x40) { // only reset
