...
typedef struct {
    ...
    __s32 counts[3];
    hal_float_t
        pos_scale; /*! \todo scale for position command FIXME should be one per axis */
} evoreg_t;
...
#define ISA_MAX 0x100000 /* allgemeiner Speicherzugriff */
    ...
    rtapi_print_msg(
        RTAPI_MSG_ERR, "EVOREG: io_base: %p \n", port_data_array->io_base);
    ...
    for (num_dac = 1; num_dac <= MAX_DAC; num_dac++) {
        retval = hal_pin_float_newf(HAL_IN,
                                    &(port_data_array->dac_out[num_dac - 1]),
                                    comp_id,
                                    "evoreg.%d.dac-%02d-out",
                                    1,
                                    num_dac);
        if (retval < 0) {
            rtapi_print_msg(
    ...
    for (num_enc = 1; num_enc <= MAX_ENC; num_enc++) {
        retval = hal_pin_float_newf(HAL_OUT,
                                    &(port_data_array->position[num_enc - 1]),
                                    comp_id,
                                    "evoreg.%d.position-%02d-in",
                                    1,
                                    num_enc);
        if (retval < 0) {
            rtapi_print_msg(
    ...
    for (i = 0; i <= 23; i++) {
        retval += hal_pin_bit_newf(HAL_IN,
                                   &(port_data_array->digital_out[i]),
                                   comp_id,
                                   "evoreg.%d.pin-%02d-out",
                                   1,
                                   i);
    ...
    retval = hal_param_float_newf(HAL_RW,
                                  &(port_data_array->pos_scale),
                                  comp_id,
                                  "evoreg.%d.position-scale",
                                  1);
    if (retval != 0) {
        return retval;
    }
    ...
    retval = hal_export_functf(update_port,
                               &(port_data_array[n]),
                               1,
                               0,
                               comp_id,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "EVOREG: installed driver for %d card(s)\n", num_ports);
    hal_ready(comp_id);
    return 0;
}
