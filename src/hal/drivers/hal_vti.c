...
static int vti_parse_dio(void)
{
	...
	while (i < strlen(dio)) {
	  switch (dio[i]) {
		...
		case 'i':
		  vti_driver->dir_bits[nibble] = 0;
		  if (dio[i + 1] == 'i') vti_driver->dir_bits[nibble + 1] = 0;
		  else if (dio[i + 1] == 'o') vti_driver->dir_bits[nibble + 1] = 1;
		  else return -1;
		  break;
		case 'o':
		  vti_driver->dir_bits[nibble] = 1;
...
static void vti_di_read(void *arg, long period) //reads digital inputs from the vti
{
	...
	for (i = 1; i < MAX_IO_PORTS; i++) {
		if (vti->dir_bits[i * 2] == 0)
			split_input(dac->dio[i - 1], &(vti->port[i][0]);
		if (vti->dir_bits[i * 2 + 1] == 0)
			split_input(dac->dio[i - 1], &(vti->port[i][4]);
	  }
}
...
static int vti_adc_init(int channels)
{
	...
	for (i = 0; i < channels; i++) {
		...
		retval = export_adc(n, vti_driver);
		if (retval != 0) {
			rtapi_print_msg(RTAPI_MSG_ERR,
...
static int vti_dio_init(int nibbles)
  {
	...
	if (vti_driver->dir_bits[0] == 1) mask |= 0x00000100; // Set mask for bits 0-3
	if (vti_driver->dir_bits[1] == 1) mask |= 0x00000200; // Set mask for bits 4-7
	...
	for (i = 0; i < 16; i++) { // Set direction for extended I/O points 0..63
		if (vti_driver->dir_bits[i + 2] == 1) mask |= (1 << i);
	  }
	dac->config0 = mask;

	mask = 0;
	for (i = 0; i < 16; i++) { // Set direction for extended I/O points 64..127
		if (vti_driver->dir_bits[i + 18] == 1) mask |= (1 << i);
	  }
	dac->config1 = mask;
...
static long vti_counter_read(int axis)
{
	...
	EncData.Long = vti_driver->count[axis];
...
static int export_dio_pins(int io_points)
{
	...
	/*          point, direction, driver */
		export_pin(i, vti_driver->dir_bits[i / 4], vti_driver); }
