...
static char *dio =
    "ii"; /* dio config - default = port A&B inputs, port C&D outputs */
...
typedef struct {
    ...
    /* dio data */
    io_pin port[MAX_IO_PORTS]
               [PINS_PER_PORT]; /* Holds MAX_IO_PORTS X PINS_PER_PORT
						   number of discrete I/O points */
    unsigned char dir_bits
        [MAX_IO_PORTS *
         2]; /* remembers config (which port is input which is output) */
...
static void vti_counter_capture(void *arg,
                                long period); //captures encoder counters
static void vti_di_read(void *arg,
                        long period); //reads digital inputs from the vti
static void vti_do_write(void *arg,
                         long period); //writes digital outputs to the vti
...
int rtapi_app_main(void)
{
    ...
    if ((num_chan <= 0) || (num_chan > MAX_CHAN)) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "VTI: ERROR: invalid num_chan: %d\n", num_chan);
        return -1;
    }
    ...
    retval = hal_export_funct(
        "vti.capture-position", vti_counter_capture, vti_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "VTI: installed %d encoder counters\n", num_chan);

    retval = hal_export_funct(
        "vti.write-dacs", vti_dacs_write, vti_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_export_funct(
        "vti.read-adcs", vti_adcs_read, vti_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval =
        hal_export_funct("vti.di-read", vti_di_read, vti_driver, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "VTI: installed %d digital inputs\n", inputpinnum);

    retval = hal_export_funct(
        "vti.do-write", vti_do_write, vti_driver, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "VTI: installed %d digital outputs\n", outpinnum);
...
static int vti_dac_init(int channels)
{
    ...
    encoder->DAC =
        DAC_IND_MODE; // Enable DACs for output independent of watchdog
    for (i = 0; i < channels; i++) {
        retval = export_dac(i, vti_driver);
        if (retval != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "VTI: ERROR: dac %d var export failed\n", i + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
...
static int vti_init_card()
{
    ...
    if (retval == 0) {
        encoder = (volatile struct encoder *)ioremap(pci_resource_start(dev, 2),
                                                     sizeof(encoder));
        dac = (volatile struct dac *)ioremap(pci_resource_start(dev, 4),
                                             sizeof(dac));
        timer = (volatile struct timer *)ioremap(pci_resource_start(dev, 3),
                                                 sizeof(timer));
...
static int vti_autodetect()
{
    ...
    if (dev) {
        ...
        rtapi_print_msg(RTAPI_MSG_INFO,
                        "VTI: Exiting with auto detect failed\n");
        return (-ENODEV);
    }
}
...
static int export_counter(int num, vti_struct *addr)
{
    ...
    retval = hal_pin_s32_newf(
        HAL_OUT, &addr->count[num], comp_id, "vti.%d.counts", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled position captured by update() */
    retval = hal_pin_float_newf(
        HAL_OUT, &addr->pos[num], comp_id, "vti.%d.position", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for scaling */
    retval = hal_param_float_newf(
        HAL_RW, &addr->pos_scale[num], comp_id, "vti.%d.position-scale", num);
    if (retval != 0) {
        return retval;
    }
...
static int export_dac(int num, vti_struct *addr)
{
    ...
    retval = hal_pin_float_newf(
        HAL_IN, &addr->dac_value[num], comp_id, "vti.%d.dac-value", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for offset */
    retval = hal_param_float_newf(
        HAL_RW, &addr->dac_offset[num], comp_id, "vti.%d.dac-offset", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for gain */
    retval = hal_param_float_newf(
        HAL_RW, &addr->dac_gain[num], comp_id, "vti.%d.dac-gain", num);
    if (retval != 0) {
        return retval;
    }
...
static int export_input_pin(int pinnum, io_pin *pin)
{
    ...
    retval =
        hal_pin_bit_newf(HAL_OUT, &(pin->data), comp_id, "vti.in-%02d", pinnum);
    if (retval != 0)
        return retval;
    /* export additional pin for inverted input data */
    retval = hal_pin_bit_newf(
        HAL_OUT, &(pin->io.not), comp_id, "vti.in-%02d-not", pinnum);
    /* initialize HAL pins */
    *(pin->data) = 0;
    *(pin->io.not) = 1;
    return retval;
...
static int export_output_pin(int pinnum, io_pin *pin)
{
    ...
    retval =
        hal_pin_bit_newf(HAL_IN, &(pin->data), comp_id, "vti.out-%02d", pinnum);
    if (retval != 0)
        return retval;
    /* export parameter for polarity */
    retval = hal_param_bit_newf(
        HAL_RW, &(pin->io.invert), comp_id, "vti.out-%02d-invert", pinnum);
