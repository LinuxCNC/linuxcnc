...
static int Device_ExportEncoderPinsParametersFunctions(Device *this,
                                                       int componentId,
                                                       int boardId);
static int Device_ExportDacPinsParametersFunctions(Device *this,
                                                   int componentId,
                                                   int boardId);
static int Device_ExportAdcPinsParametersFunctions(Device *this,
                                                   int componentId,
                                                   int boardId);
static int Device_ExportDigitalInPinsParametersFunctions(Device *this,
                                                         int componentId,
                                                         int boardId);
static int Device_ExportDigitalOutPinsParametersFunctions(Device *this,
                                                          int componentId,
                                                          int boardId);
static int Device_ExportMiscPinsParametersFunctions(Device *this,
                                                    int componentId,
                                                    int boardId);
...
int rtapi_app_main(void)
{
    ...
    // Find a MOTENC card.
    while ((i < MAX_DEVICES) &&
           ((pDev = pci_get_device(MOTENC_VENDOR_ID, MOTENC_DEVICE_ID, pDev)) !=
            NULL)) {
        ...
        if (pDevice == 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "MOTENC: ERROR: hal_malloc() failed\n");
        ...
        pCard = (MotencRegMap *)ioremap_nocache(pci_resource_start(pDev, 2),
                                                pci_resource_len(pDev, 2));
        rtapi_print_msg(RTAPI_MSG_INFO,
                        "MOTENC: Card detected in slot %2x\n",
                        PCI_SLOT(pDev->devfn));
        rtapi_print_msg(RTAPI_MSG_INFO,
                        "MOTENC: Card address @ %p, Len = %d\n",
                        pCard,
        ...
        rtapi_print_msg(RTAPI_MSG_INFO,
                        "MOTENC: Card is %s, ID: %d\n",
                        pDevice->pTypeName,
                        pDevice->boardID);
        if (pDevice->boardType == 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "MOTENC: ERROR, unknown card detected\n");
            hal_exit(driver.componentId);
            return (-ENODEV);
        }
        ...
        if (driver.idPresent[pDevice->boardID] != 0) {
            ...
            while (driver.idPresent[j] != 0) {
                ...
                if (j >= MAX_DEVICES) {
                    rtapi_print_msg(
                        RTAPI_MSG_ERR,
                        "MOTENC: ERROR, duplicate ID, can't remap\n");
                    hal_exit(driver.componentId);
                    return (-EINVAL);
                }
            }
            ...
            rtapi_print_msg(RTAPI_MSG_WARN,
                            "MOTENC: WARNING, duplicate ID, remapped to %d\n",
                            j);
        }
    ...
    if (pCard == NULL) {
        // No card present.
        rtapi_print_msg(RTAPI_MSG_WARN,
                        "MOTENC: **** No MOTENC card detected ****\n");
        hal_exit(driver.componentId);
        return -ENODEV;
    }
...
static int Device_Init(Device *this, MotencRegMap *pCard)
{
    ...
    if (status == 0) {
        ...
        rtapi_print_msg(
            RTAPI_MSG_ERR, "MOTENC: unknown board type - %i\n", status);
        this->numFpga = 0;
        //return -1;
    }
    ...
    this->boardID =
        (status & MOTENC_STATUS_BOARD_ID) >> MOTENC_STATUS_BOARD_ID_SHFT;
...
static int Device_ExportPinsParametersFunctions(Device *this, int componentId)
{
    ...
    // Export encoders.
    error =
        Device_ExportEncoderPinsParametersFunctions(this, componentId, boardId);

    // Export DACs.
    if (!error)
        error =
            Device_ExportDacPinsParametersFunctions(this, componentId, boardId);

    // Export ADCs.
    if (!error)
        error =
            Device_ExportAdcPinsParametersFunctions(this, componentId, boardId);

    // Export digital I/O.
    if (!error)
        error = Device_ExportDigitalInPinsParametersFunctions(
            this, componentId, boardId);
    if (!error)
        error = Device_ExportDigitalOutPinsParametersFunctions(
            this, componentId, boardId);

    // Export miscellaneous.
    if (!error)
        error = Device_ExportMiscPinsParametersFunctions(
            this, componentId, boardId);
                                                       ...
                                                       int boardId)
{
    ...
    halError = 0;
    for (channel = 0;
         channel < this->numFpga * MOTENC_FPGA_NUM_ENCODER_CHANNELS;
         channel++) {
        // Pins.
        if ((halError = hal_pin_s32_newf(HAL_OUT,
                                         &(this->encoder[channel].pCount),
                                         componentId,
                                         "motenc.%d.enc-%02d-count",
                                         boardId,
                                         channel)) != 0)
            break;

        if ((halError = hal_pin_float_newf(HAL_OUT,
                                           &(this->encoder[channel].pPosition),
                                           componentId,
                                           "motenc.%d.enc-%02d-position",
                                           boardId,
                                           channel)) != 0)
            break;

        if ((halError = hal_pin_bit_newf(HAL_OUT,
                                         &(this->encoder[channel].pIndex),
                                         componentId,
                                         "motenc.%d.enc-%02d-index",
                                         boardId,
                                         channel)) != 0)
            break;

        if ((halError = hal_pin_bit_newf(HAL_IO,
                                         &(this->encoder[channel].pIndexEnable),
                                         componentId,
                                         "motenc.%d.enc-%02d-index-enable",
                                         boardId,
                                         channel)) != 0)
            break;

        if ((halError = hal_pin_bit_newf(HAL_IN,
                                         &(this->encoder[channel].pReset),
                                         componentId,
                                         "motenc.%d.enc-%02d-reset",
                                         boardId,
                                         channel)) != 0)
            break;

        // Parameters.
        if ((halError = hal_param_float_newf(HAL_RW,
                                             &(this->encoder[channel].scale),
                                             componentId,
                                             "motenc.%d.enc-%02d-scale",
                                             boardId,
                                             channel)) != 0)
            break;
    ...
    // Export functions.
    if (!halError) {
        halError = hal_export_functf(Device_EncoderRead,
                                     this,
                                     1,
                                     0,
                                     componentId,
                                     "motenc.%d.encoder-read",
                                     boardId);
    }

    if (halError) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "MOTENC: ERROR: export encoder failed\n");
        return (-1);
    }
                                                   ...
                                                   int boardId)
{
    ...
    for (channel = 0; channel < MOTENC_NUM_DAC_CHANNELS; channel++) {
        // Pins.
        if ((halError = hal_pin_float_newf(HAL_IN,
                                           &(this->dac[channel].pValue),
                                           componentId,
                                           "motenc.%d.dac-%02d-value",
                                           boardId,
                                           channel)) != 0)
            break;
        ...
        // Parameters.
        if ((halError = hal_param_float_newf(HAL_RW,
                                             &(this->dac[channel].offset),
                                             componentId,
                                             "motenc.%d.dac-%02d-offset",
                                             boardId,
                                             channel)) != 0)
            break;

        if ((halError = hal_param_float_newf(HAL_RW,
                                             &(this->dac[channel].gain),
                                             componentId,
                                             "motenc.%d.dac-%02d-gain",
                                             boardId,
                                             channel)) != 0)
            break;
    ...
    // Export functions.
    if (!halError) {
        halError = hal_export_functf(Device_DacWrite,
                                     this,
                                     1,
                                     0,
                                     componentId,
                                                   ...
                                                   int boardId)
{
    ...
    for (channel = 0; channel < MOTENC_NUM_ADC_CHANNELS; channel++) {
        // Pins.
        if ((halError = hal_pin_float_newf(HAL_OUT,
                                           &(this->adc[channel].pValue),
                                           componentId,
                                           "motenc.%d.adc-%02d-value",
                                           boardId,
                                           channel)) != 0)
            break;
        ...
        // Parameters.
        if ((halError = hal_param_float_newf(HAL_RW,
                                             &(this->adc[channel].offset),
                                             componentId,
                                             "motenc.%d.adc-%02d-offset",
                                             boardId,
                                             channel)) != 0)
            break;

        if ((halError = hal_param_float_newf(HAL_RW,
                                             &(this->adc[channel].gain),
                                             componentId,
                                             "motenc.%d.adc-%02d-gain",
                                             boardId,
                                             channel)) != 0)
            break;
    ...
    // Export functions.
    if (!halError) {
        halError = hal_export_functf(Device_AdcRead,
                                     this,
                                     1,
                                     0,
                                     componentId,
                                                         ...
                                                         int boardId)
{
    ...
    halError = 0;
    for (channel = 0;
         channel < (this->numFpga * MOTENC_FPGA_NUM_DIGITAL_INPUTS - 4);
         channel++) {
        // Pins.
        if ((halError = hal_pin_bit_newf(HAL_OUT,
                                         &(this->in[channel].pValue),
                                         componentId,
                                         "motenc.%d.in-%02d",
                                         boardId,
                                         channel)) != 0)
    ...
    // Export functions.
    if (!halError) {
        halError = hal_export_functf(Device_DigitalInRead,
                                     this,
                                     0,
                                     0,
                                     componentId,
                                     "motenc.%d.digital-in-read",
                                     boardId);
    }

    if (halError) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "MOTENC: ERROR: export digital in failed\n");
        return (-1);
    }
                                                          ...
                                                          int boardId)
{
    ...
    // Export pins and parameters.
    halError = 0;
    for (channel = 0; channel < this->numFpga * MOTENC_FPGA_NUM_DIGITAL_OUTPUTS;
         channel++) {
        // Pins.
        if ((halError = hal_pin_bit_newf(HAL_IN,
                                         &(this->out[channel].pValue),
                                         componentId,
                                         "motenc.%d.out-%02d",
                                         boardId,
                                         channel)) != 0)
        ...
        // Parameters.
        if ((halError = hal_param_bit_newf(HAL_RW,
                                           &(this->out[channel].invert),
                                           componentId,
                                           "motenc.%d.out-%02d-invert",
                                           boardId,
                                           channel)) != 0)
            break;
    ...
    // Export functions.
    if (!halError) {
        halError = hal_export_functf(Device_DigitalOutWrite,
                                     this,
                                     0,
                                     0,
                                     componentId,
                                     "motenc.%d.digital-out-write",
                                     boardId);
    }

    if (halError) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "MOTENC: ERROR: export digital out failed\n");
        return (-1);
    }
                                                    ...
                                                    int boardId)
{
    ...
    // Export Pins.
    halError = hal_pin_bit_newf(HAL_OUT,
                                &(this->misc.pEstopIn),
                                componentId,
                                "motenc.%d.estop-in",
                                boardId);
    ...
    if (!halError) {
        halError = hal_pin_bit_newf(HAL_OUT,
                                    &(this->misc.pEstopInNot),
                                    componentId,
                                    "motenc.%d.estop-in-not",
                                    boardId);
    }

    if (!halError) {
        halError = hal_pin_bit_newf(HAL_IO,
                                    &(this->misc.pWatchdogReset),
                                    componentId,
                                    "motenc.%d.watchdog-reset",
                                    boardId);
    }

    // Export Parameters.
    if (!halError) {
        halError = hal_param_u32_newf(HAL_RW,
                                      &(this->misc.watchdogControl),
                                      componentId,
                                      "motenc.%d.watchdog-control",
                                      boardId);
    }
    ...
    // Export functions.
    if (!halError) {
        halError = hal_export_functf(Device_MiscUpdate,
                                     this,
                                     0,
                                     0,
                                     componentId,
                                     "motenc.%d.misc-update",
                                     boardId);
    }

    if (halError) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "MOTENC: ERROR: export miscellaneous failed\n");
        return (-1);
    }
...
static void Device_EncoderRead(void *arg, long period)
{
    ...
    for (i = 0; i < this->numFpga; i++) {
        ...
        for (j = 0; j < MOTENC_FPGA_NUM_ENCODER_CHANNELS; j++, pEncoder++) {
            ...
            if (*(pEncoder->pReset)) {
                // Reset encoder.
                pCard->fpga[i].statusControl =
                    1 << (MOTENC_CONTROL_ENCODER_RESET_SHFT + j);
            }

            // check state of hardware index pin
            *(pEncoder->pIndex) =
                (status >> (MOTENC_STATUS_INDEX_SHFT + j)) & 1;
...
static void Device_DacWrite(void *arg, long period)
{
    ...
    for (i = 0; i < MOTENC_NUM_DAC_CHANNELS; i++, pDac++) {
        ...
        dacCount = (hal_u32_t)(volts * MOTENC_DAC_SCALE_MULTIPLY /
                                   MOTENC_DAC_SCALE_DIVIDE +
                               MOTENC_DAC_COUNT_ZERO);
...
static void Device_MiscUpdate(void *arg, long period)
{
    ...
    // Update E-STOP pin.
    *(this->misc.pEstopIn) =
        (pCard->fpga[0].statusControl & MOTENC_STATUS_ESTOP) ? 1 : 0;
