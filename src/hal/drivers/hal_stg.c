...
static char *dio =
    "IIOO"; /* dio config - default = port A&B inputs, port C&D outputs */
...
typedef struct {
    ...
    hal_s32_t offset
        [MAX_CHANS]; /* offset to hold latched position from index pulse */
    ...
    hal_bit_t
        *index_latch[MAX_CHANS]; /* value of the index latch for the axis */
    ...
    unsigned char
        dir_bits; /* remembers config (which port is input which is output) */
...
#define CTRL(x)                                                                \
    (base + (2 * (x + 1)) - (x % 2)) /* Address of Control register 0 */
...
static void stg_adcs_read(
    void *arg,
    long
        period); //reads adc data from the board, check long description at the beginning of the function
static void stg_dacs_write(void *arg, long period); //writes dac's to the STG
static void stg_counter_capture(void *arg,
                                long period); //captures encoder counters
static void stg_di_read(void *arg,
                        long period); //reads digital inputs from the STG
static void stg_do_write(void *arg,
                         long period); //writes digital outputs to the STG
...
int rtapi_app_main(void)
{
    ...
    if ((num_chan <= 0) || (num_chan > MAX_CHAN)) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "STG: ERROR: invalid num_chan: %d\n", num_chan);
        return -1;
    }
    ...
    for (m = 0; m < 4; m++) {
        ...
        if ((dio[m] == 'i') || (dio[m] == 'I')) {
            ...
            rtapi_print_msg(
                RTAPI_MSG_ERR, "STG: ERROR: bad config info for port %d\n", m);
            return -1;
        }
    ...
    stg_driver->dir_bits =
        dir_bits; /* remember direction of each port, will be used in the write / read functions */
    ...
    for (n = 0; n < num_chan; n++) {
        ...
        *(stg_driver->index_latch[n]) = 0;
        ...
        if (retval != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "STG: ERROR: dac %d var export failed\n", n + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
        *(stg_driver->dac_value[n]) = 0;
        ...
        if (retval != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "STG: ERROR: adc %d var export failed\n", n + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
        *(stg_driver->adc_value[n]) = 0;
        ...
        stg_driver->adc_current_chan =
            -1; /* notify that no conversion has been started yet */
    ...
    retval = hal_export_funct(
        "stg.capture-position", stg_counter_capture, stg_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "STG: installed %d encoder counters\n", num_chan);

    retval = hal_export_funct(
        "stg.write-dacs", stg_dacs_write, stg_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval = hal_export_funct(
        "stg.read-adcs", stg_adcs_read, stg_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    retval =
        hal_export_funct("stg.di-read", stg_di_read, stg_driver, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "STG: installed %d digital inputs\n", inputpinnum);

    retval = hal_export_funct(
        "stg.do-write", stg_do_write, stg_driver, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "STG: installed %d digital outputs\n", outpinnum);
...
static void stg_adcs_read(void *arg, long period)
{
    ...
    if (stg->adc_current_chan++ >= num_chan)
        stg->adc_current_chan =
            0; //increase the channel, and roll back to 0 after all chans are done
                        ...
                        long period) //reads digital inputs from the STG
{
    ...
    if ((stg->dir_bits & 0x01) ==
        0) { // if port A is set as input, read the bits
        if (stg->model == 1)
            val = inb(base + DIO_A);
        else
            val = inb(base + PORT_A);
    ...
    if ((stg->dir_bits & 0x02) ==
        0) { // if port B is set as input, read the bits
        if (stg->model == 1)
            val = inb(base + DIO_B);
        else
            val = inb(base + PORT_B);
    ...
    if ((stg->dir_bits & 0x04) ==
        0) { // if port C is set as input, read the bits
        if (stg->model == 1)
            val = inb(base + DIO_C);
        else
            val = inb(base + PORT_C);
    ...
    if ((stg->dir_bits & 0x08) ==
        0) { // if port D is set as input, read the bits
        if (stg->model == 1)
            val = inb(base + DIO_D);
        else
            val = inb(base + PORT_D);
                         ...
                         long period) //writes digital outputs to the STG
{
    ...
    if ((stg->dir_bits & 0x01) !=
        0) { // if port A is set as output, write the bits
    ...
    if ((stg->dir_bits & 0x02) !=
        0) { // if port B is set as output, write the bits
    ...
    if ((stg->dir_bits & 0x04) !=
        0) { // if port C is set as output, write the bits
    ...
    if ((stg->dir_bits & 0x08) !=
        0) { // if port D is set as output, write the bits
...
static int stg_adc_init(int ch)
{
    ...
    if (stg_driver->model == 1)
        outb(
            0x0f,
            base +
                MIO_2); // the second 82C55 is already configured (by running stg_dio_init)
...
static int stg_dio_init(void)
{
    ...
    if ((stg_driver->dir_bits & 0x01) ==
        0) // if port A is set as input, set bit accordingly
        control |= 0x10;
    if ((stg_driver->dir_bits & 0x02) ==
        0) // if port B is set as input, set bit accordingly
        control |= 0x02;
    if ((stg_driver->dir_bits & 0x04) ==
        0) // if port C is set as input, set bits accordingly
        control |= 0x09;
    ...
    if (stg_driver->model == 1) {
        ...
        if ((stg_driver->dir_bits & 0x08) ==
            0) // if port D is set as input, set bits accordingly
            control = 0x92;
...
static int stg_init_card()
{
   ...
   */
    if ((base == 0x00) || (stg_driver->model == 0)) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "STG: ERROR: no stg1 or stg2 card could be initialised\n");
        return -ENODEV;
    }
     ...
     */
        ...
        outb(
            0x1a,
            base +
                ICW1); // initialize the 82C59 as single chip (STG docs say so:)
     ...
     */
        ...
        stg_set_interrupt(
            5); // initialize it to something, we won't use it anyways
        rtapi_print_msg(RTAPI_MSG_INFO,
                        "STG: Initialised stg%1d card at address %x\n",
                        stg_driver->model,
                        base);
        ...
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "STG: ERROR: The model stg%1d is not correct\n",
                        stg_driver->model);
        return -ENODEV;
    }
...
static int export_counter(int num, stg_struct *addr)
{
    ...
    retval = hal_pin_s32_newf(
        HAL_OUT, &addr->count[num], comp_id, "stg.%d.counts", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled position captured by update() */
    retval = hal_pin_float_newf(
        HAL_OUT, &addr->pos[num], comp_id, "stg.%d.position", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for scaling */
    retval = hal_param_float_newf(
        HAL_RW, &addr->pos_scale[num], comp_id, "stg.%d.position-scale", num);
    if (retval != 0) {
        return retval;
    }
    ...
    retval = hal_pin_bit_newf(
        HAL_IO, &addr->index_enable[num], comp_id, "stg.%d.index-enable", num);
    if (retval != 0) {
        return retval;
    }
    ...
    retval = hal_pin_bit_newf(
        HAL_OUT, &addr->index_latch[num], comp_id, "stg.%d.index-latch", num);
    if (retval != 0) {
        return retval;
    }
   ...
   */
    if (addr->model == 1) {
        ...
        retval = hal_pin_bit_newf(HAL_IN,
                                  &addr->index_polarity[num],
                                  comp_id,
                                  "stg.%d.index-polarity",
                                  num);
        if (retval != 0) {
            return retval;
        }
...
static int export_dac(int num, stg_struct *addr)
{
    ...
    retval = hal_pin_float_newf(
        HAL_IN, &addr->dac_value[num], comp_id, "stg.%d.dac-value", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for offset */
    retval = hal_param_float_newf(
        HAL_RW, &addr->dac_offset[num], comp_id, "stg.%d.dac-offset", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for gain */
    retval = hal_param_float_newf(
        HAL_RW, &addr->dac_gain[num], comp_id, "stg.%d.dac-gain", num);
    if (retval != 0) {
        return retval;
    }
...
static int export_adc(int num, stg_struct *addr)
{
    ...
    /* export pin for voltage received by the board() */
    retval = hal_pin_float_newf(
        HAL_OUT, &addr->adc_value[num], comp_id, "stg.%d.adc-value", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for offset */
    retval = hal_param_float_newf(
        HAL_RW, &addr->adc_offset[num], comp_id, "stg.%d.adc-offset", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for gain */
    retval = hal_param_float_newf(
        HAL_RW, &addr->adc_gain[num], comp_id, "stg.%d.adc-gain", num);
    if (retval != 0) {
        return retval;
    }
    /* restore saved message level */
...
static int export_input_pin(int pinnum, io_pin *pin)
{
    ...
    retval =
        hal_pin_bit_newf(HAL_OUT, &(pin->data), comp_id, "stg.in-%02d", pinnum);
    if (retval != 0) {
        return retval;
    }
    /* export additional pin for inverted input data */
    retval = hal_pin_bit_newf(
        HAL_OUT, &(pin->io.not), comp_id, "stg.in-%02d-not", pinnum);
    /* initialize HAL pins */
    *(pin->data) = 0;
    *(pin->io.not) = 1;
...
static int export_output_pin(int pinnum, io_pin *pin)
{
   ...
   */
    ...
    retval =
        hal_pin_bit_newf(HAL_IN, &(pin->data), comp_id, "stg.out-%02d", pinnum);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for polarity */
    retval = hal_param_bit_newf(
        HAL_RW, &(pin->io.invert), comp_id, "stg.out-%02d-invert", pinnum);
