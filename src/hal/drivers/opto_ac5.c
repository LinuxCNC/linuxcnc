...
static int Device_ExportPinsParametersFunctions(board_data_t *pboard,
                                                int comp_id,
                                                int boardId);
static int Device_ExportDigitalInPinsParametersFunctions(board_data_t *pboard,
                                                         int comp_id,
                                                         int boardId);
static int Device_ExportDigitalOutPinsParametersFunctions(board_data_t *pboard,
                                                          int comp_id,
                                                          int boardId);
...
int rtapi_app_main(void)
{
    ...
    if (driver.comp_id < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        " ERROR OPTO_AC5--- hal_init() failed\n");
        return (-1);
    }
    ...
    for (n = 0; n < MAX_BOARDS; n++) {
        ...
        if (pboard == NULL) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "ERROR OPTO_AC5--- hal_malloc() failed\n");
            rtapi_app_exit();
            return -1;
        }
        // save pointer
        ...
        pboard->base =
            ioremap_nocache(pci_resource_start(pDev, 0), pboard->len);
        if (pboard->base == NULL) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
...
static int Device_ExportPinsParametersFunctions(board_data_t *this,
                                                int componentId,
                                                int boardId)
{
...
static int Device_ExportDigitalInPinsParametersFunctions(board_data_t *this,
                                                         int comp_id,
                                                         int boardId)
{
    ...
    while (portnum < 2) {
        ...
        for (channel = 0; channel < 24; channel++) {
            ...
            {
                // Pins.
                if ((halError = hal_pin_bit_newf(
                         HAL_OUT,
                         &(this->port[portnum].io[channel].pValue),
                         comp_id,
                         "opto-ac5.%d.port%d.in-%02d",
                         boardId,
                         portnum,
                         channel)) != 0)
                    break;
                ...
                if ((halError = hal_pin_bit_newf(
                         HAL_OUT,
                         &(this->port[portnum].io[channel].pValueNot),
                         comp_id,
                         "opto-ac5.%d.port%d.in-%02d-not",
                         boardId,
                         portnum,
                         channel)) != 0)
                    break;
    ...
    if (halError) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "ERROR OPTO22_AC5---exporting digital inputs to HAL failed\n");
        return (-1);
    }
...
static int Device_ExportDigitalOutPinsParametersFunctions(board_data_t *this,
                                                          int comp_id,
                                                          int boardId)
{
    int halError = 0, channel, mask, portnum = 0;
    ...
    while (portnum < 2) {
        ...
        for (channel = 0; channel < 24; channel++) {
            if ((this->port[portnum].mask & mask) != 0) //phyical output?
            {
                ...
                // Parameters.
                if ((halError = hal_param_bit_newf(
                         HAL_RW,
                         &(this->port[portnum].io[channel].invert),
                         comp_id,
                         "opto-ac5.%d.port%d.out-%02d-invert",
                         boardId,
                         portnum,
                         channel)) != 0)
                    break;
    ...
    for (channel = 0; channel < 2; channel++) {
        if ((halError = hal_pin_bit_newf(HAL_IN,
                                         &(this->port[portnum].io[24].pValue),
                                         comp_id,
                                         "opto-ac5.%d.led%d",
                                         boardId,
                                         channel + portnum)) != 0)
        ...
        if ((halError = hal_pin_bit_newf(HAL_IN,
                                         &(this->port[portnum].io[25].pValue),
                                         comp_id,
                                         "opto-ac5.%d.led%d",
                                         boardId,
                                         channel + portnum + 1)) != 0)
    ...
    if (halError) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "ERROR OPTO_AC5---exporting digital outputs to HAL failed\n");
        return (-1);
    }
...
static void Device_DigitalInRead(void *arg, long period)
{
    ...
    while (portnum < 2) {
        ...
        for (i = 0; i < 24; i++, pDigital++) {
            if ((pboard->port[portnum].mask & mask) ==
                0) // is it an input bit ?
            {
                if ((pins & mask) != 0) {
                    *(pDigital->pValue) = 0;
                } else {
...
static void Device_DigitalOutWrite(void *arg, long period)
{
    ...
    while (portnum < 2) {
        ...
        // CHECK LED PINS
        pDigital =
            &pboard->port[portnum].io[23]; //one before what we want to check
        for (i = 0; i < 2; i++) {
