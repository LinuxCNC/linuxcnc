...
static int export_output_pin(int portnum,
                             int pin,
                             hal_bit_t **dbase,
                             hal_bit_t *pbase,
                             hal_bit_t *rbase,
...
int rtapi_app_main(void)
{
    ...
    for (n = 0; n < num_ports; n++) {
        ...
        retval = hal_export_functf(read_port,
                                   &(port_data_array[n]),
                                   0,
                                   0,
                                   comp_id,
        ...
        /* export write function */
        retval = hal_export_functf(reset_port,
                                   &(port_data_array[n]),
                                   0,
                                   0,
                                   comp_id,
    ...
    retval = hal_export_funct(
        "parport.read-all", read_all, port_data_array, 0, 0, comp_id);
    if (retval != 0) {
    ...
    retval = hal_export_funct(
        "parport.write-all", write_all, port_data_array, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    rtapi_print_msg(
        RTAPI_MSG_INFO, "PARPORT: installed driver for %d ports\n", num_ports);
    hal_ready(comp_id);
    return 0;
}
...
static void reset_port(void *arg, long period)
{
    ...
    unsigned char outdata =
        (unsigned char)((port->outdata & ~port->reset_mask) ^ port->reset_val);
    ...
    outdata = (unsigned char)((port->outdata_ctrl & ~port->reset_mask_ctrl) ^
                              port->reset_val_ctrl);
...
static int pins_and_params(char *argv[])
{
    ...
    for (n = 0; n < num_ports; n++) {
        ...
        retval = hal_parport_get(
            comp_id, &port_data_array[n].portdata, port_addr[n], -1, modes);
        ...
        if (data_dir[n]) {
            rtapi_outb(rtapi_inb(port_data_array[n].base_addr + 2) | 0x20,
                       port_data_array[n].base_addr + 2);
        }
...
static int export_port(int portnum, parport_t *port)
{
    ...
    if (port->data_dir != 0) {
        ...
        retval += export_output_pin(
            portnum, 2, port->data_out, port->data_inv, port->data_reset, 0);
        retval += export_output_pin(
            portnum, 3, port->data_out, port->data_inv, port->data_reset, 1);
        retval += export_output_pin(
            portnum, 4, port->data_out, port->data_inv, port->data_reset, 2);
        retval += export_output_pin(
            portnum, 5, port->data_out, port->data_inv, port->data_reset, 3);
        retval += export_output_pin(
            portnum, 6, port->data_out, port->data_inv, port->data_reset, 4);
        retval += export_output_pin(
            portnum, 7, port->data_out, port->data_inv, port->data_reset, 5);
        retval += export_output_pin(
            portnum, 8, port->data_out, port->data_inv, port->data_reset, 6);
        retval += export_output_pin(
            portnum, 9, port->data_out, port->data_inv, port->data_reset, 7);
        retval += hal_param_u32_newf(HAL_RW,
                                     &port->reset_time,
                                     comp_id,
                                     "parport.%d.reset-time",
                                     portnum);
        retval += hal_param_u32_newf(
            HAL_RW, &port->debug1, comp_id, "parport.%d.debug1", portnum);
        retval += hal_param_u32_newf(
            HAL_RW, &port->debug2, comp_id, "parport.%d.debug2", portnum);
        port->write_time = 0;
    }
    if (port->use_control_in == 0) {
        /* declare output variables (control port) */
        retval += export_output_pin(portnum,
                                    1,
                                    port->control_out,
                                    port->control_inv,
                                    port->control_reset,
                             ...
                             int n)
{
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, dbase + n, comp_id, "parport.%d.pin-%02d-out", portnum, pin);
    if (retval != 0) {
        return retval;
    }
