...
typedef struct hal_oldname_t {
    SHMFIELD(hal_oldname_t)
    next_ptr;                    /* next struct (used for free list only) */
...
typedef struct hal_data_t {
    ...
    SHMFIELD(hal_param_t)
    param_list_ptr; /* root of linked list of parameters */
...
extern int halpr_parse_types(hal_type_t type[HAL_STREAM_MAX_PINS],
                             const char *fcg);
RTAPI_END_DECLS
#endif /* HAL_PRIV_H */
