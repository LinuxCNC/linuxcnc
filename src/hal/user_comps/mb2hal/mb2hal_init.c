...
static retCode parse_pin_names(const char *const names_string,
                               mb_tx_t *const this_mb_tx)
{
...
retCode parse_transaction_section(const int mb_tx_num)
{
    ...
    if (gbl.ini_file_ptr == NULL || mb_tx_num < 0 ||
        mb_tx_num > gbl.tot_mb_tx) {
    ...
    if (tmpstr != NULL) {
        ...
        if (mb_tx_num > 0) { //previous value
            ...
            rtapi_strxcpy(this_mb_tx->cfg_link_type_str,
                          gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str);
        } else { //required 1rst time
            this_mb_tx->cfg_link_type = -1;
            rtapi_strxcpy(this_mb_tx->cfg_link_type_str, "");
            ERR(gbl.init_dbg,
                "required 1st time [%s] [%s] not found",
                section,
                tag);
            return retERR;
        }
    ...
    DBG(gbl.init_dbg,
        ...
        section,
        tag,
        this_mb_tx->cfg_link_type_str,
        this_mb_tx->cfg_link_type);
    ...
    if (iniFindInt(
            gbl.ini_file_ptr, tag, section, &this_mb_tx->mb_tx_slave_id) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->mb_tx_slave_id =
                    gbl.mb_tx[mb_tx_num - 1].mb_tx_slave_id;
            } else {
                this_mb_tx->mb_tx_slave_id = -1;
                ERR(gbl.init_dbg,
                    "required [%s] [%s] not found, and previous LINK_TYPE is "
                    "useless",
                    section,
                    tag);
                return retERR;
    ...
    if (iniFindInt(
            gbl.ini_file_ptr, tag, section, &this_mb_tx->mb_tx_1st_addr) != 0) {
        ERR(gbl.init_dbg, "required [%s] [%s] not found", section, tag);
        return retERR;
    }
    ...
    if (iniFindInt(gbl.ini_file_ptr, tag, section, &this_mb_tx->mb_tx_nelem) !=
        ...
        this_mb_tx->mb_tx_names == NULL) {
        ERR(gbl.init_dbg,
            "required [%s] [%s] or [%s] [PIN_NAMES] were not found",
            section,
            tag,
            section);
        return retERR;
    ...
    if (iniFindDouble(
            gbl.ini_file_ptr, tag, section, &this_mb_tx->cfg_update_rate) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->cfg_update_rate =
                    gbl.mb_tx[mb_tx_num - 1].cfg_update_rate;
            }
        }
    }
    DBG(gbl.init_dbg,
    ...
    this_mb_tx->mb_response_timeout_ms =
        MB2HAL_DEFAULT_MB_RESPONSE_TIMEOUT_MS; //default
    if (iniFindInt(gbl.ini_file_ptr,
                   tag,
                   section,
                   &this_mb_tx->mb_response_timeout_ms) != 0) { //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->mb_response_timeout_ms =
                    gbl.mb_tx[mb_tx_num - 1].mb_response_timeout_ms;
            }
        }
    }
    DBG(gbl.init_dbg,
    ...
    this_mb_tx->mb_byte_timeout_ms =
        MB2HAL_DEFAULT_MB_BYTE_TIMEOUT_MS; //default
    if (iniFindInt(
            gbl.ini_file_ptr, tag, section, &this_mb_tx->mb_byte_timeout_ms) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->mb_byte_timeout_ms =
                    gbl.mb_tx[mb_tx_num - 1].mb_byte_timeout_ms;
            }
        }
    }
    DBG(gbl.init_dbg,
    ...
    if (iniFindInt(gbl.ini_file_ptr, tag, section, &this_mb_tx->cfg_debug) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
    ...
    if (tmpstr != NULL) {
        ...
        for (i = 0; i < mbtxMAX; i++) {
            if (strcasecmp(tmpstr, gbl.mb_tx_fncts[i]) == 0) {
                ...
                strncpy(this_mb_tx->mb_tx_fnct_name,
                        tmpstr,
                        sizeof(this_mb_tx->mb_tx_fnct_name) - 1);
                break;
            }
        }
        mb_tx_fnct max = gbl.version < 1001 ? mbtx_01_READ_COILS : mbtxMAX;
        if (this_mb_tx->mb_tx_fnct <= mbtxERR ||
            this_mb_tx->mb_tx_fnct >= max) {
            ERR(gbl.init_dbg,
                "[%s] [%s] [%s] out of range",
                section,
                tag,
    ...
    if (tmpstr != NULL) {
        ...
        snprintf(this_mb_tx->hal_tx_name,
                 sizeof(this_mb_tx->hal_tx_name),
                 "%02d",
                 mb_tx_num);
    }
...
retCode parse_tcp_subsection(const char *section, const int mb_tx_num)
{
    ...
    if (tmpstr != NULL) {
        strncpy(
            this_mb_tx->cfg_tcp_ip, tmpstr, sizeof(this_mb_tx->cfg_tcp_ip) - 1);
    } else {
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                rtapi_strxcpy(this_mb_tx->cfg_tcp_ip,
                              gbl.mb_tx[mb_tx_num - 1].cfg_tcp_ip);
            } else {
                rtapi_strxcpy(this_mb_tx->cfg_tcp_ip, "");
                ERR(gbl.init_dbg,
                    "required [%s] [%s] not found, and previous LINK_TYPE is "
    ...
    if (iniFindInt(gbl.ini_file_ptr, tag, section, &this_mb_tx->cfg_tcp_port) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->cfg_tcp_port =
                    gbl.mb_tx[mb_tx_num - 1].cfg_tcp_port;
            }
        }
    }
...
retCode parse_serial_subsection(const char *section, const int mb_tx_num)
{
    ...
    tmpstr = iniFind(gbl.ini_file_ptr, tag, section);
    if (tmpstr != NULL) {
        strncpy(this_mb_tx->cfg_serial_device,
                tmpstr,
                sizeof(this_mb_tx->cfg_serial_device) - 1);
    } else {
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                rtapi_strxcpy(this_mb_tx->cfg_serial_device,
                              gbl.mb_tx[mb_tx_num - 1].cfg_serial_device);
            } else {
                rtapi_strxcpy(this_mb_tx->cfg_serial_device, "");
                ERR(gbl.init_dbg,
                    "required [%s] [%s] not found, and previous LINK_TYPE is "
    ...
    DBG(gbl.init_dbg,
        ...
        section,
        tag,
        this_mb_tx->cfg_serial_device);

    tag = "SERIAL_BAUD"; //1st time required
    if (iniFindInt(
            gbl.ini_file_ptr, tag, section, &this_mb_tx->cfg_serial_baud) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->cfg_serial_baud =
                    gbl.mb_tx[mb_tx_num - 1].cfg_serial_baud;
            } else {
    ...
    if (iniFindInt(
            gbl.ini_file_ptr, tag, section, &this_mb_tx->cfg_serial_data_bit) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->cfg_serial_data_bit =
                    gbl.mb_tx[mb_tx_num - 1].cfg_serial_data_bit;
            } else {
    ...
    tmpstr = iniFind(gbl.ini_file_ptr, tag, section);
    if (tmpstr != NULL) {
        strncpy(this_mb_tx->cfg_serial_parity,
                tmpstr,
                sizeof(this_mb_tx->cfg_serial_parity) - 1);
    } else {
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                rtapi_strxcpy(this_mb_tx->cfg_serial_parity,
                              gbl.mb_tx[mb_tx_num - 1].cfg_serial_parity);
            } else {
                rtapi_strxcpy(this_mb_tx->cfg_serial_parity, "");
                ERR(gbl.init_dbg,
                    "required [%s] [%s] not found, and previous LINK_TYPE is "
    ...
    if (check_str_in(3, this_mb_tx->cfg_serial_parity, "even", "odd", "none") !=
        retOK) {
        ERR(gbl.init_dbg,
            "[%s] [%s] [%s] out of range",
            section,
            tag,
            this_mb_tx->cfg_serial_parity);
        return retERR;
    }
    DBG(gbl.init_dbg,
        "[%s] [%s] [%s]",
    ...
    if (iniFindInt(
            gbl.ini_file_ptr, tag, section, &this_mb_tx->cfg_serial_stop_bit) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->cfg_serial_stop_bit =
                    gbl.mb_tx[mb_tx_num - 1].cfg_serial_stop_bit;
            } else {
    ...
    if (iniFindInt(
            gbl.ini_file_ptr, tag, section, &this_mb_tx->cfg_serial_delay_ms) !=
        0) {                 //not found
        if (mb_tx_num > 0) { //previous value?
            if (strcasecmp(this_mb_tx->cfg_link_type_str,
                           gbl.mb_tx[mb_tx_num - 1].cfg_link_type_str) == 0) {
                this_mb_tx->cfg_serial_delay_ms =
                    gbl.mb_tx[mb_tx_num - 1].cfg_serial_delay_ms;
            }
        }
    }
    DBG(gbl.init_dbg,
...
retCode init_mb_links()
{
    ...
    for (tx_counter = 0; tx_counter < gbl.tot_mb_tx; tx_counter++) {
        ...
        if (this_mb_tx->cfg_link_type == linkRTU) { //serial
            for (lk_counter = 0; lk_counter < gbl.tot_mb_links; lk_counter++) {
                if (strcasecmp(this_mb_tx->cfg_serial_device,
                               gbl.mb_links[lk_counter].lp_serial_device) ==
                    0) {
                    isNewLink = 0; //repeated link
                    this_mb_tx->mb_link_num =
                        lk_counter; //each tx know its own link
                    break;
                }
            }
        } else { //tcp
            for (lk_counter = 0; lk_counter < gbl.tot_mb_links; lk_counter++) {
                if (strcasecmp(this_mb_tx->cfg_tcp_ip,
                               gbl.mb_links[lk_counter].lp_tcp_ip) == 0) {
                    isNewLink = 0; //repeated link
                    this_mb_tx->mb_link_num =
                        lk_counter; //each tx know its own link
                    break;
        ...
        if (isNewLink != 0) { //initialize new link
            this_mb_tx->mb_link_num =
                gbl.mb_links[gbl.tot_mb_links].mb_link_num =
                    gbl.tot_mb_links; //next available unused link
            ...
            if (this_mb_link->lp_link_type == linkRTU) { //serial
                if (strlen(this_mb_tx->cfg_serial_device) >=
                    MB2HAL_MAX_DEVICE_LENGTH) {
                    ERR(gbl.init_dbg,
                        "serial_device name to long [%s]",
                        this_mb_tx->cfg_serial_device);
                    return retERR;
                }
                rtapi_strlcpy(this_mb_link->lp_serial_device,
                              this_mb_tx->cfg_serial_device,
                              MB2HAL_MAX_DEVICE_LENGTH);
                ...
                } else if (strcasecmp(this_mb_tx->cfg_serial_parity, "odd") ==
                           0) {
                ...
                this_mb_link->lp_serial_data_bit =
                    this_mb_tx->cfg_serial_data_bit;
                this_mb_link->lp_serial_stop_bit =
                    this_mb_tx->cfg_serial_stop_bit;

                this_mb_link->modbus =
                    modbus_new_rtu(this_mb_link->lp_serial_device,
                                   this_mb_link->lp_serial_baud,
                                   this_mb_link->lp_serial_parity,
                                   this_mb_link->lp_serial_data_bit,
                                   this_mb_link->lp_serial_stop_bit);
                if (this_mb_link->modbus == NULL) {
                    ERR(gbl.init_dbg,
                        "modbus_new_rtu failed [%s] [%d] [%c] [%d] [%d]",
                        this_mb_link->lp_serial_device,
                        this_mb_link->lp_serial_baud,
                        this_mb_link->lp_serial_parity,
                        this_mb_link->lp_serial_data_bit,
                        this_mb_link->lp_serial_stop_bit);
                ...
                if (strlen(this_mb_tx->cfg_tcp_ip) >=
                    sizeof(this_mb_link->lp_tcp_ip)) {
                    ERR(gbl.init_dbg,
                        "tcp_ip too long [%s]",
                        this_mb_tx->cfg_tcp_ip);
                    return retERR;
                }
                rtapi_strlcpy(this_mb_link->lp_tcp_ip,
                              this_mb_tx->cfg_tcp_ip,
                              sizeof(this_mb_tx->cfg_tcp_ip));
                this_mb_link->lp_tcp_port = this_mb_tx->cfg_tcp_port;

                this_mb_link->modbus = modbus_new_tcp(
                    this_mb_link->lp_tcp_ip, this_mb_link->lp_tcp_port);
                if (this_mb_link->modbus == NULL) {
                    ERR(gbl.init_dbg,
                        "modbus_new_tcp failed [%s] [%d]",
                        this_mb_link->lp_tcp_ip,
                        this_mb_link->lp_tcp_port);
                    return retERR;
                }
            }
    ...
    for (lk_counter = 0; lk_counter < gbl.tot_mb_links; lk_counter++) {
        ...
        if (this_mb_link->lp_link_type == linkRTU) { //serial
            DBG(gbl.init_dbg,
                "LINK %d (RTU) link_type[%d] device[%s] baud[%d] data[%d] "
                "parity[%c] stop[%d] fd[%d]",
                lk_counter,
                this_mb_link->lp_link_type,
                this_mb_link->lp_serial_device,
                this_mb_link->lp_serial_baud,
                this_mb_link->lp_serial_data_bit,
                this_mb_link->lp_serial_parity,
            ...
            DBG(gbl.init_dbg,
                "LINK %d (TCP) link_type[%d] IP[%s] port[%d] fd[%d]",
                lk_counter,
                this_mb_link->lp_link_type,
                this_mb_link->lp_tcp_ip,
                this_mb_link->lp_tcp_port,
...
retCode init_mb_tx()
{
    ...
    for (tx_counter = 0; tx_counter < gbl.tot_mb_tx; tx_counter++) {
        ...
        this_mb_tx->protocol_debug =
            (this_mb_tx->cfg_debug >= debugDEBUG) ? 1 : 0;

        if (this_mb_tx->cfg_update_rate > 0) {
            this_mb_tx->time_increment =
                1.0 / this_mb_tx->cfg_update_rate; //wait time between tx
        }
        this_mb_tx->next_time = 0; //next time for this tx

        DBG(gbl.init_dbg,
            "MB_TX %d lk_n[%d] tx_n[%d] cfg_dbg[%d] lk_dbg[%d] t_inc[%0.3f] "
            "nxt_t[%0.3f]",
            tx_counter,
            this_mb_tx->mb_link_num,
            this_mb_tx->mb_tx_num,
            this_mb_tx->cfg_debug,
            this_mb_tx->protocol_debug,
            this_mb_tx->time_increment,
            this_mb_tx->next_time);
    }
