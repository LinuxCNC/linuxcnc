...
retCode create_HAL_pins()
{
    ...
    for (tx_counter = 0; tx_counter < gbl.tot_mb_tx; tx_counter++) {
        if (create_each_mb_tx_hal_pins(&gbl.mb_tx[tx_counter]) != retOK) {
            ERR(gbl.init_dbg,
                "failed to initialize hal pins in tx_num[%d] [%d] [%s]",
                tx_counter,
                gbl.mb_tx[tx_counter].mb_tx_fnct,
                gbl.mb_tx[tx_counter].mb_tx_fnct_name);
            return retERR;
        }
    }
...
retCode create_each_mb_tx_hal_pins(mb_tx_t *mb_tx)
{
    ...
    if (mb_tx->num_errors == NULL) {
        ERR(gbl.init_dbg,
            "[%d] [%s] NULL hal_malloc num_errors",
            mb_tx->mb_tx_fnct,
            mb_tx->mb_tx_fnct_name);
        return retERR;
    }
    memset(mb_tx->num_errors, 0, sizeof(hal_u32_t *));
    ret = snprintf(hal_pin_name,
                   HAL_NAME_LEN,
                   "%s.%s.num_errors",
                   gbl.hal_mod_name,
                   mb_tx->hal_tx_name);
    if (ret >= HAL_NAME_LEN || ret < 0 ||
        0 != hal_pin_u32_newf(HAL_OUT,
                              mb_tx->num_errors,
                              gbl.hal_mod_id,
                              "%s",
                              hal_pin_name)) {
        ERR(gbl.init_dbg,
            "[%d] [%s] [%s] hal_pin_u32_newf failed",
            mb_tx->mb_tx_fnct,
            mb_tx->mb_tx_fnct_name,
            hal_pin_name);
        return retERR;
    ...
    **(mb_tx->num_errors) = 0;
    ...
    case mbtx_15_WRITE_MULTIPLE_COILS:
        ...
        if (mb_tx->bit == NULL || mb_tx->bit_inv == NULL) {
            ERR(gbl.init_dbg,
                "[%d] [%s] NULL hal_malloc [%d] elements",
                mb_tx->mb_tx_fnct,
                mb_tx->mb_tx_fnct_name,
                mb_tx->mb_tx_nelem);
            return retERR;
    ...
    case mbtx_16_WRITE_MULTIPLE_REGISTERS:
        mb_tx->float_value =
            hal_malloc(sizeof(hal_float_t *) * mb_tx->mb_tx_nelem);
        ...
        memset(
            mb_tx->float_value, 0, sizeof(hal_float_t *) * mb_tx->mb_tx_nelem);
    ...
    for (pin_counter = 0; pin_counter < mb_tx->mb_tx_nelem; pin_counter++) {
        ...
        case mbtx_15_WRITE_MULTIPLE_COILS:
            if (gbl.version < 1001) {
                if (0 != hal_pin_bit_newf(HAL_IN,
                                          mb_tx->bit + pin_counter,
                                          gbl.hal_mod_id,
                                          "%s",
                                          hal_pin_name)) {
                    ERR(gbl.init_dbg,
                        "[%d] [%s] [%s] hal_pin_bit_newf failed",
                        mb_tx->mb_tx_fnct,
                        mb_tx->mb_tx_fnct_name,
                        hal_pin_name);
                    return retERR;
        ...
        case mbtx_04_READ_INPUT_REGISTERS:
        case mbtx_03_READ_HOLDING_REGISTERS:
            if (0 != hal_pin_float_newf(HAL_OUT,
                                        mb_tx->float_value + pin_counter,
                                        gbl.hal_mod_id,
                                        "%s.float",
                                        hal_pin_name)) {
                ERR(gbl.init_dbg,
                    "[%d] [%s] [%s] hal_pin_float_newf failed",
                    mb_tx->mb_tx_fnct,
                    mb_tx->mb_tx_fnct_name,
                    hal_pin_name);
                return retERR;
            }
            if (0 != hal_pin_s32_newf(HAL_OUT,
                                      mb_tx->int_value + pin_counter,
                                      gbl.hal_mod_id,
                                      "%s.int",
                                      hal_pin_name)) {
                ERR(gbl.init_dbg,
                    "[%d] [%s] [%s] hal_pin_s32_newf failed",
                    mb_tx->mb_tx_fnct,
                    mb_tx->mb_tx_fnct_name,
                    hal_pin_name);
                return retERR;
        ...
        case mbtx_16_WRITE_MULTIPLE_REGISTERS:
            if (gbl.version < 1001) {
                if (0 != hal_pin_float_newf(HAL_IN,
                                            mb_tx->float_value + pin_counter,
                                            gbl.hal_mod_id,
                                            "%s",
                                            hal_pin_name)) {
