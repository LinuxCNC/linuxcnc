...
class HalInputDevice:
    def __init__(self, comp, idx, name, parts="KRAL"):
        ...
        if "A" in parts:
            for axis in self.device.get_bits("EV_ABS"):
                ...
                center = (absinfo.minimum + absinfo.maximum) / 2.0
                halfrange = (absinfo.maximum - absinfo.minimum) / 2.0 or 1
                self.set(name + "-counts", absinfo.value)
                self.set(name + "-position", (absinfo.value - center) / halfrange)
    ...
    def update(self):
        ...
        for a in self.abs_items:
            ...
            self.set(a + "-is-neg", (position < -0.01))
            self.set(a + "-is-pos", (position > 0.01))
        ...
        for k, v in list(self.last.items()):
            ...
            u = (
                self.comp["%s.%s" % (self.idx, k)]
                != self.comp["%s.%s-invert" % (self.idx, k)]
            )
            if u != self.last[k]:
                led = self.ledmap[k]
