    ...
    def __init__(self, comp, idx, name, parts='KRAL'):
        ...
        if 'R' in parts:
            for axis in self.device.get_bits('EV_REL'):
                ...
                comp.newpin("%s.%s-counts" % (idx, name), HAL_S32, Dir.OUT)
        ...
        if 'A' in parts:
            for axis in self.device.get_bits('EV_ABS'):
                ...
                comp.newpin("%s.%s-fuzz" % (idx, name), HAL_S32, Dir.IN)
                comp.newpin("%s.%s-flat" % (idx, name), HAL_S32, Dir.IN)
                comp.newparam("%s.%s-min" % (idx, name), HAL_S32, Dir.RO)
                comp.newparam("%s.%s-max" % (idx, name), HAL_S32, Dir.RO)
