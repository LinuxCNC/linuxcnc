...
class HalInputDevice:
    def __init__(self, comp, idx, name, parts='KRAL'):
        ...
        if 'K' in parts:
            for key in self.device.get_bits('EV_KEY'):
                key = tohalname(key)
                self.codes.add(key)
                comp.newpin("%s.%s" % (idx, key), Type.BOOL, Dir.OUT)
                comp.newpin("%s.%s-not" % (idx, key), Type.BOOL, Dir.OUT)
                self.set(key + "-not", 1)
        ...
        if 'R' in parts:
            for axis in self.device.get_bits('EV_REL'):
                name = tohalname(axis)
                self.codes.add(name)
                comp.newpin("%s.%s-position" % (idx, name), Type.REAL, Dir.OUT)
                comp.newpin("%s.%s-counts" % (idx, name), Type.SINT, Dir.OUT)
                comp.newpin("%s.%s-reset" % (idx, name), Type.BOOL, Dir.IN)
                comp.newpin("%s.%s-scale" % (idx, name), Type.REAL, Dir.IN)
        ...
        if 'A' in parts:
            for axis in self.device.get_bits('EV_ABS'):
                ...
                comp.newpin("%s.%s-position" % (idx, name), Type.REAL, Dir.OUT)
                comp.newpin("%s.%s-counts" % (idx, name), Type.SINT, Dir.OUT)
                comp.newpin("%s.%s-is-pos" % (idx, name), Type.BOOL, Dir.OUT)
                comp.newpin("%s.%s-is-neg" % (idx, name), Type.BOOL, Dir.OUT)
                comp.newpin("%s.%s-scale" % (idx, name), Type.REAL, Dir.IN)
                comp.newpin("%s.%s-offset" % (idx, name), Type.REAL, Dir.IN)
                comp.newpin("%s.%s-fuzz" % (idx, name), Type.SINT, Dir.IN)
                comp.newpin("%s.%s-flat" % (idx, name), Type.SINT, Dir.IN)
                comp.newparam("%s.%s-min" % (idx, name), Type.SINT, Dir.RO)
                comp.newparam("%s.%s-max" % (idx, name), Type.SINT, Dir.RO)
        ...
        if 'L' in parts:
            for led in self.device.get_bits('EV_LED'):
                ...
                comp.newpin("%s.%s" % (idx, name), Type.BOOL, Dir.IN)
                comp.newpin("%s.%s-invert" % (idx, name), Type.BOOL, Dir.IN)
                self.last[name] = 0
    ...
    def update(self):
        ...
        for k, v in list(self.last.items()):
            # Note: this is OK because the hal module always returns True or False for BOOL values
            u = self.comp["%s.%s" % (
