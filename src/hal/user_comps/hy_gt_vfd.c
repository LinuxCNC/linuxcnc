...
void usage(int argc, char **argv)
{
    ...
    printf(
        "\n"
        "This program interfaces the Huanyang GT-series VFD to the LinuxCNC "
        "HAL.\n"
        "\n"
        "Required arguments:\n"
        "    -S, --motor-max-speed SPEED\n"
        "        The motor's max speed in RPM.  This must match the motor "
        "speed\n"
        "        value configured in VFD register P2.03.\n"
        "    -F, --max-frequency F\n"
        "        This is the maximum output frequency of the VFD in Hz.  It "
        "must\n"
        "        correspond to the maximum output frequency configured in VFD\n"
        "        register P0.03.\n"
        "    -f, --min-frequency F\n"
        "        This is the minimum output frequency of the VFD in Hz.  It "
        "must\n"
        ...
        if (r == 1) {
            ...
            case REGISTER_FLOAT:
                *reg->hal_pin_float = data * reg->multiplier;
                break;
            case REGISTER_U32: *reg->hal_pin_u32 = data; break;
            }
            return 0;
        }
        fprintf(stderr,
...
modbus_register_t *add_modbus_register_float(modbus_t *mb,
                                             int address,
                                             const char *pin_name,
                                             float multiplier)
{
    ...
    r = hal_pin_float_newf(
        HAL_OUT, &reg->hal_pin_float, hal_comp_id, "%s.%s", modname, pin_name);
    if (r != 0) {
        return NULL;
    }
...
modbus_register_t *add_modbus_register_u32(modbus_t *mb,
                                           int address,
                                           const char *pin_name,
                                           float multiplier)
{
    ...
    r = hal_pin_u32_newf(
        HAL_OUT, &reg->hal_pin_u32, hal_comp_id, "%s.%s", modname, pin_name);
    if (r != 0) {
        return NULL;
    }
...
int main(int argc, char **argv)
{
    ...
    *haldata->modbus_errors = 0;
    ...
    if (add_modbus_register_float(mb, 0x3014, "external-counter", 1.0) ==
        NULL) {
        goto out_closeHAL;
    }
    ...
    while (done == 0) {
        ...
        if (*haldata->spindle_on) {
            ...
            *haldata->freq_cmd =
            ...
            if ((fabs(*haldata->speed_cmd - *speed_fb_reg->hal_pin_float) /
