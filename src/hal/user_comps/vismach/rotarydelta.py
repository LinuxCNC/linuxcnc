...
class DeltaTranslate(Collection):
    ...
    def apply(self):
        glPushMatrix()
        rotarydeltakins.set_geometry(
            self.comp["pfr"], self.comp["tl"], self.comp["sl"], self.comp["fr"]
        )
        f = rotarydeltakins.forward(
            self.comp["joint0"], self.comp["joint1"], self.comp["joint2"]
        )
        if f is not None:
            self.x = x = f[0]
            self.y = y = f[1]
    ...
    return Rotate(
        [
            HalTranslate(
                [
                    CylinderY(-1, 1, 6, 1),
                    HalRotate([CylinderX(c, 0, 0.5, "tl", 0.5)], c, joint, 1, 0, 1, 0),
                ],
                c,
                "pfr",
                1,
...
class Strut:
    ...
    def cylinder(self, L):
        gluCylinder(self.q, 0.5, 0.5, L, 32, 1)
        # bottom cap
        glRotatef(180, 1, 0, 0)
        gluDisk(self.q, 0, 0.5, 32, 1)
        glRotatef(180, 1, 0, 0)
            ...
            [
                Color(
                    ...
                    [
                        ...
                        HexPrismZ(c, 0, 0.5, "fr"),
                        CylinderZ(-2, 0, -1.5, 0.25),
                        CylinderZ(-1.5, 0.25, 1, 0.25),
                    ],
                )
            ],
            0,
