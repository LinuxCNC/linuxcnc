...
link2 = Collection([link3, CylinderZ(-j2_hi, j2_rad, j2_hi, j2_rad)])
# move to end of arm
link2 = Translate([link2], d4, 0.0, 0.0)
# add the arm itself
link2 = Collection([link2, CylinderX(d4, l2_rad, 1.5 * j1_rad, l2_rad)])
...
link1 = Collection(
    [
        ...
        Box(
            -1.5 * j1_rad, -0.9 * j1_rad, -j1_hi1, -1.15 * j1_rad, 0.9 * j1_rad, j1_hi1
        ),
...
# slap the arm on top
link0 = Collection([link0, Translate([link1], 0, 0, d1)])
...
work = Capture()
table = Collection(
    [work, Box(-0.35 * reach, -0.5 * reach, -0.1 * d1, 0.35 * reach, 0.5 * reach, 0.0)]
)
