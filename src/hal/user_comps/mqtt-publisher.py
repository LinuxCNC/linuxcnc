...
class LinuxCNC2MQTT:
    ...
    def mqtt_start(self, mqtt_host, mqtt_port, mqtt_username, mqtt_password):
        try:
            ...
            print(
                "error: On Debian, run 'sudo apt install python3-paho-mqtt' to install it."
            )
            print()
            raise
        mqttc = mqtt.Client()
        mqttc.on_connect = lambda client, userdata, flags, rc: print(
            "info: MQTT connected: " + mqtt.connack_string(rc)
        )
        mqttc.on_disconnect = lambda client, userdata, rc: print(
            "warning: MQTT disconnected: " + mqtt.connack_string(rc)
        )
    ...
    def update_mqtt(self):
        ...
        while hal.component_exists("halui") and running:
            ...
            if self.hal["enable"]:
                ...
                if not self.dryrun:
                    print(
                        f"info: Publishing MQTT message ({self.mqtt_prefix}):",
                        json.dumps(data),
                    )
                    self.mqttc.publish(self.mqtt_prefix, json.dumps(data))
                else:
                    print(
                        "info: Not publishing MQTT message ({self.mqtt_prefix}):",
                        json.dumps(data),
                    )
    ...
    def main():
        ...
        options = [
            ...
            Option(
                ...
                metavar="FQDN",
                default="localhost",
                help='The FQDN of the MQTT broker.  Default is "localhost".',
            ),
            Option(
                "--mqtt-port",
                dest="mqttport",
                ...
                type=int,
                default=1883,
                help="The port to use of the MQTT broker. Default is 1883.",
            ),
            Option(
                "--mqtt-user",
                dest="mqttuser",
