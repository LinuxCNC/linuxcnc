...
void HandWheelCounters::count(int8_t delta)
{
    ...
    int32_t &counter = mCounters[static_cast<
        typename std::underlying_type<CounterNameToIndex>::type>(idx)];
...
int32_t HandWheelCounters::counts() const
{
    if (mIsLeadCounterActive) {
        auto idx = static_cast<
            typename std::underlying_type<CounterNameToIndex>::type>(
            CounterNameToIndex::LEAD);
...
std::ostream &operator<<(std::ostream &os, const HandWheelCounters &data)
{
    ...
    if (data.isLeadCounterActive()) {
        os << "{counts=" << data.counts() << " activeCounter=LEAD"
           << " isLeadActive=" << data.isLeadCounterActive() << "}";
    } else if (HandWheelCounters::CounterNameToIndex::UNDEFINED ==
               data.activeCounter()) {
        os << "{counts=NA"
        ...
        os << "{counts=" << data.counts()
           << " activeCounter=" << static_cast<int16_t>(data.activeCounter())
           << " isLeadActive=" << data.isLeadCounterActive() << "}";
    }
