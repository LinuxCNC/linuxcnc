...
void usbInputResponseCallback(struct libusb_transfer *transfer)
{
    ...
    UsbRawInputListener *receiver =
        reinterpret_cast<UsbRawInputListener *>(transfer->user_data);
...
{
    assert(sizeof(UsbOutPackageBlockBuffer) ==
           sizeof(UsbOutPackageBlockFields));
}
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
ConstantUsbPackages::ConstantUsbPackages() : sleepPackage(), emptyPackage() {}
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
Usb::Usb(const char *name,
         OnUsbInputPackageListener &onDataReceivedCallback,
         Hal &hal)
    : sleepState(), inputPackageBuffer(), outputPackageBuffer(),
      mDataHandler(onDataReceivedCallback),
      mRawDataCallback(usbInputResponseCallback), mHal(hal),
      inTransfer(libusb_alloc_transfer(0)),
      outTransfer(libusb_alloc_transfer(0)), verboseTxOut(&devNull),
      verboseRxOut(&devNull), verboseInitOut(&devNull), mName(name)
{
...
void Usb::sendDisplayData()
{
    ...
    if (mIsSimulationMode) {
        *verboseTxOut << "out   0x" << outputPackageBuffer.asBlocks << endl
                      << std::dec << "out   size "
                      << sizeof(outputPackageBuffer.asBlockArray) << "B "
                      << outputPackageData << endl;
    }
...
void UsbOutPackageAxisCoordinate::setCoordinate(const float &coordinate)
{
    ...
    uint32_t scaledCoordinate =
        static_cast<uint32_t>(rint(coordinateAbs * 10000.0));
...
UsbOutPackageBlockFields::UsbOutPackageBlockFields()
    : reportId(0x06), __padding0(0), __padding1(0), __padding2(0),
      __padding3(0), __padding4(0), __padding5(0), __padding6(0)
{
}
// ----------------------------------------------------------------------
                         ...
                         const UsbOutPackageBlockFields &block)
{
    ...
    os << std::hex << std::setfill('0') << std::setw(2)
       << static_cast<unsigned short>(block.reportId) << std::setw(2)
       << static_cast<unsigned short>(block.__padding0) << std::setw(2)
       << static_cast<unsigned short>(block.__padding1) << std::setw(2)
       << static_cast<unsigned short>(block.__padding2) << std::setw(2)
       << static_cast<unsigned short>(block.__padding3) << std::setw(2)
       << static_cast<unsigned short>(block.__padding4) << std::setw(2)
       << static_cast<unsigned short>(block.__padding5) << std::setw(2)
...
// ----------------------------------------------------------------------
UsbOutPackageBlocks::UsbOutPackageBlocks() : block0(), block1(), block2() {}
// ----------------------------------------------------------------------
...
std::ostream &operator<<(std::ostream &os, const UsbOutPackageData &data)
{
    ...
    if (enableMultiLine) {
        os << std::hex << std::setfill('0') << "header       0x" << std::setw(2)
           << data.header << endl
           << "day of month   0x" << std::setw(2)
           ...
           << std::dec << "coordinate1  " << data.row1Coordinate << endl
           << "coordinate2  " << data.row2Coordinate << endl
           << "coordinate3  " << data.row3Coordinate << endl
           << "feed rate        " << data.feedRate << endl
           << "spindle feed rate      " << data.spindleFeedRate;
    } else {
        os << std::hex << std::setfill('0') << "hdr 0x" << std::setw(4)
           << data.header << " dom 0x" << std::setw(2)
           << static_cast<unsigned short>(data.seed) << " status 0x"
           << std::setw(2)
           << static_cast<unsigned short>(data.displayModeFlags.asByte)
           << std::dec << " coord1 " << data.row1Coordinate << " coord2 "
           << data.row2Coordinate << " coord3 " << data.row3Coordinate
           << " feed " << std::setw(4) << data.feedRate << " spindle feed rate "
           << std::setw(5) << data.spindleFeedRate;
    }
    os.copyfmt(init);
    return os;
}
// ----------------------------------------------------------------------
UsbOutPackageBuffer::UsbOutPackageBuffer() : asBlocks()
{
    if (false) {
        std::cout << "sizeof usb data " << sizeof(UsbOutPackageData) << endl
                  << " blocks count   "
                  << sizeof(UsbOutPackageBlocks) /
                         sizeof(UsbOutPackageBlockFields)
                  << endl
    ...
    size_t blocksCount =
        sizeof(UsbOutPackageBlocks) / sizeof(UsbOutPackageBlockFields);
    assert((sizeof(UsbOutPackageData) + blocksCount) ==
           sizeof(UsbOutPackageBlocks));
}
// ----------------------------------------------------------------------
UsbInPackage::UsbInPackage()
    : header(0), randomByte(0), buttonKeyCode1(0), buttonKeyCode2(0),
      rotaryButtonFeedKeyCode(0), rotaryButtonAxisKeyCode(0), stepCount(0),
      crc(0)
{
}
// ----------------------------------------------------------------------
UsbInPackage::UsbInPackage(const uint8_t notAvailable1,
                           ...
                           const uint8_t crc)
    : header(notAvailable1), randomByte(notAvailable2),
      buttonKeyCode1(buttonKeyCode1), buttonKeyCode2(buttonKeyCode2),
      rotaryButtonFeedKeyCode(rotaryButtonFeedKeyCode),
      rotaryButtonAxisKeyCode(rotaryButtonAxisKeyCode), stepCount(stepCount),
      crc(crc)
{
}
// ----------------------------------------------------------------------
...
void Usb::onUsbDataReceived(struct libusb_transfer *transfer)
{
    ...
    case (LIBUSB_TRANSFER_COMPLETED):
        ...
        if (sleepState.mDropNextInPackage) {
            if (Usb::ConstantPackages.sleepPackage !=
                inputPackageBuffer.asFields) {
                *verboseTxOut << "expected sleep package starting with "
                              << std::hex << std::setfill('0') << std::setw(2)
                              << static_cast<unsigned short>(
                                     Usb::ConstantPackages.sleepPackage.header)
                              << " but got " << std::hex << std::setfill('0')
                              << std::setw(2)
                              << static_cast<unsigned short>(
                                     inputPackageBuffer.asFields.header)
                              << endl;
        ...
        if (transfer->actual_length == expectedPackageSize) {
            ...
            if (Usb::ConstantPackages.emptyPackage ==
                inputPackageBuffer.asFields) {
                ...
                if (mIsSimulationMode) {
                    ...
                    *verboseTxOut
                        << "event going to sleep: device was idle for "
                        << (now.tv_sec - sleepState.mLastWakeupTimestamp.tv_sec)
                        << " seconds" << endl;
                }
            }
            // on regular package
                ...
                if (mHal.getIsPendantSleeping()) {
                    ...
                    if (mIsSimulationMode) {
                        ...
                        gettimeofday(&now, nullptr);
                        *verboseTxOut
                            << "woke up: device was sleeping for "
                            << (now.tv_sec -
                                sleepState.mLastWakeupTimestamp.tv_sec)
                            << " seconds" << endl;
                    }
                    gettimeofday(&sleepState.mLastWakeupTimestamp, nullptr);
                }
            ...
            std::cerr << "received unexpected package size: expected="
                      << (transfer->actual_length)
                      << ", current=" << expectedPackageSize << endl;
        }
...
bool Usb::init()
{
    if (getDoReconnect()) {
        int pauseSecs = 3;
        *verboseInitOut << "init  pausing " << pauseSecs
                        << "s, waiting for device to be gone ...";
        while ((pauseSecs--) >= 0) {
    ...
    */
    ...
    if (isWaitWithTimeout) {
        *verboseInitOut << "init  waiting maximum "
                        << static_cast<unsigned short>(mWaitSecs)
                        << "s for device " << mName << " vendorId=0x"
                        << std::hex << std::setfill('0') << std::setw(2)
                        << usbVendorId << " productId=0x" << std::setw(2)
                        << usbProductId << " ...";
    } else {
        *verboseInitOut << "init  not waiting for device " << mName
                        << " vendorId=0x" << std::hex << std::setfill('0')
                        << std::setw(2) << usbVendorId << " productId=0x"
                        << std::setw(2) << usbProductId << std::dec
                        << ", will continue in "
                        << static_cast<unsigned short>(mWaitSecs) << "s ...";
    }
    ...
    do {
        ...
        ssize_t devicesCount =
            libusb_get_device_list(context, &devicesReference);
        if (devicesCount < 0) {
        ...
        deviceHandle =
            libusb_open_device_with_vid_pid(context, usbVendorId, usbProductId);
...
// ----------------------------------------------------------------------
SleepDetect::SleepDetect() : mDropNextInPackage(false), mLastWakeupTimestamp()
{
}
// ----------------------------------------------------------------------
