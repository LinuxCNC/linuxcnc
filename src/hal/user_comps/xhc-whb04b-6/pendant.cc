...
HandwheelStepModeStepSize::~HandwheelStepModeStepSize() {}
// ----------------------------------------------------------------------
float HandwheelStepModeStepSize::getStepSize(
    PositionNameIndex buttonPosition) const
{
    return mSequence[static_cast<uint8_t>(buttonPosition)];
}
// ----------------------------------------------------------------------
bool HandwheelStepModeStepSize::isPermitted(
    PositionNameIndex buttonPosition) const
{
...
HandwheelMpgModeStepSize::~HandwheelMpgModeStepSize() {}
// ----------------------------------------------------------------------
float HandwheelMpgModeStepSize::getStepSize(
    PositionNameIndex buttonPosition) const
{
    return mSequence[static_cast<uint8_t>(buttonPosition)];
}
// ----------------------------------------------------------------------
bool HandwheelMpgModeStepSize::isPermitted(
    PositionNameIndex buttonPosition) const
{
    return (getStepSize(buttonPosition) > 0);
}
...
HandwheelConModeStepSize::~HandwheelConModeStepSize() {}
// ----------------------------------------------------------------------
float HandwheelConModeStepSize::getStepSize(
    PositionNameIndex buttonPosition) const
{
    return mSequence[static_cast<uint8_t>(buttonPosition)];
}
// ----------------------------------------------------------------------
bool HandwheelConModeStepSize::isPermitted(
    PositionNameIndex buttonPosition) const
{
    return (getStepSize(buttonPosition) > 0);
}
...
HandwheelLeadModeStepSize::~HandwheelLeadModeStepSize() {}
// ----------------------------------------------------------------------
float HandwheelLeadModeStepSize::getStepSize(
    PositionNameIndex buttonPosition) const
{
    return mSequence[static_cast<uint8_t>(buttonPosition)];
}
// ----------------------------------------------------------------------
bool HandwheelLeadModeStepSize::isPermitted(
    PositionNameIndex buttonPosition) const
{
...
KeyCode::KeyCode(uint8_t code, const char *text, const char *altText)
    : code(code), text(text), altText(altText)
{
}
// ----------------------------------------------------------------------
KeyCode::KeyCode(const KeyCode &other)
    : code(other.code), text(other.text), altText(other.altText)
{
}
// ----------------------------------------------------------------------
...
std::ostream &operator<<(std::ostream &os, const KeyCode &data)
{
    ...
    os << std::hex << std::setfill('0') << "{code=0x" << std::setw(2)
       << static_cast<uint16_t>(data.code) << " text='" << data.text << "'"
...
// ----------------------------------------------------------------------
bool MetaButtonCodes::containsKeys(const KeyCode &key,
                                   const KeyCode &modifier) const
{
    return (this->key.code == key.code) &&
           (this->modifier.code == modifier.code);
}
// ----------------------------------------------------------------------
...
MetaButtonsCodes::MetaButtonsCodes(const ButtonsCode &buttons)
    : reset(buttons.reset, buttons.undefined),
      ...
      buttons{&reset,          &macro11, &stop,           &macro12,
              &start,          &macro13, &feed_plus,      &macro1,
              &feed_minus,     &macro2,  &spindle_plus,   &macro3,
              &spindle_minus,  &macro4,  &machine_home,   &macro5,
              &safe_z,         &macro6,  &workpiece_home, &macro7,
              &spindle_on_off, &macro8,  &function,       &probe_z,
              &macro9,         &macro10, &macro14,        &continuous,
              &macro15,        &step,    &macro16,        &undefined}
{
}
// ----------------------------------------------------------------------
const MetaButtonCodes &MetaButtonsCodes::find(const KeyCode &keyCode,
                                              const KeyCode &modifierCode) const
{

    std::function<bool(const MetaButtonCodes *)> comparator =
        [&keyCode, &modifierCode](const MetaButtonCodes *metaButton) {
            return metaButton->containsKeys(keyCode, modifierCode);
        };

    std::list<const MetaButtonCodes *>::const_iterator button =
        std::find_if(buttons.begin(), buttons.end(), comparator);

    if (button == buttons.end()) {
        std::cerr << "failed to find metaButton={ keyCode={" << keyCode
                  << "} modifierCode={" << modifierCode << "}}" << endl;
    }
...
// ----------------------------------------------------------------------
Button::Button(const KeyCode &key) : mKey(&key) {}
// ----------------------------------------------------------------------
Button::~Button() {}
// ----------------------------------------------------------------------
...
AxisRotaryButtonCodes::AxisRotaryButtonCodes()
    : off(0x06, "OFF", ""), x(0x11, "X", ""), y(0x12, "Y", ""),
      z(0x13, "Z", ""), a(0x14, "A", ""), b(0x15, "B", ""), c(0x16, "C", ""),
      undefined(0x00, "", ""), codeMap{
                                   {off.code,       &off      },
                                   {x.code,         &x        },
                                   {y.code,         &y        },
                                   {z.code,         &z        },
...
FeedRotaryButtonCodes::FeedRotaryButtonCodes()
    : percent_2(0x0d, "0.001", "2%"), percent_5(0x0e, "0.01", "5%"),
      percent_10(0x0f, "0.1", "10%"), percent_30(0x10, "1", "30%"),
      percent_60(0x1a, "5", "60%"), percent_100(0x1b, "10", "100%"),
      lead(
          0x1c,
          "Lead",
          ""), // user jasenk2 seem to need 0x9b for xhc-whb06-4 see : https://github.com/LinuxCNC/linuxcnc/pull/987, zajc3w needs 0x9B for xhc-whb06-6 see: https://github.com/LinuxCNC/linuxcnc/issues/3485
      lead9B(
          0x9B,
          "Lead",
          ""), //previous solution ommited line 607 and key code 0x9b was still not recognised
      undefined(0x00, "", ""), codeMap{
...
ButtonsCode::ButtonsCode()
    : reset(0x01, "reset", "macro-11"), stop(0x02, "stop", "macro-12"),
      ...
      function(0x0c, "fn", "<unused>"), probe_z(0x0d, "probe-z", "macro-9"),
      macro10(0x10, "macro-10", "macro-14"),
      continuous(0x0e, "mode-continuous", "macro-15"),
      step(0x0f, "mode-step", "macro-16"), undefined(0x00, "", ""),
      codeMap{
          {reset.code,          &reset         },
          {stop.code,           &stop          },
          {start.code,          &start         },
...
// ----------------------------------------------------------------------
bool ToggleButton::containsKeys(const KeyCode &key,
                                const KeyCode &modifier) const
{
...
const std::map<const KeyCode *, HandwheelStepModeStepSize::PositionNameIndex>
    FeedRotaryButton::mStepKeycodeLut{
        {&KeyCodes::Feed.percent_2,
         HandwheelStepModeStepSize::PositionNameIndex::RotaryButton0001     },
        {&KeyCodes::Feed.percent_5,
         HandwheelStepModeStepSize::PositionNameIndex::RotaryButton0010     },
        {&KeyCodes::Feed.percent_10,
         HandwheelStepModeStepSize::PositionNameIndex::RotaryButton0100     },
        {&KeyCodes::Feed.percent_30,
         HandwheelStepModeStepSize::PositionNameIndex::RotaryButton100      },
        {&KeyCodes::Feed.percent_60,
         HandwheelStepModeStepSize::PositionNameIndex::RotaryButton500      },
        {&KeyCodes::Feed.percent_100,
         HandwheelStepModeStepSize::PositionNameIndex::RotaryButton1000     },
        {&KeyCodes::Feed.lead,
         HandwheelStepModeStepSize::PositionNameIndex::RotaryButtonUndefined}
};
const std::map<const KeyCode *, HandwheelMpgModeStepSize::PositionNameIndex>
    FeedRotaryButton::mMpgKeycodeLut{
        {&KeyCodes::Feed.percent_2,
         HandwheelMpgModeStepSize::PositionNameIndex::RotaryButton2percent  },
        {&KeyCodes::Feed.percent_5,
         HandwheelMpgModeStepSize::PositionNameIndex::RotaryButton5percent  },
        {&KeyCodes::Feed.percent_10,
         HandwheelMpgModeStepSize::PositionNameIndex::RotaryButton10percent },
        {&KeyCodes::Feed.percent_30,
         HandwheelMpgModeStepSize::PositionNameIndex::RotaryButton30percent },
        {&KeyCodes::Feed.percent_60,
         HandwheelMpgModeStepSize::PositionNameIndex::RotaryButton60percent },
        {&KeyCodes::Feed.percent_100,
         HandwheelMpgModeStepSize::PositionNameIndex::RotaryButton100percent},
        {&KeyCodes::Feed.lead,
         HandwheelMpgModeStepSize::PositionNameIndex::RotaryButtonUndefined }
};
const std::map<const KeyCode *, HandwheelConModeStepSize::PositionNameIndex>
    FeedRotaryButton::mConKeycodeLut{
        {&KeyCodes::Feed.percent_2,
         HandwheelConModeStepSize::PositionNameIndex::RotaryButton2percent  },
        {&KeyCodes::Feed.percent_5,
         HandwheelConModeStepSize::PositionNameIndex::RotaryButton5percent  },
        {&KeyCodes::Feed.percent_10,
         HandwheelConModeStepSize::PositionNameIndex::RotaryButton10percent },
        {&KeyCodes::Feed.percent_30,
         HandwheelConModeStepSize::PositionNameIndex::RotaryButton30percent },
        {&KeyCodes::Feed.percent_60,
         HandwheelConModeStepSize::PositionNameIndex::RotaryButton60percent },
        {&KeyCodes::Feed.percent_100,
         HandwheelConModeStepSize::PositionNameIndex::RotaryButton100percent},
        {&KeyCodes::Feed.lead,
         HandwheelConModeStepSize::PositionNameIndex::RotaryButtonUndefined }
};
const std::map<const KeyCode *, HandwheelLeadModeStepSize::PositionNameIndex>
    FeedRotaryButton::mLeadKeycodeLut{
        {&KeyCodes::Feed.percent_2,
         HandwheelLeadModeStepSize::PositionNameIndex::UNDEFINED},
        {&KeyCodes::Feed.percent_5,
         HandwheelLeadModeStepSize::PositionNameIndex::UNDEFINED},
        {&KeyCodes::Feed.percent_10,
         HandwheelLeadModeStepSize::PositionNameIndex::UNDEFINED},
        ...
        {&KeyCodes::Feed.percent_100,
         HandwheelLeadModeStepSize::PositionNameIndex::UNDEFINED},
        {&KeyCodes::Feed.lead,
         HandwheelLeadModeStepSize::PositionNameIndex::LEAD     }
};
// ----------------------------------------------------------------------
FeedRotaryButton::FeedRotaryButton(const KeyCode &keyCode,
                                   HandwheelStepmodes::Mode stepMode,
                                   KeyEventListener *listener)
    : RotaryButton(keyCode), mStepMode(stepMode), mIsPermitted(false),
      mStepSize(0), mIsStepMode_5_10(false), mEventListener(listener)
{
    (void)mEventListener;
}
// ----------------------------------------------------------------------
...
void FeedRotaryButton::update()
{
    ...
    if (*mKey == KeyCodes::Feed.lead || *mKey == KeyCodes::Feed.lead9B) {
        mStepSize = mLeadSizeMapper.getStepSize(
            HandwheelLeadModeStepSize::PositionNameIndex::LEAD);
        mIsPermitted = mLeadSizeMapper.isPermitted(
            HandwheelLeadModeStepSize::PositionNameIndex::LEAD);
        ...
        if (mIsStepMode_5_10 && mStepSize > 2) {
            mStepSize = 0;
        } // TODO DOES NOT WORK bool variable seems to be not synched inside pendant.h
...
AxisRotaryButton::AxisRotaryButton(const KeyCode &keyCode,
                                   KeyEventListener *listener)
    : RotaryButton(keyCode), mEventListener(listener)
{
    (void)mEventListener;
}
// ----------------------------------------------------------------------
...
Handwheel::Handwheel(const FeedRotaryButton &feedButton,
                     KeyEventListener *listener)
    : mCounters(), mFeedButton(feedButton), mEventListener(listener),
      mWheelCout(&std::cout), mPrefix("pndnt ")
{
    (void)mFeedButton;
}
// ----------------------------------------------------------------------
...
void Handwheel::count(int8_t delta)
{
    ...
    *mWheelCout << mPrefix << "wheel total counts " << std::setfill(' ')
                << std::setw(5) << mCounters << endl;
    mWheelCout->copyfmt(init);
}
// ----------------------------------------------------------------------
ButtonsState::ButtonsState(KeyEventListener *listener,
                           const ButtonsState *previousState)
    : mPressedButtons(), mCurrentMetaButton(&KeyCodes::Meta.undefined),
      mAxisButton(KeyCodes::Axis.undefined, listener),
      mFeedButton(
          KeyCodes::Feed.undefined, HandwheelStepmodes::Mode::MPG, listener),
      mPreviousState(previousState), mEventListener(listener)
{
    (void)mPreviousState;
}
// ----------------------------------------------------------------------
                          ...
                          const KeyCode &feedButton)
{
    ...
    const MetaButtonCodes &newButton =
        KeyCodes::Meta.find(keyCode, modifierCode);
...
Pendant::Pendant(Hal &hal, UsbOutPackageData &displayOutData)
    : mHal(hal), mPreviousButtonsState(this),
      ...
      mDisplay(mCurrentButtonsState, hal, displayOutData), mPrefix("pndnt "),
                           ...
                           int8_t handWheelStepCount)
{
    ...
    if (key == KeyCodes::Buttons.codeMap.end()) {
        *mPendantCout << mPrefix << "failed to interpret key code keyCode={"
                      << keyCode << "}" << endl;
        modifier = KeyCodes::Buttons.codeMap.find(0);
    }
    if (modifier == KeyCodes::Buttons.codeMap.end()) {
        *mPendantCout << mPrefix
                      << "failed to interpret modifier code keyCode={"
                      << modifierCode << "}" << endl;
        modifier = KeyCodes::Buttons.codeMap.find(0);
    }
    if (axis == KeyCodes::Axis.codeMap.end()) {
        *mPendantCout << mPrefix << "failed to interpret axis code axisCode={"
                      << rotaryButtonAxisKeyCode << "}" << endl;
        axis = KeyCodes::Axis.codeMap.find(0);
    }
    if (feed == KeyCodes::Feed.codeMap.end()) {
        *mPendantCout << mPrefix
                      << "failed to interpret axis code feed axisCode={"
                      << rotaryButtonFeedKeyCode << "}" << endl;
        feed = KeyCodes::Feed.codeMap.find(0);
    }

    processEvent(*key->second,
                 *modifier->second,
                 *axis->second,
                 *feed->second,
                 handWheelStepCount);
                           ...
                           int8_t handWheelStepCount)
{
    ...
    mCurrentButtonsState.update(keyCode,
                                modifierCode,
...
{
    return const_cast<Handwheel &>(
        const_cast<const Pendant *>(this)->handWheel());
}
// ----------------------------------------------------------------------
bool Pendant::onButtonPressedEvent(const MetaButtonCodes &metaButton)
{
    *mPendantCout << mPrefix
                  << "button pressed  event metaButton=" << metaButton << endl;
    ...
    if (metaButton == KeyCodes::Meta.reset) {
        ...
        mCurrentButtonsState.feedButton().setStepMode(
            HandwheelStepmodes::Mode::CON);
        mHal.setConMode(true);
        dispatchFeedValueToHal();
        isHandled = true;
    } else if (metaButton == KeyCodes::Meta.step) {
        mCurrentButtonsState.feedButton().setStepMode(
            HandwheelStepmodes::Mode::STEP);
...
{
    *mPendantCout << mPrefix
                  << "button released event metaButton=" << metaButton << endl;
    bool isHandled = false;
    if (metaButton == KeyCodes::Meta.reset) {
        mHal.setReset(false);
        isHandled = true;
...
void Pendant::dispatchFeedEventToHandwheel(const KeyCode &feed, bool isActive)
{
    if (feed.code == KeyCodes::Feed.lead.code ||
        feed.code ==
            KeyCodes::Feed.lead9B
                .code) // user jasenk2 seem to need 0x9b for xhc-whb06-4 see : https://github.com/LinuxCNC/linuxcnc/pull/987
    {
...
void Pendant::dispatchActiveFeedToHal(const KeyCode &feed, bool isActive)
{
    if (feed.code == KeyCodes::Feed.percent_2.code) {
        ...
        feed.code == KeyCodes::Feed.lead.code ||
        feed.code ==
            KeyCodes::Feed.lead9B
                .code) // user jasenk2 seem to need 0x9b for xhc-whb06-4 see : https://github.com/LinuxCNC/linuxcnc/pull/987
    {
...
void Pendant::dispatchFeedValueToHal()
{
    ...
    if (feedButton.isPermitted()) {
        ...
        if (feedButton.stepMode() == HandwheelStepmodes::Mode::STEP) {
            ...
            mHal.setConMode(false);
            mHal.setFeedOverrideScale(
                0); // NO SEND MOVE IN MPG MODE only used for Feed override
            ...
            mHal.setFeedOverrideScale(0);
            axisJogStepSize =
                feedButton.stepSize() * 0.0001 * mHal.getFeedOverrideMaxVel();
        } else {
        }
...
bool Pendant::onJogDialEvent(const HandWheelCounters &counters, int8_t delta)
{
    ...
    if (HandWheelCounters::CounterNameToIndex::UNDEFINED !=
        counters.activeCounter()) // && 0 != counters.counts())
    {
        *mPendantCout << mPrefix << "wheel  event " << counters.counts()
        ...
        if (0 != delta) {
            if (counters.isLeadCounterActive() &&
                mIsLeadModeSpindle) { // Spindle override mode
                if (delta > 0) {
                    ...
                    mHal.toggleSpindleOverrideDecrease();
                       ...
                       feedButton.stepMode() ==
                           HandwheelStepmodes::Mode::
                               MPG) { // FeedRate override mode
                if (delta > 0) {
                    mHal.toggleFeedrateIncrease();
                } else {
                    mHal.toggleFeedrateDecrease();
                }
            } else if (!counters.isLeadCounterActive() &&
                       (feedButton.stepMode() ==
                            HandwheelStepmodes::Mode::CON ||
                        feedButton.stepMode() ==
                            HandwheelStepmodes::Mode::STEP)) { // Normal Mode
...
{
    if (!isActive) {
        mHandWheel.counters().setActiveCounter(
        ...
        mHandWheel.counters().setActiveCounter(
            HandWheelCounters::CounterNameToIndex::AXIS_X);
    } else if (axis.code == KeyCodes::Axis.y.code) {
        mHandWheel.counters().setActiveCounter(
            HandWheelCounters::CounterNameToIndex::AXIS_Y);
    } else if (axis.code == KeyCodes::Axis.z.code) {
        mHandWheel.counters().setActiveCounter(
            HandWheelCounters::CounterNameToIndex::AXIS_Z);
    } else if (axis.code == KeyCodes::Axis.a.code) {
        mHandWheel.counters().setActiveCounter(
            HandWheelCounters::CounterNameToIndex::AXIS_A);
    } else if (axis.code == KeyCodes::Axis.b.code) {
        mHandWheel.counters().setActiveCounter(
            HandWheelCounters::CounterNameToIndex::AXIS_B);
    } else if (axis.code == KeyCodes::Axis.c.code) {
        mHandWheel.counters().setActiveCounter(
            HandWheelCounters::CounterNameToIndex::AXIS_C);
...
// ----------------------------------------------------------------------
Display::Display(const ButtonsState &currentButtonsState,
                 Hal &hal,
                 UsbOutPackageData &displayData)
    : mCurrentButtonsState(currentButtonsState), mHal(hal),
    ...
    if (metaButton == KeyCodes::Meta.continuous) {
        mDisplayData.displayModeFlags.asBitFields.stepMode =
            static_cast<typename std::underlying_type<
                DisplayIndicatorStepMode::StepMode>::type>(
                DisplayIndicatorStepMode::StepMode::CON);
        return true;
    } else if (metaButton == KeyCodes::Meta.step) {
        mDisplayData.displayModeFlags.asBitFields.stepMode =
            static_cast<typename std::underlying_type<
...
{
    if (mCurrentButtonsState.feedButton().stepMode() ==
        HandwheelStepmodes::Mode::STEP) {
        mDisplayData.displayModeFlags.asBitFields.stepMode =
            static_cast<typename std::underlying_type<
                DisplayIndicatorStepMode::StepMode>::type>(
                DisplayIndicatorStepMode::StepMode::STEP);
    } else if (mCurrentButtonsState.feedButton().stepMode() ==
               HandwheelStepmodes::Mode::MPG) {
        mDisplayData.displayModeFlags.asBitFields.stepMode =
            static_cast<typename std::underlying_type<
                DisplayIndicatorStepMode::StepMode>::type>(
...
// ----------------------------------------------------------------------
bool Display::onJogDialEvent(const HandWheelCounters & /*counters*/,
                             int8_t /*delta*/)
{
    return false;
}
...
void Display::updateData()
{
    ...
    uint32_t spindleSpeedIncrease =
        static_cast<uint32_t>(mHal.getspindleSpeedChangeIncrease());
    uint32_t spindleSpeedDecrease =
        static_cast<uint32_t>(mHal.getspindleSpeedChangeDecrease());
    uint32_t spindleFeedRate =
        static_cast<uint32_t>(mHal.getSpindleOverrideValue() * 100);
    uint32_t spindleSpeedCmd = static_cast<uint32_t>(mHal.getspindleSpeedCmd());
    uint32_t feedRate =
        static_cast<uint32_t>(mHal.getFeedOverrideValue() * 100);
    ...
    bool isAbsolutePositionRequest =
        (mAxisPositionMethod == AxisPositionMethod::ABSOLUTE);
    mDisplayData.displayModeFlags.asBitFields.isRelativeCoordinate =
        !isAbsolutePositionRequest;
    if (mActiveAxisGroup == AxisGroup::XYZ) {
        mDisplayData.row1Coordinate.setCoordinate(static_cast<float>(
            mHal.getAxisXPosition(isAbsolutePositionRequest)));
        mDisplayData.row2Coordinate.setCoordinate(static_cast<float>(
            mHal.getAxisYPosition(isAbsolutePositionRequest)));
        mDisplayData.row3Coordinate.setCoordinate(static_cast<float>(
            mHal.getAxisZPosition(isAbsolutePositionRequest)));
    } else {
        mDisplayData.row1Coordinate.setCoordinate(static_cast<float>(
            mHal.getAxisAPosition(isAbsolutePositionRequest)));
        mDisplayData.row2Coordinate.setCoordinate(static_cast<float>(
            mHal.getAxisBPosition(isAbsolutePositionRequest)));
        mDisplayData.row3Coordinate.setCoordinate(static_cast<float>(
            mHal.getAxisCPosition(isAbsolutePositionRequest)));
    }
}
