...
class mitsubishi_serial:
    ...
    def __init__(
        self, vfd_names=[["mitsub_vfd", "00"]], baudrate=9600, port="/dev/ttyUSB0"
    ):
        try:
                ...
                for index, ids in enumerate(self.comp_names):
                    ...
                    if self.h[index]["monitor"]:
                        ...
                        if chr_list != "":
                            ...
                            self.h[index]["motor-fb"] = (
                                decimal * 0.01 * self.h[index]["scale-fb"]
                            )
                            if self.h[index]["debug"] and 1 == 2:
                                print(
                                    "monitor frequency:",
                        ...
                        if chr_list != "":
                            decimal = int(string[3:7], 16)
                            self.h[index]["motor-amps"] = (
                                decimal * 0.01 * self.h[index]["scale-amps"]
                            )
                            if self.h[index]["debug"] and 1 == 2:
                                print(
                                    "monitor amps:",
                        ...
                        if chr_list != "":
                            decimal = int(string[3:7], 16)
                            self.h[index]["motor-volts"] = (
                                decimal * 0.01 * self.h[index]["scale-volts"]
                            )
                            if self.h[index]["debug"] and 1 == 2:
                                print(
                                    "monitor volts:",
                        ...
                        if chr_list != "":
                            decimal = int(string[3:7], 16)
                            self.h[index]["motor-user"] = (
                                decimal * self.h[index]["scale-user"]
                            )
                            if self.h[index]["debug"] and 1 == 2:
                                print(
                                    "monitor user selectable:",
                                    decimal,
                                    string,
                                    string[3:7],
                                    len(string),
                    ...
                    if not self["last_estop%d" % index] == self.h[index]["estop"]:
                        if not self.h[index]["estop"]:
                            ...
                            print(
                                "**** Mitsubishi VFD: %s stopped due to Estop Signal"
                                % ids[0]
                            )
                            continue
                        else:
                        ...
                        print(
                            "**** Mitsubishi VFD: Estop cleared - Must re-issue run command to start %s."
                            % ids[0]
                        )
                        self["last_estop%d" % index] = self.h[index]["estop"]
                    ...
                    if (
                        not self["last_run%d" % index] == self.h[index]["run"]
                        or not self["last_fwd%d" % index] == self.h[index]["fwd"]
                    ):
                        if self.h[index]["run"]:
                            if self.h[index]["fwd"]:
                                cmd = "FA"
                    ...
                    if not self["last_cmd%d" % index] == self.h[index]["motor-cmd"]:
                        freq = int(
                            abs(
                                self.h[index]["motor-cmd"]
                                * 100
                                * self.h[index]["scale-cmd"]
                            )
                        )
                        if freq > 40000:
                            freq = 40000
    ...
    def prepare_data(self, command="E1", data="07AD"):
        ...
        converted_data = (
            chr(0x5) + combined + hex(s)[-2:-1].upper() + hex(s)[-1:].upper()
        )
...
if __name__ == "__main__":
    ...
    for o, p in opts:
        if o in ["-p", "--port"]:
            ...
            print(
                " This is a user space component for controlling a Mitsubishi inverter over the serial port using the rs485 standard."
            )
            print(
                " Specifically for the A500 F500 E500 A500 D700 E700 F700 series - others may work or need small adjustments."
            )
            print(
                """ I referenced manual 'communication option reference manual' and 'A500 technical manual' for 500 series."""
            )
            print(
                """ 'FR-A700 F700 E700 D700 technical manual' for the 700 series, see https://dl.mitsubishielectric.com/dl/fa/document/manual/inv/sh060014/sh060014engb.pdf"""
            )
            print()
            print(
                "#################################################################################"
            ...
            print(
                " You must power cycle the inverter for some of these to register eg 79"
            )
            print(
                " PR 79 - 1 or 0                          sets the inverter to respond to the PU/computer-link"
            )
            print(
                " PR 117 station number (slave) - 1       can be optionally set 0 - 31 if component is also set"
            )
            print(
                " PR 118 communication speed 96           baud rate, can be optionally set 48,96,192 if component is also set"
            )
            print(
                """ PR 119 stop bit/data length - 1         8 bits, two stop (don't change)"""
            )
...
"""
            ...
            print(
                """loadusr mitsub_vfd --baud 4800 --port /dev/ttyUSB0 NAME=SLAVE_NUMBER
