...
// command registers
#define REG_COMMAND1                                                           \
    0xFA00 // "Communication command" - start/stop, fwd/reverse, DC break, fault reset, panel override
 ...
 */
#define POLLCYCLES                                                             \
    10 // read less important parameters only on every 10th transaction
#define MODBUS_MIN_OK                                                          \
    10 // assert the modbus-ok pin after 10 successful modbus transactions
...
typedef struct {
    ...
    hal_float_t
        *max_rpm; // calculated based on VFD max frequency setup parameter
    hal_s32_t *trip_code;
    ...
    hal_bit_t
        *modbus_ok; // the last MODBUS_OK transactions returned successfully
    hal_float_t *speed_command; // speed command input

    hal_bit_t *spindle_on; // spindle 1=on, 0=off
    hal_bit_t *
        DC_brake; // setting this will turn off the spindle and engage the DC brake
    hal_bit_t *spindle_fwd; // direction, 0=fwd, 1=rev
    hal_bit_t *spindle_rev; // on when in rev and running
    hal_bit_t
        *err_reset; // reset errors when 1  - set fault reset bit in 0xFA00
    hal_bit_t *
        jog_mode; // termed 'jog-run' in manual - might be useful for spindle positioning
    hal_s32_t *
        errorcount; // number of failed Modbus transactions - hints at logical errors
    ...
    hal_bit_t
        *acc_dec_pattern; // if set: choose ramp times as defined in F500+F501
    // if zero (default): choose ramp times as defined in ACC and DEC
    hal_bit_t *
        enabled; // if set: control VFD via Modbus commands, panel control disabled
                 // if zero (default): manual control through panel enabled
    hal_float_t
        *upper_limit_hz; // VFD setup parameter - maximum output frequency in HZ

    hal_bit_t
        *max_speed; // 1: run as fast as possible, ignore unimportant registers
...
typedef struct params {
    ...
    int modbus_ok;
    uint16_t
        failed_reg; // remember register for failed modbus transaction for debugging
    int last_errno;
    char *tcp_destip;
    int report_device;
...
static params_type param = {
    ...
    .tcp_portno =
        1502, // MODBUS_TCP_DEFAULT_PORT (502) would require root privileges
    .progname = "vfs11_vfd",
    .section = "VFS11",
    .fp = NULL,
    .inifile = NULL,
    ...
    .old_cmd1_reg = 0,
    .modbus_ok =
        0, // set modbus-ok bit if last MODBUS_OK transactions went well
    .failed_reg = 0,
    .last_errno = 0,
...
void windup(param_pointer p)
{
    if (p->haldata && *(p->haldata->errorcount)) {
        fprintf(stderr,
                "%s: %d modbus errors\n",
                p->progname,
                *(p->haldata->errorcount));
        fprintf(stderr,
                "%s: last command register: 0x%.4x\n",
                p->progname,
                p->failed_reg);
        fprintf(stderr,
                "%s: last error: %s\n",
                p->progname,
                modbus_strerror(p->last_errno));
    }
    if (p->hal_comp_id >= 0)
...
static void quit(int sig)
{
    ...
    default: connection_state = DONE; break;
    }
}
    ...
    if ((p->fp = fopen(p->inifile, "r")) != NULL) {
        ...
        if (iniFindDouble(p->fp, "RESPONSE_TIMEOUT", p->section, &f)) {
            ...
            p->response_timeout.tv_usec =
                (f - p->response_timeout.tv_sec) * 1000000;
        }
        ...
        if (iniFind(p->fp, "RTS_MODE", p->section) != NULL) {
            fprintf(
                stderr,
                "%s: warning - the RTS_MODE feature is not available with the "
                "installed libmodbus version (%s).\n"
        ...
        if (findkwd(p,
            ...
            fprintf(stderr,
                    "%s: missing required TYPE in section %s\n",
                    p->progname,
                    p->section);
            return -1;
        }
        ...
        fprintf(
            stderr, "%s:can not open inifile '%s'\n", p->progname, p->inifile);
        return -1;
    }
    return 0;
}
...
void usage(int argc, char **argv)
{
    ...
    printf(
        ...
        "Options are:\n"
        "-I or --ini <INI file>\n"
        "    Use <INI file> (default: take INI file name from environment "
...
retry:
    ...
    if ((haldata->motor_nameplate_RPM < 600) ||
        (haldata->motor_nameplate_RPM > 5000))
    ...
    *(haldata->spindle_fwd) = *(haldata->spindle_rev) ? 0 : 1;
    ...
    if (*(haldata->err_reset) &&
        !(p->old_cmd1_reg & CMD_FAULT_RESET)) { // not sent yet
    ...
    if (*(haldata->estop) &&
        !(p->old_cmd1_reg & CMD_EMERGENCY_STOP)) { // not sent yet)
    ...
    if ((*(haldata->freq_cmd) > 0.01) &&
        ((1.0 - *(haldata->freq_out) / *(haldata->freq_cmd)) <
    ...
    *(haldata->max_rpm) = *(haldata->upper_limit_hz) *
    ...
    if (p->report_device) {
        ...
        printf("%s: maximum ratings: %.1fV %.1fA %.2fHz\n",
               ...
               max_freq / 100.0);
        printf(
            "%s: versions: cpu1=%d/0x%4.4x cpu2=%d/0x%4.4x eeprom=%d/0x%4.4x\n",
            p->progname,
            cpu1,
            cpu1,
            cpu2,
...
failed:
    ...
    if (p->debug)
        fprintf(stderr,
                "%s: read_initial: modbus_read_registers(0x%4.4x): %s\n",
                p->progname,
                curr_reg,
...
failed:
    ...
    if (p->debug)
        fprintf(stderr,
                "%s: read_data: modbus_read_registers(0x%4.4x): %s\n",
                p->progname,
                curr_reg,
                modbus_strerror(errno));
    return p->last_errno;
...
{
    int status;
    PIN(hal_pin_bit_newf(
        HAL_IN, &(h->acc_dec_pattern), id, "%s.acceleration-pattern", name));
    PIN(hal_pin_s32_newf(HAL_OUT, &(h->alarm_code), id, "%s.alarm-code", name));
    PIN(hal_pin_bit_newf(HAL_OUT, &(h->at_speed), id, "%s.at-speed", name));
    PIN(hal_pin_float_newf(HAL_OUT,
                           &(h->load_current_pct),
                           id,
                           "%s.current-load-percentage",
                           name));
    ...
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->freq_cmd), id, "%s.frequency-command", name));
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->freq_out), id, "%s.frequency-out", name));
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->inv_load_pct), id, "%s.inverter-load-percentage", name));
    PIN(hal_pin_bit_newf(
        HAL_OUT, &(h->is_e_stopped), id, "%s.is-e-stopped", name)); // JET
    PIN(hal_pin_bit_newf(
        HAL_OUT, &(h->is_stopped), id, "%s.is-stopped", name)); // JET
    PIN(hal_pin_float_newf(HAL_OUT, &(h->max_rpm), id, "%s.max-rpm", name));
    PIN(hal_pin_bit_newf(
        HAL_OUT, &(h->modbus_ok), id, "%s.modbus-ok", name)); // JET
    PIN(hal_pin_float_newf(HAL_OUT, &(h->RPM), id, "%s.motor-RPM", name));
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->curr_out_pct), id, "%s.output-current-percentage", name));
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->outV_pct), id, "%s.output-voltage-percentage", name));
    PIN(hal_pin_float_newf(
        HAL_IN, &(h->speed_command), id, "%s.speed-command", name));
    PIN(hal_pin_bit_newf(
        HAL_IN, &(h->spindle_fwd), id, "%s.spindle-fwd", name));
    PIN(hal_pin_bit_newf(HAL_IN, &(h->spindle_on), id, "%s.spindle-on", name));
    PIN(hal_pin_bit_newf(
        HAL_IN, &(h->spindle_rev), id, "%s.spindle-rev", name)); //JET
    ...
    PIN(hal_pin_s32_newf(
        HAL_OUT, &(h->errorcount), id, "%s.error-count", name));
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->upper_limit_hz), id, "%s.frequency-limit", name));
    ...
    PIN(hal_param_float_newf(
        HAL_RW, &(h->motor_nameplate_hz), id, "%s.nameplate-HZ", name));
    PIN(hal_param_float_newf(
        HAL_RW, &(h->motor_nameplate_RPM), id, "%s.nameplate-RPM", name));
    PIN(hal_param_float_newf(
        HAL_RW, &(h->rpm_limit), id, "%s.rpm-limit", name));
    PIN(hal_param_float_newf(
        HAL_RW, &(h->speed_tolerance), id, "%s.tolerance", name));
...
int set_defaults(param_pointer p)
{
    ...
    *(h->max_speed) = 0;
    ...
    h->motor_nameplate_hz =
        50; // folks in The Colonies typically would use 60Hz and 1730 rpm
...
int main(int argc, char **argv)
{
    ...
    if (p->inifile) {
        ...
        fprintf(stderr,
                "%s: ERROR: no INI file - either use '--ini <INI_file>' or set "
                "INI_FILE_NAME environment variable\n",
                p->progname);
        goto finish;
    }
    ...
    if ((p->hal_comp_id < 0) || (connection_state == DONE)) {
        fprintf(stderr,
                "%s: ERROR: hal_init(%s) failed: HAL error code=%d\n",
                p->progname,
                p->modname,
                p->hal_comp_id);
    ...
    if ((p->haldata == 0) || (connection_state == DONE)) {
        fprintf(stderr,
                "%s: ERROR: unable to allocate shared memory\n",
                p->modname);
        retval = -1;
        goto finish;
    }
    ...
    case TYPE_RTU:
        ...
        if ((p->ctx = modbus_new_rtu(
                 p->device, p->baud, p->parity, p->bits, p->stopbits)) ==
            NULL) {
            fprintf(stderr,
                    "%s: ERROR: modbus_new_rtu(%s): %s\n",
                    p->progname,
                    p->device,
                    modbus_strerror(errno));
            goto finish;
        ...
        if (modbus_set_slave(p->ctx, p->slave) < 0) {
            fprintf(stderr,
                    "%s: ERROR: invalid slave number: %d\n",
                    p->modname,
                    p->slave);
            goto finish;
        }
        if ((retval = modbus_connect(p->ctx)) != 0) {
            fprintf(stderr,
                    "%s: ERROR: couldn't open serial device: %s\n",
                    p->modname,
                    modbus_strerror(errno));
            goto finish;
        }
        ...
        if ((p->serial_mode != -1) &&
            ...
            fprintf(stderr,
                    "%s: ERROR: modbus_rtu_set_serial_mode(%d): %s\n",
                    p->modname,
                    p->serial_mode,
                    modbus_strerror(errno));
            goto finish;
        ...
        if ((p->rts_mode != -1) &&
            ...
            fprintf(stderr,
                    "%s: ERROR: modbus_rtu_set_rts(%d): %s\n",
                    p->modname,
                    p->rts_mode,
                    modbus_strerror(errno));
            goto finish;
    ...
    case TYPE_TCP_CLIENT:
        if ((p->ctx = modbus_new_tcp(p->tcp_destip, p->tcp_portno)) == NULL) {
            fprintf(stderr,
                    "%s: Unable to allocate libmodbus TCP context: %s\n",
                    p->progname,
                    modbus_strerror(errno));
            goto finish;
        }
        ...
        if (modbus_connect(p->ctx) < 0) {
            fprintf(stderr,
                    "%s: TCP connection to %s:%d failed: %s\n",
                    p->progname,
                    p->tcp_destip,
                    p->tcp_portno,
                    modbus_strerror(errno));
    ...
    default:
        fprintf(stderr,
                "%s: ERROR: invalid connection type %d\n",
                p->progname,
                p->type);
        goto finish;
    }
    ...
    while (connection_state != DONE) {
        ...
        while (connection_state == CONNECTED) {
            ...
            loop_timespec.tv_nsec =
                (long)((p->haldata->looptime - loop_timespec.tv_sec) *
                       1000000000l);
        ...
        case DONE:
            ...
            if ((retval = modbus_write_register(p->ctx, REG_COMMAND1, 0)) !=
                1) {
                // not much we can do about it here if it goes wrong, so complain
                fprintf(stderr,
                        "%s: failed to release VFD from bus control (write to "
                        "register 0x%x): %s\n",
                        p->progname,
                        REG_COMMAND1,
                        modbus_strerror(errno));
            ...
            case TYPE_TCP_SERVER:
                while ((connection_state != CONNECTED) &&
                    ...
                    if (!modbus_tcp_accept(p->ctx, &socket)) {
                        fprintf(stderr,
                                "%s: recovery: modbus_tcp_accept(): %s\n",
                                p->progname,
                                modbus_strerror(errno));
                    } else {
                        connection_state = CONNECTED;
