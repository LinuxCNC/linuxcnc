...
options = [
    ...
    Option("-d", action="store_true", dest="debug", help="Enable debug output"),
    Option(
        "-g",
        dest="geometry",
        default="",
...
use -g WIDTHxHEIGHT for just setting size or -g +XOFFSET+YOFFSET for just position""",
    ...
    Option(
        "-t", dest="theme", default="", help="Set gtk theme. Default is system theme"
    ),
    Option(
        "-x",
        dest="parent",
...
def load_handlers(usermod, halcomp, builder, useropts):
    ...
    for u in usermod:
        ...
        try:
            ...
            if h and callable(h):
                LOG.debug(
                    "module '%s' : '%s' function found" % (mod.__name__, hdl_func)
                )
                ...
                LOG.debug(
                    "module '%s': no '%s' function - registering only functions as callbacks"
                    % (mod.__name__, hdl_func)
                )
                objlist = [mod]
            # extract callback candidates
            for object in objlist:
                LOG.debug(
                    "Registering handlers in module %s object %s"
                    % (mod.__name__, object)
                )
            ...
            LOG.warning(
                "gladevcp: trouble looking for handlers in '%s': %s" % (basename, e)
            )
            traceback.print_exc()
...
def main():
    ...
    LOG = logger.initBaseLogger(
        "GladeVCP-" + temp, log_file=None, log_level=logger.INFO
    )
    ...
    if window is None:
        LOG.error(
            'GLADE VCP ERROR: No window named "window1" found. The toplevel windows has to be named "window1"!'
        )
        sys.exit(0)
    ...
    if opts.gtk_rc:
        LOG.debug(
            "GLADE VCP INFO: %s reading gtkrc file '%s'" % (opts.component, opts.gtk_rc)
        )
    ...
    if opts.theme:
        LOG.debug(
            "GLADE VCP INFO: Switching %s to '%s' theme" % (opts.component, opts.theme)
        )
