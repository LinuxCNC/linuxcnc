...
int init(int argc, char *argv[])
{
    ...
    for (i = 0; i < me->num_insts; i++) {
        ...
        if (hal_pin_u32_newf(
                HAL_IN, &(hal->keycode), comp_id, "sendkeys.%i.keycode", i) <
            0) {
            free(codes);
            free(pins);
        ...
        if (hal_pin_s32_newf(HAL_OUT,
                             &(hal->current_event),
                             comp_id,
                             "sendkeys.%i.current-event",
                             i) < 0) {
            free(codes);
            free(pins);
        ...
        if (hal_pin_bit_newf(
                HAL_IN, &(hal->init), comp_id, "sendkeys.%i.init", i) < 0) {
            free(codes);
            free(pins);
        ...
        rtapi_print_msg(RTAPI_MSG_DBG,
                        "instance %i events %i triggers %i\n",
                        i,
                        codes[i],
                        pins[i]);
        ...
        for (j = 0; j < param->num_codes; j++) {
            if (hal_param_u32_newf(HAL_RW,
                                   &(hal->event[j]),
                                   comp_id,
                                   "sendkeys.%i.scan-event-%02i",
                                   i,
                                   j) < 0) {
                free(codes);
                free(pins);
                rtapi_print_msg(
                    RTAPI_MSG_ERR, "sendkeys.N.scan-event-%02i error\n", j);
                return -ENOMEM;
            }
        }
        for (j = 0; j < param->num_triggers; j++) {
            if (hal_param_u32_newf(HAL_RW,
                                   &(hal->event[j + param->num_codes]),
                                   comp_id,
                                   "sendkeys.%i.pin-event-%02i",
                                   i,
                                   j) < 0) {
                free(codes);
                free(pins);
                rtapi_print_msg(
                    RTAPI_MSG_ERR, "sendkeys.N.pin-event-%02i error\n", j);
                return -ENOMEM;
            }
        }
        // trigger pins
        ...
        for (j = 0; j < param->num_triggers; j++) {
            if (hal_pin_bit_newf(HAL_IN,
                                 &(hal->trigger[j]),
                                 comp_id,
                                 "sendkeys.%i.trigger-%02i",
                                 i,
                                 j) < 0) {
                rtapi_print_msg(
                    RTAPI_MSG_ERR, "sendkeys.N.trigger-%02i error\n", j);
                free(codes);
                free(pins);
                return -ENOMEM;
            }
        ...
        for (i = 0; i < me->num_insts; i++) {
            ...
            if (*hal->init && !param->inited) {
                ...
                if (param->fd < 0) {
                    rtapi_print_msg(
                        RTAPI_MSG_ERR,
                        "Cannot open /dev/uinput. Suggest chmod 666 "
                        "/dev/uinput\n");
                    abort();
                }
            ...
            if (*hal->keycode != param->oldcode) {
                ...
                if ((*hal->keycode & 0xC0) == 0xC0) { // keydown
                    emit(
                        param->fd, EV_KEY, hal->event[*hal->keycode & 0x3F], 1);
                    emit(param->fd, EV_SYN, SYN_REPORT, 0);
                } else if ((*hal->keycode & 0xC0) == 0x80) { // keyup
                    emit(
                        param->fd, EV_KEY, hal->event[*hal->keycode & 0x3F], 0);
                    emit(param->fd, EV_SYN, SYN_REPORT, 0);
                }
