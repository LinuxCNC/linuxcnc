...
static int
check_crc16(hycomm_param_t *hc_param, uint8_t *msg, const int msg_length)
{
    int ret;
    uint16_t crc_calc;
...
static int
hycomm_send(hycomm_param_t *hc_param, uint8_t *query, int query_length)
{
    int ret;
...
#define WAIT_DATA()                                                            \
    {                                                                          \
        while ((select_ret =                                                   \
                    select(hc_param->fd + 1, &rfds, NULL, NULL, &tv)) == -1) { \
            if (errno == EINTR) {                                              \
...
int receive_msg(hycomm_param_t *hc_param,
                ...
                int *msg_length)
{
    ...
    while (select_ret) /* loop to receive data until end of msg	or time out */
    {
        ...
        if (read_ret == -1) {
            error_treat(
                hc_param, PORT_SOCKET_FAILURE, "Read port/socket failure");
        ...
        if ((*msg_length) < msg_length_computed) {
            ...
            if ((*msg_length >= 2) && (*msg_length == msg[2] + 5)) {
                ...
                if (hc_param->debug) {
                    printf(
                        "message was incomplete, length still to read = [%.2X]",
                        length_to_read);
                    printf("\n");
                }
            }
                                 ...
                                 uint8_t *response)
{
    ...
    ret = receive_msg(
        hc_param, response_length_computed, response, &response_length);
    ...
    if (ret == 0) {
        ...
        if (hc_param->debug) {
            printf("we received a message of [%.2X] bytes, with a valid crc",
                   response_length);
            printf("\n");
        }
...
int hy_comm(hycomm_param_t *hc_param, hycomm_data_t *hc_data)
{
    ...
    if (query_ret > 0) {
        ...
        if (response_ret == 0) {
            ...
            if (hc_param->debug) {
                printf("the message function code is = [%.2X]",
                       msg_function_code);
                printf("\n");
            }
...
void hycomm_init(hycomm_param_t *hc_param,
                 const char *device,
                 int baud,
                 const char *parity,
                 int data_bit,
