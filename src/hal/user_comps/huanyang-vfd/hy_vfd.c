...
typedef struct {
    ...
    hal_float_t *
        rated_motor_voltage; // PD141 Rated Motor Voltage - as per motor name plate
    hal_float_t *
        rated_motor_current; // PD142 Rated Motor Current - as per motor name plate
    hal_u32_t
        *motor_poles; // PD143 Number of motor poles - from motor name plate
    hal_float_t *
        rated_motor_rev; // max motor speed (at max_freq).  PD144 gets set to value corresponding to RPM at 50Hz

    hal_bit_t
        *hycomm_ok; // the last HYCOMM_OK transactions returned successfully
    ...
    hal_float_t
        *spindle_speed_fb_rps; // (out) reports current spindle speed (rps)
    hal_bit_t *
        spindle_at_speed; // (out) True when spindle is on and at commanded speed
...
void usage(int argc, char **argv)
{
    ...
    printf(
        ...
        "-g or --debug\n"
        "    Turn on debugging messages.  Debug mode will cause all hycomm "
        "messages\n"
        ...
        "    Set HYCOMM target (slave) number.  This must match the device "
        "number you set on the Huanyang VFD.\n"
        ...
        "    Set VFD base frequency to <f> Hz.  This will be read from the "
        "VFD\n"
        "    register 11 if not supplied on the command line.\n"
        "-V or --motor-voltage <v>\n"
        "    Set VFD max output voltage to <v> (Volts).  This will be read "
        "from the\n"
        "    VFD register 141 if not supplied on the command line.\n"
        "-I or --motor-current <v>\n"
        "    Set VFD max output current to <i> (Amps).  This will be read from "
        "the\n"
        "    VFD register 142 if not supplied on the command line.\n"
        "-S or --motor-speed <r>\n"
        "    Set the motor's max speed to <r> (RPM).  This will be computed "
        "from the\n"
        "    50 Hz RPM value read from the VFD register P144 if not supplied "
        "on the\n"
        "    command line.\n"
        "-P or --motor-poles <n>\n"
        "    Set VFD register PD143 (Number of Motor Poles) to <n>.  This "
        "value\n"
        "    comes from the motor's name plate.\n"
        "-x or --register PDnnn=mmm\n"
        "    Set the specified register to the specified value at startup\n"
        "    Generally a one-time setting. Up to ten registers at a time can "
        "be set by repeating the command\n"
        "-y or --regdump\n"
        "    Print the values of all registers when the VFD is enabled\n");
}

int write_data(hycomm_param_t *hc_param,
               ...
               haldata_t *haldata)
{
    ...
    if ((*(haldata->spindle_forward) && !*(haldata->spindle_reverse)) &&
        *(haldata->spindle_on)) {
        freq_comp = 1;
    } else if ((*(haldata->spindle_reverse) && !*(haldata->spindle_forward)) &&
               *(haldata->spindle_on)) {
        freq_comp = -1;
    } else {
        freq_comp = 0;
    ...
    freq =
        abs((int)((*(haldata->speed_command) + freq_comp) * hz_per_rpm * 100));
...
int read_setup(hycomm_param_t *hc_param,
               hycomm_data_t *hc_data,
               haldata_t *haldata)
{
    int retval;
    ...
    for (int i = 0; hc_param->extra_reg[i] != 0; i++) {
        ...
        if (hc_param->debug)
            printf("Register %i successfully set to %i\n",
                   hc_param->extra_reg[i],
                   hc_param->extra_val[i]);
    }
...
int read_data(hycomm_param_t *hc_param,
              hycomm_data_t *hc_data,
              haldata_t *haldata)
{
    int retval;
    ...
    *(haldata->Out_F) = hc_data->ret_data * 0.01;
    ...
    if (*haldata->spindle_on && (fabs(*haldata->speed_command) > 0.0) &&
        (*haldata->Set_F > 0.0)) {
        *haldata->spindle_speed_fb =
            (*haldata->Out_F / *haldata->max_freq) * *haldata->rated_motor_rev;
        *haldata->spindle_speed_fb_rps = *haldata->spindle_speed_fb / 60.0;
        if (fabs(1 -
                 (*haldata->spindle_speed_fb / fabs(*haldata->speed_command))) <
            *haldata->spindle_at_speed_tolerance) {
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt_long(argc, argv, option_string, long_options, NULL)) !=
        ...
        case 'x': // not in list, is it an explicit PD number?
            if (++extra_index < 10) {
                retval = sscanf(optarg,
                                "PD%d=%d",
                                &extra_reg[extra_index],
                                &extra_val[extra_index]);
                if (retval < 0) {
        ...
        case 'S':
            ...
            if ((*endarg != '\0') || (motor_speed < 0.0) ||
                (motor_speed > UINT16_MAX)) {
    ...
    if (debug & 1) {
        printf("%s: device='%s', baud=%d, bits=%d, parity='%s', stopbits=%d, "
               "address=%d, debug=%d, PID=%d\n",
               modname,
               device,
               baud,
               bits,
    ...
    retval = hal_pin_bit_newf(
        HAL_IN, &(haldata->enable), hal_comp_id, "%s.enable", modname);
    if (retval != 0)
        goto out_closeHAL;

    retval = hal_pin_float_newf(
        HAL_OUT, &(haldata->Set_F), hal_comp_id, "%s.SetF", modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
    ...
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
        HAL_OUT, &(haldata->Out_A), hal_comp_id, "%s.OutA", modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
        HAL_OUT, &(haldata->RoTT), hal_comp_id, "%s.Rott", modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
    ...
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
        HAL_OUT, &(haldata->ACV), hal_comp_id, "%s.ACV", modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
    ...
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
        HAL_OUT, &(haldata->Tmp), hal_comp_id, "%s.Tmp", modname);
    if (retval != 0)
        goto out_closeHAL;

    retval = hal_pin_bit_newf(HAL_IN,
                              &(haldata->spindle_forward),
                              hal_comp_id,
                              "%s.spindle-forward",
                              modname);
    if (retval != 0)
    ...
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
        HAL_OUT, &(haldata->CNST), hal_comp_id, "%s.CNST", modname);
    if (retval != 0)
        goto out_closeHAL;

    retval = hal_pin_bit_newf(
        HAL_OUT, &(haldata->CNST_Run), hal_comp_id, "%s.CNST-run", modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_bit_newf(
        HAL_OUT, &(haldata->CNST_Jog), hal_comp_id, "%s.CNST-jog", modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_bit_newf(HAL_OUT,
                              &(haldata->CNST_Command_rf),
                              hal_comp_id,
                              "%s.CNST-command-rf",
                              modname);
    if (retval != 0)
    ...
    retval = hal_pin_float_newf(
        HAL_OUT, &(haldata->max_freq), hal_comp_id, "%s.max-freq", modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(
        HAL_OUT, &(haldata->base_freq), hal_comp_id, "%s.base-freq", modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(HAL_OUT,
                                &(haldata->freq_lower_limit),
                                hal_comp_id,
                                "%s.freq-lower-limit",
                                modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(HAL_OUT,
                                &(haldata->rated_motor_voltage),
                                hal_comp_id,
                                "%s.rated-motor-voltage",
                                modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(HAL_OUT,
                                &(haldata->rated_motor_current),
                                hal_comp_id,
                                "%s.rated-motor-current",
                                modname);
    if (retval != 0)
        goto out_closeHAL;
    retval = hal_pin_float_newf(HAL_OUT,
                                &(haldata->rated_motor_rev),
                                hal_comp_id,
                                "%s.rated-motor-rev",
                                modname);
    if (retval != 0)
    ...
    retval = hal_pin_bit_newf(
        HAL_OUT, &(haldata->hycomm_ok), hal_comp_id, "%s.hycomm-ok", modname);
    if (retval != 0)
        goto out_closeHAL;
    ...
    retval = hal_pin_u32_newf(
        HAL_OUT, &(haldata->retval), hal_comp_id, "%s.retval", modname);
    if (retval != 0)
        goto out_closeHAL;
    ...
    retval = hal_pin_float_newf(HAL_IN,
                                &(haldata->spindle_at_speed_tolerance),
                                hal_comp_id,
                                "%s.spindle-at-speed-tolerance",
                                modname);
    if (retval != 0)
        goto out_closeHAL;

    retval = hal_pin_float_newf(HAL_OUT,
                                &(haldata->spindle_speed_fb_rps),
                                hal_comp_id,
                                "%s.spindle-speed-fb-rps",
                                modname);
    if (retval != 0)
        goto out_closeHAL;
