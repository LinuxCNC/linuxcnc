...
class Compensation:
    ...
    def loadMap(self):
        ...
        self.data = np.loadtxt(
            self.filename, dtype=float, delimiter=" ", usecols=(0, 1, 2)
        )
        ...
        if self.h.method == int(Methods.CUBIC):
            ...
            method = "cubic"
            print(
                "ERROR: z_level_compensation: HAL pin interpolation \
method {} not recognised (outside 0-2). Defaulting to cubic(0).".format(
                    self.h.method
                )
            )
        # interpolate, zi has all the offset values but need to be transposed
        self.zi = griddata(
            (self.x_data, self.y_data), self.z_data, (self.xi, self.yi), method=method
        )
    ...
    def compensate(self):
        ...
        self.xpos = (
            ...
            else self.xMax if self.xpos > self.xMax else self.xpos
        )
        self.ypos = (
            self.yMin
            if self.ypos < self.yMin
            else self.yMax if self.ypos > self.yMax else self.ypos
        )
