...
h["cut-noise-in"] = 0.75
h["cycles-in"] = (
    200  # "the number of cycles that the arc voltage overshoots the cut voltage (cycles)"
)
h["on-delay-in"] = 10  # "the time from turn on until overshoot begins (cycles)"
h["overshoot-in"] = (
    50  # "the percentage of the cut voltage that the arc voltage overshoots (percent)"
)
        ...
        if h["start"]:
            ...
            if current_cycle < cycles:
                if current_cycle < on_delay:
                    ...
                    h["voltage-out"] = (
                        (voltage + overshoot_start) / on_delay * current_cycle
                    )
                    ...
                    h["voltage-out"] = (
                        voltage
                        + overshoot_start
                        + current_cycle * math.tan(angle)
                        + random_noise(current_cycle, ramp_noise)
                    )
                else:
                    ...
                    h["voltage-out"] = (
                        voltage
                        + overshoot_max
                        - (current_cycle - up_cycles) * math.tan(angle)
                        + random_noise(current_cycle, ramp_noise)
                    )
            else:
                # cut voltage voltage
                if h["void"]:
                    ...
                    h["voltage-out"] = (
                        voltage + offset + random_noise(current_cycle, cut_noise)
                    )
                    void_cycle = 0
            current_cycle += 1
        else:
