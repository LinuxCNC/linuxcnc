...
#define REG_COMMAND1                                                           \
    0x2000 // "Communication command" - start/stop, fwd/reverse, DC break, fault reset, panel override
...
int read_ini(param_pointer p)
{
    ...
    if ((p->fp = fopen(p->inifile, "r")) != NULL) {
        ...
        fprintf(
            stderr, "%s:can not open INI file '%s'\n", p->progname, p->inifile);
        return -1;
    }
    return 0;
}
...
int hal_setup(int id, haldata_t *h, const char *name)
{
    ...
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->output_current), id, "%s.output-current", name));
    ...
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->torque_ratio), id, "%s.inverter-load-percentage", name));
    ...
    PIN(hal_pin_float_newf(
        HAL_OUT, &(h->output_volt), id, "%s.output-voltage", name));
    PIN(hal_pin_float_newf(
...
int main(int argc, char **argv)
{
    ...
    if (p->inifile) {
        ...
        fprintf(stderr,
                "%s: ERROR: no inifile - either use '--ini filename' or set "
    ...
    if ((p->ctx = modbus_new_rtu(
             p->device, p->baud, p->parity, p->bits, p->stopbits)) == NULL) {
        fprintf(stderr,
