...
void DrawElement(GdkPixmap * DrawPixmap,int x,int y,int Width,int Height,StrElement Element,char DrawForToolBar)
{
    char BufTxt[50];
    StrTimer * Timer;
...
/*                           gdk_draw_rectangle (DrawPixmap,
        ...
        case ELE_OUTPUT_CALL:
                ...
                gdk_draw_arc (DrawPixmap, TheGc, FALSE,
                                x+WidDiv4, y+WidDiv4, WidDiv2, HeiDiv2, (90+20)*64, 150*64);
                gdk_draw_arc (DrawPixmap, TheGc, FALSE,
                                x+WidDiv4, y+WidDiv4, WidDiv2, HeiDiv2, (270+20)*64, 150*64);
                           break;
    }
    /* Drawing / */
    switch(Element.Type)
    ...
    {
        case ELE_OUTPUT_SET:
            gdk_draw_text(DrawPixmap, gtk_style_get_font(drawing_area->style), TheGc,
    ...
    if (DrawForToolBar)
    {
        switch(Element.Type)
        {
            case EDIT_POINTER:
                gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
                    x+WidDiv4,y+HeiDiv4, x+Width-WidDiv4,y+Height-HeiDiv4); /* \ */
                gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
                    x+WidDiv4,y+HeiDiv4, x+WidDiv3,y+HeiDiv2);  /* | */
                gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
                    x+WidDiv4,y+HeiDiv4, x+WidDiv2,y+HeiDiv3);  /* _ */
                gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
...
void GetTheSizesForRung()
{
    ...
    InfosGene->BlockWidth = InfosGene->BlockHeight = (GTK_WIDGET(drawing_area)->allocation.width - OFFSET_X -5) / RUNG_WIDTH;
...
void DrawRung(StrRung * Rung, int PosiY)
{
    ...
    for (y=0;y<RUNG_HEIGHT;y++)
    {
        ...
        {
            DrawElement(pixmap, x*InfosGene->BlockWidth+OFFSET_X, y*InfosGene->BlockHeight+OFFSET_Y+PosiY, InfosGene->BlockWidth,InfosGene->BlockHeight, Rung->Element[x][y], FALSE);
        }
    }
}
...
void DrawRungs()
{
    ...
    for (ScanY = -InfosGene->OffsetHiddenTopRungDisplayed; (ScanY < InfosGene->PageHeight) && !TheEnd; ScanY += (InfosGene->BlockHeight*RUNG_HEIGHT))
    {
...
void DrawCurrentSection( void )
{
	...
	if ( iCurrentLanguage==SECTION_IN_SEQUENTIAL )
		DrawSequentialPage( SectionArray[ InfosGene->CurrentSection ].SequentialPage );
#endif
	update_rect.x = 0;
	update_rect.y = 0;
