...
void CreateVarNameForElement(char *pBuffToWrite,
                             size_t BuffSize,
                             StrElement *pElem,
                             char SymbolsVarsNamesIfAvail)
{
    char VarIndexBuffer[20];
    if (pElem->IndexedVarType != -1) {
        // buffer for index required as CreateVarName() returns on a static buffer !
        rtapi_strxcpy(VarIndexBuffer,
                      CreateVarName(pElem->IndexedVarType,
                                    pElem->IndexedVarNum,
                                    SymbolsVarsNamesIfAvail));
        snprintf(pBuffToWrite,
                 BuffSize,
                 "%s",
                 CreateVarName(
                     pElem->VarType, pElem->VarNum, SymbolsVarsNamesIfAvail));
    } else {
        strcpy(pBuffToWrite,
               CreateVarName(
                   pElem->VarType, pElem->VarNum, SymbolsVarsNamesIfAvail));
...
char *DisplayArithmExpr(char *Expr, char SymbolsVarsNamesIfAvail)
{
    ...
    do {
        ...
        if (*Ptr == '@') {
            ...
            char VarBuffer[20];
            char VarIndexBuffer[20];
            if (IdentifyVarIndexedOrNot(
                    Ptr, &TypeVar, &NumVar, &IndexTypeVar, &IndexNumVar)) {
                if (IndexTypeVar != -1 && IndexNumVar != -1) {
                    // buffer for index required as CreateVarName() returns on a static buffer !
                    rtapi_strxcpy(VarIndexBuffer,
                                  CreateVarName(IndexTypeVar,
                                                IndexNumVar,
                                                SymbolsVarsNamesIfAvail));
                    size_t ret = snprintf(
                        VarBuffer,
                        sizeof(VarBuffer),
                        "%s[%s]",
                        CreateVarName(TypeVar, NumVar, SymbolsVarsNamesIfAvail),
                        VarIndexBuffer);
                    if (ret >= sizeof(VarBuffer))
                        snprintf(
                            VarBuffer, sizeof(VarBuffer), "<format too long>");
                } else {
                    rtapi_strxcpy(VarBuffer,
                                  CreateVarName(TypeVar,
                                                NumVar,
                                                SymbolsVarsNamesIfAvail));
                }
            } else
...
void CreateFontPangoLayout(cairo_t *cr, int BlockPxHeight, char DrawingOption)
{
    if (pFontLayout == NULL || BlockPxHeightUsedForFont != BlockPxHeight) {
        ...
        snprintf(
            BuffFontDesc, sizeof(BuffFontDesc), "Lucida Sans %d", FontHeight);
...
int DrawPangoTextOptions(cairo_t *cr,
                         int BaseX,
                         int BaseY,
                         int Width,
                         int Height,
                         char *Text,
                         char CenterAlignment)
{
    int SizeX, SizeY;
    int PosiY = BaseY;
    ...
    pango_layout_set_alignment(
        pFontLayout, CenterAlignment ? PANGO_ALIGN_CENTER : PANGO_ALIGN_LEFT);
    ...
    if (Height <= 0) {
        ...
        if (SizeY > Height) {
            ...
            PangoLayoutLine *playout_line =
                pango_layout_get_line(pFontLayout, 0);
...
int DrawPangoText(
    cairo_t *cr, int BaseX, int BaseY, int Width, int Height, char *Text)
{
    return DrawPangoTextOptions(cr,
                                BaseX,
                                BaseY,
                                Width,
                                Height,
                                Text,
                                /*CenterAlignment*/ Width > 0 ? TRUE : FALSE);
}
...
char *DrawExprForCompareOperate(cairo_t *cr,
                                int BaseX,
                                int BaseY,
                                int Width,
                                int Height,
                                ...
                                char DrawingOption)
{
    ...
    tcBuffNumExpr[0] = '\0';
    int NbrPixHeight = DrawPangoTextOptions(
        cr, BaseX, BaseY, Width, Height, Text, FALSE /*CenterAlignment*/
    );
    ...
    if (NbrPixHeight > 0) {
        if (DrawingOption == DRAW_FOR_PRINT) {
            ...
            if (pReportNumAndText) {
                int Hgt;
                snprintf(pReportNumAndText,
                         TextSize,
                         "(*%d) ",
                         PrintRightMarginNumExpr);
                strcat(pReportNumAndText, Text);
                Hgt = DrawPangoTextOptions(cr,
                                           PrintRightMarginPosiX,
                                           PrintRightMarginExprPosiY,
                                           PrintRightMarginWidth,
                                           0 /*Height*/,
                                           pReportNumAndText,
                                           FALSE /*CenterAlignment*/
                );
                PrintRightMarginExprPosiY += Hgt;
                free(pReportNumAndText);
            }
            snprintf(tcBuffNumExpr,
                     sizeof(tcBuffNumExpr),
                     "(*%d)",
                     PrintRightMarginNumExpr++);
        } else {
...
void DrawCommonElementForToolbar(
    cairo_t *cr, int x, int y, int Size, int NumElement)
{
    int SizeDiv2 = Size / 2;
    int SizeDiv3 = Size / 3;
    int SizeDiv4 = Size / 4;
    ...
    case EDIT_POINTER:
        my_cairo_draw_line(cr,
                           ...
                           y + Size - SizeDiv4); /* \ */
        my_cairo_draw_line(
            cr, x + SizeDiv4, y + SizeDiv4, x + SizeDiv3, y + SizeDiv2); /* | */
        my_cairo_draw_line(
            cr, x + SizeDiv4, y + SizeDiv4, x + SizeDiv2, y + SizeDiv3); /* _ */
        my_cairo_draw_line(
            cr, x + SizeDiv3, y + SizeDiv2, x + SizeDiv2, y + SizeDiv3);
        break;
    case EDIT_ERASER:
        my_cairo_draw_line(
            cr, x + SizeDiv3, y + SizeDiv4, x + Size - SizeDiv4, y + SizeDiv4);
        my_cairo_draw_line(cr,
                           x + SizeDiv4,
                           y + Size - SizeDiv4,
                           x + Size - SizeDiv3,
                           y + Size - SizeDiv4);
        my_cairo_draw_line(
            cr, x + SizeDiv3, y + SizeDiv4, x + SizeDiv4, y + Size - SizeDiv4);
        my_cairo_draw_line(cr,
                           x + Size - SizeDiv4,
                           y + SizeDiv4,
                           x + Size - SizeDiv3,
...
void my_cairo_draw_color_line(
    cairo_t *cr, char cColor, double x1, double y1, double x2, double y2)
{
    cairo_save(cr);
    if (cColor)
...
void my_cairo_draw_black_rectangle(
    cairo_t *cr, double x, double y, double w, double h)
{
    cairo_save(cr);
    cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
                 ...
                 char DrawingOption)
{
    ...
    char DisplayColorState =
        (DrawingOption == DRAW_NORMAL && !EditDatas.ModeEdit &&
...
//Cairo....
    ...
    gdk_gc_set_line_attributes(DynaGcOn,
                               THICK_LINE_ELE_ACTIVATED,
                               GDK_LINE_SOLID,
                               GDK_CAP_BUTT,
                               GDK_JOIN_MITER);
#endif
    ...
    if ((DrawingOption == DRAW_NORMAL) && (!EditDatas.ModeEdit) &&
    ...
    case ELE_RISING_INPUT:
        my_cairo_draw_line(
            cr, x + WidDiv3, y + HeiDiv3 * 2, x + WidDiv4 * 2, y + HeiDiv3);
        my_cairo_draw_line(
            cr, x + WidDiv4 * 2, y + HeiDiv3, x + WidDiv3 * 2, y + HeiDiv3 * 2);
        break;
    case ELE_FALLING_INPUT:
        my_cairo_draw_line(
            cr, x + WidDiv3, y + HeiDiv3, x + WidDiv4 * 2, y + HeiDiv3 * 2);
        my_cairo_draw_line(
            cr, x + WidDiv4 * 2, y + HeiDiv3 * 2, x + WidDiv3 * 2, y + HeiDiv3);
        break;
    }
    ...
    case ELE_OUTPUT_CALL:
        ...
        my_cairo_draw_line(
            cr, x + WidDiv3 * 2, y + HeiDiv2, x + Width, y + HeiDiv2);
    }
    /* Drawing || or () or --- */
    switch (Element.Type) {
    case ELE_INPUT:
    ...
    case ELE_FALLING_INPUT:
        my_cairo_draw_line(
            cr, x + WidDiv3, y + HeiDiv4, x + WidDiv3, y + Height - HeiDiv4);
        my_cairo_draw_line(cr,
                           x + WidDiv3 * 2,
                           y + HeiDiv4,
                           x + WidDiv3 * 2,
    ...
    switch (Element.Type) {
    case ELE_OUTPUT_SET: DrawPangoText(cr, x, y, Width, Height, "S"); break;
    case ELE_OUTPUT_RESET: DrawPangoText(cr, x, y, Width, Height, "R"); break;
    case ELE_OUTPUT_JUMP: DrawPangoText(cr, x, y, Width, Height, "J"); break;
    case ELE_OUTPUT_CALL: DrawPangoText(cr, x, y, Width, Height, "C"); break;
    }
    ...
    case ELE_TIMER:
        ...
        my_cairo_draw_black_rectangle(cr,
                                      ...
                                      Height + 1 * HeiDiv3);
        /* input : enable */
        my_cairo_draw_color_line(cr,
                                 Timer->InputEnable && DisplayColorState,
                                 x - Width,
                                 y + HeiDiv2,
                                 x - Width + WidDiv3,
                                 y + HeiDiv2);
        ...
        /* input : control */
        my_cairo_draw_color_line(cr,
                                 Timer->InputControl && DisplayColorState,
                                 x - Width,
                                 y + HeiDiv2 + Height,
                                 x - Width + WidDiv3,
                                 y + HeiDiv2 + Height);
        ...
        /* output : done */
        my_cairo_draw_color_line(cr,
                                 Timer->OutputDone && DisplayColorState,
                                 x + WidDiv3 * 2,
                                 y + HeiDiv2,
                                 x + Width,
                                 y + HeiDiv2);
        ...
        /* output : running */
        my_cairo_draw_color_line(cr,
                                 Timer->OutputRunning && DisplayColorState,
                                 x + WidDiv3 * 2,
                                 y + HeiDiv2 + Height,
                                 x + Width,
                                 y + HeiDiv2 + Height);
        DrawPangoText(
            cr, x + Width - WidDiv4, y + HeiDiv2 - 1 + Height, -1, -1, "R");
        /* Timer Number */
        ...
        if (DrawingOption != DRAW_FOR_PRINT && !EditDatas.ModeEdit)
            snprintf(BufTxt,
                     sizeof(BufTxt),
                     Timer->DisplayFormat,
                     (double)Timer->Value / (double)Timer->Base);
        else
            snprintf(BufTxt,
                     sizeof(BufTxt),
                     Timer->DisplayFormat,
                     (double)Timer->Preset / (double)Timer->Base);
    ...
    case ELE_MONOSTABLE:
        ...
        /* input */
        my_cairo_draw_color_line(cr,
                                 Monostable->Input && DisplayColorState,
                                 x - Width,
                                 y + HeiDiv2,
                                 x - Width + WidDiv3,
                                 y + HeiDiv2);
        ...
        /* output : running */
        my_cairo_draw_color_line(cr,
                                 Monostable->OutputRunning && DisplayColorState,
                                 x + WidDiv3 * 2,
                                 y + HeiDiv2,
                                 x + Width,
                                 y + HeiDiv2);
        ...
        if (DrawingOption != DRAW_FOR_PRINT && !EditDatas.ModeEdit)
            snprintf(BufTxt,
                     sizeof(BufTxt),
                     Monostable->DisplayFormat,
                     (double)Monostable->Value / (double)Monostable->Base);
        else
            snprintf(BufTxt,
                     sizeof(BufTxt),
                     Monostable->DisplayFormat,
                     (double)Monostable->Preset / (double)Monostable->Base);
        DrawPangoText(cr, x - Width, y, Width * 2, Height * 2, BufTxt);
        break;
#endif
    ...
    case ELE_COUNTER:
        ...
        /* input : reset */
        my_cairo_draw_color_line(cr,
                                 Counter->InputReset && DisplayColorState,
                                 x - Width,
                                 y + HeiDiv2,
                                 x - Width + WidDiv3,
                                 y + HeiDiv2);
        DrawPangoText(
            cr, x - Width + WidDiv4 / 2, y + HeiDiv2 - 1, -1, -1, "R");
        /* input : preset */
        my_cairo_draw_color_line(cr,
                                 Counter->InputPreset && DisplayColorState,
                                 x - Width,
                                 y + HeiDiv2 + Height,
                                 x - Width + WidDiv3,
                                 y + HeiDiv2 + Height);
        DrawPangoText(
            cr, x - Width + WidDiv4 / 2, y + HeiDiv2 - 1 + Height, -1, -1, "P");
        /* input : count up */
        my_cairo_draw_color_line(cr,
                                 Counter->InputCountUp && DisplayColorState,
                                 x - Width,
                                 y + HeiDiv2 + Height * 2,
                                 x - Width + WidDiv3,
                                 y + HeiDiv2 + Height * 2);
        DrawPangoText(cr,
                      x - Width + WidDiv4 / 2,
                      y + HeiDiv2 - 1 + Height * 2,
                      -1,
                      -1,
                      "U");
        /* input : count down */
        my_cairo_draw_color_line(cr,
                                 Counter->InputCountDown && DisplayColorState,
                                 x - Width,
                                 y + HeiDiv2 + Height * 3,
                                 x - Width + WidDiv3,
                                 y + HeiDiv2 + Height * 3);
        DrawPangoText(cr,
                      x - Width + WidDiv4 / 2,
                      y + HeiDiv2 - 1 + Height * 3,
                      -1,
                      -1,
                      "D");
        /* output : empty */
        my_cairo_draw_color_line(cr,
                                 Counter->OutputEmpty && DisplayColorState,
                                 x + WidDiv3 * 2,
                                 y + HeiDiv2,
                                 x + Width,
                                 y + HeiDiv2);
        ...
        /* output : done */
        my_cairo_draw_color_line(cr,
                                 Counter->OutputDone && DisplayColorState,
                                 x + WidDiv3 * 2,
                                 y + HeiDiv2 + Height,
                                 x + Width,
                                 y + HeiDiv2 + Height);
        DrawPangoText(
            cr, x + Width - WidDiv4, y + HeiDiv2 - 1 + Height, -1, -1, "D");
        /* output : full */
        my_cairo_draw_color_line(cr,
                                 Counter->OutputFull && DisplayColorState,
                                 x + WidDiv3 * 2,
                                 y + HeiDiv2 + Height * 2,
                                 x + Width,
                                 y + HeiDiv2 + Height * 2);
        DrawPangoText(
            cr, x + Width - WidDiv4, y + HeiDiv2 - 1 + Height * 2, -1, -1, "F");
        /* Counter Number */
        ...
        if (DisplayColorState)
            DrawPangoText(
                cr, x - Width, y + HeiDiv2, Width * 2, Height * 2, BufTxt);
        DrawPangoText(cr,
                      x - Width,
                      y + HeiDiv2 + HeiDiv3,
                      Width * 2,
    ...
    case ELE_TIMER_IEC:
        ...
        /* input : enable */
        my_cairo_draw_color_line(cr,
                                 TimerIEC->Input && DisplayColorState,
                                 x - Width,
                                 y + HeiDiv2,
                                 x - Width + WidDiv3,
                                 y + HeiDiv2);
        DrawPangoText(
            cr, x - Width + WidDiv4 / 1.5, y + HeiDiv2 - 1, -1, -1, "I");
        /* output : done */
        my_cairo_draw_color_line(cr,
                                 TimerIEC->Output && DisplayColorState,
                                 x + WidDiv3 * 2,
                                 y + HeiDiv2,
                                 x + Width,
                                 y + HeiDiv2);
        ...
        snprintf(BufTxt,
                 sizeof(BufTxt),
                 "%s",
                 TimersModesStrings[(int)TimerIEC->TimerMode]);
        DrawPangoText(
            cr, x - Width, y + HeiDiv3, Width * 2, Height - HeiDiv3, BufTxt);

        /* Current and Preset values */
        snprintf(BufTxt,
                 sizeof(BufTxt),
                 /*TimerIEC->DisplayFormat*/ "%d%c",
                 TimerIEC->Value,
                 CharUnitForTimer(TimerIEC->Base));
        snprintf(BufTxt2,
                 sizeof(BufTxt2),
                 /*TimerIEC->DisplayFormat*/ "%d%c",
                 TimerIEC->Preset,
    ...
    case ELE_COMPAR:
        ...
        if (!EditDatas.ModeEdit)
            DisplayArithmExpr(ArithmExpr[Element.VarNum].Expr,
                              InfosGene->DisplaySymbols);
        else
            DisplayArithmExpr(EditArithmExpr[Element.VarNum].Expr,
                              InfosGene->DisplaySymbols);
        ptcBuffNumExprMargin =
            DrawExprForCompareOperate(cr,
                                      x + WidDiv3 - (Width * 2) + 2,
                                      y + HeiDiv4,
                                      Width * 2 + 1 * WidDiv3 - Thickness,
                                      2 * HeiDiv4,
                                      BufTxt,
                                      DrawingOption);
        cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
        ...
        DrawPangoText(
            cr, x + WidDiv4 - (Width * 2) + 3, y + HeiDiv4 + 1, -1, -1, BufTxt);
        break;
    ...
    if (DrawingOption != DRAW_FOR_TOOLBAR) {
        ...
        case ELE_OUTPUT_SET:
        case ELE_OUTPUT_RESET:
            CreateVarNameForElement(
                BufTxt, sizeof(BufTxt), &Element, InfosGene->DisplaySymbols);
...
void DrawLeftRightBars(cairo_t *cr,
                       int OffX,
                       int PosiY,
                       int BlockWidth,
                       int BlockHeight,
                       int HeaderLabelAndCommentHeight,
                       int LeftRightBarsWidth,
                       int IsTheCurrentRung)
{
    int LeftRightBarsHeight =
        HeaderLabelAndCommentHeight + BlockHeight * RUNG_HEIGHT;
    ...
    if (!IsTheCurrentRung) {
        ...
        cairo_rectangle(cr,
                        OffX + LeftRightBarsWidth,
                        PosiY,
                        BlockWidth * RUNG_WIDTH,
                        LeftRightBarsHeight);
        cairo_fill(cr);
    ...
    cairo_rectangle(cr,
                    OffX + BlockWidth * RUNG_WIDTH + LeftRightBarsWidth,
                    PosiY,
                    LeftRightBarsWidth,
                    LeftRightBarsHeight);
    cairo_fill(cr);
...
void DrawGrid(cairo_t *cr, int OffX, int PosiY)
{
    ...
    for (y = InfosGene->BlockHeight; y < TOTAL_PX_RUNG_HEIGHT;
...
{
    if (EditDatas.CurrentElementSizeX > 0 &&
        EditDatas.CurrentElementSizeY > 0) {
        int Left = (EditDatas.CurrentElementPosiX -
                    EditDatas.CurrentElementSizeX + 1) *
                       InfosGene->BlockWidth +
                   OffX;
        int Top = EditDatas.CurrentElementPosiY * InfosGene->BlockHeight +
                  AddPosiY + InfosGene->HeaderLabelCommentHeight;
...
void DrawGhostZone(cairo_t *cr, int OffX, int AddPosiY)
{
    ...
    int Top = EditDatas.GhostZonePosiY * InfosGene->BlockHeight + AddPosiY +
              InfosGene->HeaderLabelCommentHeight;
...
void DrawPathLabelCommentHeader(cairo_t *cr,
                                StrRung *Rung,
                                int OffX,
                                int PosiY,
                                int BlockWidth,
                                ...
                                char DrawingOption)
{
    ...
    if (!Background)
        my_cairo_draw_line(cr,
                           OffX + Margin + Oblique + (BlockWidth * 2),
                           PosiY,
                           OffX + Margin + (BlockWidth * 2),
                           PosiY + HeaderBarHeight);
    cairo_move_to(cr, OffX + Margin + Oblique, PosiY);
    cairo_line_to(cr, OffX + BlockWidth * RUNG_WIDTH - Margin, PosiY);
    cairo_line_to(cr,
                  OffX + BlockWidth * RUNG_WIDTH - Margin - Oblique,
                  PosiY + HeaderBarHeight);
              ...
              char DrawingOption)
{
    ...
    if (DrawingOption != DRAW_FOR_PRINT) {
        ...
        DrawPathLabelCommentHeader(cr,
                                   Rung,
                                   OffX,
                                   PosiY,
                                   BlockWidth,
                                   HeaderLabelAndCommentHeight,
                                   1 /*Background*/,
                                   DrawingOption);
    }
    cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
    ...
    for (y = 0; y < RUNG_HEIGHT; y++) {
        for (x = 0; x < RUNG_WIDTH; x++) {
            DrawElement(cr,
                        x * BlockWidth + OffX,
                        HeaderLabelAndCommentHeight + y * BlockHeight + PosiY,
                        BlockWidth,
                        BlockHeight,
                        Rung->Element[x][y],
                        DrawingOption);
...
void DrawRungs(cairo_t *cr)
{
    ...
    for (ScanY = InfosGene->OffsetHiddenTopRungDisplayed * -1;
         ScanY < InfosGene->PageHeight && !TheEnd;
         ScanY = ScanY + (TOTAL_PX_RUNG_HEIGHT)) {
        char cInEdit = FALSE;
        ...
        // ghost zone
        if (cInEdit && EditDatas.GhostZonePosiX != -1 &&
            EditDatas.GhostZonePosiY != -1 && EditDatas.GhostZoneSizeX > 0 &&
            EditDatas.GhostZoneSizeY > 0)
