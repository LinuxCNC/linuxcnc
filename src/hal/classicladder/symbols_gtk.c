    ...
    for (ScanSymb = 0; ScanSymb < NBR_SYMBOLS; ScanSymb++) {
        if (SymbolArray[ScanSymb].VarName[0] == '\0') {
            ...
            gtk_list_store_set(ListStore,
                               ...
                               ScanSymb,
                               VAR_NAME,
                               SymbolArray[ScanSymb].VarName,
                               SYMBOL,
                               SymbolArray[ScanSymb].Symbol,
                               COMMENT,
                               Tempbuf,
                               -1);
            return;
        ...
        case 'W':
            snprintf(Tempbuf,
                     LGT_SYMBOL_COMMENT,
                     "%s",
                     ConvVarNameToHalSigName(SymbolArray[ScanSymb].VarName));
            break;
        default:
            snprintf(Tempbuf,
                     LGT_SYMBOL_COMMENT,
                     "%s",
                     SymbolArray[ScanSymb].Comment);
            break;
        }
...
void Callback_TextEdited(GtkCellRendererText *cell,
                         gchar *path_string,
                         gchar *new_text,
                         gpointer data)
{
    ...
    gtk_tree_model_get_iter(
        treemodel, &iter, gtk_tree_path_new_from_string(path_string));
    ...
    case VAR_NAME:
        if (new_text[0] == '\0') {
            ...
            pSymbol->Comment[0] = '\0';
            gtk_list_store_set(
                ListStore, &iter, VAR_NAME, "", SYMBOL, "", COMMENT, "", -1);
            InfosGene->AskConfirmationToQuit = TRUE;
        } else if (new_text[0] != '%') {
            ShowMessageBoxError(
                _("A variable name always start with '%' character !"));
        } else {
            if (TextParserForAVar(
                    new_text, NULL, NULL, NULL, TRUE /*PartialNames*/
                    )) {
                strncpy(pSymbol->VarName, new_text, LGT_VAR_NAME - 1);
                pSymbol->VarName[LGT_VAR_NAME - 1] = '\0';
                gtk_list_store_set(
                    ListStore, &iter, data, pSymbol->VarName, -1);
...
void SymbolsInitGtk()
{
    ...
    long ScanCol;
    char *ColName[] = {"HiddenColNbr!",
                       N_("Variable"),
                       N_("Symbol name"),
                       N_("HAL signal/Comment")};

    SymbolsWindow = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_title(GTK_WINDOW(SymbolsWindow), _("Symbols names"));
    gtk_signal_connect(GTK_OBJECT(SymbolsWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(SymbolsWindowDeleteEvent),
                       0);
    ...
    ListStore = gtk_list_store_new(
        NBR_INFOS, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
    ...
    for (ScanCol = 1; ScanCol < NBR_INFOS; ScanCol++) {
        ...
        g_signal_connect(G_OBJECT(renderer),
                         "edited",
                         G_CALLBACK(Callback_TextEdited),
                         (gpointer)ScanCol);
    ...
    gtk_scrolled_window_set_policy(
        GTK_SCROLLED_WINDOW(scrolled_win), GTK_POLICY_NEVER, GTK_POLICY_ALWAYS);
