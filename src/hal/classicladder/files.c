...
StrDatasForBase CorresDatasForBase[3] = {
    {BASE_MINS,  TIME_BASE_MINS,  "%.1fmn",   "Mins"    },
        ...
        do {
            ...
            if (LineOk) {
                ...
                case '#':
                    ...
                    break;
                default: ConvRawLineOfElements(Line, y, BufRung); y++;
                }
            }
        } while (LineOk);
        fclose(File);
    ...
    if (File) {
        ...
        for (y = 0; y < RUNG_HEIGHT; y++) {
            ...
            for (x = 0; x < RUNG_WIDTH; x++) {
                fprintf(File,
                        "%d-%d-%d/%d",
                        BufRung->Element[x][y].Type,
                        BufRung->Element[x][y].ConnectedWithTop,
                        BufRung->Element[x][y].VarType,
                        BufRung->Element[x][y].VarNum);
                if (BufRung->Element[x][y].IndexedVarType != -1)
                    fprintf(File,
                            "[%d/%d]",
                            BufRung->Element[x][y].IndexedVarType,
                            BufRung->Element[x][y].IndexedVarNum);
                if (x < RUNG_WIDTH - 1)
                    fprintf(File, " , ");
            }
            fprintf(File, E_LINE "\n");
...
void DumpRung(StrRung *TheRung)
{
    ...
    for (y = 0; y < RUNG_HEIGHT; y++) {
        for (x = 0; x < RUNG_WIDTH; x++) {
            printf("%d:%d:%d=%d , ",
                   TheRung->Element[x][y].Type,
                   TheRung->Element[x][y].ConnectedWithTop,
                   TheRung->Element[x][y].VarNum,
                   TheRung->Element[x][y].DynamicOutput);
        }
        printf("\n");
    }
}
...
{
    switch (NbrMilliSecs) {
    case TIME_BASE_MINS: return BASE_MINS;
    case TIME_BASE_SECS: return BASE_SECS;
    case TIME_BASE_100MS: return BASE_100MS;
    default:
...
char *ConvRawLineOfStringsOrNumbers(char *RawLine,
                                    int *LgtParams,
                                    char **ParamsStringsFnd,
                                    int **ParamsIntFnd)
{
    char *StartOfValue;
    char *EndOfValue;
        ...
        do {
            ...
            if (LineOk) {
                if (Line[0] != ';') {
                    ...
                    case BASE_100MS:
                        BufTimers->Base =
                            CorresDatasForBase[Params[0]].ValueInMS;
                        BufTimers->Preset = Params[1] * BufTimers->Base;
                        rtapi_strxcpy(
                            BufTimers->DisplayFormat,
                            CorresDatasForBase[Params[0]].DisplayFormat);
                        break;
                    default:
                        BufTimers->Base = 1;
                        BufTimers->Preset = 10;
    ...
    if (File) {
        ...
        do {
            fprintf(File,
                    ...
                    BufTimers->Preset / BufTimers->Base);
            BufTimers++;
            NumTimer++;
            ...
            if (LineOk) {
                if (Line[0] != ';') {
                    ...
                    case BASE_100MS:
                        BufMonostables->Base =
                            CorresDatasForBase[Params[0]].ValueInMS;
                        BufMonostables->Preset =
                            Params[1] * BufMonostables->Base;
                        rtapi_strxcpy(
                            BufMonostables->DisplayFormat,
                            CorresDatasForBase[Params[0]].DisplayFormat);
                        break;
                    default:
...
char SaveMonostablesParams(char *FileName, StrMonostable *BufMonostables)
{
    ...
    if (File) {
        ...
        do {
            fprintf(File,
                    S_LINE "%d,%d" E_LINE "\n",
                    ConvBaseInMilliSecsToId(BufMonostables->Base),
                    BufMonostables->Preset / BufMonostables->Base);
            BufMonostables++;
            NumMonostable++;
...
char SaveNewTimersParams(char *FileName)
{
    ...
    if (File) {
        ...
        do {
            ...
            fprintf(File,
                    S_LINE "%d,%d,%d" E_LINE "\n",
                    ConvBaseInMilliSecsToId(TimerIEC->Base),
                    TimerIEC->Preset,
                    TimerIEC->TimerMode);
            NumTimerIEC++;
            ...
            if (LineOk) {
                ...
                case '#':
                    if (strncmp(&Line[1], "VER=", 4) == 0) {
                        if (atoi(&Line[5]) > 1) {
                            printf(
                                _("Sections file version not supported...\n"));
                            LineOk = FALSE;
                        }
                    }
                    // #NAMExxx=....
                ...
                default:
                    ...
                    dbg_printf(
                        _("Section %d => Name=%s, Language=%d, SRnbr=%d, "
                          "FirstRung=%d, LastRung=%d, SequentialPage=%d\n"),
                        NumSection,
                        pSection->Name,
                        pSection->Language,
                        pSection->SubRoutineNumber,
                        pSection->FirstRung,
                        pSection->LastRung,
                        pSection->SequentialPage);
                    break;
                }
            }
        } while (LineOk);
    ...
    if (File) {
        ...
        for (Pass = 0; Pass < 2; Pass++) {
            ...
            for (NumConf = 0; NumConf < NbrConf; NumConf++) {
                pConf = (Pass == 0) ? &InfosGene->InputsConf[NumConf]
                                    : &InfosGene->OutputsConf[NumConf];
                /* valid mapping ? */
                if (pConf->FirstClassicLadderIO != -1) {
                    fprintf(File,
                            S_LINE "%d,%d,%d,%d,%d,%d,%d" E_LINE "\n",
                            Pass,
            ...
            if (LineOk) {
                if (Line[0] != ';' && Line[0] != '#') {
                    ...
                    if (strchr(Line, '.')) {
                        ...
                        snprintf(pConf->SlaveAdr,
                                 sizeof(pConf->SlaveAdr),
                                 "%d",
                                 IntDatas[0]);
                    }
                    if (LineOk) {
            ...
            if (LineOk) {
                ...
                case '#':
                    if (strncmp(&Line[1], "VER=", 4) == 0) {
                        if (atoi(&Line[5]) > 1) {
                            printf(
                                _("Symbols file version not supported...\n"));
                            LineOk = FALSE;
                        }
                    }
                    break;
                default:
                    ...
                    PtrStrings[3] = NULL;
                    LgtMaxStrings[3] = 0;
                    ConvRawLineOfStringsOrNumbers(
                        Line, LgtMaxStrings, PtrStrings, NULL);
...
char SaveGeneralParameters(char *FileName)
{
    ...
    if (File) {
        fprintf(File,
                S_LINE "PERIODIC_REFRESH=%d" E_LINE "\n",
                GeneralParamsMirror.PeriodicRefreshMilliSecs);
        fprintf(File,
                S_LINE "SIZE_NBR_RUNGS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_rungs);
        fprintf(File,
                S_LINE "SIZE_NBR_BITS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_bits);
        fprintf(File,
                S_LINE "SIZE_NBR_WORDS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_words);
#ifdef OLD_TIMERS_MONOS_SUPPORT
        fprintf(File,
                S_LINE "SIZE_NBR_TIMERS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_timers);
        fprintf(File,
                S_LINE "SIZE_NBR_MONOSTABLES=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_monostables);
#endif
        fprintf(File,
                S_LINE "SIZE_NBR_COUNTERS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_counters);
        fprintf(File,
                S_LINE "SIZE_NBR_TIMERS_IEC=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_timers_iec);
        fprintf(File,
                S_LINE "SIZE_NBR_PHYS_INPUTS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_phys_inputs);
        fprintf(File,
                S_LINE "SIZE_NBR_PHYS_OUTPUTS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_phys_outputs);
        fprintf(File,
                S_LINE "SIZE_NBR_ARITHM_EXPR=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_arithm_expr);
        fprintf(File,
                S_LINE "SIZE_NBR_SECTIONS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_sections);
        fprintf(File,
                S_LINE "SIZE_NBR_SYMBOLS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_symbols);
        fprintf(File,
                S_LINE "SIZE_NBR_PHYS_WORDS_INPUTS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_phys_words_inputs);
        fprintf(File,
                S_LINE "SIZE_NBR_PHYS_WORDS_OUTPUTS=%d" E_LINE "\n",
                GeneralParamsMirror.SizesInfos.nbr_phys_words_outputs);
        fclose(File);
        Okay = TRUE;
    }
...
char SaveComParameters(char *FileName)
{
    ...
    if (File) {
        fprintf(File,
                S_LINE "MODBUS_MASTER_SERIAL_PORT=%s" E_LINE "\n",
                ModbusConfig.ModbusSerialPortNameUsed);
        fprintf(File,
                S_LINE "MODBUS_MASTER_SERIAL_SPEED=%d" E_LINE "\n",
                ModbusConfig.ModbusSerialSpeed);
        fprintf(File,
                S_LINE "MODBUS_MASTER_SERIAL_DATABITS=%d" E_LINE "\n",
                ModbusConfig.ModbusSerialDataBits);
        fprintf(File,
                S_LINE "MODBUS_MASTER_SERIAL_STOPBITS=%d" E_LINE "\n",
                ModbusConfig.ModbusSerialStopBits);
        fprintf(File,
                S_LINE "MODBUS_MASTER_SERIAL_PARITY=%d" E_LINE "\n",
                ModbusConfig.ModbusSerialParity);
        fprintf(File,
                S_LINE "MODBUS_ELEMENT_OFFSET=%d" E_LINE "\n",
                ModbusConfig.ModbusEleOffset);
        fprintf(File,
                S_LINE "MODBUS_MASTER_SERIAL_USE_RTS_TO_SEND=%d" E_LINE "\n",
                ModbusConfig.ModbusSerialUseRtsToSend);
        fprintf(File,
                S_LINE "MODBUS_MASTER_TIME_INTER_FRAME=%d" E_LINE "\n",
                ModbusConfig.ModbusTimeInterFrame);
        fprintf(File,
                S_LINE "MODBUS_MASTER_TIME_OUT_RECEIPT=%d" E_LINE "\n",
                ModbusConfig.ModbusTimeOutReceipt);
        fprintf(File,
                S_LINE "MODBUS_MASTER_TIME_AFTER_TRANSMIT=%d" E_LINE "\n",
                ModbusConfig.ModbusTimeAfterTransmit);
        fprintf(File,
                S_LINE "MODBUS_DEBUG_LEVEL=%d" E_LINE "\n",
                ModbusConfig.ModbusDebugLevel);
        fprintf(File,
                S_LINE "MODBUS_MAP_COIL_READ=%d" E_LINE "\n",
                ModbusConfig.MapTypeForReadCoils);
        fprintf(File,
                S_LINE "MODBUS_MAP_COIL_WRITE=%d" E_LINE "\n",
                ModbusConfig.MapTypeForWriteCoils);
        fprintf(File,
                S_LINE "MODBUS_MAP_INPUT=%d" E_LINE "\n",
                ModbusConfig.MapTypeForReadInputs);
        fprintf(File,
                S_LINE "MODBUS_MAP_HOLDING=%d" E_LINE "\n",
                ModbusConfig.MapTypeForReadInputRegs);
        fprintf(File,
                S_LINE "MODBUS_MAP_REGISTER_READ=%d" E_LINE "\n",
                ModbusConfig.MapTypeForReadHoldRegs);
        fprintf(File,
                S_LINE "MODBUS_MAP_REGISTER_WRITE=%d" E_LINE "\n",
                ModbusConfig.MapTypeForWriteHoldRegs);
        fclose(File);
        Okay = TRUE;
    }
...
void LoadAllLadderDatas(char *DatasDirectory)
{
    ...
    snprintf(FileName,
             sizeof(FileName),
             "%s/" FILE_PREFIX "counters.csv",
             DatasDirectory);
    //	printf("Loading counters data from %s\n",FileName);
    LoadCountersParams(FileName);
    snprintf(FileName,
             sizeof(FileName),
             "%s/" FILE_PREFIX "timers_iec.csv",
             DatasDirectory);
    LoadNewTimersParams(FileName);

    snprintf(FileName,
             sizeof(FileName),
             "%s/" FILE_PREFIX "arithmetic_expressions.csv",
             DatasDirectory);
    ...
    snprintf(FileName,
             sizeof(FileName),
             "%s/" FILE_PREFIX "modbusioconf.csv",
             DatasDirectory);
...
void SaveAllLadderDatas(char *DatasDirectory)
{
    ...
    snprintf(
        FileName, sizeof(FileName), "%s/" FILE_PREFIX "rung_", DatasDirectory);
            ...
            while ((pEnt = readdir(pDir)) != NULL) {
                if (strcmp(pEnt->d_name, ".") && strcmp(pEnt->d_name, "..")) {
                    ...
                    if (strlen(FILE_PREFIX) > 0) {
                        if (strncmp(pEnt->d_name,
                                    FILE_PREFIX,
                                    strlen(FILE_PREFIX)) != 0)
                            cRemoveIt = FALSE;
                    }
                    if (cRemoveIt) {
                        size_t ret = snprintf(Buff,
                                              sizeof(Buff),
                                              "%s/%s",
                                              TmpDirectory,
                                              pEnt->d_name);
