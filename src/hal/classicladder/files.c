...
char LoadRung(char *FileName, StrRung * BufRung)
{
	...
	do {
		...
		case '#':
			...
			if (strlen(BufRung->Label) > 0
			    && BufRung->Label[strlen(BufRung->Label) - 1] ==
			    '\n')
			...
			if (strlen(BufRung->Comment) > 0
			    && BufRung->Comment[strlen(BufRung->Comment) -
				1] == '\n')
			    BufRung->Comment[strlen(BufRung->Comment) - 1] =
				'\0';
			if (strlen(BufRung->Comment) > 0
			    && BufRung->Comment[strlen(BufRung->Comment) -
				1] == '\r')
...
char SaveRung(char *FileName, StrRung * BufRung)
{
	...
	for (y = 0; y < RUNG_HEIGHT; y++) {
		...
		fprintf(File, "%d-%d-%d/%d", BufRung->Element[x][y].Type,
...
void DumpRung(StrRung * TheRung)
{
    ...
    for (y = 0; y < RUNG_HEIGHT; y++) {
	for (x = 0; x < RUNG_WIDTH; x++) {
	    printf("%d:%d:%d=%d , ", TheRung->Element[x][y].Type,
...
char LoadTimersParams(char *FileName, StrTimer * BufTimers)
{
	...
	do {
		...
		if (Line[0] != ';') {
			...
			strcpy(BufTimers->DisplayFormat,
...
char LoadMonostablesParams(char *FileName, StrMonostable * BufMonostables)
{
	...
	do {
		...
		if (Line[0] != ';') {
			...
			strcpy(BufMonostables->DisplayFormat,
			    CorresDatasForBase[Params[0]].DisplayFormat);
			break;
		    default:
...
char LoadSectionsParams(char *FileName)
{
	...
	do {
		...
		case '#':
			...
			if (atoi(&Line[5]) > 1) {
			    printf
				("Sections file version not supported...\n");
			    LineOk = FALSE;
			}
		    }
		    // #NAMExxx=....
		...
		default:
		    ...
		    dbg_printf
			("Section %d => Name=%s, Language=%d, SRnbr=%d, FirstRung=%d, LastRung=%d, SequentialPage=%d\n",
			NumSection, pSection->Name, pSection->Language,
			pSection->SubRoutineNumber, pSection->FirstRung,
			pSection->LastRung, pSection->SequentialPage);
		    break;
		}
	    }
	}
...
char SaveSectionsParams(char *FileName)
{
	...
	do {
		...
		fprintf(File, "%03d,%d,%d,%d,%d,%d\n", NumSection,
		    pSection->Language, pSection->SubRoutineNumber,
		    pSection->FirstRung, pSection->LastRung,
...
char SplitFiles(char *DirAndNameOfProject, char *TmpDirectoryFiles)
{
	...
	if (strncmp(Buff, "_FILES_CLASSICLADDER",
		strlen("_FILES_CLASSICLADDER")) == 0) {
		...
		if (!feof(pProjectFile)) {
			...
			sprintf(ParametersFile, "%s/%s", TmpDirectoryFiles,
			    &Buff[STR_LEN_FILE_HEAD]);
			ParametersFile[strlen(ParametersFile) - 1] = '\0';
			if (ParametersFile[strlen(ParametersFile) - 1] ==
			    '\r')
			    ParametersFile[strlen(ParametersFile) - 1] = '\0';
		    } else {
			/* not end line of project ? */
			if ((strncmp(Buff, "_/FILES_CLASSICLADDER",
				    strlen("_/FILES_CLASSICLADDER")) != 0)
			    && Buff[0] != '\n') {
	...
	if (strcmp(&NewDir[strlen(NewDir) - 4], ".clp") != 0) {
		...
		if (Lgt > 0) {
		    *End = '\0';
		    ...
		    printf
			("ERROR whith path directory given for project !!!\n");
		    Directory[0] = '\0';
		    return (0);
		}
	    } else {
