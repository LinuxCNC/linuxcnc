...
/* return TRUE if okay */
char TextParserForAVar(char *text,
                       int *VarTypeFound,
                       int *VarOffsetFound,
                       int *pNumberOfChars,
                       char PartialNames)
{
    ...
    if (IsOk) {
        ...
        case 'B':
            ...
            (*pPtrScanPos)++;
            if (!TextToNumberAndAdvance(
                    pPtrScanPos, 0, NBR_BITS - 1, &OffsetFound)) {
                IsOk = FALSE;
                ErrorMessageVarParser =
                    _("Unknown variable (number value out of bound)");
            }
            break;
            ...
            if (!TextToNumberAndAdvance(pPtrScanPos, 0, MaxNbr, &OffsetFound)) {
                ...
                if (PartialNames == FALSE) {
                    if (**pPtrScanPos != VAR_ATTRIBUTE_SEP) {
                        IsOk = FALSE;
                        ErrorMessageVarParser =
                            _("Unknown variable (missing '.' character before "
                              "attribute)");
                    } else {
                        (*pPtrScanPos)++;
                        switch (**pPtrScanPos) {
                        case 'D':
                            if (TimerIEC == FALSE) {
                                ...
                                IsOk = FALSE;
                                ErrorMessageVarParser =
                                    _("Unknown variable (unknown attribute)");
                            }
                            break;
                        case 'Q':
                            if (TimerIEC == TRUE) {
                        ...
                        case 'R':
                            ...
                            break;
                        case 'P':
                            TypeFound = (TimerIEC == FALSE)
                                            ? VAR_TIMER_PRESET
                                            : VAR_TIMER_IEC_PRESET;
                            break;
                        case 'V':
                            TypeFound = (TimerIEC == FALSE)
                                            ? VAR_TIMER_VALUE
        ...
        case 'M':
            (*pPtrScanPos)++;
            if (!TextToNumberAndAdvance(
                    pPtrScanPos, 0, NBR_MONOSTABLES - 1, &OffsetFound)) {
                IsOk = FALSE;
                ErrorMessageVarParser =
                    _("Unknown variable (number value out of bound)");
            } else {
                if (PartialNames == FALSE) {
                    if (**pPtrScanPos != VAR_ATTRIBUTE_SEP) {
                        ...
                        (*pPtrScanPos)++;
                        switch (**pPtrScanPos) {
                        case 'R': TypeFound = VAR_MONOSTABLE_RUNNING; break;
                        case 'P': TypeFound = VAR_MONOSTABLE_PRESET; break;
                        case 'V': TypeFound = VAR_MONOSTABLE_VALUE; break;
                        default:
                            IsOk = FALSE;
                            ErrorMessageVarParser =
                                _("Unknown variable (unknown attribute)");
        ...
        case 'C':
            (*pPtrScanPos)++;
            if (!TextToNumberAndAdvance(
                    pPtrScanPos, 0, NBR_COUNTERS - 1, &OffsetFound)) {
                IsOk = FALSE;
                ErrorMessageVarParser =
                    _("Unknown variable (number value out of bound)");
            } else {
                if (PartialNames == FALSE) {
                    if (**pPtrScanPos != VAR_ATTRIBUTE_SEP) {
                        ...
                        (*pPtrScanPos)++;
                        switch (**pPtrScanPos) {
                        case 'D': TypeFound = VAR_COUNTER_DONE; break;
                        case 'E': TypeFound = VAR_COUNTER_EMPTY; break;
                        case 'F': TypeFound = VAR_COUNTER_FULL; break;
                        case 'P': TypeFound = VAR_COUNTER_PRESET; break;
                        case 'V': TypeFound = VAR_COUNTER_VALUE; break;
                        default:
                            IsOk = FALSE;
                            ErrorMessageVarParser =
                                _("Unknown variable (unknown attribute)");
        ...
        case 'I':
            ...
            (*pPtrScanPos)++;
            if (!TextToNumberAndAdvance(
                    pPtrScanPos, 0, NBR_PHYS_INPUTS - 1, &OffsetFound)) {
                IsOk = FALSE;
                ErrorMessageVarParser =
                    _("Unknown variable (number value out of bound)");
            }
        ...
        case 'Q':
            ...
            (*pPtrScanPos)++;
            if (!TextToNumberAndAdvance(
                    pPtrScanPos, 0, NBR_PHYS_OUTPUTS - 1, &OffsetFound)) {
                IsOk = FALSE;
                ErrorMessageVarParser =
                    _("Unknown variable (number value out of bound)");
            }
        ...
        case 'W':
            ...
            (*pPtrScanPos)++;
            if (!TextToNumberAndAdvance(
                    pPtrScanPos, 0, NBR_WORDS - 1, &OffsetFound)) {
                IsOk = FALSE;
                ErrorMessageVarParser =
                    _("Unknown variable (number value out of bound)");
            }
        ...
        case 'X':
            TypeFound =
                VAR_STEP_ACTIVITY; //per default, but it could change later...
            (*pPtrScanPos)++;
            if (!TextToNumberAndAdvance(
                    pPtrScanPos, 0, NBR_STEPS - 1, &OffsetFound)) {
                IsOk = FALSE;
                ErrorMessageVarParser =
                    _("Unknown variable (number value out of bound)");
            } else {
        ...
        default:
            IsOk = FALSE;
            ErrorMessageVarParser =
                _("Unknown variable (on first character following %)");
        }
        if (IsOk) {
            if (VarTypeFound)
            ...
            if (pNumberOfChars != NULL)
                *pNumberOfChars = (VarNameFromSymbol == NULL)
                                      ? *pPtrScanPos - text
                                      : *pPtrScanPos - VarNameFromSymbol;
        } else {
...
int ComparStaticPart(char *pSearchedString, char *pComparString)
{
    ...
    do {
        iNbrCaracs++;
    } while ((pComparString[iNbrCaracs] != '%') &&
             (pComparString[iNbrCaracs] != '\0'));
...
char TextParserForAVar(char *TextToParse,
                       int *VarTypeFound,
                       int *VarOffsetFound,
                       int *pNumberOfChars,
                       char PartialNames)
    ...
    do {
        ...
        do {
            ...
            if (pConv->StringBaseVarName[iScruteCarac] == '%') {
                ...
                iTaillePartieStatique = ComparStaticPart(
                    pBalayVarCherch, &pConv->StringBaseVarName[iScruteCarac]);
    ...
    if (bFound) {
        ...
        if (bVerifCoherenceOk) {
            ...
            if (pConv->iSize3 <= 0 && pConv->iSize2 <= 0) {
                ...
                if (pConv->iSize3 <= 0)
                    iIdVar = pConv->iIdVar +
                             (iOffset1 - pConv->iFirstVal1) * pConv->iSize2 +
                             iOffset2 - pConv->iFirstVal2;
                else
                    iIdVar = pConv->iIdVar +
                             (iOffset1 - pConv->iFirstVal1) * pConv->iSize2 *
                                 pConv->iSize3 +
                             (iOffset2 - pConv->iFirstVal2) * pConv->iSize3 +
                             iOffset3 - pConv->iFirstVal3;
            }
...
StrConvIdVarName *ConvIdVarEnPtrSurEleConv(int iTypeVarToSearch,
                                           int iIdVarCherchee)
{
    int iBalayTable = 0;
    char bFound = FALSE;
    ...
    do {
...
#endif
        {
            ...
            iSizeTotal = (pConv->iSize2 <= 0) ? pConv->iSize1
                                              : pConv->iSize1 * pConv->iSize2;
            if (pConv->iSize3 > 0)
                iSizeTotal *= pConv->iSize3;
            /* Offset value between start and end values ? */
            if ((iIdFound <= iIdVarCherchee) &&
                (iIdVarCherchee < (iIdFound + iSizeTotal)))
                bFound = TRUE;
            else
                iBalayTable++;
        }
...
int GetSizeVarsForTypeVar(int iTypeVarToSearch)
{
    ...
    do {
        ...
        // same type ?
        if (pConv->iTypeVar == iTypeVarToSearch) {
            iSize = (pConv->iSize2 <= 0) ? pConv->iSize1
                                         : pConv->iSize1 * pConv->iSize2;
            if (pConv->iSize3 > 0)
                iSize *= pConv->iSize3;
        } else {
...
char *CreateVarName(int TypeVarSearched,
                    int OffsetVarSearched,
                    char SymbolNameIfAvail)
{
    ...
    if (pConv) {
        ...
        if (pConv->iSize2 <= 0 && pConv->iSize3 <= 0) {
            snprintf(tcBuffer2,
                     sizeof(tcBuffer2),
                     pConv->StringBaseVarName,
                     OffsetVarSearched - iIdFound + pConv->iFirstVal1);
        } else {
            if (pConv->iSize3 <= 0)
                snprintf(tcBuffer2,
                         sizeof(tcBuffer2),
                         pConv->StringBaseVarName,
                         (OffsetVarSearched - iIdFound) / pConv->iSize2 +
                             pConv->iFirstVal1,
                         (OffsetVarSearched - iIdFound) % pConv->iSize2 +
                             pConv->iFirstVal2);
            else
                snprintf(tcBuffer2,
                         sizeof(tcBuffer2),
                         pConv->StringBaseVarName,
                         (OffsetVarSearched - iIdFound) /
                                 (pConv->iSize2 * pConv->iSize3) +
                             pConv->iFirstVal1,
                         ((OffsetVarSearched - iIdFound) / pConv->iSize3) %
                                 pConv->iSize2 +
                             pConv->iFirstVal2,
                         (OffsetVarSearched - iIdFound) % pConv->iSize3 +
                             pConv->iFirstVal3);
