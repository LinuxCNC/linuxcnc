    ...
    if (ConvertDoublesToRungCoor(x, y, &CurrentX, &CurrentY)) {
        if (StartToClick) {
            ...
            printf("start selection %d,%d (%d,%d)\n",
                   EditDatas.GhostZonePosiX,
                   EditDatas.GhostZonePosiY,
            ...
            if (UnderSelection) {
                if (CurrentX >= EditDatas.GhostZonePosiX)
                    EditDatas.GhostZoneSizeX =
                        CurrentX - EditDatas.GhostZonePosiX + 1;
                if (CurrentY >= EditDatas.GhostZonePosiY)
                    EditDatas.GhostZoneSizeY =
                        CurrentY - EditDatas.GhostZonePosiY + 1;
...
void CopyFunctionBlockPropertiesOrExpr(int NumTypeEle,
                                       int NumBlockSrc,
                                       int NumBlockDest)
{
    switch (NumTypeEle) {
    ...
    case ELE_MONOSTABLE:
        ...
        MonostableArray[NumBlockDest].Preset =
            MonostableArray[NumBlockSrc].Preset;
        break;
#endif
    case ELE_COUNTER:
    ...
    case ELE_TIMER_IEC:
        ...
        NewTimerArray[NumBlockDest].TimerMode =
            NewTimerArray[NumBlockSrc].TimerMode;
        break;
    case ELE_COMPAR:
    case ELE_OUTPUT_OPERATE:
        rtapi_strxcpy(EditArithmExpr[NumBlockDest].Expr,
                      EditArithmExpr[NumBlockSrc].Expr);
        break;
    }
}
...
void CopyNowPartSelected(double x, double y)
{
    ...
    if (EditDatas.GhostZonePosiX != -1 && EditDatas.GhostZonePosiY != -1) {
        ...
        printf("copying...\n");
        for (srcy = PartSelectionPosiY;
             srcy < PartSelectionPosiY + PartSelectionSizeY;
             srcy++) {
            x = EditDatas.GhostZonePosiX;
            for (srcx = PartSelectionPosiX;
                 srcx < PartSelectionPosiX + PartSelectionSizeX;
                 srcx++) {
                int TypeSrc = RungSrc->Element[srcx][srcy].Type;
                EditDatas.Rung.Element[x][y].VarNum =
                    -1; //to detect if it has been modified (alloc of a complex...)
                ...
                if (PrepBeforeSettingTypeEleForComplexBlocsAndExpr(
                    ...
                    EditDatas.Rung.Element[x][y].ConnectedWithTop =
                        RungSrc->Element[srcx][srcy].ConnectedWithTop;
                    EditDatas.Rung.Element[x][y].VarType =
                        RungSrc->Element[srcx][srcy].VarType;
                    if (EditDatas.Rung.Element[x][y].VarNum == -1)
                        EditDatas.Rung.Element[x][y].VarNum =
                            RungSrc->Element[srcx][srcy].VarNum;
                    else
                        CopyFunctionBlockPropertiesOrExpr(
                            TypeSrc,
                            RungSrc->Element[srcx][srcy].VarNum,
                            EditDatas.Rung.Element[x][y].VarNum);
                    EditDatas.Rung.Element[x][y].IndexedVarType =
                        RungSrc->Element[srcx][srcy].IndexedVarType;
                    EditDatas.Rung.Element[x][y].IndexedVarNum =
                        RungSrc->Element[srcx][srcy].IndexedVarNum;
                }
                x++;
            }
            y++;
