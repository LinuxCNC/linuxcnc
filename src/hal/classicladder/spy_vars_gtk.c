...
void RefreshOneBoolVar(int Type, int Num, int Val)
{
    ...
    if (Num >= ValOffsetBoolVar[Col] &&
        ...
        gtk_toggle_button_set_active(
            (GtkToggleButton *)chkvar[Col][Num - ValOffsetBoolVar[Col]],
            (Val != 0) ? TRUE : FALSE);
}
...
void RefreshAllBoolsVars()
{
    ...
    for (NumVar = 0; NumVar < NBR_BOOLS_VAR_SPY; NumVar++) {
        gtk_toggle_button_set_active(
            (GtkToggleButton *)chkvar[0][NumVar],
            ReadVar(VAR_MEM_BIT, NumVar + ValOffsetBoolVar[0]) ? TRUE : FALSE);
        gtk_toggle_button_set_active(
            (GtkToggleButton *)chkvar[1][NumVar],
            ReadVar(VAR_PHYS_INPUT, NumVar + ValOffsetBoolVar[1]) ? TRUE
                                                                  : FALSE);
        gtk_toggle_button_set_active(
            (GtkToggleButton *)chkvar[2][NumVar],
            ReadVar(VAR_PHYS_OUTPUT, NumVar + ValOffsetBoolVar[2]) ? TRUE
                                                                   : FALSE);
    }
}
        ...
        for (OffVar = 0; OffVar < NBR_BOOLS_VAR_SPY; OffVar++) {
            ...
            case 0:
                snprintf(
                    BufNumVar,
                    256,
                    "<span foreground=\"black\" weight=\"bold\">%cB%d</span>",
                    '%',
            ...
            case 1:
                snprintf(
                    BufNumVar,
                    256,
                    "<span foreground=\"red\" weight=\"bold\">%cI%d</span>",
                    '%',
                    OffVar + ValOffsetBoolVar[ColumnVar]);
                break;
            case 2:
                snprintf(
                    BufNumVar,
                    256,
                    "<span foreground=\"blue\" weight=\"bold\">%cQ%d</span>",
                    '%',
                    OffVar + ValOffsetBoolVar[ColumnVar]);
                break;
            }
            gtk_label_set_markup(GTK_LABEL(gtk_bin_get_child(
                                     GTK_BIN(chkvar[ColumnVar][OffVar]))),
                                 BufNumVar);
        }
    }
}
...
gint BoolVarsWindowDeleteEvent(GtkWidget *widget,
                               GdkEvent *event,
                               gpointer data)
{
    (void)widget;
...
void BoolVarsWindowInitGtk()
{
    ...
    gtk_window_set_title((GtkWindow *)SpyBoolVarsWindow,
                         _("Bit Status Window"));
    ...
    for (ColumnVar = 0; ColumnVar < NBR_TYPE_BOOLS_SPY; ColumnVar++) {
        ...
        gtk_widget_set_tooltip_text(
            ...
            _("Offset for vars displayed below (press return to apply)"));
        ...
        gtk_signal_connect(GTK_OBJECT(offsetboolvar[ColumnVar]),
                           "activate",
                           GTK_SIGNAL_FUNC(OffsetBoolVar_activate_event),
                           (void *)ColumnVar);
        ...
        for (OffVar = 0; OffVar < NBR_BOOLS_VAR_SPY; OffVar++) {
            ...
            gtk_signal_connect(GTK_OBJECT(chkvar[ColumnVar][OffVar]),
                               "toggled",
                               GTK_SIGNAL_FUNC(chkvar_press_event),
                               (void *)NumCheckWidget);
            NumCheckWidget++;
        }
    }
    ...
    gtk_signal_connect(GTK_OBJECT(SpyBoolVarsWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(BoolVarsWindowDeleteEvent),
                       0);
...
static gint OpenModifyVarWindow_clicked_event(GtkWidget *widget, int NumSpy)
{
    ...
    gtk_entry_set_text(GTK_ENTRY(ModifyVariableNameEdit),
                       CreateVarName(CurrentModifyVarType,
                                     CurrentModifyVarOffset,
                                     InfosGene->DisplaySymbols));
    snprintf(BuffValue,
             sizeof(BuffValue),
             "%d",
             ReadVar(CurrentModifyVarType, CurrentModifyVarOffset));
    ...
    if (SaveModifyVarPosX != -1 && SaveModifyVarPosY != -1)
        gtk_window_move(GTK_WINDOW(ModifyVarValueWindow),
                        SaveModifyVarPosX,
                        SaveModifyVarPosY);

    return TRUE;
}
gint ModifyVarWindowDeleteEvent(GtkWidget *widget,
                                GdkEvent *event,
                                gpointer data)
{
    (void)widget;
    (void)event;
    (void)data;
    gtk_window_get_position(GTK_WINDOW(ModifyVarValueWindow),
                            &SaveModifyVarPosX,
                            &SaveModifyVarPosY);
...
void ModifyVarWindowInitGtk()
{
    ...
    gtk_window_set_title((GtkWindow *)ModifyVarValueWindow,
    ...
    gtk_box_pack_start(
        GTK_BOX(vboxMain), ModifyVariableNameEdit, TRUE, TRUE, 0);
    ...
    //	gtk_widget_set_usize( GTK_WIDGET(ModifyVariableValueEdit),110,0);
    gtk_box_pack_start(
        GTK_BOX(vboxMain), ModifyVariableValueEdit, TRUE, TRUE, 0);
    gtk_widget_show(ModifyVariableValueEdit);
    gtk_signal_connect(GTK_OBJECT(ModifyVariableValueEdit),
                       "activate",
                       GTK_SIGNAL_FUNC(ApplyModifiedVar),
                       (void *)NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonCancel),
                       "clicked",
                       GTK_SIGNAL_FUNC(ModifyVarWindowDeleteEvent),
                       NULL);

    gtk_signal_connect(GTK_OBJECT(ModifyVarValueWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(ModifyVarWindowDeleteEvent),
                       0);
}
...
void DisplayFreeVarSpy()
{
    ...
    for (NumVarSpy = 0; NumVarSpy < i; NumVarSpy++) {
        ...
        rtapi_strxcpy(DisplayFormat,
                      gtk_combo_box_get_active_text(
                          GTK_COMBO_BOX_TEXT(DisplayFormatVarSpy[NumVarSpy])));
        ...
        gtk_entry_set_text(
            (GtkEntry *)EntryVarSpy[2 * NBR_FREE_VAR_SPY + NumVarSpy],
            BufferValue);
        VarName =
            "<span foreground=\"gray\" weight=\"bold\" >Other       </span>";
        ...
        case VAR_TIMER_DONE:
            VarName = "<span foreground=\"brown\" weight=\"bold\" >Timer       "
                      "</span>";
            break;
        case VAR_TIMER_IEC_VALUE:
        case VAR_TIMER_IEC_PRESET:
        case VAR_TIMER_IEC_DONE:
            VarName = "<span foreground=\"brown\" weight=\"bold\" >IEC Timer   "
                      "</span>";
            break;
        ...
        case VAR_COUNTER_DONE:
            VarName = "<span foreground=\"brown\" weight=\"bold\" >Counter     "
                      "</span>";
            break;
        case VAR_MONOSTABLE_RUNNING:
        case VAR_MONOSTABLE_PRESET:
        case VAR_MONOSTABLE_VALUE:
            VarName = "<span foreground=\"brown\" weight=\"bold\" >Monostable  "
                      "</span>";
            break;
        case VAR_MEM_WORD:
            VarName = "<span foreground=\"black\" weight=\"bold\" >Memory      "
                      "</span>";
            break;
        case VAR_PHYS_INPUT:
            VarName =
                "<span foreground=\"red\" weight=\"bold\" >Bit In Pin  </span>";
            break;
        case VAR_PHYS_OUTPUT:
            VarName = "<span foreground=\"blue\" weight=\"bold\" >Bit Out Pin "
                      "</span>";
            break;
        case VAR_PHYS_FLOAT_INPUT:
            VarName =
                "<span foreground=\"red\" weight=\"bold\" >Floatin Pin </span>";
            break;
        case VAR_PHYS_FLOAT_OUTPUT:
            VarName = "<span foreground=\"blue\" weight=\"bold\" >Floatout "
                      "Pin</span>";
            break;
        case VAR_PHYS_WORD_INPUT:
            VarName =
                "<span foreground=\"red\" weight=\"bold\" >S32in Pin   </span>";
            break;
        case VAR_PHYS_WORD_OUTPUT:
            VarName = "<span foreground=\"blue\" weight=\"bold\" >S32out Pin  "
                      "</span>";
            break;
        case VAR_MEM_BIT:
            VarName = "<span foreground=\"black\" weight=\"bold\" >Bit Memory  "
                      "</span>";
            break;
        case VAR_ERROR_BIT:
            VarName = "<span foreground=\"gold\" weight=\"bold\" >Error Bit   "
                      "</span>";
            break;
        case VAR_STEP_ACTIVITY:
            VarName = "<span foreground=\"brown\" weight=\"bold\" >Step Active "
                      " </span>";
            break;
        case VAR_STEP_TIME:
            VarName = "<span foreground=\"brown\" weight=\"bold\" >Step Run "
                      "Time</span>";
            break;
        }
        ...
        if (InfosGene->DisplaySymbols != LastTime) {
            gtk_entry_set_text(
                (GtkEntry *)EntryVarSpy[NumVarSpy + (1 * NBR_FREE_VAR_SPY)],
                CreateVarName(VarSpy[NumVarSpy][0],
                              VarSpy[NumVarSpy][1],
                              InfosGene->DisplaySymbols));
        }
    }
...
static gint EntryVarSpy_activate_event(GtkWidget *widget, int NumSpy)
{
    ...
    if (TextParserForAVar(
            BufferVar, &NewVarType, &NewVarOffset, NULL, FALSE /*PartialNames*/
            )) {
        ...
        *NumVarSpy = NewVarOffset;
        ...
        if (OtherVarName)
            ...
            gtk_widget_set_sensitive(
                GTK_WIDGET(ModifyVarSpy[NumSpy]),
                TestVarIsReadWrite(NewVarType, NewVarOffset));
    } else {
        ...
        gtk_entry_set_text(
            (GtkEntry *)widget,
            CreateVarName(OldType, OldOffset, InfosGene->DisplaySymbols));
    }
    return TRUE;
}

gint FreeVarsWindowDeleteEvent(GtkWidget *widget,
                               GdkEvent *event,
                               gpointer data)
{
    (void)widget;
...
void FreeVarsWindowInitGtk()
{
    ...
    for (NumVarSpy = 0; NumVarSpy < i; NumVarSpy++) {
        ...
        for (ColumnVar = 0; ColumnVar < 3; ColumnVar++) {
            ...
            if (ColumnVar == 0) {
                ...
                gtk_widget_set_size_request(
                    (GtkWidget *)LabelFreeVars[NumEntry], 100, -1);
                gtk_box_pack_start(GTK_BOX(hboxfreevars[NumVarSpy]),
                                   LabelFreeVars[NumEntry],
                                   FALSE,
                                   FALSE,
                                   0);
            ...
            if (ColumnVar == 1) {
                ...
                gtk_box_pack_start(GTK_BOX(hboxfreevars[NumVarSpy]),
                                   EntryVarSpy[NumEntry],
                                   TRUE,
                                   TRUE,
                                   0);
                gtk_widget_set_size_request((GtkWidget *)EntryVarSpy[NumEntry],
                                            (ColumnVar == 1) ? 80 : 110,
                                            -1);
                char *VarName = CreateVarName(VarSpy[NumVarSpy][0],
                                              VarSpy[NumVarSpy][1],
                                              InfosGene->DisplaySymbols);
                ...
                gtk_signal_connect(GTK_OBJECT(EntryVarSpy[NumEntry]),
                                   "activate",
                                   GTK_SIGNAL_FUNC(EntryVarSpy_activate_event),
                                   (void *)(intptr_t)NumVarSpy);
            }
            if (ColumnVar == 2) {
                EntryVarSpy[NumEntry] = gtk_entry_new();
                gtk_widget_show(EntryVarSpy[NumEntry]);
        ...
        gtk_combo_box_text_append_text(
            GTK_COMBO_BOX_TEXT(DisplayFormatVarSpy[NumVarSpy]), "Dec");
        gtk_combo_box_text_append_text(
            GTK_COMBO_BOX_TEXT(DisplayFormatVarSpy[NumVarSpy]), "Hex");
        gtk_combo_box_text_append_text(
            GTK_COMBO_BOX_TEXT(DisplayFormatVarSpy[NumVarSpy]), "Bin");
        gtk_combo_box_set_active(GTK_COMBO_BOX(DisplayFormatVarSpy[NumVarSpy]),
                                 0);
        gtk_widget_set_size_request(
            (GtkWidget *)DisplayFormatVarSpy[NumVarSpy], 65, -1);
        gtk_box_pack_start(GTK_BOX(hboxfreevars[NumVarSpy]),
                           DisplayFormatVarSpy[NumVarSpy],
                           FALSE,
                           FALSE,
                           0);
        ...
        gtk_button_set_image(GTK_BUTTON(ModifyVarSpy[NumVarSpy]),
                             gtk_image_new_from_stock(
                                 GTK_STOCK_EDIT, GTK_ICON_SIZE_SMALL_TOOLBAR));
        gtk_box_pack_start(GTK_BOX(hboxfreevars[NumVarSpy]),
                           ModifyVarSpy[NumVarSpy],
                           FALSE,
                           FALSE,
                           0);
        gtk_widget_show(ModifyVarSpy[NumVarSpy]);
        gtk_signal_connect(GTK_OBJECT(ModifyVarSpy[NumVarSpy]),
                           "clicked",
                           GTK_SIGNAL_FUNC(OpenModifyVarWindow_clicked_event),
                           (void *)(intptr_t)NumVarSpy);
    }
    gtk_signal_connect(GTK_OBJECT(SpyFreeVarsWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(FreeVarsWindowDeleteEvent),
                       0);
}
