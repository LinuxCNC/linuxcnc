...
void gtk_combo_box_set_active_text(GtkComboBox *p_combo_box, const gchar *text)
{
    ...
    for (i = 0; i < nb_entry; i++) {
        ...
        if (strcmp(gtk_combo_box_get_active_text(MY_GTK_COMBO_BOX(p_combo_box)),
                   text) == 0) {
            break;
        }
    }
...
void SetProperty(int NumParam, char *LblParam, char *ValParam, char SetFocus)
{
    ...
    if (strcmp(LblParam, _("Base")) == 0) {
        ...
        gtk_combo_box_set_active_text(
            GTK_COMBO_BOX(PropEntryBaseParam[NumParam]), ValParam);
        if (SetFocus)
        ...
        if (strcmp(LblParam, _("TimerMode")) == 0) {
            ...
            gtk_combo_box_set_active_text(
                GTK_COMBO_BOX(PropEntryTimerModeParam[NumParam]), ValParam);
...
char *GetProperty(int NumParam)
{
    ...
    if (strcmp(TxtParameter, _("Base")) == 0) {
        ...
        char *pGetText = gtk_combo_box_get_active_text(
            MY_GTK_COMBO_BOX(PropEntryBaseParam[NumParam]));
        ...
        char *pGetText = gtk_combo_box_get_active_text(
            MY_GTK_COMBO_BOX(PropEntryTimerModeParam[NumParam]));
        strcpy(ValTxtParameter, pGetText);
        g_free(pGetText);
    } else {
        // others standard parameters
        strncpy(
            ValTxtParameter,
            (char *)gtk_entry_get_text((GtkEntry *)PropEntryParam[NumParam]),
            60);
...
gint PropertiesWindowDeleteEvent(GtkWidget *widget,
                                 GdkEvent *event,
                                 gpointer data)
{
    (void)widget;
...
void PropertiesInitGtk()
{
    ...
    for (NumParam = 0; NumParam < NBR_PARAMS_PER_OBJ; NumParam++) {
        ...
        gtk_box_pack_start(
            GTK_BOX(hbox[NumParam]), PropLabelParam[NumParam], FALSE, FALSE, 0);
        ...
        gtk_box_pack_start(
            GTK_BOX(hbox[NumParam]), PropEntryParam[NumParam], TRUE, TRUE, 0);
        gtk_widget_show(PropEntryParam[NumParam]);
        gtk_signal_connect(GTK_OBJECT(PropEntryParam[NumParam]),
                           "activate",
                           GTK_SIGNAL_FUNC(SaveElementProperties),
                           NULL);
    ...
    gtk_box_pack_start(
        GTK_BOX(hbox[NumParam]), ButtonApplyProperties, TRUE, FALSE, 0);
    gtk_signal_connect(GTK_OBJECT(ButtonApplyProperties),
                       "clicked",
                       GTK_SIGNAL_FUNC(SaveElementProperties),
                       0);
    gtk_widget_set_sensitive(ButtonApplyProperties, FALSE);
    ...
    gtk_signal_connect(GTK_OBJECT(PropertiesWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(PropertiesWindowDeleteEvent),
                       0);
}
