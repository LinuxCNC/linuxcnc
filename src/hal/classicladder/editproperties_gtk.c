...
char * GetProperty(int NumParam)
{
    ...
    {
        strcpy( ValTxtParameter , (char *)gtk_entry_get_text((GtkEntry *)PropEntryParam[NumParam]) );
    }
...
void PropertiesInitGtk()
{
    ...
    for (NumParam = 0 ; NumParam<NBR_PARAMS_PER_OBJ ; NumParam++)
    {
        ...
        gtk_box_pack_start (GTK_BOX (hbox[NumParam]), PropEntryParam[NumParam], FALSE, FALSE, 0);
        gtk_widget_show (PropEntryParam[NumParam]);
    ...
    gtk_box_pack_start (GTK_BOX (hbox[NumParam]), ButtonValidateProperties, TRUE, FALSE, 0);
