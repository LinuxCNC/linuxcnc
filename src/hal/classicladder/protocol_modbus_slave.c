                           ...
                           unsigned char *Response)
{
    ...
    case MODBUS_FC_READ_INPUTS:
        if (LgtQuestion >= 5) {
            ...
            if (FirstBit + 1 + NbrRealBytes * 8 >
                GetMobdusSlaveNbrVars(FunctionCode))
            ...
            if (ErrorCode == 0) {
                ...
                for (ScanByte = 0; ScanByte < NbrRealBytes; ScanByte++) {
                    ...
                    for (ScanBit = 0; ScanBit < 8; ScanBit++) {
                        if (GetVarForModbusSlave(FunctionCode,
                                                 FirstBit + ScanEle++))
        ...
        if (LgtQuestion >= 5) {
            ...
            if (ErrorCode == 0) {
                ...
                for (ScanEle = 0; ScanEle < NbrWords; ScanEle++) {
                    int ValueWord =
                        GetVarForModbusSlave(FunctionCode, FirstWord + ScanEle);
    ...
    case MODBUS_FC_WRITE_HOLD_REGS:
        if ((LgtQuestion >= 5 && Question[0] == 6) ||
            (LgtQuestion >= 8 && Question[0] == 16)) {
            int OffsetQuest = 3;
            int FirstWord = (Question[1] << 8) | Question[2];
            int NbrWords = 1;
            int ValueWord = 0;
            ...
            if (ErrorCode == 0) {
                ...
                for (ScanEle = 0; ScanEle < NbrWords; ScanEle++) {
                    ...
                    SetVarFromModbusSlave(
                        FunctionCode, FirstWord + ScanEle, ValueWord);
                }
                // Code Function for response
                Response[LgtResponse++] = FunctionCode;
                // First word
