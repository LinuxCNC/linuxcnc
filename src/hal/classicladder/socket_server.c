...
void InitSocketServer(int UseUdpMode, int PortNbr)
{
    ...
    if (server_s == SOCK_INVALID) {
        ...
        Error = bind(
            server_s, (struct sockaddr *)&server_addr, sizeof(server_addr));
        if (Error == SOCK_INVALID) {
            printf(_("ERROR CLASSICLADDER--- Failed to bind socket "
                     "server...(error=%s)\n"),
                   strerror(errno));
            CloseSocketServer();
        } else {
            ...
            if (Error == SOCK_INVALID) {
                printf(_("ERROR CLASSICLADDER--- Failed to listen socket "
                         "server...(error=%s)\n"),
                       strerror(errno));
                CloseSocketServer();
            } else {
                ...
                ThreadHandle = CreateThread(
                    ...
                    (LPTHREAD_START_ROUTINE)
                        SocketServerTcpMainLoop /* thread function*/,
...
#else
                Error =
                    pthread_create(&thread_socket_server,
                                   NULL,
...
#endif
                {
                    printf(_("ERROR CLASSICLADDER--- Failed to create thread "
                             "socket server...\n"));
                    CloseSocketServer();
                } else {
                    printf(
                        _("INFO CLASSICLADDER--- Server socket init ok (modbus "
                          "- port %d)!\n"),
                        PortNbr);
                }
            }
...
int AskAndAnswer(unsigned char *Ask, int LgtAsk, unsigned char *Answer)
{
    ...
    LgtModbusReply = ModbusRequestToRespond(
        &Ask[LGT_MODBUS_IP_HEADER], LgtAsk, &Answer[LGT_MODBUS_IP_HEADER]);
            ...
            do {
                ...
                if (ModbusConfig.ModbusDebugLevel >= 2)
                    printf(_("INFO MODBUS SERVER--- SOCKET RECEIVED=%d !\n"),
                           retcode);
                if (retcode == -1) {
                    printf(_("ERROR CLASSICLADDER--- Failed to recv socket "
                             "server...(error=%s)\n"),
                           strerror(errno));
                } else {
                    if (retcode > 0) {
