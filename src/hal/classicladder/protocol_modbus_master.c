...
int PrepPureModbusAskForCurrentReq(unsigned char *AskFrame)
{
    ...
    int FirstEle = ModbusMasterReq[CurrentReq].FirstModbusElement -
    ...
    case MODBUS_REQ_INPUTS_READ: FunctionCode = MODBUS_FC_READ_INPUTS; break;
    ...
    case MODBUS_REQ_HOLD_REGS_WRITE:
        ...
        break;
    case MODBUS_REQ_COILS_READ: FunctionCode = MODBUS_FC_READ_COILS; break;
    case MODBUS_REQ_HOLD_REGS_READ:
        FunctionCode = MODBUS_FC_READ_HOLD_REGS;
        break;
    case MODBUS_REQ_READ_STATUS: FunctionCode = MODBUS_FC_READ_STATUS; break;
    case MODBUS_REQ_DIAGNOSTICS: FunctionCode = MODBUS_FC_DIAGNOSTICS; break;
    }
    if (FunctionCode > 0) {
        ...
        case MODBUS_FC_FORCE_COIL: {
            int BitValue =
                GetVarForModbus(&ModbusMasterReq[CurrentReq], FirstEle);
        ...
        case MODBUS_FC_WRITE_HOLD_REGS: {
            ...
            AskFrame[FrameSize++] = NbrEles & 0xff;
            AskFrame[FrameSize++] =
                (NbrEles * 2) & 0xff; /* this may get truncated */
            for (i = 0; i < NbrEles; i++) {
                int Value =
                    GetVarForModbus(&ModbusMasterReq[CurrentReq], FirstEle + i);
                AskFrame[FrameSize++] = Value >> 8;
                AskFrame[FrameSize++] = Value & 0xff;
            }
            break;
...
int TreatPureModbusResponse(unsigned char *RespFrame, int SizeFrame)
{
    ...
    if (RespFrame[0] & MODBUS_FC_EXCEPTION_BIT) {
        printf(_("ERROR CLASSICLADDER-     MODBUS-EXCEPTION RECEIVED:%X "
                 "(Excep=%X)\n"),
               RespFrame[0],
               RespFrame[1]);
    } else {
        if (RespFrame[0] != CurrentFuncCode) {
            printf(
                _("ERROR CLASSICLADDER-    MODBUS -Function code received from "
                  "slave was not the same as requested!\n"));
        } else {
            ...
            case MODBUS_FC_READ_COILS: {
                ...
                if (NbrRealBytes == (NbrEles + 7) / 8 &&
                    SizeFrame >= 1 + 1 + NbrRealBytes) {
                    int ScanByte, ScanBit;
                    int ScanEle = 0;
                    // Bits values
                    for (ScanByte = 0; ScanByte < NbrRealBytes; ScanByte++) {
                        ...
                        for (ScanBit = 0; ScanBit < 8; ScanBit++) {
                            ...
                            if (ModbusMasterReq[CurrentReq].LogicInverted)
                                Value = (Value == 0) ? 1 : 0;
                            if (ScanEle < NbrEles)
                                SetVarFromModbus(&ModbusMasterReq[CurrentReq],
                                                 FirstEle + ScanEle++,
                                                 Value);
                            Mask = Mask << 1;
                        }
            ...
            case MODBUS_FC_READ_HOLD_REGS: {
                ...
                for (i = 0; i < NbrBytes; i++) {
                    ...
                    SetVarFromModbus(
                        &ModbusMasterReq[CurrentReq], FirstEle + i, value);
                }
                cError = 0;
                break;
            }
            case MODBUS_FC_FORCE_COIL:
                if (((RespFrame[1] << 8) | RespFrame[2]) == FirstEle &&
                    SizeFrame >= 1 + 2)
                    cError = 0;
                break;
            case MODBUS_FC_FORCE_COILS:
            ...
            case MODBUS_FC_DIAGNOSTICS:
                if (SizeFrame >= 1 + 2 + 2) {
                    ...
                    printf(
                        _("INFO CLASSICLADDER-   MODBUS -Echo back from slave "
                          "#%s is correct (data=257).\n"),
...
int GetModbusResponseLenghtToReceive(void)
{
    ...
    if (CurrentReq != -1) {
        ...
        case MODBUS_FC_READ_HOLD_REGS:
            LgtResp =
                LgtResp + (NbrEles * 2) +
                1; //2 bytes per data and 1 byte for number of datas (max 125)
            break;
        case MODBUS_FC_FORCE_COIL:
        ...
        case MODBUS_FC_WRITE_HOLD_REGS:
            LgtResp =
                LgtResp + (NbrEles * 2) +
                2; //testing 2 bytes per data and 2 bytes for number of datas (max 125)
            break;
        case MODBUS_FC_READ_STATUS:
        ...
        default:
            printf(
                _("INFO CLASSICLADDER-   MODBUS function code not recognized"));
        }
    }
    if (ModbusConfig.ModbusDebugLevel >= 3)
        printf(_("INFO CLASSICLADDER-   MODBUS Length we should receive=%d "
                 "(+3)\n"),
               LgtResp);
    return LgtResp;
}
...
char ExtractUsefullDatasInSerial(unsigned char *Response,
                                 int *pStartOfFrame,
                                 int *pFrameSize)
{
    char ReplyOk = FALSE;
    if (*pFrameSize >= 1 + 1 + 2) {
        ...
        do {
            if (Response[ScanChar] ==
                atoi(ModbusMasterReq[CurrentReq].SlaveAdr))
        ...
        if (AddressCharFound) {
            ...
            unsigned short CalcCRC = CRC16(
                &Response[ScanChar], /*LgtResponse*/ FrameSizeExpected - 2);
            *pStartOfFrame = ScanChar + 1; // after slave address
            // verify CRC
            //			if( CalcCRC==( (Response[ LgtResponse-2 ]<<8)|Response[ LgtResponse-1 ] ) )
            if (CalcCRC == ((Response[ScanChar + FrameSizeExpected - 2] << 8) |
                            Response[ScanChar + FrameSizeExpected - 1])) {
                ...
                *pFrameSize = FrameSizeExpected;
                ...
                debug_printf(
                    DBG_HEADER_ERR
                    "Modbus I/O module - CRC error: calc=%x, frame=%x\n",
                    CalcCRC,
                    (Response[ScanChar + FrameSizeExpected - 2] << 8) |
                        Response[ScanChar + FrameSizeExpected - 1]);
            }
        } else {
            debug_printf(
                DBG_HEADER_ERR
                "Modbus I/O module - not found first char address in frame\n");
        }
    }
    return ReplyOk;
}
...
char TreatModbusMasterResponse(unsigned char *Response, int LgtResponse)
{
    ...
    if (ModbusConfig.ModbusDebugLevel >= 1) {
        printf(
            _("INFO CLASSICLADDER-   Modbus I/O received (Hex):\n    Length=%d "
              "bytes <- "),
            LgtResponse);
    ...
    if (CurrentReq != -1) {
        ...
        // Modbus/RTU on serial used ?
        if (ModbusConfig.ModbusSerialPortNameUsed[0] != '\0') {
            FrameOk = ExtractUsefullDatasInSerial(
                Response, &OffsetHeader, &LgtResponse);
        } else {
        ...
        if (FrameOk) {
            ...
            if (TreatPureModbusResponse(&Response[OffsetHeader],
                ...
                printf(_("ERROR CLASSICLADDER-   MODBUS-INCORRECT RESPONSE "
                         "RECEIVED FROM SLAVE!!!\n"));
                ErrorCnt++;
            }
        } else {
            printf(_("ERROR CLASSICLADDER-   MODBUS-LOW LEVEL ERROR IN "
                     "RESPONSE!!!\n"));
...
void SetVarFromModbus(StrModbusMasterReq *ModbusReq, int ModbusNum, int Value)
{
    if (ModbusReq->TypeReq == MODBUS_REQ_DIAGNOSTICS) {
        ...
        int FirstEle =
            ModbusReq->FirstModbusElement - ModbusConfig.ModbusEleOffset;
        int VarNum;
        if (FirstEle < 0)
            FirstEle = 0;
