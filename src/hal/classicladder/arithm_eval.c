                            ...
                            int *ResIndexOffset)
{
    ...
    if (*ScanExpr != '@') {
        ErrorDesc =
            "Bad var coding (err=0), should start with @ for @xx/yy@ form";
        SyntaxError();
        return FALSE;
    }
    ScanExpr++;
...
int IdentifyFinalVar(char *StartExpr, int *ResType, int *ResOffset)
{
    ...
    int SyntaxOk = IdentifyVarIndexedOrNot(
        StartExpr, ResType, ResOffset, &IndexVarType, &IndexVarOffset);
    if (SyntaxOk) {
...
arithmtype Function(void)
{
    ...
    *pFonc = '\0';
    ...
    if (!strcmp(tcFonc, "ABS")) {
        ...
        if (*Expr != ')') {
            ErrorDesc =
                "Missing end ) after the only variable in ABS() function";
            SyntaxError();
            return 0;
        }
        Expr++; /* ) */
    ...
    if (!strcmp(tcFonc, "MOY") /*original french term!*/ ||
        !strcmp(tcFonc, "AVG") /*added latter!!!*/) {
        int NbrVars = 0;
        do {
            int ValVar;
            Expr++; /* ( -or- , */
...
int EvalCompare(char *CompareString)
{
    ...
    char StrCopy[ARITHM_EXPR_SIZE +
                 1]; /* used for putting null char after first expr */
    char *SearchSep;
    char *CutFirst;
    int Found = FALSE;
    int BoolRes = 0;
