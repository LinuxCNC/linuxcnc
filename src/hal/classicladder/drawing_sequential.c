...
void DrawSeqStep(
    cairo_t *cr, int x, int y, int Size, StrStep *pStep, char DrawingOption)
{
    char BufTxt[50];
    ...
    if (pStep->InitStep) {
        cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
        cairo_rectangle(cr,
                        x + PxOffsetActiveStep,
                        y + PxOffsetActiveStep,
                        Size - 2 * PxOffsetActiveStep,
                        Size - 2 * PxOffsetActiveStep);
        cairo_stroke(cr);
    }
    // step number
...
void DrawSeqTransition(cairo_t *cr,
                       int x,
                       int y,
                       int Size,
                       ...
                       char DrawingOption)
{
    ...
    my_cairo_draw_color_line(cr,
                             0,
                             x + SizeDiv2,
                             (cNoDirectStepOnTop ? y + SizeDiv3 : y) - 1,
                             x + SizeDiv2,
                             cNoDirectStepOnBottom ? y + Size - SizeDiv3
                                                   : y + Size + 1);

    // -
    my_cairo_draw_line(
        cr, x + SizeDiv3, y + SizeDiv2, x + Size - SizeDiv3, y + SizeDiv2);

    // variable for transition
    rtapi_strxcpy(BufTxt,
                  CreateVarName(pTransi->VarTypeCondi,
                                pTransi->VarNumCondi,
    ...
    for (ScanSwitch = 1; ScanSwitch < NBR_SWITCHS_MAX; ScanSwitch++) {
        ...
        if (NumStep != -1) {
            ...
            my_cairo_draw_color_line(cr,
                                     0,
                                     x + SizeDiv3,
                                     y + Size - SizeDiv4 + SizeLittle,
                                     StepX + Size - SizeDiv3,
                                     y + Size - SizeDiv4 + SizeLittle);
            my_cairo_draw_color_line(cr,
                                     0,
                                     x + SizeDiv3,
                                     y + Size - SizeDiv4 + 2 * SizeLittle + 1,
                                     StepX + Size - SizeDiv3,
                                     y + Size - SizeDiv4 + 2 * SizeLittle + 1);
    ...
    for (ScanSwitch = 1; ScanSwitch < NBR_SWITCHS_MAX; ScanSwitch++) {
        ...
        if (NumStep != -1) {
            ...
            my_cairo_draw_color_line(cr,
                                     0,
                                     x + SizeDiv3,
                                     y + SizeDiv4 - SizeLittle,
                                     StepX + Size - SizeDiv3,
                                     y + SizeDiv4 - SizeLittle);
            my_cairo_draw_color_line(cr,
                                     0,
                                     x + SizeDiv3,
                                     y + SizeDiv4 - 2 * SizeLittle - 1,
                                     StepX + Size - SizeDiv3,
                                     y + SizeDiv4 - 2 * SizeLittle - 1);
            //			my_cairo_draw_color_line(cr, 1,
        ...
        if (pTransi->PosiX != StepX || pTransi->PosiY + 1 != StepY) {
            if (pTransi->NumTransLinkedForEnd[0] == -1) {
                ...
                if (pStep->OffDrawCrossStep == 0)
                    ...
                    snprintf(BufTxt, sizeof(BufTxt), ";%d", pStep2->StepNumber);
                DrawPangoText(cr,
                              StepX + pStep->OffDrawCrossStep,
                              StepY - SizeDiv2,
                              Size,
                              -1,
                              BufTxt);
                //printf("CrossStep nbr=%d, offX=%d\n", pStep->StepNumber, pStep->OffDrawCrossStep );
                pStep->OffDrawCrossStep =
                    pStep->OffDrawCrossStep +
                    ((pStep->OffDrawCrossStep != 0) ? 3 : 0) +
                    15; // TODO: add length in pixels of the text written
            }
        }
    }
...
void DrawSeqComment(
    cairo_t *cr, int x, int y, int Size, StrSeqComment *pSeqComment)
{
    ...
    my_cairo_draw_line(
        cr, x + Margin + Oblique, y + Margin, x + Margin, y + Margin + Oblique);
    my_cairo_draw_line(
        cr, x + 4 * Size - Oblique, y + Size, x + 4 * Size, y + Size - Oblique);
    cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
    cairo_rectangle(
        cr, x + Margin, y + Margin, 4 * Size - Margin, Size - Margin);
    cairo_stroke(cr);
    ...
    for (y = 0; y <= SeqPxSize * SEQ_PAGE_HEIGHT; y = y + SeqPxSize) {
        ...
        if (z & 1) {
            for (x = 0; x <= SeqPxSize * SEQ_PAGE_WIDTH; x = x + SeqPxSize)
                my_cairo_draw_line(cr,
                                   x - InfosGene->HScrollValue,
                                   y - InfosGene->VScrollValue,
                                   x - InfosGene->HScrollValue,
                                   y - InfosGene->VScrollValue + SeqPxSize);
        }
        z++;
    }
}

void DrawSequentialCurrentElementEdited(cairo_t *cr,
                                        int SeqPxSize,
                                        int OffsetX,
                                        int OffsetY)
{
    if (EditDatas.CurrentElementSizeX > 0 &&
        ...
        cairo_rectangle(cr,
                        EditDatas.CurrentElementPosiX * SeqPxSize - OffsetX,
                        EditDatas.CurrentElementPosiY * SeqPxSize - OffsetY,
                        EditDatas.CurrentElementSizeX * SeqPxSize,
                        EditDatas.CurrentElementSizeY * SeqPxSize);
        cairo_clip(cr);
...
void DrawSequentialPage(cairo_t *cr,
                        int PageNbr,
                        int SeqPxSize,
                        char DrawingOption)
{
    ...
    for (ScanStep = 0; ScanStep < NBR_STEPS; ScanStep++) {
        ...
        if (pStep->NumPage == PageNbr)
            DrawSeqStep(cr,
                        pStep->PosiX * SeqPxSize - ShiftX,
                        pStep->PosiY * SeqPxSize - ShiftY,
                        SeqPxSize,
                        pStep,
                        DrawingOption);
    }
    ...
    for (ScanTransi = 0; ScanTransi < NBR_TRANSITIONS; ScanTransi++) {
        ...
        if (pTransi->NumPage == PageNbr)
            DrawSeqTransition(cr,
                              pTransi->PosiX * SeqPxSize - ShiftX,
                              pTransi->PosiY * SeqPxSize - ShiftY,
                              SeqPxSize,
                              pTransi,
                              DrawingOption);
    }
    // draw the comments
    for (ScanSeqComment = 0; ScanSeqComment < NBR_SEQ_COMMENTS;
         ScanSeqComment++) {
        StrSeqComment *pSeqComment = &Seq->SeqComment[ScanSeqComment];
        if (pSeqComment->NumPage == PageNbr)
            DrawSeqComment(cr,
                           pSeqComment->PosiX * SeqPxSize - ShiftX,
                           pSeqComment->PosiY * SeqPxSize - ShiftY,
                           SeqPxSize,
                           pSeqComment);
    }
    if (EditDatas.ModeEdit)
    ...
    cairo_t *cr, int x, int y, int Size, int NumElement)
{
    ...
    case ELE_SEQ_TRANSITION:
        my_cairo_draw_line(
            cr, x + SizeDiv2, y + SizeDiv4, x + SizeDiv2, y + Size - SizeDiv4);
        my_cairo_draw_line(
            cr, x + SizeDiv3, y + SizeDiv2, x + Size - SizeDiv3, y + SizeDiv2);
        break;
    case EDIT_SEQ_STEP_AND_TRANS:
        cairo_rectangle(
            cr, x + SizeDiv4, y + 2, Size - 2 * SizeDiv4, SizeDiv2 - 2);
        cairo_stroke(cr);
        my_cairo_draw_line(
            cr, x + SizeDiv2, y + SizeDiv2, x + SizeDiv2, y + Size - 2);
        my_cairo_draw_line(cr,
                           x + SizeDiv3,
                           y + Size - SizeDiv4,
                           x + Size - SizeDiv3,
    ...
    case EDIT_SEQ_START_MANY_TRANS:
        ...
        my_cairo_draw_line(
            cr, x + SizeDiv4, y + SizeDiv4 - 4, x + SizeDiv4, y + SizeDiv4 + 4);
        my_cairo_draw_line(cr,
                           x + Size - SizeDiv4,
                           y + SizeDiv4,
                           x + Size - SizeDiv4,
    ...
    case EDIT_SEQ_END_MANY_TRANS:
        my_cairo_draw_line(
            cr, x + 2, y + Size - SizeDiv4, x + Size - 2, y + Size - SizeDiv4);
        my_cairo_draw_line(cr,
                           x + SizeDiv4,
                           y + Size - SizeDiv4 - 4,
                           x + SizeDiv4,
    ...
    case EDIT_SEQ_START_MANY_STEPS:
        my_cairo_draw_line(cr, x + 2, y + SizeDiv4, x + Size - 2, y + SizeDiv4);
        my_cairo_draw_line(
            cr, x + 2, y + SizeDiv4 + 2, x + Size - 2, y + SizeDiv4 + 2);
        my_cairo_draw_line(
            cr, x + SizeDiv4, y + SizeDiv4 - 4, x + SizeDiv4, y + SizeDiv4 + 4);
        my_cairo_draw_line(cr,
                           x + Size - SizeDiv4,
    ...
    case ELE_SEQ_COMMENT:
        ...
        break;
    default: DrawCommonElementForToolbar(cr, x, y, Size, NumElement); break;
    }
}
