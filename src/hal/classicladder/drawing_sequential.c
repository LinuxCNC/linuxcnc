    ...
    StrStep * pStep, char DrawForToolBar)
{
    ...
    // init step ?
    if (pStep->InitStep) {
	gdk_draw_rectangle(DrawPixmap,	/* TheGc */
	    drawing_area->style->black_gc, FALSE /* filled */ ,
	    x + 5, y + 5, Size - 10, Size - 10);
    }
    // step number
    sprintf(BufTxt, "%d", pStep->StepNumber);
    gdk_draw_text(DrawPixmap, gtk_style_get_font(drawing_area->style),
	drawing_area->style->black_gc, x + SizeDiv3, y + SizeDiv2 + 4, BufTxt,
	strlen(BufTxt));
    ...
    StrTransition * pTransi, char DrawForToolBar)
{
	...
	x + SizeDiv2,
	cNoDirectStepOnBottom ? y + Size - SizeDiv3 : y + Size + 1);
    ...
    gdk_draw_text(DrawPixmap, gtk_style_get_font(drawing_area->style),
	drawing_area->style->black_gc, x + 3 * SizeDiv4, y + SizeDiv2 + 5,
	...
	if (NumStep != -1) {
	    ...
	    // ===
	    gdk_draw_line(DrawPixmap, DynaGcOff,
		x + SizeDiv3, y + Size - SizeDiv4 + 2,
		StepX + Size - SizeDiv3, y + Size - SizeDiv4 + 2);
	    gdk_draw_line(DrawPixmap, DynaGcOff, x + SizeDiv3,
		y + Size - SizeDiv4 + 4, StepX + Size - SizeDiv3,
		y + Size - SizeDiv4 + 4);
	    gdk_draw_line(DrawPixmap, drawing_area->style->white_gc,
		x + SizeDiv3, y + Size - SizeDiv4 + 3,
		StepX + Size - SizeDiv3, y + Size - SizeDiv4 + 3);
	    // | with the step
	    gdk_draw_line(DrawPixmap, DynaGcOff,
		StepX + SizeDiv2, y + Size - SizeDiv4 + 4, StepX + SizeDiv2,
		y + Size + 1);
	}
    }
	...
	if (NumStep != -1) {
	    ...
	    // ===
	    gdk_draw_line(DrawPixmap, DynaGcOff,
		x + SizeDiv3, y + SizeDiv4 - 2, StepX + Size - SizeDiv3,
		y + SizeDiv4 - 2);
	    gdk_draw_line(DrawPixmap, DynaGcOff, x + SizeDiv3,
		y + SizeDiv4 - 4, StepX + Size - SizeDiv3, y + SizeDiv4 - 4);
	    gdk_draw_line(DrawPixmap, drawing_area->style->white_gc,
		x + SizeDiv3, y + SizeDiv4 - 3, StepX + Size - SizeDiv3,
		y + SizeDiv4 - 3);
	    // | with the step
	    gdk_draw_line(DrawPixmap, DynaGcOff,
	...
	for (ScanSwitch = 0; ScanSwitch < NBR_SWITCHS_MAX; ScanSwitch++) {
	    ...
	    if (NumTransi != -1) {
		TransiX =
		    Seq->Transition[NumTransi].PosiX * Size -
		    InfosGene->HScrollValue;
		gdk_draw_line(DrawPixmap, DynaGcOff, x + SizeDiv2,
		    y + SizeDiv3, TransiX + SizeDiv2, y + SizeDiv3);
	    }
	}
    }
	...
	for (ScanSwitch = 0; ScanSwitch < NBR_SWITCHS_MAX; ScanSwitch++) {
		...
		TransiX =
		    ...
		    InfosGene->HScrollValue;
		gdk_draw_line(DrawPixmap, DynaGcOff, x + SizeDiv2,
		    y + Size - SizeDiv3, TransiX + SizeDiv2,
		    y + Size - SizeDiv3);
	    }
	}
    }
	...
	if (pTransi->PosiX != StepX || pTransi->PosiY + 1 != StepY) {
		...
		gdk_draw_line(DrawPixmap, DynaGcOff,
		    x + SizeDiv3, y + Size - SizeDiv3, x + SizeDiv2,
		    y + Size - SizeDiv3 + SizeDiv4);
		gdk_draw_line(DrawPixmap, DynaGcOff, x + Size - SizeDiv3,
		    y + Size - SizeDiv3, x + SizeDiv2,
		    y + Size - SizeDiv3 + SizeDiv4);
		gdk_draw_line(DrawPixmap, DynaGcOff, x + SizeDiv2,
		    y + Size - SizeDiv3, x + SizeDiv2,
		    y + Size - SizeDiv3 + SizeDiv4);
		sprintf(BufTxt, "%d", pStep->StepNumber);
		gdk_draw_text(DrawPixmap,
		    gtk_style_get_font(drawing_area->style),
		    drawing_area->style->black_gc, x + SizeDiv3,
		    y + Size - SizeDiv3 + SizeDiv4 + 11, BufTxt,
		    strlen(BufTxt));
		...
		gdk_draw_line(DrawPixmap, DynaGcOff,
		    StepX + SizeDiv3, StepY - SizeDiv2, StepX + SizeDiv2,
		    StepY - SizeDiv4);
		gdk_draw_line(DrawPixmap, DynaGcOff, StepX + Size - SizeDiv3,
		    StepY - SizeDiv2, StepX + SizeDiv2, StepY - SizeDiv4);
		gdk_draw_line(DrawPixmap, DynaGcOff, StepX + SizeDiv2,
		    StepY - SizeDiv4, StepX + SizeDiv2, StepY + 1);
		...
		gdk_draw_text(DrawPixmap,
		    ...
		    StepX + SizeDiv3 + pStep->OffDrawCrossStep,
		    StepY - SizeDiv4 - 9, BufTxt, strlen(BufTxt));
//printf("CrossStep n°=%d, offX=%d\n", pStep->StepNumber, pStep->OffDrawCrossStep );
		pStep->OffDrawCrossStep = pStep->OffDrawCrossStep + ((pStep->OffDrawCrossStep != 0) ? 3 : 0) + 15;	// TODO: 
															// 
		// add 
		// length 
		// in 
...
void DrawSequentialGrid()
{
	...
	gdk_draw_line(pixmap, DynaGcOn, 0, y - InfosGene->VScrollValue,
	    SEQ_SIZE_DEF * NBR_STEPS, y - InfosGene->VScrollValue);
	if (z & 1) {
	    for (x = 0; x <= SEQ_SIZE_DEF * SEQ_PAGE_WIDTH;
		x = x + SEQ_SIZE_DEF)
		gdk_draw_line(pixmap, DynaGcOn, x - InfosGene->HScrollValue,
		    y - InfosGene->VScrollValue, x - InfosGene->HScrollValue,
		    y - InfosGene->VScrollValue + SEQ_SIZE_DEF);
	}
	z++;
    }
    gdk_gc_unref(DynaGcOn);
...
void DrawSequentialPage(int PageNbr)
{
    ...
    gdk_draw_rectangle(pixmap, drawing_area->style->white_gc, TRUE	/* filled 
									 */ ,
	0, 0, GTK_WIDGET(drawing_area)->allocation.width,
	...
	if (pStep->NumPage == PageNbr)
	    DrawSeqStep(pixmap,
		pStep->PosiX * SEQ_SIZE_DEF - InfosGene->HScrollValue,
		pStep->PosiY * SEQ_SIZE_DEF - InfosGene->VScrollValue,
		SEQ_SIZE_DEF, pStep, FALSE);
    }
	...
	if (pTransi->NumPage == PageNbr)
	    DrawSeqTransition(pixmap,
		pTransi->PosiX * SEQ_SIZE_DEF - InfosGene->HScrollValue,
		pTransi->PosiY * SEQ_SIZE_DEF - InfosGene->VScrollValue,
		SEQ_SIZE_DEF, pTransi, FALSE);
    }
    ...
    int NumElement)
{
	...
	gdk_draw_rectangle(DrawPixmap, TheGc, FALSE /* filled */ ,
	    x + SizeDiv4, y + 2, Size - 2 * SizeDiv4, SizeDiv2 - 2);
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + SizeDiv2, y + SizeDiv2, x + SizeDiv2, y + Size - 2);
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + SizeDiv3, y + Size - SizeDiv4, x + Size - SizeDiv3,
	    y + Size - SizeDiv4);
	break;
	...
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + Size - SizeDiv4, y + SizeDiv4, x + Size - SizeDiv4,
	    y + SizeDiv4 + 4);
	break;
	...
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + SizeDiv4, y + Size - SizeDiv4 - 4, x + SizeDiv4,
	    y + Size - SizeDiv4 + 4);
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + Size - SizeDiv4, y + Size - SizeDiv4, x + Size - SizeDiv4,
	    y + Size - SizeDiv4 - 4);
	break;
	...
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + SizeDiv4, y + SizeDiv4 - 4, x + SizeDiv4, y + SizeDiv4 + 4);
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + Size - SizeDiv4, y + SizeDiv4 + 2, x + Size - SizeDiv4,
	    y + SizeDiv4 + 4);
	break;
	...
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + 2, y + Size - SizeDiv4, x + Size - 2, y + Size - SizeDiv4);
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + 2, y + Size - SizeDiv4 - 2, x + Size - 2,
	    y + Size - SizeDiv4 - 2);
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc, x + SizeDiv4,
	    y + Size - SizeDiv4 - 4, x + SizeDiv4, y + Size - SizeDiv4 + 4);
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + Size - SizeDiv4, y + Size - SizeDiv4 - 2, x + Size - SizeDiv4,
	    y + Size - SizeDiv4 - 4);
	break;
    case EDIT_SEQ_LINK:
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
	    x + SizeDiv3, y + SizeDiv4, x + Size - SizeDiv3,
	    y + Size - SizeDiv4);
	gdk_draw_arc(DrawPixmap, drawing_area->style->black_gc, FALSE	/* filled 
									 */ ,
	    x + SizeDiv3 - 2, y + SizeDiv4 - 2, 4, 4, 0 /* angle1 */ ,
	    23040 /* angle2 */ );
	gdk_draw_arc(DrawPixmap, drawing_area->style->black_gc, FALSE	/* filled 
									 */ ,
	    x + Size - SizeDiv3 - 2, y + Size - SizeDiv4 - 2, 4, 4, 0	/* angle1 
	     */ , 23040 /* angle2 */ );
	break;
    case EDIT_POINTER:
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc, x + SizeDiv4, y + SizeDiv4, x + Size - SizeDiv4, y + Size - SizeDiv4);	/* \ 
																	 */
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc, x + SizeDiv4, y + SizeDiv4, x + SizeDiv3, y + SizeDiv2);	/* | 
																 */
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc, x + SizeDiv4, y + SizeDiv4, x + SizeDiv2, y + SizeDiv3);	/* _ 
																 */
	gdk_draw_line(DrawPixmap, drawing_area->style->black_gc,
