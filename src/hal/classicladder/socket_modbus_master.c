...
int ClientSocketOpened
    [NBR_CLIENTS_MAX]; // -1 if not opened, else req nbr (to retrieve directly IP address)
...
void InitSocketModbusMaster()
{
    ...
    for (ScanClientSock = 0; ScanClientSock < NBR_CLIENTS_MAX;
         ScanClientSock++) {
...
#ifdef __WIN32__
    ThreadHandleClient = CreateThread(
        NULL /*no security attributes*/,
        16 * 1024L /*default stack size*/,
...
#else
    Error = pthread_create(&thread_socket_client,
                           NULL,
                           (void *(*)(void *))SocketModbusMasterLoop,
                           (void *)NULL);
    if (Error)
#endif
    {
        printf(_("ERROR CLASSICLADDER-   Failed to create thread I/O Modbus "
                 "master...\n"));
...
void ConfigSerialModbusMaster(void)
{
    ...
    if (ModbusConfig.ModbusSerialPortNameUsed[0] != '\0') {
        if (!SerialOpen(
                /*ModbusConfig.ModbusSerialPortNameUsed, ModbusConfig.ModbusSerialSpeed*/
                )) {
            Error = -1;
            printf(_("INFO CLASSICLADDER---I/O Modbus master Data bits %i Stop "
                     "bits %i Parity %i\n"),
...
char VerifyTcpConnection(char *SlaveAdr)
{
    ...
    do {
        if (ClientSocketOpened[ScanClientSock] != -1) {
            if (strcmp(SlaveAdr,
                       ModbusMasterReq[ClientSocketOpened[ScanClientSock]]
                           .SlaveAdr) == 0) {
    ...
    if (!AlreadyConnected) {
        ...
        if (FreeOneFound) {
            ...
            if (client_s == SOCK_INVALID) {
                ...
                char *PosiSep;
                memset(&io_module_addr,
                       0,
                       sizeof(io_module_addr)); /* Zero out structure */
                io_module_addr.sin_family =
                    AF_INET; /* Internet address family */

                // verify if port given in string ?
                PosiSep = strchr(SlaveAdr, ':');
                if (PosiSep == NULL) {
                    io_module_addr.sin_addr.s_addr =
                        inet_addr(SlaveAdr); /* Server IP address */
                } else {
                    ...
                    io_module_addr.sin_addr.s_addr =
                        inet_addr(Address); /* Server IP address */
                }
...
int SendSocketModbusMaster(char *SlaveAdr,
                           int NumPort,
                           char *Frame,
                           int LgtFrame)
{
    ...
    if (VerifyTcpConnection(SlaveAdr)) {
        if (ModbusConfig.ModbusDebugLevel >= 2)
            debug_printf(
                _("INFO CLASSICLADDER-   Sending frame to I/O module...\n"));
        /* Send the Modbus frame */
        LgtSend = send(client_s, Frame, LgtFrame, 0);
        if (LgtSend == LgtFrame)
            Status = 0;
        else
            debug_printf(_("ERROR CLASSICLADDER-  FAILED TO SEND ON SOCKET "
                           "!!!(LgtSend=%d)\n"),
                         LgtSend);
    }
    return Status;
...
int WaitRespSocketModbusMaster(char *Buff,
                               int BuffSize,
                               int TimeOutResponseMilliSecs)
{
    int ResponseSize = 0;
    if (client_s != SOCK_INVALID) {
        int recep_descrip;
        ...
        if (recep_descrip > 0) {
            ...
            if (ModbusConfig.ModbusDebugLevel >= 2)
                printf(
                    _("INFO CLASSICLADDER-   waiting for slave response...\n"));
...
void SocketModbusMasterLoop(void)
{
    ...
    while (ClientSocketRunning) {
        ...
        if (InfosGene->LadderState != STATE_RUN ||
            ...
            SizeQuestionToAsk = ModbusMasterAsk((unsigned char *)AdrIP,
                                                (unsigned char *)QuestionFrame);
            if (SizeQuestionToAsk > 0) {
                if (ModbusConfig.ModbusSerialPortNameUsed[0] == '\0') {
                    SendResultStatus = SendSocketModbusMaster(
                        AdrIP, 502, QuestionFrame, SizeQuestionToAsk);
                } else {
                    int NbrCharsWaited = 1 /*adr*/ +
                    ...
                    SerialSetResponseSize(NbrCharsWaited,
                ...
                if (SendResultStatus == 0) {
                    if (ModbusConfig.ModbusTimeAfterTransmit > 0) {
                        ...
                        if (ModbusConfig.ModbusDebugLevel >= 3)
                            printf(_("INFO CLASSICLADDER- after transmit delay "
                                     "now...%i milliseconds\n"),
                    ...
                    if (ModbusConfig.ModbusSerialPortNameUsed[0] == '\0')
                        ResponseSize = WaitRespSocketModbusMaster(
                            ResponseFrame,
                            800,
                            ModbusConfig.ModbusTimeOutReceipt);
                    else
                    ...
                    StatsNbrFrames++;
                    if (ResponseSize == 0)
                        printf(_("ERROR CLASSICLADDER-  MODBUS NO RESPONSE "
                                 "(Errs=%d/%d) !?\n"),
