...
{
    printf(CL_PRODUCT_NAME
           " v" CL_RELEASE_VER_STRING "\n" CL_RELEASE_DATE_STRING "\n"
           "\n"
    ...
    printf(
        "This version of Classicladder is adapted for use with LinuxCNC and "
        "HAL\n"
        ...
        "   --modmaster        initialize modbus master I/O ( modbus config is "
        "loaded with other objects )\n"
        "   --modslave         initialize modbus slave I/O (TCP only- B and W "
        "variables accessible\n"
        "   --modbus_port=portnumber  used for modbus slave using TCP ( "
        "ethernet )\n"
        "   --debug            sets the RTAPI debuglevel for printing debug "
        "messages\n"
        "Please also note that the classicladder realtime module must be "
        "loaded first\n"
        "eg: loadrt classicladder_rt    for default number of ladder "
        "objects\n");
...
void process_options(int argc, char *argv[])
{
    ...
    for (;;) {
        ...
        case 'n': nogui = 1; break;
        case 'm': modmaster = 1; break;
        case 's': modslave = 1; break;
        case 'd': rtapi_set_msg_level(RTAPI_MSG_ALL); break;
        case 'p': ModbusServerPort = atoi(optarg); break;
        case 'f':
            NewPath = (optarg);
            pathswitch = 1;
            break;
...
static void do_exit(int unused)
{
    (void)unused;
    hal_exit(compId);
    printf(_("ERROR CLASSICLADDER-   Error initializing classicladder user "
             "module.\n"));
    exit(0);
}
...
void DoFlipFlopRunStop(void)
{
    if (InfosGene->LadderState == STATE_RUN) {
        ...
        MessageInStatusBar(
            _("Stopped program - press run button to continue."));
...
int main(int argc, char *argv[])
{
    ...
    if (ClassicLadder_AllocAll()) {
        ...
        if (nogui == TRUE) {
            rtapi_print(
                _("INFO CLASSICLADDER-   No ladder GUI requested-Realtime runs "
                  "till HAL closes.\n"));
            ClassicLadder_InitAllDatas();
            ProjectLoadedOk =
                LoadProjectFiles(InfosGene->CurrentProjectFileName);
            if (pathswitch) {
            ...
            if ((used == 0) || ((argc - optind) != 0)) {
                ...
                MessageInStatusBar(ProjectLoadedOk
                                       ? _("Project loaded and running")
                ...
                UpdateWindowTitleWithProjectName();
                MessageInStatusBar(
                    _("GUI reloaded with existing ladder program"));
                if (modmaster) {
                    PrepareModbusMaster();
                }
            }
            ...
            gtk_main();
            rtapi_print(
                _("INFO CLASSICLADDER-   Ladder GUI closed. Realtime runs till "
                  "HAL closes\n"));
            ClassicLadder_FreeAll(TRUE);
            hal_exit(compId);
            return 0;
