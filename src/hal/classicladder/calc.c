    ...
    for (NumRung = 0; NumRung < NBR_RUNGS; NumRung++) {
        ...
        for (y = 0; y < RUNG_HEIGHT; y++) {
            for (x = 0; x < RUNG_WIDTH; x++) {
                ...
                RungArray[NumRung].Element[x][y].IndexedVarType =
                    -1; /* undefined */
            ...
            for (x = 0; x < RUNG_WIDTH; x++) {
                if ((RungArray[NumRung].Element[x][y].Type ==
                    ...
                    (RungArray[NumRung].Element[x][y].Type ==
                     ELE_FALLING_INPUT)) {
                    StateElement =
                        ReadVarForElement(&RungArray[NumRung].Element[x][y]);
                    if (RungArray[NumRung].Element[x][y].Type ==
                        ELE_FALLING_INPUT)
...
/* Elements : -| |- and -|/|- */
char CalcTypeInput(
    int x, int y, StrRung *UpdateRung, char IsNot, char OnlyFronts)
{
    char State;
    char StateElement;
    char StateVar;
...
int CalcTypeOutputCall(int x, int y, StrRung *UpdateRung)
{
    ...
    if (State)
        CallSrSection =
            SearchSubRoutineWithItsNumber(UpdateRung->Element[x][y].VarNum);
    UpdateRung->Element[x][y].DynamicInput = State;
    UpdateRung->Element[x][y].DynamicState = State;
...
void CalcTypeTimerIEC(int x, int y, StrRung *UpdateRung)
{
    ...
    if (TimerIEC->TimerMode == TIMER_IEC_MODE_OFF ||
        TimerIEC->TimerMode == TIMER_IEC_MODE_PULSE) {
    ...
    if (DoIncTime) {
        TimerIEC->ValueToReachOneBaseUnit =
            TimerIEC->ValueToReachOneBaseUnit +
            InfosGene->GeneralParams.PeriodicRefreshMilliSecs;
        if (TimerIEC->ValueToReachOneBaseUnit >= TimerIEC->Base) {
            ...
            TimerIEC->ValueToReachOneBaseUnit =
                TimerIEC->ValueToReachOneBaseUnit - TimerIEC->Base;
        }
    }
...
char CalcTypeCompar(int x, int y, StrRung *UpdateRung)
{
    ...
    StateElement =
        EvalCompare(ArithmExpr[UpdateRung->Element[x][y].VarNum].Expr);
    UpdateRung->Element[x][y].DynamicState = StateElement;
    if (x == 2) {
        State = StateElement;
    } else {
        UpdateRung->Element[x - 2][y].DynamicInput =
            StateOnLeft(x - 2, y, UpdateRung);
...
char CalcTypeOutputOperate(int x, int y, StrRung *UpdateRung)
{
    ...
    if (State)
        MakeCalc(ArithmExpr[UpdateRung->Element[x][y].VarNum].Expr,
                 FALSE /* verify mode */
        );
    UpdateRung->Element[x][y].DynamicInput = State;
    UpdateRung->Element[x][y].DynamicState = State;
    ...
    do {
        do {
            ...
            case ELE_UNUSABLE:
                ...
                break;
            /* End fix */
            case ELE_INPUT: CalcTypeInput(x, y, Rung, FALSE, FALSE); break;
            case ELE_INPUT_NOT: CalcTypeInput(x, y, Rung, TRUE, FALSE); break;
            ...
            case ELE_FALLING_INPUT:
                ...
                break;
            case ELE_CONNECTION: CalcTypeConnection(x, y, Rung); break;
#ifdef OLD_TIMERS_MONOS_SUPPORT
            case ELE_TIMER: CalcTypeTimer(x, y, Rung); break;
            case ELE_MONOSTABLE: CalcTypeMonostable(x, y, Rung); break;
#endif
            case ELE_COUNTER: CalcTypeCounter(x, y, Rung); break;
            case ELE_TIMER_IEC: CalcTypeTimerIEC(x, y, Rung); break;
            case ELE_COMPAR: CalcTypeCompar(x, y, Rung); break;
            case ELE_OUTPUT: CalcTypeOutput(x, y, Rung, FALSE); break;
            case ELE_OUTPUT_NOT: CalcTypeOutput(x, y, Rung, TRUE); break;
            case ELE_OUTPUT_SET:
            ...
            case ELE_OUTPUT_CALL:
                ...
                if (SectionToCall != -1) {
                    StrSection *pSubRoutineSection =
                        &SectionArray[SectionToCall];
                    if (pSubRoutineSection->Used &&
                        pSubRoutineSection->SubRoutineNumber >= 0)
                        RefreshASection(
                            pSubRoutineSection); //recursive call! ;-)
                    else
                        debug_printf(
                            "Refresh rungs aborted - call to a sub-routine "
                            "undefined or programmed as main !!!");
                }
                break;
            case ELE_OUTPUT_OPERATE: CalcTypeOutputOperate(x, y, Rung); break;
            }
            y++;
    ...
    do {
        ...
        if (Goto != -1) {
            if (!RungArray[Goto].Used) {
                Done = TRUE;
                debug_printf(
                    "Refresh rungs aborted - jump to an undefined rung found "
                    "in rung No%d...\n",
                    Goto);
            }
            NumRung = Goto;
            MadLoopBreak++;
            if (MadLoopBreak >
                99999) //value to set here?... or else measuring time?
            {
                Done = TRUE;
                debug_printf("Refresh rungs aborted - mad loop jump detected - "
                             "STOPPED...!\n");
...
void ClassicLadder_RefreshAllSections()
{
    ...
    for (ScanMainSection = 0; ScanMainSection < NBR_SECTIONS;
         ScanMainSection++) {
        ...
        if (pScanSection->Used && pScanSection->SubRoutineNumber == -1 &&
            pScanSection->Language == SECTION_IN_LADDER) {
            RefreshASection(pScanSection);
        }

#ifdef SEQUENTIAL_SUPPORT
        // current section defined and is in sequential language
        if (pScanSection->Used &&
            pScanSection->Language == SECTION_IN_SEQUENTIAL) {
