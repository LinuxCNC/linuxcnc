...
#else
    cairo_surface_t
        *ToolBarSurface[NBR_ELE_TOOLBAR_Y_MAX][NBR_ELE_TOOLBAR_X_MAX];
#endif
...
char *ToolBarToolTipsTextLadder[][NBR_ELE_TOOLBAR_X_MAX] = {
    {N_("Current Object\nSelector"),
     N_("Eraser"),
     N_("Select a rung part (drag and release)"),
     N_("Copy rung part selected")                                                               },
    {N_("N.O. Input"),
     N_("N.C. Input"),
     N_("Rising Edge\n Input"),
     N_("Falling Edge\n Input")                                                                  },
...
void ButtonsForStart()
{
    ...
    gtk_widget_set_sensitive(ToolbarDatas[NUM_TOOLBAR_FOR_LADDER].ToolbarTable,
                             TRUE);
#ifdef SEQUENTIAL_SUPPORT
    // ...in sequential toolbar
    SelectAnElementInToolBar(NUM_TOOLBAR_FOR_SEQ, EDIT_POINTER);
    gtk_widget_set_sensitive(ToolbarDatas[NUM_TOOLBAR_FOR_SEQ].ToolbarTable,
                             TRUE);
#endif
    MessageInStatusBar(iCurrentLanguage == SECTION_IN_LADDER
                           ? _("Current rung in edit mode...")
                           : _("Edit mode..."));
...
void ButtonsForEnd(char ForRung)
{
    if (ForRung) {
        ...
        gtk_widget_set_sensitive(
            ToolbarDatas[NUM_TOOLBAR_FOR_LADDER].ToolbarTable, FALSE);
    } else {
        gtk_widget_hide(EditorButtonAdd);
        gtk_widget_hide(EditorButtonIns);
        gtk_widget_hide(EditorButtonDel);
...
{
    ShowConfirmationBox(_("Delete"),
                        _("Do you really want to delete the current rung ?"),
                        DeleteCurrentRung);
}
    ...
    if (OpenIt) {
        ...
        if (!FirstOpenToSetPosition) {
            ...
            gtk_window_get_position(
                GTK_WINDOW(MainSectionWindow), &mainx, &mainy);
...
char ConvertNumElementInToolbarPosisXY(int NumToolbar,
                                       int NumElementWanted,
                                       int *FoundX,
                                       int *FoundY)
{
    ...
    do {
        do {
            int EleValue =
                pToolbarDatas->PtrOnToolBarElementsList[ScanY][ScanX];
...
void SelectAnElementInToolBar(int iNumToolbar, int iNumElementSelectedWithPopup)
{
    ...
    if (ConvertNumElementInToolbarPosisXY(iNumToolbar,
                                          iNumElementSelectedWithPopup,
                                          &SearchEleX,
                                          &SearchEleY))
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(ToolbarDatas[iNumToolbar]
                                  .ToolbarBtnRadio[SearchEleY][SearchEleX]),
            TRUE);
}
...
void ButtonToolbarSignal(GtkWidget *widget, gpointer data)
{
    ...
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
        ...
        if (EditDatas.NumElementSelectedInToolBar < EDIT_CNX_WITH_TOP ||
            ...
            GetSizesOfAnElement(EditDatas.NumElementSelectedInToolBar,
                                &EditDatas.GhostZoneSizeX,
                                &EditDatas.GhostZoneSizeY);
...
void InitAllForToolbar(void)
{
    ...
    ToolbarDatas[NUM_TOOLBAR_FOR_LADDER].PtrOnToolBarElementsList =
        ToolBarElementsLadder;
#ifdef SEQUENTIAL_SUPPORT
    ToolbarDatas[NUM_TOOLBAR_FOR_SEQ].PtrOnToolBarElementsList =
        ToolBarElementsSequential;
#endif
}

void CreateOneToolbar(GtkWidget *Box,
                      int NumTable,
                      char *PtrOnToolTipsText[][NBR_ELE_TOOLBAR_X_MAX])
{
    ...
    ScanToolBarX = 0;
    ScanToolBarY = 0;
    pToolbarDatas->ToolbarTable = gtk_table_new(
        NBR_ELE_TOOLBAR_X_MAX, NBR_ELE_TOOLBAR_Y_MAX, FALSE /*homogeneous*/
    );
    gtk_box_pack_start(
        GTK_BOX(Box), pToolbarDatas->ToolbarTable, TRUE, TRUE, 0);

    do {
        ToolBarEle.Type =
            pToolbarDatas->PtrOnToolBarElementsList[ScanToolBarY][ScanToolBarX];
        ToolBarEle.ConnectedWithTop = 0;

        if (ToolBarEle.Type != 0) {
            char *pHelpText =
                gettext(PtrOnToolTipsText[ScanToolBarY][ScanToolBarX]);
#if GTK_MAJOR_VERSION < 3
            GdkGC *gc = drawing_area->style->bg_gc[0];
            pToolbarDatas->ToolbarPixmap[ScanToolBarY][ScanToolBarX] =
                gdk_pixmap_new(GDK_DRAWABLE(drawing_area->window),
                               PIXELS_SIZE_IN_TOOLBAR,
                               PIXELS_SIZE_IN_TOOLBAR,
                               -1);
            gdk_draw_rectangle(
                GDK_DRAWABLE(
            ...
            cairo_t *cr = gdk_cairo_create(
                pToolbarDatas->ToolbarPixmap[ScanToolBarY][ScanToolBarX]);
#else
            pToolbarDatas->ToolBarSurface[ScanToolBarY][ScanToolBarX] =
            ...
            if (NumTable == NUM_TOOLBAR_FOR_SEQ)
                DrawSeqElementForToolBar(
                    cr, 0, 0, PIXELS_SIZE_IN_TOOLBAR, ToolBarEle.Type);
            else
#endif
                DrawElement(cr,
                            0,
...
#if GTK_MAJOR_VERSION < 3
            pToolbarDatas->ToolbarImage[ScanToolBarY][ScanToolBarX] =
            ...
            cairo_surface_destroy(
                pToolbarDatas->ToolBarSurface[ScanToolBarY][ScanToolBarX]);
#endif
            pToolbarDatas->ToolbarBtnRadio[ScanToolBarY][ScanToolBarX] =
                gtk_radio_button_new(PtrListRadiosBtn);
            PtrListRadiosBtn = gtk_radio_button_get_group(GTK_RADIO_BUTTON(
                pToolbarDatas->ToolbarBtnRadio[ScanToolBarY][ScanToolBarX]));
            ...
            gtk_widget_show(
                pToolbarDatas->ToolbarImage[ScanToolBarY][ScanToolBarX]);
            gtk_table_attach(
                GTK_TABLE(pToolbarDatas->ToolbarTable),
                pToolbarDatas->ToolbarBtnRadio[ScanToolBarY][ScanToolBarX],
                ScanToolBarX,
                ScanToolBarX + 1,
                ScanToolBarY,
            ...
            gtk_signal_connect(
                GTK_OBJECT(
                    pToolbarDatas->ToolbarBtnRadio[ScanToolBarY][ScanToolBarX]),
                "clicked",
                GTK_SIGNAL_FUNC(ButtonToolbarSignal),
                GINT_TO_POINTER((int)ToolBarEle.Type));
...
void EditorInitGtk()
{
    ...
    gtk_signal_connect(GTK_OBJECT(EditWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(EditorWindowDeleteEvent),
                       0);
