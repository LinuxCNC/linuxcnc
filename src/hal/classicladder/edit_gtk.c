...
static short int (* PtrToolBarElementsList) [ 2 ] = NULL; //ptr for array with 2 dimensions, is hard... :-(
...
void DrawToolBarElements( short int PtrOnToolBarElementsList[][2] )
{
    ...
    do
    {
        ...
        if ( PtrOnToolBarElementsList==ToolBarElementsSequential )
            DrawSeqElementForToolBar(EditorPixmap,ScanToolBarX*(BLOCK_WIDTH_DEF+2) +2 , ScanToolBarY*(BLOCK_HEIGHT_DEF+2) +2,
                    BLOCK_WIDTH_DEF, ToolBarEle.Type );
        else
#endif
            DrawElement(EditorPixmap,ScanToolBarX*(BLOCK_WIDTH_DEF+2) +2 , ScanToolBarY*(BLOCK_HEIGHT_DEF+2) +2,
                    BLOCK_WIDTH_DEF, BLOCK_HEIGHT_DEF, ToolBarEle, TRUE);
        /* draw current element selected in the toolbar (required to display default pointer selected at startup) */
        if ( ToolBarEle.Type==EditDatas.NumElementSelectedInToolBar )
        {
            draw_brush (GTK_WIDGET(EditorDrawingArea), ScanToolBarX*(BLOCK_WIDTH_DEF+2) +2, ScanToolBarY*(BLOCK_HEIGHT_DEF+2) +2, NULL);
...
static gint EditorExposeEvent( GtkWidget      *widget,
...
static gint EditorSelectElement( GtkWidget      *widget,
	...
	if (EditDatas.ModeEdit)
	{
		...
		if (event->button == 1 && pixmap != NULL)
		{
			...
			if ( (ToolBarXBak!=-1) && (ToolBarYBak!=-1) )
				draw_brush (widget, ToolBarXBak*(BLOCK_WIDTH_DEF+2) +2, ToolBarYBak*(BLOCK_HEIGHT_DEF+2) +2, widget->style->white_gc);
			if (ToolBarX>1)
				ToolBarX = 1;
			draw_brush (widget, ToolBarX*(BLOCK_WIDTH_DEF+2) +2, ToolBarY*(BLOCK_HEIGHT_DEF+2) +2, widget->style->black_gc);
			if (ToolBarY<=ToolBarYMaxi)
				EditDatas.NumElementSelectedInToolBar = PtrToolBarElementsList[ToolBarY][ToolBarX];
			else
