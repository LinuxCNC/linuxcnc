...
void LoadSeqElementProperties(void)
{
    ...
    if (TypeSeqEleEdited != -1) {
        ...
        case ELE_SEQ_COMMENT:
            SetProperty(0,
                        _("Comment"),
                        EditSeqDatas.SeqComment[OffsetSeqEleEdited].Comment,
                        TRUE);
            break;
        }
    }
...
void SaveSeqElementProperties(void)
{
    ...
    if (TypeSeqEleEdited != -1) {
        ...
        case ELE_SEQ_TRANSITION:
            ...
            break;
        case ELE_SEQ_COMMENT:
            strncpy(EditSeqDatas.SeqComment[OffsetSeqEleEdited].Comment,
                    GetProperty(0),
                    SEQ_COMMENT_LGT);
            EditSeqDatas.SeqComment[OffsetSeqEleEdited]
                .Comment[SEQ_COMMENT_LGT - 1] = '\0';
            break;
        }
...
int SearchCommentElement(int PageNumber, int PositionX, int PositionY)
{
    ...
    for (ScanComment = 0; ScanComment < NBR_SEQ_COMMENTS; ScanComment++) {
        ...
        if (pComment->NumPage == PageNumber) {
            if ((pComment->PosiX <= PositionX &&
                 PositionX < pComment->PosiX + 4) &&
                pComment->PosiY == PositionY)
                Result = ScanComment;
        }
    }
    printf(_("found=%d!!!\n"), Result);
...
int CreateComment(int page, int x, int y)
{
    ...
    if (OffsetCommentCreated != -1) {
        StrSeqComment *pComment =
            &EditSeqDatas.SeqComment[OffsetCommentCreated];
...
void DoLinkTransitionAndStep(int OffsetTransi, char TopOfTransi, int OffsetStep)
{
    ...
    if (TopOfTransi) {
        if (pTransi->NumStepToDesactiv[0] == -1)
            ...
            ShowMessageBoxError(
                _("There is already a step to deactivate for this transition "
                  "(clicked on top part)..."));
    } else {
        if (pTransi->NumStepToActiv[0] == -1)
            ...
            ShowMessageBoxError(
                _("There is already a step to activate for this transition "
                  "(clicked on bottom part)..."));
    }
}

/* return TRUE is okay */
char CommonSearchForManyStepsOrTransi(char ForManySteps,
                                      int TypeEle1,
                                      int OffEle1,
                                      int TypeEle2,
                                      int OffEle2,
                                      int *pOffsetTransiFound,
                                      int *pStepsBaseY,
                                      int *pTransitionsBaseY,
                                      int *pLeftX,
                                      int *pRightX)
{
    ...
    int LeftX, RightX;
    if (!ForManySteps &&
        (TypeEle1 == ELE_SEQ_STEP || TypeEle2 == ELE_SEQ_STEP)) {
        ShowMessageBoxError(
            _("Not selected first and last transitions to be joined !!??"));
        return FALSE;
    }
    ...
    if (TypeEle2 == ELE_SEQ_STEP) {
        ...
        if (StepsBaseY == -1) {
            ...
            if (StepsBaseY != EditSeqDatas.Step[OffEle1].PosiY) {
                ShowMessageBoxError(
                    _("First and last steps selected are not on the same line "
                      "!!??"));
                return FALSE;
            }
        }
        ...
        if (TransitionsBaseY == -1) {
            ...
            if (TransitionsBaseY != EditSeqDatas.Transition[OffEle1].PosiY) {
                ShowMessageBoxError(
                    _("First and last transitions selected are not on the same "
                      "line !!??"));
                return FALSE;
            }
        }
    ...
    *pLeftX = LeftX;
    *pRightX = RightX;
    printf(_("commonsearch: leftX=%d, rightX=%d, OffTransi=%d, StepsY=%d, "
             "TransiY=%d\n"),
           LeftX,
           RightX,
           OffsetTransiFound,
...
void DoManyStepsActOrDesact(int ForPage,
                            int ToolBarFlagStart,
                            int TypeEle1,
                            int OffEle1,
                            int TypeEle2,
                            int OffEle2)
{
    ...
    if (!CommonSearchForManyStepsOrTransi(TRUE,
                                          TypeEle1,
                                          OffEle1,
                                          TypeEle2,
                                          OffEle2,
...
void DoManyTransitionsLinked(int ForPage,
                             int ToolBarFlagStart,
                             int TypeEle1,
                             int OffEle1,
                             int TypeEle2,
                             int OffEle2)
{
    ...
    if (!CommonSearchForManyStepsOrTransi(FALSE /*TRUE*/,
                                          TypeEle1,
                                          OffEle1,
                                          TypeEle2,
                                          OffEle2,
    ...
    if (TransisBaseY == -1) {
        ...
        if (NbrTransisLinked >= 2) {
            ...
            for (ScanCurrentTransiArray = 0;
                 ScanCurrentTransiArray < NbrTransisLinked;
                 ScanCurrentTransiArray++) {
                int TheCurrentTransi =
                    ArrayNumTransiLinked[ScanCurrentTransiArray];
                int StepToAct = -1;
                int StepToDesact = -1;
                StrTransition *pTheCurrentTransi =
                    &EditSeqDatas.Transition[TheCurrentTransi];
                // init all
                for (ScanTransi = 0; ScanTransi < NBR_SWITCHS_MAX;
                     ScanTransi++) {
                    if (ToolBarFlagStart)
                        pTheCurrentTransi->NumTransLinkedForStart[ScanTransi] =
                            -1;
                    else
                        pTheCurrentTransi->NumTransLinkedForEnd[ScanTransi] =
                            -1;
                }

                // put the others transitions than itself
                ScanTransi = 0;
                for (ScanOtherTransiArray = 0;
                     ScanOtherTransiArray < NbrTransisLinked;
                     ScanOtherTransiArray++) {
                    int NumOtherTransi =
                        ArrayNumTransiLinked[ScanOtherTransiArray];
                    printf(_("having num transi linked=%d for transi=%d\n"),
                           NumOtherTransi,
                           TheCurrentTransi);
                    if (NumOtherTransi != TheCurrentTransi) {
                        printf(
                            _("->storing num transi linked=%d for transi=%d\n"),
                            NumOtherTransi,
                            TheCurrentTransi);
                        if (ToolBarFlagStart) {
                            pTheCurrentTransi
                                ->NumTransLinkedForStart[ScanTransi++] =
                                NumOtherTransi;
                            if (EditSeqDatas.Transition[NumOtherTransi]
                                    .NumStepToDesactiv[0] != -1)
                                StepToDesact =
                                    EditSeqDatas.Transition[NumOtherTransi]
                                        .NumStepToDesactiv[0];
                        } else {
                            pTheCurrentTransi
                                ->NumTransLinkedForEnd[ScanTransi++] =
                                NumOtherTransi;
                            if (EditSeqDatas.Transition[NumOtherTransi]
                                    .NumStepToActiv[0] != -1)
                                StepToAct =
            ...
            ShowMessageBoxError(
                _("Not found at least 2 transitions linked..."));
        }
    }
}
...
void EditElementInSeqPage(double x, double y)
{
    ...
    if ((PosX < SEQ_PAGE_WIDTH) && (PosY < SEQ_PAGE_HEIGHT) &&
        ...
        int TypeFound = -1;
        int OffsetFound = -1;
        char ClickedOnTopPart = y > PosY * SEQ_SIZE_DEF &&
        ...
        case EDIT_SEQ_STEP_AND_TRANS:
                        ...
                        if (TypeFound == ELE_SEQ_STEP) {
                            ...
                            if (TypeFound == -1) {
                                ...
                                if (OffsetFound != -1) {
                                    ...
                                    ShowMessageBoxError(
                                        _("Sequential memory full for steps"));
                                }
                            } else {
                                ShowMessageBoxError(
                                    _("There is already an element!"));
                            }
                        }
                    }
                } else {
                    ShowMessageBoxError(
                        _("A step can't be placed on even lines"));
                }
            }
            if (EditDatas.NumElementSelectedInToolBar == ELE_SEQ_TRANSITION ||
                ...
                if ((PosY & 1) == 0) {
                    ...
                    ShowMessageBoxError(
                        _("A transition can't be placed on odd lines"));
                }
            }
            break;
        case EDIT_SEQ_LINK:
            ...
            if (CptNbrClicksDone == 1) {
                printf(_("nbr clicks=1!!! (posi=%s), wait next point to "
                         "link...\n"),
                ...
                if (TypeFound == ELE_SEQ_STEP)
                    ...
                    MessageInStatusBar(
                        ClickedOnTopPart
                            ? (_("Now select the step that will be deactivated "
                                 "by this transition."))
                            : (_("Now select the step that will be activated "
                                 "by this transition.")));
                else
                    ShowMessageBoxError(_("You haven't selected a step or a "
                                          "transition to link!!!"));
            }
            if (CptNbrClicksDone == 2) {
                ...
                if (TypeSeqEleEditedBak == ELE_SEQ_TRANSITION &&
                    ...
                    DoLinkTransitionAndStep(OffsetSeqEleEditedBak,
                                            TopPartSeqEleEditedBak,
                                            OffsetFound);
                else if (TypeSeqEleEditedBak == ELE_SEQ_STEP &&
                         TypeFound == ELE_SEQ_TRANSITION)
                    DoLinkTransitionAndStep(
                        OffsetFound, ClickedOnTopPart, OffsetSeqEleEditedBak);
                else
                    ShowMessageBoxError(
                        _("You haven't selected a transition and then the step "
                          "to link!!!"));
                CptNbrClicksDone = 0;
                MessageInStatusBar("");
            }
            ...
            if (PosX <= SEQ_PAGE_WIDTH - 4) {
                ...
                if (SomethingHere == FALSE) {
                    ...
                    if (OffsetFound != -1) {
                        ...
                        ShowMessageBoxError(
                            _("Sequential memory full for comments"));
                    }
                } else {
                    ShowMessageBoxError(
                        _("There is already an element on 4 horizontal blocks "
                          "required!"));
                }
            } else {
        ...
        case EDIT_SEQ_END_MANY_TRANS:
            ...
            if (CptNbrClicksDone == 2) {
                if (EditDatas.NumElementSelectedInToolBar ==
                    ...
                    printf(_("DO_MANY_STEPS:nbr clicks=2!!!, TypeBak=%d, "
                             "TypeNow=%d\n"),
                           TypeSeqEleEditedBak,
                           TypeFound);
                    DoManyStepsActOrDesact(
                        CurrentSeqPage,
                        EditDatas.NumElementSelectedInToolBar ==
                            EDIT_SEQ_START_MANY_STEPS,
                        TypeSeqEleEditedBak,
                    ...
                    printf(_("DO_MANY_TRANSITIONS:nbr clicks=2!!!, TypeBak=%d, "
                             "TypeNow=%d\n"),
                           TypeSeqEleEditedBak,
                           TypeFound);
                    DoManyTransitionsLinked(
                        CurrentSeqPage,
                        EditDatas.NumElementSelectedInToolBar ==
        ...
        if (TypeSeqEleEdited != -1) {
            ...
            EditDatas.CurrentElementSizeX =
                (TypeSeqEleEdited == ELE_SEQ_COMMENT) ? 4 : 1;
