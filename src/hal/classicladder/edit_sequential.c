	...
	if ( TypeSeqEleEdited!=-1 )
	{
		switch( TypeSeqEleEdited )
		{
			case ELE_SEQ_STEP:
				...
				break;
			case ELE_SEQ_TRANSITION:
				TextParserForAVar( GetProperty(0),&EditSeqDatas.Transition[ OffsetSeqEleEdited ].VarTypeCondi,
							&EditSeqDatas.Transition[ OffsetSeqEleEdited ].VarNumCondi );
				break;
		}
...
//printf("top clicked= %d\n", TopFound );
		...
		{
			OffsetFound = SearchTransiElement( SectionArray[ InfosGene->CurrentSection ].SequentialPage, PosX, PosY );
			if ( OffsetFound!=-1 )
			{
