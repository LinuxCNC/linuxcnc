...
//ForGTK3
#if GTK_MAJOR_VERSION >= 3
    InfosGene->BlockWidth =
        ((gtk_widget_get_allocated_width(drawing_area) * 995 / 1000) /
         RUNG_WIDTH);
#else
    InfosGene->BlockWidth =
        ((GTK_WIDGET(drawing_area)->allocation.width * 995 / 1000) /
         RUNG_WIDTH);
#endif
    // keep ratio aspect (if defaults values of size block not square)
    InfosGene->BlockHeight =
        InfosGene->BlockWidth * BLOCK_HEIGHT_DEF / BLOCK_WIDTH_DEF;
...
void GetCurrentNumAndNbrRungsForCurrentSection(int *pCurrNumRung,
                                               int *pNbrRungs)
{
...
void UpdateVScrollBar()
{
    ...
    if (iCurrentLanguage == SECTION_IN_LADDER) {
        ...
        gtk_adjustment_set_step_increment(AdjustVScrollBar,
                                          InfosGene->BlockHeight);
        gtk_adjustment_set_page_increment(AdjustVScrollBar,
                                          TOTAL_PX_RUNG_HEIGHT);
    ...
    if (iCurrentLanguage == SECTION_IN_SEQUENTIAL) {
        ...
        gtk_adjustment_set_lower(AdjustVScrollBar, 0);
        gtk_adjustment_set_upper(AdjustVScrollBar,
                                 SEQ_PAGE_HEIGHT * SEQ_SIZE_DEF);
        ...
        gtk_adjustment_set_upper(AdjustHScrollBar,
                                 SEQ_PAGE_WIDTH * SEQ_SIZE_DEF);
        gtk_adjustment_set_value(AdjustHScrollBar, 0);
        gtk_adjustment_set_step_increment(AdjustHScrollBar, SEQ_SIZE_DEF);
        gtk_adjustment_set_page_increment(AdjustHScrollBar,
                                          InfosGene->PageWidth);
...
void ChoiceOfTheCurrentRung(int NbrOfRungsAfterTopRung)
{
    ...
    InfosGene->OffsetCurrentRungDisplayed =
        -1 * InfosGene->OffsetHiddenTopRungDisplayed;
                                           ...
                                           void *not_used)
{
    ...
    if (iCurrentLanguage == SECTION_IN_LADDER) {
        int NumRung =
            ((int)gtk_adjustment_get_value(ScrollBar)) / TOTAL_PX_RUNG_HEIGHT;
        int ScanRung = 0;
        ...
        ChoiceOfTheCurrentRung(
            (InfosGene->OffsetHiddenTopRungDisplayed > 0) ? 1 : 0);
    }
...
static void IncrementVScrollBar(int IncrementValue)
{
    ...
    if (IncrementValue != 0) {
        gtk_adjustment_set_value(AdjustVScrollBar,
                                 gtk_adjustment_get_value(AdjustVScrollBar) +
                                     IncrementValue);
        if (IncrementValue > 0) {
            if (gtk_adjustment_get_value(AdjustVScrollBar) >
                (gtk_adjustment_get_upper(AdjustVScrollBar) -
                 InfosGene->PageHeight))
            ...
            if (gtk_adjustment_get_value(AdjustVScrollBar) <
                gtk_adjustment_get_lower(AdjustVScrollBar))
                gtk_adjustment_set_value(
                    AdjustVScrollBar,
                    gtk_adjustment_get_lower(AdjustVScrollBar));
        }
...
static gboolean mouse_scroll_event(GtkWidget *widget, GdkEventScroll *event)
{
    ...
    if (iCurrentLanguage == SECTION_IN_LADDER) {
        if (event->direction == GDK_SCROLL_DOWN)
            IncrementVScrollBar(
                gtk_adjustment_get_step_increment(AdjustVScrollBar));
        else if (event->direction == GDK_SCROLL_UP)
            IncrementVScrollBar(
                -1 * gtk_adjustment_get_step_increment(AdjustVScrollBar));
    }
    return TRUE;
}
    ...
    if (event->button == 1 /*Cairo && pixmap != NULL*/) {
        if (EditDatas.ModeEdit) {
            ...
            int iCurrentLanguage =
                SectionArray[InfosGene->CurrentSection].Language;
            if (iCurrentLanguage == SECTION_IN_LADDER) {
                char DoSelection = TRUE;
                if (InfosGene->OffsetHiddenTopRungDisplayed > 0) {
                    if (event->y < TOTAL_PX_RUNG_HEIGHT -
                                       InfosGene->OffsetHiddenTopRungDisplayed)
                        DoSelection = FALSE;
                }
                if (DoSelection) {
                    int NbrRungsShift =
                        (event->y + InfosGene->OffsetHiddenTopRungDisplayed) /
                        TOTAL_PX_RUNG_HEIGHT;
                                    ...
                                    gpointer user_data)
{
    ...
    if (EditDatas.ModeEdit) {
        ...
        if (SectionArray[InfosGene->CurrentSection].Language ==
            SECTION_IN_LADDER) {
            char *pLadderProperties =
                GetLadderElePropertiesForStatusBar(event->x, event->y);
...
{
    rtapi_strxcpy(EditDatas.Rung.Label,
                  gtk_entry_get_text((GtkEntry *)entrylabel));
    rtapi_strxcpy(EditDatas.Rung.Comment,
...
void CheckDispSymbols_toggled()
{
    InfosGene->DisplaySymbols =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(CheckDispSymbols));
}
...
void LoadNewLadder()
{
    ...
    StoreDirectorySelected(GTK_FILE_CHOOSER(FileSelector),
    ...
    if (!ProjectLoadedOk)
        ShowMessageBox(
            _("Load Error"), _("Failed to load the project file..."), _("Ok"));

    UpdateAllGtkWindows();
    UpdateWindowTitleWithProjectName();
    MessageInStatusBar(ProjectLoadedOk ? _("Project loaded (stopped).")
                                       : _("Project failed to load..."));
#ifndef RT_SUPPORT
    OpenHardware(0);
    //        ConfigHardware( );
...
void DoActionSave()
{
    if (!SaveProjectFiles(InfosGene->CurrentProjectFileName))
        ShowMessageBox(
            _("Save Error"), _("Failed to save the project file..."), _("Ok"));
}
...
void on_filechooserdialog_save_response(GtkDialog *dialog,
                                        gint response_id,
                                        gpointer user_data)
{
    (void)user_data;
...
void on_filechooserdialog_load_response(GtkDialog *dialog,
                                        gint response_id,
                                        gpointer user_data)
{
    (void)user_data;
...
void CreateFileSelection(char *Prompt, int CreateFileSelectionType)
{
    ...
    if (CreateFileSelectionType == CREATE_FILE_SELECTION_TO_SAVE_PROJECT) {
        FileSelector = gtk_file_chooser_dialog_new(Prompt,
                                                   NULL,
        ...
        gtk_file_chooser_set_do_overwrite_confirmation(
            GTK_FILE_CHOOSER(FileSelector), TRUE);
    } else {
        FileSelector = gtk_file_chooser_dialog_new(Prompt,
                                                   NULL,
    ...
    gtk_window_set_type_hint(GTK_WINDOW(FileSelector),
    ...
    gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(FileSelector),
                                FilterOldProjects);
  ...
  g_signal_connect ((gpointer) filechooserdialog, "file_activated",
    ...
    if (CreateFileSelectionType == CREATE_FILE_SELECTION_TO_SAVE_PROJECT)
        g_signal_connect((gpointer)FileSelector,
                         "response",
    ...
    g_signal_connect_swapped((gpointer)FileSelector,
                             "close",
...
{
    ShowConfirmationBox(
        _("New"), _("Do you really want to clear all data ?"), DoNewProject);
}
void DoLoadProject()
{
...
void DoActionLoadProject()
{
    if (InfosGene->AskConfirmationToQuit)
        ShowConfirmationBox(
            _("Sure?"),
            _("Do you really want to load another project ?\nIf not saved, all "
              "modifications on the current project will be lost  \n"),
            DoLoadProject);
    else
        DoLoadProject();
}
...
void DoReset()
{
    ...
    RunBackIfStopped();
    MessageInStatusBar((InfosGene->LadderState == STATE_RUN)
                           ? _("Reset logic data - Now running.")
                           : _("Reset logic data done."));
}
...
*/
    ...
    if (comments) {
        ...
        snprintf(GtkVersionString,
                 sizeof(GtkVersionString),
                 "GTK+ version %d.%d.%d\n\n",
                 gtk_major_version,
                 gtk_minor_version,
                 gtk_micro_version);
        ...
        gtk_show_about_dialog(
            ...
            CL_PRODUCT_NAME,
            "version",
            CL_RELEASE_VER_STRING,
            "copyright",
            "Copyright (C) " CL_RELEASE_COPYRIGHT_YEARS
            " Marc Le Douarain\nmarc . le - douarain /At\\ laposte \\DoT/ net",
            //							"logo", example_logo,
            "title",
            _("About ClassicLadder"),
            "website",
...
cairo_t *
InitExportSurface(int SurfaceWidth, int SurfaceHeight, char *SvgFileToCreate)
{
    cairo_t *cr;
    if (SvgFileToCreate) {
        ExportSurface = cairo_svg_surface_create(
            SvgFileToCreate, SurfaceWidth, SurfaceHeight);
        cr = cairo_create(ExportSurface);
    } else {
        ExportSurface = cairo_image_surface_create(
            CAIRO_FORMAT_RGB24, SurfaceWidth, SurfaceHeight);
        cr = cairo_create(ExportSurface);
        //cleanup
...
void ExportSvgOrPngFile(char *FileToCreate, char GoForSvgExport)
{
    ...
    if (iCurrentLanguage == SECTION_IN_LADDER) {
        ...
        int SvgHeightTotal =
            RUNG_HEIGHT * BLOCK_HEIGHT_DEF + BLOCK_HEIGHT_DEF / 2;
        int LeftRightBarsWidth = BLOCK_WIDTH_DEF / 16;
        cr = InitExportSurface(SvgWidthTotal,
                               SvgHeightTotal,
                               GoForSvgExport ? FileToCreate : NULL);
        DrawLeftRightBars(cr,
                          0,
                          0,
                          BLOCK_WIDTH_DEF,
                          BLOCK_HEIGHT_DEF,
                          ...
                          FALSE);
        DrawRung(cr,
                 &RungArray[InfosGene->CurrentRung],
                 LeftRightBarsWidth,
                 0,
                 BLOCK_WIDTH_DEF,
                 BLOCK_HEIGHT_DEF,
...
{
    GtkWidget *dialog;
    dialog = gtk_file_chooser_dialog_new(GoForSvg ? _("Save SVG File")
                                                  : _("Save PNG File"),
                                         GTK_WINDOW(MainSectionWindow),
                                         GTK_FILE_CHOOSER_ACTION_SAVE,
                                         GTK_STOCK_CANCEL,
    ...
    gtk_file_chooser_set_current_name(GTK_FILE_CHOOSER(dialog),
                                      GoForSvg ? "classicladder_export.svg"
                                               : "classicladder_export.png");
...
void DoFunctionOfConfirmationBox(GtkWidget *widget,
                                 void *(*function_to_do)(void *))
{
    (void)widget;
    gtk_widget_destroy(ConfirmDialog);
    (function_to_do)(NULL);
}
void ShowConfirmationBoxWithChoiceOrNot(const char *title,
                                        const char *text,
                                        void *function_if_yes,
                                        char HaveTheChoice)
{
    /* From the example in gtkdialog help */
    ...
    if (HaveTheChoice) {
        ...
        g_signal_connect_swapped(GTK_OBJECT(no_button),
                                 "clicked",
                                 GTK_SIGNAL_FUNC(gtk_widget_destroy),
                                 GTK_OBJECT(ConfirmDialog));
...
void ShowConfirmationBox(const char *title,
                         const char *text,
                         void *function_if_yes)
{
...
{
    gtk_widget_destroy(
        MainSectionWindow); //sends signal "destroy" that will call QuitAppliGtk()...
}
void ConfirmQuit(void)
{
    if (InfosGene->AskConfirmationToQuit)
        ShowConfirmationBox(
            _("Warning!"),
            _("If not saved, all modifications will be lost.\nDo you really "
              "want to quit ?\n"),
            DoQuitGtkApplication);
    else if (EditDatas.ModeEdit == TRUE) {
        if (!modmaster) {
            ShowConfirmationBox(_("Confirm!"),
                                _("Do you really want to quit ?\n"),
                                DoQuitGtkApplication);
        } else {
            ShowConfirmationBox(
                _("Warning!"),
                _("MODBUS will stop if you quit.\n Do you really want to quit "
                  "?\n"),
                DoQuitGtkApplication);
        }
    } else
...
gint MainSectionWindowDeleteEvent(GtkWidget *widget,
                                  GdkEvent *event,
                                  gpointer data)
{
    (void)widget;
...
void MainSectionWindowInitGtk()
{
    ...
    gtk_window_set_title(GTK_WINDOW(MainSectionWindow),
                         _("ClassicLadder Section Display"));

    vbox = gtk_vbox_new(FALSE, 0);
    gtk_container_add(GTK_CONTAINER(MainSectionWindow), vbox);
    gtk_widget_show(vbox);

    gtk_signal_connect(GTK_OBJECT(MainSectionWindow),
                       "destroy",
                       GTK_SIGNAL_FUNC(QuitAppliGtk),
                       NULL);
    ...
    gtk_widget_set_tooltip_text(entrylabel,
                                _("Label of the current selected rung"));
    gtk_widget_show(entrylabel);
    ...
    gtk_widget_set_tooltip_text(
        entrycomment,
        _("Comment of the current selected ladder rung or sequential page"));
    gtk_widget_show(entrycomment);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(CheckDispSymbols),
                                 InfosGene->DisplaySymbols);
    gtk_signal_connect(GTK_OBJECT(CheckDispSymbols),
                       "toggled",
                       GTK_SIGNAL_FUNC(CheckDispSymbols_toggled),
                       NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(AdjustVScrollBar),
                       "value-changed",
                       GTK_SIGNAL_FUNC(VScrollBar_value_changed_event),
                       0);
    gtk_signal_connect(GTK_OBJECT(AdjustHScrollBar),
                       "value-changed",
                       GTK_SIGNAL_FUNC(HScrollBar_value_changed_event),
                       0);
    ...
    StatusBarContextId =
        gtk_statusbar_get_context_id(GTK_STATUSBAR(StatusBar), _("Statusbar"));
...
//no more used since menu/toolbar added...
    ...
    gtk_signal_connect(
        GTK_OBJECT(ButtonNew), "clicked", (GtkSignalFunc)ButtonNew_click, 0);
    ...
    gtk_signal_connect(
        GTK_OBJECT(ButtonLoad), "clicked", (GtkSignalFunc)ButtonLoad_click, 0);
    ...
    gtk_signal_connect(
        GTK_OBJECT(ButtonSave), "clicked", (GtkSignalFunc)ButtonSave_click, 0);
    gtk_widget_show(ButtonSave);
    ButtonSaveAs = gtk_button_new_with_label(_("Save As"));
    gtk_box_pack_start(GTK_BOX(hboxbottom), ButtonSaveAs, TRUE, TRUE, 0);
    gtk_signal_connect(GTK_OBJECT(ButtonSaveAs),
                       "clicked",
                       (GtkSignalFunc)ButtonSaveAs_click,
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonReset),
                       "clicked",
                       (GtkSignalFunc)ButtonReset_click,
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonRunStop),
                       "clicked",
                       (GtkSignalFunc)ButtonRunStop_click,
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonSpyVars),
                       "clicked",
                       (GtkSignalFunc)OpenSpyVarsWindow,
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonAbout),
                       "clicked",
                       (GtkSignalFunc)ButtonAbout_click,
                       0);
    ...
    gtk_signal_connect(
        GTK_OBJECT(ButtonEdit), "clicked", (GtkSignalFunc)OpenEditWindow, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonSymbols),
                       "clicked",
                       (GtkSignalFunc)OpenSymbolsWindow,
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonConfig),
                       "clicked",
                       (GtkSignalFunc)ButtonConfig_click,
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonExportSVG),
                       "clicked",
                       (GtkSignalFunc)ButtonExportSvgOrPng_click,
                       (void *)1);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonExportPNG),
                       "clicked",
                       (GtkSignalFunc)ButtonExportSvgOrPng_click,
                       0);
    ...
    gtk_box_pack_start(
        GTK_BOX(hboxbottom2), ButtonCopyToClipboard, TRUE, TRUE, 0);
    gtk_signal_connect(GTK_OBJECT(ButtonCopyToClipboard),
                       "clicked",
                       (GtkSignalFunc)ButtonCopyToClipboard_click,
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonPrintPreview),
                       "clicked",
                       (GtkSignalFunc)PrintPreviewGnome,
                       0);
    ...
    gtk_signal_connect(
        GTK_OBJECT(ButtonPrint), "clicked", (GtkSignalFunc)PrintGnome, 0);
    ...
    gtk_signal_connect_object(
        GTK_OBJECT(ButtonQuit), "clicked", ConfirmQuit, NULL);
...
#if GTK_MAJOR_VERSION >= 3
    gtk_signal_connect(
        GTK_OBJECT(drawing_area), "draw", GTK_SIGNAL_FUNC(draw_callback), NULL);
#else
    ...
    gtk_signal_connect(GTK_OBJECT(drawing_area),
                       "scroll_event",
                       GTK_SIGNAL_FUNC(mouse_scroll_event),
                       NULL);
    gtk_widget_set_events(
        drawing_area,
        GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_BUTTON_PRESS_MASK |
            GDK_BUTTON_RELEASE_MASK | GDK_SCROLL_MASK // mouse scroll
            | GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK);

    gtk_signal_connect(GTK_OBJECT(MainSectionWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(MainSectionWindowDeleteEvent),
                       0);
...
static gint PeriodicUpdateDisplay(gpointer data)
{
    ...
    if (InfosGene->LadderState == STATE_RUN) {
        ...
        snprintf(TextBuffer,
                 sizeof(TextBuffer),
                 _("%d µs"),
                 InfosGene->DurationOfLastScan / 1000);
    ...
    if (InfosGene->HardwareErrMsgToDisplay[0] != '\0') {
        ShowMessageBox(_("Config hardware error occurred!"),
                       InfosGene->HardwareErrMsgToDisplay,
                       _("Ok"));
...
void UpdateWindowTitleWithProjectName(void)
{
    ...
    if (strlen(InfosGene->CurrentProjectFileName) > 2) {
        ...
        while (ScanFileNameOnly > 0 &&
               InfosGene->CurrentProjectFileName[ScanFileNameOnly - 1] != '/' &&
               InfosGene->CurrentProjectFileName[ScanFileNameOnly - 1] != '\\')
            ScanFileNameOnly--;
    }
    snprintf(Buff,
             sizeof(Buff),
             _("Section Display of %s"),
             &InfosGene->CurrentProjectFileName[ScanFileNameOnly]);
...
void ShowErrorMessage(const char *title, const char *text, const char *button)
{
    ...
    g_signal_connect_swapped(
        okay_button, "clicked", G_CALLBACK(ErrorMessageDeleteEvent), dialog);
    g_signal_connect(
        dialog, "delete_event", G_CALLBACK(ErrorMessageDeleteEvent), 0);
    gtk_container_add(
        GTK_CONTAINER(gtk_dialog_get_action_area(GTK_DIALOG(dialog))),
        okay_button);
    gtk_widget_grab_focus(okay_button);
