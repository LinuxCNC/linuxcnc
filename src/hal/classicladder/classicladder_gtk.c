...
//printf("OffsetHiddenTopRungDisplayed=%d\n",InfosGene->OffsetHiddenTopRungDisplayed);
    ...
    {
        if ( InfosGene->CurrentRung!=InfosGene->LastRung && InfosGene->PageHeight+InfosGene->OffsetHiddenTopRungDisplayed>=InfosGene->BlockHeight*RUNG_HEIGHT)
        {
            InfosGene->CurrentRung = RungArray[ InfosGene->CurrentRung ].NextRung;
            InfosGene->OffsetCurrentRungDisplayed = (InfosGene->BlockHeight*RUNG_HEIGHT) - InfosGene->OffsetHiddenTopRungDisplayed;
        }
        else
        {
            /* The current rung is not displayed entierely (at least the top of it...) */
            InfosGene->OffsetCurrentRungDisplayed = InfosGene->OffsetHiddenTopRungDisplayed * -1;
        }
    }
...
void StoreDirectorySelected(GtkFileSelection *selector, char cForLoadingProject)
{
    char * TempDir;
    TempDir = (char *)gtk_file_selection_get_filename (GTK_FILE_SELECTION(FileSelector));
...
void RungWindowInitGtk()
{
    ...
    GtkWidget *ButtonNew,*ButtonLoad,*ButtonSave,*ButtonSaveAs,*ButtonConfig,*ButtonAbout;
...
//    gtk_entry_set_max_length((GtkEntry *)DurationOfLastScan,20);
    ...
    ButtonRunStop = gtk_button_new_with_label ((InfosGene->LadderState==STATE_RUN)? "Stop": "Run");
...
void VarsWindowInitGtk()
{
	...
	for(NumVarSpy=0; NumVarSpy<NBR_FREE_VAR_SPY; NumVarSpy++)
	{
		for(ColumnVar=0; ColumnVar<2; ColumnVar++)
		{
			...
			gtk_box_pack_start ((ColumnVar==0)?GTK_BOX(vboxvarsnames):GTK_BOX(vboxvarsvalues), EntryVarSpy[ NumEntry ], FALSE, FALSE, 0);
		...
		gtk_widget_set_usize((GtkWidget *)DisplayFormatVarSpy[NumVarSpy],40,0);
		gtk_box_pack_start (GTK_BOX(vboxvarsformats), DisplayFormatVarSpy[NumVarSpy], FALSE, FALSE, 0);
