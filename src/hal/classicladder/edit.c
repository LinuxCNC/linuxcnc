...
void LoadElementProperties(StrElement *Element)
{
    ...
    if (Element) {
        ...
        case ELE_OUTPUT_RESET:
            rtapi_strxcpy(TextToWrite,
                          CreateVarName(Element->VarType,
                                        Element->VarNum,
        ...
        case ELE_OUTPUT_JUMP:
            SetProperty(
                0, _("JumpToLabel"), RungArray[Element->VarNum].Label, TRUE);
            break;
        ...
        case ELE_TIMER:
            ...
            snprintf(TextToWrite,
                     sizeof(TextToWrite),
                     "%s",
                     CorresDatasForBase[ConvBaseInMilliSecsToId(Timer->Base)]
                         .ParamSelect);
            SetProperty(1, _("Base"), TextToWrite, FALSE);
            snprintf(TextToWrite,
                     sizeof(TextToWrite),
                     "%d",
                     Timer->Preset / Timer->Base);
        ...
        case ELE_MONOSTABLE:
            ...
            snprintf(
                ...
                sizeof(TextToWrite),
                "%s",
                CorresDatasForBase[ConvBaseInMilliSecsToId(Monostable->Base)]
                    .ParamSelect);
            SetProperty(1, _("Base"), TextToWrite, FALSE);
            snprintf(TextToWrite,
                     sizeof(TextToWrite),
                     "%d",
                     Monostable->Preset / Monostable->Base);
            SetProperty(2, _("Preset"), TextToWrite, TRUE);
            break;
#endif
        case ELE_COUNTER:
        ...
        case ELE_TIMER_IEC:
            ...
            snprintf(TextToWrite,
                     sizeof(TextToWrite),
                     "%s",
                     CorresDatasForBase[ConvBaseInMilliSecsToId(TimerIEC->Base)]
                         .ParamSelect);
            SetProperty(1, _("Base"), TextToWrite, FALSE);
...
char *GetElementPropertiesForStatusBar(StrElement *Element)
{
    ...
    if (Element) {
        ...
        case ELE_OUTPUT_SET:
        case ELE_OUTPUT_RESET:
            snprintf(PropertiesText,
                     sizeof(PropertiesText),
                     _("Variable: %s    Hal sig: %s"),
                     CreateVarName(Element->VarType,
                                   Element->VarNum,
                                   InfosGene->DisplaySymbols),
                     ConvVarNameToHalSigName(
                         CreateVarName(Element->VarType,
                                       Element->VarNum,
                                       InfosGene->DisplaySymbols)));
        ...
        case ELE_TIMER_IEC:
            ...
            snprintf(PropertiesText,
                     sizeof(PropertiesText),
                     _("%cTM%d: Preset=%d Base=%s Mode=%s"),
                     '%',
                     Element->VarNum,
                     TimerIEC->Preset,
                     CorresDatasForBase[ConvBaseInMilliSecsToId(TimerIEC->Base)]
        ...
        case ELE_OUTPUT_OPERATE:
            snprintf(PropertiesText,
                     sizeof(PropertiesText),
                     _("HAL sig: %s"),
                     FirstVariableInArithm(DisplayArithmExpr(
                         ArithmExpr[Element->VarNum].Expr, 0)));
    ...
    do {
        ...
        if ((*ptr >= 'A' && *ptr <= 'Z') || (*ptr >= 'a' && *ptr <= 'z')) {
            ...
            do {
                CharIn = FALSE;
                End++;
                if (*End == VAR_ATTRIBUTE_SEP || (*End >= 'A' && *End <= 'Z') ||
                    (*End >= 'a' && *End <= 'z') ||
                    (*End >= '0' && *End <= '9') || *End == '_')
                    CharIn = TRUE;
                if (CharIn)
        ...
        if (SymbolFound == TRUE || *ptr == '%') {
            ...
            if (TextParserForAVar(SymbolFound == TRUE ? SymbolBuff : ptr,
                                  &VarType,
                                  &VarOffset,
                                  &StringLength,
                                  FALSE /*PartialNames*/
                ...
                {
                    ItIsOk = FALSE;
                    ErrorMessageVarParser = _(
                        "Incompatible type of variable (must be an integer!)");
                }
            } else {
...
void SaveElementProperties()
{
    ...
    if (EditDatas.ElementUnderEdit) {
        ...
        case ELE_OUTPUT_SET:
        case ELE_OUTPUT_RESET:
            if (TextParserForAVar(GetProperty(0),
                                  &VarTypeEntered,
                                  &VarNumEntered,
                                  NULL,
                                  FALSE /*PartialNames*/
                ...
                if (*pStartIndexed++ == '[') {
                    ...
                    if (TextParserForAVar(pStartIndexed,
                                          &IndexedVarTypeEntered,
                                          &IndexedVarNumEntered,
                                          NULL,
                                          FALSE /*PartialNames*/
                                          )) {
                        if (TEST_VAR_IS_A_BOOL(IndexedVarTypeEntered,
                                               IndexedVarNumEntered)) {
                            ShowMessageBoxError(
                                _("Incompatible type of variable for index "
                                  "(must be an integer!)"));
                            cCorrectRules = FALSE;
                        }
                    } else {
                        ShowMessageBoxError(
                            _("Parser error for indexed variable !"));
                        cCorrectRules = FALSE;
                    }
                }
                if (!TEST_VAR_IS_A_BOOL(VarTypeEntered, VarNumEntered)) {
                    ShowMessageBoxError(
                        _("You must select a boolean variable !"));
                    cCorrectRules = FALSE;
                }
                if ((EditDatas.ElementUnderEdit->Type == ELE_OUTPUT ||
                    ...
                    ShowMessageBoxError(
                        _("You must select a read/write variable for a coil!"));
                    cCorrectRules = FALSE;
                }
                if (cCorrectRules) {
                    EditDatas.ElementUnderEdit->VarType = VarTypeEntered;
                    EditDatas.ElementUnderEdit->VarNum = VarNumEntered;
                    EditDatas.ElementUnderEdit->IndexedVarType =
                        IndexedVarTypeEntered;
                    EditDatas.ElementUnderEdit->IndexedVarNum =
                        IndexedVarNumEntered;
                }
            } else {
                if (ErrorMessageVarParser)
...
#ifdef OLD_TIMERS_MONOS_SUPPORT
        case ELE_TIMER:
            TextToNumber(GetProperty(0),
                         0,
                         NBR_TIMERS - 1,
                         &EditDatas.ElementUnderEdit->VarNum);
            ...
            rtapi_strxcpy(Timer->DisplayFormat,
                          CorresDatasForBase[IdBase].DisplayFormat);
        ...
        case ELE_MONOSTABLE:
            ...
            rtapi_strxcpy(Monostable->DisplayFormat,
                          CorresDatasForBase[IdBase].DisplayFormat);
            if (TextToNumber(GetProperty(2), 0, 999, &Preset))
        ...
        case ELE_TIMER_IEC:
            ...
            rtapi_strxcpy(TimerIEC->DisplayFormat,
                          CorresDatasForBase[IdBase].DisplayFormat);
        ...
        case ELE_OUTPUT_CALL:
            if (TextToNumber(
                    GetProperty(0), 0, NBR_SECTIONS - 1, &SubRoutineToCall)) {
        ...
        case ELE_OUTPUT_OPERATE:
            NewArithmExpr =
                TextParserForArithmExpr(GetProperty(0), ELE_OUTPUT_OPERATE);
            //printf( "expr=%s\n",EditArithmExpr[EditDatas.ElementUnderEdit->VarNum].Expr );
            if (NewArithmExpr)
                rtapi_strxcpy(
                    EditArithmExpr[EditDatas.ElementUnderEdit->VarNum].Expr,
...
void CheckForFreeingArithmExpr(int PosiX, int PosiY)
{
    ...
    if ((TypeElement == ELE_COMPAR) || (TypeElement == ELE_OUTPUT_OPERATE)) {
        /* Freeing Expr */
        EditArithmExpr[EditDatas.Rung.Element[PosiX][PosiY].VarNum].Expr[0] =
            '\0';
    }
}
    ...
    if ((NumTypeEle == ELE_COMPAR) || (NumTypeEle == ELE_OUTPUT_OPERATE)) {
        do {
            ...
            if (EditArithmExpr[NumExpr].Expr[0] == '\0') {
                ...
                rtapi_strxcpy(EditArithmExpr[NumExpr].Expr,
                              "#"); //used but invalid!
            }
            NumExpr++;
...
void SetUsedStateFunctionBlock(int Type, int Num, char Val)
{
    switch (Type) {
#ifdef OLD_TIMERS_MONOS_SUPPORT
    case ELE_TIMER: NumTimersUsedInRung[Num] = Val; break;
    case ELE_MONOSTABLE: NumMonostablesUsedInRung[Num] = Val; break;
#endif
    case ELE_COUNTER: NumCountersUsedInRung[Num] = Val; break;
    case ELE_TIMER_IEC: NumNewTimersUsedInRung[Num] = Val; break;
    }
}
            ...
            for (y = 0; y < RUNG_HEIGHT; y++) {
                for (x = 0; x < RUNG_WIDTH; x++) {
                    if (RungArray[ScanRung].Element[x][y].Type != ELE_FREE)
                        SetUsedStateFunctionBlock(
                            RungArray[ScanRung].Element[x][y].Type,
                            RungArray[ScanRung].Element[x][y].VarNum,
                            TRUE);
                }
            }
        }
...
void DeleteCurrentRung()
{
    ...
    if (!((InfosGene->CurrentRung == InfosGene->FirstRung) &&
        ...
        SectionArray[InfosGene->CurrentSection].FirstRung =
            InfosGene->FirstRung;
        ...
        for (y = 0; y < RUNG_HEIGHT; y++) {
            for (x = 0; x < RUNG_WIDTH; x++) {
                if ((RungArray[OldCurrent].Element[x][y].Type == ELE_COMPAR) ||
                    (RungArray[OldCurrent].Element[x][y].Type ==
                     ELE_OUTPUT_OPERATE)) {
                    rtapi_strxcpy(
                        ArithmExpr[RungArray[OldCurrent].Element[x][y].VarNum]
                            .Expr,
                        "");
                }
            }
    ...
    if (pRungToCheck->Element[*PosiX][*PosiY].Type == ELE_UNUSABLE) {
        ...
        for (ScanY = 0; ScanY < RUNG_HEIGHT; ScanY++) {
            for (ScanX = 0; ScanX < RUNG_WIDTH; ScanX++) {
                ...
                if (GetSizesOfAnElement(
                        pRungToCheck->Element[ScanX][ScanY].Type,
                        &RuleSizeX,
                        &RuleSizeY)) {
                    /* Have we clicked in it ? */
                    if ((*PosiX >= ScanX - RuleSizeX - 1) &&
                        (*PosiX <= ScanX) && (*PosiY >= ScanY) &&
...
int VerifyConstraintsAndRulesForElement(short int NumEle, int PosiX, int PosiY)
{
    ...
    if (GetSizesOfAnElement(NumEle, &RuleSizeX, &RuleSizeY)) {
        if ((PosiX - RuleSizeX + 1 < 0) ||
            (PosiY + RuleSizeY - 1 >= RUNG_HEIGHT))
            ItIsOk = FALSE;
    }
    ...
    if (RuleSizeX <= 1 && RuleSizeY <= 1) {
        ...
        GetSizesOfAnElement(
            OldEleType, &SizeElementToDestroyX, &SizeElementToDestroyY);
...
void CleanForBigElement(short int NumEle,
                        int PosiX,
                        int PosiY,
                        short int FillWithThis)
{
    int PassX, PassY;
    int RuleSizeX, RuleSizeY;
    if (GetSizesOfAnElement(NumEle, &RuleSizeX, &RuleSizeY)) {
        for (PassX = PosiX - RuleSizeX + 1; PassX <= PosiX; PassX++) {
            for (PassY = PosiY; PassY <= PosiY + RuleSizeY - 1; PassY++) {
                ...
                SetUsedStateFunctionBlock(
                    EditDatas.Rung.Element[PassX][PassY].Type,
                    EditDatas.Rung.Element[PassX][PassY].VarNum,
                    FALSE);
    ...
    if (NumTypeEle == EDIT_COPY) {
        ...
        *pSizeY = 1;
        ...
        do {
            ...
            RulePass++;
        } while (RulesForSpecialElements[RulePass][0] != -1 && *pSizeX == 1 &&
                 *pSizeY == 1);
        if (NumTypeEle == ELE_FREE || NumTypeEle == ELE_CONNECTION ||
            NumTypeEle == ELE_UNUSABLE) {
            *pSizeX = 0;
            *pSizeY = 0;
        }
        ...
        if (NumTypeEle >= EDIT_CNX_WITH_TOP) {
            printf(
                _("!!!Abnormal current type=%d in rung...(file %s,line %d)\n"),
                NumTypeEle,
                __FILE__,
                __LINE__);
            *pSizeX = 0;
                              ...
                              int *pRungY)
{
    ...
    if (coory >= InfosGene->HeaderLabelCommentHeight) {
        ...
        *pRungY = (coory - InfosGene->HeaderLabelCommentHeight) /
                                                    ...
                                                    int PosiY)
{
    ...
    if (!(IsASimpleElement(NumTypeEle))) {
        ...
        if (BlockNumber == -1) {
            ShowMessageBoxError(
                _("No more free function block of this type available..."));
            ResultOk = FALSE;
        } else {
            //printf( "free block nbr=%d\n", BlockNumber );
            EditDatas.Rung.Element[PosiX][PosiY].VarNum = BlockNumber;
            SetUsedStateFunctionBlock(
                NumTypeEle, EditDatas.Rung.Element[PosiX][PosiY].VarNum, TRUE);
        }
    }
    if (!CheckForAllocatingArithmExpr(NumTypeEle, PosiX, PosiY)) {
        ShowMessageBoxError(
            _("No more free arithmetic expression for this type available..."));
        ResultOk = FALSE;
    }
    return ResultOk;
}
...
void EditElementInRung(double x, double y)
{
    ...
    if (ConvertDoublesToRungCoor(x, y, &RungX, &RungY) &&
        ...
        if (EditDatas.NumElementSelectedInToolBar == EDIT_POINTER ||
            EditDatas.NumElementSelectedInToolBar == EDIT_ERASER)
        ...
        if (EditDatas.NumElementSelectedInToolBar != EDIT_CNX_WITH_TOP &&
            ...
            if (NumElement == EDIT_ERASER) {
                ...
                if (VerifyConstraintsAndRulesForElement(
                    ...
                    if (PrepBeforeSettingTypeEleForComplexBlocsAndExpr(
                            NumElement, RungX, RungY)) {

                        /* for big element with only one block "alive" we must mark the others */
                        /* blocks as unusable */
                        CleanForBigElement(
                            NumElement, RungX, RungY, ELE_UNUSABLE);
                        ...
                        if (EditDatas.Rung.Element[RungX][RungY].Type ==
                            ELE_FREE) {
                            int DefaultVarForIt =
                                SetDefaultVariableForElement(NumElement);
                            if (DefaultVarForIt != -1) {
#ifdef IDVAR_IS_TYPE_AND_OFFSET
                                EditDatas.Rung.Element[RungX][RungY].VarType =
                                    DefaultVarForIt;
#else
                                EditDatas.Rung.Element[RungX][RungY].VarType =
                                    VAR_DEFAULT_TYPE;
                                EditDatas.Rung.Element[RungX][RungY].VarNum =
                                    DefaultVarForIt;
#endif
                            }
                        }
        ...
        if (EditDatas.NumElementSelectedInToolBar == EDIT_LONG_CONNECTION) {
            int ScanX = RungX;
            while (ScanX < RUNG_WIDTH - 1 &&
                   EditDatas.Rung.Element[ScanX][RungY].Type == ELE_FREE) {
        ...
        GetSizesOfAnElement(EditDatas.Rung.Element[RungX][RungY].Type,
                            &EditDatas.CurrentElementSizeX,
                            &EditDatas.CurrentElementSizeY);
    }
}
...
{
    if (EditDatas.NumElementSelectedInToolBar < EDIT_CNX_WITH_TOP ||
        EditDatas.NumElementSelectedInToolBar == EDIT_COPY) {
        int RungX, RungY;
        if (ConvertDoublesToRungCoor(x, y, &RungX, &RungY)) {
            //printf( "Motion Position %d,%d for ghost\n", RungX, RungY );
            MovePosiForBigElementToAliveBlock(
                EditDatas.NumElementSelectedInToolBar, &RungX);
            //printf( "Position to alive one %d,%d for ghost\n", RungX, RungY );
            if (VerifyConstraintsAndRulesForElement(
                    EditDatas.NumElementSelectedInToolBar, RungX, RungY)) {
...
void EditElementInThePage(double x, double y)
{
    ...
    if (iCurrentLanguage == SECTION_IN_LADDER) {
        if ((y >= InfosGene->OffsetCurrentRungDisplayed) &&
            (y <
             InfosGene->OffsetCurrentRungDisplayed + TOTAL_PX_RUNG_HEIGHT)) {
            if (EditDatas.NumElementSelectedInToolBar == EDIT_SELECTION)
                StartOrMotionPartSelection(
                    x,
                    y - InfosGene->OffsetCurrentRungDisplayed,
                    TRUE /*StartToClick*/
                );
            else if (EditDatas.NumElementSelectedInToolBar == EDIT_COPY)
                CopyNowPartSelected(x,
                                    y - InfosGene->OffsetCurrentRungDisplayed);
            else
                EditElementInRung(x, y - InfosGene->OffsetCurrentRungDisplayed);
        } else {
            ShowMessageBoxError(_("You clicked outside of the current rung "
                                  "actually selected..."));
        }
    }
#ifdef SEQUENTIAL_SUPPORT
    if (iCurrentLanguage == SECTION_IN_SEQUENTIAL)
        EditElementInSeqPage(x + InfosGene->HScrollValue,
                             y + InfosGene->VScrollValue);
#endif
}
...
{
    if ((y >= InfosGene->OffsetCurrentRungDisplayed) &&
        (y < InfosGene->OffsetCurrentRungDisplayed + TOTAL_PX_RUNG_HEIGHT)) {
        int RungX, RungY;
        if (ConvertDoublesToRungCoor(
                x, y - InfosGene->OffsetCurrentRungDisplayed, &RungX, &RungY)) {
            StrElement *Element;
            CheckForBlocksOfBigElement(
                &RungArray[InfosGene->CurrentRung], &RungX, &RungY);
