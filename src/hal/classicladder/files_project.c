            ...
            if (pDir == NULL && errno == ENOTDIR) {
                ...
                if (Lgt > 0) {
                    *End = '\0';
                    ...
                    printf(
                        _("ERROR with path directory given for project !!!\n"));
                ...
                if (InfosGene->CurrentProjectFileName
                        [strlen(InfosGene->CurrentProjectFileName) - 1] !=
                    CAR_SEP)
...
void InitTempDir(void)
{
    ...
    snprintf(TmpDirectory,
             sizeof(TmpDirectory),
             "%s/classicladder_tmp_XXXXXX",
             TmpEnv);
#ifndef __WIN32__
    ...
    {
        snprintf(
            TmpDirectory, sizeof(TmpDirectory), "%s/classicladder_tmp", TmpEnv);
#ifndef __WIN32__
...
char LoadGeneralParamsOnlyFromProject(char *FileProject)
{
    ...
    if (FileProject[0] != '\0') {
        if (strcmp(&FileProject[strlen(FileProject) - 4], ".clp") == 0) {
            ...
            Result = SplitFiles(FileProject, TmpDirectory);
            snprintf(
                FileName, sizeof(FileName), "%s/general.txt", TmpDirectory);
            ...
            while (!feof(pProjectFile)) {
                ...
                if (!feof(pProjectFile)) {
                    ...
                    if (strncmp(Buff, FILE_HEAD, STR_LEN_FILE_HEAD) == 0) {
                        ////WIN32PORT added /
                        size_t ret = snprintf(ParametersFile,
                                              sizeof(ParametersFile),
                                              "%s/%s",
                                              TmpDirectoryFiles,
