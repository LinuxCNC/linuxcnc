...
static char *ModbusReqType[] = {
    N_("ReadInputs (to %I)"),
    N_("WriteCoils (from %Q)"),
    N_("ReadInputRegs (to %IW)"),
    N_("WriteHoldRegs (from %QW)"),
    N_("ReadCoils (to %Q)"),
    N_("ReadHoldRegs (to %QW)"),
    N_("ReadStatus (to %IW)"),
    N_("Diagnostic (from %IW/to %QW - 1stEle=sub-code used)"),
    NULL};
...
GtkWidget *CreateGeneralParametersPage(void)
{
    ...
    for (ScanSub = 0; ScanSub < NBR_SUBS_VBOX; ScanSub++) {
        ...
        if (ScanSub == 1) {
            ...
            gtk_box_pack_start(GTK_BOX(vbox_sub[ScanSub]),
                               VertSep,
                               TRUE /*expand*/,
                               TRUE /*fill*/,
                               0);
    ...
    for (NumObj = 0; NumObj < NBR_OBJECTS; NumObj++) {
        ...
        case 0:
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Rung Refresh Rate (milliseconds)"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
                     InfosGene->GeneralParams.PeriodicRefreshMilliSecs);
            break;
        case 1:
            InfoUsed = GetNbrRungsDefined() * 100 /
                       InfosGene->GeneralParams.SizesInfos.nbr_rungs;
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Number of rungs (%d%c used)"),
                     InfoUsed,
                     '%');
            snprintf(BuffValue,
                     sizeof(BuffValue),
        ...
        case 6:
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Number of BIT Inputs HAL pins"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
        ...
        case 7:
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Number of BIT Outputs HAL pins"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
        ...
        case 8:
            snprintf(
                BuffLabel, sizeof(BuffLabel), _("Number of S32in HAL pins"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
        ...
        case 9:
            snprintf(
                BuffLabel, sizeof(BuffLabel), _("Number of S32out HAL pins"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
        ...
        case 10:
            ...
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Number of Arithmetic Expressions"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
            ...
            break;
        case 11:
            InfoUsed = NbrSectionsDefined() * 100 /
                       InfosGene->GeneralParams.SizesInfos.nbr_sections;
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Number of Sections (%d%c used)"),
                     InfoUsed,
                     '%');
            snprintf(BuffValue,
                     sizeof(BuffValue),
        ...
        case 15:
            ...
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
                     GeneralParamsMirror.SizesInfos.nbr_error_bits);
            break;
        case 16:
            snprintf(
                BuffLabel, sizeof(BuffLabel), _("Number of floatin HAL pins"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
                     GeneralParamsMirror.SizesInfos.nbr_phys_float_inputs);
            break;
        case 17:
            snprintf(
                BuffLabel, sizeof(BuffLabel), _("Number of floatout HAL pins"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
                     GeneralParamsMirror.SizesInfos.nbr_phys_float_outputs);
            break;
        case 18:
            snprintf(BuffLabel, sizeof(BuffLabel), _("Current path/filename:"));
            size_t ret = snprintf(BuffValue,
                                  sizeof(BuffValue),
                                  "%s",
        ...
        gtk_box_pack_start(
            GTK_BOX(hbox[NumObj]), LabelParam[NumObj], FALSE, FALSE, 5);
        ...
        if (NumObj < NBR_OBJECTS - 1)
            gtk_widget_set_size_request(
                /*(GtkWidget *)*/ ValueParam[NumObj], 45, -1);
        gtk_box_pack_start(GTK_BOX(hbox[NumObj]),
                           ValueParam[NumObj],
                           NumObj >= NBR_OBJECTS - 1 /*expand*/,
                           NumObj >= NBR_OBJECTS - 1 /*fill*/,
                           5);
        gtk_entry_set_text(GTK_ENTRY(ValueParam[NumObj]), BuffValue);
        gtk_widget_set_sensitive(ValueParam[NumObj], FALSE);

        if (NumObj < NBR_OBJECTS - 1)
            gtk_container_add(GTK_CONTAINER(vbox_sub[CurrentSub]),
                              hbox[NumObj]);
        else {
            vbox_whitespace = gtk_vbox_new(FALSE, 0);
            gtk_box_pack_start(
                GTK_BOX(vbox_main), vbox_whitespace, TRUE, TRUE, 0);
...
int GetOneGeneralInfo(int iNumber)
{
    ...
    int value;
    strncpy(
        text, (char *)gtk_entry_get_text((GtkEntry *)ValueParam[iNumber]), 10);
    text[9] = '\0';
    value = atoi(text);
    return value;
}
...
{
    static char *Labels[] = {N_("First %"),
                             N_("Type"),
                             N_("PortAdr(0x)/SubDev"),
                             N_("First Channel"),
                             N_("Nbr Channels"),
                             N_("Logic")};
    ...
    table = gtk_table_new(NBR_IO_PARAMS,
                          1 + (ForInputs ? NBR_INPUTS_CONF : NBR_OUTPUTS_CONF),
                          FALSE);
    ...
    for (NumLine = -1;
         NumLine < (ForInputs ? NBR_INPUTS_CONF : NBR_OUTPUTS_CONF);
         NumLine++) {

        for (NumObj = 0; NumObj < NBR_IO_PARAMS; NumObj++) {
            if (NumLine == -1) {
                GtkWidget **IOParamLabel = ForInputs
                                               ? &InputParamLabel[NumObj]
                                               : &OutputParamLabel[NumObj];
                if (NumObj == 0)
                    *IOParamLabel =
                        gtk_label_new(ForInputs ? (_("1st %I mapped"))
                                                : (_("1st %Q mapped")));
                else
                    *IOParamLabel = gtk_label_new(gettext(Labels[NumObj]));
                gtk_widget_set_size_request(
                    *IOParamLabel, LabelsSize[NumObj], -1);
                ...
                case 1: {
                    int ComboValueToDisplay = 0;
                    {
                        if (pConf->DeviceType == DEVICE_TYPE_DIRECT_ACCESS)
                            ...
                            ComboValueToDisplay =
                                pConf->DeviceType - DEVICE_TYPE_COMEDI + 4;
                    }
                    {
                        GtkWidget **IOParamDevice =
                            ForInputs ? &InputDeviceParam[NumLine]
                                      : &OutputDeviceParam[NumLine];
                        *IOParamDevice = gtk_combo_box_new_text();
                        AddDevicesListToComboBox(
                            MY_GTK_COMBO_BOX(*IOParamDevice));
                        gtk_widget_set_size_request(
                            *IOParamDevice, LabelsSize[NumObj], -1);
                        gtk_table_attach_defaults(GTK_TABLE(table),
                                                  *IOParamDevice,
                                                  NumObj,
                                                  NumObj + 1,
                                                  NumLine + 1,
                                                  NumLine + 2);
                        gtk_combo_box_set_active(GTK_COMBO_BOX(*IOParamDevice),
                                                 ComboValueToDisplay);
                    }
                    break;
                }
                /* For flags */
                case 5: {
                    GtkWidget **IOParamFlag = ForInputs
                                                  ? &InputFlagParam[NumLine]
                                                  : &OutputFlagParam[NumLine];
                    *IOParamFlag =
                        gtk_check_button_new_with_label(_("Inverted"));
                    gtk_widget_set_size_request(
                        *IOParamFlag, LabelsSize[NumObj], -1);
                    gtk_table_attach_defaults(GTK_TABLE(table),
                                              *IOParamFlag,
                                              NumObj,
                                              NumObj + 1,
                    ...
                    if (pConf->FlagInverted)
                        gtk_toggle_button_set_active(
                            GTK_TOGGLE_BUTTON(*IOParamFlag), TRUE);
                    break;
                }
                /* For numbers */
                default: {
                    ...
                    case 4:
                        snprintf(BuffValue,
                                 sizeof(BuffValue),
                                 "%d",
                                 pConf->NbrConsecutivesChannels);
                        break;
                    }
                    {
                        GtkWidget **IOParamEntry =
                            ForInputs ? &InputParamEntry[NumLine][NumObj]
                                      : &OutputParamEntry[NumLine][NumObj];
                        *IOParamEntry = gtk_entry_new();
                        gtk_widget_set_size_request(
                            *IOParamEntry, LabelsSize[NumObj], -1);
                        gtk_table_attach_defaults(GTK_TABLE(table),
                                                  *IOParamEntry,
                                                  NumObj,
                                                  NumObj + 1,
...
void GetIOSettings(char ForInputs)
{
    ...
    for (NumLine = 0;
        ...
        IOParamDevice = ForInputs ? &InputDeviceParam[NumLine]
                                  : &OutputDeviceParam[NumLine];
        ...
        if (ComboVal > 0) {
            ...
            IOParamFlag = ForInputs ? &InputFlagParam[NumLine]
                                    : &OutputFlagParam[NumLine];
            if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(*IOParamFlag)))
                pConf->FlagInverted = 1;
            for (NumObj = 0; NumObj < NBR_IO_PARAMS; NumObj++) {
                IOParamEntry = ForInputs ? &InputParamEntry[NumLine][NumObj]
                                         : &OutputParamEntry[NumLine][NumObj];
                switch (NumObj) {
                case 0:
                    text =
                        (char *)gtk_entry_get_text((GtkEntry *)*IOParamEntry);
                    if (text[0] != '\0')
                        FirstIO = atoi(text);
                    break;
                case 2:
            ...
            if (FirstIO + pConf->NbrConsecutivesChannels >
                (ForInputs ? NBR_PHYS_INPUTS : NBR_PHYS_OUTPUTS)) {
                printf("Error in I/O conf: overflow for Ixx or Qxx mapping "
                       "detected...\n");
                FirstIO = -1;
            }
...
void FillComboBoxReqType(MyGtkComboBox *pComboBox, char *ListTextsCombo[])
{
    int ScanDev = 0;
    do {
        gtk_combo_box_append_text(pComboBox,
                                  gettext(ListTextsCombo[ScanDev++]));
    } while (ListTextsCombo[ScanDev]);
}
GtkWidget *CreateModbusModulesIO(void)
{
    const char *Labels[] = {N_("Slave Address"),
                            N_("Request Type"),
                            N_("1st Modbus Ele."),
                            N_("# of Ele"),
                            N_("Logic"),
                            N_("1st Variable mapped")};
    GtkWidget *table;
    GtkWidget *scrolled_win;
    int NumObj;
    int NumLine;
    ...
    if (modmaster == FALSE) {
        ...
        gtk_label_set_justify(GTK_LABEL(ModbusParamLabel[0]),
                              GTK_JUSTIFY_CENTER);
                ...
                *CurrentWidget = gtk_label_new(gettext(Labels[NumObj]));
                ...
                case 1: {
                    ...
                    *CurrentWidget = gtk_combo_box_new_text();
                    FillComboBoxReqType(MY_GTK_COMBO_BOX(*CurrentWidget),
                                        ModbusReqType);
                    gtk_combo_box_set_active(GTK_COMBO_BOX(*CurrentWidget),
                                             pConf->TypeReq);
                    break;
                }
                ...
                case 4: {
                    ...
                    *CurrentWidget =
                    ...
                    if (pConf->LogicInverted)
                        gtk_toggle_button_set_active(
                            GTK_TOGGLE_BUTTON(*CurrentWidget), TRUE);
                    break;
                }
                ...
                default: {
                    char BuffValue[30];
                    switch (NumObj) {
                    case 0: rtapi_strxcpy(BuffValue, pConf->SlaveAdr); break;
                    case 2:
                        snprintf(BuffValue,
                                 sizeof(BuffValue),
                                 "%d",
                                 pConf->FirstModbusElement);
                        break;
                    case 3:
                        snprintf(BuffValue,
                                 sizeof(BuffValue),
                                 "%d",
                                 pConf->NbrModbusElements);
                        break;
                    case 5:
                        snprintf(BuffValue,
                                 sizeof(BuffValue),
                                 "%d",
                                 pConf->OffsetVarMapped);
                        break;
                    }
                    CurrentWidget = &ModbusParamEntry[NumLine][NumObj];
    ...
    for (NumLine = 0; NumLine < NBR_MODBUS_MASTER_REQ; NumLine++) {
        ...
        for (NumObj = 0; NumObj < NBR_MODBUS_PARAMS; NumObj++) {
            ...
            case 1: //type of request
                pConf->TypeReq =
                    gtk_combo_box_get_active(GTK_COMBO_BOX(*IOParamEntry));
                break;
            ...
            case 4: //invert logic (instead of thinking of that everywhere later...)
                pConf->LogicInverted = (gtk_toggle_button_get_active(
                                           GTK_TOGGLE_BUTTON(*IOParamEntry)))
                                           ? 1
                                           : 0;
                break;
        ...
        case MODBUS_REQ_INPUT_REGS_READ:
            MaxVars =
                GetSizeVarsForTypeVar(ModbusConfig.MapTypeForReadInputRegs);
            DoVerify = TRUE;
            break;
        case MODBUS_REQ_HOLD_REGS_WRITE:
            MaxVars =
                GetSizeVarsForTypeVar(ModbusConfig.MapTypeForWriteHoldRegs);
            DoVerify = TRUE;
            break;
        case MODBUS_REQ_HOLD_REGS_READ:
            MaxVars =
                GetSizeVarsForTypeVar(ModbusConfig.MapTypeForReadHoldRegs);
            DoVerify = TRUE;
            break;
        }
        if (DoVerify) {
            if (pConf->OffsetVarMapped + pConf->NbrModbusElements > MaxVars) {
                printf(
                    _("Error in I/O modbus conf: overflow for I,Q,B,IQ, or WQ "
                      "mapping detected...ASKED=%i,MAX=%i\n"),
                    pConf->OffsetVarMapped + pConf->NbrModbusElements,
                    MaxVars);
                rtapi_strxcpy(BuffValue, "");
                ShowMessageBoxError(_(
                    "Overflow error for I,Q,B,IQ,WQ or W mapping detected..."));
            }
        }
        /* done at the end, do not forget multi-task ! */
...
int SplitCommasFieldsInPointersArray(char *LineDatas,
                                     char *PtrFieldsDatasFound[],
                                     int NbrMaxFields)
{
    int ScanField;
...
GtkWidget *CreateModbusComParametersPage(void)
{
    ...
    //	gtk_widget_show (vbox);
    table =
        gtk_table_new(MODBUS_COM_PARAMS_SIZE_X_TABLE, NBR_COM_PARAMS, FALSE);
    gtk_box_pack_start(
        GTK_BOX(hbox), table, TRUE /*expand*/, FALSE /*fill*/, 0);
    for (NumLine = 0; NumLine < NBR_COM_PARAMS; NumLine++) {
        ...
        case 2:
            ...
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d,5,6,7,8",
                     ModbusConfig.ModbusSerialDataBits - 5);
            break;
        case 3:
        ...
        case 5:
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("After transmit pause - milliseconds"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
        ...
        case 6:
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("After receive pause - milliseconds"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
        ...
        case 7:
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Request Timeout length - milliseconds"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d",
        ...
        case 11:
            ...
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d,%%B,%%Q",
                     ModbusConfig.MapTypeForReadInputs == VAR_MEM_BIT
                         ? 0
                         : (ModbusConfig.MapTypeForReadInputs == VAR_PHYS_OUTPUT
                                ? 1
                                : 2));
            break;
        case 12:
            snprintf(
                BuffLabel, sizeof(BuffLabel), _("Read Coils/inputs map to"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d,%%B,%%Q",
                     ModbusConfig.MapTypeForReadCoils == VAR_MEM_BIT
                         ? 0
        ...
        case 13:
            ...
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d,%%B,%%Q,%%I",
                     ModbusConfig.MapTypeForWriteCoils == VAR_MEM_BIT
                         ? 0
        ...
        case 14:
            snprintf(
                BuffLabel, sizeof(BuffLabel), _("Read input registers map to"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d,%%W,%%QW",
                     ModbusConfig.MapTypeForReadInputRegs == VAR_MEM_WORD
                         ? 0
        ...
        case 15:
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Read register/holding map to"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d,%%W,%%QW",
                     ModbusConfig.MapTypeForReadHoldRegs == VAR_MEM_WORD
                         ? 0
        ...
        case 16:
            snprintf(BuffLabel,
                     sizeof(BuffLabel),
                     _("Write hold registers map from"));
            snprintf(BuffValue,
                     sizeof(BuffValue),
                     "%d,%%W,%%QW,%%IW",
                     ModbusConfig.MapTypeForWriteHoldRegs == VAR_MEM_WORD
                         ? 0
        ...
        if (NumLine <= 1 || (NumLine >= 5 && NumLine <= 7)) {
            ...
            gtk_table_attach_defaults(GTK_TABLE(table),
                                      EntryComParam[NumLine],
                                      1,
                                      MODBUS_COM_PARAMS_SIZE_X_TABLE,
                                      NumLine,
                                      NumLine + 1);
            ...
            int CreateRadioBut;
            for (CreateRadioBut = 0; CreateRadioBut < NBR_RADIO_BUT_COM_PARAMS;
                 CreateRadioBut++)
                RadioButComParams[NumLine][CreateRadioBut] = NULL;
            int NbrInfos =
                SplitCommasFieldsInPointersArray(BuffValue, PtrArraysCsv, 10);
            if (NbrInfos > 2) {
                int ValueSelected = atoi(PtrArraysCsv[0]);
                for (CreateRadioBut = 0; CreateRadioBut < NbrInfos - 1;
                     CreateRadioBut++) {
                    char *label = PtrArraysCsv[1 + CreateRadioBut];
                    if (CreateRadioBut == 0)
                        RadioButComParams[NumLine][CreateRadioBut] =
                            gtk_radio_button_new_with_label(NULL, label);
                    else
                        RadioButComParams[NumLine][CreateRadioBut] =
                            gtk_radio_button_new_with_label_from_widget(
                                GTK_RADIO_BUTTON(RadioButComParams[NumLine][0]),
                                label);
...
int GetRadioButValueSelected(int NumLineToSee)
{
    int Sel = 0;
    int ScanRadioBut;
    for (ScanRadioBut = 0; ScanRadioBut < NBR_RADIO_BUT_COM_PARAMS;
         ScanRadioBut++) {
        if (RadioButComParams[NumLineToSee][ScanRadioBut] != NULL) {
            if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                    RadioButComParams[NumLineToSee][ScanRadioBut])))
                Sel = ScanRadioBut;
        }
    }
    return Sel;
...
{
    rtapi_strxcpy(ModbusConfig.ModbusSerialPortNameUsed,
                  gtk_entry_get_text(GTK_ENTRY(EntryComParam[0])));
    ModbusConfig.ModbusSerialSpeed =
        atoi(gtk_entry_get_text(GTK_ENTRY(EntryComParam[1])));
    ...
    ModbusConfig.ModbusTimeAfterTransmit =
        atoi(gtk_entry_get_text(GTK_ENTRY(EntryComParam[5])));
    ModbusConfig.ModbusTimeInterFrame =
        atoi(gtk_entry_get_text(GTK_ENTRY(EntryComParam[6])));
    ModbusConfig.ModbusTimeOutReceipt =
        atoi(gtk_entry_get_text(GTK_ENTRY(EntryComParam[7])));
    ...
    ModbusConfig.MapTypeForReadInputs =
        GetRadioButValueSelected(11) == 0
            ? VAR_MEM_BIT
            : (GetRadioButValueSelected(11) == 1 ? VAR_PHYS_OUTPUT
                                                 : VAR_PHYS_INPUT);
    ModbusConfig.MapTypeForReadCoils =
        GetRadioButValueSelected(12) == 0
            ? VAR_MEM_BIT
            : (GetRadioButValueSelected(12) == 1 ? VAR_PHYS_OUTPUT
                                                 : VAR_PHYS_INPUT);
    ModbusConfig.MapTypeForWriteCoils =
        GetRadioButValueSelected(13) == 0
            ? VAR_MEM_BIT
            : (GetRadioButValueSelected(13) == 1 ? VAR_PHYS_OUTPUT
                                                 : VAR_PHYS_INPUT);
    ModbusConfig.MapTypeForReadInputRegs =
        GetRadioButValueSelected(14) == 0
            ? VAR_MEM_WORD
            : (GetRadioButValueSelected(14) == 1 ? VAR_PHYS_WORD_OUTPUT
                                                 : VAR_PHYS_WORD_INPUT);
    ModbusConfig.MapTypeForReadHoldRegs =
        GetRadioButValueSelected(15) == 0
            ? VAR_MEM_WORD
            : (GetRadioButValueSelected(15) == 1 ? VAR_PHYS_WORD_OUTPUT
                                                 : VAR_PHYS_WORD_INPUT);
    ModbusConfig.MapTypeForWriteHoldRegs =
        GetRadioButValueSelected(16) == 0
            ? VAR_MEM_WORD
            : (GetRadioButValueSelected(16) == 1 ? VAR_PHYS_WORD_OUTPUT
                                                 : VAR_PHYS_WORD_INPUT);
}
#endif
...
#ifdef MODBUS_IO_MASTER
    gtk_notebook_append_page(GTK_NOTEBOOK(nbook),
                             CreateModbusComParametersPage(),
                             gtk_label_new(_("Modbus communication setup")));
    gtk_notebook_append_page(GTK_NOTEBOOK(nbook),
                             CreateModbusModulesIO(),
