...
int SerialBaudCorres[] = {
    B300, B600, B1200, B2400, B4800, B9600, B19200, B38400, B57600, B115200, 0};
...
char SerialOpen()
{
    ...
    fd = open(ModbusConfig.ModbusSerialPortNameUsed,
              O_RDWR | O_NOCTTY | O_NDELAY /*don't wait DTR*/
    );
    if (fd >= 0) {
        int BaudRate = -1;
        int ScanBaudRate = 0;
        ...
        if (BaudRate != -1) {
            ...
            newtio.c_cflag = BaudRate | DATABITS | STOPBITS | PARITYON |
                             PARITY | CLOCAL | CREAD;
