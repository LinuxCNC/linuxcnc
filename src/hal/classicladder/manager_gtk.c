...
void ManagerDisplaySections()
{
	...
	if (pSection->Used) {
	    ...
	    sprintf(buffer_debug, "F=%d, L=%d, P=%d", pSection->FirstRung,
...
void SelectRowSignal(GtkCList * clist, gint row, gint column,
    GdkEventButton * event, gpointer user_data)
{
    if (gtk_clist_get_text(GTK_CLIST(SectionsList), row, 0,
	    (gchar **) & pNameSectionSelected)) {
...
void ButtonAddSectionDoneClickSignal()
{
    ...
    // get if main or sub-routine (and which number if sub, used in the
    // 'C'all coils)
    strcpy(SubNbrValue, (char *) gtk_entry_get_text((GtkEntry *) ((GtkCombo *)
		CycleSubRoutineNbr)->entry));
    if (SubNbrValue[0] == 'S' && SubNbrValue[1] == 'R')
	SubNbr = atoi(&SubNbrValue[2]);

    // verify if name already exist...
    if (VerifyIfSectionNameAlreadyExist((char *)
	    gtk_entry_get_text(GTK_ENTRY(EditName)))) {
	ShowMessageBox("Error",
	    "This section name already exist or is incorrect !!!", "Ok");
    } else {
	if (SubNbr >= 0 && VerifyIfSubRoutineNumberExist(SubNbr)) {
	    ShowMessageBox("Error",
		"This sub-routine number for calls is already defined !!!",
		"Ok");
	} else {
	    // create the new section
	    if (!AddSection((char *) gtk_entry_get_text(GTK_ENTRY(EditName)),
		    Language, SubNbr))
		ShowMessageBox("Error", "Failed to add a new section. Full?",
		    "Ok");
...
gint AddSectionWindowDeleteEvent(GtkWidget * widget, GdkEvent * event,
    gpointer data)
{
...
void ButtonDelClickSignal()
{
	...
	if (NbrSectionsDefined() > 1) {
	    ...
	    ShowMessageBox("Error", "You can not delete the last section...",
		"Ok");
	}
    }
}
...
void ButtonMoveUpClickSignal()
{
	...
	if (gtk_clist_get_text(GTK_CLIST(SectionsList),
		RowSectionSelected - 1, 0,
		(gchar **) & pNameSectionToSwapWith)) {
	    SwapSections(pNameSectionSelected, pNameSectionToSwapWith);
	}
    } else {
	ShowMessageBox("Error",
	    "This section is already executed the first !", "Ok");
    }
    ManagerDisplaySections();
}
...
gint ManagerWindowDeleteEvent(GtkWidget * widget, GdkEvent * event,
    gpointer data)
{
    // we do not want that the window be destroyed.
    return TRUE;
...
void AddSectionWindowInit()
{
	...
	SubRoutinesNbrItems =
	    g_list_append(SubRoutinesNbrItems, ArrayNumSub[NumSub]);
    }
	...
	case 0:
	    ...
	    gtk_entry_set_max_length(GTK_ENTRY(EditName),
		LGT_SECTION_NAME - 1);
	...
	case 1:
	    ...
	    gtk_combo_set_value_in_list(GTK_COMBO(CycleLanguage), TRUE	/* val 
		 */ , FALSE /* ok_if_empty */ );
	    gtk_combo_set_popdown_strings(GTK_COMBO(CycleLanguage),
		LangageItems);
	    gtk_box_pack_start(GTK_BOX(hbox[Line]), CycleLanguage, TRUE, TRUE,
		0);
	    gtk_widget_show(CycleLanguage);
	    break;
	case 2:
	    CycleSubRoutineNbr = gtk_combo_new();
	    gtk_combo_set_value_in_list(GTK_COMBO(CycleSubRoutineNbr), TRUE	/* val 
		 */ , FALSE /* ok_if_empty */ );
	    gtk_combo_set_popdown_strings(GTK_COMBO(CycleSubRoutineNbr),
		SubRoutinesNbrItems);
	    gtk_box_pack_start(GTK_BOX(hbox[Line]), CycleSubRoutineNbr, TRUE,
		TRUE, 0);
