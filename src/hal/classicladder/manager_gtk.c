    ...
    for (NumSec = 0; NumSec < NBR_SECTIONS; NumSec++) {
        ...
        if (pSection->Used) {
            ...
            if (pSection->SubRoutineNumber >= 0)
                snprintf(BufferForSRx,
                         sizeof(BufferForSRx),
                         "SR%d",
                         pSection->SubRoutineNumber);
            snprintf(buffer_debug,
                     sizeof(buffer_debug),
                     "F=%d, L=%d, P=%d",
                     pSection->FirstRung,
                     pSection->LastRung,
            ...
            gtk_list_store_set(ListStore,
                               ...
                               pSection->Name,
                               SECTION_LANGUAGE,
                               (pSection->Language == SECTION_IN_SEQUENTIAL)
                                   ? (_("Sequential"))
                                   : (_("Ladder")),
                               SECTION_TYPE,
                               BufferForSRx,
            ...
            if (SectionNbrSelected == -1 || SectionNbrSelected == NumSec) {
                ...
                selection = gtk_tree_view_get_selection(
                    GTK_TREE_VIEW(ListViewSections));
...
void TreeViewCursorChangedSignal(GtkTreeView *treeview, gpointer user_data)
{
    ...
    if (!BlockTreeViewCursorChangedSignal) {
        ...
        GtkTreeIter iter;
        selection =
            gtk_tree_view_get_selection(GTK_TREE_VIEW(ListViewSections));
...
{
    int SearchSectionNext =
        GetPrevNextSection(SectionNbrSelected, TRUE /*NextSearch*/);
...
{
    int SearchSectionPrev =
        GetPrevNextSection(SectionNbrSelected, FALSE /*NextSearch*/);
...
void ButtonAddSectionDoneClickSignal()
{
    ...
    if (VerifyIfSectionNameAlreadyExist(pSectionNameEntered)) {
        ShowMessageBoxError(
            _("This section name already exists or is incorrect !!!"));
    } else {
        if (ModifyNotAdding) {
            ModifySectionProperties(SectionNbrSelected /*pNameSectionSelected*/,
                                    pSectionNameEntered);
            ...
            SubNbr =
                gtk_combo_box_get_active(GTK_COMBO_BOX(CycleSubRoutineNbr)) - 1;

            if (SubNbr >= 0 && VerifyIfSubRoutineNumberExist(SubNbr)) {
                ShowMessageBoxError(_("This sub-routine number for calls is "
                                      "already defined !!!"));
            } else {
                // create the new section
                int NewSectionNbrAllocated =
                    AddSection(pSectionNameEntered, Language, SubNbr);
                if (NewSectionNbrAllocated == -1) {
                    ShowMessageBoxError(
                        _("Failed to add a new section. Full?"));
                } else {
...
void ButtonAddClickSignal()
{
    if (InfosGene->LadderState == STATE_RUN) {
        ...
        gtk_window_set_title(GTK_WINDOW(AddSectionWindow),
                             _("Add a new section..."));
...
void ButtonPropertiesClickSignal()
{
    ...
    if (SectionNbrSelected != -1) {
        ...
        gtk_combo_box_set_active(GTK_COMBO_BOX(CycleLanguage),
                                 pSection->Language);
        //		strcpy( Buff, "Main");
        //		if ( pSection->SubRoutineNumber>=0 )
        //			sprintf( Buff, "SR%d", pSection->SubRoutineNumber );
        //		gtk_entry_set_text((GtkEntry*)((GtkCombo *)CycleSubRoutineNbr)->entry,Buff);
        gtk_combo_box_set_active(GTK_COMBO_BOX(CycleSubRoutineNbr),
...
gint AddSectionWindowDeleteEvent(GtkWidget *widget,
                                 GdkEvent *event,
                                 gpointer data)
{
    (void)widget;
    ...
    if (InfosGene->LadderState == STATE_RUN) {
        ...
        if (SectionNbrSelected != -1) {
            if (NbrSectionsDefined() > 1) {
                ShowConfirmationBox(
                    _("New"),
                    _("Do you really want to delete the section ?"),
                    DeleteCurrentSection);
            } else {
                ShowMessageBoxError(
                    _("You can not delete the last section..."));
            }
        }
    }
}
...
void AddSectionWindowInit()
{
    ...
    for (Line = 0; Line < 3; Line++) {
        ...
        case 1:
            ...
            gtk_combo_box_append_text(MY_GTK_COMBO_BOX(CycleLanguage),
                                      _("Ladder"));
#ifdef SEQUENTIAL_SUPPORT
            gtk_combo_box_append_text(MY_GTK_COMBO_BOX(CycleLanguage),
                                      _("Sequential"));
#endif
            gtk_box_pack_start(
                GTK_BOX(hbox[Line]), CycleLanguage, TRUE, TRUE, 0);
        ...
        case 2:
            ...
            gtk_combo_box_append_text(MY_GTK_COMBO_BOX(CycleSubRoutineNbr),
                                      _("Main"));
            for (NumSub = 0; NumSub < 10; NumSub++) {
            ...
            gtk_box_pack_start(
                GTK_BOX(hbox[Line]), CycleSubRoutineNbr, TRUE, TRUE, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonOk),
                       "clicked",
                       GTK_SIGNAL_FUNC(ButtonAddSectionDoneClickSignal),
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(AddSectionWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(AddSectionWindowDeleteEvent),
                       0);
}
...
static GtkActionEntry ManagerEntriesArray[] = {
    {"AddSection",
     GTK_STOCK_ADD,        N_("Add section"),
     NULL, N_("Add New Section"),
     G_CALLBACK(ButtonAddClickSignal)       },
    {"DelSection",
     GTK_STOCK_DELETE,     N_("Delete section"),
     NULL, N_("Delete Section"),
     G_CALLBACK(ButtonDelClickSignal)       },
    {"MoveUpSection",
     GTK_STOCK_GO_UP,      N_("Move up"),
     NULL, N_("Priority order Move up"),
     G_CALLBACK(ButtonMoveUpClickSignal)    },
    {"MoveDownSection",
     GTK_STOCK_GO_DOWN,    N_("Move down"),
     NULL, N_("Priority order Move down"),
     G_CALLBACK(ButtonMoveDownClickSignal)  },
    {"PropertiesSection",
     GTK_STOCK_PROPERTIES, N_("Properties"),
     NULL, N_("Section Properties"),
     G_CALLBACK(ButtonPropertiesClickSignal)}
};
...
void ManagerInitGtk()
{
    ...
    char *ColName[] = {
        N_("Nbr"), N_("Section Name"), N_("Language"), N_("Type"), N_("debug")};
    ...
    gtk_action_group_add_actions(MgrActionGroup,
                                 ManagerEntriesArray,
                                 G_N_ELEMENTS(ManagerEntriesArray),
                                 NULL);

    gtk_ui_manager_insert_action_group(ManageruiManager, MgrActionGroup, 0);
    if (!gtk_ui_manager_add_ui_from_string(
            ManageruiManager, Manager_ui_strings, -1 /*length*/, &error)) {
    ...
    ListStore = gtk_list_store_new(SECTION_NBR_INFOS,
                                   G_TYPE_INT,
                                   G_TYPE_STRING,
                                   G_TYPE_STRING,
                                   G_TYPE_STRING,
    ...
    for (ScanCol = 1; ScanCol < SECTION_NBR_INFOS; ScanCol++) {
        ...
        column = gtk_tree_view_column_new_with_attributes(
            gettext(ColName[ScanCol]), renderer, "text", ScanCol, NULL);
    ...
    gtk_tree_view_set_grid_lines(GTK_TREE_VIEW(ListViewSections),
    ...
    gtk_scrolled_window_set_policy(
        GTK_SCROLLED_WINDOW(ScrollWin), GTK_POLICY_NEVER, GTK_POLICY_ALWAYS);
    gtk_box_pack_start(GTK_BOX(vbox), ScrollWin, TRUE, TRUE, 0);
    gtk_widget_show(ListViewSections);
    gtk_widget_show(ScrollWin);

    gtk_signal_connect(GTK_OBJECT(ListViewSections),
                       "cursor-changed",
                       GTK_SIGNAL_FUNC(TreeViewCursorChangedSignal),
                       0);

    //v0.9.20	ManagerDisplaySections( );
    gtk_signal_connect(GTK_OBJECT(ManagerWindow),
                       "delete_event",
                       GTK_SIGNAL_FUNC(ManagerWindowDeleteEvent),
                       0);
