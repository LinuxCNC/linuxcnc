            ...
            if (LineOk) {
                ...
                case 'T':
                    ConvRawLineOfNumbers(
                        &Line[1], 1 + 4 * NBR_SWITCHS_MAX + 3, StepData);
                    ScanData = 0;
                ...
                case 'C':
                    ...
                    pSearch++;
                    ConvTypeOffsetVar(
                        pSearch, &pTransi->VarTypeCondi, &pTransi->VarNumCondi);
                    break;
                case 'N':
                    pCommentString =
                        ConvRawLineOfNumbers(&Line[1], 4, StepData);
                    ScanData = 0;
...
char SaveSequential(char *FileName)
{
    ...
    if (File) {
        ...
        for (NumSeqComment = 0; NumSeqComment < NBR_SEQ_COMMENTS;
             NumSeqComment++) {
