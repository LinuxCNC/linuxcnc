...
char LoadSequential(char *FileName)
{
    ...
    char Line[300];
    char *LineOk;
    int NumStep, NumTransi, ScanStep;
    StrStep *pStep;
	...
	do {
		...
		case 'T':
			...
			1 + 4 * NBR_SWITCHS_MAX + 3, StepData);
		    ScanData = 0;
		    NumTransi = StepData[ScanData++];
		    pTransi = &Sequential->Transition[NumTransi];
		    for (ScanStep = 0; ScanStep < NBR_SWITCHS_MAX; ScanStep++)
			pTransi->NumStepToActiv[ScanStep] =
			    StepData[ScanData++];
		    for (ScanStep = 0; ScanStep < NBR_SWITCHS_MAX; ScanStep++)
			pTransi->NumStepToDesactiv[ScanStep] =
			    StepData[ScanData++];
		    for (ScanStep = 0; ScanStep < NBR_SWITCHS_MAX; ScanStep++)
			pTransi->NumTransLinkedForStart[ScanStep] =
			    StepData[ScanData++];
		    for (ScanStep = 0; ScanStep < NBR_SWITCHS_MAX; ScanStep++)
			pTransi->NumTransLinkedForEnd[ScanStep] =
			    StepData[ScanData++];
		...
		case 'C':
		    ...
		    pSearch++;
		    pSearch++;
		    ConvTypeOffsetVar(pSearch, &pTransi->VarTypeCondi,
			&pTransi->VarNumCondi);
		    break;
		}
	    }
...
char SaveSequential(char *FileName)
{
	...
	for (NumStep = 0; NumStep < NBR_STEPS; NumStep++) {
	    pStep = &Sequential->Step[NumStep];
	    if (pStep->NumPage != (char) -1) {
		fprintf(File, "S%d,%d,%d,%d,%d,%d", NumStep, pStep->InitStep,
		    pStep->StepNumber, pStep->NumPage, pStep->PosiX,
		    pStep->PosiY);
		fprintf(File, "\n");
	    }
	}
	for (NumTransi = 0; NumTransi < NBR_TRANSITIONS; NumTransi++) {
		...
		fprintf(File, "%d,%d,%d", pTransi->NumPage, pTransi->PosiX,
		    pTransi->PosiY);
		fprintf(File, "\n");
	    }
	}
	for (NumTransi = 0; NumTransi < NBR_TRANSITIONS; NumTransi++) {
	    pTransi = &Sequential->Transition[NumTransi];
	    if (pTransi->NumPage != (char) -1) {
		fprintf(File, "C%d,0,%d/%d", NumTransi, pTransi->VarTypeCondi,
		    pTransi->VarNumCondi);
		fprintf(File, "\n");
	    }
	}
