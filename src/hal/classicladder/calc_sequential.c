...
void InitSequential(void)
{
    ...
    for (NumTrans = 0; NumTrans < NBR_TRANSITIONS; NumTrans++) {
        ...
        for (NumSwitch = 0; NumSwitch < NBR_SWITCHS_MAX; NumSwitch++) {
            ...
            Sequential->Transition[NumTrans].NumTransLinkedForStart[NumSwitch] =
                -1;
...
int RefreshTransi(StrTransition *pTransi)
{
    ...
    if (pTransi->Activated) {
        ...
        while (ScanStep < NBR_SWITCHS_MAX &&
               pTransi->NumStepToDesactiv[ScanStep] != -1 && AllStepsOn) {
            if (!Sequential->Step[pTransi->NumStepToDesactiv[ScanStep]]
                     .Activated)
                AllStepsOn = FALSE;
            ScanStep++;
        }
        ...
        if (AllStepsOn) {
            ...
            ScanStep = 0;
            while (ScanStep < NBR_SWITCHS_MAX &&
                   pTransi->NumStepToDesactiv[ScanStep] != -1) {
                Sequential->Step[pTransi->NumStepToDesactiv[ScanStep]]
                    .Activated = FALSE;
                ScanStep++;
            }
            ...
            ScanStep = 0;
            while (ScanStep < NBR_SWITCHS_MAX &&
                   pTransi->NumStepToActiv[ScanStep] != -1) {
                Sequential->Step[pTransi->NumStepToActiv[ScanStep]].Activated =
                    TRUE;
                ScanStep++;
            }
        }
    ...
    for (NumStep = 0; NumStep < NBR_STEPS; NumStep++) {
        ...
        if (Sequential->Step[NumStep].NumPage != -1) {
            if (Sequential->Step[NumStep].Activated)
                Sequential->Step[NumStep].TimeActivated +=
                    InfosGene->GeneralParams.PeriodicRefreshMilliSecs;
            else
                Sequential->Step[NumStep].TimeActivated = 0;

            /* refresh the vars for the step number associated to this step */
            WriteVar(VAR_STEP_ACTIVITY,
                     Sequential->Step[NumStep].StepNumber,
                     Sequential->Step[NumStep].Activated);
            WriteVar(VAR_STEP_TIME,
                     Sequential->Step[NumStep].StepNumber,
                     Sequential->Step[NumStep].TimeActivated / 1000);
        }
    }
}
