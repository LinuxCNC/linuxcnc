                        ...
                        gpointer user_data)
{
    ...
    PrintLadderBlockHeight =
        PrintLadderBlockWidth * BLOCK_HEIGHT_DEF / BLOCK_WIDTH_DEF;
    ...
    if (iCurrentLanguage == SECTION_IN_LADDER) {
        ...
        int the_height = PrintHeaderLabelCommentHeight +
                         RUNG_HEIGHT * PrintLadderBlockHeight;

        int NbrTotalRungs = 0;
        GetCurrentNumAndNbrRungsForCurrentSection(NULL, &NbrTotalRungs);
        NbrRungsPerPage = PageHeight / (the_height + SpaceBetweenRungsY);
        NbrPagesToPrint =
            (NbrTotalRungs + NbrRungsPerPage - 1) / NbrRungsPerPage;
        printf("rung page: w=%d, h=%d, PageWidth=%d, NbrTotalRungs=%d "
               "NbrRungsPerPage=%d  NbrPagesToPrint=%d\n",
               the_width,
               the_height,
               PageWidth,
               NbrTotalRungs,
               ...
               NbrPagesToPrint);
        printf(
            "print sizes: PrintLadderBlockWidth=%d, PrintLadderBlockHeight=%d, "
            "PrintSeqSize=%d\n",
            PrintLadderBlockWidth,
            PrintLadderBlockHeight,
            PrintSeqSize);
...
draw_page(GtkPrintOperation *operation, GtkPrintContext *context, int page_nr)
{
    ...
    CreateFontPangoLayout(cr,
                          (iCurrentLanguage == SECTION_IN_SEQUENTIAL)
                              ? PrintSeqSize
                              : PrintLadderBlockHeight,
                          DRAW_FOR_PRINT);
    snprintf(Buffer,
             sizeof(Buffer),
             CL_PRODUCT_NAME CL_RELEASE_VER_STRING "  -  %s:%s  -  %s:%d/%d",
             _("Section"),
             SectionArray[InfosGene->CurrentSection].Name,
             _("Page"),
             page_nr + 1,
             NbrPagesToPrint);
    DrawPangoText(cr,
                  OffsetPrintX,
                  gtk_print_context_get_height(context) - 10,
                  -1,
                  -1,
                  Buffer);
    ...
    if (iCurrentLanguage == SECTION_IN_LADDER) {
        ...
        do {
            DrawLeftRightBars(cr,
                              ...
                              PrintLadderBlockWidth,
                              PrintLadderBlockHeight,
                              PrintHeaderLabelCommentHeight,
                              PrintLeftRightBarsWidth,
                              FALSE);
            DrawRung(cr,
                     &RungArray[ScanRungToPrint],
                     OffsetPrintX + PrintLeftRightBarsWidth,
                     ScanPosiY,
                     PrintLadderBlockWidth,
                     PrintLadderBlockHeight,
                     PrintHeaderLabelCommentHeight,
