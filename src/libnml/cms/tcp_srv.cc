...
CMS_SERVER_REMOTE_TCP_PORT::CMS_SERVER_REMOTE_TCP_PORT(CMS_SERVER *_cms_server)
    : CMS_SERVER_REMOTE_PORT(_cms_server), dtimeout(20.0), read_fd_set{},
      write_fd_set{}, maxfdpl(0), subscription_buffers(NULL),
      connection_socket(0), connection_port(0), request(NULL), temp_buffer{},
      current_poll_interval_millis(30000), polling_enabled(0),
...
void CMS_SERVER_REMOTE_TCP_PORT::run()
{
    ...
    while (1) {
        if (polling_enabled) {
            ...
            ready_descriptors = select(maxfdpl,
                                       &read_fd_set,
                                       &write_fd_set,
                                       (fd_set *)NULL,
        ...
        while (NULL != client_port_to_check) {
            if (FD_ISSET(client_port_to_check->socket_fd, &read_fd_set)) {
                ioctl(client_port_to_check->socket_fd,
                      FIONREAD,
                      (caddr_t)&bytes_ready);
                if (bytes_ready <= 0) {
                    rcs_print_debug(
                        PRINT_SOCKET_CONNECT,
                    ...
                    if (client_port_to_check->blocking) {
                        if (client_port_to_check->threadId > 0) {
                            rcs_print_debug(
                                PRINT_SERVER_THREAD_ACTIVITY,
                                "Data received from %s:%d when it should be "
                                "blocking (bytes_ready=%d).\n",
                                inet_ntoa(
            ...
            client_port_to_check = (CLIENT_TCP_PORT *)client_ports->get_next();
        }
        if (FD_ISSET(connection_socket, &read_fd_set) &&
            ...
            new_client_port->socket_fd =
                accept(connection_socket,
                       (struct sockaddr *)&new_client_port->address,
            ...
            if (NULL != client_ports) {
                client_ports->store_at_tail(
                    new_client_port, sizeof(new_client_port), 0);
            }
...
void *tcpsvr_handle_blocking_request(void *_req)
{
    ...
    read_reply = (REMOTE_BLOCKING_READ_REPLY *)server->process_request(
        blocking_read_req);
    ...
    if (read_reply->size < (0x2000 - 20) && read_reply->size > 0) {
        ...
        _client_tcp_port->blocking = 0;
        if (sendn(_client_tcp_port->socket_fd,
                  temp_buffer,
                  20 + read_reply->size,
                  0,
                  dtimeout) < 0) {
...
void CMS_SERVER_REMOTE_TCP_PORT::handle_request(
    CLIENT_TCP_PORT *_client_tcp_port)
{
    ...
    if (NULL == server) {
        rcs_print_error(
            "CMS_SERVER_REMOTE_TCP_PORT::handle_request() Cannot find server "
            "object for pid = %d.\n",
            pid);
        return;
    }
    ...
    if (received_serial_number != _client_tcp_port->serial_number) {
        rcs_print_error(
            "received_serial_number (%ld) does not equal expected serial "
            "number.(%ld)\n",
    ...
    rcs_print_debug(PRINT_ALL_SOCKET_REQUESTS,
                    "TCPSVR request received: fd = %d, serial_number=%ld, "
...
void CMS_SERVER_REMOTE_TCP_PORT::switch_function(
    CLIENT_TCP_PORT *_client_tcp_port,
    CMS_SERVER *server,
    long request_type,
    long buffer_number,
...
)
{
    ...
    case REMOTE_CMS_SET_DIAG_INFO_REQUEST_TYPE: {
        if (NULL == _client_tcp_port->diag_info) {
            _client_tcp_port->diag_info = new REMOTE_SET_DIAG_INFO_REQUEST();
        }
        if (recvn(_client_tcp_port->socket_fd,
                  server->set_diag_info_buf,
                  68,
                  0,
                  -1,
                  NULL) < 0) {
        ...
        if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
            _client_tcp_port->diag_info->rcslib_ver =
                (double)tcp_svr_reverse_double(
                    (double)_client_tcp_port->diag_info->rcslib_ver);
        }
    } break;
    ...
    case REMOTE_CMS_GET_DIAG_INFO_REQUEST_TYPE: {
        ...
        diagreply =
            (REMOTE_GET_DIAG_INFO_REPLY *)server->process_request(&diagreq);
        if (NULL == diagreply) {
            putbe32(temp_buffer, _client_tcp_port->serial_number);
            putbe32(temp_buffer + 4, CMS_SERVER_SIDE_ERROR);
            if (sendn(
                    _client_tcp_port->socket_fd, temp_buffer, 24, 0, dtimeout) <
                0) {
                _client_tcp_port->errors++;
            }
            return;
        ...
        if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
            reversed_temp = (double)tcp_svr_reverse_double((double)curtime);
        ...
        if (NULL != diagreply->cdi->dpis) {
            ...
            while ((dpi_offset < ((int)0x2000 - sizeof(CMS_DIAG_PROC_INFO))) &&
                ...
                *((uint32_t *)((char *)temp_buffer + dpi_offset)) =
                ...
                if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
                    reversed_temp =
                        (double)tcp_svr_reverse_double((double)dpi->rcslib_ver);
                    memcpy(temp_buffer + dpi_offset, &reversed_temp, 8);
                } else {
                    memcpy(temp_buffer + dpi_offset, &(dpi->rcslib_ver), 8);
                }
                dpi_offset += 8;
                *((uint32_t *)((char *)temp_buffer + dpi_offset)) =
                ...
                *((uint32_t *)((char *)temp_buffer + dpi_offset)) =
                ...
                dpi_offset += 4;
                if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
                    reversed_temp = (double)tcp_svr_reverse_double(
                        (double)dpi->bytes_moved);
                    memcpy(temp_buffer + dpi_offset, &reversed_temp, 8);
                } else {
                    memcpy(temp_buffer + dpi_offset, &(dpi->bytes_moved), 8);
                }
                dpi_offset += 8;
                if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
                    reversed_temp = (double)tcp_svr_reverse_double(
                        (double)dpi->bytes_moved_across_socket);
                    memcpy(temp_buffer + dpi_offset, &reversed_temp, 8);
                } else {
                ...
                if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
                    reversed_temp = (double)tcp_svr_reverse_double(
                        (double)dpi->last_access_time);
                    memcpy(temp_buffer + dpi_offset, &reversed_temp, 8);
                } else {
                    memcpy(
                        temp_buffer + dpi_offset, &(dpi->last_access_time), 8);
                }
                dpi_offset += 8;
                if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
                    reversed_temp = (double)tcp_svr_reverse_double(
                        (double)dpi->first_access_time);
                    memcpy(temp_buffer + dpi_offset, &reversed_temp, 8);
                } else {
                    memcpy(
                        temp_buffer + dpi_offset, &(dpi->first_access_time), 8);
                }
                dpi_offset += 8;
                if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
                    reversed_temp = (double)tcp_svr_reverse_double(
                        (double)dpi->min_difference);
                    memcpy(temp_buffer + dpi_offset, &reversed_temp, 8);
                } else {
                    memcpy(temp_buffer + dpi_offset, &(dpi->min_difference), 8);
                }
                dpi_offset += 8;
                if (_client_tcp_port->diag_info->reverse_flag == 0x44332211) {
                    reversed_temp = (double)tcp_svr_reverse_double(
                        (double)dpi->max_difference);
                    memcpy(temp_buffer + dpi_offset, &reversed_temp, 8);
                } else {
                    memcpy(temp_buffer + dpi_offset, &(dpi->max_difference), 8);
                }
                dpi_offset += 8;
                ...
                *((uint32_t *)((char *)temp_buffer + dpi_offset)) =
                ...
                dpi_offset += 4;
                dpi = (CMS_DIAG_PROC_INFO *)diagreply->cdi->dpis->get_next();
            }
        }
    ...
    case REMOTE_CMS_GET_BUF_NAME_REQUEST_TYPE: {
        ...
        if (NULL != namereply) {
            ...
            if (sendn(
                    _client_tcp_port->socket_fd, temp_buffer, 40, 0, dtimeout) <
                0) {
                _client_tcp_port->errors++;
                return;
            }
    ...
    case REMOTE_CMS_BLOCKING_READ_REQUEST_TYPE: {
        ...
        blocking_read_req->access_type = ntohl(*((uint32_t *)temp_buffer + 3));
        blocking_read_req->last_id_read = ntohl(*((uint32_t *)temp_buffer + 4));
        ...
        if (total_subdivisions > 1) {
            ...
            blocking_read_req->subdiv = ntohl(*((uint32_t *)temp_buffer + 6));
        } else {
            if (recvn(_client_tcp_port->socket_fd,
                      (char *)(((uint32_t *)temp_buffer) + 5),
                      4,
...
#ifdef POSIX_THREADS
        int thr_retval = pthread_create(
            &(_client_tcp_port->threadId), /* ptr to new-thread-id */
        ...
        if (thr_retval != 0) {
            ...
            *((uint32_t *)temp_buffer) = htonl(_client_tcp_port->serial_number);
            *((uint32_t *)temp_buffer + 1) =
...
#else
        rcs_print_error("Blocking read not supported on this platform.\n");
        *((uint32_t *)temp_buffer) = htonl(_client_tcp_port->serial_number);
        *((uint32_t *)temp_buffer + 1) =
            htonl((unsigned long)CMS_SERVER_SIDE_ERROR);
        putbe32(temp_buffer + 8, 0);  /* size */
    ...
    case REMOTE_CMS_READ_REQUEST_TYPE:
        ...
        if (server->read_reply->size < (0x2000 - 20) &&
            ...
            memcpy(temp_buffer + 20,
                   server->read_reply->data,
                   server->read_reply->size);
            if (sendn(_client_tcp_port->socket_fd,
                      temp_buffer,
                      20 + server->read_reply->size,
                      0,
                      dtimeout) < 0) {
                _client_tcp_port->errors++;
                return;
            ...
            if (sendn(
                    _client_tcp_port->socket_fd, temp_buffer, 20, 0, dtimeout) <
                0) {
                _client_tcp_port->errors++;
                return;
            }
    ...
    case REMOTE_CMS_WRITE_REQUEST_TYPE:
        ...
        if (server->write_req.size > 0) {
            if (recvn(_client_tcp_port->socket_fd,
                      server->write_req.data,
                      server->write_req.size,
                      0,
                      -1,
                      NULL) < 0) {
                _client_tcp_port->errors++;
        ...
        server->write_reply = reply =
            (REMOTE_WRITE_REPLY *)server->process_request(&server->write_req);
        if (((min_compatible_version < 2.58) &&
            ...
            if (NULL == server->write_reply) {
                ...
                sendn(
                    _client_tcp_port->socket_fd, temp_buffer, 12, 0, dtimeout);
                return;
            }
            putbe32(temp_buffer, reply->write_id);
    ...
    case REMOTE_CMS_GET_MSG_COUNT_REQUEST_TYPE:
        ...
        server->get_msg_count_reply =
            (REMOTE_GET_MSG_COUNT_REPLY *)server->process_request(
                &server->get_msg_count_req);
    ...
    case REMOTE_CMS_GET_QUEUE_LENGTH_REQUEST_TYPE:
        ...
        server->get_queue_length_reply =
            (REMOTE_GET_QUEUE_LENGTH_REPLY *)server->process_request(
                &server->get_queue_length_req);
    ...
    case REMOTE_CMS_GET_SPACE_AVAILABLE_REQUEST_TYPE:
        ...
        server->get_space_available_reply =
            (REMOTE_GET_SPACE_AVAILABLE_REPLY *)server->process_request(
    ...
    case REMOTE_CMS_CLEAR_REQUEST_TYPE:
        ...
        server->clear_reply =
            (REMOTE_CLEAR_REPLY *)server->process_request(&server->clear_req);
    ...
    case REMOTE_CMS_GET_KEYS_REQUEST_TYPE:
        ...
        server->get_keys_reply =
            (REMOTE_GET_KEYS_REPLY *)server->process_request(
                &server->get_keys_req);
        if (NULL == server->get_keys_reply) {
            ...
            putbe32(temp_buffer, _client_tcp_port->serial_number);
            memcpy(((char *)temp_buffer) + 4, server->get_keys_reply->key1, 8);
            memcpy(((char *)temp_buffer) + 12, server->get_keys_reply->key2, 8);
            /* successful ? */
            sendn(_client_tcp_port->socket_fd, temp_buffer, 20, 0, dtimeout);
            return;
        }
    ...
    case REMOTE_CMS_LOGIN_REQUEST_TYPE:
        ...
        server->login_reply =
            (REMOTE_LOGIN_REPLY *)server->process_request(&server->login_req);
    ...
    case REMOTE_CMS_SET_SUBSCRIPTION_REQUEST_TYPE:
        ...
        server->set_subscription_reply =
            (REMOTE_SET_SUBSCRIPTION_REPLY *)server->process_request(
...
void CMS_SERVER_REMOTE_TCP_PORT::add_subscription_client(
    int buffer_number,
    ...
    CLIENT_TCP_PORT *clnt)
{
    ...
    if (NULL == temp_clnt_info) {
        ...
        temp_clnt_info->subscription_list_id =
            clnt->subscriptions->store_at_tail(
                temp_clnt_info, sizeof(*temp_clnt_info), 0);
        buf_info->sub_clnt_info->store_at_tail(
            temp_clnt_info, sizeof(*temp_clnt_info), 0);
    }
...
void CMS_SERVER_REMOTE_TCP_PORT::remove_subscription_client(
        ...
        if (temp_clnt_info->buffer_number == buffer_number) {
            if (NULL != temp_clnt_info->sub_buf_info) {
                if (NULL != temp_clnt_info->sub_buf_info->sub_clnt_info) {
                    temp_clnt_info->sub_buf_info->sub_clnt_info->delete_node(
                        temp_clnt_info->subscription_list_id);
...
void CMS_SERVER_REMOTE_TCP_PORT::update_subscriptions()
{
    ...
    if (NULL == server) {
        rcs_print_error(
            "CMS_SERVER_REMOTE_TCP_PORT::update_subscriptions Cannot find "
            "server object for pid = %d.\n",
            pid);
        return;
    }
    if (NULL == subscription_buffers) {
    ...
    while (NULL != buf_info) {
        ...
        while (temp_clnt_info != NULL) {
            ...
            rcs_print_debug(PRINT_SERVER_SUBSCRIPTION_ACTIVITY,
                            ...
                            time_diff_millis);
            if (((temp_clnt_info->subscription_type ==
                      CMS_POLLED_SUBSCRIPTION &&
                  time_diff_millis + 10 >=
...
CLIENT_TCP_PORT::CLIENT_TCP_PORT()
    : serial_number(0), errors(0), max_errors(50), socket_fd(-1),
      subscriptions(NULL), tid(-1), pid(-1), blocking(0), threadId(0),
      blocking_read_req(NULL), diag_info(NULL)
{
