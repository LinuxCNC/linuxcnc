...
int CMS_ASCII_UPDATER::set_mode(CMS_UPDATER_MODE _mode)
{
    ...
    default: rcs_print_error("CMS updater in invalid mode.\n"); return (-1);
    }
    return (0);
}
...
int CMS_ASCII_UPDATER::check_pointer(char *_pointer, long _bytes)
{
    ...
    if (length_current_string + _bytes * neutral_size_factor >
        max_length_current_string) {
        rcs_print_error(
            "CMS_ASCII_UPDATER: length of current string(%ld) + bytes to add "
            "of(%ld) exceeds maximum of %ld.\n",
            length_current_string,
            _bytes * neutral_size_factor,
            max_length_current_string);
        return (-1);
    }
...
CMS_STATUS CMS_ASCII_UPDATER::update(short int &x)
{
    ...
    if (encoding) {
        ...
        if (end_current_string[7] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) short with value %-6d caused an "
                "overflow.\n",
                x);
        }
        end_current_string[7] = 0;
        ...
        if (errno != 0) {
            rcs_print_error(
                "CMS_ASCII_UPDATER: Error %d: %s occurred during strtol "
                "of(%s).\n",
                errno,
                strerror(errno),
                end_current_string);
        ...
        if ((number < SHRT_MIN || SHRT_MAX < number) &&
            ...
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER:  (warning) Number %ld out of range for "
                "short(%d,%d)\n",
                number,
                SHRT_MIN,
                SHRT_MAX);
...
CMS_STATUS CMS_ASCII_UPDATER::update(unsigned short int &x)
{
    ...
    if (encoding) {
        ...
        if (end_current_string[7] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) unsigned short with value %-6d "
                "caused an overflow.\n",
                x);
        }
        end_current_string[7] = 0;
        ...
        if (errno != 0) {
            rcs_print_error(
                "CMS_ASCII_UPDATER: Error %d: %s occurred during strtoul of "
                "(%s).\n",
                errno,
                strerror(errno),
                end_current_string);
        ...
        if (number > USHRT_MAX && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: Number %lu out of range for unsigned "
                "short(0,%d)\n",
                number,
                USHRT_MAX);
        }
...
CMS_STATUS CMS_ASCII_UPDATER::update(int &x)
{
    ...
    if (encoding) {
        ...
        if (end_current_string[7] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) int with value %-6d caused an "
                "overflow.\n",
                x);
        }
        end_current_string[7] = 0;
        ...
        if (errno != 0) {
            rcs_print_error(
                "CMS_ASCII_UPDATER: Error %d: %s occurred during strtol of "
                "(%s).\n",
                errno,
                strerror(errno),
                end_current_string);
...
CMS_STATUS CMS_ASCII_UPDATER::update(unsigned int &x)
{
    ...
    if (encoding) {
        if (x > 9999999 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: unsigned int %d is too large. (Use type "
                "long.)\n",
                x);
        }
        end_current_string[7] = 0;
        ...
        if (end_current_string[7] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning)unsigned int with value %-6d "
                "caused an overflow.\n",
                x);
        }
        end_current_string[7] = 0;
        ...
        if (UINT_MAX < number && warning_count < warning_count_max) {
            rcs_print_error(
                "CMS_ASCII_UPDATER: Number %lu out of range for unsigned int "
                "(0,%u)\n",
                number,
                UINT_MAX);
        }
...
CMS_STATUS CMS_ASCII_UPDATER::update(long int &x)
{
    ...
    if (encoding) {
        ...
        if (end_current_string[15] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) long with value %-14ld caused an "
                "overflow\n",
                x);
        }
        end_current_string[15] = 0;
...
CMS_STATUS CMS_ASCII_UPDATER::update(unsigned long int &x)
{
    ...
    if (encoding) {
        ...
        if (end_current_string[15] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) unsigned long with value %-14ld "
                "caused an overflow\n",
                x);
        }
        end_current_string[15] = 0;
        ...
        if (errno != 0) {
            rcs_print_error(
                "CMS_ASCII_UPDATER: Error %d:%s occurred during strtoul "
                "of(%s).\n",
                errno,
                strerror(errno),
                end_current_string);
...
CMS_STATUS CMS_ASCII_UPDATER::update(float &x)
{
    ...
    if (encoding) {
        ...
        if (end_current_string[15] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) float with value %-13.7e caused "
                "an overflow\n",
                x);
        }
        end_current_string[15] = 0;
        ...
        if ((number < -FLT_MAX || FLT_MAX < number) &&
            ...
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) Number %lf out of range for "
                "float(%f,%f)\n",
                number,
                -FLT_MAX,
                FLT_MAX);
...
CMS_STATUS CMS_ASCII_UPDATER::update(double &x)
{
    ...
    if (encoding) {
        ...
        if (end_current_string[15] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) double with value %-13.7e caused "
                "an overflow\n",
                x);
        }
        end_current_string[15] = 0;
...
CMS_STATUS CMS_ASCII_UPDATER::update(long double &x)
{
    ...
    if (encoding) {
        ...
        if (end_current_string[15] != 0 && warning_count < warning_count_max) {
            warning_count++;
            rcs_print_error(
                "CMS_ASCII_UPDATER: (warning) long double with value %-13.7Le "
                "caused an overflow\n",
                x);
        }
        end_current_string[15] = 0;
