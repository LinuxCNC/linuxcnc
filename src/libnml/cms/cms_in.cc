...
/* This function should be called by classes which overload the main_access
    function. */
CMS_STATUS
CMS::internal_access(PHYSMEM_HANDLE *_global, void *_local, int *serial_number)
{
    ...
    if (total_subdivisions >= 1 && current_subdivision > 0 &&
        ...
        handle_to_global_data->size = ((current_subdivision + 1) * subdiv_size);
    ...
    if (!queuing_enabled) {
        if (neutral) {
            ...
            case CMS_CHECK_IF_READ_ACCESS: check_if_read_encoded(); break;
            case CMS_READ_ACCESS: read_encoded(); break;
            case CMS_PEEK_ACCESS: peek_encoded(); break;
            case CMS_WRITE_ACCESS: write_encoded(); break;
            case CMS_WRITE_IF_READ_ACCESS: write_if_read_encoded(); break;
            case CMS_GET_MSG_COUNT_ACCESS: get_msg_count_encoded(); break;
            default: return (status = CMS_INTERNAL_ACCESS_ERROR);
            }
        } else {
            switch (internal_access_type) {
            case CMS_CHECK_IF_READ_ACCESS: check_if_read_raw(); break;
            case CMS_READ_ACCESS: read_raw(); break;
            case CMS_PEEK_ACCESS: peek_raw(); break;
            case CMS_WRITE_ACCESS: write_raw(_local, serial_number); break;
            case CMS_WRITE_IF_READ_ACCESS:
                write_if_read_raw(_local, serial_number);
                break;
            case CMS_GET_MSG_COUNT_ACCESS: get_msg_count_raw(); break;
            default: return (status = CMS_INTERNAL_ACCESS_ERROR);
            }
        }
    } else {
        if (neutral) {
            switch (internal_access_type) {
            case CMS_CHECK_IF_READ_ACCESS: queue_check_if_read_encoded(); break;
            case CMS_READ_ACCESS: queue_read_encoded(); break;
            case CMS_PEEK_ACCESS: queue_peek_encoded(); break;
            case CMS_WRITE_ACCESS: queue_write_encoded(); break;
            case CMS_WRITE_IF_READ_ACCESS: queue_write_if_read_encoded(); break;
            ...
            case CMS_GET_SPACE_AVAILABLE_ACCESS:
                ...
                break;
            case CMS_GET_MSG_COUNT_ACCESS: queue_get_msg_count_encoded(); break;
            default: return (status = CMS_INTERNAL_ACCESS_ERROR);
            }
        } else {
            switch (internal_access_type) {
            case CMS_CHECK_IF_READ_ACCESS: queue_check_if_read_raw(); break;
            case CMS_READ_ACCESS: queue_read_raw(); break;
            case CMS_PEEK_ACCESS: queue_peek_raw(); break;
            ...
            case CMS_GET_SPACE_AVAILABLE_ACCESS:
                ...
                break;
            case CMS_GET_MSG_COUNT_ACCESS: queue_get_msg_count_raw(); break;
            default: return (status = CMS_INTERNAL_ACCESS_ERROR);
            }
        }
    }
...
int CMS::check_if_read_encoded()
{
    ...
    /* Read the header for the buffer. */
    if (-1 ==
        handle_to_global_data->read(encoded_header, encoded_header_size)) {
        rcs_print_error(
            "CMS: Error reading from global memory for %s at %s:%d\n",
            BufferName,
            __FILE__,
...
int CMS::queue_get_space_available_raw()
{
    ...
    if (queuing_header.tail > queuing_header.head) {
        ...
        if (handle_to_global_data->size - queuing_header.tail -
            ...
            0) {
            free_space = handle_to_global_data->size - queuing_header.tail -
                         queuing_header_offset;
        }
    ...
    if (queuing_header.queue_length == 0) {
        free_space = handle_to_global_data->size - sizeof(CMS_QUEUING_HEADER) -
                     queuing_header_offset;
    }
...
int CMS::queue_get_space_available_encoded()
{
    ...
    if (queuing_header.queue_length == 0) {
        free_space = handle_to_global_data->size - encoded_queuing_header_size -
                     queuing_header_offset;
    }
...
CMS_STATUS CMS::read_raw()
{
    ...
    if (!read_permission_flag) {
        rcs_print_error(
            "CMS: %s was not configured to read %s\n", ProcessName, BufferName);
    ...
    if (check_id(header.write_id) == CMS_READ_OK) {
        ...
        if (-1 == handle_to_global_data->read(subdiv_data,
...
CMS_STATUS CMS::queue_read_raw()
{
    ...
    /* Check the size of the message. */
    if (header.in_buffer_size > max_message_size) {
        rcs_print_error("CMS:(%s) Message size of %ld exceeds maximum of %ld\n",
                        BufferName,
                        header.in_buffer_size,
                        max_message_size);
        return (status = CMS_INTERNAL_ACCESS_ERROR);
...
CMS_STATUS CMS::write_raw(void *user_data, int *serial_number)
{
    ...
    if (!disable_final_write_raw_for_dma) {
        handle_to_global_data->offset += sizeof(CMS_HEADER);
        if (-1 == handle_to_global_data->write(
                      user_data, (long)current_header_in_buffer_size)) {
            rcs_print_error(
                "CMS:(%s) Error writing %ld bytes to global memory at offset "
                "%p\n (See  %s line %d.)\n",
                BufferName,
                header.in_buffer_size,
                user_data,
...
CMS_STATUS CMS::queue_write_raw(void *user_data, int *serial_number)
{
    ...
    if (free_space < ((long)(header.in_buffer_size + sizeof(CMS_HEADER)))) {
        if (cms_print_queue_free_space || cms_print_queue_full_messages) {
            ...
            rcs_print_error(
                "(continued) CMS: Message requires %ld bytes but only %ld "
                "bytes are left.\n",
                header.in_buffer_size,
                free_space);
        }
    ...
    handle_to_global_data->offset += sizeof(CMS_HEADER);
    if (-1 ==
        handle_to_global_data->write(user_data, (long)header.in_buffer_size)) {
        rcs_print_error("CMS:(%s) Error writing to global memory at %s:%d\n",
                        BufferName,
                        __FILE__,
                        __LINE__);
...
CMS_STATUS CMS::write_encoded()
{
    ...
    if (header.in_buffer_size > max_message_size) {
        rcs_print_error(
            "CMS:(%s) Message size %ld exceeds maximum for this buffer of "
            "%ld.\n",
            BufferName,
            header.in_buffer_size,
            max_message_size);
