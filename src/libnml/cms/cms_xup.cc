    ...
    : CMS_UPDATER(_cms_parent, 0, 2)
{
    ...
    xdrmem_create((XDR *)encode_header_stream,
                  (char *)encoded_header,
                  (int)neutral_size_factor * sizeof(CMS_HEADER),
                  XDR_ENCODE);
    ...
    xdrmem_create((XDR *)decode_header_stream,
                  (char *)encoded_header,
                  (int)neutral_size_factor * sizeof(CMS_HEADER),
                  XDR_DECODE);
                                       ...
                                       long _encoded_data_size)
{
    ...
    xdrmem_create(encode_data_stream, (char *)encoded_data, nsize, XDR_ENCODE);
    xdrmem_create(decode_data_stream, (char *)encoded_data, nsize, XDR_DECODE);
}
...
int CMS_XDR_UPDATER::set_mode(CMS_UPDATER_MODE _mode)
{
    ...
    switch (mode) {
    case CMS_NO_UPDATE: current_stream = (XDR *)NULL; break;

    case CMS_ENCODE_DATA: current_stream = encode_data_stream; break;

    case CMS_DECODE_DATA: current_stream = decode_data_stream; break;

    case CMS_ENCODE_HEADER: current_stream = encode_header_stream; break;

    case CMS_DECODE_HEADER: current_stream = decode_header_stream; break;
...
int CMS_XDR_UPDATER::check_pointer(char *_pointer, long _bytes)
{
    ...
    if ((current_stream == encode_data_stream) ||
        (current_stream == decode_data_stream) || (mode == CMS_ENCODE_DATA) ||
        (mode == CMS_DECODE_DATA)) {
        int xdr_pos = xdr_getpos(current_stream);
        if (xdr_pos + _bytes > encoded_data_size) {
            rcs_print_error(
                "Encoded message buffer full. "
                "(xdr_pos=%d,_bytes=%ld,(xdr_pos+_bytes)=%ld,encoded_data_size="
                "%ld)\n",
                xdr_pos,
                _bytes,
                (xdr_pos + _bytes),
...
int CMS_XDR_UPDATER::get_encoded_msg_size()
{
    if (NULL == current_stream) {
        rcs_print_error(
            "CMS_XDR_UPDATER can not provide encoded_msg_size because the "
            "current stream is NULL.\n");
        return (-1);
    }
...
CMS_STATUS CMS_XDR_UPDATER::update(short *x, unsigned int len)
{
    ...
    if (xdr_vector(current_stream,
        ...
        rcs_print_error("CMS_XDR_UPDATER: xdr_vector(... xdr_short) failed.\n");
        return (status = CMS_UPDATE_ERROR);
    }
    return (status);
}
...
CMS_STATUS CMS_XDR_UPDATER::update(int *x, unsigned int len)
{
    ...
    if (xdr_vector(
            current_stream, (char *)x, len, sizeof(int), (xdrproc_t)xdr_int) !=
        TRUE) {
...
CMS_STATUS CMS_XDR_UPDATER::update(unsigned int *x, unsigned int len)
{
    ...
    if (xdr_vector(current_stream,
                   ...
                   len,
                   sizeof(unsigned int),
                   (xdrproc_t)xdr_u_int) != TRUE) {
        rcs_print_error("CMS_XDR_UPDATER: xdr_vector(... xdr_u_int) failed.\n");
        return (status = CMS_UPDATE_ERROR);
    }
    return (status);
}
...
CMS_STATUS CMS_XDR_UPDATER::update(float *x, unsigned int len)
{
    ...
    if (xdr_vector(current_stream,
        ...
        rcs_print_error("CMS_XDR_UPDATER: xdr_vector(... xdr_float) failed.\n");
        return (status = CMS_UPDATE_ERROR);
    }
    return (status);
}
