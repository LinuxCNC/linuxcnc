...
struct CONFIG_FILE_INFO {
    CONFIG_FILE_INFO() : lines_list(NULL), file_name{} {};
...
int cms_copy(CMS **dest, CMS *src, int set_to_server, int set_to_master)
{
    ...
    return cms_create_from_lines(
        dest, src->BufferLine, src->ProcessLine, set_to_server, set_to_master);
}
               ...
               int set_to_master)
{
    ...
    case NO_PROCESS_LINE:
        rcs_print_error(
            "No process-line entry found for process %s connecting to buffer "
            "%s in config file %s and no applicable defaults were found.\n",
            procname,
            bufname,
            filename);
        break;
...
int hostname_matches_bufferline(char *bufline)
{
    ...
    while (j < num_my_hostent_addresses && j < 16) {
        ...
        while (k < 16 && buffer_hostent_ptr->h_addr_list[k] != 0) {
            if (!memcmp(my_hostent_addresses[j],
                        buffer_hostent_ptr->h_addr_list[k],
                        my_hostent.h_length)) {
                return 1;
            }
            k++;
...
void find_proc_and_buffer_lines(CONFIG_SEARCH_STRUCT *s)
{
    ...
    while (1) {
        ...
        if (line_len > LINELEN) {
            rcs_print_error(
                "cms_cfg: Line length of line number %d in %s exceeds max "
                "length of %d",
                line_number,
                s->filename,
                LINELEN);
        ...
        if (!s->bufline_found && !strcmp(word[1], s->bufname) &&
            ...
            rcs_print_debug(PRINT_CMS_CONFIG_INFO,
                   ...
                   line[0] == 'P' &&
                   !strcmp(word[2], s->bufname_for_procline)) {
            ...
            case CMS_NORMAL_CONNECTION_MODE:
                ...
                if (!strncmp(s->proc_type, "AUTO", 4)) {
                    if (!s->bufline_found) {
                        rcs_print_error(
                            "Can't use process type AUTO unless the buffer "
                            "line for %s is found earlier in the config "
                            "file.\n",
                            s->bufname);
...
int cms_create_from_lines(CMS **cms,
                          const char *buffer_line_in,
                          const char *proc_line_in,
                          int set_to_server,
                          int set_to_master)
{
...
/*! \todo Another #if 0 */
	    ...
	    *cms = new STCPMEM(buffer_line, proc_line);
		...
		return (-1);
            ...
            *cms = new TCPMEM(buffer_line, proc_line);
            ...
            if (NULL == *cms) {
                ...
                if (verbose_nml_error_messages) {
                    rcs_print_error(
                        "cms_config: Error  %d(%s) occurred during TPCMEM "
                        "create.\n",
                        (*cms)->status,
                        (*cms)->status_string((*cms)->status));
                }
        ...
        if (!strcmp(buffer_type, "SHMEM")) {
            *cms =
                new SHMEM(buffer_line, proc_line, set_to_server, set_to_master);
            rcs_print_debug(PRINT_CMS_CONFIG_INFO,
                            "%p = new SHMEM(%s,%s,%d,%d)\n",
                            *cms,
                            buffer_line,
            ...
            if (NULL == *cms) {
                ...
                if (verbose_nml_error_messages) {
                    rcs_print_error(
                        "cms_config: %d(%s) Error occurred during SHMEM "
                        "create.\n",
                        (*cms)->status,
                        (*cms)->status_string((*cms)->status));
                }
        ...
        if (!strcmp(buffer_type, "LOCMEM")) {
            *cms = new LOCMEM(
            ...
            if ((*cms)->status < 0) {
                if (verbose_nml_error_messages) {
                    rcs_print_error(
                        "cms_config: %d(%s) Error occurred during LOCMEM "
                        "create.\n",
                        (*cms)->status,
                        (*cms)->status_string((*cms)->status));
                }
