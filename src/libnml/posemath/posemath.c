...
int pmCartAbsMax(PmCartesian const * const v, double * out)
{
    *out = fmax(fmax(fabs(v->x),fabs(v->y)),fabs(v->z));
    return pmErrno = 0;
}
    ...
    double expected_angle_rad)
{
    ...
    // KLUDGE explicit +0 is the default so this comparison is safe
    if (expected_angle_rad != 0.) {
        double missing_turns = round((fabs(expected_angle_rad) - circle->angle)/(PM_TAU));
        circle->angle += missing_turns * PM_TAU;
    }
