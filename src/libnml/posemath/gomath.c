...
go_pose go_pose_this(go_real x,
                     go_real y,
                     go_real z,
                     go_real rs,
                     go_real rx,
...
{
    if (GO_TRAN_CLOSE(v1->x, v2->x) && GO_TRAN_CLOSE(v1->y, v2->y) &&
...
int go_cart_cart_dot(const go_cart *v1, const go_cart *v2, go_real *d)
{
...
int go_cart_cart_cross(const go_cart *v1, const go_cart *v2, go_cart *vout)
{
    go_cart cp1, cp2;
...
int go_cart_cart_disp(const go_cart *v1, const go_cart *v2, go_real *d)
{
    *d = sqrt(go_sq(v2->x - v1->x) + go_sq(v2->y - v1->y) +
              go_sq(v2->z - v1->z));

    return GO_RESULT_OK;
}

int go_cart_cart_add(const go_cart *v1, const go_cart *v2, go_cart *vout)
{
    vout->x = v1->x + v2->x;
    vout->y = v1->y + v2->y;
    vout->z = v1->z + v2->z;
...
int go_cart_cart_sub(const go_cart *v1, const go_cart *v2, go_cart *vout)
{
    vout->x = v1->x - v2->x;
    vout->y = v1->y - v2->y;
    vout->z = v1->z - v2->z;
...
int go_cart_cart_rot(const go_cart *v1, const go_cart *v2, go_quat *quat)
{
    int retval;
    go_cart u1;
    go_cart u2;
...
int go_cart_cart_proj(const go_cart *v1, const go_cart *v2, go_cart *vout)
{
    go_cart v2u;
    go_real d;
    int retval;
...
int go_cart_plane_proj(const go_cart *v, const go_cart *normal, go_cart *vout)
{
    go_cart par;
    int retval;
...
int go_cart_cart_angle(const go_cart *v1, const go_cart *v2, go_real *a)
{
    go_real dot, m1, m2;
...
int go_cart_centroid(const go_cart *varray, go_integer num, go_cart *centroid)
{
    go_integer i;
...
void go_complex_cbrt(go_complex z,
                     go_complex *z1,
                     go_complex *z2,
                     go_complex *z3)
{
...
int go_quadratic_solve(const go_quadratic *quad, go_complex *z1, go_complex *z2)
{
    go_real discr;
                     ...
                     go_complex *z4)
{
  ...
  */
    ...
    a4 = a2 * a2;
    g = quart->d - 3.0 / 256.0 * a4 + 1.0 / 16.0 * a2 * quart->b -
        0.25 * (quart->a * quart->c);
    /*
  ...
  */
    ...
    r = go_complex_div(
        fc, go_complex_scale(go_complex_mult(p, q), -8.0), &retval);
    if (GO_RESULT_OK != retval)
        return retval;
...
*/
int go_tridiag_ql(go_real *d, go_real *e, go_integer n, go_real **z)
{
...
int go_cart_cart_pose(const go_cart *v1,
                      const go_cart *v2,
                      go_cart *v1c,
                      go_cart *v2c,
                      go_integer num,
...
static int trilaterate(go_real x2,
                       go_real x3,
                       go_real y3,
                       go_real l1,
                       go_real l2,
                       ...
                       go_cart *p)
{
    ...
    p->y = (go_sq(l1) - 2.0 * p->x * x3 + go_sq(x3) + go_sq(y3) - go_sq(l3)) /
           (2.0 * y3);
    ...
    if (discr < 0.0)
        discr = 0.0; /* make slightly negative numbers 0 */
...
{
    if (GO_ROT_CLOSE(r1->x, r2->x) && GO_ROT_CLOSE(r1->y, r2->y) &&
...
#define COL_IS_UNIT(r)                                                         \
    GO_TRAN_CLOSE(go_sq((r).x) + go_sq((r).y) + go_sq((r).z), 1)
    return (COL_IS_UNIT(m->x) && COL_IS_UNIT(m->y) && COL_IS_UNIT(m->z) &&
...
int go_mat_cart_mult(const go_mat *m, const go_cart *v, go_cart *vout)
{
    go_cart cp;
...
{
    if (GO_ROT_CLOSE(q1->s, q2->s) && GO_TRAN_CLOSE(q1->x, q2->x) &&
        GO_TRAN_CLOSE(q1->y, q2->y) && GO_TRAN_CLOSE(q1->z, q2->z)) {
        return 1;
    }

    /* note (0, x, y, z) = (0, -x, -y, -z) */
    if (!GO_ROT_SMALL(q1->s) || GO_TRAN_CLOSE(q1->x, -q2->x) ||
        GO_TRAN_CLOSE(q1->y, -q2->y) || GO_TRAN_CLOSE(q1->z, -q2->z)) {
        return 0;
    }
...
{
    return GO_TRAN_CLOSE(
        go_sq(q1->s) + go_sq(q1->x) + go_sq(q1->y) + go_sq(q1->z), 1);
}
...
int go_quat_quat_mult(const go_quat *q1, const go_quat *q2, go_quat *qout)
{
    go_quat cp1, cp2;
    ...
    if (qout->s >= 0) {
        ...
        qout->s = -qout->s;
        qout->x =
            -cp1.s * cp2.x - cp1.x * cp2.s - cp1.y * cp2.z + cp1.z * cp2.y;
        qout->y =
            -cp1.s * cp2.y + cp1.x * cp2.z - cp1.y * cp2.s - cp1.z * cp2.x;
        qout->z =
            -cp1.s * cp2.z - cp1.x * cp2.y + cp1.y * cp2.x - cp1.z * cp2.s;
    }

    return GO_RESULT_OK;
}

int go_quat_cart_mult(const go_quat *q1, const go_cart *v2, go_cart *vout)
{
    go_cart c;
...
int go_pose_pose_interp(go_real t1,
                        const go_pose *p1,
                        go_real t2,
                        const go_pose *p2,
                        go_real t3,
...
int go_pose_screw_mult(const go_pose *pose,
                       const go_screw *screw,
                       go_screw *out)
{
    go_pose poseinv;
    go_cart wxp;
...
int go_line_from_poGO_RESULT_direction(const go_cart *point,
                                       const go_cart *direction,
                                       go_line *line)
{
    int retval;
...
int go_line_from_points(const go_cart *point1,
                        const go_cart *point2,
                        go_line *line)
{
    go_cart direction;
    int retval;
...
int go_line_from_planes(const go_plane *plane1,
                        const go_plane *plane2,
                        go_line *line)
{
    enum { X, Y, Z } which;
    go_real max, ymax, denominv;

    (void)go_cart_cart_cross(
        &plane1->normal, &plane2->normal, &line->direction);
    ...
    case X:
        ...
        line->point.y =
            (plane1->normal.z * plane2->d - plane2->normal.z * plane1->d) *
            denominv;
        line->point.z =
    ...
    case Y:
        ...
        line->point.z =
            (plane1->normal.x * plane2->d - plane2->normal.x * plane1->d) *
            denominv;
        line->point.x =
            (plane2->normal.z * plane1->d - plane1->normal.z * plane2->d) *
            denominv;
        line->point.y = 0;
        break;
    default:
        ...
        line->point.x =
            (plane1->normal.y * plane2->d - plane2->normal.y * plane1->d) *
            denominv;
        line->point.y =
...
int go_poGO_RESULT_line_distance(const go_cart *point,
                                 const go_line *line,
                                 go_real *distance)
{
    *distance = sqrt(go_sq(line->direction.z * (point->y - line->point.y) -
...
int go_poGO_RESULT_line_proj(const go_cart *point,
                             const go_line *line,
                             go_cart *pout)
{
    go_cart vp;
...
int go_poGO_RESULT_plane_proj(const go_cart *point,
                              const go_plane *plane,
                              go_cart *proj)
{
    go_real denom;
    go_real k;

    denom = go_sq(plane->normal.x) + go_sq(plane->normal.y) +
            go_sq(plane->normal.z);

    if (GO_TRAN_SMALL(denom))
        return GO_RESULT_DIV_ERROR;

    k = -(plane->normal.x * point->x + plane->normal.y * point->y +
...
int go_line_plane_proj(const go_line *line,
                       const go_plane *plane,
                       go_line *proj)
{
    int retval;
    ...
    retval =
        go_cart_plane_proj(&line->direction, &plane->normal, &proj->direction);
    if (GO_RESULT_OK != retval)
        return retval;
...
int go_plane_from_poGO_RESULT_normal(const go_cart *point,
                                     const go_cart *normal,
                                     go_plane *plane)
{
    if (GO_RESULT_OK != go_cart_unit(normal, &plane->normal))
        return GO_RESULT_ERROR;
    /* D = -(Ax + By + Cz) */
    plane->d = -(plane->normal.x * point->x + plane->normal.y * point->y +
                 plane->normal.z * point->z);

    return GO_RESULT_OK;
}

int go_plane_from_abcd(
    go_real A, go_real B, go_real C, go_real D, go_plane *plane)
{
    go_real mag;
...
int go_plane_from_points(const go_cart *point1,
                         const go_cart *point2,
                         const go_cart *point3,
                         go_plane *plane)
{
    go_cart v12, v23;
    ...
    if (GO_RESULT_OK != go_cart_unit(&plane->normal, &plane->normal))
        return GO_RESULT_ERROR;
    /* D = -(Ax + By + Cz) */
    plane->d = -(plane->normal.x * point1->x + plane->normal.y * point1->y +
                 plane->normal.z * point1->z);

    return GO_RESULT_OK;
}

int go_plane_from_poGO_RESULT_line(const go_cart *point,
                                   const go_line *line,
                                   go_plane *plane)
{
    go_cart p0, p1;
...
{
    if (!go_cart_cart_compare(&plane1->normal, &plane2->normal))
        return 0;
    return GO_CLOSE(plane1->d, plane2->d);
}

int go_poGO_RESULT_plane_distance(const go_cart *point,
                                  const go_plane *plane,
                                  go_real *distance)
{
    *distance = plane->normal.x * point->x + plane->normal.y * point->y +
                plane->normal.z * point->z + plane->d;
                      ...
                      go_cart *point)
{
    ...
    if (GO_RESULT_OK != go_cart_normal(&plane->normal, &v1))
        return GO_RESULT_ERROR;
...
*/
int go_line_plane_intersect(const go_line *line,
                            const go_plane *plane,
                            go_cart *point,
                            go_real *distance)
{
...
int lubksb(go_real **a, go_integer n, go_integer *indx, go_real *b)
{
...
int go_cart_vector_convert(const go_cart *c, go_real *v)
{
...
int go_vector_cart_convert(const go_real *v, go_cart *c)
{
...
int go_quat_matrix_convert(const go_quat *quat, go_matrix *matrix)
{
    go_mat mat;
    int retval;
...
int go_mat_matrix_convert(const go_mat *mat, go_matrix *matrix)
{
    /* check for an initialized matrix */
    if (0 == matrix->el[0])
        return GO_RESULT_ERROR;
    ...
    matrix->el[0][0] = mat->x.x, matrix->el[0][1] = mat->y.x,
    matrix->el[0][2] = mat->z.x;
    matrix->el[1][0] = mat->x.y, matrix->el[1][1] = mat->y.y,
    matrix->el[1][2] = mat->z.y;
    matrix->el[2][0] = mat->x.z, matrix->el[2][1] = mat->y.z,
    matrix->el[2][2] = mat->z.z;

    return GO_RESULT_OK;
}

int go_matrix_matrix_add(const go_matrix *a, const go_matrix *b, go_matrix *apb)
{
    go_integer row, col;

    /* check for an initialized matrix */
    if (0 == a->el[0] || 0 == b->el[0] || 0 == apb->el[0])
        return GO_RESULT_ERROR;
    /* check for matching rows and cols */
    if (a->rows != b->rows || a->cols != b->cols || b->rows != apb->rows ||
        b->cols != apb->cols)
        return GO_RESULT_ERROR;
...
int go_matrix_matrix_copy(const go_matrix *src, go_matrix *dst)
{
    go_integer row, col;

    /* check for an initialized matrix */
    if (0 == src->el[0] || 0 == dst->el[0])
        return GO_RESULT_ERROR;
...
int go_matrix_matrix_mult(const go_matrix *a, const go_matrix *b, go_matrix *ab)
{
    go_real **ptrin;
    go_real **ptrout;
    ...
    /* check for an initialized matrix */
    if (0 == a->el[0] || 0 == b->el[0] || 0 == ab->el[0])
        return GO_RESULT_ERROR;
    /* check for consistent rows and cols */
    if (a->cols != b->rows || a->rows != ab->rows || b->cols != ab->cols)
        return GO_RESULT_ERROR;
                           ...
                           go_matrix *axv)
{
    ...
    for (col = 0; col < a->cols; col++) {
        ...
        ptrin[0][col] = cross.x, ptrin[1][col] = cross.y,
        ptrin[2][col] = cross.z;
    }
...
int go_matrix_transpose(const go_matrix *a, go_matrix *at)
{
    go_real **ptrin;
    go_real **ptrout;
    go_integer row, col;
...
extern int go_mat3_inv(const go_real a[3][3], go_real ainv[3][3])
{
    go_real a11a22;
    go_real a11a23;
    go_real a11a32;
    ...
    denom = +a11a22 * a[2][2] - a11a23 * a[2][1] - a12a33 * a[1][0] +
            a13a21 * a[2][1] - a13a22 * a[2][0] + a12a23 * a[2][0];
...
int go_mat3_vec3_mult(const go_real a[3][3], const go_real v[3], go_real axv[3])
{
    go_real work[3];
    go_integer i, j;
...
extern int go_mat4_inv(const go_real a[4][4], go_real ainv[4][4])
{
    go_real work[4][4];
    go_real denom;
    ...
    denom = +a12a24a33 * a[3][0] + a13a24a31 * a[3][1] - a12a21a33 * a[3][3] -
            a13a24a32 * a[3][0] - a14a21a32 * a[3][2] - a12a24a31 * a[3][2] +
            a14a23a32 * a[3][0] + a13a21a32 * a[3][3] + a14a21a33 * a[3][1] +
            a12a23a31 * a[3][3] - a14a23a31 * a[3][1] + a12a21a34 * a[3][2] -
            a12a23a34 * a[3][0] - a13a21a34 * a[3][1] - a13a22a31 * a[3][3] +
            a14a22a31 * a[3][2] + a13a22a34 * a[3][0] - a14a22a33 * a[3][0] +
            a11a22a33 * a[3][3] - a11a22a34 * a[3][2] + a11a24a32 * a[3][2] -
            a11a23a32 * a[3][3] + a11a23a34 * a[3][1] - a11a24a33 * a[3][1];
    ...
    work[0][0] = (+a22a33 * a[3][3] - a22a34 * a[3][2] + a24a32 * a[3][2] -
                  a23a32 * a[3][3] + a23a34 * a[3][1] - a24a33 * a[3][1]) *
                 denom;
    work[0][1] = (-a12a33 * a[3][3] + a12a34 * a[3][2] - a13a34 * a[3][1] +
                  a13a32 * a[3][3] - a14a32 * a[3][2] + a14a33 * a[3][1]) *
                 denom;
    work[0][2] = (+a12a23 * a[3][3] - a12a24 * a[3][2] - a13a22 * a[3][3] +
                  a13a24 * a[3][1] + a14a22 * a[3][2] - a14a23 * a[3][1]) *
                 denom;
    work[0][3] = (-a12a23a34 + a12a24a33 + a13a22a34 - a13a24a32 - a14a22a33 +
                  a14a23a32) *
                 denom;

    work[1][0] = (+a23a31 * a[3][3] - a23a34 * a[3][0] - a21a33 * a[3][3] +
                  a21a34 * a[3][2] + a24a33 * a[3][0] - a24a31 * a[3][2]) *
                 denom;
    work[1][1] = (+a11a33 * a[3][3] - a11a34 * a[3][2] - a13a31 * a[3][3] +
                  a14a31 * a[3][2] + a13a34 * a[3][0] - a14a33 * a[3][0]) *
                 denom;
    work[1][2] = (-a11a23 * a[3][3] + a11a24 * a[3][2] + a13a21 * a[3][3] +
                  a14a23 * a[3][0] - a14a21 * a[3][2] - a13a24 * a[3][0]) *
                 denom;
    work[1][3] = (+a11a23a34 - a11a24a33 - a13a21a34 + a14a21a33 - a14a23a31 +
                  a13a24a31) *
                 denom;

    work[2][0] = (+a22a34 * a[3][0] - a22a31 * a[3][3] - a24a32 * a[3][0] +
                  a21a32 * a[3][3] - a21a34 * a[3][1] + a24a31 * a[3][1]) *
                 denom;
    work[2][1] = (+a11a34 * a[3][1] - a11a32 * a[3][3] - a12a34 * a[3][0] -
                  a14a31 * a[3][1] + a12a31 * a[3][3] + a14a32 * a[3][0]) *
                 denom;
    work[2][2] = (-a11a24 * a[3][1] - a14a22 * a[3][0] + a11a22 * a[3][3] +
                  a12a24 * a[3][0] + a14a21 * a[3][1] - a12a21 * a[3][3]) *
                 denom;
    work[2][3] = (-a11a22a34 + a11a24a32 + a14a22a31 - a12a24a31 + a12a21a34 -
                  a14a21a32) *
                 denom;

    work[3][0] = (+a21a33 * a[3][1] + a23a32 * a[3][0] - a22a33 * a[3][0] -
                  a23a31 * a[3][1] + a22a31 * a[3][2] - a21a32 * a[3][2]) *
                 denom;
    work[3][1] = (+a12a33 * a[3][0] - a13a32 * a[3][0] - a12a31 * a[3][2] +
                  a13a31 * a[3][1] + a11a32 * a[3][2] - a11a33 * a[3][1]) *
                 denom;
    work[3][2] = (-a12a23 * a[3][0] + a12a21 * a[3][2] - a13a21 * a[3][1] +
                  a13a22 * a[3][0] - a11a22 * a[3][2] + a11a23 * a[3][1]) *
                 denom;
    work[3][3] = (+a13a21a32 + a12a23a31 - a12a21a33 - a13a22a31 + a11a22a33 -
                  a11a23a32) *
                 denom;
...
int go_mat4_vec4_mult(const go_real a[4][4], const go_real v[4], go_real axv[4])
{
    go_real work[4];
    go_integer i, j;
...
int go_mat6_inv(const go_real a[6][6], go_real ainv[6][6])
{
    go_real cpy[6][6];
    go_real *cpyptr[6];
...
int go_mat6_vec6_mult(const go_real a[6][6], const go_real v[6], go_real axv[6])
{
    go_real work[6];
    go_integer i, j;
...
int go_link_joint_set(const go_link *link, go_real joint, go_link *linkout)
{
    ...
    if (GO_LINK_PP == link->type) {
        ...
        if (GO_QUANTITY_LENGTH == link->quantity) {
            pose.tran.z = joint;
            return go_pose_pose_mult(
                &link->u.pp.pose, &pose, &linkout->u.pp.pose);
        }
        /* else revolute */
...
int go_link_pose_build(const go_link *link_params,
                       go_integer num,
                       go_pose *pose)
{
    go_pose p;
