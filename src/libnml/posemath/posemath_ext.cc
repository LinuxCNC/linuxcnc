...
int PM_CIRCLE::init(PM_POSE start, PM_POSE end,
    PM_CARTESIAN _center, PM_CARTESIAN _normal, int turn, double expected_angle_rad)
{
    int retval;
    retval = pmCircleInit(this, &start.tran, &end.tran, &_center, &_normal, turn, expected_angle_rad);
    return retval;
}

int PM_CIRCLE::point(double angle_, PM_POSE * point)
{
    return pmCirclePoint(this, angle_, &point->tran);
}
