...
void set_default_nml_config_file(const char *const cfg_file)
{
    ...
    if (default_nml_config_file)
        free(default_nml_config_file);
...
void *NML::operator new(size_t size)
{
    if (size < sizeof(NML)) {
        rcs_print_error(
            "void *NML::operator new() called with size (%zd) < sizeof(NML) "
            "(%zu) the code calling NML was probably not compiled with the "
            "correct header file version.\n",
            size,
            sizeof(NML));
        size = sizeof(NML);
    }
    ...
    if (NULL != Dynamically_Allocated_NML_Objects) {
        dynamic_list_id = Dynamically_Allocated_NML_Objects->store_at_tail(
            nml_space, sizeof(NML), 0);
...
void NML::operator delete(void *nml_space)
{
    ...
    rcs_print_debug(
        PRINT_NML_DESTRUCTORS, "NML::operator delete(%p)\n", nml_space);
         ...
         const int set_to_master)
{
    ...
    if (rcs_errors_printed >= max_rcs_errors_to_print &&
        ...
        rcs_print(
            "\nResetting rcs_errors_printed because a new NML channel is being "
            "created.\n");
    }
...
void NML::reconstruct(NML_FORMAT_PTR f_ptr,
                      const char *buf,
                      const char *proc,
                      const char *file,
                      const int set_to_server,
                      const int set_to_master)
{
    ...
    if (-1 == cms_config(&cms, buf, proc, file, set_to_server, set_to_master)) {
        ...
        if (NULL != cms) {
            rcs_print_debug(
                PRINT_NML_DESTRUCTORS, " delete (CMS *) %p;\n", cms);
            delete cms;
            cms = (CMS *)NULL;
        }
        return;
    ...
    // NML/CMS
    // overhead.
    if (!cms->is_phantom && cms->ProcessType == CMS_LOCAL_TYPE &&
...
{
    rcs_print_debug(
        PRINT_NML_DESTRUCTORS, "deleting NML (%d)\n", channel_list_id);
...
NMLTYPE NML::read()
{
    ...
    switch (cms->status) {
    case CMS_READ_OLD: error_type = NML_NO_ERROR; return (0);
    case CMS_READ_OK:
        error_type = NML_NO_ERROR;
        if (((NMLmsg *)cms->subdiv_data)->_type <= 0 && !cms->isserver) {
            rcs_print_error(
...
NMLTYPE NML::blocking_read(double blocking_timeout)
{
    ...
    if (fast_mode) {
        ...
        case CMS_READ_OK:
            ...
            return (((NMLmsg *)cms->subdiv_data)->_type);
        case CMS_TIMED_OUT: error_type = NML_NO_ERROR; return 0;
...
int NML::write(NMLmsg *nml_msg, int *serial_number)
{
    ...
    if (CMS_RAW_IN == cms->mode) {
        cms->write(nml_msg,
                   serial_number); /* Write the unformatted message.  */
    } else {
        cms->write(cms->subdiv_data,
                   serial_number); /* Write the formatted message.  */
    }
...
int NML::set_error()
{
    ...
    /* Choose return value. */
    switch (cms->status) {
    case CMS_TIMED_OUT: error_type = NML_TIMED_OUT; return (-1);

    case CMS_QUEUE_FULL: error_type = NML_QUEUE_FULL_ERROR; break;

    case CMS_NO_MASTER_ERROR: error_type = NML_NO_MASTER_ERROR; break;

    case CMS_WRITE_WAS_BLOCKED: error_type = NML_BUFFER_NOT_READ; break;
    ...
    case CMS_CONFIG_ERROR: error_type = NML_INVALID_CONFIGURATION; break;

    case CMS_MISC_ERROR:
    default: error_type = NML_INTERNAL_CMS_ERROR; break;
    }
...
NMLTYPE NML::str2msg(const char *string)
{
    ...
    case CMS_TIMED_OUT: error_type = NML_TIMED_OUT; return -1;
    case CMS_MISC_ERROR:
    case CMS_NO_MASTER_ERROR: error_type = NML_INTERNAL_CMS_ERROR;
    /* Fallthrough */
    default: return -1;
    }
}
...
***************************************************************************/
void NML::print_info(const char *const bufname,
...
int NML::print_queue_info()
{
    ...
    if (cms->ProcessType != CMS_LOCAL_TYPE) {
        rcs_print_error(
            "NML::print_queue_info() - REMOTE Connection: Queueing Data Not "
            "Available.\n");
        return (-1);
    }
    rcs_print("head = %ld(0x%lX); tail=%ld(0x%lX); "
              "queue_length=%ld,end_queue_space=%ld(0x%lX); write_id=%ld\n",
              cms->queuing_header.head,
              cms->queuing_header.head,
              cms->queuing_header.tail,
              cms->queuing_header.tail,
              cms->queuing_header.queue_length,
              cms->queuing_header.end_queue_space,
              cms->queuing_header.end_queue_space,
...
NML *nmlWaitOpen(
    NML_FORMAT_PTR fPtr, char *buffer, char *name, char *file, double sleepTime)
{
    NML *nmlChannel = 0;
