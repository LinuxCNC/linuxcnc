        ...
        if (NULL != _nml->cms) {
            if (CMS_REMOTE_TYPE != _nml->cms->ProcessType) {
                ...
                if (_nml->cms->isserver &&
                    (0 == _set_to_master ||
                     (_nml->cms->is_local_master == 1 && _set_to_master == 1) ||
                ...
                rcs_print_error("NML_SERVER:(ERROR) ProcessType was REMOTE.\n");
            }
        } else {
...
void NML_SERVER::delete_from_list()
{
    ...
    if (NULL != NML_Default_Super_Server) {
        if (NULL != NML_Default_Super_Server->servers) {
            NML_Default_Super_Server->servers->delete_node(
...
NML_SERVER_LOCAL_PORT::NML_SERVER_LOCAL_PORT(NML *_nml)
    : CMS_SERVER_LOCAL_PORT((CMS *)NULL), batch_list_id(0)
{
...
REMOTE_READ_REPLY *NML_SERVER_LOCAL_PORT::reader(REMOTE_READ_REQUEST *_req)
{
    ...
    case CMS_READ_ACCESS: nml->read(); break;
    case CMS_PEEK_ACCESS: nml->peek(); break;
    default:
...
REMOTE_READ_REPLY *
NML_SERVER_LOCAL_PORT::blocking_read(REMOTE_READ_REQUEST *_req)
{
    if ((NULL == cms) || (NULL == nml)) {
...
REMOTE_WRITE_REPLY *NML_SERVER_LOCAL_PORT::writer(REMOTE_WRITE_REQUEST *_req)
{
    ...
    temp->size = _req->size;
    int *serial_number =
        cms->serial ? &(((RCS_CMD_MSG *)temp)->serial_number) : NULL;

    switch (_req->access_type) {
    case CMS_WRITE_ACCESS: nml->write(*temp, serial_number); break;
...
NML_SERVER_LOCAL_PORT::set_diag_info(REMOTE_SET_DIAG_INFO_REQUEST *_req)
{
    if (NULL == _req) {
        return (NULL);
    }
...
NML_SERVER_LOCAL_PORT::get_diag_info(REMOTE_GET_DIAG_INFO_REQUEST * /*_req*/)
{
...
NML_SERVER_LOCAL_PORT::get_msg_count(REMOTE_GET_DIAG_INFO_REQUEST * /*_req*/)
{
    return (NULL);
}
    ...
    if (NULL != servers) {
        ...
        if (NULL == server) {
            ...
            if (_nml->cms->isserver) {
                ...
                if (NULL == local_port) {
                    rcs_print_error(
                        "NML_SERVER: Unable to create local port.\n");
                    return;
                }
    ...
    if (NULL != NML_Default_Super_Server) {
        if (NML_Default_Super_Server->servers != NULL) {
            ...
            if (NML_Default_Super_Server->unspawned_servers <= 0) {
                rcs_print_error(
                    "run_nml_servers(): No buffers without servers already "
                    "spawned for them.\n");
                return;
            }
            if (NML_Default_Super_Server->unspawned_servers == 1) {
                ...
                while (sole_server != NULL) {
                    ...
                    sole_server =
                        (NML_SERVER *)
                            NML_Default_Super_Server->servers->get_next();
                }
                if (NULL == sole_server) {
                    rcs_print_error(
            ...
            rcs_print_error(
                "run_nml_servers(): No buffers without servers already spawned "
                "for them.\n");
        }
    } else {
        rcs_print_error(
            "run_nml_servers(): No buffers without servers already spawned for "
            "them.\n");
    }
    run_nml_server_exit(-1);
}
