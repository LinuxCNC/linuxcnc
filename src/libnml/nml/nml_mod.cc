...
const char *NML_MODULE_INI_INFO::getString(const char *tag, const char *def)
{
    if (NULL == inif) {
	return def;
    }
...
double NML_MODULE_INI_INFO::getDouble(const char *tag, double def)
{
...
long NML_MODULE_INI_INFO::getLongInt(const char *tag, long def)
{
    const char *temp = getString(tag, NULL);
    if (NULL == temp) {
	return def;
...
int NML_MODULE::setSubordinates(int number)
{
	...
	if (NULL != old_subs && NULL != subs) {
		...
		previousNumSubordinates * sizeof(NML_SUBORDINATE_STRUCT *));
	}
...
void NML_MODULE::DECISION_PROCESS(void)
{
...
void NML_MODULE::READ_COMM_BUFFERS(void)
{
    read_command_in();
...
void NML_MODULE::PRE_PROCESS(void)
{

}

void NML_MODULE::POST_PROCESS(void)
{
...
void NML_MODULE::WRITE_COMM_BUFFERS(void)
{
    write_status_out();
...
int NML_MODULE::stateMatch(char *_source_file, int _source_line, int st,
    int conds)
{
...
int NML_MODULE::stateMatch(int st, int conds)
{
    if (matched) {
	return 0;
    }
