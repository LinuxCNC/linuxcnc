...
int recvn(
    int fd, void *vptr, int n, int flags, double _timeout, int *bytes_read_ptr)
{
    int nleft, nrecv;
    char *ptr;
    double start_time;
    ...
    if (NULL != bytes_read_ptr) {
        if (*bytes_read_ptr >= n) {
            rcs_print_error(
                "recvn: Invalid parameter -- (*bytes_read_ptr = %d) must be "
                "less than (n = %d).\n",
                *bytes_read_ptr,
                n);
            return -1;
        ...
        if (_timeout > 0.0) {
            ...
            if (timeleft <= 0.0) {
                if (print_recvn_timeout_errors) {
                    ...
                    if (NULL == bytes_read_ptr) {
                        rcs_print_error(
                            "recvn(fd=%d, vptr=%p, int n=%d, int flags=%d, "
                            "double _timeout=%f) failed.\n",
                            fd,
                            vptr,
                            n,
                            flags,
                        ...
                        rcs_print_error(
                            "recvn(fd=%d, vptr=%p, int n=%d, int flags=%d, "
                            "double _timeout=%f,bytes_read=%d) failed.\n",
                            fd,
                            vptr,
                            n,
                            flags,
            ...
            case -1:
                ...
                if (NULL == bytes_read_ptr) {
                    rcs_print_error(
                        "recvn(fd=%d, vptr=%p, int n=%d, int flags=%d, double "
                        "_timeout=%f) failed.\n",
                        fd,
                        vptr,
                        n,
                    ...
                    rcs_print_error(
                        "recvn(fd=%d, vptr=%p, int n=%d, int flags=%d, double "
                        "_timeout=%f,bytes_read=%d) failed.\n",
                        fd,
                        vptr,
                        n,
                        flags,
