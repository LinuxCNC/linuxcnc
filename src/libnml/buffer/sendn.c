            ...
            if (_timeout > 0) {
                ...
                if (timeleft <= 0.0) {
                    if (print_sendn_timeout_errors) {
                        rcs_print_error(
                            "sendn(fd=%d, vptr=%p, int n=%d, int flags=%d, "
                            "double _timeout=%f) timed out.\n",
                            fd,
                            vptr,
                            n,
                            _flags,
                ...
                select_ret = select(
                    fd + 1, (fd_set *)NULL, &send_fd_set, (fd_set *)NULL, NULL);
            }
            switch (select_ret) {
            case -1:
                rcs_print_error(
                ...
                rcs_print_error(
                    "sendn(fd=%d, vptr=%p, int n=%d, int _flags=%d, double "
                    "_timeout=%f) failed.\n",
                    fd,
                    vptr,
                    n,
