...
SHMEM::SHMEM(const char * /*n*/, long s, int /*nt*/, key_t k, int m)
    : CMS(s), key(k), bsem_key(-1), second_read(0), shm(NULL), sem(NULL),
      master(m), sem_delay(0.00001), mao{}, use_os_sem(1), use_os_sem_only(1),
      mutex_type(OS_SEM_MUTEX), shm_addr_offset(NULL), bsem(NULL),
...
SHMEM::SHMEM(const char *bufline,
             ...
             int set_to_server,
             int set_to_master)
    : CMS(bufline, procline, set_to_server), bsem_key(-1), second_read(0),
      shm(NULL), sem(NULL), sem_delay(0.00001), mao{}, use_os_sem(1),
      use_os_sem_only(1), mutex_type(OS_SEM_MUTEX), shm_addr_offset(NULL),
      bsem(NULL), autokey_table_size(0)
{
    ...
    if (NULL != (semdelay_equation = strstr(proclineupper, "SEMDELAY="))) {
        sem_delay = strtod(semdelay_equation + 9, (char **)NULL);
               ...
               (semdelay_equation = strstr(buflineupper, "SEMDELAY="))) {
        sem_delay = strtod(semdelay_equation + 9, (char **)NULL);
    }
...
/*! \todo Another #if 0 */
    ...
    if (master) {
        ...
        if (shm->addr == NULL) {
            ...
            case EACCES: status = CMS_PERMISSIONS_ERROR; break;

            case EEXIST: status = CMS_RESOURCE_CONFLICT_ERROR; break;

            case ENOMEM:
            case ENOSPC: status = CMS_CREATE_ERROR; break;

            default: status = CMS_MISC_ERROR;
            }
            delete shm;
            shm = NULL;
            return -1;
        ...
        if (use_os_sem) {
            sem = new RCS_SEMAPHORE(key,
                                    RCS_SEMAPHORE_CREATE,
                                    timeout,
                                    (int)MODE,
                                    (use_os_sem_only != 0));
            if (NULL == sem) {
        ...
        if (bsem_key > 0) {
            bsem = new RCS_SEMAPHORE(
                bsem_key, RCS_SEMAPHORE_CREATE, timeout, (int)MODE, 0);
            if (NULL == bsem) {
                rcs_print_error("CMS: couldn't create RCS_SEMAPHORE.\n");
                rcs_print_error(" Possibly out of memory?\n");
                status = CMS_CREATE_ERROR;
        ...
        if (shm->addr == NULL) {
            ...
            case ENOENT: status = CMS_NO_MASTER_ERROR; break;
        ...
        if (bsem_key > 0) {
            bsem = new RCS_SEMAPHORE(bsem_key, RCS_SEMAPHORE_NOCREATE, timeout);
            if (NULL == bsem) {
                rcs_print_error("CMS: couldn't create RCS_SEMAPHORE.\n");
                rcs_print_error(" Possibly out of memory?\n");
                status = CMS_CREATE_ERROR;
    ...
    if (min_compatible_version > 2.57 || min_compatible_version <= 0) {
        if (!shm->created) {
            ...
            if (strncmp(cptr, BufferName, 31)) {
                rcs_print_error("Shared memory buffers %s and %s may conflict. "
                                "(key=%d(0x%X))\n",
                                BufferName,
                                cptr,
                                key,
...
CMS_STATUS SHMEM::main_access(void *_local, int *serial_number)
{
    ...
    if (bsem == NULL && not_zero(blocking_timeout)) {
        rcs_print_error("No blocking semaphore available. Can not call "
                        "blocking_read(%f).\n",
                        blocking_timeout);
        second_read = 0;
    ...
    if (NULL != bsem && (internal_access_type == CMS_WRITE_ACCESS ||
    ...
    case MAO_MUTEX:
    case MAO_MUTEX_W_OS_SEM: mem_release_access(&mao); break;

    case OS_SEM_MUTEX: sem->post(); break;
    case NO_INTERRUPTS_MUTEX:
    ...
    case CMS_READ_ACCESS:
        if (NULL != bsem && status == CMS_READ_OLD &&
            ...
            if (second_read > 10 && total_subdivisions <= 1) {
                status = CMS_MISC_ERROR;
                rcs_print_error(
                    "CMS: Blocking semaphore error. The semaphore wait has "
                    "returned %d times but there is still no new data.\n",
                    second_read);
                second_read = 0;
                return (status);
            }
    ...
    case CMS_WRITE_IF_READ_ACCESS: break;
