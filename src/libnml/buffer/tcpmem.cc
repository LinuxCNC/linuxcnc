...
TCPMEM::TCPMEM(const char *_bufline, const char *_procline)
    : CMS(_bufline, _procline), diag_info_buf{}, recvd_bytes(0),
      serial_number(0), returned_serial_number(0),
      subscription_type(CMS_NO_SUBSCRIPTION), poll_interval_millis(30000),
      server_host_entry(NULL), socket_fd(0), temp_buffer{}, timedout_request(),
      bytes_to_throw_away(0), write_socket_fd(0), read_socket_fd(0),
      write_serial_number(0), read_serial_number(0),
      timedout_request_status(CMS_STATUS_NOT_SET), timedout_request_writeid(0),
      max_consecutive_timeouts(DEFAULT_MAX_CONSECUTIVE_TIMEOUTS),
      waiting_for_message(0), waiting_message_size(0), waiting_message_id(0),
      autoreconnect(1), reconnect_needed(0), sigpipe_count(0),
      old_handler((void (*)(int))SIG_ERR), subscription_count(0)
{
    ...
    if (NULL != sub_info_string) {
        if (!strncmp(sub_info_string + 4, "none", 4)) {
            ...
            poll_interval_millis = ((int)(atof(sub_info_string + 4) * 1000.0));
...
void TCPMEM::send_diag_info()
{
    ...
    serial_number++;
    rcs_print_debug(
        PRINT_ALL_SOCKET_REQUESTS,
        "TCPMEM sending request: fd = %d, serial_number=%ld, request_type=%d, "
        "buffer_number=%ld\n",
        socket_fd,
        serial_number,
        ntohl(*((uint32_t *)diag_info_buf + 1)),
        buffer_number);
    reenable_sigpipe();
}
...
void TCPMEM::verify_bufname()
{
    ...
    rcs_print_debug(PRINT_ALL_SOCKET_REQUESTS,
                    "TCPMEM received_reply: fd = %d, serial_number=%ld, "
                    "buffer_number=%ld\n",
                    socket_fd,
    ...
    if (returned_serial_number != serial_number) {
        rcs_print_error(
            "TCPMEM: Returned serial number(%ld) does not match expected "
            "serial number(%ld).\n",
            returned_serial_number,
            serial_number);
        reconnect_needed = 1;
    ...
    if (strncmp(temp_buffer + 8, BufferName, 31)) {
        rcs_print_error(
            "TCPMEM: The buffer (%s) is registered on TCP port %d of host %s "
            "with buffer number %ld.\n",
            ((char *)temp_buffer + 8),
            tcp_port_number,
            BufferHost,
            buffer_number);
        rcs_print_error(
            "TCPMEM: However, this process (%s) is attempting to connect to "
            "the buffer %s at the same location.\n",
            ProcessName,
            BufferName);
...
CMS_DIAGNOSTICS_INFO *TCPMEM::get_diagnostics_info()
{
    ...
    if (dpi_max_size > 32 && dpi_max_size < 0x2000) {
        ...
        for (int i = 0; i < dpi_count && dpi_offset < dpi_max_size; i++) {
            ...
            dpi_offset += 8;
            cms_dpi.access_type = (CMS_INTERNAL_ACCESS_TYPE)ntohl(
...
void TCPMEM::reconnect()
{
    ...
    if (connect(socket_fd,
        ...
        if (EINPROGRESS == errno) {
            ...
            rcs_print_error(
                "connect error: %d -- %s\n", errno, strerror(errno));
            rcs_print_error(
                "TCPMEM: Error trying to connect to TCP port %d of host "
                "%s(%s). sin_family=%d\n",
    ...
    if (polling) {
        ...
        rcs_print_debug(PRINT_CMS_CONFIG_INFO, "Binding . . . \n");
        if (bind(write_socket_fd,
                 (struct sockaddr *)&cli_addr,
                 sizeof(cli_addr)) < 0) {
            rcs_print_error(
                "TCPMEM: bind error %d = %s\n", errno, strerror(errno));
            status = CMS_CREATE_ERROR;
        }
        rcs_print_debug(PRINT_CMS_CONFIG_INFO, "Connecting . . .\n");
        if (connect(write_socket_fd,
                    (struct sockaddr *)&server_socket_address,
                    sizeof(server_socket_address)) < 0) {
            if (EINPROGRESS == errno) {
                FD_ZERO(&fds);
...
CMS_STATUS TCPMEM::handle_old_replies()
{
    ...
    case REMOTE_CMS_READ_REQUEST_TYPE:
        if (!waiting_for_message) {
                ...
                if (recvn_timedout) {
                    if (polling) {
                        ...
                        if (consecutive_timeouts > max_consecutive_timeouts &&
                            ...
                            rcs_print_error(
                                "CMS: %d consecutive timeouts have occurred. "
                                "-- Stop trying.\n",
                                consecutive_timeouts);
                            fatal_error_occurred = 1;
                            reconnect_needed = 1;
            ...
            if (returned_serial_number != serial_number) {
                rcs_print_error(
                    "TCPMEM: Returned serial number(%ld) does not match "
                    "expected serial number(%ld).\n",
                    returned_serial_number,
                    serial_number);
                if (subscription_type == CMS_NO_SUBSCRIPTION) {
                    fatal_error_occurred = 1;
        ...
        if (recvn(socket_fd, temp_buffer, 12, 0, timeout, &recvd_bytes) < 0) {
            if (recvn_timedout) {
                ...
                if (consecutive_timeouts > max_consecutive_timeouts &&
                    max_consecutive_timeouts > 0) {
                    rcs_print_error(
                        "CMS: %d consecutive timeouts have occurred. -- Stop "
                        "trying.\n",
                        consecutive_timeouts);
                    reconnect_needed = 1;
                    fatal_error_occurred = 1;
...
CMS_STATUS TCPMEM::write(void *user_data, int *serial_number_out)
{
    ...
    if (header.in_buffer_size < 0x2000 - 20 && header.in_buffer_size > 0) {
        memcpy(
            temp_buffer + send_header_size, user_data, header.in_buffer_size);
        if (sendn(socket_fd,
                  temp_buffer,
                  ...
                  timeout) < 0) {
            rcs_print_error(
                "TCPMEM: Failed to send message of size %ld + header of size "
                "%d  to the server.\n",
    ...
    if ((min_compatible_version < 2.58 && min_compatible_version > 1e-6) ||
        ...
        if (serial_number_out)
            *serial_number_out = returned_serial_number;
        rcs_print_debug(PRINT_ALL_SOCKET_REQUESTS,
                        "TCPMEM received_reply: fd = %d, serial_number=%ld, "
                        "buffer_number=%ld\n",
                        socket_fd,
