...
void set_rcs_print_list_sizing(int _new_max_size,
        ...
        while (NULL != string_from_list) {
            ...
            if (NULL == next_line) {
                if (NULL == temp_buf) {
                    ...
                    rtapi_strlcpy(temp_buf,
                                  string_from_list,
                                  strlen(string_from_list) + 1);
                } else {
                    char *ra = (char *)realloc(
                        temp_buf,
                        strlen(temp_buf) + strlen(string_from_list) + 1);
                    if (ra) {
                        temp_buf = ra;
                ...
                if (temp_buf != NULL) {
                    ...
                    if (ra) {
                        ...
                        strcat(temp_buf, string_from_list);
                        rcs_print_list->delete_current_node();
                        rcs_print_list->store_after_current_node(
                            temp_buf, strlen(temp_buf) + 1, 1);
                    }
                    free(temp_buf);
                    temp_buf = NULL;
                } else if (next_line[1] != 0) {
                    rcs_print_list->store_after_current_node(
                        next_line + 1, strlen(next_line + 1) + 1, 1);
                    next_line[1] = 0;
                }
            }
            string_from_list = (char *)rcs_print_list->get_next();
...
void update_lines_table()
{
    ...
    if (NULL != rcs_print_list) {
        ...
        rcs_lines_table =
            (char **)malloc(sizeof(char *) * rcs_print_list->list_size);
        if (NULL != rcs_lines_table) {
            char *string_from_list;
            string_from_list = (char *)rcs_print_list->get_head();
            int i = 0;
    ...
    if (NULL != _str) {
        ...
        case RCS_PRINT_TO_LIST:
            if (NULL == rcs_print_list) {
                rcs_print_list = new LinkedList;
                if (NULL != rcs_print_list) {
                    rcs_print_list->set_list_sizing_mode(256, DELETE_FROM_HEAD);
                }
            }
            if (NULL != rcs_print_list) {
                if (-1 == rcs_print_list->store_at_tail(
                              (void *)_str, (retval = strlen(_str)) + 1, 1)) {
                    retval = EOF;
                }
            }
            break;
        case RCS_PRINT_TO_NULL: retval = strlen(_str); break;
...
{
    switch (rcs_print_destination) {
    case RCS_PRINT_TO_LIST: clean_print_list(); break;
...
int rcs_print_sys_error(int error_source, const char *_fmt, ...)
{
    ...
    default: rcs_puts(temp_string); break;
    }
