...
int main(int argc, char *argv[])
{
    ...
    for (int t = 1; t < argc; t++) {
        if (!strcmp(argv[t], "-ini")) {
            if (t == argc - 1) {
                ...
                fprintf(
                    stderr, "%s: INI file not specified after -ini\n", argv[0]);
                exit(-1);
            } else {
                path = argv[t + 1];
            ...
            if (t == argc - 1) {
                ...
                if (errno || *endPtr != '\0' || result < 0 ||
                    result > INT_MAX) {
                    fprintf(stderr, "%s: invalid number after -num\n", argv[0]);
                    exit(-1);
                }
            ...
            /* invalid argument */
            fprintf(stderr,
                    "%s: -var <variable> [-tildeexpand] [-sec <section>] [-num "
                    "<occurrence_number>] [-ini <INI file>]\n",
                    argv[0]);
            exit(-1);
        }
    }
    ...
    if (iniString) {
        ...
        fprintf(stderr,
                "Can not find -sec %s -var %s -num %i \n",
                section,
                variable,
                num);
        retval = 1;
