...
shm_t *rcs_shm_open(key_t key, size_t size, int oflag, /* int mode */...)
{
    ...
    if (!existed_before) {
        ...
        if (-1 == fstat(shm->id, &statbuf)) {
            rcs_print_error(
                "fstat failed. (errno=%d) %s\n", errno, strerror(errno));
            shm->create_errno = errno;
            return shm;
        }
        if (statbuf.st_size != size + 16) {
            rcs_print_error(
                "Shared memory buffer %s already exists but has the wrong size "
                "of % d instead of the expected size of % d. \n ",
                shm->name,
                statbuf.st_size,
                size + 16);
            shm->create_errno = -1;
    ...
    shm->addr =
        mmap(0, size + 16, PROT_READ | PROT_WRITE, MAP_SHARED, shm->id, 0);
    if (shm->addr == MAP_FAILED) {
        rcs_print_error(
            "mmap(0,%d,PROT_READ | PROT_WRITE, MAP_SHARED,%d,0) failed: %s "
            "%d\n",
            shm->id,
            size,
            strerror(errno),
    ...
    if ((shm->id = shmget(key, (int)size, shmflg)) == -1) {
        ...
        case EINVAL:
            rcs_print_error(
                "Either the size is too big or the shared memory buffer "
                "already exists but is of the wrong size.\n");
            break;

        case ENOSPC:
            rcs_print_error(
                "The system imposed limit on the maximum number of shared "
                "memory segments has been exceeded.\n");
            break;
