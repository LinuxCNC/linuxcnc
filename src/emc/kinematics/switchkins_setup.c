...
*  itself from its own state, a copy outside RT replays setup once, so the
                 ...
                 kins_module_info *info)
{
    ...
    if (!replayed) {
        ...
        replayed = switchkinsRunSetup(&kp, sparm ? sparm_seen : NULL) ? -1 : 1;
    } else if (strncmp(sparm_seen, s, sizeof(sparm_seen) - 1)) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "kinsDescribe: already set up with sparm '%s',"
                        " not '%s'\n", sparm_seen, s);
        return -1;
    }
