                            ...
                            KINEMATICS_INVERSE_FLAGS * iflags)
{
    ...
    double tool_length  = p->geometry[P_TOOL_LENGTH];
    PmCartesian r = s2r(pivot_length + tool_length + joints[JW],
                        joints[JC],
                        180.0 - joints[JB]);
                            ...
                            KINEMATICS_FORWARD_FLAGS * fflags)
{
    ...
    double pivot_length = p->geometry[P_PIVOT_LENGTH];
    double tool_length  = p->geometry[P_TOOL_LENGTH];
    PmCartesian r = s2r(pivot_length + tool_length + pos->w,
                        pos->c,
                        180.0 - pos->b);
                             ...
                             const KINEMATICS_INVERSE_FLAGS * iflags)
{
    (void)joints;
    (void)iflags;
    const double R  = p->geometry[P_PIVOT_LENGTH] + p->geometry[P_TOOL_LENGTH] + pos->w;
