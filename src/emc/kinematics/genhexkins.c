...
static int MatInvert(double J[][NUM_STRUTS], double InvJ[][NUM_STRUTS])
{
    ...
    for (j = 0; j <= 5; ++j) {
        for (k = 0; k <= 5;
             ++k) { /* Assign J matrix to first 6 columns of AugJ */
            JAug[j][k] = J[j][k];
        }
        for (k = 6; k <= 11;
             ++k) { /* Assign I matrix to last six columns of AugJ */
            if (k - 6 == j) {
                JAug[j][k] = 1;
            } else {
                JAug[j][k] = 0;
    ...
    for (k = 0; k <= 4; ++k) { /* Pivot        */
        ...
        for (j = k + 1; j <= 5; ++j) { /* Pivot */
            ...
            for (n = 0; n <= 11; ++n) {
                JAug[j][n] =
                    JAug[j][n] + m * JAug[k][n]; /* (Row j) + m * (Row k) */
    ...
    while (iterate) {
     ...
     and compute inv J while we're at it */
        for (i = 0; i < NUM_STRUTS; i++) {
            ...
            pmCartCartCross(
                &RMatrix_a, &InvKinStrutVectUnit, &RMatrix_a_cross_Strut);
...
static int
genhexKinematicsSetup(const int comp_id, const char *coordinates, kparms *kp)
{
    (void)coordinates;
    int i, res = 0;
    ...
    if (!haldata) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "genhexKinematicsSetup: hal_malloc fail\n");
        return -1;
    }
    ...
    for (i = 0; i < kp->max_joints; i++) {
        ...
        res += hal_pin_float_newf(HAL_IN,
                                  ...
                                  i);
        res += hal_pin_float_newf(HAL_IN,
                                  &haldata->platformnx[i],
                                  comp_id,
                                  "%s.platform-n.%d.x",
                                  kp->halprefix,
                                  i);
        res += hal_pin_float_newf(HAL_IN,
                                  &haldata->platformny[i],
                                  comp_id,
                                  "%s.platform-n.%d.y",
                                  kp->halprefix,
                                  i);
        res += hal_pin_float_newf(HAL_IN,
                                  &haldata->platformnz[i],
                                  comp_id,
                                  "%s.platform-n.%d.z",
                                  kp->halprefix,
                                  i);
        res += hal_pin_float_newf(HAL_OUT,
                                  &haldata->correction[i],
                                  comp_id,
                                  "%s.correction.%d",
                                  kp->halprefix,
                                  i);
    ...
    res += hal_pin_u32_newf(
        HAL_OUT, &haldata->last_iter, comp_id, "genhexkins.last-iterations");
    *haldata->last_iter = 0;
    res += hal_pin_u32_newf(
        HAL_OUT, &haldata->max_iter, comp_id, "genhexkins.max-iterations");
    *haldata->max_iter = 0;
    res += hal_pin_float_newf(
        HAL_IN, &haldata->max_error, comp_id, "genhexkins.max-error");
    *haldata->max_error = 500.0;
    res += hal_pin_float_newf(HAL_IN,
                              &haldata->conv_criterion,
                              comp_id,
                              "genhexkins.convergence-criterion");
    *haldata->conv_criterion = 1e-9;
    res += hal_pin_u32_newf(
        HAL_IN, &haldata->iter_limit, comp_id, "genhexkins.limit-iterations");
    *haldata->iter_limit = 120;
    res += hal_pin_float_newf(
        HAL_IN, &haldata->tool_offset, comp_id, "genhexkins.tool-offset");
    *haldata->tool_offset = 0.0;
    res += hal_pin_float_newf(
        HAL_IN, &haldata->spindle_offset, comp_id, "genhexkins.spindle-offset");
    *haldata->spindle_offset = 0.0;
    res += hal_pin_float_newf(
        HAL_IN, &haldata->screw_lead, comp_id, "genhexkins.screw-lead");
    ...
    *haldata->platformnz[5] = DEFAULT_PLATFORM_5_NZ;
    ...
    res += hal_pin_bit_newf(
        HAL_OUT, &haldata->fwd_kins_fail, comp_id, "genhexkins.fwd-kins-fail");
