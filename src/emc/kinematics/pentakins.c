...
int InvKins(const double *coord, double *struts)
{
...
int rtapi_app_main(void)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        if ((res = hal_param_float_newf(HAL_RW,
                                        &(haldata->basex[i]),
                                        comp_id,
                                        "pentakins.base.%d.x",
                                        i)) < 0)
        ...
        if ((res = hal_param_float_newf(HAL_RW,
                                        &haldata->effectorz[i],
                                        comp_id,
                                        "pentakins.effector.%d.z",
                                        i)) < 0)
            goto error;
    }

    if ((res = hal_pin_u32_newf(HAL_OUT,
                                &haldata->last_iter,
                                comp_id,
    ...
    *haldata->max_iter = 0;
    ...
    if ((res = hal_pin_float_newf(
             HAL_IO, &haldata->max_error, comp_id, "pentakins.max-error")) < 0)
        goto error;
    *haldata->max_error = 100;

    if ((res = hal_pin_float_newf(HAL_IO,
    ...
    *haldata->conv_criterion = 1e-9;
    ...
    if ((res = hal_pin_u32_newf(HAL_IO,
                                &haldata->iter_limit,
                                comp_id,
    ...
    *haldata->iter_limit = 120;
    ...
    if ((res = hal_pin_float_newf(
             HAL_IN, &haldata->tool_offset, comp_id, "pentakins.tool-offset")) <
        0)
        goto error;
