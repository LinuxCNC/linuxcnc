...
int tpRunCycle(TP_STRUCT *tp)
{
  ...
  for (t = 0; t < tcqLen(&tp->queue); t++) {
    ...
    /* If either this move or the last move was a pure rotation
       reset the velocity and acceleration and block any blending */
	...
	preVMax = 0.0;
      ...
      /* The combined acceleration of this move and the next one 
	 will be square root(currentAcceleration^2 + nextAccel^2 + 2*the dot product).
	 to prevent the combined move from exceeding vMax preVMax may need adjustment. 
	 tcRunCycle will subtract preVMax from vMax and clamp the acceleration to 
	 this value.
      */
      pmCartMag(accelCart,&currentAccelMag);
      if (currentAccelMag >= TP_ACCEL_EPSILON) {
	pmCartCartDot(accelCart, unitCart, &dot);
	preAMax = thisTc->aMax + dot - pmSqrt(pmSq(dot) - pmSq(currentAccelMag) + pmSq(thisTc->aMax));
      }
      else {
	preAMax = 0.0;
      }
  ...
  if (tp->aborting &&
    ...
    /* first set the motion outputs to the end values for the current move */
    if (tp->douts &&
	0 != thisTc) {
    /* Fred's original code..
      tcDoutByte |= (thisTc->douts & thisTc->doutends);
      tcDoutByte &= (~thisTc->douts | thisTc->doutends);
