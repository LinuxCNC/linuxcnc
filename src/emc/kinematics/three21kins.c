...
static int three21_jacobian(const kins_params *p, const double * joint,
                            const EmcPose * world,
                            ...
                            const KINEMATICS_INVERSE_FLAGS * iflags)
{
   ...
   const double a[6] = { 0, p->geometry[P_A1], p->geometry[P_A2],
                         p->geometry[P_A3], 0, 0 };
   const double d[6] = { p->geometry[P_D1], p->geometry[P_D2],
                         p->geometry[P_D3], p->geometry[P_D4], 0, 0 };

   return kinsJacobianFromDhArm(alpha, a, d, joint, p->geometry[P_D6], world, jac);
} // three21_jacobian()
                   ...
                   )
{
    ...
    if (kp->sparm && strstr(kp->sparm,"identityfirst")) {
        ...
        switchkinsRegisterOps(1, &three21_ops);
    } else {
