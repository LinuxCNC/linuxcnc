                                 ...
                                 KINEMATICS_INVERSE_FLAGS *iflags)
{
    ...
    /* determine flags.                   */
    sumSq =
        hom.tran.x * hom.tran.x + hom.tran.y * hom.tran.y - PUMA_D3 * PUMA_D3;
                                 ...
                                 KINEMATICS_FORWARD_FLAGS *fflags)
{
    ...
    *fflags = 0;
    ...
    s5 = hom.rot.z.z * (s23 * c4) - hom.rot.z.x * (c1 * c23 * c4 + s1 * s4) -
    ...
    c6 = hom.rot.x.x * ((c1 * c23 * c4 + s1 * s4) * c5 - c1 * s23 * s5) +
         hom.rot.x.y * ((s1 * c23 * c4 - c1 * s4) * c5 - s1 * s23 * s5) -
...
int pumaKinematicsSetup(const int comp_id, const char *coordinates, kparms *kp)
{
    (void)coordinates;
    int res = 0;
    ...
    res += hal_pin_float_newf(
        HAL_IN, &(haldata->a2), comp_id, "%s.A2", kp->halprefix);
    res += hal_pin_float_newf(
        HAL_IN, &(haldata->a3), comp_id, "%s.A3", kp->halprefix);
    res += hal_pin_float_newf(
        HAL_IN, &(haldata->d3), comp_id, "%s.D3", kp->halprefix);
    res += hal_pin_float_newf(
        HAL_IN, &(haldata->d4), comp_id, "%s.D4", kp->halprefix);
    res += hal_pin_float_newf(
        HAL_IN, &(haldata->d6), comp_id, "%s.D6", kp->halprefix);
    if (res) {
        goto error;
    }
