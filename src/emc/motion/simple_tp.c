...
void simple_tp_update_normal(simple_tp_t *tp, double period)
{
    ...
    if (tp->enable) {
	   ...
	   avoid sqrt(negative) */
        if (pos_err > tiny_dp) {
            vel_req =
                -max_dv + sqrt(2.0 * tp->max_acc * pos_err + max_dv * max_dv);
...
#define Vm tp->max_vel
    ...
    if (dir != 0) {
        ...
        if (abort == 1) {
            ...
            simple_tp_update_normal(tp, T);
            return;
        } else if (fabs(fabs(pos_err) - decLen) <= 0.000001 ||
                   fabs(pos_err) - moveL * dir <= decLen2 ||
                   fabs(pos_err) <= decLen || tp->status == 1) {
            nextSpeed(Vc,
                      Ac,
                      T,
                      0 * dir,
            ...
            if (error > 0 &&
                (error < tp->last_move_length * dir && error > move * dir)) {
    ...
    tp->last_move_length = curr_vel * period +
                           0.5 * curr_acc * period * period +
                           (tp->curr_jerk * period * period * period) / 6;
