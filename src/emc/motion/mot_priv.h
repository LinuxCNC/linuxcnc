...
typedef struct {
    ...
    hal_float_t
        *spindle_speed_cmd_rps; /* spindle speed command without SO applied */
...
typedef struct {
    ...
    hal_float_t
        *motor_offset; /* RPI: motor offset, for checking homing stability */
...
typedef struct {
    ...
    hal_bit_t
        *jog_stop; /* RPI: set TRUE to stop jogging following accel values*/
    hal_bit_t
        *jog_stop_immediate;  /* RPI: set TRUE to stop jogging immediately*/
    hal_bit_t *jog_is_active; /* RPI: TRUE if active jogging*/
    hal_bit_t
        *tp_reverse; /* Set true if trajectory planner is running in reverse*/
    ...
    hal_s32_t
        *motion_type; /* RPA: type (feed/rapid) of currently commanded motion */
    hal_float_t *current_vel; /* RPI: velocity magnitude in machine units */
    hal_float_t
        *requested_vel; /* RPI: requested velocity magnitude in machine units */
    ...
    hal_bit_t *synch_do
        [EMCMOT_MAX_DIO]; /* WPI array: output pins for motion synched IO */
    hal_bit_t *synch_di
        [EMCMOT_MAX_DIO]; /* RPI array: input pins for motion synched IO */
    hal_float_t *analog_input
        [EMCMOT_MAX_AIO]; /* RPI array: input pins for analog Inputs */
    hal_float_t *analog_output
        [EMCMOT_MAX_AIO]; /* RPI array: output pins for analog Inputs */
    hal_bit_t *misc_error
        [EMCMOT_MAX_MISC_ERROR]; /* RPI array: output pins for misc error Inputs */
    ...
    extrajoint_hal_t
        ejoint[EMCMOT_MAX_EXTRAJOINTS]; /* data for each extrajoint */
...
extern void reportError(const char *fmt, ...)
    __attribute__((format(printf, 1, 2))); /* Use the rtapi_print call */
...
#define GET_MOTION_ERROR_FLAG()                                                \
    (emcmotStatus->motionFlag & EMCMOT_MOTION_ERROR_BIT ? 1 : 0)
...
#define GET_MOTION_COORD_FLAG()                                                \
    (emcmotStatus->motionFlag & EMCMOT_MOTION_COORD_BIT ? 1 : 0)
...
#define GET_MOTION_TELEOP_FLAG()                                               \
    (emcmotStatus->motionFlag & EMCMOT_MOTION_TELEOP_BIT ? 1 : 0)
...
#define GET_MOTION_ENABLE_FLAG()                                               \
    (emcmotStatus->motionFlag & EMCMOT_MOTION_ENABLE_BIT ? 1 : 0)
...
#define GET_JOINT_ENABLE_FLAG(joint)                                           \
...
#define GET_JOINT_ERROR_FLAG(joint)                                            \
    ((joint)->flag & EMCMOT_JOINT_ERROR_BIT ? 1 : 0)
...
#define GET_JOINT_PHL_FLAG(joint)                                              \
...
#define GET_JOINT_NHL_FLAG(joint)                                              \
...
#define GET_JOINT_FERROR_FLAG(joint)                                           \
    ((joint)->flag & EMCMOT_JOINT_FERROR_BIT ? 1 : 0)
...
#define GET_JOINT_FAULT_FLAG(joint)                                            \
    ((joint)->flag & EMCMOT_JOINT_FAULT_BIT ? 1 : 0)
