...
static bool home_do_moving_checks(int jno)
{
    ...
    if ((&joints[jno])->on_pos_limit || (&joints[jno])->on_neg_limit) {
        ...
        if (!(H[jno].home_flags & HOME_IGNORE_LIMITS)) {
            ...
            rtapi_print_msg(RTAPI_MSG_ERR,
                            _("j%d hit limit in home state %d"),
                            jno,
                            H[jno].home_state);
    ...
    if (!(&joints[jno])->free_tp.active) {
        ...
        rtapi_print_msg(RTAPI_MSG_ERR,
                        _("j%d end of move in home state %d"),
                        jno,
                        H[jno].home_state);
        H[jno].home_state = HOME_ABORT;
        return 1; // abort reqd
...
static int base_make_joint_home_pins(int id, int njoints)
{
    ...
    if (joint_home_data == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        _("HOMING: all_joints_home_data_t malloc failed\n"));
        return -1;
    }
    ...
    for (jno = 0; jno < njoints; jno++) {
        ...
        retval += hal_pin_bit_newf(
            HAL_IN, &(addr->home_sw), id, "joint.%d.home-sw-in", jno);
        retval += hal_pin_bit_newf(
            HAL_OUT, &(addr->homing), id, "joint.%d.homing", jno);
        retval += hal_pin_bit_newf(
            HAL_OUT, &(addr->homed), id, "joint.%d.homed", jno);
        retval += hal_pin_s32_newf(
            HAL_OUT, &(addr->home_state), id, "joint.%d.home-state", jno);
        retval += hal_pin_bit_newf(
            HAL_IO, &(addr->index_enable), id, "joint.%d.index-enable", jno);
    }
    return retval;
...
static void do_homing_sequence(void)
{
            ...
            for (i = 0; i < all_joints; i++) {
                if (H[i].home_state == HOME_START) {
                    if (sequence_is_set &&
                        ...
                        rtapi_print_msg(
                            RTAPI_MSG_ERR,
                            _("homing.c Unexpected joint=%d jseq=%d "
                              "current_seq=%d\n"),
                            i,
                            H[i].home_sequence,
                            current_sequence);
    ...
    default:
        ...
        rtapi_print_msg(RTAPI_MSG_ERR,
                        _("unknown state '%d' during homing sequence"),
                        sequence_state);
        sequence_state = HOME_SEQUENCE_IDLE;
        homing_active = 0;
        break;
                            ...
                            emcmot_joint_t *pjoints)
{
    ...
    if (base_make_joint_home_pins(id, all_joints)) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "%s: base_make_joint_home_pins fail\n",
                        __FUNCTION__);
        return -1;
    }
...
static void base_set_unhomed(int jno, motion_state_t motstate)
{
    ...
    if (GET_JOINT_ACTIVE_FLAG(joint)) {
        ...
        rtapi_print_msg(
            RTAPI_MSG_ERR, _("Cannot unhome inactive joint %d\n"), jno);
    }
} // base_set_unhomed()
    ...
    do {
        ...
        case HOME_START:
            ...
            if (H[joint_num].home_flags & HOME_ABSOLUTE_ENCODER) {
                H[joint_num].home_flags &=
                    ~HOME_IS_SHARED; // shared not applicable
        ...
        case HOME_UNLOCK_WAIT:
            ...
            if (H[joint_num].home_search_vel == 0.0) {
                if (H[joint_num].home_latch_vel == 0.0) {
                    ...
                    rtapi_print_msg(
                        RTAPI_MSG_ERR,
                        _("invalid homing config: non-zero LATCH_VEL needs "
                          "either SEARCH_VEL or USE_INDEX"));
                    H[joint_num].home_state = HOME_ABORT;
                    immediate_state = 1;
                }
            } else {
                if (H[joint_num].home_latch_vel != 0.0) {
                    ...
                    rtapi_print_msg(
                        RTAPI_MSG_ERR,
                        _("invalid homing config: non-zero SEARCH_VEL needs "
                          "LATCH_VEL"));
                    H[joint_num].home_state = HOME_ABORT;
                    immediate_state = 1;
                }
        ...
        default:
            /* should never get here */
            rtapi_print_msg(RTAPI_MSG_ERR,
                            _("unknown state '%d' during homing j=%d"),
                            H[joint_num].home_state,
                            joint_num);
            H[joint_num].home_state = HOME_ABORT;
            immediate_state = 1;
            break;
