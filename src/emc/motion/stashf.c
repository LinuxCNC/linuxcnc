...
int vstashf(struct dbuf_iter *o, const char *fmt, va_list ap)
{
    ...
    while ((fmt = strchr(fmt, '%'))) {
        ...
        case 'X':
            if (modifier_l) {
            case 'p': dbuf_put_long(o, va_arg(ap, long));
            } else {
        ...
        case 'F':
        case 'g':
        case 'G': dbuf_put_double(o, va_arg(ap, double)); break;
        case 's': dbuf_put_string(o, va_arg(ap, const char *)); break;
        default: return SET_ERRNO(-EINVAL); break;
        }
    }
    return 0;
}
