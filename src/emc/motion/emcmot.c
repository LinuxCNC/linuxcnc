...
static int logIt = 0;
static int logStartTime;	/* set when logging is started, and subtracted
				   off each log time for better resolution */
...
static void setTrajCycleTime(double secs)
{
  ...
  for (t = 0; t < EMCMOT_MAX_AXIS; t++) {
    ...
    cubicSetInterpolationRate(&emcmotDebug->cubic[t], emcmotConfig->interpolationRate);
  }
...
static int emcmotCommandHandler(void)
{
  ...
  if (emcmotCommand->commandNum != emcmotStatus->commandNumEcho) {
	...
	if (!emcmotDebug->allHomed) {
	  reportError("all axes must be homed before going into coordinated mode");
	  emcmotDebug->coordinating = 0;
	  break;
	}
      }
	...
	break;
      ...
      emcmotDebug->inverseOutputScale[axis] = 1.0 / emcmotStatus->outputScale[axis];
      break;
	...
	break;
      ...
      emcmotDebug->inverseInputScale[axis] = 1.0 / emcmotStatus->inputScale[axis];
      break;
	...
	if (GET_AXIS_HOMED_FLAG(axis)) {
	  ...
	  emcmotDebug->freePose.tran.x = emcmotDebug->jointPos[axis] + AXRANGE(axis);
	}
      }
      else {
	if (GET_AXIS_HOMED_FLAG(axis)) {
	...
	break;
      ...
      if (emcmotCommand->vel > 0.0) {
	emcmotDebug->freePose.tran.x = emcmotDebug->jointPos[axis] + emcmotCommand->offset; /* FIXME-- use 'goal' instead */
	if (GET_AXIS_HOMED_FLAG(axis)) {
	...
	else if (emcmotDebug->freePose.tran.x < emcmotConfig->minLimit[axis]) {
	  emcmotDebug->freePose.tran.x = emcmotConfig->minLimit[axis];
	}
      }
	...
	break;
      ...
      pidSetGains(&emcmotConfig->pid[axis],
		  emcmotCommand->pid);
      break;
	...
	emcmotStatus->logTriggerThreshold = emcmotCommand->logTriggerThreshold;
	if (axis >= 0 &&
	    axis < EMCMOT_MAX_AXIS &&
	    emcmotStatus->logTriggerType == EMCLOG_DELTA_TRIGGER) 
	  {
		...
		emcmotStatus->logStartVal = emcmotDebug->ferrorCurrent[loggingAxis];
		break;
		...
		emcmotStatus->logStartVal = emcmotDebug->jointPos[loggingAxis] - emcmotDebug->oldJointPos[loggingAxis];
		break;
	...
	reportError("probe move %d out of range",
		    emcmotCommand->id);
	emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_PARAMS;
	tpAbort(&emcmotDebug->queue);
	SET_MOTION_ERROR_FLAG(1);
    ...
    case EMCMOT_SET_AOUT:
      tpSetAout(&emcmotDebug->queue, emcmotCommand->index, emcmotCommand->minLimit, emcmotCommand->maxLimit);
      break;

    case EMCMOT_SET_DOUT:
      tpSetDout(&emcmotDebug->queue, emcmotCommand->index, emcmotCommand->start, emcmotCommand->end);
      break;
    ...
    if (nominput > nominal[index]) {
      if (index == total - 1) {
        ...
        if (denom < DBL_MIN) {
          ...
          compin = ptr[lower] + (nominput - nominal[lower]) * (ptr[upper] - ptr[lower]) / denom;
        }
        break;
      }
      else {
...
static void emcmotController(void *arg)
{
...
#ifdef COMPING
  int dir[EMCMOT_MAX_AXIS] = {1}; /* flag for direction, used for axis comp */
#endif /* COMPING */
  ...
  for (;;) {
    ...
    /*
    This function does nothing in any of the external
    function implementations, except simmot.c
    extMotCycle(0);
    */
	...
	emcmotDebug->oldInput[axis] = emcmotStatus->input[axis];
      ...
      emcmotStatus->input[axis] =
        (emcmotDebug->rawInput[axis] - emcmotStatus->inputOffset[axis]) *
        emcmotDebug->inverseInputScale[axis] - emcmotDebug->bcompincr[axis];

#ifdef COMPING
      /* adjust feedback using compensation tables */
      if (GET_AXIS_HOMED_FLAG(axis)) {
        emcmotStatus->input[axis] = axisComp(axis, dir[axis], emcmotStatus->input[axis]);
      }
#endif
      if (first || !emcmotDebug->oldInputValid[axis] ) {
	emcmotDebug->oldInput[axis] = emcmotStatus->input[axis];
...
#ifndef SIMULATED_MOTORS
      if (fabs(emcmotStatus->input[axis] - emcmotDebug->oldInput[axis]) /
	...
	if (++positionInputDebounce[axis] > POSITION_INPUT_DEBOUNCE) {
	  ...
	  emcmotStatus->input[axis] = emcmotDebug->oldInput[axis] +
	    emcmotDebug->jointVel[axis] * emcmotConfig->servoCycleTime;
	}
	else {
	  /* we've exceeded the max number of debounces allowed, so
	     hold position. We should flag an error here, abort the move,
	     disable motion, etc. but for now we'll rely on following error
	     to do this */
	  emcmotStatus->input[axis] = emcmotDebug->oldInput[axis];
	}	  
	/* FIXME-- testing */
	...
	positionInputDebounce[axis] = 0;
      ...
      /* read limit switches and amp fault from external
         interface, and set 'emcmotDebug->enabling' to zero if tripped
      ...
      if (GET_AXIS_ACTIVE_FLAG(axis)) {
        ...
        if (isLimit == GET_AXIS_PHL_POLARITY(axis)) {
          if (++emcmotDebug->maxLimitSwitchCount[axis] > LIMIT_SWITCH_DEBOUNCE) {
            SET_AXIS_PHL_FLAG(axis, 1);
            emcmotDebug->maxLimitSwitchCount[axis] = LIMIT_SWITCH_DEBOUNCE;
            if (emcmotStatus->overrideLimits ||
                isHoming()) {
            }
            else {
      ...
      if (GET_AXIS_ACTIVE_FLAG(axis)) {
        ...
        if (isLimit == GET_AXIS_NHL_POLARITY(axis)) {
          if (++emcmotDebug->minLimitSwitchCount[axis] > LIMIT_SWITCH_DEBOUNCE) {
            SET_AXIS_NHL_FLAG(axis, 1);
            emcmotDebug->minLimitSwitchCount[axis] = LIMIT_SWITCH_DEBOUNCE;
            if (emcmotStatus->overrideLimits ||
                isHoming()) {
            }
            else {
	...
	break;
      ...
      case EMCLOG_TRIGGER_ON_VEL:
	val = emcmotDebug->jointPos[loggingAxis] - emcmotDebug->oldJointPos[loggingAxis];
	break;
	...
	if (emcmotStatus->logStartVal - val < -emcmotStatus->logTriggerThreshold ||
	    emcmotStatus->logStartVal - val > emcmotStatus->logTriggerThreshold) {
	  emcmotStatus->logStarted = 1;
	}
	break;
    ...
    if (emcmotDebug->enabling && ! GET_MOTION_ENABLE_FLAG()) {
      ...
      mmxavgInit(&emcmotDebug->fyMmxavg, emcmotDebug->fyMmxavgSpace, MMXAVG_SIZE);
      ...
      if (GET_AXIS_HOMING_FLAG(axis)) {
        if (tpIsDone(&emcmotDebug->freeAxis[axis])) {
          ...
          if (emcmotDebug->homingPhase[axis] == 5) {
            ...
            kinematicsHome(&worldHome, emcmotDebug->jointHome, &fflags, &iflags);

	    /* clear flag that indicates all joints need rehoming if any
	       joint is moved, for machines with no forward kins */
	    rehomeAll = 0;

            /* set input offset to value such that resulting input
               is the emcmotDebug->jointHome[] value, with:
               input = (raw - offset) / scale ->
               offset = raw - input * scale ->
               offset = raw - emcmotDebug->jointHome * scale */

            emcmotStatus->inputOffset[axis] = emcmotDebug->rawInput[axis] -
              emcmotDebug->jointHome[axis] * emcmotStatus->inputScale[axis];
            ...
            cubicOffset(&emcmotDebug->cubic[axis], emcmotDebug->jointHome[axis] - emcmotDebug->coarseJointPos[axis]);

            /* set axis position to emcmotDebug->jointHome upon homing */
            emcmotDebug->jointPos[axis] = emcmotDebug->jointHome[axis];
	    emcmotDebug->coarseJointPos[axis] = emcmotDebug->jointHome[axis];
            emcmotDebug->freePose.tran.x = emcmotDebug->jointHome[axis];
            tpSetPos(&emcmotDebug->freeAxis[axis], emcmotDebug->freePose);

            SET_AXIS_HOMING_FLAG(axis, 0);
            SET_AXIS_HOMED_FLAG(axis, 1);

            /* set emcmotDebug->allHomed flag if all active axes are homed;
	       this will signify that kinematics functions can be called */
	    ...
	    if ((retval = tpAddLine(&emcmotDebug->freeAxis[axis], emcmotDebug->freePose)) == 0) {
	      /* Advance homing sequence only if motion is added to the tp */
    ...
    if (GET_MOTION_ENABLE_FLAG()) {
      ...
      while (cubicNeedNextPoint(&emcmotDebug->cubic[0])) {
        ...
        /* check to see whether we're in teleop, coordinated or free mode, to
           decide which motion planner to call */
	if (GET_MOTION_TELEOP_FLAG()) {
	  double accell_mag;

	  emcmotDebug->teleop_data.desiredAccell.tran.x = (emcmotDebug->teleop_data.desiredVel.tran.x - emcmotDebug->teleop_data.currentVel.tran.x)/emcmotConfig->trajCycleTime;
	  emcmotDebug->teleop_data.desiredAccell.tran.y = (emcmotDebug->teleop_data.desiredVel.tran.y - emcmotDebug->teleop_data.currentVel.tran.y)/emcmotConfig->trajCycleTime;
	  emcmotDebug->teleop_data.desiredAccell.tran.z = (emcmotDebug->teleop_data.desiredVel.tran.z - emcmotDebug->teleop_data.currentVel.tran.z)/emcmotConfig->trajCycleTime;
	  pmCartMag(emcmotDebug->teleop_data.desiredAccell.tran, &accell_mag);
	  emcmotDebug->teleop_data.desiredAccell.a = (emcmotDebug->teleop_data.desiredVel.a - emcmotDebug->teleop_data.currentVel.a)/emcmotConfig->trajCycleTime;
	  emcmotDebug->teleop_data.desiredAccell.b = (emcmotDebug->teleop_data.desiredVel.b - emcmotDebug->teleop_data.currentVel.b)/emcmotConfig->trajCycleTime;
	  emcmotDebug->teleop_data.desiredAccell.c = (emcmotDebug->teleop_data.desiredVel.c - emcmotDebug->teleop_data.currentVel.c)/emcmotConfig->trajCycleTime;
	  ...
	  if (accell_mag > emcmotStatus->acc) {
	    pmCartScalMult(emcmotDebug->teleop_data.desiredAccell.tran, emcmotStatus->acc/accell_mag,
			   &emcmotDebug->teleop_data.currentAccell.tran);
	    emcmotDebug->teleop_data.currentAccell.a =
	      emcmotDebug->teleop_data.desiredAccell.a * emcmotStatus->acc/accell_mag;
	    emcmotDebug->teleop_data.currentAccell.b =
	      emcmotDebug->teleop_data.desiredAccell.b * emcmotStatus->acc/accell_mag;
	    emcmotDebug->teleop_data.currentAccell.c =
	      emcmotDebug->teleop_data.desiredAccell.c * emcmotStatus->acc/accell_mag;
	    emcmotDebug->teleop_data.currentVel.tran.x += emcmotDebug->teleop_data.currentAccell.tran.x*emcmotConfig->trajCycleTime;
	    emcmotDebug->teleop_data.currentVel.tran.y += emcmotDebug->teleop_data.currentAccell.tran.y*emcmotConfig->trajCycleTime;
	    emcmotDebug->teleop_data.currentVel.tran.z += emcmotDebug->teleop_data.currentAccell.tran.z*emcmotConfig->trajCycleTime;
	    emcmotDebug->teleop_data.currentVel.a += emcmotDebug->teleop_data.currentAccell.a*emcmotConfig->trajCycleTime;
	    emcmotDebug->teleop_data.currentVel.b += emcmotDebug->teleop_data.currentAccell.b*emcmotConfig->trajCycleTime;
	    emcmotDebug->teleop_data.currentVel.c += emcmotDebug->teleop_data.currentAccell.c*emcmotConfig->trajCycleTime;
	  }
	  else {
	  ...
	  emcmotStatus->pos.tran.x += emcmotDebug->teleop_data.currentVel.tran.x*emcmotConfig->trajCycleTime;
	  emcmotStatus->pos.tran.y += emcmotDebug->teleop_data.currentVel.tran.y*emcmotConfig->trajCycleTime;
	  emcmotStatus->pos.tran.z += emcmotDebug->teleop_data.currentVel.tran.z*emcmotConfig->trajCycleTime;	      
	  emcmotStatus->pos.a += emcmotDebug->teleop_data.currentVel.a*emcmotConfig->trajCycleTime;
	  emcmotStatus->pos.b += emcmotDebug->teleop_data.currentVel.b*emcmotConfig->trajCycleTime;
	  emcmotStatus->pos.c += emcmotDebug->teleop_data.currentVel.c*emcmotConfig->trajCycleTime;	      

	  /* convert to joints */
	  kinematicsInverse(&emcmotStatus->pos, emcmotDebug->coarseJointPos, &iflags, &fflags);

	  /* spline joints up-- note that we may be adding points that
	     fail soft limits, but we'll abort at the end of this cycle
	     so it doesn't really matter */
	  for (axis = 0; axis < EMCMOT_MAX_AXIS; axis++) {
	    cubicAddPoint(&emcmotDebug->cubic[axis], emcmotDebug->coarseJointPos[axis]);
	  }
	  ...
	  if (GET_MOTION_COORD_FLAG()) {
	    ...
	    for (axis = 0; axis < EMCMOT_MAX_AXIS; axis++) {
		 ...
		 We need a TP_STRUCT with a single scalar element. */
	      emcmotDebug->coarseJointPos[axis] = tpGetPos(&emcmotDebug->freeAxis[axis]).tran.x;
	    ...
	    if (kinType == KINEMATICS_IDENTITY) {
	      ...
	      kinematicsForward(emcmotDebug->coarseJointPos, &emcmotStatus->pos, &fflags, &iflags);
	    }
	    else if (kinType != KINEMATICS_INVERSE_ONLY) {
	      /* here is where we call the forward kinematics repeatedly,
		 when we're in free mode, so that the world coordinates are
		 kept up to date when joints are moving. This is only done
		 if we have the kinematics. emcmotStatus->pos needs to be set
		 with an estimate for the kinematics to converge, which is
		 true when we enter free mode from coordinated mode or after
		 the machine is homed. */
	      EmcPose temp = emcmotStatus->pos;
	      if (0 == kinematicsForward(emcmotStatus->input, &temp, &fflags, &iflags)) {
		...
		emcmotStatus->actualPos = temp;
		 ...
		 estimate of world coords, and we have to leave them alone */
	    }
      ...
      /* we're still in motion enabled section. For coordinated mode,
         ...
         Cartesian pose, emcmotDebug->coarseJointPos[] contains the results of the
         ...
         happens, and note where in this comment. emcmotDebug->coarseJointPos[]
      ...
      */
      ...
      for (axis = 0; axis < EMCMOT_MAX_AXIS; axis++) {
        ...
        if (GET_AXIS_HOMED_FLAG(axis)) {
          if (emcmotDebug->coarseJointPos[axis] > emcmotConfig->maxLimit[axis]) {
            SET_AXIS_ERROR_FLAG(axis, 1);
            SET_AXIS_PSL_FLAG(axis, 1);
            emcmotDebug->onLimit = 1;
          }
          else if (emcmotDebug->coarseJointPos[axis] < emcmotConfig->minLimit[axis]) {
            SET_AXIS_ERROR_FLAG(axis, 1);
      ...
      /* reset emcmotDebug->wasOnLimit flag iff all joints are free of
         ...
         No need to do this for hard limits, since emcmotDebug->wasOnLimit
      ...
      if (emcmotDebug->onLimit) {
        ...
        /* not on a limit, so clear emcmotDebug->wasOnLimit so aborts fire next
      ...
      if (whichCycle == 2) {
        ...
        if (emcmotStatus->logStarted &&
          ...
          emcmotDebug->newVel.tran.x = (emcmotStatus->pos.tran.x - emcmotDebug->oldPos.tran.x) /
            emcmotConfig->trajCycleTime;
          emcmotDebug->newVel.tran.y = (emcmotStatus->pos.tran.y - emcmotDebug->oldPos.tran.y) /
            emcmotConfig->trajCycleTime;
          emcmotDebug->newVel.tran.z = (emcmotStatus->pos.tran.z - emcmotDebug->oldPos.tran.z) /
            emcmotConfig->trajCycleTime;
          emcmotDebug->oldPos = emcmotStatus->pos;
          emcmotDebug->newAcc.tran.x = (emcmotDebug->newVel.tran.x - emcmotDebug->oldVel.tran.x) /
            emcmotConfig->trajCycleTime;
          emcmotDebug->newAcc.tran.y = (emcmotDebug->newVel.tran.y - emcmotDebug->oldVel.tran.y) /
            emcmotConfig->trajCycleTime;
          emcmotDebug->newAcc.tran.z = (emcmotDebug->newVel.tran.z - emcmotDebug->oldVel.tran.z) /
            emcmotConfig->trajCycleTime;
          ...
          case EMCMOT_LOG_TYPE_TRAJ_VEL:
            if (logSkip-- <= 0) {
              ...
              pmCartMag(emcmotDebug->newVel.tran, &ls.item.trajVel.mag);
              logSkip = emcmotStatus->logSkip;
              logIt = 1;
            }
            break;
          ...
          case EMCMOT_LOG_TYPE_TRAJ_ACC:
            if (logSkip-- <= 0) {
              ...
              pmCartMag(emcmotDebug->newAcc.tran, &ls.item.trajAcc.mag);
              logSkip = emcmotStatus->logSkip;
              logIt = 1;
            }
            break;
      ...
      for (axis = 0; axis < EMCMOT_MAX_AXIS; axis++) {
        ...
        emcmotDebug->jointPos[axis] = cubicInterpolate(&emcmotDebug->cubic[axis], 0, 0, 0, 0);
        emcmotDebug->jointVel[axis] = (emcmotDebug->jointPos[axis] - emcmotDebug->oldJointPos[axis]) / emcmotConfig->servoCycleTime;
#ifdef COMPING
        ...
        if (GET_AXIS_ACTIVE_FLAG(axis)) {
          ...
          /* FIXME-- make backlash part of the EMC status proper, not
             the PID structure */
          if (emcmotDebug->jointPos[axis] - emcmotDebug->oldJointPos[axis] > 0.0) {
            oldbcomp = emcmotDebug->bcomp[axis];
            emcmotDebug->bcomp[axis] = + emcmotConfig->pid[axis].backlash / 2.0;
            if (emcmotDebug->bcompdir[axis] != + 1) {
              emcmotDebug->bac_done[axis] = 0;
              emcmotDebug->bac_di[axis] = emcmotDebug->bcompincr[axis];
              emcmotDebug->bac_d[axis] = 0;
              emcmotDebug->bac_D[axis] = emcmotDebug->bcomp[axis] - oldbcomp;
              emcmotDebug->bac_halfD[axis] = 0.5 * emcmotDebug->bac_D[axis];
              emcmotDebug->bac_incrincr[axis] = emcmotStatus->acc *
                emcmotConfig->servoCycleTime * emcmotConfig->servoCycleTime;
              emcmotDebug->bac_incr[axis] = - 0.5 * emcmotDebug->bac_incrincr[axis];
              emcmotDebug->bcompdir[axis] = + 1;
            }
          }
          else if (emcmotDebug->jointPos[axis] - emcmotDebug->oldJointPos[axis] < 0.0) {
            oldbcomp = emcmotDebug->bcomp[axis];
            emcmotDebug->bcomp[axis] = - emcmotConfig->pid[axis].backlash / 2.0;
            if (emcmotDebug->bcompdir[axis] != - 1) {
              emcmotDebug->bac_done[axis] = 0;
          ...
          if (! emcmotDebug->bac_done[axis]) {
            if (emcmotDebug->bac_D[axis] > 0.0) {
              emcmotDebug->bcompincr[axis] = emcmotDebug->bac_di[axis] + emcmotDebug->bac_d[axis];
              if (emcmotDebug->bac_d[axis] < emcmotDebug->bac_halfD[axis]) {
                emcmotDebug->bac_incr[axis] += emcmotDebug->bac_incrincr[axis];
                emcmotDebug->bac_d[axis] += emcmotDebug->bac_incr[axis];
              }
              else if (emcmotDebug->bac_d[axis] < emcmotDebug->bac_D[axis]) {
                emcmotDebug->bac_incr[axis] -= emcmotDebug->bac_incrincr[axis];
                emcmotDebug->bac_d[axis] += emcmotDebug->bac_incr[axis];
              }
              if (emcmotDebug->bac_d[axis] >= emcmotDebug->bac_D[axis] || emcmotDebug->bac_incr[axis] <= 0.0) {
          ...
          /* ADJUST OUTPUT:
            this computes outJointPos[] from emcmotDebug->jointPos[], by adding backlash
            compensation and rounding result to nearest input unit
          */
          emcmotDebug->outJointPos[axis] = emcmotDebug->jointPos[axis] + emcmotDebug->bcompincr[axis];

#ifndef NO_ROUNDING
          numres = emcmotDebug->outJointPos[axis] * emcmotStatus->inputScale[axis];
          if (numres < 0.0) {
            emcmotDebug->outJointPos[axis] = emcmotDebug->inverseInputScale[axis] * ((int) (numres - 0.5));
          }
          else {
            emcmotDebug->outJointPos[axis] = emcmotDebug->inverseInputScale[axis] * ((int) (numres + 0.5));
          }
	  ...
	  /* Here's where binfunc_trajupdate() was called. */
          ...
          */
          ...
          /* note that we have to compare adjusted output 'outJointPos'
             with the input, but the input has already had backlash comp
             taken out, while the output has just had it added in. So,
             we need to add it to the input for this calculation */
          emcmotStatus->output[axis] =
            pidRunCycle(&emcmotConfig->pid[axis],
                        emcmotStatus->input[axis] + emcmotDebug->bcompincr[axis],
                        emcmotDebug->outJointPos[axis]);

          /* COMPUTE FOLLOWING ERROR: */

          /* compute signed following error and magnitude */
          thisFerror[axis] = emcmotDebug->jointPos[axis] - emcmotStatus->input[axis];
	  ...
	  if (emcmotDebug->latchFlag[axis]) {
	    ...
	    /* call for an abort-- when it's finished, code
	       above sets inputOffset[] to emcmotDebug->saveLatch[] */
	    if (tpAbort(&emcmotDebug->freeAxis[axis]) == 0) {
	      /* Only advance the homing sequence if the motion is really aborted */
	      emcmotDebug->homingPhase[axis] = 4;
	      /* get latched position in RAW UNITS */
	      emcmotDebug->saveLatch[axis] = emcmotDebug->rawInput[axis];
	    }
	...
	} /* end of: if(emcmotDebug->homingPhase[axis]==1){ */
      } /* end of: for (axis = 0; ...) */
    } /* end of: if (GET_MOTION_ENABLE_FLAG()) */
    else {
      /*
	we're not enabled, so no motion planning or interpolation has
	been done. emcmotDebug->jointPos[] is set to emcmotStatus->input[],
	and likewise with emcmotDebug->coarseJointPos[], which is normally
	updated at the traj rate but it's convenient to do them here at the
	same time at the servo rate. emcmotStatus->pos, ->actualPos need to be
	run through forward kinematics.  Note that we are running at the servo
	...
	mode) or forward kins (free mode), and ->actualPos was set by forward
	...
	{
	  if(emcmotDebug->cur_time - emcmotDebug->last_time > 10*emcmotConfig->servoCycleTime)
	    {
      ...
      if(DEBUG_EMCMOT)
	{
	  mmxavgAdd(&emcmotDebug->yMmxavg, (emcmotDebug->cur_time - emcmotDebug->last_time));
    ...
    if (emcmotStatus->logStarted &&
        ...
        if (logSkip-- <= 0) {
          ...
          for (axis = 0;
               axis < EMCMOT_LOG_NUM_AXES &&
                 axis < EMCMOT_MAX_AXIS;
               axis++) {
            ls.item.allOutpos.output[axis] = emcmotDebug->jointPos[axis];
          }
          logSkip = emcmotStatus->logSkip;
          logIt = 1;
        }
      ...
      case EMCMOT_LOG_TYPE_AXIS_VEL:
        if (logSkip-- <= 0) {
          ...
          ls.item.axisVel.cmdVel = emcmotDebug->jointPos[loggingAxis] -
        ...
        if (logSkip-- <= 0) {
          ...
          for (axis = 0;
               axis < EMCMOT_LOG_NUM_AXES &&
                 axis < EMCMOT_MAX_AXIS;
               axis++) {
            ls.item.allFerror.ferror[axis] = emcmotDebug->ferrorCurrent[axis];
          }
          logSkip = emcmotStatus->logSkip;
          logIt = 1;
        }
      ...
      case EMCMOT_LOG_TYPE_POS_VOLTAGE:
        ...
        if (logSkip-- <= 0) {
          ...
          for (axis = 0;
            ...
            ls.item.posVoltage.voltage = emcmotDebug->rawOutput[loggingAxis];
          }
          logSkip = emcmotStatus->logSkip;
          logIt = 1;
        }
...
int init_module(void)
{
  ...
  if (period == 0) {
    ...
    if (period < 0) {
      rtapi_print("emcmot init: rtapi_clock_set_period failed with %ld\n",
		  period);
      rtapi_exit(module);
      return -1;
    }
  ...
  emcmotIo    = (EMCMOT_IO *) &emcmotStruct->io;      /* set address of struct JE 8/21/2001 */
  ...
  for (axis = 0; axis < EMCMOT_MAX_AXIS; axis++) {
    if (-1 == tpCreate(&emcmotDebug->freeAxis[axis], FREE_AXIS_QUEUE_SIZE, emcmotDebug->freeAxisTcSpace[axis])) {
      rtapi_print("can't create axis emcmotDebug->queue %d\n", axis);
      return -1;
    }
    tpInit(&emcmotDebug->freeAxis[axis]);
    tpSetCycleTime(&emcmotDebug->freeAxis[axis], emcmotConfig->trajCycleTime);
  ...
  emcmot_task = rtapi_task_new(emcmotController, (void *) 0, emcmot_prio, module, EMCMOT_TASK_STACKSIZE, RTAPI_USES_FP);
  if (emcmot_task < 0) {
...
void cleanup_module(void)
{
  ...
  /* There is no point in clearing the trajectory queue since the
     planner should be dead by now anyway. */
