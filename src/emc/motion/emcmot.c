/********************************************************************
*
* Description: emcmot.c
*
*	Real-time motion controller for queued 3-axis motion planning,
*	independent axis motion, cubic subinterpolation, and stepper
*	or servo control.
*
*	The original changelog has been removed (to be placed in a
*	seperate text file perhaps..). Splitting the code into smaller
*	chunks makes it easier to handle and has the added advantage
*	that we can compile against a 2.6 kernel without the headache
*	of multiple binaries from a single source file.
*
*    
* Author:
* License:
...
extern int DEBUG_EMCMOT;

extern int emcmotTask_started;
...
int logIt = 0;
int logStartTime;	/* set when logging is started, and subtracted
...
static RTIME starting_nanos = 0;
static int starting_nanos_initialized = 0;

double etime(void)
{
  RTIME current_nanos = 0;
  double current_nanos_d = 0.0;
  if (!starting_nanos_initialized) {
    starting_nanos = rt_get_time_ns();
    current_nanos = 0;
    starting_nanos_initialized = 1;
  } else {
    current_nanos = rt_get_time_ns() - starting_nanos;
  }
  current_nanos_d = (double) current_nanos;
  return (current_nanos_d * 1.0e-9);
}
...
static double axisComp(int axis, int dir, double nominput)
{
  ...
  while (index >= 0 && index < total) {
	...
	if (denom < DBL_MIN) {
	  ...
	  compin =
	    ptr[lower] + (nominput - nominal[lower]) * (ptr[upper] -
							ptr[lower]) / denom;
	}
	break;
      } else {
...
/* FIXME-- testing */
extern int debouncecount[EMCMOT_MAX_AXIS];

/*
    actuatorPosition[] is the "motor shaft position", scaled to user units but not including backlash
*/

static double actuatorPosition[EMCMOT_MAX_AXIS] = { 0.0 };
...
/* some variables kasunich */
  ...
  if (!emcmotTask_started) {
    diagnostics("emcmotController started.\n");
  }
  emcmotTask_started = 1;
  ...
  for (;;) {
    ...
    if (emcmotDebug->wdEnabling && !emcmotDebug->wdEnabled) {
      /* just turned WD on */
      emcmotDebug->wdCount = emcmotDebug->wdWait;
      emcmotDebug->wdToggle = 0;
      emcmotDebug->wdEnabled = 1;
    }
    if (!emcmotDebug->wdEnabling && emcmotDebug->wdEnabled) {
      /* just turned WD off */
      /* clear the bit */
      ...
      emcmotDebug->wdEnabled = 0;
    }

    if (emcmotDebug->wdEnabled && emcmotDebug->wdCount-- <= 0) {
      soundByte = inb(SOUND_PORT);
      emcmotDebug->wdToggle = !emcmotDebug->wdToggle;
      if (emcmotDebug->wdToggle) {
	soundByte |= SOUND_MASK;
      } else {
	soundByte &= ~SOUND_MASK;
      }
	...
	emcmotDebug->oldInput[axis] = emcmotStatus->input[axis];
      ...
      /*
         compute actuator input in user units, but without backlash
       */
      actuatorPosition[axis] =
	(emcmotDebug->rawInput[axis] -
	 emcmotStatus->inputOffset[axis]) *
	emcmotDebug->inverseInputScale[axis];

      /* set adjusted input to include backlash */
      emcmotStatus->input[axis] =
	actuatorPosition[axis] - emcmotDebug->bcompincr[axis];
	...
	if (isLimit == GET_AXIS_NHL_POLARITY(axis)) {
	  if (++emcmotDebug->minLimitSwitchCount[axis] >
	    ...
	    if (emcmotStatus->overrideLimits || isHoming()) {
	      SET_AXIS_NHL_FLAG(axis,0);			// Smithy hack to allow homing off a limit switch.
	    } else {
	      SET_AXIS_ERROR_FLAG(axis, 1);
	      emcmotDebug->enabling = 0;
	    }
	...
	if (tpIsDone(&emcmotDebug->freeAxis[axis])) {
	  ...
	  if (emcmotDebug->homingPhase[axis] == 5) {
	    ...
	    kinematicsHome(&worldHome, emcmotDebug->jointHome, &fflags,
			   &iflags);
	    ...
	    /* set input offset to value such that resulting input
	       ...
	       inputPos = jointHome
	       actuatorPos = inputPos + bcompincr

	       actuatorPos = (raw - offset) / scale ->
	       offset = raw - actuatorPos * scale
	     */
	    ...
	    emcmotStatus->inputOffset[axis] = emcmotDebug->rawInput[axis] -
	      actuatorPosition[axis] * emcmotStatus->inputScale[axis];
	    ...
	    for (t = 0; t < EMCMOT_MAX_AXIS; t++) {
	      if (GET_AXIS_ACTIVE_FLAG(t) && !GET_AXIS_HOMED_FLAG(t)) {
	  ...
	  if (emcmotDebug->homingPhase[axis] == 4) {
	    ...
	    emcmotDebug->freePose.tran.x =
	      (emcmotDebug->saveLatch[axis] -
	       emcmotStatus->inputOffset[axis]) *
	      emcmotDebug->inverseInputScale[axis] -
	      emcmotDebug->bcompincr[axis] + emcmotConfig->homeOffset[axis];
	...
	if (GET_MOTION_TELEOP_FLAG()) {
	  ...
	  if (accell_mag > emcmotStatus->acc) {
	    ...
	    emcmotDebug->teleop_data.currentAccell.a =
	      emcmotDebug->teleop_data.desiredAccell.a * emcmotStatus->acc /
	      accell_mag;
	    emcmotDebug->teleop_data.currentAccell.b =
	      emcmotDebug->teleop_data.desiredAccell.b * emcmotStatus->acc /
	      accell_mag;
	    emcmotDebug->teleop_data.currentAccell.c =
	      emcmotDebug->teleop_data.desiredAccell.c * emcmotStatus->acc /
	      accell_mag;
	    emcmotDebug->teleop_data.currentVel.tran.x +=
	  ...
	  /* convert to joints */
	  kinematicsInverse(&emcmotStatus->pos, emcmotDebug->coarseJointPos,
			    &iflags, &fflags);
	  ...
	  if (kinType == KINEMATICS_IDENTITY) {
	    ...
	    kinematicsForward(emcmotStatus->input, &emcmotStatus->actualPos,
			      &fflags, &iflags);
	  } else {
	  ...
	  if (GET_MOTION_COORD_FLAG()) {
	    ...
	    if (kinType == KINEMATICS_IDENTITY) {
	      ...
	      if (0 ==
		  kinematicsForward(emcmotStatus->input, &temp, &fflags,
				    &iflags)) {
	...
	emcmotDebug->jointVel[axis] =
	  (emcmotDebug->jointPos[axis] -
	   emcmotDebug->oldJointPos[axis]) / emcmotConfig->servoCycleTime;
#ifdef COMPING
	...
	if (GET_AXIS_ACTIVE_FLAG(axis)) {
	  ...
	  /* FIXME-- make backlash part of the EMC status proper, not
	     the PID structure */
	  /* compute step size for backlash ramper */
	  emcmotDebug->bac_incr[axis] =
	    emcmotConfig->axisLimitVel[axis] * emcmotConfig->servoCycleTime;

	  /* detect direction */
	  deltaPos =
	    emcmotDebug->jointPos[axis] - emcmotDebug->oldJointPos[axis];
	  if (deltaPos > 0.0) {
	    emcmotDebug->bcomp[axis] =
	      +emcmotConfig->pid[axis].backlash / 2.0;
	  } else if (deltaPos < 0.0) {
	    emcmotDebug->bcomp[axis] =
	      -emcmotConfig->pid[axis].backlash / 2.0;
	  }
	  /* else no motion, so leave emcmotDebug->bcomp what it was */

	  /* mete out the backlash according using a linear ramp */
	  if (emcmotDebug->bcompincr[axis] > emcmotDebug->bcomp[axis]) {
	    /* too hi */
	    if (emcmotDebug->bcompincr[axis] >
		(emcmotDebug->bcomp[axis] + emcmotDebug->bac_incr[axis])) {
	      /* really hi, take a full increment */
	      emcmotDebug->bcompincr[axis] -= emcmotDebug->bac_incr[axis];
	    } else {
	      /* almost there, just do it */
	      emcmotDebug->bcompincr[axis] = emcmotDebug->bcomp[axis];
	    }
	  } else {
	    if (emcmotDebug->bcompincr[axis] <
		(emcmotDebug->bcomp[axis] - emcmotDebug->bac_incr[axis])) {
	      /* really lo, take a full increment */
	      emcmotDebug->bcompincr[axis] += emcmotDebug->bac_incr[axis];
	    } else {
	      /* almost there, just do it */
	      emcmotDebug->bcompincr[axis] = emcmotDebug->bcomp[axis];
	  ...
	  emcmotDebug->outJointPos[axis] =
	    emcmotDebug->jointPos[axis] + emcmotDebug->bcompincr[axis];
#ifndef NO_ROUNDING
	  numres =
	    emcmotDebug->outJointPos[axis] * emcmotStatus->inputScale[axis];
	  if (numres < 0.0) {
	    emcmotDebug->outJointPos[axis] =
	      emcmotDebug->inverseInputScale[axis] * ((int) (numres - 0.5));
	  } else {
	    emcmotDebug->outJointPos[axis] =
	      emcmotDebug->inverseInputScale[axis] * ((int) (numres + 0.5));
	  }
	  ...
	  limitFerror = fabs(emcmotConfig->maxFerror[axis] /
	...
	if (logSkip-- <= 0) {
	  ...
	  for (axis = 0;
	       axis < EMCMOT_LOG_NUM_AXES && axis < EMCMOT_MAX_AXIS; axis++) {
