/*
   ...
   13-May-2003 JMK fixed a typo that would drop the FF terms from PID output if SMOOTH_D_FACTOR is defined
   6-Jul-2001  FMP took out pidGetCycleTime(), pidGetGains(), since as Max and Will noted they are pretty useless.
   6-Jul-2001 Max-Heise fixed bug in pidGetCycleTime (commited by WPS).
   7-Nov-2000 WPS added code to prevent  a problem when ff1 is not zero and the pid is reset on a non-zero position.
   When the machine came out of estop, the lastSetPoint would be zero, the setpoint could be large making
   ff1 *(setpoint - lastSetPoint)  large. Now when coming out of estop the real effect of pidReset should
   be to set a flag declaring lastSetpoint invalid, it will be set to the next setpoint in pidRunCycle.
