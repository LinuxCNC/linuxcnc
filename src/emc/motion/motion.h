...
typedef struct emcmot_command_t {
    ...
    unsigned char now, out, start,
        end; /* these are related to synched AOUT/DOUT. now=whether now or synched, out = which gets set, start=start value, end=end value */
    unsigned char mode; /* used for turning overrides etc. on/off */
    double comp_nominal, comp_forward,
        comp_reverse; /* compensation triplet, nominal, forward, reverse */
    unsigned char
        probe_type; /* ~1 = error if probe operation is unsuccessful (ngc default)
    ...
    unsigned char
        wait_for_spindle_at_speed; // EMCMOT_SPINDLE_ON now carries this, for next feed move
...
typedef struct emcmot_status_t {
    ...
    emcmot_joint_status_t
        joint_status[EMCMOT_MAX_JOINTS]; /* all joint status data */
    emcmot_axis_status_t
        axis_status[EMCMOT_MAX_AXIS]; /* all axis status data */
    ...
    int synch_di
        [EMCMOT_MAX_DIO]; /* inputs to the motion controller, queried by G-code */
    int synch_do
        [EMCMOT_MAX_DIO]; /* outputs to the motion controller, queried by G-code */
    double analog_input
        [EMCMOT_MAX_AIO]; /* inputs to the motion controller, queried by G-code */
    double analog_output
        [EMCMOT_MAX_AIO]; /* outputs to the motion controller, queried by G-code */
...
extern int
emcmotErrorPutfv(emcmot_error_t *errlog, const char *fmt, va_list ap);
...
#define GET_JOINT_ACTIVE_FLAG(joint)                                           \
    ((joint)->flag & EMCMOT_JOINT_ACTIVE_BIT ? 1 : 0)
#define GET_JOINT_INPOS_FLAG(joint)                                            \
    ((joint)->flag & EMCMOT_JOINT_INPOS_BIT ? 1 : 0)
