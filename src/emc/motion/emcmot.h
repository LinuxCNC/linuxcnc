...
enum {
    ...
    EMCMOT_SET_TELEOP_VECTOR,	/* Move at a given velocity but in world
				   cartesian coordinates, not in joint space
				   like EMCMOT_JOG_* */
...
typedef struct {
    ...
    double minOutput[EMCMOT_MAX_AXIS];	/* minimum output value allowed,
					   volts */
    double maxOutput[EMCMOT_MAX_AXIS];	/* maximum output value allowed,
					   volts */
    ...
    double setup_time[EMCMOT_MAX_AXIS];	/* number of periods before step
					   occurs that dir changes */
    double hold_time[EMCMOT_MAX_AXIS];	/* number of periods that step line
					   is held low/high after transition 
					 */
...
typedef struct {
    ...
    double yMin, yMax, yAvg;	/* min, max, avg times cycle times rather
				   than compute */
    double fMin, fMax, fAvg;	/* min, max, avg times frequency */
    double fyMin, fyMax, fyAvg;	/* min, max, avg times frequency cycle times
				   rather than compute */
    ...
    /* 
       stepperCount[] contains the accumulated pulses that have been output,
       which are used as the "encoder feedback" for open-loop stepping. */
