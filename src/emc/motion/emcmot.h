/********************************************************************
*
* Description: emcmot.h
*
*	Structures used throughout EMC along with macros and function
*	declarations for the realtime code.
*
*    
* Author:
* License:
...
typedef struct {
  ...
  unsigned char OutBytes[EMCMOT_IO_SIZE]; /* reserve place for up to 8 bytes to be written */
  unsigned char InBytes[EMCMOT_IO_SIZE];  /* reserve place for up to 8 bytes to be read */
} EMCMOT_IO;
...
typedef struct
{
  ...
  EMCMOT_DEBUG debug;
  EMCMOT_ERROR error;
  EMCMOT_IO io;              /* rt<=> Linux comm for digital output  JE 7/x/2001 */
  EMCMOT_LOG log;
...
/* value of compensator output below which it's effectively off, so that
   the frequency generator can be turned off */
#define OUTPUT_DELTA 1.0e-6
...
extern EMCMOT_ERROR *emcmotError;	/* unused for RT_FIFO */
extern EMCMOT_LOG *emcmotLog;	/* unused for RT_FIFO */
extern EMCMOT_IO *emcmotIo;	/* new struct added 8/21/2001 JME */
extern EMCMOT_COMP *emcmotComp[EMCMOT_MAX_AXIS];	/* unused for RT_FIFO */
...
extern void clearHomes(int axis);
extern int checkLimits(void);
extern int checkJog(int axis, double vel);
extern int inRange(EmcPose pos);


extern void setTrajCycleTime(double secs);
extern void setServoCycleTime(double secs);
extern void emcmotController(int arg);

/* Need these two headers for the RTIME & RT_TASK definitions */
#include <rtai.h>
#include <rtai_sched.h>
extern RTIME rtaiTickPeriod;
extern RTIME now;
extern long rtaiTickPeriod_long;
...
extern int PERIOD;	/* fundamental period for timer interrupts */

extern int iperiod;
extern RTIME iperiod_rtime;
extern RT_TASK emcmotTask;
