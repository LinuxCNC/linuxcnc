...
STATIC int inRange(EmcPose pos, int id, char *move_type)
{
    ...
    if (EMCMOT_MAX_AXIS != 9) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "BUG: %s(): invalid number of axes defined",
                        __func__);
    } else {
    ...
    if (kinematicsInverse(&pos, joint_pos, &iflags, &fflags) != 0) {
        reportError(
            _("%s move on line %d fails kinematicsInverse"), move_type, id);
        return 0;
    }
    ...
    for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
        ...
        if (!isfinite(joint_pos[joint_num])) {
            reportError(
                _("%s move on line %d gave non-finite joint location on joint "
                  "%d"),
                move_type,
                id,
                joint_num);
        ...
        if (joint_pos[joint_num] > joint->max_pos_limit) {
            in_range = 0;
            reportError(
                _("%s move on line %d would exceed joint %d's positive limit "
                  "max:[%f]"),
                move_type,
                id,
                joint_num,
        ...
        if (joint_pos[joint_num] < joint->min_pos_limit) {
            in_range = 0;
            reportError(
                _("%s move on line %d would exceed joint %d's negative limit "
                  "min:[%f]"),
                move_type,
                id,
                joint_num,
...
int emcmotGetRotaryIsUnlocked(int jnum)
{
    ...
    if (NULL == emcmot_hal_data->joint[jnum].unlock) {
        if (!gave_message) {
            reportError(
                "emcmotGetRotaryUnlocked(): No unlock pin configured for joint "
                "%d\n"
                "   Use motmod parameter: unlock_joints_mask=%X'",
                jnum,
                1 << jnum);
...
void emcmotDioWrite(int index, char value)
{
    if ((index >= emcmotConfig->numDIO) || (index < 0)) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "ERROR: index out of range, %d not in [0..%d] (increase "
            "num_dio/EMCMOT_MAX_DIO=%d)\n",
            index,
            emcmotConfig->numDIO,
            EMCMOT_MAX_DIO);
    } else {
...
STATIC int is_feed_type(int motion_type)
{
    ...
    case EMC_MOTION_TYPE_PROBING: return 1;
    default:
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    case EMC_MOTION_TYPE_INDEXROTARY: return 0;
    }
}
    ...
    if (emcmotCommand->commandNum != emcmotStatus->commandNumEcho) {
        ...
        if (emcmotCommand->command == EMCMOT_JOG_CONT ||
            ...
            if (!GET_MOTION_TELEOP_FLAG() &&
                ...
                rtapi_print_msg(
                    RTAPI_MSG_ERR,
                    "Joint jog requested for undefined joint number=%d "
                    "(min=0,max=%d)",
                    joint_num,
                    ALL_JOINTS - 1);
                return;
            ...
            if (GET_MOTION_TELEOP_FLAG()) {
                if ((emcmotCommand->axis >= 0) &&
                    (axis_get_locking_joint(emcmotCommand->axis) >= 0)) {
                    rtapi_print_msg(
                        RTAPI_MSG_ERR,
                        "Cannot jog a locking indexer AXIS_%c,joint_num=%d\n",
                        "XYZABCUVW"[emcmotCommand->axis],
                        axis_get_locking_joint(emcmotCommand->axis));
                    return;
                }
            }
        }
            ...
            if ((emcmotCommand->command == EMCMOT_JOG_CONT ||
                ...
                if (emcmotConfig->kinType == KINEMATICS_IDENTITY) {
                    rtapi_print_msg(
                        ...
                        "because joint (%d) home_sequence is synchronized "
                        "(%d)\n",
                        joint_num,
                        get_home_sequence(joint_num));
                } else {
                    rtapi_print_msg(
                        RTAPI_MSG_ERR,
                        "Cannot jog joint %d because home_sequence is "
                        "synchronized (%d)\n",
                        joint_num,
                        get_home_sequence(joint_num));
                }
        ...
        case EMCMOT_COORD:
	       ...
	       controller cycle */
            ...
            if (emcmotConfig->kinType != KINEMATICS_IDENTITY) {
                if (!get_allhomed()) {
                    reportError(_("all joints must be homed before going into "
                                  "coordinated mode"));
                    emcmotInternal->coordinating = 0;
                    break;
                }
	    ...
	    */
	       ...
	       stop the jog. */
            ...
            if (!GET_MOTION_TELEOP_FLAG()) {
	           ...
	           since homing, otherwise just do this one */
                ...
                axis_jog_cont(
                    emcmotCommand->axis, emcmotCommand->vel, servo_period);
            }
            break;
        ...
        case EMCMOT_JOG_INCR:
            ...
            if (!GET_MOTION_TELEOP_FLAG()) {
	           ...
	           since homing, otherwise just do this one */
                ...
                axis_jog_incr(emcmotCommand->axis,
                              emcmotCommand->offset,
                              emcmotCommand->vel,
                              servo_period);
                for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
        ...
        case EMCMOT_JOG_ABS:
            ...
            if (!GET_MOTION_TELEOP_FLAG()) {
                ...
                // TELEOP JOG_ABS
                axis_jog_abs(emcmotCommand->axis,
                             emcmotCommand->offset,
                             emcmotCommand->vel);
                for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
                    joint = &joints[joint_num];
        ...
        case EMCMOT_SET_TERM_COND:
            ...
            tpSetTermCond(&emcmotInternal->coord_tp,
                          emcmotCommand->termCond,
                          emcmotCommand->tolerance);
            break;

        case EMCMOT_SET_SPINDLESYNC:
            tpSetSpindleSync(&emcmotInternal->coord_tp,
                             emcmotCommand->spindle,
                             emcmotCommand->spindlesync,
                             emcmotCommand->flags);
            break;
        ...
        case EMCMOT_SET_LINE:
            ...
            if (!GET_MOTION_COORD_FLAG() || !GET_MOTION_ENABLE_FLAG()) {
                ...
                break;
                           ...
                           emcmotCommand->pos, emcmotCommand->id, "Linear")) {
            ...
            if (!is_feed_type(emcmotCommand->motion_type) &&
                emcmotStatus->spindle_status[emcmotCommand->spindle]
                    .css_factor) {
            ...
            if (res_addline < 0) {
                reportError(
                    _("can't add linear move at line %d, error code %d"),
                    emcmotCommand->id,
                    res_addline);
        ...
        case EMCMOT_SET_CIRCLE:
            ...
            if (!GET_MOTION_COORD_FLAG() || !GET_MOTION_ENABLE_FLAG()) {
                ...
                break;
                           ...
                           emcmotCommand->pos, emcmotCommand->id, "Circular")) {
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_PARAMS;
                tpAbort(&emcmotInternal->coord_tp);
                SET_MOTION_ERROR_FLAG(1);
                break;
            ...
            /* append it to the emcmotInternal->coord_tp */
            tpSetId(&emcmotInternal->coord_tp, emcmotCommand->id);
            int res_addcircle = tpAddCircle(&emcmotInternal->coord_tp,
                                            emcmotCommand->pos,
                                            emcmotCommand->center,
            ...
            if (res_addcircle < 0) {
                reportError(
                    _("can't add circular move at line %d, error code %d"),
                    emcmotCommand->id,
                    res_addcircle);
                emcmotStatus->commandStatus = EMCMOT_COMMAND_BAD_EXEC;
                tpAbort(&emcmotInternal->coord_tp);
        ...
        case EMCMOT_SET_VEL:
            ...
            tpSetVmax(&emcmotInternal->coord_tp,
                      emcmotStatus->vel,
                      emcmotCommand->ini_maxvel);
            break;
        ...
        case EMCMOT_SET_JERK:
            ...
            /* can do it at any time */
            rtapi_print_msg(
                RTAPI_MSG_DBG, "SET_JERK, jerk(%f)", emcmotCommand->jerk);
        ...
        case EMCMOT_SPINDLE_SCALE:
            ...
            emcmotStatus->spindle_status[emcmotCommand->spindle].scale =
                emcmotCommand->scale;
            break;
        ...
        case EMCMOT_JOINT_HOME:
	       ...
	       homing.c does the rest */
            ...
            if (*(emcmot_hal_data->homing_inhibit)) {
                reportError(
                    _("Homing denied by motion.homing-inhibit joint=%d\n"),
                    joint_num);
                return;
            }
            if (get_homing_is_active()) {
                reportError(
                    "Homing not possible until current homing process is "
                    "finished.\n");
                return;
            }
            if (!GET_MOTION_ENABLE_FLAG()) {
        ...
        case EMCMOT_PROBE:
            ...
            if (!GET_MOTION_COORD_FLAG() || !GET_MOTION_ENABLE_FLAG()) {
                ...
                break;
                           ...
                           emcmotCommand->pos, emcmotCommand->id, "Probe")) {
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_PARAMS;
                tpAbort(&emcmotInternal->coord_tp);
                SET_MOTION_ERROR_FLAG(1);
                break;
                ...
                if (probeval != probe_whenclears) {
                    ...
                    if (probe_whenclears)
                        reportError(
                            _("Probe is already clear when starting G38.4 or "
                              "G38.5 move"));
                    else
                        reportError(
                            _("Probe is already tripped when starting G38.2 or "
                              "G38.3 move"));
        ...
        case EMCMOT_RIGID_TAP:
            ...
            int res_addtap = tpAddRigidTap(&emcmotInternal->coord_tp,
                                           ...
                                           emcmotCommand->tag);
            if (res_addtap < 0) {
                emcmotStatus->atspeed_next_feed =
                    0; /* rigid tap always waits for spindle to be at-speed */
                reportError(
                    _("can't add rigid tap move at line %d, error code %d"),
                    emcmotCommand->id,
                    res_addtap);
                tpAbort(&emcmotInternal->coord_tp);
                SET_MOTION_ERROR_FLAG(1);
        ...
        case EMCMOT_SET_AOUT:
            ...
            if (emcmotCommand->now) { //we set it right away
                ...
                tpSetAout(&emcmotInternal->coord_tp,
                          emcmotCommand->out,
        ...
        case EMCMOT_SET_DOUT:
            ...
            if (emcmotCommand->now) { //we set it right away
                ...
                tpSetDout(&emcmotInternal->coord_tp,
                          emcmotCommand->out,
                          emcmotCommand->start,
                          emcmotCommand->end);
            }
        ...
        case EMCMOT_SET_SPINDLE_PARAMS:
            rtapi_print_msg(RTAPI_MSG_DBG,
                            ...
                            emcmotCommand->spindle,
                            emcmotConfig->numSpindles,
                            emcmotCommand->maxLimit,
                            emcmotCommand->min_pos_speed,
                            emcmotCommand->max_neg_speed,
                            emcmotCommand->minLimit,
                            emcmotCommand->search_vel,
                            emcmotCommand->home,
                            emcmotCommand->home_sequence);
            ...
            emcmotStatus->spindle_status[spindle_num].max_pos_speed =
                emcmotCommand->maxLimit;
            emcmotStatus->spindle_status[spindle_num].min_neg_speed =
                emcmotCommand->minLimit;
            emcmotStatus->spindle_status[spindle_num].max_neg_speed =
                emcmotCommand->max_neg_speed;
            emcmotStatus->spindle_status[spindle_num].min_pos_speed =
                emcmotCommand->min_pos_speed;
            emcmotStatus->spindle_status[spindle_num].home_search_vel =
                emcmotCommand->search_vel;
            emcmotStatus->spindle_status[spindle_num].home_sequence =
                emcmotCommand->home_sequence;
            emcmotStatus->spindle_status[spindle_num].increment =
                emcmotCommand->offset;
        ...
        case EMCMOT_SPINDLE_ON:
            ...
            emcmotStatus->atspeed_next_feed =
                emcmotCommand->wait_for_spindle_at_speed;

            // check whether it's passed correctly
            if (!emcmotStatus->atspeed_next_feed) {
                rtapi_print_msg(RTAPI_MSG_DBG,
                                "SPINDLE_ON without wait-for-atspeed");
            }
            break;
        ...
        case EMCMOT_SPINDLE_OFF:
            ...
            spindle_num = emcmotCommand->spindle;
            if (spindle_num >= emcmotConfig->numSpindles) {
                reportError(_("Attempt to stop non-existent spindle <%d>"),
                            spindle_num);
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_COMMAND;
                break;
            }
            ...
            for (n = s0; n <= s1; n++) {
                ...
                if (*(emcmot_hal_data->spindle[n].spindle_orient))
                    rtapi_print_msg(RTAPI_MSG_DBG,
                                    "SPINDLE_ORIENT cancelled by SPINDLE_OFF");
                if (*(emcmot_hal_data->spindle[n].spindle_locked)) {
                    rtapi_print_msg(RTAPI_MSG_DBG,
                                    "spindle-locked cleared by SPINDLE_OFF");
                    *(emcmot_hal_data->spindle[n].spindle_locked) = 0;
                }
                *(emcmot_hal_data->spindle[n].spindle_orient) = 0;
                emcmotStatus->spindle_status[n].orient_state =
        ...
        case EMCMOT_SPINDLE_ORIENT:
            ...
            spindle_num = emcmotCommand->spindle;
            if (spindle_num >= emcmotConfig->numSpindles) {
                reportError(_("Attempt to orient non-existent spindle <%d>"),
                            spindle_num);
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_COMMAND;
                break;
            }
            ...
            for (n = s0; n <= s1; n++) {
                ...
                if (n > emcmotConfig->numSpindles) {
                    rtapi_print_msg(RTAPI_MSG_ERR,
                                    "spindle number <%d> too high in M19",
                                    n);
                    break;
                }
                if (*(emcmot_hal_data->spindle[n].spindle_orient)) {
                    rtapi_print_msg(RTAPI_MSG_DBG,
                                    "orient already in progress");
                ...
                *(emcmot_hal_data->spindle[n].spindle_orient_angle) =
                    emcmotCommand->orientation;
                *(emcmot_hal_data->spindle[n].spindle_orient_mode) =
                    emcmotCommand->mode;
                *(emcmot_hal_data->spindle[n].spindle_locked) = 0;
                *(emcmot_hal_data->spindle[n].spindle_orient) = 1;

                // mirror in spindle status
                emcmotStatus->spindle_status[n].orient_fault =
                    0; // this pin read during spindle-orient == 1
        ...
        case EMCMOT_SPINDLE_INCREASE:
            ...
            spindle_num = emcmotCommand->spindle;
            if (spindle_num >= emcmotConfig->numSpindles) {
                reportError(_("Attempt to increase non-existent spindle <%d>"),
                            spindle_num);
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_COMMAND;
                break;
            }
            ...
            for (n = s0; n <= s1; n++) {
                if (emcmotStatus->spindle_status[n].speed > 0) {
                    emcmotStatus->spindle_status[n].speed +=
                        emcmotStatus->spindle_status[n].increment;
        ...
        case EMCMOT_SPINDLE_DECREASE:
            ...
            spindle_num = emcmotCommand->spindle;
            if (spindle_num >= emcmotConfig->numSpindles) {
                reportError(_("Attempt to decrease non-existent spindle <%d>."),
                            spindle_num);
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_COMMAND;
                break;
            }
            ...
            for (n = s0; n <= s1; n++) {
                if (emcmotStatus->spindle_status[n].speed >
                    emcmotStatus->spindle_status[n].increment) {
                    emcmotStatus->spindle_status[n].speed -=
                        emcmotStatus->spindle_status[n].increment;
                } else if (emcmotStatus->spindle_status[n].speed <
                           -1.0 * emcmotStatus->spindle_status[n].increment) {
                    emcmotStatus->spindle_status[n].speed +=
                        emcmotStatus->spindle_status[n].increment;
                }
                apply_spindle_limits(&emcmotStatus->spindle_status[n]);
        ...
        case EMCMOT_SPINDLE_BRAKE_ENGAGE:
            ...
            if (spindle_num >= emcmotConfig->numSpindles) {
                reportError(
                    _("Attempt to engage brake of non-existent spindle <%d>"),
                    spindle_num);
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_COMMAND;
                break;
            }
        ...
        case EMCMOT_SPINDLE_BRAKE_RELEASE:
            ...
            if (spindle_num >= emcmotConfig->numSpindles) {
                reportError(
                    _("Attempt to release brake of non-existent spindle <%d>"),
                    spindle_num);
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_COMMAND;
                break;
            }
        ...
        case EMCMOT_SET_JOINT_COMP:
            rtapi_print_msg(
                RTAPI_MSG_DBG, "SET_JOINT_COMP for joint %d", joint_num);
            if (joint == 0) {
                break;
            }
        ...
        case EMCMOT_SET_AXIS_POSITION_LIMITS:
            ...
            axis_set_min_pos_limit(emcmotCommand->axis,
                                   emcmotCommand->minLimit);
            axis_set_max_pos_limit(emcmotCommand->axis,
                                   emcmotCommand->maxLimit);
            break;
        ...
        case EMCMOT_SET_AXIS_VEL_LIMIT:
            ...
            axis_set_ext_offset_vel_limit(emcmotCommand->axis,
                                          emcmotCommand->ext_offset_vel);
            break;
        ...
        case EMCMOT_SET_AXIS_ACC_LIMIT:
            ...
            axis_set_ext_offset_acc_limit(emcmotCommand->axis,
                                          emcmotCommand->ext_offset_acc);
            break;
        ...
        case EMCMOT_SETUP_ARC_BLENDS:
            ...
            emcmotConfig->arcBlendFallbackEnable =
                emcmotCommand->arcBlendFallbackEnable;
            ...
            emcmotConfig->arcBlendTangentKinkRatio =
                emcmotCommand->arcBlendTangentKinkRatio;
            break;
        case EMCMOT_SET_PROBE_ERR_INHIBIT:
            emcmotConfig->inhibit_probe_jog_error =
                emcmotCommand->probe_jog_err_inhibit;
            emcmotConfig->inhibit_probe_home_error =
                emcmotCommand->probe_home_err_inhibit;
            break;

        } /* end of: command switch */
        if (emcmotStatus->commandStatus != EMCMOT_COMMAND_OK) {
            rtapi_print_msg(
                RTAPI_MSG_DBG, "ERROR: %d", emcmotStatus->commandStatus);
        }
        rtapi_print_msg(RTAPI_MSG_DBG, "\n");
        /* synch tail count */
