...
/* printing of commands for troubleshooting */
	...
	case EMCMOT_AXIS_ABORT: //FIXME-AJ: rename
		...
		/* tell joint planner to stop */
		simple_tp_jog_abort(&(joint->free_tp));
		/* stop homing if in progress */
		if ( joint->home_state != HOME_IDLE ) {
		    joint->home_state = HOME_ABORT;
		}
...
//		joint->free_tp.pos_cmd = joint->max_jog_limit;
                simple_tp_jog_cont(&(joint->free_tp), joint->max_pos_limit, fabs(emcmotCommand->vel), joint->vel_limit, joint->acc_limit, 5000);
	    } else {
//		joint->free_tp.pos_cmd = joint->min_jog_limit;
                simple_tp_jog_cont(&(joint->free_tp), joint->min_pos_limit, fabs(emcmotCommand->vel), joint->vel_limit, joint->acc_limit, 5000);
	    }
	    /* set velocity of jog */
//	    joint->free_tp.vel_cmd = fabs(emcmotCommand->vel);
	    /* use max joint accel */
//	    joint->free_tp.acc_cmd = joint->acc_limit;
...
/*! \todo Another #if 0 */
	...
	case EMCMOT_PROBE:
	    ...
	    if (-1 == tpAddLine(&emcmotDebug->queue, emcmotCommand->pos, emcmotCommand->motion_type, emcmotCommand->vel,
            emcmotCommand->ini_maxvel, emcmotCommand->acc, emcmotCommand->jerk, emcmotStatus->enables_new, 0, -1)) {
	...
	case EMCMOT_RIGID_TAP:
	    ...
	    /* append it to the emcmotDebug->queue */
	    tpSetId(&emcmotDebug->queue, emcmotCommand->id);
	    if (-1 == tpAddRigidTap(&emcmotDebug->queue, emcmotCommand->pos, emcmotCommand->vel, emcmotCommand->ini_maxvel,
