...
/* printing of commands for troubleshooting */
	...
	case EMCMOT_TELEOP:
	    ...
	    if (emcmotConfig->kinType != KINEMATICS_IDENTITY) {
		if (!checkAllHomed()) {
		    reportError("all joints must be homed before going into teleop mode");
		    emcmotDebug->teleoperating = 0;
		    break;
		}
	    }
...
/*! \todo Another #if 0 */
	...
	case EMCMOT_JOINT_UNHOME:
            ...
            if (joint_num < 0) {
                ...
                reportError("Cannot unhome invalid joint %d (max %d)", joint_num, (emcmotConfig->numJoints-1));
                return;
            }
	...
	case EMCMOT_PROBE:
	    ...
	    /* append it to the emcmotDebug->coord_tp */
	    tpSetId(&emcmotDebug->coord_tp, emcmotCommand->id);
	    if (-1 == tpAddLine(&emcmotDebug->coord_tp, emcmotCommand->pos, emcmotCommand->motion_type, emcmotCommand->vel, emcmotCommand->ini_maxvel, emcmotCommand->acc, emcmotStatus->enables_new, 0)) {
	...
	case EMCMOT_RIGID_TAP:
	    ...
	    if (!GET_MOTION_COORD_FLAG() || !GET_MOTION_ENABLE_FLAG()) {
		reportError("need to be enabled, in coord mode for rigid tap move");
		emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_COMMAND;
		SET_MOTION_ERROR_FLAG(1);
		break;
