...
static int num_joints =
    EMCMOT_MAX_JOINTS; /* default number of joints present */
...
RTAPI_MP_INT(num_extrajoints,
             "number of extra joints (not used in kinematics)");
...
RTAPI_MP_ARRAY_STRING(names_misc_errors,
...
static int tp_init()
{
    if (-1 == tpCreate(&emcmotInternal->coord_tp,
                       DEFAULT_TC_QUEUE_SIZE,
                       mot_comp_id)) {
...
int rtapi_app_main(void)
{
    ...
    if ((num_extrajoints > 0) && (kinematicsType() != KINEMATICS_BOTH)) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        _("\nMOTION: nonzero num_extrajoints requires "
                          "KINEMATICS_BOTH\n\n"));
        return -1;
    }
    ...
    if (num_dio != NOT_INITIALIZED && (names_dout[0] || names_din[0])) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        _("MOTION: Can't specify both names and number for "
                          "digital pins\n"));
        return -1;
    }
    ...
    if (num_aio != NOT_INITIALIZED && (names_aout[0] || names_ain[0])) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            _("MOTION: Can't specify both names and number for analog pins\n"));
        return -1;
    }
    ...
    if (num_misc_error != NOT_INITIALIZED && (names_misc_errors[0])) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            _("MOTION: Can't specify both names and number for misc error\n"));
        return -1;
    }
...
static int init_hal_io(void)
{
    ...
    CALL_CHECK(hal_pin_bit_newf(HAL_IN,
                                &(emcmot_hal_data->probe_input),
                                mot_comp_id,
                                "motion.probe-input"));
    CALL_CHECK(hal_pin_float_newf(HAL_IN,
                                  &(emcmot_hal_data->adaptive_feed),
                                  mot_comp_id,
                                  "motion.adaptive-feed"));
    CALL_CHECK(hal_pin_bit_newf(HAL_IN,
                                &(emcmot_hal_data->feed_hold),
                                mot_comp_id,
                                "motion.feed-hold"));
    CALL_CHECK(hal_pin_bit_newf(HAL_IN,
                                &(emcmot_hal_data->feed_inhibit),
                                mot_comp_id,
                                "motion.feed-inhibit"));
    CALL_CHECK(hal_pin_bit_newf(HAL_IN,
    ...
    CALL_CHECK(hal_pin_bit_newf(HAL_IN,
                                &(emcmot_hal_data->jog_inhibit),
                                mot_comp_id,
                                "motion.jog-inhibit"));
    CALL_CHECK(hal_pin_bit_newf(
        HAL_IN, &(emcmot_hal_data->jog_stop), mot_comp_id, "motion.jog-stop"));
    CALL_CHECK(hal_pin_bit_newf(HAL_IN,
                                &(emcmot_hal_data->jog_stop_immediate),
                                mot_comp_id,
                                "motion.jog-stop-immediate"));
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                &(emcmot_hal_data->tp_reverse),
                                mot_comp_id,
                                "motion.tp-reverse"));
    CALL_CHECK(hal_pin_bit_newf(
        HAL_IN, &(emcmot_hal_data->enable), mot_comp_id, "motion.enable"));
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                &(emcmot_hal_data->is_all_homed),
                                mot_comp_id,
                                "motion.is-all-homed"));

    /* state tags pins */
    CALL_CHECK(hal_pin_float_newf(
        HAL_OUT, &(emcmot_hal_data->feed_upm), mot_comp_id, "motion.feed-upm"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->feed_inches_per_minute),
                                  mot_comp_id,
                                  "motion.feed-inches-per-minute"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->feed_inches_per_second),
                                  mot_comp_id,
                                  "motion.feed-inches-per-second"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->feed_mm_per_minute),
                                  mot_comp_id,
                                  "motion.feed-mm-per-minute"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->feed_mm_per_second),
                                  mot_comp_id,
    ...
    for (n = 0; n < num_dio; n++) {
        if (n < num_din && names_din[n]) {
            ...
            CALL_CHECK(hal_pin_bit_newf(HAL_IN,
                                        &(emcmot_hal_data->synch_di[n]),
                                        mot_comp_id,
                                        "motion.digital-in-%02d",
                                        in++));
        }
        if (n < num_dout && names_dout[n]) {
            CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                        &(emcmot_hal_data->synch_do[n]),
                                        mot_comp_id,
                                        "motion.dout-%s",
                                        names_dout[n]));
        } else {
    ...
    for (n = 0; n < num_aio; n++) {
        if (n < num_ain && names_ain[n]) {
            CALL_CHECK(hal_pin_float_newf(HAL_IN,
                                          &(emcmot_hal_data->analog_input[n]),
                                          mot_comp_id,
                                          "motion.ain-%s",
                                          names_ain[n]));
        } else {
            CALL_CHECK(hal_pin_float_newf(HAL_IN,
                                          &(emcmot_hal_data->analog_input[n]),
                                          mot_comp_id,
                                          "motion.analog-in-%02d",
                                          in++));
        }
        if (n < num_aout && names_aout[n]) {
            CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                          &(emcmot_hal_data->analog_output[n]),
                                          mot_comp_id,
                                          "motion.aout-%s",
                                          names_aout[n]));
        } else {
    ...
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                &(emcmot_hal_data->in_position),
                                mot_comp_id,
                                "motion.in-position"));
    CALL_CHECK(hal_pin_s32_newf(HAL_OUT,
                                &(emcmot_hal_data->motion_type),
                                mot_comp_id,
                                "motion.motion-type"));
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                ...
                                "motion.coord-mode"));
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                &(emcmot_hal_data->teleop_mode),
                                mot_comp_id,
                                "motion.teleop-mode"));
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                &(emcmot_hal_data->coord_error),
                                mot_comp_id,
                                "motion.coord-error"));
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                &(emcmot_hal_data->on_soft_limit),
                                mot_comp_id,
    ...
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  ...
                                  "motion.current-vel"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->requested_vel),
                                  mot_comp_id,
                                  "motion.requested-vel"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->distance_to_go),
                                  mot_comp_id,
                                  "motion.distance-to-go"));
    CALL_CHECK(hal_pin_s32_newf(HAL_OUT,
                                &(emcmot_hal_data->program_line),
                                mot_comp_id,
                                "motion.program-line"));
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                &(emcmot_hal_data->jog_is_active),
                                mot_comp_id,
    ...
    CALL_CHECK(hal_param_bit_newf(HAL_RO,
                                  &(emcmot_hal_data->debug_bit_0),
                                  mot_comp_id,
                                  "motion.debug-bit-0"));
    CALL_CHECK(hal_param_bit_newf(HAL_RO,
                                  &(emcmot_hal_data->debug_bit_1),
                                  mot_comp_id,
                                  "motion.debug-bit-1"));
    CALL_CHECK(hal_param_float_newf(HAL_RO,
                                    &(emcmot_hal_data->debug_float_0),
                                    mot_comp_id,
                                    "motion.debug-float-0"));
    CALL_CHECK(hal_param_float_newf(HAL_RO,
                                    &(emcmot_hal_data->debug_float_1),
                                    mot_comp_id,
                                    "motion.debug-float-1"));
    CALL_CHECK(hal_param_float_newf(HAL_RO,
                                    &(emcmot_hal_data->debug_float_2),
                                    mot_comp_id,
                                    "motion.debug-float-2"));
    CALL_CHECK(hal_param_float_newf(HAL_RO,
                                    &(emcmot_hal_data->debug_float_3),
                                    mot_comp_id,
                                    "motion.debug-float-3"));
    CALL_CHECK(hal_param_s32_newf(HAL_RO,
                                  &(emcmot_hal_data->debug_s32_0),
                                  mot_comp_id,
                                  "motion.debug-s32-0"));
    CALL_CHECK(hal_param_s32_newf(HAL_RO,
                                  &(emcmot_hal_data->debug_s32_1),
                                  mot_comp_id,
                                  "motion.debug-s32-1"));
    ...
    CALL_CHECK(hal_param_float_newf(
        HAL_RO, &(emcmot_hal_data->traj_pos_out), mot_comp_id, "traj.pos_out"));
    CALL_CHECK(hal_param_float_newf(
        HAL_RO, &(emcmot_hal_data->traj_vel_out), mot_comp_id, "traj.vel_out"));
    CALL_CHECK(hal_param_u32_newf(HAL_RO,
                                  &(emcmot_hal_data->traj_active_tc),
                                  mot_comp_id,
                                  "traj.active_tc"));

    for (n = 0; n < 4; n++) {
        CALL_CHECK(hal_param_float_newf(HAL_RO,
                                        &(emcmot_hal_data->tc_pos[n]),
                                        mot_comp_id,
                                        "tc.%d.pos",
                                        n));
        CALL_CHECK(hal_param_float_newf(HAL_RO,
                                        &(emcmot_hal_data->tc_vel[n]),
                                        mot_comp_id,
                                        "tc.%d.vel",
                                        n));
        CALL_CHECK(hal_param_float_newf(HAL_RO,
                                        &(emcmot_hal_data->tc_acc[n]),
                                        mot_comp_id,
                                        "tc.%d.acc",
                                        n));
    }
    ...
    CALL_CHECK(hal_pin_u32_newf(HAL_OUT,
                                &(emcmot_hal_data->last_period),
                                mot_comp_id,
                                "motion.servo.last-period"));
#ifdef HAVE_CPU_KHZ
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->last_period_ns),
                                  mot_comp_id,
    ...
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_x),
                                  mot_comp_id,
                                  "motion.tooloffset.x"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_y),
                                  mot_comp_id,
                                  "motion.tooloffset.y"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_z),
                                  mot_comp_id,
                                  "motion.tooloffset.z"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_a),
                                  mot_comp_id,
                                  "motion.tooloffset.a"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_b),
                                  mot_comp_id,
                                  "motion.tooloffset.b"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_c),
                                  mot_comp_id,
                                  "motion.tooloffset.c"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_u),
                                  mot_comp_id,
                                  "motion.tooloffset.u"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_v),
                                  mot_comp_id,
                                  "motion.tooloffset.v"));
    CALL_CHECK(hal_pin_float_newf(HAL_OUT,
                                  &(emcmot_hal_data->tooloffset_w),
                                  mot_comp_id,
                                  "motion.tooloffset.w"));
    ...
    *(emcmot_hal_data->last_period) = 0;
    ...
    for (n = 0; n < num_spindles; n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, _("MOTION: spindle %d pin export failed"), n);
            return -1;
        }
    }
    ...
    for (n = 0; n < num_extrajoints; n++) {
        ...
        retval =
            export_extrajoint(n + num_joints - num_extrajoints, ejoint_data);
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            _("MOTION: ejoint %d pin/param export failed\n"),
                            n);
            return -1;
        }
    }
    ...
    CALL_CHECK(hal_pin_bit_newf(HAL_OUT,
                                &(emcmot_hal_data->eoffset_active),
                                mot_comp_id,
                                "motion.eoffset-active"));

    /* Done! */
    rtapi_print_msg(
        RTAPI_MSG_INFO, "MOTION: init_hal_io() complete, %d axes.\n", n);
    return 0;
}
...
static int export_spindle(int num, spindle_hal_t *addr)
{
    ...
    if ((retval = hal_pin_bit_newf(HAL_IO,
                                   &(addr->spindle_index_enable),
                                   mot_comp_id,
                                   "spindle.%d.index-enable",
                                   num)) != 0)
        return retval;

    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->spindle_on),
                                   mot_comp_id,
                                   "spindle.%d.on",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->spindle_forward),
                                   mot_comp_id,
                                   "spindle.%d.forward",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->spindle_reverse),
                                   mot_comp_id,
                                   "spindle.%d.reverse",
                                   num)) != 0)
        return retval;
    ...
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->spindle_speed_out),
                                     mot_comp_id,
                                     "spindle.%d.speed-out",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->spindle_speed_out_abs),
                                     mot_comp_id,
                                     "spindle.%d.speed-out-abs",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->spindle_speed_out_rps),
                                     mot_comp_id,
                                     "spindle.%d.speed-out-rps",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->spindle_speed_out_rps_abs),
                                     mot_comp_id,
                                     "spindle.%d.speed-out-rps-abs",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->spindle_speed_cmd_rps),
                                     mot_comp_id,
                                     "spindle.%d.speed-cmd-rps",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_IN,
                                   &(addr->spindle_inhibit),
                                   mot_comp_id,
                                   "spindle.%d.inhibit",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_IN,
                                   &(addr->spindle_amp_fault),
                                   mot_comp_id,
                                   "spindle.%d.amp-fault-in",
                                   num)) != 0)
        return retval;
    *(addr->spindle_inhibit) = 0;
    ...
    // spindle orient pins
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->spindle_orient_angle),
                                     mot_comp_id,
                                     "spindle.%d.orient-angle",
                                     num)) < 0)
        return retval;
    if ((retval = hal_pin_s32_newf(HAL_OUT,
                                   &(addr->spindle_orient_mode),
                                   mot_comp_id,
                                   "spindle.%d.orient-mode",
                                   num)) < 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->spindle_orient),
    ...
    if ((retval = hal_pin_bit_newf(HAL_OUT,
        ...
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_IN,
                                   &(addr->spindle_is_oriented),
                                   mot_comp_id,
                                   "spindle.%d.is-oriented",
                                   num)) < 0)
        return retval;
    if ((retval = hal_pin_s32_newf(HAL_IN,
                                   &(addr->spindle_orient_fault),
                                   mot_comp_id,
                                   "spindle.%d.orient-fault",
                                   num)) < 0)
        return retval;
    ...
    *(addr->spindle_orient) = 0;
    ...
    if ((retval = hal_pin_float_newf(HAL_IN,
        ...
        return retval;
    if ((retval = hal_pin_float_newf(HAL_IN,
                                     &(addr->spindle_speed_in),
                                     mot_comp_id,
                                     "spindle.%d.speed-in",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_IN,
                                   &(addr->spindle_is_atspeed),
                                   mot_comp_id,
                                   "spindle.%d.at-speed",
                                   num)) != 0)
        return retval;
...
static int export_joint(int num, joint_hal_t *addr)
{
    ...
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->motor_pos_cmd),
                                     mot_comp_id,
                                     "joint.%d.motor-pos-cmd",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_IN,
                                     &(addr->motor_pos_fb),
                                     mot_comp_id,
                                     "joint.%d.motor-pos-fb",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->motor_offset),
                                     mot_comp_id,
                                     "joint.%d.motor-offset",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_IN,
                                   &(addr->pos_lim_sw),
                                   mot_comp_id,
                                   "joint.%d.pos-lim-sw-in",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_IN,
                                   &(addr->neg_lim_sw),
                                   mot_comp_id,
                                   "joint.%d.neg-lim-sw-in",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->amp_enable),
                                   mot_comp_id,
                                   "joint.%d.amp-enable-out",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_IN,
                                   &(addr->amp_fault),
                                   mot_comp_id,
                                   "joint.%d.amp-fault-in",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_s32_newf(HAL_IN,
                                   &(addr->jjog_counts),
    ...
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->backlash_corr),
                                     mot_comp_id,
                                     "joint.%d.backlash-corr",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->backlash_filt),
                                     mot_comp_id,
                                     "joint.%d.backlash-filt",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->backlash_vel),
                                     mot_comp_id,
                                     "joint.%d.backlash-vel",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->f_error),
    ...
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->free_pos_cmd),
                                     mot_comp_id,
                                     "joint.%d.free-pos-cmd",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_float_newf(HAL_OUT,
                                     &(addr->free_vel_lim),
                                     mot_comp_id,
                                     "joint.%d.free-vel-lim",
                                     num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->free_tp_enable),
                                   mot_comp_id,
                                   "joint.%d.free-tp-enable",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->kb_jjog_active),
                                   mot_comp_id,
                                   "joint.%d.kb-jog-active",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->wheel_jjog_active),
                                   mot_comp_id,
                                   "joint.%d.wheel-jog-active",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->in_position),
    ...
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->phl),
                                   mot_comp_id,
                                   "joint.%d.pos-hard-limit",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->nhl),
                                   mot_comp_id,
                                   "joint.%d.neg-hard-limit",
                                   num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(
             HAL_OUT, &(addr->active), mot_comp_id, "joint.%d.active", num)) !=
        0)
        return retval;
    if ((retval = hal_pin_bit_newf(
             HAL_OUT, &(addr->error), mot_comp_id, "joint.%d.error", num)) != 0)
        return retval;
    if ((retval = hal_pin_bit_newf(HAL_OUT,
                                   &(addr->f_errored),
                                   mot_comp_id,
    ...
    if ((retval = hal_pin_bit_newf(HAL_OUT,
        ...
        return retval;
    if ((retval = hal_pin_float_newf(HAL_IN,
                                     &(addr->jjog_accel_fraction),
                                     mot_comp_id,
                                     "joint.%d.jog-accel-fraction",
                                     num)) != 0)
        return retval;
    *addr->jjog_accel_fraction = 1.0; // fraction of accel for wheel jjogs
    ...
    if (joint_is_lockable(num)) {
        ...
        rtapi_print_msg(RTAPI_MSG_WARN,
                        "motion.c: Creating unlock hal pins for joint %d\n",
                        num);
        if ((retval = hal_pin_bit_newf(HAL_OUT,
                                       &(addr->unlock),
                                       mot_comp_id,
...
static int init_comm_buffers(void)
{
    ...
    emcmotConfig->numExtraJoints =
        num_extrajoints; // from motmod num_extrajoints=
...
static int init_threads(void)
{
    ...
    retval = hal_export_funct("motion-controller",
                              emcmotController,
                              0 /* arg
	 */
                              ,
    ...
    retval = hal_export_funct("motion-command-handler",
