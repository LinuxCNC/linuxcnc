/*
   ...
   Analysis taken in part from Curtis S. Wilson,
   "How Close Do You Have to Specify Points In a Contouring Application?",
   Delta Tau Data Systems (unpublished).
   ...
   cubicAddPoint
   22-Oct-1998  FMP added main() section; loaded cubic completely with
   first point; removed '_' from _filled and _needNextPoint
                              ...
                              double deltaT)
{
  ...
  /* now the hard ones */
  retval.b = 3 * (xn - x0) / (deltaT*deltaT) -
    (2 * v0 + vn) / deltaT;
...
int cubicOffset(CUBIC_STRUCT *ci, double offset)
{
  ...
  /* leave velp0, velp1 alone, since these are velocity points and
     are unaffected by position offsets */
...
int main(int argc, char *argv[])
{
  ...
  if (1 != sscanf(argv[1], "%lf", &segmentTime) ||
      segmentTime <= 0.0)
    {
