...
void emcmotController(void *arg, long period)
{
    ...
    *(emcmot_hal_data->last_period_ns) = this_run * 1e6 / cpu_khz;
    ...
    if (!emcmotStatus->on_soft_limit &&
        !*emcmot_hal_data
              ->jog_inhibit) { // change from teleop to move off joint soft limit
    ...
    get_pos_cmds(period);
    compute_screw_comp();
    *(emcmot_hal_data->eoffset_active) = axis_plan_external_offsets(
        servo_period, GET_MOTION_ENABLE_FLAG(), get_allhomed());
    output_to_hal();
...
static bool joint_jog_is_active(void)
{
    ...
    for (jno = 0; jno < EMCMOT_MAX_JOINTS; jno++) {
        if ((&joints[jno])->kb_jjog_active ||
            (&joints[jno])->wheel_jjog_active) {
            return 1;
        }
    }
    return 0;
...
static void handle_kinematicsSwitch(void)
{
    ...
    for (anum = 0; anum < EMCMOT_MAX_AXIS; anum++) {
        fprintf(stderr,
                "anum=%d before:%8.3g after:%8.3g delta=%8.3g\n",
                anum,
                beforePose[anum],
                *pcmd_p[anum],
                *pcmd_p[anum] - beforePose[anum]);
    }
#endif
...
static void process_inputs(void)
{
    ...
    for (spindle_num = 0; spindle_num < emcmotConfig->numSpindles;
         spindle_num++) {
    ...
    if (enables & AF_ENABLED) {
        ...
        if (adaptive_feed_in < 0.0 &&
            emcmotInternal->coord_tp.reverse_run == TC_DIR_FORWARD) {
            // User commands feed in reverse direction, but we're not running in reverse yet
            if (tpSetRunDir(&emcmotInternal->coord_tp, TC_DIR_REVERSE) !=
                TP_ERR_OK) {
                // Need to decelerate to a stop first
                adaptive_feed_out = 0.0;
            }
        } else if (adaptive_feed_in > 0.0 &&
                   emcmotInternal->coord_tp.reverse_run == TC_DIR_REVERSE) {
            // User commands feed in forward direction, but we're running in reverse
            if (tpSetRunDir(&emcmotInternal->coord_tp, TC_DIR_FORWARD) !=
                TP_ERR_OK) {
                // Need to decelerate to a stop first
                adaptive_feed_out = 0.0;
            }
    ...
    for (spindle_num = 0; spindle_num < emcmotConfig->numSpindles;
        ...
        if (*(emcmot_hal_data->spindle[spindle_num].spindle_orient)) {
            if (*(emcmot_hal_data->spindle[spindle_num].spindle_orient_fault)) {
                emcmotStatus->spindle_status[spindle_num].orient_state =
                ...
                *(emcmot_hal_data->spindle[spindle_num].spindle_orient) = 0;
                ...
                reportError(
                    _("fault %d during orient in progress"),
                    emcmotStatus->spindle_status[spindle_num].orient_fault);
                emcmotStatus->commandStatus = EMCMOT_COMMAND_INVALID_COMMAND;
                tpAbort(&emcmotInternal->coord_tp);
                SET_MOTION_ERROR_FLAG(1);
            } else if (*(emcmot_hal_data->spindle[spindle_num]
                             .spindle_is_oriented)) {
                *(emcmot_hal_data->spindle[spindle_num].spindle_orient) = 0;
    ...
    if (enables & *(emcmot_hal_data->jog_is_active) &&
        (*(emcmot_hal_data->jog_stop) ||
...
*/
   ...
   at the traj rate */
    ...
    case KINEMATICS_IDENTITY:
        kinematicsForward(
            joint_pos, &emcmotStatus->carte_pos_fb, &fflags, &iflags);
        if (get_allhomed()) {
    ...
    default: emcmotStatus->carte_pos_fb_ok = 0; break;
    }
}
...
static void process_probe_inputs(void)
{
    ...
    if (emcmotStatus->probing) {
        ...
        if (!GET_MOTION_INPOS_FLAG() &&
            tpQueueDepth(&emcmotInternal->coord_tp)) {
            // running an command
...
static void check_for_faults(void)
{
    ...
    for (spindle_num = 0; spindle_num < emcmotConfig->numSpindles;
         spindle_num++) {
        if (emcmotStatus->spindle_status[spindle_num].fault &&
            GET_MOTION_ENABLE_FLAG()) {
...
static void set_operating_mode(void)
{
    ...
    if (emcmotInternal->teleoperating && !GET_MOTION_TELEOP_FLAG()) {
        if (GET_MOTION_INPOS_FLAG()) {
            ...
            kinematicsForward(
                positions, &emcmotStatus->carte_pos_cmd, &fflags, &iflags);
        ...
        if (emcmotInternal->coordinating && !GET_MOTION_COORD_FLAG()) {
            if (GET_MOTION_INPOS_FLAG()) {
                ...
                tpSetPos(&emcmotInternal->coord_tp,
                         &emcmotStatus->carte_pos_cmd);
                /* drain the cubics so they'll synch up */
                for (joint_num = 0; joint_num < NO_OF_KINS_JOINTS;
                     joint_num++) {
...
static void handle_jjogwheels(void)
{
    ...
    for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
        ...
        if ((*(joint_data->jjog_accel_fraction) > 1) ||
            ...
            jaccel_limit =
                (*(joint_data->jjog_accel_fraction)) * joint->acc_limit;
        }
        ...
        if (get_home_is_synchronized(joint_num)) {
            if (emcmotConfig->kinType == KINEMATICS_IDENTITY) {
                ...
                rtapi_print_msg(
                    RTAPI_MSG_ERR,
                    "Cannot wheel jog joint %d because home_sequence "
                    "synchronized (%d)\n",
                    joint_num,
                    get_home_sequence(joint_num));
            }
...
static void get_pos_cmds(long period)
{
    ...
    case EMCMOT_MOTION_FREE:
	   ...
	   is complete, and the limits should be re-enabled */
        ...
        case KINEMATICS_INVERSE_ONLY: emcmotStatus->carte_pos_cmd_ok = 0; break;

        default: emcmotStatus->carte_pos_cmd_ok = 0; break;
        }
        /* end of FREE mode */
        break;
    ...
    case EMCMOT_MOTION_COORD:
        ...
        while (cubicNeedNextPoint(&(joints[0].cubic))) {
            ...
            result = kinematicsInverse(
                &emcmotStatus->carte_pos_cmd, positions, &iflags, &fflags);
            if (result == 0) {
                /* copy to joint structures and spline them up */
                for (joint_num = 0; joint_num < NO_OF_KINS_JOINTS;
                     joint_num++) {
                    if (!isfinite(positions[joint_num])) {
                        reportError(_("kinematicsInverse gave non-finite joint "
                                      "location on joint %d"),
                                    joint_num);
                        SET_MOTION_ERROR_FLAG(1);
                        emcmotInternal->enabling = 0;
        ...
        for (joint_num = 0; joint_num < NO_OF_KINS_JOINTS; joint_num++) {
            ...
            joint->pos_cmd = cubicInterpolate(&(joint->cubic),
                                              0,
                                              &(joint->vel_cmd),
                                              &(joint->acc_cmd),
                                              &(joint->jerk_cmd));
    ...
    case EMCMOT_MOTION_TELEOP:
        ...
        if (result == 0) {
            for (joint_num = 0; joint_num < NO_OF_KINS_JOINTS; joint_num++) {
                if (!isfinite(positions[joint_num])) {
                    reportError(
                        _("kinematicsInverse gave non-finite joint location on "
                          "joint %d"),
                        joint_num);
                    SET_MOTION_ERROR_FLAG(1);
                    emcmotInternal->enabling = 0;
    ...
    */
    ...
    if (onlimit) {
        ...
        **
        */
            for (joint_num = 0; joint_num < emcmotConfig->numJoints;
                 joint_num++) {
                if (joint_limit[joint_num][0] == 1) {
                    joint = &joints[joint_num];
                    reportError(
                        _("Exceeded NEGATIVE soft limit (%.5f) on joint %d\n"),
                        joint->min_pos_limit,
                        joint_num);
                    if (emcmotConfig->kinType == KINEMATICS_IDENTITY) {
                        reportError(_("Joint must be unhomed, jogged into "
                                      "limits, rehomed"));
                    } else {
                        reportError(_("Hint: switch to joint mode to jog off "
                                      "soft limit"));
                    }
                } else if (joint_limit[joint_num][1] == 1) {
                    joint = &joints[joint_num];
                    reportError(
                        _("Exceeded POSITIVE soft limit (%.5f) on joint %d\n"),
                        joint->max_pos_limit,
                        joint_num);
                    if (emcmotConfig->kinType == KINEMATICS_IDENTITY) {
                        reportError(_("Joint must be unhomed, jogged into "
    ...
    for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
        ...
        if (comp->entries > 0) {
            ...
            if (joint->vel_cmd > 0.0) {
                ...
                joint->backlash_corr =
                    comp->entry->fwd_trim + comp->entry->fwd_slope * dpos;
            } else if (joint->vel_cmd < 0.0) {
                /* moving "down". apply reverse screw comp */
                joint->backlash_corr =
                    comp->entry->rev_trim + comp->entry->rev_slope * dpos;
            } else {
	 ...
	 */
        ...
        if (joint->backlash_corr >= joint->backlash_filt) {
            ...
            if (s_to_go > 0) {
                ...
                ds_stop =
                    0.5 * (v + dv_acc) * (v + dv_acc) / a_max; /* abs val */
...
static void output_to_hal(void)
{
    ...
    *(emcmot_hal_data->on_soft_limit) = emcmotStatus->on_soft_limit;
    ...
    case EMC_MOTION_TYPE_ARC:
        ...
        *(emcmot_hal_data->feed_upm) =
            emcmotStatus->tag.fields_float[GM_FIELD_FLOAT_FEED] *
            emcmotStatus->net_feed_scale;
        *(emcmot_hal_data->feed_inches_per_minute) =
            *emcmot_hal_data->feed_upm * inch_mult;
        *(emcmot_hal_data->feed_inches_per_second) =
            *emcmot_hal_data->feed_inches_per_minute / 60;
        *(emcmot_hal_data->feed_mm_per_minute) =
            *emcmot_hal_data->feed_inches_per_minute * 25.4;
        *(emcmot_hal_data->feed_mm_per_second) =
            *emcmot_hal_data->feed_mm_per_minute / 60;
        break;
    default:
    ...
    for (spindle_num = 0; spindle_num < emcmotConfig->numSpindles;
        ...
        if (emcmotStatus->spindle_status[spindle_num].css_factor) {
            double denom =
                fabs(emcmotStatus->spindle_status[spindle_num].xoffset -
                     emcmotStatus->carte_pos_cmd.tran.x);
            double maxpositive;
            if (denom > 0)
                speed = emcmotStatus->spindle_status[spindle_num].css_factor /
                        denom;
            else
        ...
        if (speed > 0) {
            ...
            } else if (speed < emcmotStatus->spindle_status[spindle_num]
                                   .min_pos_speed) {
    ...
    for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
        ...
        *(joint_data->faulted) = GET_JOINT_FAULT_FLAG(joint);
        ...
        if (IS_EXTRA_JOINT(joint_num) && get_homed(joint_num)) {
            ...
            *(joint_data->motor_pos_cmd) =
                *(ejoint_data->posthome_cmd) + joint->motor_offset;
            continue;
        }
    } // for joint_num
    ...
    *(emcmot_hal_data->jog_is_active) =
        axis_jog_is_active() || joint_jog_is_active();
}
...
static void update_status(void)
{
    ...
    for (misc_error = 0; misc_error < emcmotConfig->numMiscError;
         misc_error++) {
        emcmotStatus->misc_error[misc_error] =
            *(emcmot_hal_data->misc_error[misc_error]);
    }
