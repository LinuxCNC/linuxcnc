...
static void handle_jogwheels(void)
{
	...
	if ( *(joint_data->jog_vel_mode) ) {
        ...
        joint->free_tp.vel_cmd = joint->vel_limit;
...
static void output_to_hal(void)
{
    ...
    *(emcmot_hal_data->distance_to_go) = emcmotStatus->distance_to_go;
    if(GET_MOTION_COORD_FLAG()) {
        ...
        *(emcmot_hal_data->requested_vel) = 0.0;
        ...
        double v2 = 0.0;
        for(i=0; i<num_joints; i++)
            if(GET_JOINT_ACTIVE_FLAG(&(joints[i])))
                v2 += simple_tp_get_curr_vel(&(joints[i].free_tp))*simple_tp_get_curr_vel(&(joints[i].free_tp));
        if(v2 > 0.0)
	...
	*(joint_data->free_vel_lim) = joint->free_tp.vel_cmd;
