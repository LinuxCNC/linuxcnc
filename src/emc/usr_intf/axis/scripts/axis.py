...
class AxisPreferences(cp):
    ...
    def getpref(self, option, default=False, type=bool):
        ...
        if type == repr and len(default) == 0:
            default = ""
        try:
...
def General_Halt():
    ...
    if not root_window.tk.call(
        "nf_dialog", ".error", _("Confirm Close"), text, "warning", 1, _("Yes"), _("No")
    ):
        os.system("xset r on")
        root_window.destroy()
...
def install_help(app):
    ...
    for i in range(len(help1)):
        a, b = help1[i]
        Label(keys, text=a, font=fixed, padx=4, pady=0, highlightthickness=0).grid(
            row=i, column=0, sticky="w"
        )
        Label(keys, text=b, padx=4, pady=0, highlightthickness=0).grid(
...
def parse_color(c):
    if c == "":
        return (1, 0, 0)
    return tuple([i / 65535.0 for i in root_window.winfo_rgb(c)])
...
class MyOpengl(GlCanonDraw, Opengl):
    def __init__(self, *args, **kw):
        ...
        self.set_eyepoint(5.0)
        self.get_resources()
        self.realize()
        self.init_glcanondraw(trajcoordinates=trajcoordinates, kinsmodule=kinsmodule)
    ...
    def select_cancel(self, event):
        if self.select_primed and (
            event.x != self.select_primed.x or event.y != self.select_primed.y
        ):
            self.select_primed = None

    def select_fire(self, event):
        if self.select_primed:
            self.queue_select(event)
    ...
    def get_show_commanded(self):
        return vars.display_type.get()

    def get_show_rapids(self):
        return vars.show_rapids.get()
    ...
    def get_program_alpha(self):
        return vars.program_alpha.get()

    def get_a_axis_wrapped(self):
        return a_axis_wrapped

    def get_b_axis_wrapped(self):
        return b_axis_wrapped

    def get_c_axis_wrapped(self):
        return c_axis_wrapped

    def set_current_line(self, line):
        if line == vars.running_line.get():
            return
    ...
    def get_show_program(self):
        return vars.show_program.get()

    def get_show_offsets(self):
        return vars.show_offsets.get()

    def get_show_extents(self):
        return vars.show_extents.get()

    def get_grid_size(self):
        return vars.grid_size.get()

    def get_show_metric(self):
        return vars.metric.get()

    def get_show_live_plot(self):
        return vars.show_live_plot.get()

    def get_show_machine_speed(self):
        return vars.show_machine_speed.get()

    def get_show_distance_to_go(self):
        return vars.show_distance_to_go.get()
    ...
    def get_view(self):
        ...
        if str(widgets.view_x["relief"]) == "sunken":
            ...
            str(widgets.view_y["relief"]) == "sunken"
            or str(widgets.view_y2["relief"]) == "sunken"
        ):
            view = y
        elif (
            str(widgets.view_z["relief"]) == "sunken"
            or str(widgets.view_z2["relief"]) == "sunken"
        ):
            view = z
        else:
            view = p
    ...
    def get_show_relative(self):
        return vars.coord_type.get()

    def get_show_limits(self):
        return vars.show_machine_limits.get()

    def get_show_tool(self):
        return vars.show_tool.get()
    ...
    def redraw_dro(self):
        ...
        if not hasattr(self, "font_width"):
            self.font_width = text.tk.call("font", "measure", (font, -100, "bold"), "0")
            self.font_vertspace = (
                text.tk.call("font", "metrics", (font, -100, "bold"), "-linespace")
                - 100
            )
            self.last_font = None
...
class LivePlotter:
    ...
    def start(self):
        ...
        o.after_idle(lambda: _thread.start_new_thread(self.logger.start, (0.01,)))

        global feedrate_blackout, rapidrate_blackout, spindlerate_blackout, maxvel_blackout
        feedrate_blackout = rapidrate_blackout = spindlerate_blackout = (
            maxvel_blackout
        ) = (time.time() + 1)
    ...
    def update(self):
        ...
        if (self.stat.motion_mode == linuxcnc.TRAJ_MODE_COORD) and (
            self.stat.task_mode == linuxcnc.MODE_MANUAL
        ):
            set_motion_teleop(1)
        if (
            self.stat.motion_mode == linuxcnc.TRAJ_MODE_TELEOP
        ...
        if enable_tab_change and (self.stat.task_mode != self.last_task_mode):
        ...
        if (
            self.stat.tool_offset != o.last_tool_offset
        ...
        if (
            ...
            or abs(speed - self.last_speed) > 0.01
        ):
            o.redraw_soon()
        ...
        if time.time() > spindlerate_blackout:
            vupdate(vars.spindlerate, int(100 * self.stat.spindle[0]["override"] + 0.5))
        if time.time() > feedrate_blackout:
            vupdate(vars.feedrate, int(100 * self.stat.feedrate + 0.5))
        if time.time() > rapidrate_blackout:
            vupdate(vars.rapidrate, int(100 * self.stat.rapidrate + 0.5))
        ...
        if current_tool is None:
            ...
            vupdate(
                vars.tool,
                _("Tool %(tool)d, offset %(zo)g, diameter %(dia)g") % tool_data,
            )
        else:
            vupdate(
                vars.tool,
                _("Tool %(tool)d, zo %(zo)g, xo %(xo)g, dia %(dia)g") % tool_data,
            )
        active_codes = []
        for i in self.stat.gcodes[1:]:
            ...
            if i % 10 == 0:
                ...
                active_codes.append(
                    "G%(ones)d.%(tenths)d" % {"ones": i / 10, "tenths": i % 10}
                )
...
def running(do_poll=True):
    if do_poll:
        s.poll()
    return (
        s.task_mode == linuxcnc.MODE_AUTO or s.task_mode == linuxcnc.MODE_MDI
...
def manual_ok(do_poll=True):
    ...
    return s.interp_state == linuxcnc.INTERP_IDLE or (
        s.task_mode == linuxcnc.MODE_MDI
        and s.queued_mdi_commands < vars.max_queued_mdi_commands.get()
    )
...
class Progress:
    def __init__(self, phases, total):
        ...
        root_window.tk.call(
            "pack", ".info.progress", "-side", "left", "-fill", "both", "-expand", "1"
        )
        root_window.tk.call(
            ".info.progress",
            "create",
    ...
    def update(self, count, force=0):
        if force or count - self.lastcount > 400:
            ...
            t.tk.call(
                ".info.progress", "coords", "1", (0, 0, int(fraction * width), height)
            )
    ...
    def set_text(self, text):
        if self.text is None:
            ...
            root_window.tk.call(".info.progress", "itemconfigure", text, "-text", text)
...
def filter_program(program_filter, infilename, outfilename):
    ...
    p = subprocess.Popen(
        ["sh", "-c", "%s '%s'" % (program_filter, infilename_q)],
        ...
        if program_filter:
            ...
            if exitcode:
                root_window.tk.call(
                    ...
                    _("Filter failed"),
                    _(
                        "The program %(program)r exited with code %(code)d.  "
    ...
    try:
        ...
        if not interpname:
            ...
            for i in range(9):
                if s.axis_mask & (1 << i):
                    ...
                    if (
                        (axis == "A" and a_axis_wrapped)
                        or (axis == "B" and b_axis_wrapped)
                        or (axis == "C" and c_axis_wrapped)
                    ):
        ...
        if result > gcode.MIN_ERROR:
            ...
            root_window.tk.call(
                ...
                % {"seq": seq, "f": f, "error_str": error_str},
                "error",
                0,
                _("OK"),
            )
        ...
        o.lp.set_depth(
            from_internal_linear_unit(o.get_foam_z()),
...
for j in range(linuxcnc.MAX_JOINTS):
    widget_list.append(
        ("joint_" + str(j), Radiobutton, tabs_manual + ".joints.joint" + str(j))
    )
...
def activate_ja_widget(i, force=0):
    if not force and not manual_ok():
        return
    if get_jog_mode() and (
        not kins_is_trivkins
        or (kins_is_trivkins and s.kinematics_type == linuxcnc.KINEMATICS_BOTH)
    ):
...
def parse_increment(jogincr):
    if jogincr.endswith("mm"):
        ...
        scale = from_internal_linear_unit(0.001 / 25.4)
    elif jogincr.endswith("in") or jogincr.endswith("inch"):
        scale = from_internal_linear_unit(1.0)
    elif jogincr.endswith("mil"):
        scale = from_internal_linear_unit(0.001)
    else:
        scale = 1
...
def set_hal_jogincrement():
    if not "comp" in globals():
        return  # this is called once during startup before comp exists
...
class DummyCanon:
    ...
    def set_g5x_offset(*args):
        pass

    def set_g92_offset(*args):
        pass

    def set_xy_rotation(*args):
        pass
    ...
    def __init__(self, title, text_pattern, default, tool_only, defaultsystem):
        ...
        if not tool_only:
            for i in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
                t.bind(
                    "<Alt-KeyPress-%s>" % i,
                    lambda event, system=systems[i - 1]: c.set(system),
                )
        if tool_only:
...
def get_jog_speed(a):
    if vars.teleop_mode.get():
        if axis_type[a] == "LINEAR":
            return vars.jog_speed.get() / 60.0
        else:
            return vars.jog_aspeed.get() / 60.0
    else:
...
def get_jog_speed_map(a):
    if get_jog_mode() and a >= num_joints:
        return 0
    if not get_jog_mode():
        if a >= len(jog_order):
            return 0
...
def get_max_jog_speed(a):
    ...
    if vars.metric.get():
        max_linear_speed = max_linear_speed * 25.4
...
def run_warn():
    ...
    if o.canon:
        ...
        for i in range(3):  # Does not enforce angle limits
            if not (s.axis_mask & (1 << i)):
                continue
            if o.canon.min_extents_notool[i] < machine_limit_min[i]:
                warnings.append(
                    _("Program exceeds machine minimum on axis %s") % "XYZABCUVW"[i]
                )
            if o.canon.max_extents_notool[i] > machine_limit_max[i]:
                warnings.append(
                    _("Program exceeds machine maximum on axis %s") % "XYZABCUVW"[i]
                )
    if warnings:
        text = "\n".join(warnings)
        return (
...
def motion_modename(x):
    if x == linuxcnc.TRAJ_MODE_FREE:
        return "FREE"
    if x == linuxcnc.TRAJ_MODE_COORD:
        return "COORD"
    if x == linuxcnc.TRAJ_MODE_TELEOP:
        return "TELEOP"


def task_modename(x):
    if x == linuxcnc.MODE_MDI:
        return "MDI"
    if x == linuxcnc.MODE_MANUAL:
        return "MANUAL"
    if x == linuxcnc.MODE_AUTO:
        return "AUTO"


def task_statename(x):
    if x == linuxcnc.STATE_ESTOP:
        return "STATE_ESTOP"
    if x == linuxcnc.STATE_ESTOP_RESET:
        return "STATE_ESTOP_RESET"
    if x == linuxcnc.STATE_OFF:
        return "STATE_OFF"
    if x == linuxcnc.STATE_ON:
        return "STATE_ON"


def interp_statename(x):
    if x == linuxcnc.INTERP_IDLE:
        return "IDLE"
    if x == linuxcnc.INTERP_READING:
        return "READING"
    if x == linuxcnc.INTERP_PAUSED:
        return "PAUSED"
    if x == linuxcnc.INTERP_WAITING:
        return "WAITING"
...
class TclCommands(nf.TclCommands):
    ...
    def gcode_properties(event=None):
        ...
        if not loaded_file:
            ...
            g1 = sum(dist(l[1][:3], l[2][:3]) for l in o.canon.feed) + sum(
                dist(l[1][:3], l[2][:3]) for l in o.canon.arcfeed
            )
            gt = (
                sum(dist(l[1][:3], l[2][:3]) / min(mf, l[3]) for l in o.canon.feed)
            ...
            props["g0"] = "%f %s".replace("%f", fmt) % (
                from_internal_linear_unit(g0, conv),
                units,
            )
            props["g1"] = "%f %s".replace("%f", fmt) % (
                from_internal_linear_unit(g1, conv),
                units,
            )
            if gt > 120:
    ...
    def set_maxvel(newval):
        ...
        c.maxvel(newval / 60.0)
        maxvel_blackout = time.time() + 1

    def copy_line(*args):
        line = -1
        if vars.running_line.get() != -1:
            line = vars.running_line.get()
    ...
    def onoff_clicked(event=None):
        ...
        if s.task_state == linuxcnc.STATE_ESTOP_RESET:
            ...
            if homing_prompt:
                run_homing = prompt_areyousure(
                    _("Homing request"),
                    _(
                        "After turning On the machine power,\nYou need find axes origins.\n\n            Run homing process?"
                    ),
                )
                if run_homing:
    ...
    def open_file(*event):
        ...
        types = (
            ((_("All machinable files"), all_extensions), (_("rs274ngc files"), ".ngc"))
            + extensions
            + ((_("All files"), "*"),)
        )
        f = root_window.tk.call(
            "tk_getOpenFile", "-initialdir", open_directory, "-filetypes", types
        )
        if type(f) is tuple and not len(f):
            return
        if not len(str(f)):
            return
        o.set_highlight_line(None)
    ...
    def edit_program(*event):
        if loaded_file is None:
            ...
            if omode == 1:
                root_window.tk.call("save_gcode", "my_" + showname)
                return
            elif omode == 2:
                return
    ...
    def mdi_up_cmd(*args):
        if args and args[0].char:
            return  # e.g., for KP_Up with numlock on
    ...
    def send_mdi_command(command):
        ...
        if command != "":
            ...
            if (
                history_size > 1
                and widgets.mdi_history.get(history_size - 2) == command
            ):
                new_entry = 0
            if new_entry != 0:
    ...
    def mdi_history_write_to_file(file_name, history_size):
        ...
        if history_size > 1 or history_size == -1:
            ...
            try:
                ...
                print(
                    "Can't open MDI history file [%s] for writing" % file_name,
                    file=sys.stderr,
                )
        ...
        try:
            ...
            for data in root_window.selection_get(selection="CLIPBOARD").split("\n"):
                if data != "":
                    ...
                    if (
                        history_size > 1
                        and widgets.mdi_history.get(history_size - 2) == data
                    ):
                        new_entry = 0
                    if new_entry != 0:
                        # if command is already at end of list, don't add it again
    ...
    def mdi_history_butt_1(event):
        ...
        if (
            bbox
            and (event.y <= (bbox[1] + bbox[3]))
            and (cursel < widgets.mdi_history.size() - 1)
        ):
    ...
    def ensure_manual(*event):
        ...
        if (
            ...
            or (s.motion_mode == linuxcnc.TRAJ_MODE_COORD)
        ):
            set_motion_teleop(1)
        else:
            ensure_mode(linuxcnc.MODE_MANUAL)
    ...
    def set_grid_size_custom(*event):
        if vars.metric.get():
            ...
            unit_str = " " + _("in")
        v = prompt_float(_("Custom Grid"), _("Enter grid size"), "", unit_str) or 0
        if v <= 0:
            return
        if vars.metric.get():
            v /= 25.4
    ...
    def home_all_joints(event=None):
        ...
        doHoming = True
        if isHomed:
            doHoming = prompt_areyousure(
                _("Warning"),
                _("Joint is already homed, are you sure you want to re-home?"),
            )
        if doHoming:
            go_home(-1)
    ...
    def home_joint(event=None):
        ...
        if s.homed[jnum]:
            doHoming = prompt_areyousure(
                _("Warning"),
                _("This joint is already homed, are you sure you want to re-home?"),
            )
        if doHoming:
            ensure_mode(linuxcnc.MODE_MANUAL)
            go_home(jnum)
    ...
    def touch_off_system(event=None, new_axis_value=None):
        ...
        touchoff_actual_position = inifile.find(
            f"AXIS_{vars.ja_rbutton.get().upper()}", "TOUCHOFF_ACTUAL"
        )
        offset_axis = trajcoordinates.index(vars.ja_rbutton.get())
        if new_axis_value is None:
            new_axis_value, system = prompt_touchoff(
                title=_(
                    f"Touch Off ({'system' if touchoff_actual_position is None else 'system ACTUAL'})"
                ),
                text=_("Enter %s coordinate relative to %%s:")
                % vars.ja_rbutton.get().upper(),
                default=0.0,
                tool_only=False,
        ...
        linear_axis = vars.ja_rbutton.get() in "xyzuvw"
        if linear_axis and vars.metric.get():
            scale = 1 / 25.4
        else:
            scale = 1
        ...
        if touchoff_actual_position is not None:
            new_axis_value = str(
                ...
                + (-1.0 if touchoff_actual_position.upper() == "MINUS" else 1.0)
                * s.actual_position[offset_axis]
            )

        offset_command = "G10 L20 %s %c[[%s]*%.12f]" % (
            system.split()[0],
            vars.ja_rbutton.get(),
            new_axis_value,
            scale,
    ...
    def touch_off_tool(event=None, new_axis_value=None):
        ...
        offset_command = "G10 L%d P%d %c[[%s]*%.12f]" % (
            lnum,
            s.tool_in_spindle,
            vars.ja_rbutton.get(),
            new_axis_value,
    ...
    def set_axis_offset(event=None):
        commands.touch_off_system(new_axis_value=0.0)
...
def nomodifier(f):
    def g(event):
        if event.state & (1 | 4 | 8 | 32 | 64 | 128):
            return ""
        return f(event)
...
def get_jog_mode():
    s.poll()
    if (s.kinematics_type == linuxcnc.KINEMATICS_IDENTITY) and all_homed():
        teleop_mode = 1
        jjogmode = False
    else:
        ...
        if s.motion_mode == linuxcnc.TRAJ_MODE_FREE:
            ...
            teleop_mode = 1
            jjogmode = False
    if (jjogmode and s.motion_mode != linuxcnc.TRAJ_MODE_FREE) or (
        not jjogmode and s.motion_mode != linuxcnc.TRAJ_MODE_TELEOP
    ):
...
def jog_on(a, b):
    if not manual_ok() or not manual_tab_visible() or running():
        return
    if a < 3 or a > 5:
        if vars.metric.get():
            b = b / 25.4
    ...
    if jogincr != _("Continuous"):
        ...
        if not a in cjogindices:
            cjogindices.append(a)
...
def bind_axis(a, b, d):
    root_window.bind(
        "<KeyPress-%s>" % a,
        kp_wrap(lambda e: jog_on_map(d, -get_jog_speed_map(d)), "KeyPress"),
    )
    root_window.bind(
        "<KeyPress-%s>" % b,
        kp_wrap(lambda e: jog_on_map(d, get_jog_speed_map(d)), "KeyPress"),
    ...
    root_window.bind(
        "<Shift-KeyPress-%s>" % a, lambda e: jog_on_map(d, -get_max_jog_speed_map(d))
    )
    root_window.bind(
        "<Shift-KeyPress-%s>" % b, lambda e: jog_on_map(d, get_max_jog_speed_map(d))
    )
...
max_spindle_override = float(
    inifile.find("DISPLAY", "MAX_SPINDLE_OVERRIDE") or max_feed_override
)
...
for a in range(linuxcnc.MAX_AXIS):
    ...
    letter = "xyzabcuvw"[a]
    if not (letter in trajcoordinates):
        continue
    if letter in "abc":
        axis_type[a] = "ANGULAR"
    else:
...
has_angular_joint_or_axis = ("ANGULAR" in joint_type) or ("ANGULAR" in axis_type)
...
try:
    if has_angular_joint_or_axis:
        ...
        if default_jog_angular_speed is None:
            default_jog_angular_speed = max_angular_speed
...
print(
    "note: MAXV     max: %.3f units/sec %.3f units/min"
    % (float(max_velocity), 60 * float(max_velocity)),
    file=sys.stderr,
)
if has_linear_joint_or_axis:
    print(
        "note: LJOG     max: %.3f units/sec %.3f units/min"
        % (float(max_linear_speed), 60 * float(max_linear_speed)),
        file=sys.stderr,
    )
    print(
        "note: LJOG default: %.3f units/sec %.3f units/min"
        % (float(default_jog_linear_speed), 60 * float(default_jog_linear_speed)),
        file=sys.stderr,
    )
if has_angular_joint_or_axis:
    print(
        "note: AJOG     max: %.3f units/sec %.3f units/min"
        % (float(max_angular_speed), 60 * float(max_angular_speed)),
        file=sys.stderr,
    )
    print(
        "note: AJOG default: %.3f units/sec %.3f units/min"
...
if db_program is not None and tooltable is not None:
    print(
        "%s:DB_PROGRAM specified: TOOL_TABLE=%s is not used" % (sys.argv[0], tooltable)
    )

default_tooleditor = "tooledit"
if db_program is not None:
    default_tooleditor = None
...
if coordinate_display:
    if coordinate_display.lower() in ("mm", "metric"):
        vars.metric.set(1)
    else:
        vars.metric.set(0)
else:
    if lu in [0.001, 0.01, 0.1, 1, 10]:
        vars.metric.set(1)
    else:
        vars.metric.set(0)
if lu == 1:
...
ct = float(inifile.find("DISPLAY", "CYCLE_TIME") or 0.020)
if ct < 1:
...
while (s.joints == 0) or (s.kinematics_type < linuxcnc.KINEMATICS_IDENTITY):
    print(
        "waiting for s.joints<%d>, s.kinematics_type<%d>"
        % (s.joints, s.kinematics_type)
    )
    time.sleep(statwait)
    statfail += 1
    statwait *= 2
...
if homing_order_defined:
    ...
    root_window.tk.call(
        ...
        widgets.homebutton,
        "-text",
        _("Home all %s [Ctrl-Home]") % ja_name,
    )
    widgets.homemenu.add_command(command=commands.home_all_joints)
    root_window.tk.call(
        "setup_menu_accel", widgets.homemenu, "end", _("Home All %s") % ja_name
    )
widgets.unhomemenu.add_command(command=commands.unhome_all_joints)
root_window.tk.call(
    "setup_menu_accel", widgets.unhomemenu, "end", _("Unhome All %s") % ja_name
)
...
if duplicate_coord_letters != "":
    ...
    print(
        _("Warning: Forward kinematics must handle duplicate coordinate letters:%s")
        % duplicate_coord_letters
    )
if len(trajcoordinates) > jointcount:
    print(
        _("Note: number of [TRAJ]COORDINATES=%(t)s exceeds [KINS]JOINTS=%(l)d")
...
def lathe_historical_config():
    ...
    if lathe and (trajcoordinates.upper() == "XZ") and (num_joints == 3):
        return True
    return False
    ...
    if s.kinematics_type == linuxcnc.KINEMATICS_IDENTITY:
        ...
        if ja_id.lower() in duplicate_coord_letters:
            if ja_id not in gave_individual_homing_message:
                ...
                print(
                    _("KINEMATICS_IDENTITY with duplicate axis letter <%s>\n") % ja_id
                )
...
for a in range(linuxcnc.MAX_AXIS):
    ...
    try:
        ...
        if f != 0:
            step_size_tmp = min(step_size, 1.0 / f)
            if a in "XYZ":
                step_size = astep_size = step_size_tmp
            elif a in "ABC":
                astep_size = step_size_tmp
            else:
                step_size = step_size_tmp

if inifile.find("DISPLAY", "MIN_LINEAR_VELOCITY"):
    root_window.tk.call(
        "set_slider_min", float(inifile.find("DISPLAY", "MIN_LINEAR_VELOCITY")) * 60
    )
elif inifile.find("DISPLAY", "MIN_VELOCITY"):
    root_window.tk.call(
        "set_slider_min", float(inifile.find("DISPLAY", "MIN_VELOCITY")) * 60
    )
elif step_size != 1:
...
if inifile.find("DISPLAY", "MIN_ANGULAR_VELOCITY"):
    root_window.tk.call(
        "set_aslider_min", float(inifile.find("DISPLAY", "MIN_ANGULAR_VELOCITY")) * 60
    )
elif inifile.find("DISPLAY", "MIN_VELOCITY"):
    root_window.tk.call(
        "set_aslider_min", float(inifile.find("DISPLAY", "MIN_VELOCITY")) * 60
    )
elif astep_size != 1:
...
if jog_invert is None:
    if lathe and not lathe_backtool:
        jog_invert = "X"
    else:
        jog_invert = ""
...
if (s.axis_mask & 56 == 0) and not (  # 56==0x38== 000111000 (ABC)
    "ANGULAR" in joint_type
):
...
def setup_grid_menu(grids):
    for i in grids.split():
        ...
        widgets.menu_grid.add_radiobutton(
            value=v, label=i, variable="grid_size", command="set_grid_size"
        )
    match_grid_size(vars.grid_size.get())


grids = inifile.find("DISPLAY", "GRIDS") or "10mm 20mm 50mm 100mm 1in 2in 5in 10in"
...
def get_coordinate_font(large):
    ...
    if coordinate_font not in font_cache:
        font_cache[coordinate_font] = glnav.use_pango_font(coordinate_font, 0, 128)
    fontbase, coordinate_charwidth, coordinate_linespace = font_cache[coordinate_font]
...
def check_dynamic_tabs():
    for c, cmd, w in list(_dynamic_childs.values()):
        ...
        if r is None:
            break
        print('Embeded tab command "%s" exited with error: %s' % (" ".join(cmd), r))
        raise SystemExit(r)
    else:
        if postgui_halfile is not None:
            for f in postgui_halfile:
                if f.lower().endswith(".tcl"):
                    res = os.spawnvp(
                        os.P_WAIT, "haltcl", ["haltcl", "-i", vars.emcini.get(), f]
                    )
                else:
                    res = os.spawnvp(
                        os.P_WAIT,
                        "halcmd",
                        ["halcmd", "-i", vars.emcini.get(), "-f", f],
                    )
                if res:
...
icons = (
    root_window.tk.call("load_image", "axis-48x48"),
...
def forget(widget, *pins):
    if "AXIS_NO_AUTOCONFIGURE" in os.environ:
        return
    if hal_present == 1:
        for p in pins:
            if hal.pin_has_writer(p):
                return
...
if os.path.exists(rcfile):
    try:
        ...
        root_window.tk.call(
            "nf_dialog", ".error", _("Error in ~/.axisrc"), tb, "error", 0, _("OK")
        )
