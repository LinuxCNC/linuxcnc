...
class _prompt_touchoff(_prompt_float):
    def __init__(self, title, text, default, defaultsystem):
        ...
        for i in [1,2,3,4,5,6,7,8,9]:
            t.bind("<Alt-KeyPress-%s>" % i, lambda event, system=all_systems[i-1]: c.set(system))
        if current_tool != None:
            t.bind("<Alt-t>", lambda event: c.set(all_systems[9]))
            t.bind("<Alt-0>", lambda event: c.set(all_systems[9]))
