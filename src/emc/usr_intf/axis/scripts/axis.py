...
GSTAT = GStat()
GSTAT.forced_update()
GSTAT.connect('jograte-changed', lambda w, data: set_jogspeed(data))
GSTAT.connect('jograte-angular-changed', lambda w, data: set_ajogspeed(data))
...
if hal_present == 1 :
    ...
    comp.newpin('mpg-enable', hal.Type.BOOL, hal.Dir.IN)
    comp.newpin('mpg-in', hal.HAL_S32, hal.Dir.IN)
...
def set_jogspeed(data):
    vars.jog_speed.set(data)
    root_window.tk.eval("${pane_top}.jogspeed.s set [setval $jog_speed $max_speed]")

def set_ajogspeed(data):
    vars.jog_aspeed.set(data)
    root_window.tk.eval("${pane_top}.ajogspeed.s set [setval $jog_aspeed $max_aspeed]")
...
def run_macro(data):
    ...
    for code in o_codes[1:]:
        ...
        param = prompt_float("Macro", f"Enter a value for: {code}:",
                "0", unit_str)
        if param == '': 
            notifications.add("error", _('Bad Macro Parameter Input'))
            return
...
def softkey_pressed(index):
    try:
        # call a softkey function directed by index
        globals()[f'softkey_action_{index}']()
    except:
        pass

# standard softkey functions. Can be overridden in user file
def softkey_action_0():
    root_window.tk.call('.pane.top.tabs','raise','manual')
def softkey_action_1():
    root_window.tk.call('.pane.top.tabs','raise','mdi')
def softkey_action_2():
    root_window.tk.call('.pane.top.right','raise','preview')
def softkey_action_3():
    root_window.tk.call('.pane.top.right','raise','numbers')
def softkey_action_4():
