...
def show_floats(s):
    return " ".join(show_float(p) for p in s)


def show_ints(s):
    return " ".join(str(bool(p)) for p in s)
...
maps = {
    ...
    "kinematics_type": {
        linuxcnc.KINEMATICS_IDENTITY: "identity",
        linuxcnc.KINEMATICS_FORWARD_ONLY: "forward_only",
        linuxcnc.KINEMATICS_INVERSE_ONLY: "inverse_only",
        linuxcnc.KINEMATICS_BOTH: "both",
    },
    "mcodes": show_mcodes,
    "gcodes": show_gcodes,
    "poll": None,
...
def gui():
    ...
    t.tag_configure(
        "value", foreground="black", font=fixed_font, lmargin1=150, lmargin2=150
    )
    t.tag_configure(
        "changedvalue",
        foreground="black",
    ...
    b = tkinter.Button(
        text=_("Copy All"), command="%s tag add sel 0.0 end; tk_textCopy %s" % (t, t)
    )
