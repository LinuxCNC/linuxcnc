...
if is_stepper:
    print("Appears to be a stepper configuration")
else:
    print("Appears to be a servo configuration")

for i in range(njoints):
    joint = "JOINT_%d" % i
    if ini.find(joint, "INPUT_SCALE") is None:
        continue
    scale = get_float(joint, "INPUT_SCALE")
    if is_stepper:
        if ini.find(joint, "STEPGEN_MAXVEL") is None:
            continue
        ...
        if headroom_vel < vel * 1.01:
            report_problem(
                "Less than 1%% velocity headroom from %g to %g", vel, headroom_vel
            )
        ...
        if headroom_acc < acc * 1.01:
            report_problem(
                "Less than 1%% acceleration headroom from %g to %g", acc, headroom_acc
            )
