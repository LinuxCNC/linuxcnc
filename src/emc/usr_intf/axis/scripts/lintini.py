...
if ini.find("EMCMOT", "BASE_PERIOD") is None:
    ...
    period = ini.getint("EMCMOT", "BASE_PERIOD", fallback=0)
njoints = ini.getint("KINS", "JOINTS", fallback=0)
...
for i in range(njoints):
    ...
    scale = ini.getfloat(joint, "INPUT_SCALE")
    if is_stepper:
        if ini.find(joint,"STEPGEN_MAXVEL") is None: continue
        vel = ini.getfloat(joint, "STEPGEN_MAXVEL")
        cycles_per_step = 2
        ...
        vel = ini.getfloat(joint, "MAX_VELOCITY")
        headroom_vel = ini.getfloat(joint, "STEPGEN_MAXVEL")
        if headroom_vel < vel * 1.01:
            report_problem(
                "Less than 1%% velocity headroom from %g to %g",
                vel, headroom_vel)

        acc = ini.getfloat(joint, "MAX_ACCELERATION")
        headroom_acc = ini.getfloat(joint, "STEPGEN_MAXACCEL")
