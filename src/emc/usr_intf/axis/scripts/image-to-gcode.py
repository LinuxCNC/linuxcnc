...
def progress(a, b):
    if "AXIS_PROGRESS_BAR" in os.environ:
        print("FILTER_PROGRESS=%d" % int(a * 100.0 / b + 0.5), file=sys.stderr)
        sys.stderr.flush()
...
class Converter:
    ...
    def convert(self):
        ...
        if self.roughing_delta and self.roughing_offset:
            ...
            nim1[int(tw / 2) : int(tw / 2 + w), int(th / 2) : int(th / 2 + h)] = (
                base_image
            )
...
def ui(im, nim, im_name):
    ...
    def intentry(f, v):
        ...
        var.set(v)
        w = tkinter.Entry(
            f, textvariable=var, validatecommand=validate_int, validate="key", width=10
        )
        return w, var
    ...
    def intscale(k, v, min=1, max=100):
        ...
        w = tkinter.Scale(
            g, orient="h", variable=var, from_=min, to=max, showvalue=False
        )
    ...
    def _optionmenu(k, v, *options):
        ...
        f.tk.call(
            ...
            0,
            "-width",
            max(len(opt) for opt in options) + 3,
            "-textvariable",
            svar._name,
            "-background",
            "white",
    ...
    constructors = [
        ...
        (
            "pattern",
            optionmenu(
                _("Rows"), _("Columns"), _("Rows then Columns"), _("Columns then Rows")
            ),
        ),
        (
            "converter",
    ...
    bb = tkinter.Button(
        b, text=_("OK"), command=lambda: status.set(1), width=8, default="active"
    )
    bb.pack(side="left", padx=4, pady=4)
    bb = tkinter.Button(
        b, text=_("Cancel"), command=lambda: status.set(-1), width=8, default="normal"
    )
    bb.pack(side="left", padx=4, pady=4)
...
def main():
    if len(sys.argv) > 1:
        ...
        im_name = tkFileDialog.askopenfilename(
            ...
            filetypes=((_("Depth images"), ".gif .png .jpg"), (_("All files"), "*")),
        )
        if not im_name:
            raise SystemExit
    ...
    nim = (
        numpy.frombuffer(tobytes(im), dtype=numpy.uint8)
        .reshape((h, w))
