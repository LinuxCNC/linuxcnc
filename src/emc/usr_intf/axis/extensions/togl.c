...
static LRESULT(CALLBACK *tkWinChildProc)(HWND hwnd,
                                         UINT message,
                                         WPARAM wParam,
                                         LPARAM lParam) = NULL;
...
static Tk_ConfigSpec configSpecs[] = {
    {TK_CONFIG_PIXELS,
     ...
     Tk_Offset(struct Togl, Height),
     0, NULL},

    {TK_CONFIG_PIXELS,
     "-width", "width",
     "Width", DEFAULT_WIDTH,
     Tk_Offset(struct Togl, Width),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-rgba", "rgba",
     "Rgba", "true",
     Tk_Offset(struct Togl, RgbaFlag),
     0, NULL},

    {TK_CONFIG_INT,
     "-redsize", "redsize",
     "RedSize", "1",
     Tk_Offset(struct Togl, RgbaRed),
     0, NULL},

    {TK_CONFIG_INT,
     "-greensize", "greensize",
     "GreenSize", "1",
     Tk_Offset(struct Togl, RgbaGreen),
     0, NULL},

    {TK_CONFIG_INT,
     "-bluesize", "bluesize",
     "BlueSize", "1",
     Tk_Offset(struct Togl, RgbaBlue),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-double", "double",
     "Double", "false",
     Tk_Offset(struct Togl, DoubleFlag),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-depth", "depth",
     "Depth", "false",
     Tk_Offset(struct Togl, DepthFlag),
     0, NULL},

    {TK_CONFIG_INT,
     "-depthsize", "depthsize",
     "DepthSize", "1",
     Tk_Offset(struct Togl, DepthSize),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-accum", "accum",
     "Accum", "false",
     Tk_Offset(struct Togl, AccumFlag),
     0, NULL},

    {TK_CONFIG_INT,
     "-accumredsize", "accumredsize",
     "AccumRedSize", "1",
     Tk_Offset(struct Togl, AccumRed),
     0, NULL},

    {TK_CONFIG_INT,
     "-accumgreensize", "accumgreensize",
     "AccumGreenSize", "1",
     Tk_Offset(struct Togl, AccumGreen),
     0, NULL},

    {TK_CONFIG_INT,
     "-accumbluesize", "accumbluesize",
     "AccumBlueSize", "1",
     Tk_Offset(struct Togl, AccumBlue),
     0, NULL},

    {TK_CONFIG_INT,
     "-accumalphasize", "accumalphasize",
     "AccumAlphaSize", "1",
     Tk_Offset(struct Togl, AccumAlpha),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-alpha", "alpha",
     "Alpha", "false",
     Tk_Offset(struct Togl, AlphaFlag),
     0, NULL},

    {TK_CONFIG_INT,
     "-alphasize", "alphasize",
     "AlphaSize", "1",
     Tk_Offset(struct Togl, AlphaSize),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-stencil", "stencil",
     "Stencil", "false",
     Tk_Offset(struct Togl, StencilFlag),
     0, NULL},

    {TK_CONFIG_INT,
     "-stencilsize", "stencilsize",
     "StencilSize", "1",
     Tk_Offset(struct Togl, StencilSize),
     0, NULL},

    {TK_CONFIG_INT,
     "-auxbuffers", "auxbuffers",
     "AuxBuffers", "0",
     Tk_Offset(struct Togl, AuxNumber),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-privatecmap", "privateCmap",
     "PrivateCmap", "false",
     Tk_Offset(struct Togl, PrivateCmapFlag),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-overlay", "overlay",
     "Overlay", "false",
     Tk_Offset(struct Togl, OverlayFlag),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-stereo", "stereo",
     "Stereo", "false",
     Tk_Offset(struct Togl, StereoFlag),
     0, NULL},
    ...
    {TK_CONFIG_INT,
     "-time", "time",
     "Time", DEFAULT_TIME,
     Tk_Offset(struct Togl, TimerInterval),
     0, NULL},

    {TK_CONFIG_STRING,
     "-sharelist", "sharelist",
     "ShareList", NULL,
     Tk_Offset(struct Togl, ShareList),
     0, NULL},

    {TK_CONFIG_STRING,
     "-sharecontext", "sharecontext",
     "ShareContext", NULL,
     Tk_Offset(struct Togl, ShareContext),
     0, NULL},

    {TK_CONFIG_STRING,
     "-ident", "ident",
     "Ident", DEFAULT_IDENT,
     Tk_Offset(struct Togl, Ident),
     0, NULL},

    {TK_CONFIG_BOOLEAN,
     "-indirect", "indirect",
     "Indirect", "false",
     Tk_Offset(struct Togl, Indirect),
     0, NULL},
 ...
 */
static Colormap get_rgb_colormap(Display *dpy,
                                 int scrnum,
                                 ...
                                 Tk_Window tkwin)
{
    ...
    if (using_mesa) {
        ...
        if (hp_cr_maps
            ...
            status = XGetRGBColormaps(
                dpy, root, &standardCmaps, &numCmaps, hp_cr_maps);
            if (status) {
    ...
    */
    ...
    if (status == 1) {
        status = XGetRGBColormaps(
            dpy, root, &standardCmaps, &numCmaps, XA_RGB_DEFAULT_MAP);
        if (status == 1) {
            for (i = 0; i < numCmaps; i++) {
...
static Colormap Win32CreateRgbColormap(PIXELFORMATDESCRIPTOR pfd)
{
    ...
    n = 1 << pfd.cColorBits;
    pPal = (PLOGPALETTE)LocalAlloc(
        LMEM_FIXED, sizeof(LOGPALETTE) + n * sizeof(PALETTEENTRY));
...
int Togl_Init(Tcl_Interp *interp)
{
    ...
    if (major >= 8 && minor >= 4) {
        TCL_ERR(
            interp,
            "Sorry, this instance of Togl was not compiled to work with Tcl/Tk "
            "8.4 or higher.");
    }
#endif
...
static void Togl_Timer(ClientData clientData)
{
    ...
    if (togl->TimerProc) {
       ...
       */
        ...
        togl->timerHandler = Tcl_CreateTimerHandler(
            togl->TimerInterval, Togl_Timer, (ClientData)togl);
#else
        togl->timerHandler = Tk_CreateTimerHandler(
...
#elif defined(X11)
    glXMakeCurrent(
        Tk_Display(togl->TkWin), Tk_WindowId(togl->TkWin), togl->GlCtx);
#if defined(__sgi) && defined(STEREO)
    stereoMakeCurrent(
        Tk_Display(togl->TkWin), Tk_WindowId(togl->TkWin), togl->GlCtx);
#endif /*__sgi STEREO */
...
static void SetMacBufRect(struct Togl *togl)
{
    ...
    wrect[1] = ((TkWindow *)(togl->TkWin))
                   ->privatePtr->toplevel->portPtr->portRect.bottom -
...
static void Togl_Render(ClientData clientData)
{
    ...
    if (togl->DisplayProc) {
        ...
        parentWin = (GrafPtr)(((MacDrawable *)(Tk_WindowId(togl->TkWin)))
                                  ->toplevel->portPtr);
        if (!parentWin)
            return;
#endif
...
#elif defined(X11)
        glXMakeCurrent(
            Tk_Display(togl->TkWin), togl->OverlayWindow, togl->OverlayCtx);
#if defined(__sgi) && defined(STEREO)
        stereoMakeCurrent(
            Tk_Display(togl->TkWin), togl->OverlayWindow, togl->OverlayCtx);
#endif /*__sgi STEREO */
 ...
 */
int Togl_Configure(
    Tcl_Interp *interp, struct Togl *togl, int argc, char *argv[], int flags)
{
...
int Togl_Widget(ClientData clientData,
                Tcl_Interp *interp,
                int argc,
                char *argv[])
{
    ...
    if (!strncmp(argv[1], "configure", MAX(1, strlen(argv[1])))) {
        if (argc == 2) {
            ...
            if (strcmp(argv[2], "-extensions") == 0) {
                ...
                result = Tk_ConfigureInfo(
                    interp, togl->TkWin, configSpecs, (char *)togl, argv[2], 0);
            }
        } else {
            /* Execute a configuration change */
            result = Togl_Configure(
                interp, togl, argc - 2, argv + 2, TK_CONFIG_ARGV_ONLY);
        }
        ...
        if (argc == 3) {
            ...
            result = TCL_ERROR;
                   ...
                   argv[1], "measurebitmapfont", MAX(1, strlen(argv[1])))) {
        if (argc != 3) {
            Tcl_AppendResult(interp, "wrong # args", NULL);
            result = TCL_ERROR;
        } else {
...
static int SetupOverlay(struct Togl *togl)
{
    ...
    static int ovAttributeList[] = {GLX_BUFFER_SIZE,
                                    2,
                                    GLX_LEVEL,
                                    1,
   ...
   */
    ...
    hPtr = Tcl_CreateHashEntry(
        &winPtr->dispPtr->winTable, (char *)togl->OverlayWindow, &new_flag);
...
static LRESULT CALLBACK Win32WinProc(HWND hwnd,
                                     UINT message,
                                     WPARAM wParam,
                                     LPARAM lParam)
{
    ...
    default:
        ...
        if (tkWinChildProc == NULL) {
            GetClassInfo(
                Tk_GetHINSTANCE(), TK_WIN_CHILD_CLASS_NAME, &childClass);
...
#define MAX_ATTEMPTS 12
    ...
    if (togl->ShareContext && FindTogl(togl->ShareContext)) {
         ...
         */
            ...
            visinfo =
                glXChooseVisual(dpy, Tk_ScreenNumber(togl->TkWin), attrib_list);
            if (visinfo) {
                /* found a GLX visual! */
                break;
            }
        ...
        if (visinfo == NULL) {
            Tcl_SetResult(
                togl->Interp, "Togl: couldn't get visual", TCL_STATIC);
            return DUMMY_WINDOW;
        }
       ...
       */
        ...
        if (togl->GlCtx == NULL) {
            Tcl_SetResult(
                togl->Interp, "could not create rendering context", TCL_STATIC);
            return DUMMY_WINDOW;
        }
    }
    ...
    if (ToglClassInitialized == 0) {
        ...
        if (!RegisterClass(&ToglClass)) {
            Tcl_SetResult(
                togl->Interp, "unable register Togl window class", TCL_STATIC);
            return DUMMY_WINDOW;
        }
    }

    hwnd = CreateWindow(TOGL_CLASS_NAME,
                        NULL,
    ...
    SetWindowPos(
        hwnd, HWND_TOP, 0, 0, 0, 0, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE);
    ...
    */
    ...
    if ((pixelformat = ChoosePixelFormat(togl->tglGLHdc, &pfd)) == 0) {
        Tcl_SetResult(
            togl->Interp, "Togl: couldn't choose pixel format", TCL_STATIC);
        return DUMMY_WINDOW;
    }
    ...
    if (togl->RgbaFlag) {
        ...
        if (togl->PrivateCmapFlag) {
            ...
            cmap = XCreateColormap(dpy,
                                   XRootWindow(dpy, visinfo->screen),
                                   visinfo->visual,
                                   AllocAll);
#elif defined(WIN32)
    ...
    if (togl->ShareContext && FindTogl(togl->ShareContext)) {
        ...
        */
        ...
        if ((togl->aglCtx = aglCreateContext(fmt, shareCtx)) == NULL) {
            ...
            Tcl_SetResult(
                togl->Interp, "Togl: couldn't create context", TCL_STATIC);
            return DUMMY_WINDOW;
        }
        ...
        if (!aglSetDrawable(togl->aglCtx,
            ...
            Tcl_SetResult(
                togl->Interp, "Togl: couldn't set drawable", TCL_STATIC);
            return DUMMY_WINDOW;
        }
...
static void Togl_EventProc(ClientData clientData, XEvent *eventPtr)
{
    ...
    case ConfigureNotify:
        if (togl->Width != Tk_Width(togl->TkWin) ||
            ...
            togl->Height = Tk_Height(togl->TkWin);
            XResizeWindow(Tk_Display(togl->TkWin),
                          Tk_WindowId(togl->TkWin),
                          togl->Width,
                          togl->Height);
#if defined(X11)
 ...
 */
static void noFaultXAllocColor(
    Display *dpy, Colormap cmap, int cmapSize, XColor *color, int *exact)
{
...
Togl_AllocColor(const struct Togl *togl, float red, float green, float blue)
{
...
#if defined(X11)
    {
        ...
        noFaultXAllocColor(Tk_Display(togl->TkWin),
                           Tk_Colormap(togl->TkWin),
                           Tk_Visual(togl->TkWin)->map_entries,
                           &xcol,
                           &exact);
...
#if defined(X11)
    XFreeColors(
        Tk_Display(togl->TkWin), Tk_Colormap(togl->TkWin), &pixel, 1, 0);
#elif defined(WIN32)
...
int Togl_BitmapFontMetrics(const struct Togl *togl,
                           const char *fontname,
                           int *width,
                           int *linespace)
{
...
GLuint Togl_LoadBitmapFont(const struct Togl *togl, const char *fontname)
{
    ...
    */
    ...
    aglUseFont(togl->aglCtx,
               1,
               0,
               14, /* for now, only app font, regular 14-point */
               10,
               118,
               fontbase + first);
#endif
        ...
        for (r = 0; r < RLEVELS; r++)
            for (g = 0; g < GLEVELS; g++)
                for (b = 0; b < BLEVELS; b++) {
                    ...
                    noFaultXAllocColor(
                        display, colormap, ncolors, &xcol, &exact);
...
void Togl_StereoFrustum(GLfloat left,
                        GLfloat right,
                        GLfloat bottom,
                        GLfloat top,
                        GLfloat near,
