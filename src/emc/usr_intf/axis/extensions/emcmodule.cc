...
struct pyIniFile {
    PyObject_HEAD IniFile *i;
};

struct pyStatChannel {
    PyObject_HEAD RCS_STAT_CHANNEL *c;
    EMC_STAT status;
};
...
{
    char *inifile;
    if (!PyArg_ParseTuple(a, "s", &inifile))
        return -1;
...
static PyObject *Ini_find(pyIniFile *self, PyObject *args)
{
    const char *s1, *s2;
    int num = 1;
    if (!PyArg_ParseTuple(args, "ss|i:find", &s1, &s2, &num))
        return NULL;
...
static PyObject *Ini_findall(pyIniFile *self, PyObject *args)
{
    const char *s1, *s2;
    int num = 1;
    if (!PyArg_ParseTuple(args, "ss:findall", &s1, &s2))
        return NULL;
...
static PyMethodDef Ini_methods[] = {
    {"find",
     (PyCFunction)Ini_find,
     METH_VARARGS, "Find value in inifile as string.  This uses the ConfigParser-style "
     "(section,option) order, not the linuxcnc order."},
    {"findall",
     (PyCFunction)Ini_findall,
     METH_VARARGS, "Find value in inifile as a list.  This uses the ConfigParser-style "
     "(section,option) order, not the linuxcnc order."},
    {}
};

static PyTypeObject Ini_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "linuxcnc.ini", /*tp_name*/
...
static RCS_STATUS emcWaitCommandComplete(pyCommandChannel *s, double timeout)
{
    ...
    do {
        ...
        if (s->s->peek() == EMC_STAT_TYPE) {
            ...
            if (serial_diff == 0 && (stat->status == RCS_STATUS::DONE ||
...
static int emcSendCommand(pyCommandChannel *s, RCS_CMD_MSG &cmd)
{
    ...
    while (etime() - start < EMC_COMMAND_TIMEOUT) {
        EMC_STAT *stat = (EMC_STAT *)s->s->get_address();
        int serial_diff = stat->echo_serial_number - s->serial;
        if (s->s->peek() == EMC_STAT_TYPE && serial_diff >= 0) {
            return 0;
        }
...
static bool check_stat(RCS_STAT_CHANNEL *emcStatusBuffer)
{
    if (!emcStatusBuffer->valid()) {
        PyErr_Format(error,
                     "emcStatusBuffer invalid err=%d",
                     emcStatusBuffer->error_type);
        return false;
    }
    return true;
}
    ...
    */
    ...
    if (!initialized) {
        PyErr_Format(PyExc_ValueError,
                     "toolinfo: NOT READY (initial poll reqd)\n");
        return NULL;
    }
    if (!PyArg_ParseTuple(o, "i", &toolno))
        return NULL;
...
#define TOOL_0_EXCEPTION
    ...
    */
    if (toolno == 0) {
        PyErr_Format(
            PyExc_ValueError,
            "toolinfo: for tool in spindle: use linuxnc.stat.tool_table[0]");
        return NULL;
    }
#endif
    ...
    if (tooldata_get(&tdata, idx) != IDX_OK) {
        PyErr_Format(
            PyExc_ValueError, "toolinfo: NO tooldata for toolno=%d", toolno);
        return NULL;
    }
    ...
    PyDict_SetItemString(
        res, "comment", o = PyUnicode_FromString(tdata.comment));
    Py_DECREF(o);
...
static PyMethodDef Stat_methods[] = {
    ...
    {"toolinfo",
     (PyCFunction)toolinfo,
     METH_VARARGS, "toolinfo(toolnumber):\n"
...
static PyMemberDef Stat_members[] = {
    // stat
    {(char *)"echo_serial_number",
     T_INT, O(echo_serial_number),
     READONLY, NULL},
    {(char *)"echo_serial_number",
     T_INT, O(echo_serial_number),
     READONLY, NULL},
    ...
    {(char *)"task_state",
     T_INT, O(task.state),
     READONLY, "Current Task state.  Possible values:\n"
    ...
    {(char *)"interpreter_errcode",
     T_INT, O(task.interpreter_errcode),
     READONLY, NULL},
    {(char *)"optional_stop",
     T_BOOL, O(task.optional_stop_state),
     READONLY, NULL},
    {(char *)"block_delete",
     T_BOOL, O(task.block_delete_state),
     READONLY, NULL},
    ...
    {(char *)"ini_filename",
     T_STRING_INPLACE, O(task.ini_filename),
     READONLY, NULL},
    {(char *)"delay_left", T_DOUBLE, O(task.delayLeft), READONLY, NULL},
    {(char *)"queued_mdi_commands",
     T_INT, O(task.queuedMDIcommands),
     READONLY, (char *)"Number of MDI commands queued waiting to run."},
    ...
    {(char *)"linear_units",
     ...
     READONLY, NULL},
    {(char *)"angular_units",
     T_DOUBLE, O(motion.traj.angularUnits),
     READONLY, NULL},
    ...
    {(char *)"motion_mode",
     T_INT, O(motion.traj.mode),
     READONLY, (char *)"The current mode of the Motion controller.  One of "
             "TRAJ_MODE_FREE,\n"
    ...
    {(char *)"acceleration",
     T_DOUBLE, O(motion.traj.acceleration),
     READONLY, NULL},
    {(char *)"max_velocity",
     T_DOUBLE, O(motion.traj.maxVelocity),
     READONLY, NULL},
    {(char *)"max_acceleration",
     T_DOUBLE, O(motion.traj.maxAcceleration),
     READONLY, NULL},
    {(char *)"probe_tripped",
    ...
    {(char *)"kinematics_type",
     T_INT, O(motion.traj.kinematics_type),
     READONLY, NULL},
    {(char *)"motion_type",
     T_INT, O(motion.traj.motion_type),
     READONLY, (char *)"The type of the currently executing motion (one of "
             "MOTION_TYPE_TRAVERSE,\n"
             "MOTION_TYPE_FEED, MOTION_TYPE_ARC, MOTION_TYPE_TOOLCHANGE,\n"
             "MOTION_TYPE_PROBING, or MOTION_TYPE_INDEXROTARY), or 0 if no "
             "motion is\n"
             "currently taking place."},
    {(char *)"distance_to_go",
     T_DOUBLE, O(motion.traj.distance_to_go),
     READONLY, NULL},
    {(char *)"current_vel",
     T_DOUBLE, O(motion.traj.current_vel),
     READONLY, NULL},
    {(char *)"feed_override_enabled",
     T_BOOL, O(motion.traj.feed_override_enabled),
     READONLY, NULL},
    {(char *)"adaptive_feed_enabled",
     T_BOOL, O(motion.traj.adaptive_feed_enabled),
     READONLY, NULL},
    {(char *)"feed_hold_enabled",
     T_BOOL, O(motion.traj.feed_hold_enabled),
     READONLY, NULL},
    {(char *)"num_extrajoints",
     T_INT, O(motion.numExtraJoints),
     READONLY, NULL},
    ...
    {(char *)"pocket_prepped",
     T_INT, O(io.tool.pocketPrepped),
     READONLY, (char *)"The index into the stat.tool_table list of the tool currently "
             "prepped for\n"
             "tool change, or -1 no tool is prepped.  On a Random toolchanger "
             "this is the\n"
             "same as the tool's pocket number.  On a Non-random toolchanger "
             "it's a random\n"
             "small integer."},
    {(char *)"tool_in_spindle",
     T_INT, O(io.tool.toolInSpindle),
     READONLY, (char *)"The tool number of the currently loaded tool, or 0 if no tool is "
             "loaded."},
    {(char *)"tool_from_pocket",
     T_INT, O(io.tool.toolFromPocket),
     READONLY, (char *)"The pocket number that the currently loaded tool was retrieved "
             "from,\n"
...
static PyObject *Stat_joint_position(pyStatChannel *s, void *)
{
    ...
    for (int i = 0; i < EMCMOT_MAX_JOINTS; i++) {
        PyTuple_SetItem(
            res, i, PyFloat_FromDouble(s->status.motion.joint[i].output));
    }
    return res;
}
...
static PyObject *Stat_joint_actual(pyStatChannel *s, void *)
{
    ...
    for (int i = 0; i < EMCMOT_MAX_JOINTS; i++) {
        PyTuple_SetItem(
            res, i, PyFloat_FromDouble(s->status.motion.joint[i].input));
    }
    return res;
}
...
static PyObject *Stat_homed(pyStatChannel *s, void *)
{
    ...
    for (int i = 0; i < EMCMOT_MAX_JOINTS; i++) {
        PyTuple_SET_ITEM(
            res, i, PyLong_FromLong(s->status.motion.joint[i].homed));
    }
    return res;
}
...
static PyGetSetDef Stat_getsetlist[] = {
    ...
    {(char *)"homed",
     (getter)Stat_homed,
     (setter)NULL,
     (char *)"An array of integers indicating the 'homed' status of each joint "
             "(0 or 1).", NULL},
    ...
    {(char *)"joint_actual_position",
     (getter)Stat_joint_actual,
     NULL, NULL,
     NULL},
    {(char *)"probed_position", (getter)Stat_probed, NULL, NULL, NULL},
    {(char *)"settings",
     (getter)Stat_activesettings,
     (setter)NULL,
     (char
          *)"This is an array containing the Interp active settings: sequence "
            "number,\n"
    ...
    {(char *)"tool_table",
     (getter)Stat_tool_table,
     (setter)NULL,
     (char *)"The tooltable, expressed as a list of tools.  Each tool is a "
             "dict with the\n"
...
static PyTypeObject Stat_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "linuxcnc.stat", /*tp_name*/
    ...
    /*tp_flags*/         // need to make sure we need Py_TPFLAGS_BASETYPE here
    0,                   /*tp_doc*/
    0,                   /*tp_traverse*/
    0,                   /*tp_clear*/
    0,                   /*tp_richcompare*/
...
static int
Command_init(pyCommandChannel *self, PyObject * /*a*/, PyObject * /*k*/)
{
    const char *file = get_nmlfile();
    if (file == NULL)
        return -1;
...
static PyObject *mode(pyCommandChannel *s, PyObject *o)
{
    ...
    case EMC_TASK_MODE::AUTO: break;
    default:
        PyErr_Format(PyExc_ValueError,
                     "Mode should be MODE_MDI, MODE_MANUAL, or MODE_AUTO");
        return NULL;
    }
    emcSendCommand(s, m);
    Py_INCREF(Py_None);
...
static PyObject *spindleoverride(pyCommandChannel *s, PyObject *o)
{
    ...
    m.spindle = 0;
    if (!PyArg_ParseTuple(o, "d|i", &m.scale, &m.spindle))
        return NULL;
    emcSendCommand(s, m);
    Py_INCREF(Py_None);
    return Py_None;
...
static PyObject *spindle(pyCommandChannel *s, PyObject *o)
{
    ...
    int arg3 = 0;
    if (!PyArg_ParseTuple(o, "i|ddi", &dir, &arg1, &arg2, &arg3))
        return NULL;
    switch (dir) {
    ...
    default:
        PyErr_Format(
            PyExc_ValueError,
            "Spindle direction should be SPINDLE_FORWARD, SPINDLE_REVERSE, "
            "SPINDLE_OFF, SPINDLE_INCREASE, SPINDLE_DECREASE, or "
            "SPINDLE_CONSTANT");
        return NULL;
    }
    Py_INCREF(Py_None);
...
static PyObject *mdi(pyCommandChannel *s, PyObject *o)
{
    ...
    if (unsigned(len) > sizeof(m.command) - 1) {
        PyErr_Format(PyExc_ValueError,
                     "MDI commands limited to %zu characters",
                     sizeof(m.command) - 1);
        return NULL;
    }
...
static PyObject *state(pyCommandChannel *s, PyObject *o)
{
    ...
    if (!PyArg_ParseTuple(o, "i", &m.state))
        return NULL;
    switch (m.state) {
    case EMC_TASK_STATE::ESTOP:
    case EMC_TASK_STATE::ESTOP_RESET:
    case EMC_TASK_STATE::ON:
    case EMC_TASK_STATE::OFF: break;
    default:
        PyErr_Format(
            PyExc_ValueError,
            "Machine state should be STATE_ESTOP, STATE_ESTOP_RESET, STATE_ON, "
            "or STATE_OFF");
        return NULL;
    }
    emcSendCommand(s, m);
...
static PyObject *brake(pyCommandChannel *s, PyObject *o)
{
    int dir;
    int spindle = 0;
    if (!PyArg_ParseTuple(o, "i|i", &dir, &spindle))
        return NULL;
    switch (dir) {
    ...
    default:
        PyErr_Format(PyExc_ValueError,
                     "BRAKE should be BRAKE_ENGAGE or BRAKE_RELEASE");
        return NULL;
    }
    Py_INCREF(Py_None);
    return Py_None;
...
static PyObject *program_open(pyCommandChannel *s, PyObject *o)
{
    ...
    if (unsigned(len) > sizeof(m.file) - 1) {
        PyErr_Format(PyExc_ValueError,
                     "File name limited to %zu characters",
                     sizeof(m.file) - 1);
        return NULL;
    }
    strcpy(m.file, file);
    ...
    if (s->s->cms->ProcessType == CMS_REMOTE_TYPE &&
        strcmp(s->s->cms->ProcessName, "emc") != 0) {
        /* open file */
        FILE *fd;
        if (!(fd = fopen(file, "r"))) {
            PyErr_Format(
                PyExc_OSError, "fopen(%s) error: %s", file, strerror(errno));
        ...
        if (fseek(fd, 0L, SEEK_END) != 0) {
            fclose(fd);
            PyErr_Format(
                PyExc_OSError, "fseek(%s) error: %s", file, strerror(errno));
            return PyErr_SetFromErrno(PyExc_OSError);
        }
        long ftpos = ftell(fd);
        ...
        if (ftpos < 0) {
            fclose(fd);
            PyErr_Format(
                PyExc_OSError, "ftell(%s) error: %s", file, strerror(errno));
            return PyErr_SetFromErrno(PyExc_OSError);
        }
        ...
        while (!(feof(fd))) {
            size_t bytes_read =
                fread(&m.remote_buffer, 1, sizeof(m.remote_buffer), fd);
            /* read error? */
...
static PyObject *emcauto(pyCommandChannel *s, PyObject *o)
{
    ...
    if (PyArg_ParseTuple(o, "ii", &fn, &run.line) && fn == LOCAL_AUTO_RUN) {
        ...
        if (!PyArg_ParseTuple(o, "i", &fn))
            return NULL;
        switch (fn) {
        case LOCAL_AUTO_PAUSE: emcSendCommand(s, pause); break;
        case LOCAL_AUTO_RESUME: emcSendCommand(s, resume); break;
        case LOCAL_AUTO_STEP: emcSendCommand(s, step); break;
        case LOCAL_AUTO_REVERSE: emcSendCommand(s, reverse); break;
        case LOCAL_AUTO_FORWARD: emcSendCommand(s, forward); break;
        default:
...
static PyObject *set_traj_mode(pyCommandChannel *s, PyObject *o)
{
    ...
    if (!PyArg_ParseTuple(o, "i", &mo.mode))
        return NULL;
...
static PyMemberDef Command_members[] = {
    {(char *)"serial",
     T_INT, offsetof(pyCommandChannel, serial),
     READONLY, NULL},
    {}
};
...
static PyMethodDef Command_methods[] = {
    ...
    {"state",
     (PyCFunction)state,
     METH_VARARGS, "state(NEW_STATE) - Set the machine E-Stop & Power-On state.\n"
     "Possible values for `NEW_STATE` are:\n"
     "    STATE_ESTOP: Power off and enter E-Stop mode.\n"
     "    STATE_ESTOP_RESET: Reset (leave) E-Stop mode, but remain powered "
     "off.\n"
    ...
    {"set_spindle_override",
     (PyCFunction)set_spindle_override,
     METH_VARARGS, NULL},
    ...
    {"error_msg",
     (PyCFunction)error_msg,
     METH_VARARGS, "Send operator error message"},
    {"text_msg",
     (PyCFunction)text_msg,
     METH_VARARGS, "Send operator text message"},
    {"display_msg",
     (PyCFunction)display_msg,
     METH_VARARGS, "Send operator display message"},
    {}
};

static PyTypeObject Command_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "linuxcnc.command", /*tp_name*/
...
static PyTypeObject Error_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "linuxcnc.error_channel", /*tp_name*/
...
static void vertex9(const double pt[9], double p[3], const char *geometry)
{
    ...
    for (; *geometry; geometry++) {
        ...
        case '-': sign = -1; break;
        case 'X':
            translate(p, pt[0] * sign, 0, 0);
            sign = 1;
            break;
        case 'Y':
            translate(p, 0, pt[1] * sign, 0);
            sign = 1;
            break;
        case 'Z':
            translate(p, 0, 0, pt[2] * sign);
            sign = 1;
            break;
        case 'U':
            translate(p, pt[6] * sign, 0, 0);
            sign = 1;
            break;
        case 'V':
            translate(p, 0, pt[7] * sign, 0);
            sign = 1;
            break;
        case 'W':
            translate(p, 0, 0, pt[8] * sign);
            sign = 1;
            break;
        case 'A':
            if (roffsets.axis_mask & AXIS_MASK_A) {
                rotate_x(p, pt[3] * sign);
            }
            sign = 1;
            break;
        case 'B':
            if (roffsets.axis_mask & AXIS_MASK_B) {
                rotate_y(p, pt[4] * sign);
            }
            sign = 1;
            break;
        case 'C':
            if (roffsets.axis_mask & AXIS_MASK_C) {
...
static void line9(const double p1[9], const double p2[9], const char *geometry)
{
    if (p1[3] != p2[3] || p1[4] != p2[4] || p1[5] != p2[5]) {
        double dc = std::max(
            {fabs(p2[3] - p1[3]), fabs(p2[4] - p1[4]), fabs(p2[5] - p1[5])});
...
static PyObject *pygui_respect_offsets(PyObject * /*s*/, PyObject *o)
{
    ...
    if (roffsets.respect_offsets) {
        ...
        if (strchr(coords, 'A'))
            roffsets.axis_mask |= AXIS_MASK_A;
        if (strchr(coords, 'B'))
            roffsets.axis_mask |= AXIS_MASK_B;
        if (strchr(coords, 'C'))
            roffsets.axis_mask |= AXIS_MASK_C;
    }
...
static PyObject *pydraw_lines(PyObject * /*s*/, PyObject *o)
{
    ...
    for (i = 0; i < PyList_GET_SIZE(li); i++) {
        ...
        if (!PyArg_ParseTuple(it,
                              "i(ddddddddd)(ddddddddd)|OOO",
                              &n,
                              p1 + 0,
                              p1 + 1,
                              p1 + 2,
        ...
        if (first || memcmp(p1, pl, sizeof(p1)) || (for_selection && n != nl)) {
            if (!first)
                glEnd();
...
static PyObject *pydraw_dwells(PyObject * /*s*/, PyObject *o)
{
    ...
    for (i = 0; i < PyList_GET_SIZE(li); i++) {
        ...
        int axis;
        if (!PyArg_ParseTuple(
                it, "i(ddd)dddi", &n, &red, &green, &blue, &x, &y, &z, &axis)) {
            return NULL;
        }
        if (for_selection != 1)
...
typedef struct {
    PyObject_HEAD int npts, mpts, lpts;
...
static inline bool colinear(float xa,
                            float ya,
                            float za,
                            float xb,
                            float yb,
...
static int Logger_init(pyPositionLogger *self, PyObject *a, PyObject * /*k*/)
{
    ...
    if (!PyArg_ParseTuple(a,
                          "O!(BBBB)(BBBB)(BBBB)(BBBB)(BBBB)(BBBB)s|i",
                          &Stat_Type,
                          &self->st,
                          &c[0].r,
                          &c[0].g,
...
{
    double z, w;
    if (!PyArg_ParseTuple(o, "dd:logger.set_depth", &z, &w))
        return NULL;
    s->foam_z = z;
    s->foam_w = w;
    Py_INCREF(Py_None);
...
static PyObject *Logger_start(pyPositionLogger *s, PyObject *o)
{
    ...
    if (!PyArg_ParseTuple(o, "d:logger.start", &interval))
        return NULL;
    ...
    Py_BEGIN_ALLOW_THREADS while (!s->exit)
    {
        if (s->clear) {
            s->npts = 0;
            s->lpts = 0;
        ...
        if (s->st->c->valid() && s->st->c->peek() == EMC_STAT_TYPE) {
            ...
            if (colornum < 0 || colornum >= NUMCOLORS)
                colornum = 0;
            ...
            if (s->is_xyuv) {
                x = status->motion.traj.position.tran.x -
                    status->task.toolOffset.tran.x,
                y = status->motion.traj.position.tran.y -
                    status->task.toolOffset.tran.y,
                z = s->foam_z;
                ...
                add_point = add_point || !colinear(rx,
                                                   ry,
                                                   rz,
                                                   op->rx,
                                                   op->ry,
                ...
                double pt[9] = {
                    status->motion.traj.position.tran.x -
                        status->task.toolOffset.tran.x,
                    status->motion.traj.position.tran.y -
                        status->task.toolOffset.tran.y,
                    status->motion.traj.position.tran.z -
                        status->task.toolOffset.tran.z,
            ...
            if (add_point) {
                ...
                if (s->npts + 2 > s->mpts) {
                    ...
                    if (s->mpts >= MAX_POINTS) {
                        ...
                        s->mpts = 2 * s->mpts + 2;
                        s->changed = 1;
                        s->p = (struct logger_point *)realloc(
                            s->p, sizeof(struct logger_point) * s->mpts);
                    }
                    UNLOCK();
                    op = &s->p[s->npts - 1];
                    oop = &s->p[s->npts - 2];
    ...
    Py_END_ALLOW_THREADS Py_DECREF(s->st);
    Py_INCREF(Py_None);
    return Py_None;
}
...
static PyObject *Logger_call(pyPositionLogger *s, PyObject * /*o*/)
{
    if (!s->clear) {
        ...
        if (s->is_xyuv) {
            if (s->changed) {
                glVertexPointer(
                    3, GL_FLOAT, sizeof(struct logger_point) / 2, &s->p->x);
                glColorPointer(4,
                               GL_UNSIGNED_BYTE,
                               sizeof(struct logger_point) / 2,
                               &s->p->c);
            ...
            if (s->changed) {
                glVertexPointer(
                    3, GL_FLOAT, sizeof(struct logger_point), &s->p->x);
                glColorPointer(
                    4, GL_UNSIGNED_BYTE, sizeof(struct logger_point), &s->p->c);
                glEnableClientState(GL_COLOR_ARRAY);
                glEnableClientState(GL_VERTEX_ARRAY);
                s->changed = 0;
            }
...
{
    int flag = 1;
    if (!PyArg_ParseTuple(o, "|i:emc.positionlogger.last", &flag))
        return NULL;
    PyObject *result = NULL;
    LOCK();
...
static PyMethodDef Logger_methods[] = {
    {"start",
     (PyCFunction)Logger_start,
     METH_VARARGS, "Start the position logger and run every ARG seconds"},
    {"clear",
     (PyCFunction)Logger_clear,
     METH_NOARGS, "Clear the position logger"},
    {"stop", (PyCFunction)Logger_stop, METH_NOARGS, "Stop the position logger"},
    {"call", (PyCFunction)Logger_call, METH_NOARGS, "Plot the backplot now"},
    {"set_depth",
     (PyCFunction)Logger_set_depth,
     METH_VARARGS, "set the Z and W depths for foam cutter"},
    {"set_colors",
     (PyCFunction)Logger_set_colors,
     METH_VARARGS, "set the plotting colors"},
    {"get_colors",
     (PyCFunction)Logger_get_colors,
     METH_NOARGS, "get the plotting colors"},
    {"last",
     (PyCFunction)Logger_last,
     METH_VARARGS, "Return the most recent point on the plot or None"},
    {},
};

static PyTypeObject PositionLoggerType = {
    PyVarObject_HEAD_INIT(NULL, 0) "linuxcnc.positionlogger", /*tp_name*/
...
#define METH(name, doc) {#name, (PyCFunction)py##name, METH_VARARGS, doc}
    ...
    METH(line9,
         "Draw a single line in the 'rs274.glcanon' format; assumes "
         "glBegin(GL_LINES)"),
...
PyMODINIT_FUNC PyInit_linuxcnc(void)
{
    ...
    error =
        PyErr_NewException((char *)"linuxcnc.error", PyExc_RuntimeError, NULL);
    ...
    PyModule_AddIntConstant(
        m, "STATE_ESTOP_RESET", (int)EMC_TASK_STATE::ESTOP_RESET);
    ...
    PyModule_AddIntConstant(
        m, "EXEC_WAITING_FOR_MOTION", (int)EMC_TASK_EXEC::WAITING_FOR_MOTION);
    PyModule_AddIntConstant(m,
                            "EXEC_WAITING_FOR_MOTION_QUEUE",
                            (int)EMC_TASK_EXEC::WAITING_FOR_MOTION_QUEUE);
    PyModule_AddIntConstant(
        m, "EXEC_WAITING_FOR_IO", (int)EMC_TASK_EXEC::WAITING_FOR_IO);
    PyModule_AddIntConstant(m,
                            "EXEC_WAITING_FOR_MOTION_AND_IO",
                            (int)EMC_TASK_EXEC::WAITING_FOR_MOTION_AND_IO);
    PyModule_AddIntConstant(
        m, "EXEC_WAITING_FOR_DELAY", (int)EMC_TASK_EXEC::WAITING_FOR_DELAY);
    PyModule_AddIntConstant(m,
                            "EXEC_WAITING_FOR_SYSTEM_CMD",
                            (int)EMC_TASK_EXEC::WAITING_FOR_SYSTEM_CMD);
    PyModule_AddIntConstant(m,
                            "EXEC_WAITING_FOR_SPINDLE_ORIENTED",
                            (int)EMC_TASK_EXEC::WAITING_FOR_SPINDLE_ORIENTED);
