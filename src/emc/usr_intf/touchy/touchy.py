...
class touchy:
    def __init__(self, inifile):
        ...
        self.dro_font_name = self.prefs.getpref(
            "dro_font", "FreeMono 10 Pitch Bold 16", str
        )
        ...
        self.listing = listing.listing(
            Gtk, linuxcnc, listing_labels, listing_eventboxes
        )

        # emc interface
        self.linuxcnc = emc_interface.emc_control(
            linuxcnc, self.listing, self.wTree.get_object("error")
        )
        self.linuxcnc.continuous_jog_velocity(self.mv_val)
        self.hal = hal_interface.hal_interface(
            self, self.linuxcnc, self.mdi_control, linuxcnc
        )
        ...
        for i in range(self.num_filechooser_labels):
            ...
            filechooser_eventboxes.append(
                self.wTree.get_object("eventbox_filechooser%d" % i)
            )
        self.filechooser = filechooser.filechooser(
            Gtk, linuxcnc, filechooser_labels, filechooser_eventboxes, self.listing
        )
        ...
        for i in [
            ...
            if w:
                ...
                if "r" in i and not self.rel_textcolor == "default":
                    w.modify_fg(
                        Gtk.StateFlags.NORMAL, Gdk.color_parse(self.rel_textcolor)
                    )
                elif "a" in i and not self.abs_textcolor == "default":
                    w.modify_fg(
                        Gtk.StateFlags.NORMAL, Gdk.color_parse(self.abs_textcolor)
                    )
                elif "d" in i and not self.dtg_textcolor == "default":
                    w.modify_fg(
                        Gtk.StateFlags.NORMAL, Gdk.color_parse(self.dtg_textcolor)
                    )
