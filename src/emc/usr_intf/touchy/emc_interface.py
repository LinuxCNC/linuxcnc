...
class emc_control:
    ...
    def continuous_jog_velocity(self, velocity):
        ...
        for i in range(9):
            if self.isjogging[i]:
                self.emccommand.jog(
                    self.emc.JOG_CONTINUOUS, 0, i, self.isjogging[i] * self.jog_velocity
                )
    ...
    def continuous_jog(self, axis, direction):
        ...
        if direction == 0:
            ...
            self.emccommand.jog(
                self.emc.JOG_CONTINUOUS, 0, axis, direction * self.jog_velocity
            )
...
class emc_status:
    ...
    def periodic(self):
        ...
        set_text(
            self.status["spindledir"], sd[self.emcstat.spindle[0]["direction"] + 1]
        )
        ...
        tt = ""
        for p, t in zip(
            list(range(len(self.emcstat.tool_table))), self.emcstat.tool_table
        ):
            if t.id != -1:
        ...
        if lathe:
            set_text(
                self.status["tlo"],
                "X:%.4f Z:%.4f"
                % (self.emcstat.tool_offset[0], self.emcstat.tool_offset[2]),
            )
        else:
        ...
        for i in range(len(self.emcstat.g5x_offset)):
            letter = "XYZABCUVW"[i]
            if self.emcstat.g5x_offset[i] != 0:
                g5x += "%s%.4f " % (letter, self.emcstat.g5x_offset[i])
            if self.emcstat.g92_offset[i] != 0:
                g92 += "%s%.4f " % (letter, self.emcstat.g92_offset[i])
        ...
        if self.emcstat.motion_id == 0 and (
            ...
            self.listing.highlight_line(
                self.emcstat.motion_id or self.emcstat.motion_line
            )
