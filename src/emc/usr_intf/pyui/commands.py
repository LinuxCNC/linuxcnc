...
def load_handlers(usermod, halcomp, builder, commands, master):
    ...
    for u in usermod:
        ...
        try:
            ...
            if h and callable(h):
                DBG(
                    "panelui: module '%s' : '%s' function found"
                ...
                DBG(
                    "panelui: module '%s': no '%s' function - registering only functions as callbacks"
                    % (mod.__name__, hdl_func)
                )
                objlist = [mod]
            ...
            for object in objlist:
                ...
                for method, f in methods:
                    ...
                    if callable(f):
                        DBG(
                            "panelui: Register callback '%s' in %s" % (method, basename)
                        )
                        add_handler(method, f)
        except Exception as e:
            print(
                "**** PANELUI ERROR: trouble looking for handlers in '%s': %s"
                % (basename, e)
            )
...
class CNC_COMMANDS:
    def __init__(self, master):
        ...
        try:
            handlers, self.handler_module, self.handler_instance = load_handlers(
                [CONFIGDIR], self.emcstat, self.emccommand, self, master
            )
        except Exception as e:
            print(e)
        ...
        if velocity is not None:
            ...
            for axisnum in (0, 1, 2, 6, 7, 8):
                if self.isjogging[axisnum]:
                    ...
                    self.emccommand.jog(
                        self.emc.JOG_CONTINUOUS,
                        jjogmode,
                        j_or_a,
                        self.isjogging[axisnum] * rate,
                    )
    ...
    def continuous_jog(self, wname, cmd):
        ...
        if direction == 0:
            ...
            self.emccommand.jog(
                self.emc.JOG_CONTINUOUS, jjogmode, j_or_a, direction * rate
            )
    ...
    def incremental_jog(self, wname, cmd):
        ...
        if axisnum in (3, 4, 5):
            ...
            rate = self.jog_velocity
        self.emccommand.jog(
            self.emc.JOG_INCREMENT, jjogmode, j_or_a, direction * rate, distance
        )
        self.isjogging[axisnum] = 0
    ...
    def ok_for_mdi(self):
        ...
        return (
            ...
            and s.enabled
            and s.homed
            and (s.interp_state == self.emc.INTERP_IDLE)
        )
