...
class keyboard:
    ...
    def build(self, dbg_state=0):
        ...
        def build_widget(widget, array, idname):
            ...
            widget.hal_init(
                self, self.hal, idname, metadata, self.cmd, self.widgets, DBG_state
            )
        ...
        for basewidget in self.config:
            if basewidget == "HAL_PREFIX":
                continue
            if basewidget == "ZMQ_SETUP":
                continue
                ...
                for idname in self.config[basewidget]:
                    ...
                    if basewidget == "TOGGLE_BUTTONS":
                        ...
                        for group in self.config[basewidget][idname]:
                            ...
                            build_widget(
                                widget2, self.config[basewidget][idname][group], group
                            )
                        DBG(group_list, 2)
    ...
    def update(self, *arg):
        ...
        try:
            ...
            if DBG_state > 1:
                ...
                print(
                    (
                        "PYUI ERROR: no button or command assigned to keycode %s?"
                        % button
                    )
                )
                print(e)
