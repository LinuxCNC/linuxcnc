...
def DBG(str):
    if not DBG_state or DBG_suppress:
        return
    print(str)
...
class _ToggleBase(_WidgetBase):
    def _hal_init(self):
        ...
        if pintype not in (None, "COMMAND", "ZMQ"):
            ...
            self.hal_pin_not = self.hal.newpin(
                self.hal_name + "-not", pintype, hal.HAL_OUT
            )
        # If there is a status requested make there pins
        if self.status_pin:
            self.hal_status_pin = self.hal.newpin(
                self.hal_name + "-state", hal.HAL_BIT, hal.HAL_OUT
            )
            self.hal_status_pin_not = self.hal.newpin(
                self.hal_name + "-state-not", hal.HAL_BIT, hal.HAL_OUT
            )
