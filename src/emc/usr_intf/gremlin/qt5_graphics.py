...
class Lcnc_3dGraphics(QGLWidget,  glcanon.GlCanonDraw, glnav.GlNavBase):
    ...
    def dro_format(self,s,spd,dtg,limit,homed,positions,axisdtg,g5x_offset,g92_offset,tlo_offset):
        ...
        if self.metric_units:
            mu_format = "% 6s:% 9.3f"
            if self.show_dtg:
                droformat = " " + mu_format + "  DTG %1s:% 9.3f"
            else:
                droformat = " " + mu_format
            ...
            mu_format = "% 6s:% 9.4f"
            if self.show_dtg:
                droformat = " " + mu_format + "  DTG %1s:% 9.4f"
            else:
                droformat = " " + mu_format
        ...
        for i in range(9):
            a = "XYZABCUVW"[i]
            if s.axis_mask & (1 << i):
                posstrs.append(mu_format % (a, positions[i]))
                if self.show_dtg:
        ...
        if self.is_lathe():
            ...
            if self.show_lathe_radius:
                posstrs.insert(1, mu_format % ("Rad", positions[0]))
            else:
                posstrs.insert(1, mu_format % ("Dia", positions[0]*2.0))
            droposstrs[0] = ""
            if self.show_dtg:
                if self.show_lathe_radius:
        ...
        if self.show_velocity:
            posstrs.append(mu_format % ("Vel", spd))
            pos = 0
            for i in range(9):
                if s.axis_mask & (1 << i):
                    pos += 1
            if self.is_lathe:
                pos += 1
            droposstrs.insert(pos, " " + mu_format % ("Vel", spd))

        if self.show_dtg:
            posstrs.append(mu_format % ("DTG", dtg))
