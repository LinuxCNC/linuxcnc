...
class MaterialConverter(QMainWindow, object):
    ...
    def __init__(self, parent=None):
        ...
        self.iconPath = (
            "share/icons/hicolor/scalable/apps/linuxcnc_alt/linuxcncicon_plasma.svg"
        )
        appPath = os.path.realpath(os.path.dirname(sys.argv[0]))
        self.iconBase = (
            "/usr"
            if appPath == "/usr/bin"
            else appPath.replace("/bin", "/debian/extras/usr")
        )
        ...
        self.fCM = "1"
        wid.setStyleSheet(
            "* {color: #ffee06; background: #16160e; font: 12pt DejaVuSans} \
    ...
    def on_convert_clicked(self):
        ...
        if self.inManual.isChecked():
            ...
            self.materialCutS = (
                f"CUT_SPEED          = {float(self.fCS) / self.divisor:.0f}"
            )
            ...
            with open(self.inFileName, "r") as f_in:
                ...
                for line in f_in:
                    if line.startswith("[Tool") and "Custom" not in line:
                        ...
                        b = line.split("=")[1]
                        self.materialNum = (
                            f'[MATERIAL_NUMBER_{b.strip().replace("]","")}]'
                        )
                    ...
                    elif line.startswith(r"Cut\ current") or (
                        line.startswith(r"Preset\ current")
                        and self.materialCutA == "CUT_AMPS           = 0"
                    ):
                        b = line.split("=", 1)[1]
                        self.materialCutA = f"CUT_AMPS           = {b.strip()}"
                    elif line.startswith(r"Cut\ voltage") or (
                        line.startswith(r"Preset\ volts")
                    ...
                    elif line.startswith(r"Gas\ pressure") or (
                        line.startswith(r"Preset\ Air\ Pressure")
                        and self.materialGasP == "GAS_PRESSURE       = 0"
                    ):
                        b = line.split("=", 1)[1]
                        self.materialGasp = f"GAS_PRESSURE       = {b.strip()}"
                    elif line.startswith(r"Cut\ mode") or (
                        line.startswith(r"Preset\ mode")
            ...
            for t in jdata["data"]:
                ...
                self.materialCutH = f"CUT_HEIGHT         = {float(self.fCH) / self.divisor:.{self.precision}f}"
                self.materialCutS = (
                    f"CUT_SPEED          = {float(lCS) / self.divisor:.0f}"
                )
                self.materialCutA = f"CUT_AMPS           = {self.fCA}"
                self.materialCutV = f"CUT_VOLTS          = {self.fCV}"
                self.materialPauseE = f"PAUSE_AT_END       = {self.fPE}"
    ...
    def fusion_dialog(self):
        ...
        dialog.setStyleSheet(
            "* { color: #ffee06; background: #16160e; font: 10pt DejaVuSans } \
