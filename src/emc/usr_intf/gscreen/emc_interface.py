...
class emc_control:
    ...
    def continuous_jog_velocity(self, velocity, angular=None):
        ...
        for i in range(9):
            if self.isjogging[i]:
                self.emccommand.jog(
                    self.emc.JOG_CONTINUOUS, 0, i, self.isjogging[i] * rate
                )
    ...
    def continuous_jog(self, axis, direction):
        ...
        if direction == 0:
            ...
            self.emccommand.jog(self.emc.JOG_CONTINUOUS, 0, axis, direction * rate)
    ...
    def incremental_jog(self, axis, direction, distance):
        ...
        if axis in (3, 4, 5):
            ...
            rate = self.jog_velocity
        self.emccommand.jog(self.emc.JOG_INCREMENT, 0, axis, direction * rate, distance)
        self.isjogging[axis] = 0
...
class emc_status:
    ...
    def periodic(self):
        ...
        if self.emcstat.pocket_prepped == -1:
            ...
            self.data.preppedtool = self.emcstat.tool_table[
                self.emcstat.pocket_prepped
            ].id
