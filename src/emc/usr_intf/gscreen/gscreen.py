...
try:
    ...
    LOG.warning(
        _("No audio alerts are available - Is gir1.2-gstreamer-1.0 package installed?")
    )
...
if not os.path.exists(soundsdir):
    try:
        ...
        if "mint" in distro.id().lower():
            ...
            if not os.path.exists(soundsdir):
                LOG.error(
                    f"Audio player - Mint sound file directory not found: {soundsdir}"
                )
                soundsdir = None
        else:
            LOG.error(
                f"Audio player - Could not find sounds directory for {distro.id()} distro"
            )
            soundsdir = None
    except:
        LOG.error(
            f"Audio player - Could not find sounds directory - Is python3-distro installed?"
        )
        soundsdir = None
...
class Widgets:
    ...
    def __getattr__(self, attr):
        ...
        if r is None:
            raise AttributeError(_("No widget '{}'").format(attr))
        return r

    def __getitem__(self, attr):
        r = self._xml.get_object(attr)
        if r is None:
            raise IndexError(_("No widget '{}'").format(attr))
        return r
    ...
    for u in usermod:
        ...
        try:
            ...
            for temp in ("periodic", "connect_signals", "initialize_widgets"):
                ...
                if h and callable(h):
                    LOG.info(
                        _("Module '{}': '{}' function found").format(mod.__name__, temp)
                    )
            ...
            if h and callable(h):
                LOG.info(
                    _("Module '{}': '{}' function found").format(mod.__name__, hdl_func)
                )
                ...
                LOG.debug(
                    _(
                        "Module '{}': no '{}' function - registering only functions as callbacks"
            ...
            for object in objlist:
                LOG.debug(
                    _("Registering handlers in module '{}' object '{}'").format(
                        mod.__name__, object
                    )
                )
                ...
                for method, f in methods:
                    ...
                    if callable(f):
                        LOG.debug(
                            ("Register callback '{}' in {}").format(method, basename)
                        )
...
class Gscreen:
    ...
    def __init__(self):
        ...
        if os.path.exists(localglade):
            ...
            localglade = os.path.join(
                SKINPATH, "%s/%s.glade" % (self.skinname, self.skinname)
            )
            if os.path.exists(localglade):
        ...
        if _AUDIO_AVAILABLE:
            try:
                ...
                LOG.warning(
                    _("Audio test failed - Is gstreamer0.10-plugins-base installed?")
                )
        ...
        self.data.axis_list = []
        for letter in temp:
            if letter.lower() in self.data.axis_list:
                continue
        ...
        if units == None:
            ...
            if units == None:
                self.add_alarm_entry(
                    _("No UNITS entry found in [TRAJ] or [AXIS_X] of INI file")
                )
        ...
        skin_handler_path = os.path.join(
            SKINPATH, "%s/%s" % (self.skinname, HANDLER_FN)
        )
        ...
        handlers, self.handler_module, self.handler_instance = load_handlers(
            temp, self.halcomp, self.xml, [], self
        )
        ...
        if increments:
            ...
            self.add_alarm_entry(
                _("No default jog increments entry found in [DISPLAY] of INI file")
            )
        ...
        if increments:
            if "," in increments:
                self.data.angular_jog_increments = [
                    i.strip() for i in increments.split(",")
                ]
            else:
        ...
        if temp:
            ...
            self.add_alarm_entry(
                _(
                    "No DEFAULT_LINEAR_VELOCITY entry found in [DISPLAY] of INI file: using internal default of %s"
                    % temp
                )
            )
        ...
        if temp:
            ...
            self.add_alarm_entry(
                _(
                    "No MAX_LINEAR_VELOCITY entry found in [DISPLAY] of INI file: using internal default of %s"
                    % temp
                )
            )
        ...
        if temp == None:
            self.add_alarm_entry(
                _("No MAX_LINEAR_VELOCITY found in [TRAJ] of the INI file")
            )
            temp = 1.0
        ...
        if (
            "a" in self.data.axis_list
            or "b" in self.data.axis_list
            or "c" in self.data.axis_list
        ):
            ...
            if temp:
                ...
                self.add_alarm_entry(
                    _(
                        "No DEFAULT_ANGULAR_VELOCITY entry found in [DISPLAY] of INI file: using internal default of %s"
                        % temp
                    )
                )
            ...
            if temp:
                ...
                self.add_alarm_entry(
                    _(
                        "No MAX_ANGULAR_VELOCITY entry found in [DISPLAY] of INI file: using internal default of %s"
                        % temp
                    )
                )
        ...
        if temp:
            ...
            self.add_alarm_entry(
                _("No MAX_SPINDLE_OVERRIDE entry found in [DISPLAY] of INI file")
            )
        ...
        if temp:
            ...
            self.add_alarm_entry(
                _("No MIN_SPINDLE_OVERRIDE entry found in [DISPLAY] of INI file")
            )
        ...
        if temp:
            ...
            self.add_alarm_entry(
                _("No MAX_FEED_OVERRIDE entry found in [DISPLAY] of INI file")
            )
        ...
        if not temp:
            self.add_alarm_entry(
                _("CYCLE_TIME in [DISPLAY] of INI file is missing: defaulting to 100ms")
            )
            temp = 100
        elif float(temp) < 50:
            self.add_alarm_entry(
    ...
    def init_dro_pref(self):
        ...
        """
        self.data.abs_textcolor = self.prefs.getpref(
            "abs_textcolor", "#0000FFFF0000", str
        )
        self.data.dtg_textcolor = self.prefs.getpref(
            "dtg_textcolor", "#00000000FFFF", str
        )
        self.data.rel_textcolor = self.prefs.getpref(
            "rel_textcolor", "#FFFF00000000", str
        )
    ...
    def init_theme_pref(self):
        ...
        self.data.theme_name = self.prefs.getpref(
            "gtk_theme", "Local Config Theme", str
        )
    ...
    def init_window_geometry_pref(self):
        ...
        self.data.window_geometry = self.prefs.getpref(
            "window_geometry", "default", str
        )
        self.data.window_max = self.prefs.getpref("window_force_max", False, bool)
        self.data.window2_geometry = self.prefs.getpref(
            "window2_geometry", "default", str
        )
    ...
    def init_keybinding_pref(self):
        ...
        Escape
        """
        self.keylookup.add_binding(
            "Right", self.prefs.getpref("Key_Right", "XPOS", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "Left", self.prefs.getpref("Key_Left", "XNEG", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "Up", self.prefs.getpref("Key_Up", "YPOS", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "Down", self.prefs.getpref("Key_Down", "YNEG", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "Page_Up", self.prefs.getpref("Key_Page_Up", "ZPOS", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "Page_Down", self.prefs.getpref("Key_Page_Down", "ZNEG", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "bracketleft",
            self.prefs.getpref("Key_bracketleft", "APOS", str, "KEYCODES"),
        )
        self.keylookup.add_binding(
            "bracketright",
            self.prefs.getpref("Key_bracketright", "ANEG", str, "KEYCODES"),
        )
        self.keylookup.add_binding(
            "i", self.prefs.getpref("Key_i", "INCREMENTS", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "I", self.prefs.getpref("Key_I", "INCREMENTS", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "F1", self.prefs.getpref("Key_F1", "ESTOP", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "F2", self.prefs.getpref("Key_F2", "POWER", str, "KEYCODES")
        )
        self.keylookup.add_binding(
            "Escape", self.prefs.getpref("Key_Escape", "ABORT", str, "KEYCODES")
        )
    ...
    def init_general_pref(self):
        ...
        """
        self.data.alert_sound = self.prefs.getpref(
            "audio_alert", self.data.alert_sound, str
        )
        ...
        self.data.error_sound = self.prefs.getpref(
            "audio_error", self.data.error_sound, str
        )
        self.data.error_font_name = self.prefs.getpref(
            "error_font", "Sans Bold 10", str
        )
        ...
        self.data.plot_view = self.prefs.getpref(
            "view", ("p", "x", "y", "y2", "z", "z2"), repr
        )
        self.data.show_offsets = self.prefs.getpref("show_offsets", True, bool)
        self.data.spindle_start_rpm = self.prefs.getpref(
            "spindle_start_rpm", 300, float
        )
        self.data.unlock_code = self.prefs.getpref("unlock_code", "123", str)
        self.data.embedded_keyboard = self.prefs.getpref(
            "embedded_keyboard", True, bool
        )
        ...
        for letter in ("x", "y", "z", "a", "b", "c", "u", "v", "w"):
            ...
            if letter in self.data.axis_list:
                ...
                if letter in ("a", "b", "c", "u", "v", "w"):
                    self.widgets.image6.hide()
                    # make more room for axis display
            else:
                # hide unneeded frames for axes not in use
                frame_for_letter.hide()
                # frame not relevant
    ...
    def init_gremlin(self):
        ...
        self.widgets.gremlin.set_property(
            "metric_units", (self.data.dro_units == self.data._MM)
        )
    ...
    def init_statusbar(self):
        ...
        self.homed_status_message = self.widgets.statusbar1.push(
            1, _("Ready For Homing")
        )
    ...
    def init_embeded_terminal(self):
        ...
        try:
            ...
            self.widgets._terminal.connect(
                "child-exited", lambda term, event: Gtk.main_quit()
            )
            self.widgets._terminal.spawn_async(
            ...
            self.widgets.terminal_window.connect(
                "delete-event", lambda window, event: Gtk.main_quit()
            )
            self.widgets.terminal_window.show()
        except:
            LOG.error(
                _(
                    "Could not initialize Vte terminal:\n"
    ...
    def init_screen1_geometry(self):
        ...
        if self.widgets.fullscreen1.get_active():
            ...
            try:
                ...
                LOG.warning(
                    _("Invalid window size specified '{}', defaulted to {}x{}").format(
                        self.data.window_geometry, x, y
                    )
                )
        else:
    ...
    def init_running_options(self):
        ...
        """
        self.widgets.button_block_delete.set_active(
    ...
    def init_sensitive_override_mode(self):
        ...
        """
        self.data.sensitive_override_mode = [
            "spindle_preset",
            "spindle_control",
            "spindle_increase",
            "spindle_decrease",
    ...
    def init_sensitive_graphics_mode(self):
        ...
        """
        self.data.sensitive_graphics_mode = [
            "button_override",
            "button_homing",
            "button_mode",
            "button_zero_origin",
    ...
    def init_sensitive_origin_mode(self):
        ...
        """
        self.data.sensitive_origin_mode = [
            "button_override",
            "button_graphics",
            "button_homing",
            "button_mode",
    ...
    def init_state(self):
        ...
        try:
            ...
            jogincr = self.data.angular_jog_increments[
                self.data.current_angular_jogincr_index
            ]
    ...
    def init_control_pins(self):
        ...
        """
        self.data["cycle_start"] = hal_glib.GPin(
            self.halcomp.newpin("cycle-start", hal.HAL_BIT, hal.HAL_IN)
        )
        self.data["cycle_start"].connect("value-changed", self.on_cycle_start_changed)
        self.data["abort"] = hal_glib.GPin(
            self.halcomp.newpin("abort", hal.HAL_BIT, hal.HAL_IN)
        )
        self.data["abort"].connect("value-changed", self.on_abort_changed)
        self.data["feed_hold"] = hal_glib.GPin(
            self.halcomp.newpin("feed-hold", hal.HAL_BIT, hal.HAL_IN)
        )
    ...
    def initialize_manual_toolchange(self):
        ...
        self.data["change-tool"] = hal_glib.GPin(
            self.halcomp.newpin("change-tool", hal.HAL_BIT, hal.HAL_IN)
        )
        # you can override manual tool change
        if "on_tool_change" in dir(self.handler_instance):
            self.data["change-tool"].connect(
                "value-changed", self.handler_instance.on_tool_change
            )
        else:
    ...
    def search_fwd(self, widget):
        ...
        self.widgets.gcode_view.text_search(
            direction=True, mixed_case=CASE, text=self.widgets.search_entry.get_text()
        )
    ...
    def search_bwd(self, widget):
        ...
        try:
            ...
            CASE = True
        self.widgets.gcode_view.text_search(
            direction=False, mixed_case=CASE, text=self.widgets.search_entry.get_text()
        )
    ...
    def replace_text(self, widget):
        ...
        self.widgets.gcode_view.replace_text_search(
            ...
            re_text=self.widgets.search_entry1.get_text(),
            replace_all=RE_ALL,
        )
    ...
    def on_key_event(self, widget, event, state):
        ...
        LOG.verbose(
            _("Key {} ({}) was pressed state: {} last: {}").format(
                keyname, event.keyval, state, self.data.key_event_last
            )
        )
        ...
        if keyname in ("Shift_L", "Shift_R"):
            return True  # ignore shift key press
        if (
            self.data.key_event_last[0] == keyname
            and self.data.key_event_last[1] == state
        ):
            return True
            ...
            if method:
                try:
                    try:
                        ...
                        self.show_try_errors()
                        return self.handler_instance.keybindings[method](
                            state, SHIFT, CNTRL, ALT
                        )
                except:
                    self.show_try_errors()
    ...
    def on_spindle_speed_adjust(self, widget):
        ...
        if self.mdi_control.mdi_is_reading():
            ...
            self.notify(
                _("INFO:"), _("can't start spindle manually in Auto mode"), INFO_ICON
            )
            return
    ...
    def on_spindle_control_clicked(self, *args):
        ...
        if not self.data.spindle_speed == 0:
            ...
            return
        if (
            not self.widgets.s_display_fwd.get_active()
            and not self.widgets.s_display_rev.get_active()
        ):
    ...
    def on_preset_spindle(self, *args):
        ...
        if self.data.preset_spindle_dialog:
            return
        label = Gtk.Label()
        label.set_text(_("Spindle Speed Preset Entry"))
        label.modify_font(pango.FontDescription("sans 20"))
        self.data.preset_spindle_dialog = Gtk.Dialog(
            _("Spindle Speed Preset Entry"),
        ...
        calc.entry.connect(
            "activate",
            lambda w: self.data.preset_spindle_dialog.emit(
        ...
        self.data.preset_spindle_dialog.connect(
            "response", self.on_preset_spindle_return, calc
        )
    ...
    def on_index_tool(self, *args):
        ...
        if self.data.index_tool_dialog:
            return
        self.data.index_tool_dialog = Gtk.Dialog(
            _("Manual Tool Index Entry"),
            self.widgets.window1,
            0,
            destroy_with_parent=True,
        )
        ...
        calc.entry.connect(
            "activate",
            lambda w: self.data.index_tool_dialog.emit(
                "response", Gtk.ResponseType.ACCEPT
            ),
        )
    ...
    def on_gremlin_motion(self, widget, event):
        ...
        if self.widgets.button_graphics.get_active():
            ...
            if self.data.hide_cursor:
                ...
                self.widgets.button_zoom.get_active()
                or self.widgets.button_pan_v.get_active()
            ):
                return
            elif (
                self.widgets.button_pan_h.get_active()
                or self.widgets.button_rotate_v.get_active()
            ):
                return
            elif self.widgets.button_rotate_h.get_active():
                return
            ...
            self.widgets.gremlin.set_property(
                "use_default_controls", not self.data.hide_cursor
            )

    # display calculator for input
    def launch_numerical_input(
        self,
        callback="on_numerical_entry_return",
        data=None,
        data2=None,
        title=_("Entry dialog"),
    ):
        ...
        """
        if self.data.entry_dialog:
            return
        label = Gtk.Label()
        label.set_text(title)
        label.modify_font(pango.FontDescription("sans 20"))
        self.data.entry_dialog = Gtk.Dialog(
            title, self.widgets.window1, 0, destroy_with_parent=True
        )
        calc = gladevcp.Calculator()
        calc.set_editable(True)
        ...
        calc.entry.connect(
            "activate",
            lambda w: self.data.entry_dialog.emit("response", Gtk.ResponseType.ACCEPT),
        )
        ...
        if callback in dir(self.handler_instance):
            self.data.entry_dialog.connect(
                "response", self.handler_instance[callback], calc, data, data2
            )
        else:
            self.data.entry_dialog.connect(
                "response", self[callback], calc, data, data2
            )
    ...
    def on_calibration(self, *args):
        ...
        p = os.popen(
            "tclsh %s/bin/emccalib.tcl -- -ini %s > /dev/null &"
            % (TCLPATH, self.inipath),
            "w",
        )
    ...
    def on_ladder(self, *args):
        ...
        if hal.component_exists("classicladder_rt"):
            ...
            self.add_alarm_entry(
                _("ladder not available - is the realtime component loaded?")
            )
    ...
    def on_offset_origin_clicked(self, widget):
        ...
        if self.widgets.button_override.get_active():
            self.launch_numerical_input(
                "on_adj_overrides_entry_return", widget, True, title=_("Override Entry")
            )
        # offset origin
        else:
    ...
    def on_move_to_clicked(self, widget):
        ...
        if (
            self.data.mode_order[0] == self.data._MAN
            and self.widgets.button_jog_mode.get_active()
        ):
        ...
        if self.data.ob:
            ...
            if self.data.mode_order[0] == self.data._MDI:
                try:
                    ...
                    LOG.debug(
                        _(
                            "Cannot set focus to hal_mdihistory when Onboard launched - Is this widget name in glade file?"
                        )
                    )
            elif self.data.mode_order[0] == self.data._AUTO:
                try:
                    ...
                    LOG.debug(
                        _(
                            "Cannot set focus to gcode_view when Onboard launched - Is this widget name in glade file?"
                        )
                    )
    ...
    def check_mode(self):
        ...
        if (
            self.data.mode_order[0] == self.data._MAN
            and self.widgets.notebook_main.get_current_page() == 0
        ):
    ...
    def spindle_dialog(self):
        ...
        if not self.data.spindle_control_dialog:
            self.data.spindle_control_dialog = Gtk.Dialog(
                _("Manual Spindle Control"),
                self.widgets.window1,
                0,
                destroy_with_parent=True,
            )
            self.data.spindle_control_dialog.add_buttons(
            ...
            self.data.spindle_control_dialog.connect(
                "delete_event", self.spindle_dialog_return
            )
            self.data.spindle_control_dialog.connect(
                "response", self.spindle_dialog_return
            )
    ...
    def launch_keyboard(self, args="", x="", y=""):
        ...
        def load_keyboard():
            if os.path.isfile("/usr/bin/onboard"):
                ...
                LOG.error(
                    _(
                        "Error with launching on-screen keyboard program - cannot find program"
                    )
                )
                self.add_alarm_entry(
                    _(
        ...
        def dialog_keyboard():
            if self.data.keyboard_dialog:
                ...
                if load_keyboard():
                    ...
                    self.data.keyboard_dialog = Gtk.Dialog(
                        _("Keyboard"), self.widgets.window1, 0, destroy_with_parent=True
                    )
    ...
    def connect_signals(self, handlers):
        ...
        signal_list = [
            ...
            [
                "",
                "button_toggle_readout",
                "clicked",
                "on_button_toggle_readout_clicked",
            ],
            ["", "button_jog_mode", "clicked", "on_button_jog_mode_clicked"],
            [
                "",
                "button_spindle_controls",
                "clicked",
                "on_button_spindle_controls_clicked",
            ],
            ...
            [
                ...
                "button_jog_speed",
                "clicked",
                "on_button_overrides_clicked",
                "jog_speed",
            ],
            [
                "",
                "button_select_rotary_adjust",
                "clicked",
                "on_button_select_rotary_adjust_clicked",
            ],
            [
                "block",
                "button_jog_increments",
                "clicked",
                "on_button_overrides_clicked",
                "jog_increments",
            ],
            ...
            [
                ...
                "eventbox_gremlin",
                "leave_notify_event",
                "on_eventbox_gremlin_leave_notify_event",
            ],
            [
                "",
                "eventbox_gremlin",
                "enter_notify_event",
                "on_eventbox_gremlin_enter_notify_event",
            ],
        ...
        for axis in self.data.axis_list:
            ...
            try:
                self.data[i] = int(
                    self.widgets[cb].connect("clicked", self.on_axis_selection_clicked)
                )
            except:
                self.show_try_errors()
    ...
    def parse_increment(self, jogincr):
        if jogincr.endswith("mm"):
            ...
            scale = self.from_internal_linear_unit(0.001 / 25.4)
        elif jogincr.endswith("in") or jogincr.endswith("inch"):
            scale = self.from_internal_linear_unit(1.0)
        elif jogincr.endswith("mil"):
            scale = self.from_internal_linear_unit(0.001)
        else:
            scale = 1
    ...
    def add_alarm_entry(self, message):
        try:
            ...
            textbuffer.insert_at_cursor(
                strftime("%a, %d %b %Y %H:%M:%S     -", localtime()) + message + "\n"
            )
        except:
            self.show_try_errors()
    ...
    def set_feed_override(self, percent_rate, absolute=False):
        ...
        if rate > self.data.feed_override_max:
            rate = self.data.feed_override_max
    ...
    def set_rapid_override(self, percent_rate, absolute=False):
        ...
        if rate > self.data.rapid_override_max:
            rate = self.data.rapid_override_max
    ...
    def set_spindle_override(self, percent_rate, absolute=False):
        ...
        if rate > self.data.spindle_override_max:
            rate = self.data.spindle_override_max
        elif rate < self.data.spindle_override_min:
            rate = self.data.spindle_override_min
    ...
    def set_jog_rate(self, step=None, absolute=None):
        ...
        if rate < 0:
            rate = 0
        if rate > self.data[j_rate + "_max"]:
            rate = self.data[j_rate + "_max"]
        rate = round(rate, 1)
        if self.data.angular_jog_adjustment_flag:
    ...
    def adjust_overrides(self, widget, action, number, absolute):
        ...
        if self.widgets.button_feed_override.get_active() and action:
            ...
            if absolute:
                ...
                self.set_spindle_override(
                    (change * self.data.spindle_override_inc), absolute
                )
            ...
            if absolute:
                ...
                self.set_velocity_override(
                    (change * self.data.velocity_override_inc), absolute
                )
    ...
    def change_origin_system(self, system, direction=None):
        ...
        if not system:
            if direction > 0 and not current == 59.3:
                self.mdi_control.set_user_system(system_list[self.data.system + 1])
            elif direction < 0 and not current == 54:
                self.mdi_control.set_user_system(system_list[self.data.system - 1])
            self.reload_plot()
    ...
    def message_setup(self):
        ...
        if len(m_text) != len(m_type):
            LOG.error(
                _(
                    "Invalid message configuration (missing text or type) in INI File [DISPLAY] section"
                )
            )
        ...
        if len(m_text) != len(m_boldtext):
            LOG.error(
                _(
                    "Invalid message configuration (missing boldtext) in INI File [DISPLAY] section"
                )
            )
        for bt, t, c, name in zip(m_boldtext, m_text, m_type, m_pinname):
            ...
            if not ("status" in c) and not ("dialog" in c) and not ("okdialog" in c):
                LOG.error(
                    _("Invalid message type ({})in INI File [DISPLAY] section").format(
                        c
                    )
                )
                continue
            if not name == None:
                ...
                self.data["name"] = hal_glib.GPin(
                    self.halcomp.newpin(name, hal.HAL_BIT, hal.HAL_IN)
                )
                self.data["name"].connect(
                    "value-changed", self.on_printmessage, name, bt, t, c
                )
                if "dialog" in c:
                    self.halcomp.newpin(name + "-waiting", hal.HAL_BIT, hal.HAL_OUT)
                    if not ("ok" in c):
                        self.halcomp.newpin(
                            name + "-response", hal.HAL_BIT, hal.HAL_OUT
                        )
    ...
    def warning_dialog(self, message, displaytype, secondary=None, pinname=None):
        ...
        try:
            if "dialog_return" in dir(self.handler_instance):
                dialog.connect(
                    "response",
                    self.handler_instance.dialog_return,
                    self,
                    displaytype,
                    pinname,
                )
            ...
            dialog.destroy()
            raise NameError(
                _("Dialog error - Is the dialog handler missing from the handler file?")
            )
    ...
    def dialog_return(self, widget, result, dialogtype, pinname):
        if pinname == "TOOLCHANGE":
            ...
            widget.destroy()
            try:
                self.widgets.statusbar1.remove(
                    self.statusbar_id, self.data.tool_message
                )
            except:
                self.show_try_errors()
            return
    ...
    def restart_dialog(self):
        ...
        self.data.restart_dialog = Gtk.Dialog(
            _("Restart Entry"), self.widgets.window1, 0, destroy_with_parent=True
        )
        label = Gtk.Label()
    ...
    def set_abs_color(self):
        ...
        self.prefs.putpref(
            "abs_textcolor",
            self.rgba_to_str(self.widgets.abs_colorbutton.get_rgba()),
            str,
        )
    ...
    def set_rel_color(self):
        ...
        self.prefs.putpref(
            "rel_textcolor",
            self.rgba_to_str(self.widgets.rel_colorbutton.get_rgba()),
            str,
        )
    ...
    def set_dtg_color(self):
        ...
        self.prefs.putpref(
            "dtg_textcolor",
            self.rgba_to_str(self.widgets.dtg_colorbutton.get_rgba()),
            str,
        )
    ...
    def block(self, widget_name):
        self.widgets["%s" % (widget_name)].handler_block(
            self.data["_sighandler_%s" % (widget_name)]
        )

    def unblock(self, widget_name):
        self.widgets["%s" % (widget_name)].handler_unblock(
            self.data["_sighandler_%s" % (widget_name)]
        )
    ...
    def update_active_axis_buttons(self, widget):
        ...
        for i in self.data.axis_list:
            ...
            if (
                self.widgets.button_jog_mode.get_active()
                or self.widgets.button_homing.get_active()
            ):
        ...
        if count == 0:
            self.data.active_axis_buttons.append((None, None))
    ...
    def jog_mode(self):
        ...
        if (
            len(self.data.active_axis_buttons) > 1
            and self.widgets.button_jog_mode.get_active()
        ):
            for i in self.data.axis_list:
                self.widgets["axis_%s" % i].set_active(False)
            ...
            if len(self.data.active_axis_buttons) > 1:
                ...
                if not self.data.active_axis_buttons[0][0] == "s":
                    ...
                    if self.data.active_axis_buttons[0][0] in ("a", "b", "c"):
                        ...
                        jogincr = self.data.jog_increments[
                            self.data.current_jogincr_index
                        ]
                    # print(jogincr)
                    if jogincr == ("continuous"):  # continuous jog
                        # print("active axis jog:",self.data.active_axis_buttons[0][1])
                        self.emc.continuous_jog(
                            self.data.active_axis_buttons[0][1], cmd
                        )
                    else:
                        # print("jog incremental")
                        if cmd == 0:
                            return  # don't want release of button to stop jog
                        distance = self.parse_increment(jogincr)
                        self.emc.incremental_jog(
                            self.data.active_axis_buttons[0][1], cmd, distance
                        )
    ...
    def do_key_jog(self, axis, direction, action):
        if self.data._JOG in self.check_mode():  # jog mode active:
            ...
            if self.data.jog_increments[self.data.current_jogincr_index] == (
                "continuous"
            ):  # continuous jog
                ...
                if self.mdi_control.mdi.emcstat.state != 1:
                    return
                jogincr = self.data.jog_increments[self.data.current_jogincr_index]
                distance = self.parse_increment(jogincr)
    ...
    def on_focus_out(self, widget, data=None):
        ...
        if (
            self.emcstat.enabled
            and self.emcstat.task_mode == linuxcnc.MODE_MANUAL
            and self.emcstat.current_vel > 0
        ):
    ...
    def spindle_adjustment(self, direction, action):
        if (
            action
            and not self.widgets.s_display_fwd.get_active()
            and not self.widgets.s_display_rev.get_active()
        ):
            self.notify(_("INFO:"), _("No direction selected for spindle"), INFO_ICON)
            return
    ...
    def do_jog_to_position(self, data):
        if len(self.data.active_axis_buttons) > 1:
            ...
            if not self.data.active_axis_buttons[0][0] == "s":
                ...
                self.mdi_control.go_to_position(
                    self.data.active_axis_buttons[0][0], pos, rate
                )
    ...
    def home_selected(self):
        ...
        if len(self.data.active_axis_buttons) > 1:
            self.notify(
                _("INFO:"),
                _("Can't home multiple axis - select HOME ALL instead"),
                INFO_ICON,
            )
            # print(self.data.active_axis_buttons)
        elif self.data.active_axis_buttons[0][0] == None:
    ...
    def set_axis_checks(self):
        if self.data.active_axis_buttons[0][0] == None:
            self.notify(
                _("INFO:"), _("No axis selected for origin touch-off"), INFO_ICON
            )
            return
        ...
        if not editor == None:
            ...
            if res:
                self.notify(
                    _("Error Message"),
                    _(
                        "Tool editor error - is the %s editor available?" % editor,
                        ALERT_ICON,
                        3,
                    ),
                )
    ...
    def update_mdi_spindle_button(self):
        try:
            self.widgets.at_speed_label.set_label(
                _("%d RPM" % abs(self.data.spindle_speed))
            )
        except Exception as e:
            pass
        ...
        for i in self.data.axis_list:
            for j in range(0, 3):
                ...
                if current == _ABS:
                    ...
                    label = "DTG"
                h = " "
                if current == _ABS and self.data["%s_is_homed" % i]:
                    h = "*"
                if self.data.diameter_mode and i == "x":
                    data = data * 2.0
                if self.data.dro_units == self.data._MM:
                    text = "%s% 10.3f" % (h, data)
                else:
    ...
    def update_feed_speed_label(self):
        ...
        if self.data.dro_units == self.data._MM:
            ...
            text = "%.3f" % (data)
        self.widgets.active_feed_speed_label.set_label(
            "F%s    S%s   V%s"
            % (self.data.active_feed_command, self.data.active_spindle_command, text)
        )
    ...
    def update_tool_label(self):
        ...
        if tool == None:
            tool = "None"
        self.widgets.system.set_text(
            (
                _("Tool %(t)s     %(l)s")
                % ({"t": tool, "l": systemlabel[int(self.data.system)]})
            )
        )
    ...
    def update_override_label(self):
        ...
        self.widgets.so.set_text(
            "SO: %d%%" % (round(self.data.spindle_override, 2) * 100)
        )
        self.widgets.mv.set_text(
            "VO: %d%%" % (round((self.data.velocity_override), 2) * 100)
        )
    ...
    def update_mode_label(self):
        ...
        self.widgets.mode_label.set_label(
            _("%(n)s   View -%(v)s               %(t)s")
            % (
                {
    ...
    def update_hal_jog_pins(self):
        for i in self.data.axis_list:
            if (
                self.widgets.button_jog_mode.get_active()
                and self.widgets["axis_%s" % i].get_active()
            ):
