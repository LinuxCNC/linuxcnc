...
typedef enum { cuIgnore = -1, cuOn, cuOff, cuUnder, cuBlock } cursorType;
...
typedef enum { kmShared, kmExclusive } keyModeType;
...
typedef enum { jtCont, jtStep1, jtStep01, jtStep001, jtStep0001 } jogModeType;
...
char hbStrs[][6] = {"on", "off", "open"};

char blStrs[][8] = {"on", "off", "toggle", "open", "blink", "flash"};

char cuStrs[][6] = {"on", "off", "under", "block"};

char typeStrs[][10] = {
    "string", "title", "hbar", "vbar", "icon", "scroller", "frame", "num"};

char keyModes[][8] = {"shared", "excl"};
...
static const char *
widgetSetStr(int widgetNo, const char *newStr, const char *oldStr)
{
...
static void strScreenSet(const char *screen, const char *attr, const char *s)
{
    if (s != NULL) {
        snprintf(
            sockStr, sizeof(sockStr), "screen_set %s %s %s\n", screen, attr, s);
...
static int createWidgets()
{
    ...
    for (i = 0; i < WIDGET_COUNT; i++) {
        snprintf(sockStr,
                 ...
                 widgets[i].widgetName,
                 typeStrs[widgets[i].type]);
        sockSendStr(sockfd, sockStr);
        switch (widgets[i].type) {
        case wtString: widgetSetStr(i, widgets[i].text, ""); break;
        case wtTitle: break;
        case wtHbar: widgetSetStr(i, widgets[i].text, ""); break;
        case wtVbar: break;
        case wtIcon: break;
    ...
    while ((entry = readdir(dp)) != NULL) {
        ...
        if (!S_ISDIR(statbuf.st_mode)) {
            if (strstr(entry->d_name, "ngc") != NULL) {
                strncpy(namebuf,
                        entry->d_name,
                        (strlen(entry->d_name) < 24) ? strlen(entry->d_name) - 4
                                                     : 24);
                ...
                snprintf(sockStr,
                         sizeof(sockStr),
                         "menu_set_item open %s -menu_result quit\n",
                         namebuf);
                sockSendStr(sockfd, sockStr);
            }
        }
    ...
    if ((f = popen(ifacetypestr, "r")) != NULL) {
        if (fread(buffer, sizeof(char), sizeof(buffer), f) > 0) {
            ...
            if (strcmp(pch, "inet") == 0) {
                ...
                snprintf(sockStr,
                         sizeof(sockStr),
                         "menu_set_item tcpip addrtype -value {%d}\n",
                         strcmp(pch, "dhcp") ? 1 : 0);
                sockSendStr(sockfd, sockStr);
            }
        }
    ...
    if ((f = popen(ipconfigstr, "r")) != NULL) {
        if (fread(buffer, sizeof(char), sizeof(buffer), f) > 0) {
            ...
            if (strcmp(pch, "addr") == 0) {
                ...
                snprintf(sockStr,
                         sizeof(sockStr),
                         "menu_set_item tcpip address -value {%s}\n",
                         pch);
                sockSendStr(sockfd, sockStr);
                pch = strtok(NULL, delims);
                if (strcmp(pch, "Bcast") == 0) {
                    ...
                    if (strcmp(pch, "Mask:") == 0) {
                        ...
                        snprintf(sockStr,
                                 sizeof(sockStr),
                                 "menu_set_item tcpip netmask -value {%s}\n",
                                 pch);
                        sockSendStr(sockfd, sockStr);
                    }
                }
    ...
    if ((f = popen(gatewaystr, "r")) != NULL) {
        if (fread(buffer, sizeof(char), sizeof(buffer), f) > 0) {
            ...
            if (strcmp(pch, "default") == 0) {
                ...
                snprintf(sockStr,
                         sizeof(sockStr),
                         "menu_set_item tcpip gateway -value {%s}\n",
                         pch);
                sockSendStr(sockfd, sockStr);
            }
        }
    ...
    if ((f = popen(dnsstr, "r")) != NULL) {
        if (fread(buffer, sizeof(char), sizeof(buffer), f) > 0) {
            ...
            if (strcmp(pch, "nameserver") == 0) {
                ...
                snprintf(sockStr,
                         sizeof(sockStr),
                         "menu_set_item tcpip dns1 -value {%s}\n",
                         pch);
                sockSendStr(sockfd, sockStr);
                pch = strtok(NULL, delims);
                if (strcmp(pch, "nameserver") == 0) {
                    ...
                    snprintf(sockStr,
                             sizeof(sockStr),
                             "menu_set_item tcpip dns2 -value {%s}\n",
                             pch);
                    sockSendStr(sockfd, sockStr);
                }
            }
...
static int createMenus()
{
    ...
    sockSendStr(
        sockfd,
        "menu_add_item Run feed_slider slider {Feed Override} -mintext {0} "
        "-maxtext {125} -value {50}\n");
    sockSendStr(
        sockfd,
        "menu_add_item Run laser_slider slider {Laser Override} -mintext {0} "
        "-maxtext {100} -value {50}\n");

    sockSendStr(sockfd, "menu_add_item {} Jog menu {Jog}\n");
    sockSendStr(sockfd, "menu_add_item Jog jog_speed numeric {Speed}\n");
    sockSendStr(sockfd,
                "menu_add_item Jog jog_mode ring {Mode} -strings "
                "{Cont\t1.0\t0.1\t0.01\t0.001}\n");

    sockSendStr(sockfd, "menu_add_item {} setup menu {Setup}\n");

    sockSendStr(sockfd, "menu_add_item setup display menu {Main Display}\n");
    sockSendStr(sockfd,
                "menu_add_item display posdisplay action {Positions}\n");
    ...
    sockSendStr(sockfd,
                "menu_add_item tcpip addrtype ring {Addr Type} -strings "
                "{dhcp\tstatic}\n");
    sockSendStr(sockfd,
                "menu_add_item tcpip address ip {IP Address} -v6 false -value "
                "{192.168.1.250}\n");
    sockSendStr(sockfd,
                "menu_add_item tcpip netmask ip {Subnet mask} -v6 false -value "
                "{255.255.255.0}\n");
    sockSendStr(sockfd,
                "menu_add_item tcpip gateway ip {Gateway} -v6 false -value "
                "{192.168.1.1}\n");
    sockSendStr(sockfd,
                "menu_add_item tcpip dns1 ip {Primary DNS} -v6 false -value "
                "{192.168.1.1}\n");
    sockSendStr(sockfd,
                "menu_add_item tcpip dns2 ip {Secondary DNS} -v6 false -value "
                "{0.0.0.0}\n");
    ...
    sockSendStr(
        sockfd,
        "menu_add_item utility home ring {Home} -strings {all\tX\tY\tZ}\n");
    sockSendStr(
        sockfd,
        "menu_add_item utility limits ring {Limits} -strings {On\tOff}\n");
    sockSendStr(
        sockfd,
        "menu_add_item utility laser ring {Laser} -strings {Off\tOn}\n");
...
static void parseConnect()
{
    ...
    while (pch != NULL) {
        ...
        case cpVersion:
            rtapi_strxcpy(lcdParms.version, strtok(NULL, delims));
            break;
        case cpProtocol:
            rtapi_strxcpy(lcdParms.protocol, strtok(NULL, delims));
            break;
        case cpLCD: break;
        case cpWidth:
            pch = strtok(NULL, delims);
...
static int jog(int jnum, float speed)
{
    ...
    case jtCont:
        if (jogging ==
            1) { // toggle if driver does not support key down / key up
            jogging = 0;
    ...
    case jtStep1:
        return sendJogIncr(jnum, JOGMODE, speed, stepSize / conversion);
        break;
    case jtStep01:
        return sendJogIncr(
            jnum, JOGMODE, speed, (stepSize / 10.0) / conversion);
        break;
    case jtStep001:
        return sendJogIncr(
            jnum, JOGMODE, speed, (stepSize / 100.0) / conversion);
        break;
    case jtStep0001:
        return sendJogIncr(
            jnum, JOGMODE, speed, (stepSize / 1000.0) / conversion);
        break;
    default: return -1;
    }
    //    }
...
static int enterKey()
{
    ...
    case stOpen: break;
    case stMain2: break;
    case stStats: setNewScreen(stMain); break;
...
{
    switch (k) {
    case ktLeftPress: leftKey(); break;
    case ktLeftRelease: leftKeyRelease(); break;
    case ktRightPress: rightKey(); break;
    case ktRightRelease: rightKeyRelease(); break;
    case ktUpPress: upKey(); break;
    case ktUpRelease: upKeyRelease(); break;
    case ktDownPress: downKey(); break;
    case ktDownRelease: downKeyRelease(); break;
    ...
    case ktStopPress: stopKey(); break;
    case ktStopRelease: break;
    case ktStepPress: stepKey(); break;
    ...
    case ktNextPress: nextKey(); break;
    case ktNextRelease: break;
    case ktPrevPress: prevKey(); break;
...
static int parseLine()
{
    ...
    case rmHuh: pch = strtok(NULL, delims); break;
    case rmListen:
        pch = strtok(NULL, delims);
        curScreen = lookupScreen(pch);
        break;
    case rmIgnore: pch = strtok(NULL, delims); break;
    case rmKey:
        pch = strtok(NULL, delims);
        doKey(lookupKey(pch));
        break;
...
static void slowLoop()
{
    ...
    if (emcStatus->task.file[0] != 0) {
        ...
        if (strcmp(fname, programName) != 0) {
            rtapi_strxcpy(programName,
                          widgetSetStr(PROG_WIDGET1, fname, programName));
            rtapi_strxcpy(programName,
                          widgetSetStr(PROG_WIDGET2, fname, programName));
    ...
    default:
        ...
        } else if (emcStatus->task.state != EMC_TASK_STATE::ON) {
            rtapi_strxcpy(status, widgetSetStr(STATUSWIDGET, "  Off", status));
            widgetSetStr(JOG_WIDGET, "    ", "");
        } else if (emcStatus->task.mode == EMC_TASK_MODE::MANUAL) {
...
static void updatePositions()
{
    ...
    if (units == unmm)
        snprintf(numStr,
                 sizeof(numStr),
                 "%7.2f",
                 emcStatus->motion.traj.actualPosition.tran.x * conversion);
    else
        snprintf(numStr,
                 sizeof(numStr),
                 "%7.3f",
    ...
    if (units == unmm)
        snprintf(numStr,
                 sizeof(numStr),
                 "%7.2f",
                 emcStatus->motion.traj.actualPosition.tran.y * conversion);
    else
        snprintf(numStr,
                 sizeof(numStr),
                 "%7.3f",
    ...
    if (units == unmm)
        snprintf(numStr,
                 sizeof(numStr),
                 "%6.2f",
                 emcStatus->motion.traj.actualPosition.tran.z * conversion);
    else
        snprintf(numStr,
                 sizeof(numStr),
                 "%6.2f",
...
static int sockMain()
{
    ...
    if (sockfd == -1) {
        ...
        while ((len = sockRecvString(sockfd, buffer, sizeof(buffer) - 1)) >=
               0) {
            if (len == 0) {
                if (quitting)
                    break;
