...
class Data:
    def __init__(self, app, private_data_instance, base_dir, linuxcnc_version):
        ...
        self.mpgincrvalue1 = 0.0001  # incr-select-a  high
        self.mpgincrvalue2 = 0.0005  # b
        self.mpgincrvalue3 = 0.001  # ab
        self.mpgincrvalue4 = 0.005  # c
        self.mpgincrvalue5 = 0.01  # ac
        self.mpgincrvalue6 = 0.05  # bc
        self.mpgincrvalue7 = 0.1  # abc
        self.mpgincrvalue8 = 0.125  # d
        self.mpgincrvalue9 = 0.125  # ad
        self.mpgincrvalue10 = 0.125  # bd
        self.mpgincrvalue11 = 0.125  # abd
        self.mpgincrvalue12 = 0.125  # cd
        self.mpgincrvalue13 = 0.125  # acd
        self.mpgincrvalue14 = 0.125  # bcd
        self.mpgincrvalue15 = 0.125  # abcd
        self.mpgdebounce = True
        self.mpgdebouncetime = 0.2
            ...
            for channel in range(0, _PD._NUM_CHANNELS):
                ...
                for i in range(0, _PD._SSCOMBOLEN):
                    ...
                    if i < 24:
                        ...
                        self[pinname] = _PD.UNUSED_OUTPUT
                    pinname = "mesa%dsserial%d_%dpin%dtype" % (
                        boardnum,
                        port,
                        channel,
                        i,
                    ...
                    if i < 24:
                        ...
                        self[pinname] = _PD.GPIOO
                    pinname = "mesa%dsserial%d_%dpin%dinv" % (
                        boardnum,
                        port,
                        channel,
                        i,
        ...
        self.ahomelatchvel = 0.5
        self.aminferror = 0.05
        self.amaxferror = 0.5
    ...
    def set_axis_unit_defaults(self, units=True):
        if units:  # imperial
            ...
            self.default_linear_velocity = 0.25  # 15 inches per min
            self.min_linear_velocity = 0.01667
            ...
            for i in ("x", "y", "z"):
                ...
                self[i + "maxacc"] = 30
                self[i + "homevel"] = 0.05
                self[i + "homesearchvel"] = 0.05
                self[i + "homelatchvel"] = 0.025
                self[i + "minferror"] = 0.05
                self[i + "maxferror"] = 0.5
                if not i == "z":
                    self[i + "minlim"] = 0
                    self[i + "maxlim"] = 8
                else:
    ...
    def load(self, filename, app=None, force=False):
        ...
        if i:
            _PD.human_encoder_input_names[end][1] = temp
        # resolvers
        temp = []
        i = False
        ...
        if i:
            _PD.human_resolver_input_names[end][1] = temp
        # 8I20 amplifier card
        temp = []
        i = False
        ...
        if i:
            _PD.human_8i20_input_names[end][1] = temp
        # potentiometer
        temp = []
        i = False
        ...
        if i:
            _PD.human_pot_output_names[end][1] = temp
        # analog input
        temp = []
        i = False
        ...
        if i:
            _PD.human_analog_input_names[end][1] = temp
        # pwm
        temp = []
        i = False
        ...
        if i:
            _PD.human_pwm_output_names[end][1] = temp
        # tppwm
        temp = []
        i = False
        ...
        if i:
            _PD.human_tppwm_output_names[end][1] = temp
        # GPIO Input
        temp = []
        i = False
        ...
        if i:
            _PD.human_input_names[end][1] = temp
        # GPIO Output
        temp = []
        i = False
        ...
        if i:
            _PD.human_output_names[end][1] = temp
        # steppers
        temp = []
        i = False
        ...
        if i:
            _PD.human_stepper_names[end][1] = temp
...
If you have a REALLY large config that you wish to convert to this newer version of PNConf - ask on the LinuxCNC forum - it may be possible.."
        ...
        for f, m in self.md5sums:
            ...
            if m1 and m != m1:
                warnings2.append(
                    _("File %r was modified since it was written by PNCconf") % f
                )

        # no warnings ? return to pncconf APP
        if not warnings and not warnings2:
            return
        ...
        if warnings2:
            ...
            warnings.append(
                _(
                    "Saving this configuration file will discard configuration changes made outside PNCconf."
                )
            )
        if warnings:
    ...
    def save(self, basedir):
        ...
        d = xml.dom.minidom.getDOMImplementation().createDocument(None, "pncconf", None)
        e = d.documentElement
        ...
        if self.frontend == _PD._TOUCHY:
            ...
            templist = {
                "touchyabscolor": "abs_textcolor",
                "touchyrelcolor": "rel_textcolor",
                "touchydtgcolor": "dtg_textcolor",
                "touchyerrcolor": "err_textcolor",
            }
        ...
        if self.frontend == _PD._AXIS and (
            self.axisposition[0] or self.axissize[0] or self.axisforcemax
        ):
            ...
            if _APP.warning_dialog(_PD.MESS_REPLACE_RC_FILE, False):
                ...
                if self.axisposition[0] or self.axissize[0]:
                    ...
                    geo = "%s%s" % (size, pos)
                    print(
                        """root_window.tk.call("wm","geometry",".","%s")""" % (geo),
                        file=f1,
                    )
                if self.axisforcemax:
                    ...
                    print(
                        """   fullsize=str(maxgeo[0]) + 'x' + str(maxgeo[1])""", file=f1
                    )
                    print(
                        """root_window.tk.call("wm","geometry",".",fullsize)""", file=f1
                    )
        ...
        if self.createshortcut and os.path.exists(desktop):
            ...
            print(
                "Comment=" + _("Desktop Launcher for LinuxCNC config made by PNCconf"),
                file=file,
            )
