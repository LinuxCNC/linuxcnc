...
class HAL:
    ...
    def write_halfile(self, base):
        ...
        if os.path.exists(filename):
            shutil.copy2(
                filename, filename.replace(".hal", "_" + str(int(time.time())) + ".hal")
            )
        ...
        if not self.d.select_qtplasmac:
            ...
            encoder, counter, probe, pwm, pump = (
                False,
                False,
                False,
                False,
        ...
        if self.d.classicladder:
            print(
                "loadrt classicladder_rt numPhysInputs=%d numPhysOutputs=%d numS32in=%d numS32out=%d numFloatIn=%d numFloatOut=%d"
                % (
                    self.d.digitsin,
                    self.d.digitsout,
                    self.d.s32in,
        ...
        if pump:
            print("addf charge-pump base-thread", file=file)
        if pwm:
            print("addf pwmgen.make-pulses base-thread", file=file)
        ...
        if pwm:
            print("addf pwmgen.update servo-thread", file=file)
        if self.d.pyvcphaltype == 1 and self.d.pyvcpconnect == 1:
            if encoder:
        ...
        if encoder:
            ...
            if SIG.PHB not in inputs:
                print("setp encoder.0.position-scale %f" % self.d.spindlecpr, file=file)
                print("setp encoder.0.counter-mode 1", file=file)
            else:
                print(
                    "setp encoder.0.position-scale %f" % (4.0 * int(self.d.spindlecpr)),
                    file=file,
                )
            print(
                "net spindle-position encoder.0.position => spindle.0.revs", file=file
            )
            print(
        ...
        for i in range(4):
            ...
            if din in inputs:
                ...
                print(
                    "%snet %s => motion.digital-in-%02d" % (comment, din, i), file=file
                )
        self.tandemsigs = {}
        ...
        print(file=file)
        self.outputlist = []
        for o in (1, 2, 3, 4, 5, 6, 7, 8, 9, 14, 16, 17):
            self.connect_output(file, o)
        if self.d.number_pports > 1:
            if self.d.pp2_direction:  # Input option
                pinlist = (1, 14, 16, 17)
            else:
            ...
            print(file=file)
            for i in pinlist:
                self.connect_output(file, i, 1)
            print(file=file)

        self.inputlist = []
        for i in (10, 11, 12, 13, 15):
            self.connect_input(file, i)
        if self.d.number_pports > 1:
            if self.d.pp2_direction:  # Input option
        ...
        if limits_homes:
            ...
            for j in self.d.axislist:
                print(
                    "net {0}homing <= joint.{1}.homing => lut5.0.in-{1}".format(
                        j, self.d.axislist.index(j)
                    ),
                    file=file,
                )
        ...
        if self.d.select_qtplasmac and self.d.sim_hardware:
            ...
            print(
                "net sim:estop-out estop_not.out iocontrol.0.emc-enable-in", file=file
            )
        else:
            # standard estop handling
            print("net estop-out <= iocontrol.0.user-enable-out", file=file)
            if (
                self.d.classicladder
                and self.d.ladderhaltype == 1
                and self.d.ladderconnect
                ...
                print(file=file)
                print(
                    _("# **** Setup for external estop ladder program -END ****"),
                    file=file,
                )
        ...
        if not self.d.select_qtplasmac:
            ...
            print(
                _("# These files are loaded post GUI, in the order they appear"),
                file=f1,
            )
        ...
        if not self.d.select_qtplasmac:
            ...
            if (
                self.d.pyvcp and self.d.pyvcphaltype == 1 and self.d.pyvcpconnect
            ):  # spindle speed/tool # display
                print(
                    _("# These files are loaded post GUI, in the order they appear"),
                    file=f1,
                )
                ...
                if encoder:
                    ...
                    print(
                        ("setp lowpass.0.gain %f") % self.d.spindlefiltergain, file=f1
                    )
                    print(
                        (
                    ...
                    if self.d.usespindleatspeed:
                        ...
                        print(
                            ("setp near.0.scale %f") % self.d.spindlenearscale, file=f1
                        )
                    else:
                        print(
                            (
                    ...
                    print(file=f1)
                    print(
                        ("net spindle-cmd-rpm-abs    => pyvcp.spindle-speed"), file=f1
                    )
                    print(file=f1)
                    print(
                        (
                    ...
                    print(file=f1)
                    print(
                        ("net spindle-at-speed => pyvcp.spindle-at-speed-led"), file=f1
                    )
                    print(("sets spindle-at-speed true"), file=f1)
                f1.close()
            else:
    ...
    def connect_joint(self, file, num, let):
        ...
        print(
            "setp stepgen.%d.maxaccel [JOINT_%d]STEPGEN_MAXACCEL" % (num, num),
            file=file,
        )
        print(
            "net %spos-cmd joint.%d.motor-pos-cmd => stepgen.%d.position-cmd"
            % (let, num, num),
            file=file,
        )
        print(
            "net %spos-fb stepgen.%d.position-fb => joint.%d.motor-pos-fb"
            % (let, num, num),
            file=file,
        )
        ...
        print(
            "net %senable joint.%d.amp-enable-out => stepgen.%d.enable"
            % (let, num, num),
            file=file,
        )
    ...
    def sim_hardware_halfile(self, base):
        ...
        if self.d.sim_hardware:
            ...
            for j in self.d.axislist:
                print(
                    "net {:16}joint.{}.pos-fb   sim-hardware.{}current-pos".format(
                        j.upper() + "joint-pos-fb", self.d.axislist.index(j), j.upper()
                    ),
                    file=f1,
                )
            print(file=f1)
            for j in self.d.axislist:
                if j[0] == "a":
                    ...
                    limit = 1000
                print(
                    "setp sim-hardware.{:14}{}".format(
                        j.upper() + "maxsw-upper", limit
                    ),
                    file=f1,
                )
                print(
                    "setp sim-hardware.{:14}[JOINT_{}]MAX_LIMIT".format(
                        j.upper() + "maxsw-lower", self.d.axislist.index(j)
                    ),
                    file=f1,
                )
                print(
                    "setp sim-hardware.{:14}[JOINT_{}]MIN_LIMIT".format(
                        j.upper() + "minsw-upper", self.d.axislist.index(j)
                    ),
                    file=f1,
                )
                print(
                    "setp sim-hardware.{:14}-{}".format(
                        j.upper() + "minsw-lower", limit
                    ),
                    file=f1,
                )
                print(
                    "setp sim-hardware.{:14}[JOINT_{}]HOME_OFFSET".format(
                        j.upper() + "homesw-pos", self.d.axislist.index(j)
                    ),
                    file=f1,
                )
            ...
            for j in ["x", "x2", "y", "y2", "z", "a", "u", "v"]:
                print(
                    "net {:21}sim-hardware.{}bothsw-out".format(
                        "fake-both-" + j, j.upper()
                    ),
                    file=f1,
                )
                print(
                    "net {:21}sim-hardware.{}maxsw-out".format(
                        "fake-max-" + j, j.upper()
                    ),
                    file=f1,
                )
                print(
                    "net {:21}sim-hardware.{}minsw-out".format(
                        "fake-min-" + j, j.upper()
                    ),
                    file=f1,
                )
            ...
            for j in ["x", "x2", "y", "y2", "z", "a", "u", "v"]:
                print(
                    "net {:21}sim-hardware.{}homesw-out".format(
                        "fake-home-" + j, j.upper()
                    ),
                    file=f1,
                )
            ...
            for j in ["x", "x2", "y", "y2", "z", "a", "u", "v"]:
                print(
                    "net {:21}sim-hardware.{}bothsw-homesw-out".format(
                        "fake-both-home-" + j, j.upper()
                    ),
                    file=f1,
                )
                print(
                    "net {:21}sim-hardware.{}maxsw-homesw-out".format(
                        "fake-max-home-" + j, j.upper()
                    ),
                    file=f1,
                )
                print(
                    "net {:21}sim-hardware.{}minsw-homesw-out".format(
                        "fake-min-home-" + j, j.upper()
                    ),
                    file=f1,
                )
            ...
            for j in self.d.axislist:
                if SIG.ALL_LIMIT_HOME in inputs:
                    print(
                        "net {}homing => sim-hardware.{}homing".format(j, j.upper()),
                        file=f1,
                    )
                else:
                    print(
                        "net {}homing  joint.{}.homing => sim-hardware.{}homing".format(
                            j, self.d.axislist.index(j), j.upper()
                        ),
                        file=f1,
                    )
    ...
    def connect_input(self, file, num, port=0, fake=False):
        ...
        if p not in self.inputlist:
            ...
            if not fake:
                print(
                    'duplicate input "%s" will not be connected to "parport.%d.pin-%02d-in%s"'
                    % (
                        SIG.human_input_names[SIG.hal_input_names.index(p)],
                        port,
                        num,
                        ending,
                    )
        ...
        if i and not fake:
            print(
                "%snet %s <= parport.%d.pin-%02d-in-not%s"
                % (comment, p, port, num, ending),
                file=file,
            )
        else:
            print(
                "%snet %s <= parport.%d.pin-%02d-in%s"
                % (comment, p, port, num, ending),
                file=file,
            )
    ...
    def connect_output(self, file, num, port=0, fake=False):
        ...
        if p not in self.outputlist:
            ...
            if not fake:
                print(
                    'duplicate output "%s" will not be connected to "parport.%d.pin-%02d-out%s"'
                    % (
                        SIG.human_output_names[SIG.hal_output_names.index(p)],
                        port,
                        num,
                        ending,
                    )
        ...
        if i and not fake:
            print(
                "setp parport.%d.pin-%02d-out-invert%s 1" % (port, num, ending),
                file=file,
            )
        ...
        if sig in self.tandemsigs:
            ...
            if value == 2:
                signame = signame.replace(
                    "{} ".format(sig), "{}{}{}".format(sig[0], value, sig[1:])
                )
        ...
        if self.d.select_qtplasmac:
            if signame.strip()[-7:] in ["dout-01", "dout-02", "dout-03"]:
                ...
                comment = "#qtplasmac disabled for sim mode:      "
        print(
            "%snet %s => parport.%d.pin-%02d-out%s"
            % (comment, signame, port, num, ending),
            file=file,
        )
        if self.a.doublestep() and not fake:
            if p in (
                ...
                print(
                    "setp parport.0.pin-%02d-out-reset%s 1" % (num, ending), file=file
                )

    def min_lim_sig(self, axis):
        inputs = self.a.build_input_set()
        thisaxisminlimits = set(
            (
                SIG.ALL_LIMIT,
                SIG.ALL_LIMIT_HOME,
                "min-" + axis,
    ...
    def qtplasmac_connections(self, file, base):
        ...
        print(
            "{}net plasmac:breakaway      => db_breakaway.in".format(comment), file=file
        )
        ...
        print(
            "{}net plasmac:arc-voltage-in => plasmac.arc-voltage-in".format(comment),
            file=file,
        )
        ...
        print(
            "{}net plasmac:move-up        => plasmac.move-up".format(comment), file=file
        )
        print(
            "{}net plasmac:move-down      => plasmac.move-down".format(comment),
            file=file,
        )
        ...
        if self.d.thcadenc:
            ...
            print(
                "{}addf encoder.update-counters base-thread".format(comment), file=file
            )
            print(
                "{}addf encoder.capture-position servo-thread".format(comment),
                file=file,
            )
            ...
            print(
                "{}net plasmac:arc-voltage-raw encoder.0.phase-A".format(comment),
                file=file,
            )
            print(
                "{}net plasmac:arc-voltage-in encoder.0.velocity".format(comment),
                file=file,
            )
        # if ohmic sensing contact
        if self.d.ohmiccontact:
            print(
                "\n{}net plasmac:ohmic-probe <= plasmac.ohmic-sense-out".format(
                    comment
                ),
                file=file,
            )
        ...
        print(
            _("# This file will not be overwritten when you run stepconf again"),
            file=f1,
        )
        ...
        print(
            "# This file will not be overwritten when you run stepconf again", file=f1
        )
        f1.close()
        # if a sim config
        if self.d.sim_hardware:
