...
class HAL:
    ...
    def write_halfile(self, base):
		...
		print >>f1
		if (self.d.pyvcp):
			print >>f1, "source pyvcp_postgui.hal"
		print >>f1, "source custom_postgui.hal"
		f1.close()

		# pyvcp_postgui.hal
		if self.d.pyvcp and self.d.pyvcptype == 0: # Default pyvcp selected
			# If the user asked for pyvcp sample panel add the HAL commands too
			pyfilename = os.path.join(base, "pyvcp_postgui.hal")
		...
		if self.d.pyvcp or self.d.customhal or self.d.classicladder or self.d.halui_custom:
		...
		# gladevcp
		if self.d.gladevcp:
			calllist_filename = os.path.join(base, "gvcp_call_list.hal")
			f1 = open(calllist_filename, "w")
			...
			print >>f1, _("# overwritten when you run stepconf again").encode('utf-8')
			print >>f1
			if self.d.gladevcptype == 0: #default
				print >>f1, "source gvcp_default.hal"
			print >>f1, "source gvcp_custom.hal"
			f1.close()

		# gvcp_default.hal
		if self.d.gladevcp and self.d.gladevcptype == 0:
			filename = os.path.join(base, "gvcp_default.hal")
			f1 = open(filename, "w")
			print >>f1, _("# These files are loaded post gladevcp, in the order they appear")
			print >>f1, _("# Generated by stepconf 1.1 at %s") % time.asctime()
			print >>f1, _("# If you make changes to this file, they will be").encode('utf-8')
			...
			print >>f1, _("# Put custom HAL commands in gvcp_custom.hal")

			#for hal_command in self.d.hal_postgui_list:
			#	print >>f1, hal_command
			f1.close()
		# gvcp_custom.hal
		#if self.d.gladevcp and self.d.gladevcptype == 1:
		if self.d.gladevcp: # everytime put custom file
			filename = os.path.join(base, "gvcp_custom.hal")
