...
class HAL:
	...
	def write_halfile(self, base):
		...
		custom = os.path.join(base, FILE_CUSTOM_HALFILE)
		if not os.path.exists(custom):
			f1 = open(custom, "w")
			print >>f1, _("# Include your HAL commands here")
		...
		if self.d.pyvcp == True:
			calllist_filename = os.path.join(base, FILE_POSTGUI_CALL_LIST)
			f1 = open(calllist_filename, "w")
			print >>f1, _("# These files are loaded post GUI, in the order they appear")
			print >>f1, _("# Generated by stepconf %(version)s at %(currenttime)s") % {'version':STEPCONF_VERSION, 'currenttime':time.asctime()}
			...
			print >>f1, "source %s" % FILE_PYVCP_CUSTOM_HAL
			f1.close()

		# pyvcp_postgui.hal
		if self.d.pyvcp == True and self.d.pyvcptype == PYVCP_DEFAULT: # Default pyvcp selected
			# If the user asked for pyvcp sample panel add the HAL commands too
			pyfilename = os.path.join(base, FILE_PYVCP_POSTGUI)
		...
		if self.d.pyvcp == True: # everytime put custom file
			filename = os.path.join(base, FILE_PYVCP_CUSTOM_HAL)
		...
		# gladevcp
		if self.d.gladevcp:
			calllist_filename = os.path.join(base, FILE_GLADEVCP_CALL_LIST)
			f1 = open(calllist_filename, "w")
			...
			print >>f1, _("# overwritten when you run stepconf again").encode('utf-8')
			print >>f1
			if self.d.gladevcptype == GLADEVCP_DEFAULT: #default
				print >>f1, "source %s" % FILE_GLADEVCP_DEFAULT_HAL
			print >>f1, "source %s" % FILE_GLADEVCP_CUSTOM_HAL
			f1.close()

		# gladevcp_default.hal
		if self.d.gladevcp and self.d.gladevcptype == GLADEVCP_DEFAULT:
			filename = os.path.join(base, FILE_GLADEVCP_DEFAULT_HAL)
			f1 = open(filename, "w")
			print >>f1, _("# These files are loaded post gladevcp, in the order they appear")
			print >>f1, _("# Generated by stepconf %(version)s at %(currenttime)s") % {'version':STEPCONF_VERSION, 'currenttime':time.asctime()}
			print >>f1, _("# If you make changes to this file, they will be").encode('utf-8')
			...
			f1.close()
		# gladevcp_custom.hal
		if self.d.gladevcp: # everytime put custom file
			filename = os.path.join(base, FILE_GLADEVCP_CUSTOM_HAL)
			if not os.path.exists(filename):
				f1 = open(filename, "w")
				print >>f1, _("# Include your HAL commands here")
				print >>f1, _("# This file will not be overwritten when you run stepconf again").encode('utf-8')
		...
		# gui_merge.hal
		if (self.d.mix_gladevcp_pyvcp == True):
			filename = os.path.join(base, FILE_GUIMERGE)
			f1 = open(filename, "w")
			print >>f1, _("# These files are loaded post gladevcp and post pyvcp")
			print >>f1, _("# Generated by stepconf %(version)s at %(currenttime)s") % {'version':STEPCONF_VERSION, 'currenttime':time.asctime()}
			print >>f1, _("# If you make changes to this file, they will be").encode('utf-8')
			print >>f1, _("# overwritten when you run stepconf again").encode('utf-8')
			print >>f1, _("# _DO NOT_ include your HAL commands here.")
