...
class INI:
    ...
    def write_inifile(self, base):
        ...
        if os.path.exists(filename):
            shutil.copy2(
                filename, filename.replace(".ini", "_" + str(int(time.time())) + ".ini")
            )
        ...
        if self.d.axes == 0:
            ...
            num_joints = 4  # X Y Z A
        elif self.d.axes == 2:
            num_joints = 2  # X Z
        elif self.d.axes == 3:
            num_joints = 4  # X Y U V
        elif self.d.axes == 4:
            num_joints = 2  # X Y
        else:
        ...
        for axis in self.d.axislist:
            if "2" in axis:
                ...
                tandem = False
            self.write_one_axis(
                file, self.d.axislist.index(axis), axis[0], "LINEAR", all_homes, tandem
            )
    ...
    def write_one_axis(self, file, num, letter, type, all_homes, tandem):
        ...
        ignore = set(
            ("min-home-" + letter, "max-home-" + letter, "both-home-" + letter)
        )
        ...
        if homes:
            ...
            if all_homes:
                if self.d.axes == 3:  # XYUV
                    ...
                    if letter in self.d.tandemjoints:
                        print(
                            "HOME_SEQUENCE = -%s" % order["xyza".index(letter)],
                            file=file,
                        )
                    else:
                        print(
    ...
    def minperiod(self, steptime=None, stepspace=None, latency=None):
        ...
        if stepspace is None:
            stepspace = self.d.stepspace
        if latency is None:
            latency = self.d.latency
    ...
    def ideal_maxvel(self, scale):
        if self.a.doublestep():
            return abs(0.95 * 1e9 / self.d.ideal_period() / scale)
        else:
            return abs(0.95 * 0.5 * 1e9 / self.d.ideal_period() / scale)
