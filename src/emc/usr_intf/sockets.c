...
static int sockInitSockaddr(sockaddr_in *name,
                            const char *hostname,
...
int sockConnect(char *host, unsigned short int port)
{
...
#else
    {
        ...
        if (ioctlsocket(sock, FIONBIO, &tmp) == SOCKET_ERROR)
            rcs_print_error(
                "sock_connect: Error setting socket to non-blocking\n");
    }
#endif
...
char *sockGetError(void)
{
    ...
    strncpy(retString + strlen(retString),
            tmp,
...
int sockPrintfError(int fd, const char *format, ... /*args*/)
{
    ...
    va_start(ap, format);
    size = vsnprintf(
        buf + (sizeof(huh) - 1), sizeof(buf) - (sizeof(huh) - 1), format, ap);
    buf[sizeof(buf) - 1] = '\0';
    va_end(ap);
