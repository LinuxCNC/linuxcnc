                        ...
                        Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 1) {
        switch (emcWaitType) {
        case EMC_WAIT_RECEIVED: setresult(interp, "received"); break;
        case EMC_WAIT_DONE: setresult(interp, "done"); break;
        default: setresult(interp, "(invalid)"); break;
        }
        return TCL_OK;
    }
                    ...
                    Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 1) {
        ...
        case EMC_TASK_MODE::MANUAL: setresult(interp, "manual"); break;
        case EMC_TASK_MODE::AUTO: setresult(interp, "auto"); break;
        case EMC_TASK_MODE::MDI: setresult(interp, "mdi"); break;
        default: setresult(interp, "?"); break;
        }
        return TCL_OK;
    }
                     ...
                     Tcl_Obj *CONST objv[])
{
    ...
    setresult(interp, "emc_flood: need 'on', 'off', or no args");
    return TCL_ERROR;
}
                       ...
                       Tcl_Obj *CONST objv[])
{
    ...
    if (objc >= 2) {
        if (Tcl_GetIntFromObj(interp, objv[1], &spindle) !=
            TCL_OK) { // not a likely spindle index first, then
            spindle = 0;
            objstr = Tcl_GetStringFromObj(objv[1], 0);
        } else {
            if (spindle < 0 ||
                spindle >
                    EMCMOT_MAX_SPINDLES) { // should really be num_spindles, but not sure we know that here
    ...
    setresult(interp,
              "emc_spindle: need 'on', 'off', a spindle index or no args");
    return TCL_ERROR;
}
                     ...
                     Tcl_Obj *CONST objv[])
{
    ...
    if (objc >= 2) {
        if (Tcl_GetIntFromObj(interp, objv[1], &spindle) !=
            ...
            if (spindle < 0 ||
                spindle >
                    EMCMOT_MAX_SPINDLES) { // FIXME: should really be num_spindles, but not sure we know that here
                setresult(interp, "invalid spindle index number");
                return TCL_ERROR;
            }
            objstr = Tcl_GetStringFromObj(objv[2], 0);
                           ...
                           Tcl_Obj *CONST objv[])
{
    ...
    case 'x':
    case 'X':
        tlobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->task.toolOffset.tran.x));
        break;
    case 'y':
    case 'Y':
        tlobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->task.toolOffset.tran.y));
        break;
    case 'z':
    case 'Z':
        tlobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->task.toolOffset.tran.z));
        break;
    case 'a':
    case 'A':
        tlobj =
            Tcl_NewDoubleObj(convertAngularUnits(emcStatus->task.toolOffset.a));
        break;
    case 'b':
    case 'B':
        tlobj =
            Tcl_NewDoubleObj(convertAngularUnits(emcStatus->task.toolOffset.b));
        break;
    case 'c':
    case 'C':
        tlobj =
            Tcl_NewDoubleObj(convertAngularUnits(emcStatus->task.toolOffset.c));
        break;
    case 'u':
    case 'U':
        tlobj =
            Tcl_NewDoubleObj(convertLinearUnits(emcStatus->task.toolOffset.u));
        break;
    case 'v':
    case 'V':
        tlobj =
            Tcl_NewDoubleObj(convertLinearUnits(emcStatus->task.toolOffset.v));
        break;
    case 'w':
    case 'W':
        tlobj =
            Tcl_NewDoubleObj(convertLinearUnits(emcStatus->task.toolOffset.w));
        break;
    default:
                           ...
                           Tcl_Obj *CONST objv[])
{
    ...
    case 'X':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.position.tran.x));
        break;
    case 'y':
    case 'Y':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.position.tran.y));
        break;
    case 'z':
    case 'Z':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.position.tran.z));
        break;
    case 'a':
    case 'A':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.position.a));
        break;
    case 'b':
    case 'B':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.position.b));
        break;
    case 'c':
    case 'C':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.position.c));
        break;
    case 'u':
    case 'U':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.position.u));
        break;
    case 'v':
    case 'V':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.position.v));
        break;
    case 'w':
    case 'W':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.position.w));
        break;
    default:
                           ...
                           Tcl_Obj *CONST objv[])
{
    ...
    case 'X':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.actualPosition.tran.x));
        break;
    case 'y':
    case 'Y':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.actualPosition.tran.y));
        break;
    case 'z':
    case 'Z':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.actualPosition.tran.z));
        break;
    case 'a':
    case 'A':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.actualPosition.a));
        break;
    case 'b':
    case 'B':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.actualPosition.b));
        break;
    case 'c':
    case 'C':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.actualPosition.c));
        break;
    case 'u':
    case 'U':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.actualPosition.u));
        break;
    case 'v':
    case 'V':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.actualPosition.v));
        break;
    case 'w':
    case 'W':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.actualPosition.w));
        break;
    default:
                           ...
                           Tcl_Obj *CONST objv[])
{
    ...
    case 'a':
    case 'A':
        d = convertAngularUnits(
            emcStatus->motion.traj.position.a - emcStatus->task.g5x_offset.a -
            emcStatus->task.g92_offset.a - emcStatus->task.toolOffset.a);
        break;
    case 'b':
    case 'B':
        d = convertAngularUnits(
            emcStatus->motion.traj.position.b - emcStatus->task.g5x_offset.b -
            emcStatus->task.g92_offset.b - emcStatus->task.toolOffset.b);
        break;
    case 'c':
    case 'C':
        d = convertAngularUnits(
            emcStatus->motion.traj.position.c - emcStatus->task.g5x_offset.c -
            emcStatus->task.g92_offset.c - emcStatus->task.toolOffset.c);
        break;
    case 'u':
    case 'U':
        d = convertLinearUnits(
            emcStatus->motion.traj.position.u - emcStatus->task.g5x_offset.u -
            emcStatus->task.g92_offset.u - emcStatus->task.toolOffset.u);
        break;
    case 'v':
    case 'V':
        d = convertLinearUnits(
            emcStatus->motion.traj.position.v - emcStatus->task.g5x_offset.v -
            emcStatus->task.g92_offset.v - emcStatus->task.toolOffset.v);
        break;
    case 'w':
    case 'W':
        d = convertLinearUnits(
            emcStatus->motion.traj.position.w - emcStatus->task.g5x_offset.w -
                          ...
                          Tcl_Obj *CONST objv[])
{
    ...
    case 'X':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->task.g5x_offset.tran.x +
                               emcStatus->task.g92_offset.tran.x));
        break;
    case 'y':
    case 'Y':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->task.g5x_offset.tran.y +
                               emcStatus->task.g92_offset.tran.y));
        break;
    case 'z':
    case 'Z':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->task.g5x_offset.tran.z +
                               emcStatus->task.g92_offset.tran.z));
        break;
    case 'a':
    case 'A':
                        ...
                        Tcl_Obj *CONST objv[])
{
    ...
    if (objc != 5) {
        setresult(interp,
                  "emc_jog_incr: need jjogmode,joint, speed, and increment");
        return TCL_ERROR;
    }
                             ...
                             Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 1) {
        ...
        feedobj =
            Tcl_NewIntObj((int)(emcStatus->motion.traj.scale * 100.0 + 0.5));
                              ...
                              Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 1) {
        ...
        rapidobj = Tcl_NewIntObj(
            (int)(emcStatus->motion.traj.rapid_scale * 100.0 + 0.5));
                                ...
                                Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 1) {
        ...
        feedobj = Tcl_NewIntObj(
            (int)(emcStatus->motion.spindle[spindle].spindle_scale * 100.0 +
                  0.5));
        Tcl_SetObjResult(interp, feedobj);
        return TCL_OK;
    }
...
)
{
    ...
    default: setresult(interp, "idle"); return TCL_OK;
    }
...
)
{
    ...
    snprintf(
        string, sizeof(string), "F%.0f ", emcStatus->task.activeSettings[1]);
    rtapi_strxcat(codes_string, string);
    snprintf(string,
             sizeof(string),
             "S%.0f",
             fabs(emcStatus->task.activeSettings[2]));
    rtapi_strxcat(codes_string, string);
                          ...
                          Tcl_Obj *CONST objv[])
{
    ...
    if (TCL_OK == Tcl_GetIntFromObj(0, objv[1], &joint)) {
        ...
        case EMC_LINEAR: setresult(interp, "linear"); break;
        case EMC_ANGULAR: setresult(interp, "angular"); break;
        default: setresult(interp, "custom"); break;
        }
                           ...
                           Tcl_Obj *CONST objv[])
{
    ...
    if (TCL_OK == Tcl_GetIntFromObj(0, objv[1], &joint)) {
        ...
        case EMC_LINEAR:
            /* try mm */
            if (CLOSE(emcStatus->motion.joint[joint].units,
                      1.0,
                      LINEAR_CLOSENESS)) {
                setresult(interp, "mm");
                return TCL_OK;
            ...
            /* now try inch */
            else if (CLOSE(emcStatus->motion.joint[joint].units,
                           INCH_PER_MM,
                           LINEAR_CLOSENESS)) {
...
)
{
    ...
    /* now try inch */
    else if (CLOSE(emcStatus->motion.traj.linearUnits,
                   INCH_PER_MM,
                   LINEAR_CLOSENESS)) {
        setresult(interp, "inch");
        return TCL_OK;
...
)
{
    ...
    /* now try radians */
    else if (CLOSE(emcStatus->motion.traj.angularUnits,
                   RAD_PER_DEG,
                   ANGULAR_CLOSENESS)) {
        setresult(interp, "rad");
        return TCL_OK;
...
)
{
    ...
    case LINEAR_UNITS_INCH: setresult(interp, "inch"); break;
    case LINEAR_UNITS_MM: setresult(interp, "mm"); break;
    case LINEAR_UNITS_CM: setresult(interp, "cm"); break;
    case LINEAR_UNITS_AUTO:
        switch (emcStatus->task.programUnits) {
        case CANON_UNITS_MM: setresult(interp, "(mm)"); break;
        case CANON_UNITS_INCHES: setresult(interp, "(inch)"); break;
        case CANON_UNITS_CM: setresult(interp, "(cm)"); break;
        }
        break;
    default: setresult(interp, "custom"); break;
    }
...
)
{
    ...
    case ANGULAR_UNITS_DEG: setresult(interp, "deg"); break;
    case ANGULAR_UNITS_RAD: setresult(interp, "rad"); break;
    case ANGULAR_UNITS_GRAD: setresult(interp, "grad"); break;
                                      ...
                                      Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 1) {
        ...
        case LINEAR_UNITS_MM: setresult(interp, "mm"); break;
        case LINEAR_UNITS_CM: setresult(interp, "cm"); break;
        case LINEAR_UNITS_AUTO: setresult(interp, "auto"); break;
        default: setresult(interp, "custom"); break;
        }
        return TCL_OK;
    }
    ...
    setresult(interp,
              "emc_linear_unit_conversion: need 'inch', 'mm', 'cm', 'auto', "
              "'custom', or no args");
    return TCL_ERROR;
}
                                       ...
                                       Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 1) {
        ...
        case ANGULAR_UNITS_RAD: setresult(interp, "rad"); break;
        case ANGULAR_UNITS_GRAD: setresult(interp, "grad"); break;
        case ANGULAR_UNITS_AUTO: setresult(interp, "auto"); break;
        default: setresult(interp, "custom"); break;
        }
        return TCL_OK;
    }
    ...
    setresult(interp,
              "emc_angular_unit_conversion: need 'deg', 'rad', 'grad', 'auto', "
              "'custom', or no args");
    return TCL_ERROR;
}
                              ...
                              Tcl_Obj *CONST objv[])
{
    ...
    // get joint number
    if (0 != Tcl_GetIntFromObj(0, objv[1], &joint) || joint < 0 ||
        joint >= EMCMOT_MAX_JOINTS) {
        setresult(interp,
                  "emc_joint_backlash: need joint as integer, "
                  "0..EMCMOT_MAX_JOINTS-1");
        return TCL_ERROR;
    }
    // test for get or set
    if (objc == 2) {
                            ...
                            Tcl_Obj *CONST objv[])
{
    ...
    if (0 != Tcl_GetIntFromObj(0, objv[1], &joint) || joint < 0 ||
        ...
        setresult(
            interp,
            "emc_joint_enable: need joint as integer, 0..EMCMOT_MAX_JOINTS-1");
        return TCL_ERROR;
    }
                               ...
                               Tcl_Obj *CONST objv[])
{
    ...
    if (0 != Tcl_GetIntFromObj(0, objv[1], &joint) || joint < 0 ||
        joint >= EMCMOT_MAX_JOINTS) {
        setresult(interp,
                  "emc_joint_load_comp: need joint as integer, "
                  "0..EMCMOT_MAX_JOINTS-1");
        return TCL_ERROR;
    }
                      ...
                      Tcl_Obj *CONST objv[])
{
    ...
    Tcl_SetObjResult(
        interp,
        Tcl_NewIntObj(emcStatus->motion.traj.mode == EMC_TRAJ_MODE::TELEOP));
    return TCL_OK;
}
                          ...
                          Tcl_Obj *CONST objv[])
{
    ...
    case 'X':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.probedPosition.tran.x));
        break;
    case 'y':
    case 'Y':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.probedPosition.tran.y));
        break;
    case 'z':
    case 'Z':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.probedPosition.tran.z));
        break;
    case 'a':
    case 'A':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.probedPosition.a));
        break;
    case 'b':
    case 'B':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.probedPosition.b));
        break;
    case 'c':
    case 'C':
        posobj = Tcl_NewDoubleObj(
            convertAngularUnits(emcStatus->motion.traj.probedPosition.c));
        break;
    case 'u':
    case 'U':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.probedPosition.u));
        break;
    case 'v':
    case 'V':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.probedPosition.v));
        break;
    case 'w':
    case 'W':
        posobj = Tcl_NewDoubleObj(
            convertLinearUnits(emcStatus->motion.traj.probedPosition.w));
        break;
    default:
                       ...
                       Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 2) {
        ...
        if ((!strcmp(port, "/dev/psaux")) | (!strcmp(port, "/dev/ttyS0")) |
                ...
                if (strcmp(port, "/dev/psaux")) { // For Serial mice
                    ...
                    if (inBytes[1] != 77) {       // If first byte not "M"
                        fseek(
                            inFile,
                            0,
                            SEEK_CUR); // C standard: write-after-read needs this
                      ...
                      Tcl_Obj *CONST objv[])
{
    ...
    Tcl_SetObjResult(
        interp, Tcl_NewIntObj(val < 0.0 ? (int)(val - 0.5) : (int)(val + 0.5)));
    return TCL_OK;
}
...
extern "C" int Linuxcnc_Init(Tcl_Interp *interp);
int Linuxcnc_Init(Tcl_Interp *interp)
{
    ...
    Tcl_CreateObjCommand(interp,
                         "emc_task_command_number",
                         emc_task_command_number,
                         (ClientData)NULL,
                         (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateObjCommand(interp,
                         "emc_task_command_status",
                         emc_task_command_status,
                         (ClientData)NULL,
                         (Tcl_CmdDeleteProc *)NULL);
    ...
    Tcl_CreateObjCommand(interp,
                         "emc_io_command_number",
                         emc_io_command_number,
                         (ClientData)NULL,
                         (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateObjCommand(interp,
                         "emc_io_command_status",
                         emc_io_command_status,
                         (ClientData)NULL,
                         (Tcl_CmdDeleteProc *)NULL);
    ...
    Tcl_CreateObjCommand(interp,
                         "emc_motion_command_number",
                         emc_motion_command_number,
                         (ClientData)NULL,
                         (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateObjCommand(interp,
                         "emc_motion_command_status",
                         emc_motion_command_status,
                         (ClientData)NULL,
                         (Tcl_CmdDeleteProc *)NULL);
    ...
    Tcl_CreateObjCommand(
        interp, "int", localint, (ClientData)NULL, (Tcl_CmdDeleteProc *)NULL);
