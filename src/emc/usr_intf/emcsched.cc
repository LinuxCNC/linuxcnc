...
SchedEntry::SchedEntry()
    : priority(0), tagId(0), xpos(0.0), ypos(0.0), zpos(0.0), zone(0),
      programName(""), fileName(""), feedOverride(100.0),
      spindleOverride(100.0), tool(1)
{
}
...
{
    if ((qs == qsResume) && (queueStatus == qsPause))
        queueStatus = qsRun;
    else
        queueStatus = qs;
}
            ...
            if (isIdle()) {
                ...
                if (interlocksOk()) {
                    sendFeedOverride(((double)q.front().getFeedOverride()) /
                                     100.0);
                    sendSpindleOverride(
                        0, ((double)q.front().getSpindleOverride()) / 100.0);
                    sendMdi();
                    sendMdiCmd("G92.1\n");
...
int getProgramByIndex(int idx, qRecType *qRec)
{
    ...
    for (i = q.begin(); idx > 0; ++i) {
        if (i == q.end())
            return -1; // return if idx was larger than number of entries
        --idx;
    }
    qRec->priority = i->getPriority();
    qRec->tagId = i->getTagId();
...
int deleteProgramById(int id)
{
    ...
    for (i = q.begin(); i != q.end(); ++i) {
        if (i->getTagId() == id) {
            if (i->getPriority() == (int)MAX_PRIORITY)
                return -1;
            i = q.erase(i);
            return 0;
        }
