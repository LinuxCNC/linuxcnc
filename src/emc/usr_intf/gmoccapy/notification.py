...
class Notification(Gtk.Window):
    ...
    __gproperties__ = {
        "icon_size": (
            GObject.TYPE_INT,
            "Icon Size",
            "Sets the size of the displayed button icon",
            1,
            96,
            16,
        ...
        "message_width": (
            GObject.TYPE_INT,
            "Message Width",
            "Sets the message width in pixel",
            0,
            800,
            300,
            GObject.ParamFlags.READWRITE | GObject.ParamFlags.CONSTRUCT,
        ...
        "x_pos": (
            GObject.TYPE_INT,
            "Frame X Position",
            "Sets the frame X position in pixel",
            0,
            1280,
            0,
            GObject.ParamFlags.READWRITE | GObject.ParamFlags.CONSTRUCT,
        ...
        "y_pos": (
            GObject.TYPE_INT,
            "Frame Y Position",
            "Sets the frame Y position in pixel",
            0,
            1024,
            0,
            GObject.ParamFlags.READWRITE | GObject.ParamFlags.CONSTRUCT,
        ...
        "top_to_bottom": (
            ...
            "Show the second notification under the first one or on top?",
            True,
            GObject.ParamFlags.READWRITE | GObject.ParamFlags.CONSTRUCT,
        ),
        "use_frames": (
            GObject.TYPE_BOOLEAN,
            "Use Frames for messages",
            "You can separate the messages using frames, but you will need more space",
            True,
            GObject.ParamFlags.READWRITE | GObject.ParamFlags.CONSTRUCT,
        ),
        "icon_theme_path": (
            GObject.TYPE_STRING,
            "Icon theme lookup path",
            "Pathes where to look for icon themes",
            "",
            GObject.ParamFlags.READWRITE | GObject.ParamFlags.CONSTRUCT,
        ),
        "icon_theme_name": (
            GObject.TYPE_STRING,
            "Icon theme name",
            "Name set in gmoccapy preferences",
            "classic",
            GObject.ParamFlags.READWRITE | GObject.ParamFlags.CONSTRUCT,
        ),
    }
    ...
    def _show_message(self, message):
        ...
        pixbuf = icon_theme_helper.load_symbolic_from_icon_theme(
            self.icon_theme, icon_name, self.icon_size, default_style
        )
        ...
        image = Gtk.Image()
        pixbuf = icon_theme_helper.load_symbolic_from_icon_theme(
            self.icon_theme, "window_close", self.icon_size, default_style
        )
    ...
    def del_message(self, messagenumber):
        ...
        if messagenumber == -1:
            ...
            self.add_message(
                _(
                    "Error trying to delete the message with number {0}".format(
                        messagenumber
                    ),
                    None,
                )
        ...
        try:
            ...
            if name in list(self.__gproperties.keys()):
                ...
                raise AttributeError(
                    "unknown notification set_property %s" % property.name
                )
        except AttributeError as e:
            print(
                "Attribute error in property:",
                property,
                "type:",
                type(value),
                "value:",
...
def main():
    ...
    notification.icon_theme.append_search_path(
        "../../../../share/gmoccapy/icons/"
    )  # relative from this file location
    ...
    notification.add_message(
        "Hallo World this is a long string that have a linebreak ", "dialog_information"
    )
