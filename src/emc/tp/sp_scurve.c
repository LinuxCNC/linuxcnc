    ...
    *len = 0;
    ...
    if (fabs(A) < NEAR_ZERO && fabs(B) < NEAR_ZERO) {
        ...
        double Y1_value = (Y1 == 0.0) ? 0.0
                                      : (Y1 > 0.0 ? exp(log(Y1) / 3.0)
                                                  : -exp(log(-Y1) / 3.0));
        double Y2_value = (Y2 == 0.0) ? 0.0
                                      : (Y2 > 0.0 ? exp(log(Y2) / 3.0)
                                                  : -exp(log(-Y2) / 3.0));
        ...
        double i_value =
            sqrt(3.0) / 2.0 * (Y1_value - Y2_value) / (3.0 * a_norm);
        if (fabs(i_value) < 1e-1) {
            res[(*len)++] =
                (-b_norm + 0.5 * (Y1_value + Y2_value)) / (3.0 * a_norm);
        }
    } else {
        ...
        res[(*len)++] =
            (-b_norm + sqrt_A * (cos_S3 + sqrt_3 * sin_S3)) / (3.0 * a_norm);
        res[(*len)++] =
            (-b_norm + sqrt_A * (cos_S3 - sqrt_3 * sin_S3)) / (3.0 * a_norm);
    }
               ...
               double *req_a)
{
    ...
    if (V1 >=
        maxV /
            2) { // Handle case where jerk is small, causing T2 and T6 segments to not exist
        T1 = sqrt(maxV / maxJ);
    }
    ...
    if (phase == 4) {   //
        if (v < maxV) { // Need to accelerate to target speed
            ...
            if (phase == 7) {
                ...
                phase ==
                5) { // Need compensation; after discretization each segment is s = v * t, so distance may be more reliable
            ...
            v >
            maxV) { // Need to decelerate to target speed; triangular acceleration algorithm used here; entered when velocity is modified
            double tt = fabs(a) / maxJ;
        ...
        } else if (needTime > T1 + T2 &&
                   needTime <= T1 + T2 + T3) { // Handle S3 segment
               ...
               double *J4)
{
    ...
    *J2 = *J4 = Jm;
    ...
    //printf("Y1: %f | Y2: %f \n", Y1, Y2);
    if (Y1 >
        0) { // Residual velocity Y1 * Jm * T^2 exists; n2 needs to increase by one period to distribute residual velocity
        *J2 =
            -1 * (2 * Y1 + n[2] * (n[2] + 1)) * Jm / ((n[2] + 1) * (n[2] + 2));
        rn2 = n[2] + 1;
    } else
        *J2 = -(*J2);

    if (Y2 >
        0) { // Residual velocity Y2 * Jm * T^2 exists; n6 needs to increase by one period to distribute residual velocity
    ...
    double Verr =
        Serr / T; // This velocity needs to be inserted at appropriate location
...
getNextPoint(simple_tp_t *tp, int n, double T, double *req_v, double *req_a)
{
    ...
    if (n <= tp->n0) { // S0 segment
        ...
        phase = 4;
               ...
               n <= tp->n0 + tp->n1 + tp->n2 + tp->n3 + tp->n4 + tp->n5) {
        phase = 5;
        double x = n - (tp->n0 + tp->n1 + tp->n2 + tp->n3 + tp->n4);
        cal_v = tp->v5 + x * tp->a6 * T;
        cal_a = tp->a6;
    } else if (n <=
               tp->n0 + tp->n1 + tp->n2 + tp->n3 + tp->n4 + tp->n5 + tp->n6) {
        phase = 6;
        double x =
            tp->n0 + tp->n1 + tp->n2 + tp->n3 + tp->n4 + tp->n5 + tp->n6 - n;
        cal_v = tp->v7 + fabs(tp->j4) * T * T * x * (x + 1) / 2.0;
        cal_a = tp->a6 - x * fabs(tp->j4) * T;
    } else {
        if (tp->n0 + tp->n1 + tp->n2 + tp->n3 + tp->n4 + tp->n5 + tp->n6 ==
            n + 1) {
            tp->curr_n++;
            phase = 7;
            cal_v = tp->ve;
...
double
stoppingDist(double v, double a, double maxA, double maxJ /*, int* phase*/)
{
    // Already stopped
    //*phase = 0;
    if (fabs(v) < 0.0001)
...
double finishWithSpeedDist(
    double v, double ve, double a, double maxA, double maxJ /*, int* phase*/
)
{
    // Already stopped
    //*phase = 0;
    ...
    double t, double targetV, double v, double a, double maxA, double maxJ)
{
    ...
    vel_err = targetV - v;
    if (vel_err > tiny_da) {
        acc_req = -max_da + sqrt(2.0 * maxJ * vel_err + max_da * max_da);
...
double
calculate_t2_optimized(double Ve, double distence, double maxA, double maxJ)
{
    ...
    const double term3 =
        (maxA_squared * maxA_squared) * (0.25 * maxJ_inv * maxJ_inv);
    ...
    double t2_1 =
        fma(-(2.0 * Ve - 2.0 * sqrt_result + common_term), factor, 0.0);
    double t2_2 =
        fma(-(2.0 * Ve + 2.0 * sqrt_result + common_term), factor, 0.0);
...
calcDecelerateTimes(double v, double amax, double jerk, double *t1, double *t2)
{
    ...
    if (V1 >=
        v / 2) { // Handle case where jerk is small, causing T2 and T6 segments to not exist
        T1 = sqrt(v / jerk);
    }
    if (V1 < v / 2) {
        T2 = (v / amax) - T1;
