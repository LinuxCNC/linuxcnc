...
int PythonPlugin::call(const char *module,
                       const char *callable,
                       bp::object tupleargs,
                       bp::object kwargs,
                       bp::object &retval)
{
    ...
    try {
        ...
        // this does
        PyObject *rv =
            PyObject_Call(function.ptr(), tupleargs.ptr(), kwargs.ptr());
        if (PyErr_Occurred())
...
bool PythonPlugin::is_callable(const char *module, const char *funcname)
{
    ...
    reload();
    if ((status != PLUGIN_OK) || (funcname == NULL)) {
        return false;
    }
    try {
        if (module == NULL) { // default to function in toplevel module
    ...
    if (unexpected)
        logPP(0,
              "is_callable(%s%s%s): unexpected exception:\n%s",
              module ? module : "",
              module ? "." : "",
              funcname,
              exception_msg.c_str());
    ...
    if (Py_IsInitialized()) {
        try {
            ...
            for (unsigned i = 0; i < inittab_entries.size(); i++) {
                main_namespace[inittab_entries[i]] =
                    bp::import(inittab_entries[i].c_str());
            }
            if (toplevel) // only execute a file if there's one configured.
                bp::object result =
                    bp::exec_file(abs_path, main_namespace, main_namespace);
            status = PLUGIN_OK;
        } catch (bp::error_already_set &) {
            if (PyErr_Occurred()) {
                exception_msg = handle_pyerror();
        ...
        if (status == PLUGIN_INIT_EXCEPTION) {
            logPP(-1,
                  "initialize: module '%s' init failed: \n%s",
                  abs_path,
                  exception_msg.c_str());
        }
    } else {
...
PythonPlugin::PythonPlugin(struct _inittab *inittab)
    : status(0), module_mtime(0), reload_on_change(0), toplevel(0), abs_path(0),
      log_level(0)
{
    PyConfig config;
...
int PythonPlugin::configure(const char *iniFilename, const char *section)
{
    ...
    int n = 1;
    int lineno;
    while ((inistring = inifile.Find("PATH_PREPEND", "PYTHON", n, &lineno))) {
        if (inifile.TildeExpansion(
                *inistring, expandinistring, sizeof(expandinistring))) {
            logPP(-1, "TildeExpansion failed  '%s'", toplevel);
        ...
        if (PyRun_SimpleString(pycmd)) {
            logPP(-1,
                  "%s:%d: exception running '%s'",
                  iniFilename,
                  lineno,
                  pycmd);
            exception_msg =
                "exception running:" + std::string((const char *)pycmd);
            status = PLUGIN_EXCEPTION_DURING_PATH_PREPEND;
            return status;
        }
        n++;
    ...
    n = 1;
    while ((inistring = inifile.Find("PATH_APPEND", "PYTHON", n, &lineno))) {
        if (inifile.TildeExpansion(
                *inistring, expandinistring, sizeof(expandinistring))) {
            logPP(-1, "TildeExpansion failed  '%s'", toplevel);
