...
void print_nc_line_number()
{
    ...
    if ((k < 256) && ((text[k] == 'n') || (text[k] == 'N'))) {
        fputc('N', _outfile);
        for (k++, m = 0; ((k < 256) && (text[k] >= '0') && (text[k] <= '9'));
             k++, m++)
...
)
{
    ...
    ECHO_WITH_ARGS(
        "%d, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f", index, x, y, z, a, b, c);
    ...
    _sai._program_position_a =
        _sai._program_position_a + _sai._g5x_a - a; /*AA*/
    _sai._program_position_b =
        _sai._program_position_b + _sai._g5x_b - b; /*BB*/
    _sai._program_position_c =
        _sai._program_position_c + _sai._g5x_c - c; /*CC*/
...
)
{
    ...
    _sai._program_position_a =
        _sai._program_position_a + _sai._g92_a - a; /*AA*/
    _sai._program_position_b =
        _sai._program_position_b + _sai._g92_b - b; /*BB*/
    _sai._program_position_c =
        _sai._program_position_c + _sai._g92_c - c; /*CC*/
...
)
{
    ...
    _sai._program_position_x =
        nurbs_control_points[nurbs_control_points.size() - 1].NURBS_X;
    _sai._program_position_y =
        nurbs_control_points[nurbs_control_points.size() - 1].NURBS_Y;
}
...
void RIGID_TAP(
    int /*line_number*/, double x, double y, double z, double /*scale*/
)
{
...
void START_SPINDLE_CLOCKWISE(int spindle, int /*wait_for_atspeed*/)
{
    ...
    _sai._spindle_turning[spindle] =
        ((_sai._spindle_speed[spindle] == 0) ? CANON_STOPPED : CANON_CLOCKWISE);
}
...
void START_SPINDLE_COUNTERCLOCKWISE(int spindle, int /*wait_for_atspeed*/)
{
    ...
    _sai._spindle_turning[spindle] =
        ((_sai._spindle_speed[spindle] == 0) ? CANON_STOPPED
...
/* Tool Functions */
void SET_TOOL_TABLE_ENTRY(int idx,
                          int toolno,
                          ...
                          int orientation)
{
    ...
    ECHO_WITH_ARGS(
        "%d, %d, %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f, %.4f, %.4f, %d",
        idx,
        toolno,
        offset.tran.x,
        offset.tran.y,
...
void CLAMP_AXIS(CANON_AXIS axis)
{
    PRINT("CLAMP_AXIS(%s)\n",
          ...
          : (axis == CANON_AXIS_C) ? "CANON_AXIS_C"
                                   : "UNKNOWN");
}
...
{
    _sai.block_delete = state;
} //state == ON, means we don't interpret lines starting with "/"

bool GET_BLOCK_DELETE()
{
    return _sai.block_delete;
} //state == ON, means we  don't interpret lines starting with "/"

void SET_OPTIONAL_PROGRAM_STOP(bool state)
{
    _sai.optional_program_stop = state;
} //state == ON, means we stop

bool GET_OPTIONAL_PROGRAM_STOP()
{
    return _sai.optional_program_stop;
} //state == ON, means we stop
...
{
    return _sai._external_length_units;
}
...
StandaloneInterpInternals::StandaloneInterpInternals()
    : _active_plane(CANON_PLANE::XY), _active_slot(1), _selected_tool(0),
      _feed_mode(0), _feed_rate(0.0), _flood(0), _external_length_units(1.0),
      _length_unit_factor(1), /* 1 for MM 25.4 for inch */
      _length_unit_type(CANON_UNITS_MM), _line_number(1), _mist(0),
      _motion_mode(CANON_CONTINUOUS), _probe_position_a(0), /*AA*/
      _probe_position_b(0),                                 /*BB*/
      _probe_position_c(0),                                 /*CC*/
      _probe_position_x(0), _probe_position_y(0), _probe_position_z(0),
      _g5x_x(0), _g5x_y(0.0), _g5x_z(0.0), _g5x_a(0.0), _g5x_b(0.0),
      _g5x_c(0.0), _g92_x(0.0), _g92_y(0.0), _g92_z(0.0), _g92_a(0.0),
      _g92_b(0.0), _g92_c(0.0), _program_position_a(0), /*AA*/
      _program_position_b(0),                           /*BB*/
      _program_position_c(0),                           /*CC*/
      _program_position_x(0), _program_position_y(0), _program_position_z(0),
      _spindle_speed{0}, _spindle_turning{CANON_STOPPED},
      _pockets_max(CANON_POCKETS_MAX), _tools{},
      ...
      motion_tolerance(0.), naivecam_tolerance(0.),
      /* Dummy status variables */
      _traverse_rate(0.0),

      _tool_offset({}), _toolchanger_fault(false), _toolchanger_reason(0)
{
}
