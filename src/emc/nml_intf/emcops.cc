...
EMC_AXIS_STAT::EMC_AXIS_STAT()
    : EMC_AXIS_STAT_MSG(EMC_AXIS_STAT_TYPE, sizeof(EMC_AXIS_STAT)),
      minPositionLimit(0.0), maxPositionLimit(0.0), velocity(0.0)
{
}

EMC_JOINT_STAT::EMC_JOINT_STAT()
    : EMC_JOINT_STAT_MSG(EMC_JOINT_STAT_TYPE, sizeof(EMC_JOINT_STAT)),
      jointType(EMC_LINEAR), units(1.0), backlash(0.0), minPositionLimit(-1.0),
      maxPositionLimit(1.0), maxFerror(1.0), minFerror(1.0), ferrorCurrent(0.0),
      ferrorHighMark(0.0), output(0.0), input(0.0), velocity(0.0), inpos(1),
      homing(0), homed(0), fault(0), enabled(0), minSoftLimit(0),
...
EMC_TRAJ_STAT::EMC_TRAJ_STAT()
    : EMC_TRAJ_STAT_MSG(EMC_TRAJ_STAT_TYPE, sizeof(EMC_TRAJ_STAT)),
      linearUnits(1.0), angularUnits(1.0), cycleTime(0.0), joints(1),
      spindles(1), axis_mask(1), mode(EMC_TRAJ_MODE::FREE), enabled(OFF),
      inpos(ON), queue(0), activeQueue(0), queueFull(OFF), id(0), paused(OFF),
      scale(0.0), rapid_scale(0.0),

      position{}, actualPosition{},

      velocity(1.0), acceleration(1.0), maxVelocity(1.0), maxAcceleration(1.0),

      probedPosition{}, probe_tripped(OFF), probing(OFF), probeval(0),

      kinematics_type(0), motion_type(0), distance_to_go(0.0), dtg{},
      current_vel(0.0), feed_override_enabled(OFF), adaptive_feed_enabled(OFF),
      feed_hold_enabled(OFF), tag()
{
}

EMC_MOTION_STAT::EMC_MOTION_STAT()
    : EMC_MOTION_STAT_MSG(EMC_MOTION_STAT_TYPE, sizeof(EMC_MOTION_STAT)),
      synch_di{}, synch_do{}, analog_input{}, analog_output{}, misc_error{},
      debug(0), on_soft_limit(0), external_offsets_applied(0), eoffset_pose{},
      numExtraJoints(0), jogging_active(0)
{
}

EMC_TASK_STAT::EMC_TASK_STAT()
    : EMC_TASK_STAT_MSG(EMC_TASK_STAT_TYPE, sizeof(EMC_TASK_STAT)),
      mode(EMC_TASK_MODE::MANUAL), state(EMC_TASK_STATE::ESTOP),
      execState(EMC_TASK_EXEC::DONE), interpState(EMC_TASK_INTERP::IDLE),
      callLevel(0), motionLine(0), currentLine(0), readLine(0),
      optional_stop_state(OFF), block_delete_state(OFF), input_timeout(OFF),
      file{}, command{}, ini_filename{}, g5x_offset{}, g5x_index(0),
      g92_offset{}, rotation_xy(0.0), toolOffset{}, activeSettings{},
      programUnits(CANON_UNITS_MM), interpreter_errcode(0), task_paused(0),
      delayLeft(0.0), queuedMDIcommands(0)
{
    int t;
...
EMC_AUX_STAT::EMC_AUX_STAT()
    : EMC_AUX_STAT_MSG(EMC_AUX_STAT_TYPE, sizeof(EMC_AUX_STAT)), estop(1)
{
}

EMC_SPINDLE_STAT::EMC_SPINDLE_STAT()
    : EMC_SPINDLE_STAT_MSG(EMC_SPINDLE_STAT_TYPE, sizeof(EMC_SPINDLE_STAT)),
      speed(0.0), spindle_scale(1.0), css_maximum(0.0), css_factor(1.0),
      state(0), direction(0), brake(1), increasing(0), enabled(0),
      orient_state(0), orient_fault(0), spindle_override_enabled(OFF),
      homed(OFF)
{
}
...
EMC_STAT::EMC_STAT() : EMC_STAT_MSG(EMC_STAT_TYPE, sizeof(EMC_STAT)), debug(0)
{
}
