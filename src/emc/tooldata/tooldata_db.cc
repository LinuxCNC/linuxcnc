...
static int read_reply(char reply[], int len)
{
    ...
    if (!readready()) {
        fprintf(stderr,
                "!!!! tooldata_db:read_reply fail: timeout_ms=%d\n",
                READ_TIMEOUT_MS);
        return -1;
    }
...
{
    if (getenv((char *)"DB_DEBUG")) {
        db_debug = 1;
    }
    if (getenv((char *)"DB_SHOW")) {
        db_show = 1;
    }
    int child_argc = 0;
    ...
    // block for response
    fprintf(stderr,
            "====Waiting for %s version reply from %s\n",
            DB_VERSION,
            child_argv[0]);
    char reply[CANON_TOOL_ENTRY_LEN];
    ...
    if (read_reply(reply, sizeof(reply)) < 0) {
        fprintf(stderr,
                "!!!! tooldata_db_init: read_reply initial read failed\n");
        db_live = 0;
        return -1;
    } else {
                       ...
                       CANON_TOOL_TABLE tdata)
{
    ...
    case SPINDLE_UNLOAD: // 'u' command
        ...
        if (db_debug) {
            fprintf(stderr, "SPINDLE_UNLOAD:%s\n", msg);
        }
        break;
