...
int main(int argc, char *argv[])
{
    ...
    if (0 != loadToolTable(tool_table_file, emcioStatus.tool.toolTable, ttcomments, random_toolchanger)) {
	...
	case EMC_TOOL_INIT_TYPE:
	    ...
	    loadToolTable(tool_table_file, emcioStatus.tool.toolTable, ttcomments, random_toolchanger);
	...
	case EMC_TOOL_LOAD_TOOL_TABLE_TYPE:
	{
	    ...
	    if (0 != loadToolTable(filename, emcioStatus.tool.toolTable, ttcomments, random_toolchanger))
		emcioStatus.status = RCS_ERROR;
	    else
		reload_tool_number(emcioStatus.tool.toolInSpindle);
	}
