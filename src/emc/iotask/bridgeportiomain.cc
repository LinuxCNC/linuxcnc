/*
  ...
  Main controller for Bridgeport I/O: spindle, coolant,
  lube, estop auxiliary

  Modification history:

  15-May-2002 P.C. Changed PARPORT_IO_BASE_ADDRESS to IO_BASE_ADDRESS
  10-Nov-1999 FMP If'd out RCS_TIMER::wait() calls.
  3-Sep-1999  FMP looked for PARPORT_IO_ADDRESS in EMCIO section first;
  used EMC_DEBUG to inhibit version printing
  31-Aug-1999  FMP changed to bridgeportiomain.cc; took out -nml option
  ...
  2-Apr-1998  FMP created from emciomain.cc
  */
...
#include "parport.h"            // IO_BASE_ADDRESS
...
static int iniLoad(const char *filename)
{
  ...
  // do miscellaneous ones

  saveInt = IO_BASE_ADDRESS;
  // look for it as [EMCIO] section first
  if (NULL != (inistring = inifile.find("IO_BASE_ADDRESS", "EMCIO"))) {
    if (1 == sscanf(inistring, "%li", &IO_BASE_ADDRESS)) {
      // found it
    }
    else {
      // found, but invalid
      IO_BASE_ADDRESS = saveInt;
      rcs_print("invalid [EMCIO] IO_BASE_ADDRESS in %s (%s); using default %X\n",
                filename, inistring, IO_BASE_ADDRESS);
    }
  }
  else if (NULL != (inistring = inifile.find("IO_BASE_ADDRESS", "EMC"))) {
    if (1 == sscanf(inistring, "%li", &IO_BASE_ADDRESS)) {
      // found it
    }
    else {
      // found, but invalid
      IO_BASE_ADDRESS = saveInt;
      rcs_print("invalid [EMC] IO_BASE_ADDRESS in %s (%s); using default %X\n",
                filename, inistring, IO_BASE_ADDRESS);
    }
  }
  else {
    // not found, using default
    rcs_print("[EMC] IO_BASE_ADDRESS not found in %s; using default %X\n",
              filename, IO_BASE_ADDRESS);
  }
...
/*
  syntax: a.out {-ini <inifile>}
  */
int main(int argc, char *argv[])
{
  EMC_TOOL_MODULE *tool;
  EMC_SPINDLE_MODULE *spindle;
  EMC_COOLANT_MODULE *coolant;
  EMC_AUX_MODULE *aux;
  EMC_LUBE_MODULE *lube;
  RCS_TIMER *timer;
  ...
  spindle = new EMC_SPINDLE_MODULE();
  coolant = new EMC_COOLANT_MODULE();
  aux = new EMC_AUX_MODULE();
  lube = new EMC_LUBE_MODULE();
