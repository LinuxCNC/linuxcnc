...
static int loadCoolant()
{
  ...
  if (NULL != (inistring = coolantInifile->find("MIST_COOLANT_INDEX", "EMCIO")))
    {
  ...
  if (NULL != (inistring = coolantInifile->find("FLOOD_COOLANT_INDEX", "EMCIO")))
    {
      if (1 == sscanf(inistring, "%d", &i))
        {
          // found, and valid
  ...
  if (NULL != (inistring = coolantInifile->find("MIST_COOLANT_POLARITY", "EMCIO")))
    {
      if (1 == sscanf(inistring, "%d", &i))
        {
          // found, and valid
          if (0 != emcCoolantSetMistPolarity(i))
            {
              printf("bad return value from emcCoolantSetMistPolarity\n");
              retval = -1;
            }
        }
      else
  ...
  if (NULL != (inistring = coolantInifile->find("FLOOD_COOLANT_POLARITY", "EMCIO")))
    {
      if (1 == sscanf(inistring, "%d", &i))
        {
          // found, and valid
          if (0 != emcCoolantSetFloodPolarity(i))
            {
              printf("bad return value from emcCoolantSetFloodPolarity\n");
              retval = -1;
            }
        }
      else
        {
          // found, but invalid, so warn
          printf("invalid inifile value for FLOOD_COOLANT_POLARITY: %s\n",
                          inistring);
        }
    }
  // else ignore omission
