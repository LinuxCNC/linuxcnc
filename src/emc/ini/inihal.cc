...
#define CHANGED_IDX(NAME, IDX)                                                 \
    ((old_inihal_data.NAME[IDX]) - (new_inihal_data.NAME[IDX]) >               \
     +(EPSILON)) ||                                                            \
        ((old_inihal_data.NAME[IDX]) - (new_inihal_data.NAME[IDX]) <           \
         -(EPSILON))
...
#define MAKE_BIT_PIN(NAME, DIR)                                                \
    do {                                                                       \
        retval = hal_pin_bit_newf(                                             \
            DIR, &(the_inihal_data->NAME), comp_id, PREFIX #NAME);             \
        if (retval < 0)                                                        \
            return retval;                                                     \
    } while (0)
...
#define MAKE_S32_PIN_IDX(NAME, HALPIN_NAME, DIR, IDX)                          \
    do {                                                                       \
        retval = hal_pin_s32_newf(DIR,                                         \
                                  &(the_inihal_data->NAME[IDX]),               \
                                  comp_id,                                     \
                                  PREFIX "%d." #HALPIN_NAME,                   \
                                  IDX);                                        \
        if (retval < 0)                                                        \
            return retval;                                                     \
    } while (0)
...
int ini_hal_init(int numjoints)
{
    ...
    for (int idx = 0; idx < numjoints; idx++) {
        ...
        MAKE_FLOAT_PIN_IDX(
            joint_max_acceleration, max_acceleration, HAL_IN, idx);
    ...
    for (int idx = 0; idx < EMCMOT_MAX_AXIS; idx++) {
        ...
        MAKE_FLOAT_PIN_LETTER(
            axis_max_velocity, max_velocity, HAL_IN, idx, letter);
        MAKE_FLOAT_PIN_LETTER(
            axis_max_acceleration, max_acceleration, HAL_IN, idx, letter);
...
int check_ini_hal_items(int numjoints)
{
    ...
    if (CHANGED(traj_default_velocity)) {
        ...
        if (0 != emcTrajSetVelocity(0, NEW(traj_default_velocity))) {
            rcs_print("check_ini_hal_items:bad return value from "
                      "emcTrajSetVelocity\n");
        }
    }
    ...
    if (CHANGED(traj_planner_type)) {
        if (debug)
            SHOW_CHANGE_INT(traj_planner_type)
    ...
    for (int idx = 0; idx < numjoints; idx++) {
        ...
        if (CHANGED_IDX(joint_min_limit, idx)) {
            ...
            if (0 !=
                emcJointSetMinPositionLimit(idx, NEW(joint_min_limit[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from "
                                    "emcJointSetMinPositionLimit\n");
                }
            }
        }
        if (CHANGED_IDX(joint_max_limit, idx)) {
            ...
            if (0 !=
                emcJointSetMaxPositionLimit(idx, NEW(joint_max_limit[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from "
                                    "emcJointSetMaxPositionLimit\n");
                }
            }
        }
        if (CHANGED_IDX(joint_max_velocity, idx)) {
            ...
            if (0 !=
                emcJointSetMaxVelocity(idx, NEW(joint_max_velocity[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from "
                                    "emcJointSetMaxVelocity\n");
                }
            }
        }
        if (CHANGED_IDX(joint_max_acceleration, idx)) {
            ...
            if (0 != emcJointSetMaxAcceleration(
                         idx, NEW(joint_max_acceleration[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from "
                                    "emcJointSetMaxAcceleration\n");
                }
            }
        }
        if (CHANGED_IDX(joint_jerk, idx)) {
            if (debug)
                SHOW_CHANGE_IDX(joint_jerk, idx);
        ...
        if (CHANGED_IDX(joint_home, idx) ||
            ...
            if (0 !=
                emcJointUpdateHomingParams(idx,
                                           NEW(joint_home[idx]),
                                           NEW(joint_home_offset[idx]),
                                           NEW(joint_home_sequence[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from "
                                    "emcJointUpdateHomingParams\n");
                }
            }
        }
    ...
    for (int idx = 0; idx < EMCMOT_MAX_AXIS; idx++) {
        if (CHANGED_IDX(axis_min_limit, idx)) {
            ...
            if (0 !=
                emcAxisSetMinPositionLimit(idx, NEW(axis_min_limit[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from "
        ...
        if (CHANGED_IDX(axis_max_limit, idx)) {
            ...
            if (0 !=
                emcAxisSetMaxPositionLimit(idx, NEW(axis_max_limit[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from "
        ...
        if (CHANGED_IDX(axis_max_velocity, idx)) {
            ...
            if (0 != emcAxisSetMaxVelocity(idx,
                                           (1 - ext_offset_a_or_v_ratio[idx]) *
                                               NEW(axis_max_velocity[idx]),
                                           (ext_offset_a_or_v_ratio[idx]) *
                                               NEW(axis_max_velocity[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from "
