...
if version == "$Revision$":
    ...
    t = (
        """The supplied INI file is already at version %s and should not need
    updating"""
        % version
    )
    if dialogs:
...
if ini.find("KINS", "JOINTS") and not force and not version == "1.0":
    if dialogs:
        ...
        if (
            input(
                "The supplied INI file already has a [KINS] section."
...
backupdir = os.path.join(
    basedir, os.path.splitext(os.path.basename(filename))[0] + ".old"
)
...
for f in os.listdir(basedir):
    if os.path.isdir(os.path.join(basedir, os.path.basename(f))):
        ...
        shutil.copy(
            os.path.join(basedir, os.path.basename(f)),
...
for halfile in halfiles:
    if os.path.isfile(os.path.join(basedir, halfile)):
        ...
        os.system(
            "cp "
            + os.path.join(basedir, "hallib", halfile)
            + " "
...
if version == "$Revision$" or version < "1.0":
    ...
    newini.write(
        "# This config file was created %s by the update_ini script\n"
        % datetime.datetime.now()
    )
    newini.write(
        "# The original config files may be found in the %s directory\n\n" % backupdir
    )
    ...
    if section != None:
        ...
        if features != None:
            ...
            if features & 0x2:
                section += "OWORD_NARGS = 1\n"
            if features & 0x10:
                section += "NO_DOWNCASE_OWORD = 1\n"
            if features & 0x20:
                section += "OWORD_WARNONLY = 1\n"
    section += "\n"
    newini.write(section)
    ...
    if coordinates != None:
        joints = len(coordinates)
    if joints == None:
        joints = ini.find("TRAJ", "AXES")
    if joints == None:
        joints = "3"
    joints = int(joints)
    if coordinates == None:
    ...
    kinstype = None
    coords_entry = False
    if coordinates != "XYZABCUVW"[:joints]:
        coords_entry = True
    ...
    if section != None:
        if not re.search("MAX_LINEAR_VELOCITY", section):
            if re.search("MAX_VELOCITY", section):
                ...
                mv = re.findall(
                    r"MAX_VELOCITY[\s=]+(\d*(\.\d+)?)", inistring, re.MULTILINE
                )
                section = (
                    "\n# this value based on fastest single axis"
                    + "\nMAX_LINEAR_VELOCITY = %s" % max(mv)[0]
                    + section
                )
        ...
        if not re.search("DEFAULT_ACCELERATION", section):
            section = re.sub(
                "DEFAULT_ACCELERATION", "DEFAULT_LINEAR_ACCELERATION", section
            )
        print("COORDINATES = %s\n" % "".join(coordinates))
        section = re.sub(
            "COORDINATES.*", "COORDINATES = %s" % "".join(coordinates[:joints]), section
        )
    ...
    if coords_entry:
        newini.write(" coordinates=%s" % "".join(coordinates[:joints]))
    if kinstype:
        newini.write(" kinstype=%s" % kinstype)
    newini.write("\n")
    newini.write(
    ...
    for L in list("XYZABCUVW"):
        if L in L2J:
            ...
            if (
                hs == "-1" or hs == None
            ):  # -1 used to exclude a joint now we use no entry
                sequence = ""
            ...
            for J in L2J[L]:
                ...
                if section:
                    ...
                    print(
                        "File parsing error, found an [AXIS_%i] section, but no content"
                        % J
                    )
                    exit()
    # We no longer need the [AXIS_N] data
    for j in range(0, 8):
        if ("AXIS_%i" % j) in all_sections:
...
if version == "$Revision$" or version < "1.0":
    ...
    subs2.update(
        {
            r"^\s*setp\s+(joint\S+)\s+(axis\S+)\s+(\S+)\s*$": "setp \\1 \\3\nsetp \\2 \\3\n",
            "@ax0@": "axis.x",
            "@ax1@": "axis.y",
            "@ax2@": "axis.z",
            "@ax3@": "axis.a",
