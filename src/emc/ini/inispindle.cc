...
static int loadSpindle(int spindle, EmcIniFile *spindleIniFile)
{
    ...
    if (spindleIniFile->Find(&limit, "MAX_FORWARD_VELOCITY", spindleString) ==
        0) {
        fastest_pos = limit;
        fastest_neg = -1.0 * limit;
    }
    // set min positive speed limit
    if (spindleIniFile->Find(&limit, "MIN_FORWARD_VELOCITY", spindleString) ==
        0) {
        slowest_pos = limit;
        slowest_neg = -1.0 * limit;
    }
    // set min negative speed limit
    if (spindleIniFile->Find(&limit, "MIN_REVERSE_VELOCITY", spindleString) ==
        0) {
        slowest_neg = -1.0 * fabs(limit);
    }
    // set max negative speed limit
    if (spindleIniFile->Find(&limit, "MAX_REVERSE_VELOCITY", spindleString) ==
        0) {
    ...
    // set home velocity
    if (spindleIniFile->Find(&limit, "HOME_SEARCH_VELOCITY", spindleString) ==
        0) {
    ...
    if (0 != emcSpindleSetParams(spindle,
                                 fastest_pos,
                                 slowest_pos,
                                 slowest_neg,
                                 fastest_neg,
