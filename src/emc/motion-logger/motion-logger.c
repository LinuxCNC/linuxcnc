...
static int init_comm_buffers(void)
{
    ...
    shmem_id = rtapi_shmem_new(
        DEFAULT_SHMEM_KEY, mot_comp_id, sizeof(emcmot_struct_t));
    if (shmem_id < 0) {
...
int main(int argc, char *argv[])
{
    ...
    int r;
    if ((r = hal_pin_bit_new("motion-logger.reopen-log",
                             HAL_IO,
                             &motion_logger_data->reopen,
                             mot_comp_id)) < 0) {
        hal_exit(mot_comp_id);
        errno = -r;
    ...
    *motion_logger_data->reopen = 0;
    ...
    while (!quit) {
        ...
        switch (c->command) {
        case EMCMOT_ABORT: log_print("ABORT\n"); break;
        ...
        case EMCMOT_ENABLE_WATCHDOG: log_print("ENABLE_WATCHDOG\n"); break;

        case EMCMOT_DISABLE_WATCHDOG: log_print("DISABLE_WATCHDOG\n"); break;
        ...
        case EMCMOT_PAUSE: log_print("PAUSE\n"); break;

        case EMCMOT_RESUME: log_print("RESUME\n"); break;

        case EMCMOT_STEP: log_print("STEP\n"); break;
        ...
        case EMCMOT_SPINDLE_SCALE: log_print("SPINDLE_SCALE\n"); break;

        case EMCMOT_SS_ENABLE: log_print("SS_ENABLE\n"); break;

        case EMCMOT_FEED_SCALE: log_print("FEED_SCALE\n"); break;

        case EMCMOT_RAPID_SCALE: log_print("RAPID_SCALE\n"); break;

        case EMCMOT_FS_ENABLE: log_print("FS_ENABLE\n"); break;

        case EMCMOT_FH_ENABLE: log_print("FH_ENABLE\n"); break;

        case EMCMOT_AF_ENABLE: log_print("AF_ENABLE\n"); break;

        case EMCMOT_OVERRIDE_LIMITS: log_print("OVERRIDE_LIMITS\n"); break;
        ...
        case EMCMOT_JOG_CONT: log_print("JOG_CONT\n"); break;

        case EMCMOT_JOG_INCR: log_print("JOG_INCR\n"); break;

        case EMCMOT_JOG_ABS: log_print("JOG_ABS\n"); break;

        case EMCMOT_SET_LINE:
            log_print(
                "SET_LINE x=%.6g, y=%.6g, z=%.6g, a=%.6g, b=%.6g, c=%.6g, "
                "u=%.6g, v=%.6g, w=%.6g, id=%d, motion_type=%d, vel=%.6g, "
                "ini_maxvel=%.6g, acc=%.6g, turn=%d\n",
                c->pos.tran.x,
                c->pos.tran.y,
                c->pos.tran.z,
                c->pos.a,
        ...
        case EMCMOT_SET_CIRCLE:
            ...
            log_print(
                "    pos: x=%.6g, y=%.6g, z=%.6g, a=%.6g, b=%.6g, c=%.6g, "
                "u=%.6g, v=%.6g, w=%.6g\n",
                c->pos.tran.x,
                c->pos.tran.y,
                c->pos.tran.z,
            ...
            log_print("    normal: x=%.6g, y=%.6g, z=%.6g\n",
                      ...
                      c->normal.y,
                      c->normal.z);
            log_print("    id=%d, motion_type=%d, vel=%.6g, ini_maxvel=%.6g, "
                      "acc=%.6g, turn=%d\n",
                      c->id,
                      c->motion_type,
                      c->vel,
        ...
        case EMCMOT_SET_TELEOP_VECTOR: log_print("SET_TELEOP_VECTOR\n"); break;

        case EMCMOT_CLEAR_PROBE_FLAGS: log_print("CLEAR_PROBE_FLAGS\n"); break;

        case EMCMOT_PROBE: log_print("PROBE\n"); break;

        case EMCMOT_RIGID_TAP: log_print("RIGID_TAP\n"); break;
        ...
        case EMCMOT_SET_AXIS_JERK_LIMIT:
            log_print(
                "SET_AXIS_JERK_LIMIT axis=%d, jerk=%.6g\n", c->axis, c->jerk);
            break;
        ...
        case EMCMOT_SET_JOINT_VEL_LIMIT:
            log_print(
                "SET_JOINT_VEL_LIMIT joint=%d, vel=%.6g\n", c->joint, c->vel);
            break;

        case EMCMOT_SET_AXIS_ACC_LIMIT:
            log_print(
                "SET_AXIS_ACC_LIMIT axis=%d, acc=%.6g\n", c->axis, c->acc);
            break;

        case EMCMOT_SET_JOINT_ACC_LIMIT:
            log_print(
                "SET_JOINT_ACC_LIMIT joint=%d, acc=%.6g\n", c->joint, c->acc);
            break;

        case EMCMOT_SET_ACC: log_print("SET_ACC acc=%.6g\n", c->acc); break;

        case EMCMOT_SET_JERK: log_print("SET_JERK jerk=%.6g\n", c->jerk); break;
        ...
        case EMCMOT_SET_WORLD_HOME:
            log_print("SET_WORLD_HOME x=%.6g, y=%.6g, z=%.6g, a=%.6g, b=%.6g, "
                      "c=%.6g, u=%.6g, v=%.6g, w=%.6g\n",
                      c->pos.tran.x,
                      c->pos.tran.y,
                      c->pos.tran.z,
        ...
        case EMCMOT_SET_JOINT_HOMING_PARAMS:
            log_print(
                "SET_JOINT_HOMING_PARAMS joint=%d, offset=%.6g home=%.6g, "
                "final_vel=%.6g, search_vel=%.6g, latch_vel=%.6g, "
                "flags=0x%08x, sequence=%d, volatile=%d\n",
                c->joint,
                c->offset,
                c->home,
                c->home_final_vel,
        ...
        case EMCMOT_UPDATE_JOINT_HOMING_PARAMS:
            log_print(
                "UPDATE_JOINT_HOMING_PARAMS joint=%d, offset=%.6g home=%.6g "
                "home_sequence=%d\n",
                c->joint,
                c->offset,
                c->home,
        ...
        case EMCMOT_SET_DEBUG: log_print("SET_DEBUG\n"); break;

        case EMCMOT_SET_DOUT: log_print("SET_DOUT\n"); break;

        case EMCMOT_SET_AOUT: log_print("SET_AOUT\n"); break;
        ...
        case EMCMOT_SPINDLE_INCREASE: log_print("SPINDLE_INCREASE\n"); break;

        case EMCMOT_SPINDLE_DECREASE: log_print("SPINDLE_DECREASE\n"); break;
        ...
        case EMCMOT_SPINDLE_ORIENT: log_print("SPINDLE_ORIENT\n"); break;

        case EMCMOT_SET_JOINT_MOTOR_OFFSET:
            log_print("SET_JOINT_MOTOR_OFFSET\n");
            break;

        case EMCMOT_SET_JOINT_COMP: log_print("SET_JOINT_COMP\n"); break;

        case EMCMOT_SET_OFFSET:
            log_print(
                "SET_OFFSET x=%.6g, y=%.6g, z=%.6g, a=%.6g, b=%.6g, c=%.6g "
                "u=%.6g, v=%.6g, w=%.6g\n",
                c->tool_offset.tran.x,
                c->tool_offset.tran.y,
                c->tool_offset.tran.z,
                c->tool_offset.a,
                c->tool_offset.b,
                c->tool_offset.c,
                c->tool_offset.u,
        ...
        case EMCMOT_SETUP_ARC_BLENDS: log_print("SETUP_ARC_BLENDS\n"); break;
        ...
        default: log_print("ERROR: unknown command %d\n", c->command); break;
        }
