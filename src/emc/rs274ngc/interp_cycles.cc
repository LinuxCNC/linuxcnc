                            ...
                            setup_pointer settings)     //!< pointer to machine settings                   
{
  ...
  case G_82:
    CHK(((settings->motion_mode != G_82) && (block->p_number == -1.0)),
        NCE_DWELL_TIME_P_WORD_MISSING_WITH_G82);
    /* Need to do checks for negative P values for G82, G86, G88, & G89
       as Keith Rumley noted. */
    CHK((block->p_number < 0.0), NCE_NEGATIVE_P_WORD_USED);
    block->p_number =
  ...
  case G_83:
    ...
    /* Moved the check for negative Q values here as a sign may be used with
       user defined M functions.
       Thanks to Billy Singleton for pointing it out.. */
    block->q_number =
