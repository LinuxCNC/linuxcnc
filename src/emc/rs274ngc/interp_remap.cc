...
bool Interp::is_m_code_remappable(int m_code)
{
    ...
    return ((m_code > 199 && m_code < 1000) ||
            (m_code > 0 && m_code < 100 && ems[m_code] == -1) || m_code == 0 ||
            m_code == 1 || m_code == 6 || m_code == 7 || m_code == 8 ||
            m_code == 9 || m_code == 60 || m_code == 61 || m_code == 62 ||
            m_code == 63 || m_code == 64 || m_code == 65 || m_code == 66 ||
            m_code == 67 || m_code == 68);
}
...
bool Interp::is_user_defined_m_code(block_pointer block,
                                  ...
                                  int number)
{
    ...
    case 'M': remap = settings->m_remapped[number]; break;
    case 'G': remap = settings->g_remapped[number]; break;
    default:
        key[0] = letter;
        key[1] = '\0';
    ...
    if ((_setup.debugmask & EMC_DEBUG_REMAP) && (_setup.loggingLevel > 2)) {
                           ...
                           char *posarglist)
{
    ...
    logNP(
        "add_parameters code=%s argspec=%s call_level=%d r=%s o=%s pydict=%d\n",
        code,
        argspec.data(),
        settings->call_level,
        required.c_str(),
    ...
    auto STORE{[&](const char *name, double value) -> void {
        if (pydict) {
            try {
    ...
    // special cases:
    // N...add line number
    if (required.find_first_of('n') != std::string::npos ||
        required.find_first_of('N') != std::string::npos) {
...
int Interp::parse_remap(const char *inistring, int lineno)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strncasecmp(kw, "argspec", kwlen)) {
            size_t pos = strspn(
                arg, "ABCDEFGHIJKLMNPQRSTUVWXYZabcdefghijklmnpqrstuvwxyz>^@");
            if (pos != strlen(arg)) {
        ...
        if (!strncasecmp(kw, "prolog", kwlen)) {
            if (PYUSABLE) {
                ...
                Error("Python plugin required for prolog=, but not available: "
                      "%d:REMAP = %s",
                      lineno,
                      inistring);
                errored = true;
        ...
        if (!strncasecmp(kw, "epilog", kwlen)) {
            if (PYUSABLE) {
                ...
                Error("Python plugin required for epilog=, but not available: "
                      "%d:REMAP = %s",
                      lineno,
                      inistring);
                errored = true;
        ...
        if (!strncasecmp(kw, "ngc", kwlen)) {
            if (r.remap_py) {
                Error("can\'t remap to an ngc file and a Python function: -  "
                      "%d:REMAP = %s",
                      lineno,
                      inistring);
                errored = true;
        ...
        if (!strncasecmp(kw, "python", kwlen)) {
            ...
            if (!PYUSABLE) {
                Error(
                    "iNTERP_REMAP: Python plugin required for python=, but not "
                    "available:\nREMAP INI line:%d = %s\n",
                    lineno,
                    inistring);
                errored = true;
    ...
    if ((r.remap_ngc == NULL) && (r.remap_py == NULL)) {
        Error("code '%s' - no remap function given, use either "
              "'python=<function>' or 'ngc=<basename>' : %d:REMAP = %s",
              code,
              lineno,
              inistring);
        goto fail;
...
#define CHECK(bad, fmt, ...)                                                   \
    ...
    case 'f':
        CHECK((strlen(code) > 1),
              "%d: %c remap - only single letter code allowed",
              lineno,
              *code);
        CHECK((r.modal_group != -1),
              "%d: %c remap - modal group setting ignored - fixed sequencing",
              lineno,
              *code);
    ...
    case 'm':
        if (sscanf(code + 1, "%d", &mcode) != 1) {
            Error("parsing M-code: expecting integer like 'M420', got '%s' : "
                  "%d:REMAP = %s",
                  code,
                  lineno,
                  inistring);
        ...
        if (r.modal_group == -1) {
            Error(
                "warning: code '%s' : no modalgroup=<int> given, using default "
                "group %d : %d:REMAP = %s",
                code,
                MCODE_DEFAULT_MODAL_GROUP,
                lineno,
                inistring);
        ...
        if (!M_MODE_OK(r.modal_group)) {
            Error(
                "error: code '%s' : invalid modalgroup=<int> given (currently "
                "valid: 4..10) : %d:REMAP = %s",
                code,
                lineno,
                inistring);
            goto fail;
    ...
    case 'g':
        ...
        if (!G_MODE_OK(r.modal_group)) {
            Error(
                "error: code '%s' : %s modalgroup=<int> given  : %d:REMAP = %s",
                argv[0],
    ...
    default:
        ...
        if ((r.prolog_func || r.remap_py || r.epilog_func) && (!PYUSABLE)) {
            fprintf(stderr,
                    "fatal: REMAP requires the Python plugin, which did not "
                    "initialize\n");
            break;
        }
        Log("REMAP BUG=%s %d:REMAP = %s", code, lineno, inistring);
    }
    return INTERP_OK;
