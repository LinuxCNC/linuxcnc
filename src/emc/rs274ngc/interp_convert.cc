...
#include "units.h"
#define TOOL_INSIDE_ARC(side, turn)                                            \
    (((side) == CUTTER_COMP::LEFT && (turn) > 0) ||                            \
     ((side) == CUTTER_COMP::RIGHT && (turn) < 0))
#define DEBUG_EMC
                             ...
                             double *z)
{
    ...
    case CANON_PLANE::XZ:
        ...
        *z = settings->current_y;
        break;
    default: ERS("BUG: Invalid plane in comp_get_current");
    }
    return INTERP_OK;
}
                             ...
                             double z)
{
    ...
    case CANON_PLANE::XZ:
        ...
        break;
    default: ERS("BUG: Invalid plane in comp_set_current");
    }
    return INTERP_OK;
}
                                ...
                                double *z)
{
    ...
    case CANON_PLANE::XZ:
        ...
        *z = settings->program_y;
        break;
    default: ERS("BUG: Invalid plane in comp_get_programmed");
    }
    return INTERP_OK;
}
                                ...
                                double z)
{
    ...
    case CANON_PLANE::XZ:
        ...
        break;
    default: ERS("BUG: Invalid plane in comp_set_programmed");
    }
    return INTERP_OK;
}
...
*/
static unsigned int Q_G6_option =
    0; // (NICU, NICL, NICC see publication from Lo Valvo and Drago)
...
int Interp::convert_nurbs(int mode, block_pointer block, setup_pointer settings)
{
    ...
    if (mode == G_5_2) {
        ...
        if (settings->plane == CANON_PLANE::XY) {
            CHKS((((block->x_flag) && !(block->y_flag)) ||
                 ...
                 (_("You must specify both X and Y coordinates for Control "
                    "Points")));
            CHKS((!(block->x_flag) && !(block->y_flag) &&
                  (block->p_number > 0) && (!nurbs_g5_control_points.empty())),
                 (_("Can specify P without X and Y only for the first control "
                    "point")));
        }
        if (settings->plane == CANON_PLANE::YZ) {
            CHKS((((block->y_flag) && !(block->z_flag)) ||
                  (!(block->y_flag) && (block->z_flag))),
                 (_("You must specify both Y and Z coordinates for Control "
                    "Points")));
            CHKS((!(block->y_flag) && !(block->z_flag) &&
                  (block->p_number > 0) && (!nurbs_g5_control_points.empty())),
                 (_("Can specify P without Y and Z only for the first control "
                    "point")));
        }
        if (settings->plane == CANON_PLANE::XZ) {
            CHKS((((block->x_flag) && !(block->z_flag)) ||
                  (!(block->x_flag) && (block->z_flag))),
                 (_("You must specify both X and Z coordinates for Control "
                    "Points")));
            CHKS((!(block->x_flag) && !(block->z_flag) &&
                  (block->p_number > 0) && (!nurbs_g5_control_points.empty())),
                 (_("Can specify P without X and Z only for the first control "
                    "point")));
        }
...
*/
        ...
        CHKS((nurbs_g5_control_points.size() < nurbs_order),
             _("You must specify a number of control points at least equal to "
               "the order L = %d"),
             nurbs_order);

        if (settings->plane == CANON_PLANE::XY) {
            settings->current_x =
                nurbs_g5_control_points[nurbs_g5_control_points.size() - 1]
                    .NURBS_X;
            settings->current_y =
                nurbs_g5_control_points[nurbs_g5_control_points.size() - 1]
                    .NURBS_Y;
...
*/
        NURBS_G5_FEED(block->line_number,
    ...
    if (mode == G_6_2) {
        ...
        CHKS(
            ((!block->p_flag) && (nurbs_g6_control_points.empty())),
        ...
        CHKS(((block->q_number <= 0) && (nurbs_g6_control_points.empty())),
             (_("Program the nurbs Q value (1,2,3) in the first block of nurbs "
                "instruction")));
        int q;
        if (block->q_flag) {
        ...
        CHKS(((block->r_number <= 0) && (!nurbs_g6_control_points.empty())),
             (_("Must specify positive nurbs weight R for every Control "
                "Point")));
        ...
        if (ff_G6_line_counter == 0) {
            Q_G6_option =
                block
                    ->q_number; // Q_G6_option=1...3 (NICU, NICL, NICC see publication from Lo Valvo and Drago)
        }
        ...
        if (nurbs_g6_control_points.size() > nurbs_order) {
            if (nurbs_g6_control_points[ff_G6_line_counter - 1].NURBS_K ==
                nurbs_g6_control_points[ff_G6_line_counter - nurbs_order + 1 -
                                        1]
                    .NURBS_K) {
                CHKS((nurbs_g6_control_points.size() < nurbs_order),
                     _("You must specify a number of control points at least "
                       "equal to the order P = %d"),
                     nurbs_order);
                settings->current_x =
                    nurbs_g6_control_points[nurbs_g6_control_points.size() - 1]
                        .NURBS_X;
                settings->current_y =
                    nurbs_g6_control_points[nurbs_g6_control_points.size() - 1]
                        .NURBS_Y;
...
*/
    ...
    //
    if (mode ==
        G_6_3) // jjf this is missing from Lo Valvo code! not used there and here!
    {
...
{ //!< pointer to machine settings
    ...
    CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
         _("Cannot convert spline with cutter radius compensation")); // XXX
    ...
    if ((mode == G_5_1) || (mode == G_6_1)) {
        ...
        NURBS_G5_FEED(
            block->line_number, nurbs_g5_control_points, 3, settings->plane);
        nurbs_g5_control_points.clear();
...
*/
        NURBS_G5_FEED(
            block->line_number, nurbs_g5_control_points, 4, settings->plane);
        nurbs_g5_control_points.clear();
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    CHKS((settings->arc_not_allowed),
         (_("The move just after exiting cutter compensation mode must be "
            "straight, not an arc")));
    ...
    CHKS(((!block->r_flag) && (!ijk_flag)),
         NCE_R_I_J_K_WORDS_ALL_MISSING_FOR_ARC);
    CHKS(((block->r_flag) && (ijk_flag)), NCE_MIXED_RADIUS_IJK_FORMAT_FOR_ARC);
    if (settings->feed_mode == FEED_MODE::UNITS_PER_MINUTE) {
        CHKS((settings->feed_rate == 0.0),
             NCE_CANNOT_MAKE_ARC_WITH_ZERO_FEED_RATE);
    ...
    if (ijk_flag) {
        ...
        if (settings->plane == CANON_PLANE::XY) {
            CHKS(((!block->x_flag) && (!block->y_flag) &&
                  (!block->radius_flag) && (!block->theta_flag)),
                 NCE_X_AND_Y_WORDS_MISSING_FOR_ARC_IN_XY_PLANE);
        } else if (settings->plane == CANON_PLANE::YZ) {
            CHKS(((!block->y_flag) && (!block->z_flag)),
                 NCE_Y_AND_Z_WORDS_MISSING_FOR_ARC_IN_YZ_PLANE);
    ...
    double offset2)
{
    ...
    double spiral_abs_tolerance =
        (settings->length_units == CANON_UNITS_INCHES)
            ? settings->center_arc_radius_tolerance_inch
            : settings->center_arc_radius_tolerance_mm;
    ...
    if (block->r_flag) {
        CHP(arc_data_r(move,
                       ...
                       *current2,
                       ...
                       end2,
                       block->r_number,
                       block->p_flag ? round_to_int(block->p_number) : 1,
                       &center1,
                       &center2,
                       &turn,
                       radius_tolerance));
        ...
        CHP(arc_data_ijk(
            ...
            *current2,
            ...
            &turn,
            radius_tolerance,
            spiral_abs_tolerance,
            SPIRAL_RELATIVE_TOLERANCE));
    }
    inverse_time_rate_arc(*current1,
                          *current2,
                          *current3,
                          center1,
                          center2,
...
int Interp::convert_arc_comp1(
    ...
    double u_end,
    double v_end,
    double w_end) //!< uvw at end of arc
{
    ...
    CHKS((hypot((end_x - cx), (end_y - cy)) <= tool_radius),
         _("Radius of cutter compensation entry arc is not greater than the "
           "tool radius"));

    if (block->r_flag) {
        CHP(arc_data_comp_r(move,
                            plane,
                            side,
                            tool_radius,
                            cx,
    ...
    if (settings->cutter_comp_orientation != 0 &&
        ...
        enqueue_STRAIGHT_FEED(settings,
                              block->line_number,
                              0,
                              0,
                              0,
    ...
    double w) //!< uvw at end of arc
{
    ...
    beta = (beta > (1.5 * M_PIl)) ? (beta - (2 * M_PIl))
           : (beta < -M_PI_2l)    ? (beta + (2 * M_PIl))
                                  : beta;

    if (((side == CUTTER_COMP::LEFT) && (move == G_3)) ||
    ...
    if (beta < -small || beta > M_PIl + small ||
        ...
        if (qc().front().type != QARC_FEED) {
            ...
            if TOOL_INSIDE_ARC (side, turn) {
                ...
                CHKS((l > 1.0 || l < -1.0),
                     _("Arc move in concave corner cannot be reached by the "
                       "tool without gouging"));
                if (turn > 0) {
            ...
            CHKS(
                (oldrad == 0 || arc_cc == 0),
                _("Arc to arc motion is invalid because the arcs have the same "
                  "center"));
            a = (SQ(oldrad) + SQ(arc_cc) - SQ(newrad)) / (2 * oldrad * arc_cc);

            CHKS((a > 1.0 || a < -1.0),
                 (_("Arc to arc motion makes a corner the compensated tool "
                    "can't fit in without gouging")));
            pullback = acos(a);
...
int Interp::convert_axis_offsets(
...
int Interp::convert_comment(char *comment,
                            bool enqueue) //!< string with comment
{
    enum {
        LC_SIZE = 256,
        EX_SIZE = 2 * LC_SIZE
    ...
    if (startswith(lc, MSG_STR)) {
        ...
        convert_param_comment(
            comment + start + strlen(DEBUG_STR), expanded, EX_SIZE);
        ...
        if (fd) {
            convert_param_comment(
                comment + start + strlen(PRINT_STR), expanded, EX_SIZE);
            fprintf(fd, "%s\n", expanded);
            fflush(fd);
        }
        return INTERP_OK;
        ...
        convert_param_comment(
            comment + start + strlen(LOG_STR), expanded, EX_SIZE);
        LOG(expanded);
        return INTERP_OK;
        ...
        convert_param_comment(
            comment + start + strlen(ABORT_STR), expanded, EX_SIZE);
...
int Interp::convert_coordinate_system(
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
         (_("Cannot change coordinate systems with cutter radius "
            "compensation on")));
    ...
    find_current_in_system(settings,
                           origin,
                           &settings->current_x,
                           &settings->current_y,
                           &settings->current_z,
...
int Interp::convert_cutter_compensation(
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (g_code == G_40) {
        ...
        CHP(convert_cutter_compensation_on(
            CUTTER_COMP::RIGHT, block, settings));
...
int Interp::convert_cutter_compensation_off(
    setup_pointer settings) //!< pointer to machine settings
{
#ifdef DEBUG_EMC
...
int Interp::convert_cutter_compensation_on(
    CUTTER_COMP side,       //!< side of path cutter is on (LEFT or RIGHT)
    block_pointer block,    //!< pointer to a block of RS274 instructions
    setup_pointer settings) //!< pointer to machine settings
{
    double radius;
    ...
    CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
         NCE_CANNOT_TURN_CUTTER_RADIUS_COMP_ON_WHEN_ON);
    if (block->g_modes[GM_CUTTER_COMP] == G_41_1 ||
        block->g_modes[GM_CUTTER_COMP] == G_42_1) {
        CHKS((!block->d_flag),
             _("G%d.1 with no D word"),
             block->g_modes[GM_CUTTER_COMP] / 10);
        radius = block->d_number_float / 2;
        if (block->l_number != -1) {
            CHKS((settings->plane != CANON_PLANE::XZ),
        ...
        CHKS((settings->plane != CANON_PLANE::XZ && orientation != 0 &&
              orientation != 9),
             _("G%d with lathe tool, but plane is not G18"),
             block->g_modes[GM_CUTTER_COMP] / 10);
    }
    if (radius <
        0.0) { /* switch side & make radius positive if radius negative */
        radius = -radius;
...
int Interp::convert_distance_mode(
...
int Interp::convert_ijk_distance_mode(
    int g_code, //!< g_code being executed (must be G_90_1 or G_91_1)
    setup_pointer settings) //!< pointer to machine settings
{
    if (g_code == G_90_1) {
        if (settings->ijk_distance_mode != DISTANCE_MODE::ABSOLUTE) {
...
int Interp::convert_lathe_diameter_mode(
    int g_code,          //!< g_code being executed (must be G_90_1 or G_91_1)
...
int Interp::convert_feed_mode(
    int g_code, //!< g_code being executed (must be G_93, G_94 or G_95)
    setup_pointer settings) //!< pointer to machine settings
{
    if (g_code == G_93) {
#ifdef DEBUG_EMC
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if ((block->g_modes[GM_LENGTH_UNITS] != -1) && ONCE(STEP_LENGTH_UNITS)) {
        status =
            convert_length_units(block->g_modes[GM_LENGTH_UNITS], settings);
        CHP(status);
    }
    if ((block->g_modes[GM_LATHE_DIAMETER_MODE] != -1) &&
        ONCE(STEP_LATHE_DIAMETER_MODE)) {
        status = convert_lathe_diameter_mode(
            block->g_modes[GM_LATHE_DIAMETER_MODE], block, settings);
        CHP(status);
    }
    if ((block->g_modes[GM_CUTTER_COMP] != -1) && ONCE(STEP_CUTTER_COMP)) {
        status = convert_cutter_compensation(
            block->g_modes[GM_CUTTER_COMP], block, settings);
        CHP(status);
    }
    if ((block->g_modes[GM_TOOL_LENGTH_OFFSET] != -1) &&
        ONCE(STEP_TOOL_LENGTH_OFFSET)) {
        status = convert_tool_length_offset(
            block->g_modes[GM_TOOL_LENGTH_OFFSET], block, settings);
        CHP(status);
    }
    if ((block->g_modes[GM_COORD_SYSTEM] != -1) && ONCE(STEP_COORD_SYSTEM)) {
        status = convert_coordinate_system(block->g_modes[GM_COORD_SYSTEM],
                                           settings);
        CHP(status);
    }
    ...
    if ((block->g_modes[GM_DISTANCE_MODE] != -1) && ONCE(STEP_DISTANCE_MODE)) {
        status =
            convert_distance_mode(block->g_modes[GM_DISTANCE_MODE], settings);
        CHP(status);
    }
    if ((block->g_modes[GM_IJK_DISTANCE_MODE] != -1) &&
        ONCE(STEP_IJK_DISTANCE_MODE)) {
        status = convert_ijk_distance_mode(block->g_modes[GM_IJK_DISTANCE_MODE],
                                           settings);
        CHP(status);
    }
    if ((block->g_modes[GM_RETRACT_MODE] != -1) && ONCE(STEP_RETRACT_MODE)) {
        status =
            convert_retract_mode(block->g_modes[GM_RETRACT_MODE], settings);
        CHP(status);
    }
    ...
    if ((block->g_modes[GM_G92_IS_APPLIED] != -1) &&
        ONCE(STEP_G92_IS_APPLIED)) {
        status = convert_g92_is_applied(
            block->g_modes[GM_G92_IS_APPLIED], block, settings);
    }
...
int Interp::convert_savehome(int code, block_pointer /*block*/, setup_pointer s)
{
    ...
    double z = PROGRAM_TO_USER_LEN(s->current_z + s->tool_offset.tran.z +
                                   s->origin_offset_z + s->axis_offset_z);
    double a = PROGRAM_TO_USER_ANG(s->AA_current + s->tool_offset.a +
                                   s->AA_origin_offset + s->AA_axis_offset);
    double b = PROGRAM_TO_USER_ANG(s->BB_current + s->tool_offset.b +
                                   s->BB_origin_offset + s->BB_axis_offset);
    double c = PROGRAM_TO_USER_ANG(s->CC_current + s->tool_offset.c +
                                   s->CC_origin_offset + s->CC_axis_offset);
    double u = PROGRAM_TO_USER_LEN(s->u_current + s->tool_offset.u +
                                   s->u_origin_offset + s->u_axis_offset);
    double v = PROGRAM_TO_USER_LEN(s->v_current + s->tool_offset.v +
                                   s->v_origin_offset + s->v_axis_offset);
    double w = PROGRAM_TO_USER_LEN(s->w_current + s->tool_offset.w +
                                   s->w_origin_offset + s->w_axis_offset);
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (AA_end != settings->AA_current && (-1 != settings->a_indexer_jnum))
        issue_straight_index(
            3, settings->a_indexer_jnum, AA_end, block->line_number, settings);
    if (BB_end != settings->BB_current && (-1 != settings->b_indexer_jnum))
        issue_straight_index(
            4, settings->b_indexer_jnum, BB_end, block->line_number, settings);
    if (CC_end != settings->CC_current && (-1 != settings->c_indexer_jnum))
        issue_straight_index(
            5, settings->c_indexer_jnum, CC_end, block->line_number, settings);
    ...
    if (!block->x_flag && !block->y_flag && !block->z_flag && !block->a_flag &&
        !block->b_flag && !block->c_flag && !block->u_flag && !block->v_flag &&
        !block->w_flag) {
        end_x = end_x_home;
        end_y = end_y_home;
        end_z = end_z_home;
...
int Interp::convert_length_units(
 ...
 * their state.
 */
int Interp::gen_settings(int *int_current,
                         int *int_saved, // G-codes
                         double * /*float_current*/,
                         double *float_saved, // S, F, other
                         std::string &cmd)    // command buffer
{
        ...
        if (val != int_current[i]) {
            ...
            case 16: // - whether g92 is applied
                ...
                if (val != -1) { // FIXME not sure if this is correct!
                    ...
                    if (val % 10) {
                        snprintf(
                            buf, sizeof(buf), " G%d.%d", val / 10, val % 10);
                    } else {
    ...
    if ((int_saved[11] == G_64) && g64_changed) {
        ...
        else if (float_saved[GM_FIELD_FLOAT_NAIVE_CAM_TOLERANCE] < 0)
            // Only P arg
            snprintf(buf,
                     sizeof(buf),
                     " G64 P%f",
                            ...
                            std::string &cmd)
{
    ...
    if (current_g[5] != saved_g[5]) {
        ...
        CHKS(execute(buf) != INTERP_OK,
             _("gen_restore G20/G21 failed: '%s'"),
             cmd.c_str());
    }

    if ((res = gen_settings(
             current_g, saved_g, current_settings, saved_settings, cmd))) {
        logStateTags("gen_restore_cmd():  error restoring settings (%d)", res);
        return INTERP_ERROR;
    }
...
int Interp::save_settings(setup_pointer settings)
{
    ...
    active_g_codes(
        (int *)settings->sub_context[settings->call_level].saved_g_codes);
    active_m_codes(
        (int *)settings->sub_context[settings->call_level].saved_m_codes);
    active_settings(
        (double *)settings->sub_context[settings->call_level].saved_settings);
    ...
    int from_level) //!< call level of context to restore from
{
    ...
    if (settings->active_g_codes[5] !=
        settings->sub_context[from_level].saved_g_codes[5]) {
        char buf[LINELEN];
        snprintf(buf,
                 sizeof(buf),
                 "G%d",
                 settings->sub_context[from_level].saved_g_codes[5] / 10);
        CHKS(execute(buf) != INTERP_OK,
             _("M7x: restore_settings G20/G21 failed: '%s'"),
             cmd.c_str());
    }
    gen_settings((int *)settings->active_g_codes,
                 ...
                 cmd);
    gen_m_codes((int *)settings->active_m_codes,
    ...
    if (!cmd.empty()) {
        ...
        while (s != NULL) {
            ...
            if (status != INTERP_OK) {
                ...
                CHKS(status,
                     _("M7x: restore_settings failed executing: '%s': %s"),
                     s,
                     currentError);
            }
            s = strtok_r(NULL, "\n", &stateptr);
    ...
    if (!cmd.empty()) {
        ...
        while (s != NULL) {
            ...
            if (status != INTERP_OK) {
                ...
                CHKS(status,
                     _("Failed to restore interp state on abort "
                       "'%s': %s"),
                     s,
                     currentError);
            }
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (is_user_defined_m_code(block, settings, 5) &&
        STEP_REMAPPED_IN_BLOCK(block, STEP_M_5) && ONCE_M(5)) {
        return convert_remapped_code(
            block, settings, STEP_M_5, 'm', block->m_modes[5]);
    } else if ((block->m_modes[5] == 62) && ONCE_M(5)) {
        CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
             (_("Cannot set motion output with cutter radius compensation "
                "on"))); // XXX
        ...
        CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
             (_("Cannot set motion digital output with cutter radius "
                "compensation on"))); // XXX
        ...
        CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
             (_("Cannot set auxiliary digital output with cutter radius "
                "compensation on"))); // XXX
        ...
        CHKS(((block->q_number <= 0) && (block->l_flag) &&
        ...
        CHKS(((block->e_flag) && (block->l_flag) &&
              (round_to_int(block->l_number) != 0)),
        ...
        if (block->p_flag) { // got a digital input
            if (round_to_int(block->p_number) <
                0) // safety check for negative words
            ...
            CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
                 (_("Cannot wait for digital input with cutter radius "
                    "compensation on")));

            int ret = WAIT(
                round_to_int(block->p_number), DIGITAL_INPUT, type, timeout);
            ...
            CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
                 (_("Cannot wait for analog input with cutter radius "
                    "compensation on")));

            int ret = WAIT(round_to_int(block->e_number),
                           ANALOG_INPUT,
                           0,
                           0); //WAIT returns 0 on success, -1 for out of bounds
        ...
        //Q-word = analog value
        CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
             (_("Cannot set motion analog output with cutter radius "
                "compensation on"))); // XXX
        CHKS((!block->e_flag) || (round_to_int(block->e_number) < 0),
        ...
        //Q-word = analog value
        CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
             (_("Cannot set auxiliary analog output with cutter radius "
                "compensation on"))); // XXX
        CHKS((!block->e_flag) || (round_to_int(block->e_number) < 0),
    ...
    if ((block->m_modes[6] != -1) && ONCE_M(6)) {
        ...
        case 6:
            if (is_user_defined_m_code(block, settings, 6) &&
                remapped_in_block) {
                return convert_remapped_code(
                    block, settings, STEP_M_6, 'm', block->m_modes[6]);
            } else {
        ...
        case 61:
            if (is_user_defined_m_code(block, settings, 6) &&
                ...
                CHKS((toolno < 0),
                     (_("Need non-negative Q-word to specify tool number with "
                        "M61")));
    ...
    if (is_user_defined_m_code(block, settings, 7) && ONCE_M(7)) {
        return convert_remapped_code(
            block, settings, STEP_M_7, 'm', block->m_modes[7]);
    } else if ((block->m_modes[7] == 3) && ONCE_M(7)) {
        if (block->dollar_flag) {
            CHKS((block->dollar_number >= settings->num_spindles ||
                  block->dollar_number < -1),
                 (_("Spindle ($) number out of range in M3 "
                    "Command\nnum_spindles =%i. $=%d\n")),
                 settings->num_spindles,
                 (int)block->dollar_number);
            if (block->dollar_number == -1) { // all spindles
                ...
                settings->spindle_turning[(int)block->dollar_number] =
                    CANON_CLOCKWISE;
            }
        ...
        if (block->dollar_flag) {
            CHKS((block->dollar_number >= settings->num_spindles ||
                  block->dollar_number < -1),
                 (_("Spindle ($) number out of range in M4 "
                    "Command\nnum_spindles =%i. $=%d\n")),
                 settings->num_spindles,
                 (int)block->dollar_number);
            if (block->dollar_number == -1) { // all spindles
        ...
        if (block->dollar_flag) {
            CHKS((block->dollar_number >= settings->num_spindles ||
                  block->dollar_number < -1),
                 (_("Spindle ($) number out of range in M5 "
                    "Command\nnum_spindles =%i. $=%d\n")),
                 settings->num_spindles,
                 (int)block->dollar_number);
            if (block->dollar_number == -1) { // all spindles
        ...
        if (block->r_flag || block->p_flag)
            enqueue_ORIENT_SPINDLE(
                block->dollar_flag ? block->dollar_number : 0,
                block->r_flag ? (block->r_number + settings->orient_offset)
                              : settings->orient_offset,
                block->p_flag ? block->p_number : 0);
        if (block->q_flag) {
            CHKS((block->q_number <= 0.0),
                 (_("Q word with M19 requires a value > 0")));
            enqueue_WAIT_ORIENT_SPINDLE_COMPLETE(
                block->dollar_flag ? block->dollar_number : 0, block->q_number);
        }
        ...
        if (block->m_modes[7] == 73) {
            if (_setup.call_level == 0) {
                MSG("Warning - M73 at top level: nothing to return to; storing "
                    "context anyway\n");
            } else {
                _setup.sub_context[_setup.call_level].context_status |=
                    CONTEXT_RESTORE_ON_RETURN;
            }
        }
        ...
        CHKS((!(_setup.sub_context[_setup.call_level].context_status &
                CONTEXT_VALID)),
             (_("Cannot restore context from invalid stack frame - missing "
                "M70/M73?")));
    ...
    if (is_user_defined_m_code(block, settings, 8) &&
        ...
        return convert_remapped_code(
            block, settings, STEP_M_8, 'm', block->m_modes[8]);
...
*/
    if (is_user_defined_m_code(block, settings, 9) && ONCE_M(9)) {
        return convert_remapped_code(
            block, settings, STEP_M_9, 'm', block->m_modes[9]);
    } else if ((block->m_modes[9] == 48) && ONCE_M(9)) {
        CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
             (_("Cannot enable overrides with cutter radius compensation "
                "on"))); // XXX
        ...
        CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
             (_("Cannot disable overrides with cutter radius compensation "
                "on"))); // XXX
    ...
    if ((block->m_modes[9] == 50) && ONCE_M(9)) {
        if (block->p_number != 0) {
            ...
            CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
                 (_("Cannot disable overrides with cutter radius "
                    "compensation on"))); // XXX
            DISABLE_FEED_OVERRIDE();
            settings->feed_override = false;
        }
    ...
    if (is_user_defined_m_code(block, settings, 10) && ONCE_M(10)) {
        return convert_remapped_code(
            block, settings, STEP_M_10, 'm', block->m_modes[10]);
...
int Interp::convert_g92_is_applied(int code,
                                   block_pointer block,
                                   setup_pointer settings)
{
    CHP(convert_axis_offsets(code, block, settings));
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    CHKS((ai && (xyzuvw_flag || block->b_flag || block->c_flag)),
         (_("Indexing axis %c can only be moved alone")),
         'A');
    ...
    } else if (motion == G_38_2 || motion == G_38_3 || motion == G_38_4 ||
               motion == G_38_5) {
    ...
    } else if ((motion == G_5) || (motion == G_5_1) || (motion == G_6) ||
               (motion == G_6_1)) { // jjf
        write_canon_state_tag(block, settings);
    ...
    } else if (motion == G_70 || motion == G_71 || motion == G_71_1 ||
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    CHKS(((!(probe_type & 1)) && settings->current_x == end_x &&
          ...
          settings->AA_current == AA_end && settings->BB_current == BB_end &&
          settings->CC_current == CC_end && settings->u_current == u_end &&
          settings->v_current == v_end && settings->w_current == w_end),
         NCE_START_POINT_TOO_CLOSE_TO_PROBE_POINT);

    TURN_PROBE_ON();
    STRAIGHT_PROBE(block->line_number,
                   end_x,
                   end_y,
                   end_z,
                   AA_end,
...
int Interp::convert_retract_mode(
...
int Interp::convert_setup_tool(block_pointer block, setup_pointer settings)
{
    ...
    CHKS(!(block->x_flag || block->y_flag || block->z_flag || block->a_flag ||
           block->b_flag || block->c_flag || block->u_flag || block->v_flag ||
           block->w_flag || block->r_flag || block->q_flag || block->i_flag ||
           block->j_flag),
         _("G10 L1 without offsets has no effect"));

    if (direct) {
        if (block->x_flag)
            settings->tool_table[idx].offset.tran.x =
                PROGRAM_TO_USER_LEN(block->x_number);
        if (block->y_flag)
            settings->tool_table[idx].offset.tran.y =
                PROGRAM_TO_USER_LEN(block->y_number);
        if (block->z_flag)
            settings->tool_table[idx].offset.tran.z =
                PROGRAM_TO_USER_LEN(block->z_number);
        if (block->a_flag)
            settings->tool_table[idx].offset.a =
                PROGRAM_TO_USER_ANG(block->a_number);
        if (block->b_flag)
            settings->tool_table[idx].offset.b =
                PROGRAM_TO_USER_ANG(block->b_number);
        if (block->c_flag)
            settings->tool_table[idx].offset.c =
                PROGRAM_TO_USER_ANG(block->c_number);
        if (block->u_flag)
            settings->tool_table[idx].offset.u =
                PROGRAM_TO_USER_LEN(block->u_number);
        if (block->v_flag)
            settings->tool_table[idx].offset.v =
                PROGRAM_TO_USER_LEN(block->v_number);
        if (block->w_flag)
            settings->tool_table[idx].offset.w =
                PROGRAM_TO_USER_LEN(block->w_number);
    } else {
        ...
        int destination_system =
            to_fixture
                ? 9
                : settings
                      ->origin_index; // maybe 9 (g59.3) should be user configurable?

        find_current_in_system_without_tlo(settings,
                                           destination_system,
                                           &tx,
                                           &ty,
                                           &tz,
        ...
        if (block->x_flag && block->y_flag) {
            ...
            ty -= block->y_number;
            rotate(
                &tx, &ty, settings->parameters[5210 + destination_system * 20]);
            ...
            rotate(&ox,
                   &oy,
                   -settings->parameters[5210 + destination_system * 20]);
            ox = 0;
            rotate(
                &ox, &oy, settings->parameters[5210 + destination_system * 20]);
            ...
            settings->tool_table[idx].offset.tran.x =
                PROGRAM_TO_USER_LEN(tx + ox);
            settings->tool_table[idx].offset.tran.y =
                PROGRAM_TO_USER_LEN(ty + oy);
        ...
        if (block->z_flag)
            settings->tool_table[idx].offset.tran.z =
                PROGRAM_TO_USER_LEN(tz - block->z_number);
        if (block->a_flag)
            settings->tool_table[idx].offset.a =
                PROGRAM_TO_USER_ANG(ta - block->a_number);
        if (block->b_flag)
            settings->tool_table[idx].offset.b =
                PROGRAM_TO_USER_ANG(tb - block->b_number);
        if (block->c_flag)
            settings->tool_table[idx].offset.c =
                PROGRAM_TO_USER_ANG(tc - block->c_number);
        if (block->u_flag)
            settings->tool_table[idx].offset.u =
                PROGRAM_TO_USER_LEN(tu - block->u_number);
        if (block->v_flag)
            settings->tool_table[idx].offset.v =
                PROGRAM_TO_USER_LEN(tv - block->v_number);
        if (block->w_flag)
            settings->tool_table[idx].offset.w =
                PROGRAM_TO_USER_LEN(tw - block->w_number);
    }
    ...
    if (block->q_number != -1.0) {
        CHKS((!is_near_int(&q, block->q_number)),
             _("Q number in G10 is not an integer"));
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    CHKS((block->i_flag || block->j_flag),
         _("I J words not allowed with G10 L2"));
    ...
    CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF &&
          p_int == settings->origin_index),
         (_("Cannot change the active coordinate system with cutter radius "
            "compensation on")));

    find_current_in_system(
        settings, p_int, &cx, &cy, &cz, &ca, &cb, &cc, &cu, &cv, &cw);
    ...
    if (p_int == settings->origin_index) { /* system is currently used */
        ...
        rotate(
            &settings->current_x, &settings->current_y, settings->rotation_xy);
...
int Interp::convert_set_plane(
    int g_code,             //!< must be G_17, G_18, or G_19
    setup_pointer settings) //!< pointer to machine settings
{
    CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF && g_code == G_17 &&
          settings->plane != CANON_PLANE::XY),
         NCE_CANNOT_CHANGE_PLANES_WITH_CUTTER_RADIUS_COMP_ON);
    CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF && g_code == G_18 &&
          settings->plane != CANON_PLANE::XZ),
         NCE_CANNOT_CHANGE_PLANES_WITH_CUTTER_RADIUS_COMP_ON);
...
int Interp::convert_speed(
    int spindle,            //The spindle ($) or -1 if none
    block_pointer block,    //!< pointer to a block of RS274 instructions
    setup_pointer settings) //!< pointer to machine settings
{
...
int Interp::convert_spindle_mode(int dollar_number,
                                 block_pointer block,
                                 setup_pointer settings)
{
    for (int s = 0; s < settings->num_spindles; s++) {
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (block->m_modes[4] == 0) {
        ...
        return INTERP_EXECUTE_FINISH; // tell task to issue any queued commands
               ...
               (block->m_modes[4] == 99 &&
                !_setup.loop_on_main_m99)) { /* reset stuff here */
        ...
        if (_setup.percent_flag && _setup.file_pointer) {
            ...
            for (;
                ...
                if (length ==
                    (LINELEN -
                     1)) { // line is too long. need to finish reading the line
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (move == G_1) {
        if (settings->feed_mode == FEED_MODE::UNITS_PER_MINUTE) {
            ...
            CHKS((settings->feed_rate == 0.0),
                 NCE_CANNOT_DO_G1_WITH_ZERO_FEED_RATE);
            CHKS((settings->speed[settings->active_spindle] == 0.0),
                 (_("Cannot feed with zero spindle speed in feed per rev "
                    "mode")));
        } else if (settings->feed_mode == FEED_MODE::INVERSE_TIME) {
            CHKS((!block->f_flag),
                 NCE_F_WORD_MISSING_WITH_INVERSE_TIME_G1_MOVE);
    ...
    if (move == G_1) {
        inverse_time_rate_straight(end_x,
                                   end_y,
                                   end_z,
                                   AA_end,
                                   BB_end,
    ...
    if ((settings->cutter_comp_side != CUTTER_COMP::OFF) && /* ! "== true" */
        ...
        if (settings->plane == CANON_PLANE::XZ) {
            if (settings->cutter_comp_firstmove)
                status = convert_straight_comp1(move,
                                                block,
                                                settings,
                                                end_z,
                                                end_x,
                ...
                status = convert_straight_comp2(move,
                                                block,
                                                settings,
                                                end_z,
                                                end_x,
        ...
        CHKS(((settings->spindle_turning[settings->active_spindle] !=
              ...
              (settings->spindle_turning[settings->active_spindle] !=
               CANON_COUNTERCLOCKWISE)),
             _("Chosen spindle (%i) not turning in G76"),
...
int Interp::convert_straight_indexer(int axis,
                                     int jnum,
                                     block_pointer block,
                                     setup_pointer settings)
{
    ...
    CHKS((end_x != settings->current_x || end_y != settings->current_y ||
          end_z != settings->current_z || u_end != settings->u_current ||
          v_end != settings->v_current || w_end != settings->w_current ||
    ...
    case 5:
        ...
        break;
    default: ERS((_("BUG: trying to index incorrect axis")));
    }
    return INTERP_OK;
}

int Interp::issue_straight_index(
    int axis, int jnum, double target, int lineno, setup_pointer settings)
{
...
static void threading_pass(setup_pointer settings,
                           block_pointer block,
                           int boring,
                           double safe_x,
                           double depth,
                           ...
                           double target_z)
{
    ...
    if (taper_dist && entry_taper) {
        ...
        STRAIGHT_FEED(block->line_number,
                      boring ? safe_x + depth - full_threadheight
                             : safe_x - depth + full_threadheight,
                      start_y,
                      start_z - zoff,
                      AABBCC); //in
        STRAIGHT_FEED(block->line_number,
                      boring ? safe_x + depth : safe_x - depth, //angled in
                      start_y,
                      start_z - zoff - taper_dist,
                      AABBCC);
        START_SPEED_FEED_SYNCH(settings->active_spindle, pitch, 0);
    } else {
        STRAIGHT_TRAVERSE(block->line_number,
                          boring ? safe_x + depth : safe_x - depth,
                          start_y,
                          start_z - zoff,
                          AABBCC); //in
        DISABLE_FEED_OVERRIDE();
    ...
    if (taper_dist && exit_taper) {
        STRAIGHT_FEED(block->line_number,
                      boring ? safe_x + depth : safe_x - depth, //over
                      start_y,
                      target_z - zoff + taper_dist,
                      AABBCC);
        START_SPEED_FEED_SYNCH(settings->active_spindle, taper_pitch, 0);
        STRAIGHT_FEED(block->line_number,
                      boring ? safe_x + depth - full_threadheight
                                    ...
                                    double end_z)
{
    ...
    CHKS((settings->cutter_comp_side != CUTTER_COMP::OFF),
         (_("Cannot use G76 threading cycle with cutter radius compensation "
            "on")));
    ...
    double taper_pitch =
        taper_dist > 0.0
            ? pitch * hypot(taper_dist, full_threadheight) / taper_dist
            : pitch;
...
int Interp::convert_straight_comp1(
    int move,               //!< either G_0 or G_1
    block_pointer block,    //!< pointer to a block of RS274 instructions
    setup_pointer settings, //!< pointer to machine settings
    ...
    double w_end)
{
    ...
    CHKS(((side != CUTTER_COMP::LEFT) && (side != CUTTER_COMP::RIGHT)),
         NCE_BUG_SIDE_NOT_RIGHT_OR_LEFT);
    CHKS((distance <= radius),
         _("Length of cutter compensation entry move is not greater than the "
           "tool radius"));

    alpha = atan2(py - cy, px - cx) +
            (side == CUTTER_COMP::LEFT ? M_PIl / 2. : -M_PIl / 2.);
    ...
    if (move == G_0) {
        enqueue_STRAIGHT_TRAVERSE(settings,
                                  block->line_number,
                                  cos(alpha),
                                  sin(alpha),
                                  0,
...
int Interp::convert_straight_comp2(
    int move,               //!< either G_0 or G_1
    block_pointer block,    //!< pointer to a block of RS274 instructions
    setup_pointer settings, //!< pointer to machine settings
    ...
    double w_end)
{
    ...
    if ((py == opy) && (px == opx)) { /* no XY motion */
        ...
        // some XY motion
        double beta =
            0.0; // initializing to avoid confusion over else branch below
        double gamma =
            0.0; // that does not define value but returns form function.
        CUTTER_COMP side = settings->cutter_comp_side;
        double radius =
            settings->cutter_comp_radius; /* will always be positive */
        ...
        if (!concave && (beta > small)) { /* ARC NEEDED */
            ...
            if (qc().front().type != QARC_FEED) {
                ...
                CHKS((halfcorner == 0.0),
                     (_("Zero degree inside corner is invalid for cutter "
                        "compensation")));
                ...
                theta =
                    (prev.turn > 0) ? base_dir + M_PI_2l : base_dir - M_PI_2l;
                ...
                if TOOL_INSIDE_ARC (side, prev.turn) {
                    ...
                    double l = d2 / oldrad;
                    CHKS((l > 1.0 || l < -1.0),
                         _("Arc to straight motion makes a corner the "
                           "compensated tool can't fit in without gouging"));
                    if (prev.turn > 0)
        ...
        (move == G_0 ? enqueue_STRAIGHT_TRAVERSE
                     : enqueue_STRAIGHT_FEED)(settings,
                                              block->line_number,
                                              px - opx,
                                              py - opy,
...
int Interp::convert_tool_change(
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (settings->tool_change_at_g30) {
        ...
        if (AA_end != settings->AA_current && (-1 != settings->a_indexer_jnum))
            issue_straight_index(
                3, settings->a_indexer_jnum, AA_end, -1, settings);
        if (BB_end != settings->BB_current && (-1 != settings->b_indexer_jnum))
            issue_straight_index(
                4, settings->b_indexer_jnum, BB_end, -1, settings);
        if (CC_end != settings->CC_current && (-1 != settings->c_indexer_jnum))
            issue_straight_index(
                5, settings->c_indexer_jnum, CC_end, -1, settings);
...
int Interp::convert_tool_length_offset(
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (g_code == G_49) {
        ...
        logDebug("convert_tool_length_offset h_flag=%d h_number=%d "
                 "toolchange_flag=%d current_pocket=%d\n",
                 block->h_flag,
                 block->h_number,
        ...
        logDebug("convert_tool_length_offset: using index=%d spindle_toolno=%d "
                 "pocket_toolno=%d",
                 idx,
                 settings->tool_table[0].toolno,
                 settings->tool_table[settings->current_pocket].toolno);

        tool_offset.tran.x =
            USER_TO_PROGRAM_LEN(settings->tool_table[idx].offset.tran.x);
        tool_offset.tran.y =
            USER_TO_PROGRAM_LEN(settings->tool_table[idx].offset.tran.y);
        tool_offset.tran.z =
            USER_TO_PROGRAM_LEN(settings->tool_table[idx].offset.tran.z);
        ...
        CHKS((block->h_flag &&
              (block->x_flag || block->y_flag || block->z_flag ||
               block->a_flag || block->b_flag || block->c_flag ||
               block->u_flag || block->v_flag || block->w_flag)),
             (_("G43.2: Can not have both H and axis words")));
        CHKS((!block->h_flag && !block->x_flag && !block->y_flag &&
              !block->z_flag && !block->a_flag && !block->b_flag &&
              !block->c_flag && !block->u_flag && !block->v_flag &&
              !block->w_flag),
             (_("G43.2: No axes specified and H word missing")));
        tool_offset = settings->tool_offset;
        if (block->h_flag) {
            CHP((find_tool_index(settings, block->h_number, &idx)));
            tool_offset.tran.x +=
            ...
            tool_offset.tran.z +=
                USER_TO_PROGRAM_LEN(settings->tool_table[idx].offset.tran.z);
            tool_offset.a +=
                USER_TO_PROGRAM_ANG(settings->tool_table[idx].offset.a);
            tool_offset.b +=
                USER_TO_PROGRAM_ANG(settings->tool_table[idx].offset.b);
            tool_offset.c +=
                USER_TO_PROGRAM_ANG(settings->tool_table[idx].offset.c);
            tool_offset.u +=
                USER_TO_PROGRAM_LEN(settings->tool_table[idx].offset.u);
            tool_offset.v +=
                USER_TO_PROGRAM_LEN(settings->tool_table[idx].offset.v);
            tool_offset.w +=
                USER_TO_PROGRAM_LEN(settings->tool_table[idx].offset.w);
        } else {
            if (block->x_flag)
                tool_offset.tran.x += block->x_number;
            if (block->y_flag)
...
int Interp::convert_tool_select(
    block_pointer block,    //!< pointer to a block of RS274 instructions
    setup_pointer settings) //!< pointer to machine settings
{
    int idx;
