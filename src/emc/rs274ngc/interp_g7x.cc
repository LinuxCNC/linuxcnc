...
class motion_base
{
  public:
    ...
    virtual void circular_move(bool ccw,
...
class segment
{
  ...
  public:
    segment(double sz, double sx, double ez, double ex)
        : start(sz, sx), end(ez, ex)
    {
    }
    ...
    virtual void flip_imag()
    {
        start = conj(start);
        end = conj(end);
    }
    virtual void flip_real()
    {
        start = -conj(start);
        end = -conj(end);
    }
    ...
    virtual void move(std::complex<double> d)
    {
        start += d;
        end += d;
    }
  ...
  protected:
    std::complex<double> corner_finish(segment *prev, segment *next)
    {
        ...
        if (real(p->end - n->start) > 1e-2)
            throw(std::string("Corner finish failed at ") +
                  to_string(prev->end));
        p->intersect(n.get());
  ...
  public:
    ...
    void offset(double distance) override
    {
        std::complex<double> d =
            I * distance * (start - end) / abs(start - end);
        start += d;
        end += d;
    }
...
void straight_segment::intersection_z(double x, intersections_t &is)
{
    ...
    if (std::abs(imag(start - end)) < tolerance) {
        ...
        is.push_back((x - start.imag()) / (end.imag() - start.imag()) *
                         (end.real() - start.real()) +
                     start.real());
}
...
class round_segment : public segment
{
  ...
  public:
    ...
    bool dive(std::complex<double> &, double, motion_base *, bool) override;
    void climb_only(std::complex<double> &, motion_base *) override;
    void draw(motion_base *out) override
    {
        out->circular_move(ccw, center, end);
    }
    void offset(double distance) override
    {
    ...
    {
        return std::min(abs(start - end), abs(start - center));
    }
    void flip_imag() override
    {
        ccw = !ccw;
        start = conj(start);
        end = conj(end);
    ...
    void move(std::complex<double> d) override
    {
        start += d;
        center += d;
        end += d;
...
bool round_segment::climb(std::complex<double> &location, motion_base *output)
{
    if (!ccw) { // G2
...
template <int swap> class swapped_motion : public motion_base
{
    motion_base *orig;
  ...
  public:
                       ...
                       std::complex<double> end) override
    {
        ...
        case 5:
            orig->circular_move(!ccw, conj(I * center), conj(I * end));
            break;
        case 6:
            orig->circular_move(!ccw, -conj(I * center), -conj(I * end));
            break;
...
class g7x : public std::list<std::unique_ptr<segment>>
{
  ...
  public:
    g7x() = default;
    g7x(g7x const &other)
        : std::list<std::unique_ptr<segment>>(), delta(other.delta),
          escape(other.escape), flip_state(other.flip_state)
    {
    ...
    */
    void do_g70(motion_base *out,
                double x,
                double z,
                double d,
                double e,
                int p,
                CUTTER_COMP *cutter_comp_side)
    {
    ...
    */
                ...
                bool do_rotate = false)
    {
        ...
        if (imag(back()->ep()) < imag(front()->sp())) {
            ...
            emplace_back(std::make_unique<straight_segment>(back()->ep(), ep));
        }
        pocket_starts.push_back(front()->sp());
        pocket(cycle, front()->sp(), begin(), swapped_out.get());
    }

    void do_g72(motion_base *out,
                int cycle,
                double x,
                double z,
                double u,
...
void g7x::add_distance(double distance)
{
    ...
    while (current_distance != distance) {
        ...
        max_distance =
            std::min(max_distance, std::abs(distance - current_distance));
        if (distance < 0)
    ...
    setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (settings->plane != CANON_PLANE::XZ)
        ERS("G%d.%d can only be used in XZ plane (G18)", cycle, subcycle);
    ...
    CHP(read((std::string("O") +
              std::to_string(static_cast<int>(block->q_number)) + " CALL")
                 .c_str()));
    for (;;) {
        if (block->o_name != 0)
        ...
        if (block->i_flag)
            center.imag(block->i_number);
        if (block->k_flag)
            center.real(block->k_number);
        ...
        if (start != end) {
            ...
            case 10:
                path.emplace_back(
                    std::make_unique<straight_segment>(start, end));
                break;
            case 20:
            case 30:
                if (block->r_flag) {
                ...
                if (!block->r_flag) {
                    ...
                    CHP(arc_data_ijk(
                        (settings->motion_mode == 30) ? G_3 : G_2,
                        settings->plane,
                        settings->current_z,
                        settings->current_x,
                        tpz,
                        ...
                        (block->k_flag) ? block->k_number : 0.0,
                        (block->i_flag) ? block->i_number : 0.0,
            ...
            if (settings->motion_mode == 0 || settings->motion_mode == 10 ||
                ...
                if (block->a_flag)
                    path.emplace_back(
                        std::make_unique<fillet_segment>(block->a_number, end));
                if (block->c_flag)
    ...
    int p = 1;
    if (original_block.d_flag)
        d = original_block.d_number_float;
    if (original_block.e_flag)
        e = original_block.e_number;
    if (original_block.i_flag)
        i = original_block.i_number;
    if (original_block.p_flag)
        p = static_cast<int>(original_block.p_number);
    if (original_block.r_flag)
        r = original_block.r_number;
    if (original_block.u_flag)
        u = original_block.u_number;
    if (original_block.w_flag)
        w = original_block.w_number;
    if (original_block.x_flag)
        x = original_block.x_number;
    if (original_block.z_flag)
        z = original_block.z_number;
    ...
    try {
        ...
        case 70:
            ...
            break;
        case 71:
            if (std::abs(real(dback)) > 0 &&
                imag(dfront) * (x - imag(start)) < 0) {
            ...
            break;
        case 72:
            if (std::abs(imag(dback)) > 0 &&
                real(dfront) * (z - real(start)) < 0) {
