    ...
    if (exists) {
        *double_ptr = value;
        ...
        logNP("%s: referencing undefined named parameter '%s' level=%d",
              name,
              paramNameBuf,
              (paramNameBuf[0] == '_') ? 0 : _setup.call_level);
...
int Interp::fetch_ini_param(const char *nameBuf, int *status, double *value)
{
    ...
    *status = 0;
    ...
    if ((n > 7) && ((s = (char *)strchr(&nameBuf[6], ']')) != NULL)) {
        ...
        if ((iniFileName = getenv("INI_FILE_NAME")) == NULL) {
            logNP("warning: referencing INI parameter '%s': no INI file",
                  nameBuf);
            *status = 0;
            return INTERP_OK;
        }
        ...
        if ((retval = inifile.Find(
                 value, &capName[closeBracket + 1], &capName[5])) == 0) {
            *status = 1;
            inifile.Close();
        } else {
            inifile.Close();
            *status = 0;
            ERS(_("Named INI parameter #<%s> not found in INI file '%s': "
                  "error=0x%x"),
                nameBuf,
                iniFileName,
                retval);
...
int Interp::fetch_hal_param(const char *nameBuf, int *status, double *value)
{
    ...
    *status = 0;
    if (!comp_id) {
        ...
        comp_id =
            hal_init(hal_comp); // manpage says: NULL ok - which fails miserably
        CHKS(comp_id < 0,
             _("fetch_hal_param: hal_init(%s): %d"),
             hal_comp,
             comp_id);
        CHKS((retval = hal_ready(comp_id)),
             _("fetch_hal_param: hal_ready(): %d"),
             retval);
    }
    char *s;
    int n = strlen(nameBuf);
    ...
    *status = 0;
    ...
    if (pi == frame->named_params.end()) { // not found
        ...
        if (FEATURE(INI_VARS) && (strncasecmp(nameBuf, "_ini[", 5) == 0)) {
            ...
            if (exists) {
                logNP(
                    "parameter '%s' retrieved from INI: %f", nameBuf, inivalue);
                *value = inivalue;
                *status = 1;
        ...
        if (FEATURE(HAL_PIN_VARS) && (strncasecmp(nameBuf, "_hal[", 5) == 0)) {
            ...
            if (exists) {
                logNP(
                    "parameter '%s' retrieved from HAL: %f", nameBuf, inivalue);
                *value = inivalue;
                *status = 1;
                return INTERP_OK;
            }
        ...
        *status = 0;
        ...
        if (pv->attr & PA_USE_LOOKUP) {
            ...
            *status = 1;
            ...
            python_plugin->call(
                NAMEDPARAMS_MODULE, nameBuf, tupleargs, kwargs, retval);
            CHKS(python_plugin->plugin_status() == PLUGIN_EXCEPTION,
                 "named param - pycall(%s):\n%s",
                 nameBuf,
                 python_plugin->last_exception().c_str());
            CHKS(retval.ptr() == Py_None,
                 "Python namedparams.%s returns no value",
                 nameBuf);
            ...
            *status = 0;
            ...
            Py_XDECREF(res_str);
            ERS("Python call %s.%s returned '%s' - expected double, int or "
                "string, got %s",
                NAMEDPARAMS_MODULE,
                nameBuf,
...
)
{
    ...
    if (pi == frame->named_params.end()) {
        ...
        CHKS(((pv->attr & PA_GLOBAL) && level),
             "BUG: variable '%s' marked global, but assigned at level %d",
             nameBuf,
             level);
...
int Interp::lookup_named_param(const char *nameBuf, double index, double *value)
{
    ...
    case NP_PLANE: // _plane
        switch (_setup.plane) {
        case CANON_PLANE::XY: *value = G_17; break;
        case CANON_PLANE::XZ: *value = G_18; break;
        case CANON_PLANE::YZ: *value = G_19; break;
        case CANON_PLANE::UV: *value = G_17_1; break;
        case CANON_PLANE::UW: *value = G_18_1; break;
        case CANON_PLANE::VW: *value = G_19_1; break;
        }
        break;

    case NP_CCOMP: // _ccomp - cutter compensation
        *value = (_setup.cutter_comp_side == CUTTER_COMP::RIGHT)  ? G_42
                 : (_setup.cutter_comp_side == CUTTER_COMP::LEFT) ? G_41
                                                                  : G_40;
        break;
    ...
    case NP_COORD_SYSTEM: // _coord_system - 0-9
        *value = (_setup.origin_index < 7) ? (530 + (10 * _setup.origin_index))
                                           : (584 + _setup.origin_index);
        break;

    case NP_TOOL_OFFSET: // _tool_offset
        *value = (_setup.tool_offset.tran.x || _setup.tool_offset.tran.y ||
                  _setup.tool_offset.tran.z || _setup.tool_offset.a ||
                  _setup.tool_offset.b || _setup.tool_offset.c ||
                  _setup.tool_offset.u || _setup.tool_offset.v ||
                  _setup.tool_offset.w);
        break;
    ...
    case NP_CURRENT_TOOL: *value = _setup.parameters[5400]; break;

    case NP_SELECTED_POCKET:
        if (_setup
                .random_toolchanger) { //random changers already report the real pocket number
            *value = _setup.selected_pocket;
        } else { //non random get it from the tool table
            if (_setup.tool_table[_setup.selected_pocket].pocketno ==
                0) { //pocket 0 is special on non-random changers
                *value = -1;
            } else {
    ...
    case NP_SELECTED_TOOL: *value = _setup.selected_tool; break;
    ...
    case NP_VALUE: *value = _setup.return_value; break;

    // predicate: the last NGC procedure did/did not return a value
    case NP_VALUE_RETURNED: *value = _setup.value_returned; break;

    case NP_CALL_LEVEL: *value = _setup.call_level; break;

    case NP_REMAP_LEVEL: *value = _setup.remap_level; break;
...
int Interp::init_named_parameters()
{
    ...
    const char *version_major =
        "_vmajor"; // named_parameter name (use lower case)
    const char *version_minor =
        "_vminor"; // named_parameter name (use lower case)
    const char *metric_machine =
        "_metric_machine"; // named_parameter name (use lower case)
    ...
    // 1.0 if G94 is on
    init_readonly_param(
        "_units_per_minute", NP_UNITS_PER_MINUTE, PA_USE_LOOKUP);
    ...
    // 1 if G97 is on
    init_readonly_param(
        "_spindle_rpm_mode", NP_SPINDLE_RPM_MODE, PA_USE_LOOKUP);

    init_readonly_param(
        "_spindle_css_mode", NP_SPINDLE_CSS_MODE, PA_USE_LOOKUP);

    // 1 if G90.1 is on
    init_readonly_param(
        "_ijk_absolute_mode", NP_IJK_ABSOLUTE_MODE, PA_USE_LOOKUP);

    // 1 if G7 is on
    init_readonly_param(
        "_lathe_diameter_mode", NP_LATHE_DIAMETER_MODE, PA_USE_LOOKUP);

    // 1 if G8 is on
    init_readonly_param(
        "_lathe_radius_mode", NP_LATHE_RADIUS_MODE, PA_USE_LOOKUP);
