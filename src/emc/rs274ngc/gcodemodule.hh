...
public:
    ...
    // `n` program points where the transform as it stands puts them: the
    // same machine and display coordinates a move endpoint on this line
    // lands at. `in` is `n` 9-DOF points; `machine` and `display` each take
    // three doubles per point. False when this canon does not transform,
    // which is what `RendererCanon.transform` reports as "not a rendered
    // parse".
    virtual bool transform_points(const Point9 * /*in*/, size_t /*n*/,
                                  double * /*machine*/,
                                  double * /*display*/) const { return false; }
