...
std::vector<unsigned int> nurbs_G5_knot_vector_creator(unsigned int n,
                                                       unsigned int k)
{
...
double nurbs_G5_Nmix(unsigned int i,
                     unsigned int k,
                     double u,
                     const std::vector<unsigned int> &knot_vector)
{
    if (k == 1) {
        ...
        if ((knot_vector[i + k - 1] - knot_vector[i] == 0) &&
            ...
            return ((knot_vector[i + k] - u) *
                    nurbs_G5_Nmix(i + 1, k - 1, u, knot_vector)) /
                   (knot_vector[i + k] - knot_vector[i + 1]);
        } else if ((knot_vector[i + k] - knot_vector[i + 1] == 0) &&
                   (knot_vector[i + k - 1] - knot_vector[i] != 0)) {
            return ((u - knot_vector[i]) *
                    nurbs_G5_Nmix(i, k - 1, u, knot_vector)) /
              ...
              const std::vector<unsigned int> &knot_vector)
{
    ...
    for (i = 0; i < (nurbs_control_points.size()); i++) {
        d = d + nurbs_G5_Nmix(i, k, u, knot_vector) *
    ...
    for (i = 0; i < (nurbs_control_points.size()); i++) {
        point.NURBS_X =
            ...
            nurbs_control_points[i].NURBS_X *
                nurbs_G5_Nmix(i, k, u, knot_vector) *
                nurbs_control_points[i].NURBS_W /
                nurbs_G5_Rden(u, k, nurbs_control_points, knot_vector);
        point.NURBS_Y =
            point.NURBS_Y +
                 ...
                 const std::vector<unsigned int> &knot_vector)
{
    ...
    NURBS_PLANE_POINT P1 =
        nurbs_G5_point(ulo, k, nurbs_control_points, knot_vector);
    NURBS_PLANE_POINT P3 =
        nurbs_G5_point(uhi, k, nurbs_control_points, knot_vector);
    NURBS_PLANE_POINT r = {(P3.NURBS_X - P1.NURBS_X) / (uhi - ulo),
                           (P3.NURBS_Y - P1.NURBS_Y) / (uhi - ulo)};
    unit(r);
    return r;
}
...
std::vector<double> nurbs_g6_knot_vector_creator(
    unsigned int n,
    unsigned int k,
...
double nurbs_G6_Nmix(unsigned int i,
                     unsigned int k,
                     double u,
                     const std::vector<double> &knot_vector_)
{
    if (k == 1) {
        ...
        if (k > 1) {
            if ((knot_vector_[i + k - 1] - knot_vector_[i] == 0) &&
                (knot_vector_[i + k] - knot_vector_[i + 1] != 0)) {
                return ((knot_vector_[i + k] - u) *
                        nurbs_G6_Nmix(i + 1, k - 1, u, knot_vector_)) /
                       (knot_vector_[i + k] - knot_vector_[i + 1]);
            } else {
                if ((knot_vector_[i + k] - knot_vector_[i + 1] == 0) &&
                    (knot_vector_[i + k - 1] - knot_vector_[i] != 0)) {
                    return ((u - knot_vector_[i]) *
                            nurbs_G6_Nmix(i, k - 1, u, knot_vector_)) /
                           (knot_vector_[i + k - 1] - knot_vector_[i]);
                } else {
                    if ((knot_vector_[i + k - 1] - knot_vector_[i] == 0) &&
                        (knot_vector_[i + k] - knot_vector_[i + 1] == 0)) {
                        return 0;
...
std::vector<std::vector<double>> nurbs_G6_Nmix_creator(
    double u, unsigned int k, double n, const std::vector<double> &knot_vector)
{
            ...
            const std::vector<double> &knot_vector)
{
    ...
    for (i = 0; i < (nurbs_control_points.size() - k); i++) {
        d = d + nurbs_G6_Nmix(i, k, u, knot_vector) *
                    nurbs_control_points[i].NURBS_R;
    }
    return d;
}
               ...
               const std::vector<double> &knot_vector)
{
    ...
    for (i = 0; i < (nurbs_control_points.size() - k); i++) {
        point.NURBS_X =
            point.NURBS_X +
            nurbs_control_points[i].NURBS_X *
                nurbs_G6_Nmix(i, k, u, knot_vector) *
                nurbs_control_points[i].NURBS_R /
                nurbs_Rden_(u, k, nurbs_control_points, knot_vector);
        point.NURBS_Y =
            point.NURBS_Y +
            nurbs_control_points[i].NURBS_Y *
                nurbs_G6_Nmix(i, k, u, knot_vector) *
...
double
nurbs_Rdenx(double /*u*/,
            unsigned int k,
            const std::vector<NURBS_G6_CONTROL_POINT> &nurbs_control_points,
            const std::vector<double> & /*knot_vector*/,
            const std::vector<std::vector<double>> &A6)
{
    unsigned int i;
    double d = 0.0;
    for (i = 0; i < (nurbs_control_points.size() - k); i++) {
                ...
                const std::vector<std::vector<double>> &A6)
{
    ...
    for (i = 0; i < (nurbs_control_points.size() - k); i++) {
        point.NURBS_X =
            point.NURBS_X +
            nurbs_control_points[i].NURBS_X * (A6[i][k]) *
                nurbs_control_points[i].NURBS_R /
                nurbs_Rdenx(u, k, nurbs_control_points, knot_vector, A6);
        point.NURBS_Y =
            point.NURBS_Y +
            nurbs_control_points[i].NURBS_Y * (A6[i][k]) *
                nurbs_control_points[i].NURBS_R /
                nurbs_Rdenx(u, k, nurbs_control_points, knot_vector, A6);
    }
    return point;
...
NURBS_PLANE_POINT nurbs_G6_point_x(
    double u,
    unsigned int k,
    const std::vector<NURBS_G6_CONTROL_POINT> &nurbs_control_points,
    const std::vector<double> &knot_vector) // è una funzione
{
    ...
    if (m == 10) {
        pointx.NURBS_X =
            nurbs_control_points[nurbs_control_points.size() - k - 1].NURBS_X;
        pointx.NURBS_Y =
            nurbs_control_points[nurbs_control_points.size() - k - 1].NURBS_Y;
    } else {
        ...
        for (int j = 0; j <= (kk - s); ++j) {
            CPP.NURBS_X = nurbs_control_points[j].NURBS_X *
                          nurbs_control_points[j].NURBS_R;
            CPP.NURBS_Y = nurbs_control_points[j].NURBS_Y *
                          nurbs_control_points[j].NURBS_R;
        ...
        double Pi_r_X, Pi_r_Y,
            Pi_r_W; // warning: ‘Pi_r_X, Pi_r_Y, Pi_r_W’ may be used uninitialized
        Pi_r_X = 0.0;
        Pi_r_Y = 0.0;
        Pi_r_W = 0.0; // jf
        ...
        for (unsigned r = 1; r <= h; ++r) {
            for (int j = 0; j <= kk - s; ++j) {
                nurbs_control_points_x[j].NURBS_X =
                    nurbs_control_points_a[j].NURBS_X;
                nurbs_control_points_x[j].NURBS_Y =
                    nurbs_control_points_a[j].NURBS_Y;
                nurbs_control_points_x[j].NURBS_W =
                    nurbs_control_points_a[j].NURBS_W;
            }
            for (int i = (kk - p + r); i <= kk - s; ++i) {
                ai_r = (u - knot_vector[i]) /
                       (knot_vector[i + p - r + 1] - knot_vector[i]);
                Pi_r_X = (1 - ai_r) * nurbs_control_points_x[i - 1].NURBS_X +
                         ai_r * nurbs_control_points_x[i].NURBS_X;
                nurbs_control_points_a[i].NURBS_X = Pi_r_X;
                Pi_r_Y = ((1 - ai_r) * nurbs_control_points_x[i - 1].NURBS_Y) +
                         (ai_r * nurbs_control_points_x[i].NURBS_Y);
                nurbs_control_points_a[i].NURBS_Y = Pi_r_Y;
                Pi_r_W = ((1 - ai_r) * nurbs_control_points_x[i - 1].NURBS_W) +
                         (ai_r * nurbs_control_points_x[i].NURBS_W);
...
std::vector<double> nurbs_G6_new_control_point_nurbs1(
    double u,
    unsigned int k,
    const std::vector<NURBS_G6_CONTROL_POINT> &nurbs_control_points,
    const std::vector<double> &knot_vector)
{
    ...
    for (int j = 0; j <= (kk - s); ++j) {
        CPP.NURBS_X =
            nurbs_control_points[j].NURBS_X * nurbs_control_points[j].NURBS_R;
        CPP.NURBS_Y =
            nurbs_control_points[j].NURBS_Y * nurbs_control_points[j].NURBS_R;
        CPP.NURBS_W = nurbs_control_points[j].NURBS_R;
        nurbs_control_points_a.push_back(CPP);
    }
...
// è una funzione
std::vector<double> nurbs_G6_new_control_point_nurbs2(
    double u,
    unsigned int k,
    const std::vector<NURBS_G6_CONTROL_POINT> &nurbs_control_points,
    const std::vector<double> &knot_vector)
{
    ...
    for (unsigned j = 0; j < knot_vector.size(); ++j) {
        if ((knot_vector[j] > u) && (kk == -1)) {
            kk =
                j -
                1; //deve ritornare l'indice del passo precedente, il quale individua Pk
            // printf("j=%d, knot=%d\n",j,knot_vector[j]);
            //break;
        }
    }
    ...
    /*********************************************************************************************/
    for (unsigned i = kk - s; i < (nurbs_control_points.size() - k);
         ++i) //aggiusta la dimensione -k
    {
        new_control_pointx.push_back(nurbs_control_points[i].NURBS_X);
        new_control_pointx.push_back(nurbs_control_points[i].NURBS_Y);
        new_control_pointx.push_back(nurbs_control_points[i].NURBS_R);
...
std::vector<double> nurbs_G6_knot_vector_new_creator_sgment(
    unsigned int k,
    const std::vector<NURBS_G6_CONTROL_POINT> &nurbs_control_points)
{
             ...
             double Rsk)
{
    ...
    double Nik, DNik;
    Nik =
        nurbs_G6_Nmix(i, k, u, knot_vector_); // calcolo della funzione di base
    DNik = Nderv(
        i, k, u, knot_vector_); // derivata prima della funzione di basa Ni,k
    d = ((DNik * nurbs_control_points[i].NURBS_R) / Rsk) -
        (Nik * nurbs_control_points[i].NURBS_R * f / (Rsk * Rsk));
    return d;
}

// LA funzione Dnurbs_point calcola   la derivata della nurbs C'(u) in u  è una funzione
NURBS_G6_DPLANE_POINT
Dnurbs_point(double u,
             unsigned int k,
             const std::vector<NURBS_G6_CONTROL_POINT> &nurbs_control_points,
             const std::vector<double> &knot_vector_)
{
    ...
    for (long unsigned int ii = 0; ii < (nurbs_control_points.size() - k);
         ii++) {
        f = f +
            Nderv(ii, k, u, knot_vector_) * nurbs_control_points[ii].NURBS_R;
    }
...
std::vector<double> nurbs_interval_span_knot_vector_creator(
    unsigned int n, unsigned int k, const std::vector<double> &knot_vector_)
{
               ...
               const std::vector<double> &knot_vector_)
{
    ...
    c1 = (a1 + b1) / 2;
    Sa1_b1_ =
        (h / 3) * (lderv(a1, k, nurbs_control_points, knot_vector_) +
                   (4 * lderv(c1, k, nurbs_control_points, knot_vector_)) +
                   lderv(b1, k, nurbs_control_points, knot_vector_));
    return Sa1_b1_;
}
...
std::vector<double> nurbs_lenght_vector_creator(
    unsigned int k,
    const std::vector<NURBS_G6_CONTROL_POINT> &nurbs_control_points,
    const std::vector<double> &knot_vector_,
    const std::vector<double> &span_knot_vector)
{
    ...
    {
        a = Sa1_b1_length_(span_knot_vector[i],
                           span_knot_vector[i + 1],
                           k,
...
double nurbs_lenght_tot(int j,
                        const std::vector<double> & /*span_knot_vector*/,
                        const std::vector<double> &lenght_vector)
{
    double ltot = 0;
    for (int i = 0; i <= j; ++i) {
    ...
    if (u == 0) {
        ...
        for (unsigned ii = 0; ii < span_knot_vector.size(); ++ii) {
            if ((span_knot_vector[ii] >= u) && (at == 0)) {
                ...
                a = Sa1_b1_length_(
                    a1, x, k, nurbs_control_points, knot_vector_);
                b = Sa1_b1_length_(
                    x, a1b1, k, nurbs_control_points, knot_vector_);
                c = Sa1_b1_length_(
                    a1b1, q, k, nurbs_control_points, knot_vector_);
                d = Sa1_b1_length_(
                    q, b1, k, nurbs_control_points, knot_vector_);
                e = Sa1_b1_length_(
                    b1, m, k, nurbs_control_points, knot_vector_);
                f = Sa1_b1_length_(
                    m, b1b2, k, nurbs_control_points, knot_vector_);
                g = Sa1_b1_length_(
                    b1b2, w, k, nurbs_control_points, knot_vector_);
                h = Sa1_b1_length_(
                    w, b2, k, nurbs_control_points, knot_vector_);
...
std::vector<double> nurbs_Du_span_knot_vector_creator(
    unsigned int k,
    const std::vector<NURBS_G6_CONTROL_POINT> &nurbs_control_points,
    const std::vector<double> &knot_vector_,
    const std::vector<double> &span_knot_vector)
{
    ...
    for (unsigned ii = 0; ii < span_knot_vector.size(); ++ii) {
        double Dl_u =
            lderv(span_knot_vector[ii], k, nurbs_control_points, knot_vector_);
        double Du_l = 1 / Dl_u;
...
std::vector<double>
nurbs_costant_crator(const std::vector<double> &span_knot_vector,
                     const std::vector<double> &lenght_vector,
                     const std::vector<double> &Du_span_knot_vector)
{
...
double nurbs_uj_l(double l,
                  const std::vector<double> &span_knot_vector,
                  const std::vector<double> &lenght_vector,
                  const std::vector<double> &nurbs_costant)
{
    ...
    if (l == 0) {
        ...
        for (unsigned ii = 0; ii < nurbs_costant.size() - 5; ii = ii + 6) {
            if ((l <= lenght_vector[0]) && (at2 == 0) && (l > 0)) {
                ...
                u_l =
                    a + b * (l) + (c * pow(l, 2)) + (d * pow(l, 2) * (l - lj1));
                break;
            } else if ((nurbs_lenght_tot(j, span_knot_vector, lenght_vector) >
                        l) &&
                       (at == 0) && (u_l == 0)) {
                at += 1;
