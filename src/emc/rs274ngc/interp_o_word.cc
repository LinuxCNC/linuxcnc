    ...
    char *foundFileDirect) // where to store the result
{
    ...
    while ((aFile = readdir(aDir))) {
        if (aFile->d_type == DT_DIR && (0 != strncmp(aFile->d_name, "..", 3)) &&
 ...
 */
int Interp::control_save_offset(
    ...
    setup_pointer settings) /* pointer to machine settings */
{
    ...
    if (control_find_oword(block, settings, &op) == INTERP_OK) {
        if (settings->sequence_number - 1 == op->sequence_number)
            ...
            // already exists
            ERS(_("File:%s line:%d redefining sub: o|%s| already defined in "
                  "file:%s"),
                settings->filename,
                settings->sequence_number,
                block->o_name,
...
const char *o_ops[] = {
    "O_none",      "O_sub",      "O_endsub",   "O_call",   "O_do",
    "O_while",     "O_if",       "O_elseif",   "O_else",   "O_endif",
    "O_break",     "O_continue", "O_endwhile", "O_return", "O_repeat",
    "O_endrepeat", "M_98",       "M_99",       "O_",
                         ...
                         int call_type)
{
    ...
    context_pointer previous_frame =
        &settings->sub_context[settings->call_level - 1];
    ...
    case CT_NGC_M98_SUB:
        ...
        if (FEATURE(OWORD_N_ARGS)) {
            ...
            CHP(store_named_param(settings,
                                  "n_args",
        ...
        case CS_REEXEC_PYOSUB:
            ...
            status =
                pycall(settings,
                       ...
                       OWORD_MODULE,
                       current_frame->subName,
                       settings->call_state == CS_NORMAL ? PY_OWORDCALL
                                                         : PY_FINISH_OWORDCALL);
            CHKS(status == INTERP_ERROR,
                 "pycall(%s.%s) failed",
                 OWORD_MODULE,
                 current_frame->subName);
            switch (
                status = handler_returned(
                    settings, current_frame, current_frame->subName, true)) {
    ...
    case CT_REMAP:
        ...
        case CS_NORMAL:
            if (remap->remap_py || remap->prolog_func || remap->epilog_func) {
                CHKS(!PYUSABLE,
                     "%s (remapped) uses Python functions, but the Python "
        ...
        case CS_REEXEC_PROLOG:
            if (remap->prolog_func) {
                ...
                CHKS(status == INTERP_ERROR,
                     ...
                     REMAP_MODULE,
                     remap->prolog_func);
                switch (status = handler_returned(settings,
                                                  current_frame,
                                                  current_frame->subName,
                                                  false)) {
                case INTERP_EXECUTE_FINISH:
        ...
        case CS_REEXEC_PYBODY:
            if (remap->remap_py) {
                status =
                    pycall(settings,
                           ...
                           REMAP_MODULE,
                           remap->remap_py,
                           settings->call_state == CS_NORMAL ? PY_BODY
                                                             : PY_FINISH_BODY);
                CHP(status);
                switch (status = handler_returned(settings,
                                                  current_frame,
            ...
            if (remap->remap_ngc) {
                CHP(execute_call(settings, current_frame, CT_NGC_OWORD_SUB));
            }
        }
    }
    return status;
...
int Interp::execute_return(setup_pointer settings,
                           context_pointer current_frame,
                           int call_type)
{
    int status = INTERP_OK;
    ...
    case CT_NONE: // sub definition
        if (settings->call_level != 0) {
            ...
            if (previous_frame->position == -1) {
                ...
                if (0 != strcmp(settings->filename, previous_frame->filename)) {
                    ...
                    settings->file_pointer =
                        fopen(previous_frame->filename, "r");
                    if (settings->file_pointer == NULL) {
                ...
                fseek(
                    settings->file_pointer, previous_frame->position, SEEK_SET);
                settings->sequence_number = previous_frame->sequence_number;
            ...
            if (eblock->o_type == O_endsub) {
                ...
                if (settings->skipping_o) {
                    logOword(
                        "case O_endsub in defn -- no longer skipping to:|%s|",
                        settings->skipping_o);
                    settings->skipping_o = NULL;
                }
    ...
    setup_pointer settings) // pointer to machine settings
{
    ...
    if (it != settings->offset_map.end()) {
        op = &it->second;
        if ((settings->filename[0] != 0) & (settings->file_pointer == NULL)) {
            ERS(NCE_FILE_NOT_OPEN);
        }
        if (0 != strcmp(settings->filename, op->filename)) {
            // open the new file...
            ...
            strncpy(
                settings->filename, op->filename, sizeof(settings->filename));
...
int Interp::handler_returned(setup_pointer settings,
                             ...
                             bool osub)
{
    ...
    case RET_DOUBLE:
        if (osub) { // float values are ok for osubs
            settings->return_value =
                active_frame->pystuff.impl->py_returned_double;
    ...
    case RET_INT:
        if (osub) { // let's be liberal with types - widen to double return value
            settings->return_value =
                (double)active_frame->pystuff.impl->py_returned_int;
            settings->value_returned = 1;
        } else
            return active_frame->pystuff.impl->py_returned_int;
        break;
    case RET_ERRORMSG: status = INTERP_ERROR; break;
    }
    return status;
}
...
{
    context_pointer leaving_frame =
        &settings->sub_context[settings->call_level];
...
int Interp::convert_control_functions(
    block_pointer block,    // pointer to a block of RS274/NGC instructions
    setup_pointer settings) // pointer to machine settings
{
    int status = INTERP_OK;
    ...
    case M_99:
        ...
        if ((settings->call_level == 0) && (settings->sub_name == NULL)) {
        ...
        CHP(execute_return(settings, current_frame, current_frame->call_type));
        break;
    ...
    case M_98:
        ...
        current_frame = &settings->sub_context[settings->call_level];
        CHP(execute_call(settings, current_frame, current_frame->call_type));
        break;
    ...
    case O_repeat:
        ...
        if ((status != INTERP_OK) || (op->type == block->o_type)) {
            ...
            if (op->repeat_count == -1)
                op->repeat_count = round_to_int(settings->test_value);
            ...
            if (op->repeat_count > 0) {
                ...
                // skip forward
                logOword("skipping forward: [%s] in 'repeat'", block->o_name);
                settings->skipping_o = block->o_name;
                settings->skipping_start = settings->sequence_number;
    ...
    case O_while:
        ...
        if ((status != INTERP_OK) || (op->type == block->o_type)) {
            ...
            if (settings->test_value != 0.0) // true - execute forward
                logOword("executing forward: [%s] in 'while'", block->o_name);
            else {
                // false -  skip forward
                logOword("skipping forward: [%s] in 'while'", block->o_name);
                settings->skipping_o = block->o_name;
                settings->skipping_start = settings->sequence_number;
            }
        } else {
            ...
            if ((settings->test_value != 0.0) && !settings->doing_break) {
                // true - loop on back
                logOword("looping back to: [%s] in 'do while'", block->o_name);
    ...
    case O_if:
        if (control_find_oword(block, settings, &op) == INTERP_OK) {
            if (settings->sequence_number != (op->sequence_number + 1))
                OERR(_("%d: duplicate O-word label - already defined in line "
                       "%d: '%s'"),
                     settings->sequence_number,
                     op->sequence_number + 1,
                     settings->linetext);
        ...
        if (settings->test_value != 0.0) {
            //true
            logOword("executing forward: [%s] in 'if'", block->o_name);
            settings->skipping_o = NULL;
            settings->executed_if = 1;
        } else {
            //false
            logOword("skipping forward: [%s] in 'if'", block->o_name);
            settings->skipping_o = block->o_name;
            settings->skipping_start = settings->sequence_number;
    ...
    case O_else:
        ...
        if ((settings->skipping_o) &&
            ...
            logOword("stop skipping forward: [%s] in 'else'", block->o_name);
            settings->executed_if = 1;
            settings->skipping_o = NULL;
        } else {
            // we were not skipping -- so skip
            logOword("start skipping forward: [%s] in 'else'", block->o_name);
        }
        break;
    ...
    case O_endif:
        ...
        settings->skipping_o = NULL;
        logOword("stop skipping forward: [%s] in 'endif'", block->o_name);
    ...
    case O_break:
        ...
        if ((op->type != O_while) && (op->type != O_do))
            OERR(_("%d: no matching while/do label: '%s' (found '%s' in line "
                   "%d)"),
                 settings->sequence_number,
                 settings->linetext,
                 o_ops[op->type] + 2,
        ...
        logOword("start skipping forward: [%s] in 'break'", block->o_name);
        break;
    ...
    case O_continue:
        ...
        if ((settings->skipping_o) &&
            (0 == strcmp(settings->skipping_o, block->o_name))) {
            logOword("already skipping: [%s] in 'continue'", block->o_name);
            return INTERP_OK;
        }
        // start skipping
        settings->skipping_o = block->o_name;
        ...
        logOword("start skipping forward: [%s] in 'continue'", block->o_name);
        break;
