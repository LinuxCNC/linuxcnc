...
void enqueue_SET_FEED_RATE(double feed)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate set feed rate %f\n", feed);
        SET_FEED_RATE(feed);
        return;
    }
    queued_canon q;
    ...
    if (debug_qc)
        printf("enqueue set feed rate %f\n", feed);
    qc().push_back(q);
}
...
void enqueue_SET_FEED_MODE(int spindle, int mode)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate set feed mode %d\n", mode);
    ...
    if (debug_qc)
        printf("enqueue set feed mode %d\n", mode);
    qc().push_back(q);
}
...
void enqueue_START_SPINDLE_CLOCKWISE(int spindle)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate spindle clockwise\n");
    ...
    if (debug_qc)
        printf("enqueue spindle clockwise\n");
    qc().push_back(q);
}
...
void enqueue_START_SPINDLE_COUNTERCLOCKWISE(int spindle)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate spindle counterclockwise\n");
    ...
    q.data.set_spindle_speed.spindle = spindle;
    if (debug_qc)
        printf("enqueue spindle counterclockwise\n");
    qc().push_back(q);
}
...
void enqueue_ORIENT_SPINDLE(int spindle, double orientation, int mode)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate spindle orient\n");
...
void enqueue_WAIT_ORIENT_SPINDLE_COMPLETE(int spindle, double timeout)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate wait spindle orient complete\n");
    ...
    if (debug_qc)
        printf("enqueue wait spindle orient complete\n");
    qc().push_back(q);
}
...
void enqueue_SET_SPINDLE_MODE(int spindle, double mode)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate spindle mode %f\n", mode);
...
void enqueue_SET_SPINDLE_SPEED(int spindle, double speed)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate set spindle speed %f\n", speed);
    ...
    if (debug_qc)
        printf("enqueue set spindle speed %f\n", speed);
    qc().push_back(q);
}
                          ...
                          double w)
{
    ...
    if (debug_qc)
        printf(
            "enqueue straight feed lineno %d to %f %f %f direction %f %f %f\n",
            l,
            x,
            y,
            z,
                              ...
                              double w)
{
    ...
    if (debug_qc)
        printf(
            "enqueue straight traverse lineno %d to %f %f %f direction %f %f "
            "%f\n",
            l,
            x,
            y,
                      ...
                      double w)
{
    ...
    if (debug_qc)
        printf(
            "enqueue arc lineno %d to %f %f center %f %f turn %d sweeping %f\n",
            l,
            end1,
            end2,
            center1,
...
void enqueue_M_USER_COMMAND(int index, double p_number, double q_number)
{
    if (qc().empty()) {
        if (debug_qc)
            printf("immediate M_USER_COMMAND index=%d p=%f q=%f\n",
                   index,
                   p_number,
                   q_number);
        (*(USER_DEFINED_FUNCTION[index - 100]))(
            index - 100, p_number, q_number);
        return;
    }
    queued_canon q;
    ...
    if (debug_qc)
        printf("enqueue M_USER_COMMAND index=%d p=%f q=%f\n",
               index,
               p_number,
               q_number);
    qc().push_back(q);
...
void qc_scale(double scale)
{
    ...
    if (qc().empty()) {
        if (debug_qc)
            printf("not scaling because qc is empty\n");
        return;
    }

    if (debug_qc)
        printf("scaling qc by %f\n", scale);
...
void dequeue_canons(setup_pointer settings)
{
    ...
    if (debug_qc)
        printf("dequeueing: endpoint is now invalid\n");
    endpoint_valid = 0;
    ...
    for (unsigned int i = 0; i < qc().size(); i++) {
        ...
        case QSTRAIGHT_FEED:
            ...
            STRAIGHT_FEED(q.data.straight_feed.line_number,
                          ...
                          q.data.straight_feed.a,
                          q.data.straight_feed.b,
                          q.data.straight_feed.c,
                          q.data.straight_feed.u,
                          q.data.straight_feed.v,
                          q.data.straight_feed.w);
            break;
        case QSTRAIGHT_TRAVERSE:
            if (debug_qc)
        ...
        case QWAIT_ORIENT_SPINDLE_COMPLETE:
            if (debug_qc)
                printf("issuing wait orient spindle complete\n");
            WAIT_SPINDLE_ORIENT_COMPLETE(
                q.data.wait_orient_spindle_complete.spindle,
...
int Interp::move_endpoint_and_flush(setup_pointer settings, double x, double y)
{
    ...
    for (unsigned int i = 0; i < qc().size(); i++) {
        ...
        case QARC_FEED:
            ...
            r2 =
                hypot(x - q.data.arc_feed.center1, y - q.data.arc_feed.center2);
            l2 = find_turn(endpoint[0],
                           endpoint[1],
            ...
            if (debug_qc)
                printf(
                    "moving endpoint of arc lineno %d old sweep %f new sweep "
                    "%f\n",
                    q.data.arc_feed.line_number,
                    l1,
                    l2);
            ...
            if (fabs(r1 - r2) > .01)
                ERS(_("BUG: cutter compensation has generated an invalid arc "
                      "with mismatched radii r1 %f r2 %f\n"),
                    r1,
                    r2);
            if (l1 != 0.0 && endpoint_valid &&
                fabs(l2) > fabs(l1) + (settings->length_units == CANON_UNITS_MM
                                           ? .0254
                                           : .001)) {
                ERS(_("Arc move in concave corner cannot be reached by the "
        ...
        case QSTRAIGHT_TRAVERSE:
            ...
            case CANON_PLANE::XY:
                x1 =
                    q.data.straight_traverse.dx; // direction of original motion
            ...
            case CANON_PLANE::XZ:
                x1 =
                    q.data.straight_traverse.dz; // direction of original motion
                y1 = q.data.straight_traverse.dx;
                x2 = x - endpoint[0]; // new direction after clipping
                y2 = y - endpoint[1];
                break;
            default: ERS(_("BUG: Unsupported plane in cutter compensation"));
            }

            dot = x1 * x2 +
                  y1 * y2; // not normalized; we only care about the angle
            if (debug_qc)
                printf(
                    "moving endpoint of traverse old dir %f new dir %f dot %f "
                    "endpoint_valid %d\n",
                    atan2(y1, x1),
                    atan2(y2, x2),
                    dot,
            ...
            if (endpoint_valid && dot < 0) {
                ...
                ERS(_("Straight traverse in concave corner cannot be reached "
        ...
        case QSTRAIGHT_FEED:
            ...
            dot = x1 * x2 + y1 * y2;
            if (debug_qc)
                printf("moving endpoint of feed old dir %f new dir %f dot %f "
                       "endpoint_valid %d\n",
                       atan2(y1, x1),
                       atan2(y2, x2),
                       dot,
            ...
            if (endpoint_valid && dot < 0) {
                ...
                // path has crossed because we backed up further
                // than the line is long.  this will gouge.
                ERS(_("Straight feed in concave corner cannot be reached by "
                      "the tool without gouging"));
            }
            switch (settings->plane) {
            case CANON_PLANE::XY:
                q.data.straight_feed.x = x;
