...
std::string handle_pyerror()
{
    ...
    bp::handle<> hexc(bp::allow_null(exc)), hval(bp::allow_null(val)),
    ...
    if (!hval) {
        ...
        if (!tb) {
            bp::object format_exception_only(
                   ...
                   int calltype)
{
    ...
    case PY_FINISH_EPILOG:
        ...
        try {
            ...
            if (PyErr_Occurred()) {
                ...
                if (PyErr_ExceptionMatches(PyExc_StopIteration)) {
                    ...
                    bp::handle_exception();
                    PyErr_Clear();
                    logPy("pycall: call generator - exception: %s",
                          msg.c_str());
        ...
        break;
    default:
        python_plugin->call(module,
                            funcname,
                            frame->pystuff.impl->tupleargs,
                            frame->pystuff.impl->kwargs,
                            retval);
        CHKS(python_plugin->plugin_status() == PLUGIN_EXCEPTION,
             "pycall(%s):\n%s",
             funcname,
    ...
    try {
        ...
        case PY_FINISH_EPILOG:
            ...
            if (retval.ptr() != Py_None) {
                if (PyGen_Check(retval.ptr())) {
                    ...
                    frame->pystuff.impl->generator_next =
                        bp::getattr(retval, "__next__");
                    // and  call it for the first time.
                    // Expect execution up to first 'yield INTERP_EXECUTE_FINISH'.
                    frame->pystuff.impl->py_returned_int =
                        bp::extract<int>(frame->pystuff.impl->generator_next());
                    ...
                    frame->pystuff.impl->py_returned_int =
                        bp::extract<int>(retval);
                    frame->pystuff.impl->py_return_type = RET_INT;
                    logPy("Python call %s.%s returned int: %d",
                          module,
                          funcname,
                          frame->pystuff.impl->py_returned_int);
                } else if (PyFloat_Check(retval.ptr())) {
                    frame->pystuff.impl->py_returned_double =
                        bp::extract<double>(retval);
                    frame->pystuff.impl->py_return_type = RET_DOUBLE;
                    logPy("Python call %s.%s returned float: %f",
                          module,
                          funcname,
                          frame->pystuff.impl->py_returned_double);
                } else {
                    ...
                    Py_XDECREF(res_str);
                    ERM("Python call %s.%s returned '%s' - expected generator, "
                        "int, or float value, got %s",
                        module,
                        funcname,
        ...
        case PY_PLUGIN_CALL:
            ...
            if ((retval.ptr() != Py_None) && (PyLong_Check(retval.ptr()))) {

                // FIXME check new return value convention
                status = frame->pystuff.impl->py_returned_int =
                    bp::extract<int>(retval);
                frame->pystuff.impl->py_return_type = RET_INT;
                logPy("pycall(%s):  PY_INTERNAL/PY_PLUGIN_CALL: return code=%d",
                      funcname,
                      status);
            } else {
                logPy("pycall(%s):  PY_INTERNAL: expected an int return code",
                      funcname);
                res_str = PyObject_Str(retval.ptr());
                ERM("Python internal function '%s' expected tuple or int "
                    "return value, got '%s' (%s)",
                    funcname,
                    PyUnicode_AsUTF8(res_str),
                    Py_TYPE(retval.ptr())->tp_name);
                Py_XDECREF(res_str);
