...
double ParamClass::getitem(bp::object sub)
{
    ...
    if (IS_STRING(sub)) {
        ...
        throw std::runtime_error(
            "params subscript type must be integer or string");
    }
    return retval;
}
...
double ParamClass::setitem(bp::object sub, double dvalue)
{
    if (IS_STRING(sub)) {
        char const *varname = bp::extract<char const *>(sub);
        int status =
            interp.add_named_param(varname, varname[0] == '_' ? PA_GLOBAL : 0);
        ...
        if ((index < 0) || (index > RS274NGC_MAX_PARAMETERS - 1)) {
            ...
            sstr << "params subscript out of range : " << index
                 << " - must be between 0 and " << RS274NGC_MAX_PARAMETERS;
...
{
    bp::list result =
        namelist(interp._setup.sub_context[interp._setup.call_level]);
    result.extend(namelist(interp._setup.sub_context[0]));
    return result;
};

int ParamClass::length()
{
