...
int Interp::write_g_codes(
    block_pointer block,    //!< pointer to a block of RS274/NGC instructions
    setup_pointer settings) //!< pointer to machine settings
{
    settings->active_g_codes[0] = settings->sequence_number;
    settings->active_g_codes[1] = settings->motion_mode;
    settings->active_g_codes[2] =
        ((block == NULL) ? -1 : block->g_modes[GM_MODAL_0]);
    switch (settings->plane) {
    case CANON_PLANE::XY: settings->active_g_codes[3] = G_17; break;
    case CANON_PLANE::XZ: settings->active_g_codes[3] = G_18; break;
    case CANON_PLANE::YZ: settings->active_g_codes[3] = G_19; break;
    case CANON_PLANE::UV: settings->active_g_codes[3] = G_17_1; break;
    case CANON_PLANE::UW: settings->active_g_codes[3] = G_18_1; break;
    case CANON_PLANE::VW: settings->active_g_codes[3] = G_19_1; break;
    }
    settings->active_g_codes[4] =
        (settings->cutter_comp_side == CUTTER_COMP::RIGHT)  ? G_42
        : (settings->cutter_comp_side == CUTTER_COMP::LEFT) ? G_41
                                                            : G_40;
    settings->active_g_codes[5] =
        (settings->length_units == CANON_UNITS_INCHES) ? G_20 : G_21;
    settings->active_g_codes[6] =
        (settings->distance_mode == DISTANCE_MODE::ABSOLUTE) ? G_90 : G_91;
    settings->active_g_codes[7] =
        (settings->feed_mode == FEED_MODE::INVERSE_TIME)       ? G_93
        : (settings->feed_mode == FEED_MODE::UNITS_PER_MINUTE) ? G_94
                                                               : G_95;
    settings->active_g_codes[8] = (settings->origin_index < 7)
                                      ? (530 + (10 * settings->origin_index))
                                      : (584 + settings->origin_index);
    settings->active_g_codes[9] =
        (settings->tool_offset.tran.x || settings->tool_offset.tran.y ||
         settings->tool_offset.tran.z || settings->tool_offset.a ||
         settings->tool_offset.b || settings->tool_offset.c ||
         settings->tool_offset.u || settings->tool_offset.v ||
         settings->tool_offset.w)
            ? G_43
            : G_49;
    settings->active_g_codes[10] =
    ...
    settings->active_g_codes[11] =
        ...
        : (settings->control_mode == CANON_EXACT_PATH) ? G_61
                                                       : G_61_1;
    settings->active_g_codes[12] = -1;
    settings->active_g_codes
        [13] = //I don't even know how to display the mode of an arbitrary number of spindles (andypugh 17/6/16)
        (settings->spindle_mode[0] == SPINDLE_MODE::CONSTANT_RPM) ? G_97 : G_96;
    settings->active_g_codes[14] =
        (settings->ijk_distance_mode == DISTANCE_MODE::ABSOLUTE) ? G_90_1
                                                                 : G_91_1;
    ...
    } else if (settings->parameters[5210] == 1 &&
               block->g_modes[GM_MODAL_0] ==
                   -1) { // this handles aborts, m2/m30
        settings->active_g_codes[16] = G_92_3;
    } else {
...
int Interp::write_m_codes(
    block_pointer block,    //!< pointer to a block of RS274/NGC instructions
    setup_pointer settings) //!< pointer to machine settings
{
    settings->active_m_codes[0] = settings->sequence_number; /* 0 seq number  */
    settings->active_m_codes[1] =
        (block == NULL) ? -1 : block->m_modes[4]; /* 1 stopping    */
    settings->active_m_codes[2] =
        (settings->spindle_turning[0] == CANON_STOPPED) ? 5
                                                        : /* 2 spindle     */
            (settings->spindle_turning[0] == CANON_CLOCKWISE) ? 3
                                                              : 4;
    ...
    if (settings->feed_override) {
        if (settings->speed_override[0])
            settings->active_m_codes[6] = 48;
        else
            settings->active_m_codes[6] = 50;
    } else if (settings->speed_override[0]) {
        settings->active_m_codes[6] = 51;
    } else
        settings->active_m_codes[6] = 49;
...
int Interp::write_settings(
    setup_pointer settings) //!< pointer to machine settings
{
    settings->active_settings[0] =
        settings->sequence_number;                      /* 0 sequence number */
    ...
    settings->active_settings[4] =
        settings->naivecam_tolerance; /* 4 naive CAM tolerance */
                            ...
                            StateTag &state)
{
    ...
    bool external_sub =
        strcmp(settings->filename, settings->sub_context[0].filename);
    ...
    switch (settings->plane) {
    case CANON_PLANE::XY: state.fields[GM_FIELD_PLANE] = G_17; break;
    case CANON_PLANE::XZ: state.fields[GM_FIELD_PLANE] = G_18; break;
    case CANON_PLANE::YZ: state.fields[GM_FIELD_PLANE] = G_19; break;
    case CANON_PLANE::UV: state.fields[GM_FIELD_PLANE] = G_17_1; break;
    case CANON_PLANE::UW: state.fields[GM_FIELD_PLANE] = G_18_1; break;
    case CANON_PLANE::VW: state.fields[GM_FIELD_PLANE] = G_19_1; break;
    }
    ...
    state.fields_float[GM_FIELD_FLOAT_PATH_TOLERANCE] = settings->tolerance;
