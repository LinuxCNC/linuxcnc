...
const char *Interp::interp_status(int status)
{
    ...
    static const char *msgs[] = {"INTERP_OK",
                                 "INTERP_EXIT",
    ...
    snprintf(statustext,
             sizeof(statustext),
             "%s%s%d",
             ((status >= INTERP_OK) && (status <= INTERP_ERROR))
                 ? msgs[status]
                 : "unknown interpreter error",
             (status > INTERP_MIN_ERROR) ? " - error: " : " - ",
             status);
    return statustext;
}
...
Interp::Interp() : log_file(stderr), _setup{}
{
    _setup.init_once = 1;
...
// KLUDGE just to get unit tests to stop complaining about python modules we won't use anyway
    ...
    try {
        ...
        bp::object retval;
        python_plugin->run_string(
            "from interpreter import this", retval, false);
...
void Interp::doLog(
    unsigned int flags, const char *file, int line, const char *fmt, ...)
{
    struct timeval tv;
    struct tm *tm;
    va_list ap;
...
int Interp::_execute(const char *command)
{
    ...
    logDebug(
        "execute:%s %s='%s' mdi_int=%d o_type=%s o_name=%s cl=%d rl=%d type=%s "
        "state=%s",
        MDImode ? "MDI" : "auto",
    ...
    for (
        n = 0; n < _setup.parameter_occurrence;
    ...
    for (
        n = 0; n < _setup.named_parameter_occurrence;
        n++) { // copy parameter settings from parameter buffer into parameter table

        logDebug("storing param:|%s|", _setup.named_parameters[n]);
        CHP(store_named_param(&_setup,
	  ...
	  for (int i = _setup.remap_level - 1; i > 0; i--) {
		  ...
		  ERS("recursive remapping for %s detected", cblock->executing_remap->name);
            ...
            if (status < 0) {
                ...
                // sanity:
                if (cblock->remappings.find(-status) ==
                    cblock->remappings.end()) {
                    ERS("BUG: execute_block: got %d - not in remappings() !! "
                        "(next_remap=%d)",
                        -status,
                        next_remap);
                }
                ...
                if (MDImode) {
                    ...
                    if ((eblock->o_name != 0) || (_setup.mdi_interrupt)) {
                        ...
                        while (
                            MDImode &&
                            _setup.call_level) { // we are still in a subroutine
                            CHP(read(
                                0)); // reads from current file and calls parse
                            status =
                                execute(); // special handling for mdi errors
                            if (status == INTERP_EXECUTE_FINISH)
                                _setup.mdi_interrupt = true;
                            CHP(status);
                        }
            ...
            if ((status == INTERP_EXIT) && (_setup.remap_level > 0) &&
                (_setup.call_level > 0)) {
                // an M2 was encountered while executing a handler.
                logRemap("standard case status=%s remap_level=%d call_level=%d "
                         "blocktext='%s' MDImode=%d",
                         interp_status(status),
                         _setup.remap_level,
                         _setup.call_level,
                         _setup.blocktext,
...
int Interp::execute(const char *command, int line_number)
{
    ...
    if ((_setup.call_level == 0) && (status == INTERP_EXECUTE_FINISH) &&
        (_setup.mdi_interrupt)) {
        logDebug(" execute() clearing mdi_interrupt");
        _setup.mdi_interrupt =
            false; // seems to work ok! FIXME mah is this needed?
    }
    return status;
}
...
int Interp::remap_finished(int phase)
{
    ...
    if (phase < 0) {
        ...
        if (it != cblock->remappings.end()) {
            ...
            logRemap("starting phase %d  (remap_level=%d call_level=%d)",
                     next_remap,
                     _setup.remap_level,
                     _setup.call_level);
            ...
            status = execute_block(cblock, &_setup);
            logRemap("phase %d started,  execute_block() returns %d",
                     next_remap,
                     status);
            ...
            logRemap(
                "no more remaps in controlling_block found (remap_level=%d "
                "call_level=%d), remappings size=%zd, dropping",
                _setup.remap_level,
                _setup.call_level,
                cblock->remappings.size());

            status = execute_block(cblock, &_setup);

            if ((status < 0) || (status > INTERP_MIN_ERROR)) {
                // status < 0 is a bug; might happen if find_remappings() failed to indicate the next remap
                logRemap("executing block leftover items: %s status=%s  "
                         "remap_level=%d call_level=%d (failing)",
                         status < 0 ? "BUG" : "ERROR",
                         interp_status(status),
                         _setup.remap_level,
                         _setup.call_level);
                if (status < 0)
                    ERS("BUG - check find_remappings()!! status=%d nesting=%d",
                        status,
                        _setup.remap_level);
            } else {
                ...
                if (status != INTERP_EXIT) // Don't drop remap lev. at prog exit
                    CHP(leave_remap());
                logRemap("executing block leftover items complete, status=%s  "
                         "remap_level=%d call_level=%d tc=%d probe=%d input=%d "
                         "mdi_interrupt=%d  line=%d backtoline=%d",
                         interp_status(status),
                         _setup.remap_level,
                         _setup.call_level,
...
int Interp::find_remappings(block_pointer block, setup_pointer settings)
{
    ...
    if (is_user_defined_m_code(block, settings, 4)) {
        ...
        if (remap_in_progress("M0") || remap_in_progress("M1") ||
...
int Interp::exit()
{
    ...
    save_parameters(((file_name[0] == 0) ? RS274NGC_PARAMETER_FILE_NAME_DEFAULT
                                         : file_name),
                    _setup.parameters);
    reset();
...
void Interp::set_loglevel(int level)
{
    _setup.loggingLevel = level;
}
...
int Interp::init()
{
    ...
    if (iniFileName != NULL) {
        ...
        if (inifile.Open(iniFileName) == false) {
            ...
            inifile.Find(
                &_setup.tool_change_at_g30, "TOOL_CHANGE_AT_G30", "EMCIO");
            inifile.Find(
                &_setup.tool_change_quill_up, "TOOL_CHANGE_QUILL_UP", "EMCIO");
            inifile.Find(&_setup.tool_change_with_spindle_on,
            ...
            inifile.Find(
                &_setup.random_toolchanger, "RANDOM_TOOLCHANGER", "EMCIO");
            inifile.Find(&_setup.num_spindles, "SPINDLES", "TRAJ");

            inifile.Find(
                &_setup.tolerance_default, "G64_DEFAULT_TOLERANCE", "RS274NGC");
            inifile.Find(&_setup.naivecam_tolerance_default,
                         "G64_DEFAULT_NAIVETOLERANCE",
                         "RS274NGC");
            ...
            inifile.Find(&_setup.parameter_g73_peck_clearance,
                         "PARAMETER_G73_PECK_CLEARANCE",
                         "RS274NGC");
            inifile.Find(&_setup.parameter_g83_peck_clearance,
                         "PARAMETER_G83_PECK_CLEARANCE",
                         "RS274NGC");
            ...
            if ((inistring = inifile.Find("PROGRAM_PREFIX", "DISPLAY"))) {
                ...
                if (inifile.TildeExpansion(
                        *inistring, expandinistring, sizeof(expandinistring))) {
            ...
            if ((inistring = inifile.Find("SUBROUTINE_PATH", "RS274NGC"))) {
                ...
                while (1) {
                    ...
                    if (inifile.TildeExpansion(
                            nextdir, expandnextdir, sizeof(expandnextdir))) {
            ...
            if ((inistring = inifile.Find("TOPLEVEL", "PYTHON"))) {
                ...
                if (status != PLUGIN_OK) {
                    Error("Python plugin configure() failed, status = %d",
                          status);
                }
            }
            ...
            _setup.remaps.clear();
            while (
                (inistring = inifile.Find("REMAP", "RS274NGC", n, &lineno))) {
            ...
            r = inifile.Find(&_setup.center_arc_radius_tolerance_inch,
            ...
            if ((r != IniFile::ERR_NONE) && (r != IniFile::ERR_TAG_NOT_FOUND)) {
                Error(
                    "invalid [RS274NGC]CENTER_ARC_RADIUS_TOLERANCE_INCH in INI "
                    "file\n");
            }

            r = inifile.Find(&_setup.center_arc_radius_tolerance_mm,
                             MIN_CENTER_ARC_RADIUS_TOLERANCE_MM,
                             CENTER_ARC_RADIUS_TOLERANCE_MM,
                             "CENTER_ARC_RADIUS_TOLERANCE_MM",
                             "RS274NGC");
            if ((r != IniFile::ERR_NONE) && (r != IniFile::ERR_TAG_NOT_FOUND)) {
                Error("invalid [RS274NGC]CENTER_ARC_RADIUS_TOLERANCE_MM in INI "
                      "file\n");
            }
    ...
    _setup.ijk_distance_mode =
        DISTANCE_MODE::INCREMENTAL; // backwards compatibility
    ...
    if ((iniFileName != NULL) && _setup.init_once && PYUSABLE) {
        ...
        try {
            bp::object npmod =
                python_plugin->main_namespace[NAMEDPARAMS_MODULE];
            bp::dict predef_dict =
                bp::extract<bp::dict>(npmod.attr("__dict__"));
            ...
            PyErr_Clear();
            CHKS(unexpected,
                 "exception adding Python predefined named parameter: %s",
                 exception_msg.c_str());
        }
    ...
    const char *filename) //!< string: the name of the input NC-program file
{
    ...
    for (index = -1; index == -1;) { /* skip blank lines */
        CHKS((fgets(line, LINELEN, _setup.file_pointer) == NULL),
...
int Interp::read_inputs(setup_pointer settings)
{
    ...
    if (settings->input_flag) {
        ...
        if (settings->input_digital) { // we are checking for a digital input
            settings->parameters[5399] = GET_EXTERNAL_DIGITAL_INPUT(
                settings->input_index, (settings->parameters[5399] != 0.0));
        } else { // checking for analog input
            settings->parameters[5399] = GET_EXTERNAL_ANALOG_INPUT(
                settings->input_index, settings->parameters[5399]);
        }
...
int Interp::_read(const char *command) //!< may be NULL or a string to read
{
  ...
  if (_setup.input_flag) {
    ...
    if ((_setup.call_state > CS_NORMAL) &&
        ...
        ((eblock->o_type == O_call) || (eblock->o_type == M_98) ||
         (eblock->o_type == O_return) || (eblock->o_type == O_endsub) ||
         (eblock->o_type == M_99))) {
    ...
    if (command)
        logDebug("%s:[cmd]:|%s|", name, command);
    else
        logDebug("%s:|%s|", name, _setup.linetext);

    if ((read_status == INTERP_EXECUTE_FINISH) || (read_status == INTERP_OK)) {
        if (_setup.line_length != 0) {
            CHP(parse_line(
                _setup.blocktext, &(EXECUTING_BLOCK(_setup)), &_setup));
        }
        ...
        CHKS((_setup.skipping_o != NULL),
             "Failed to find sub 'O%s' before EOF",
             _setup.skipping_o);
    } else
        ERP(read_status);
    return read_status;
...
int Interp::unwind_call(int status,
                        const char *file,
                        int line,
                        const char *function)
{
    ...
    const char *filename) //!< name of parameter file to read
{
    ...
    while (feof(infile) == 0) {
        ...
        if (sscanf(line, "%d %lf", &variable, &value) == 2) {
            CHKS(((variable <= 0) || (variable >= RS274NGC_MAX_PARAMETERS)),
                            ...
                            const double parameters[]) //!< parameters to save
{
    ...
    fclose(outfile);
    std::string bakfile =
        std::string(filename) + RS274NGC_PARAMETER_FILE_BACKUP_SUFFIX;
                         ...
                         StateTag const &tag)
{
    ...
    g_codes[7] = tag.flags[GM_FLAG_FEED_INVERSE_TIME] ? G_93
                 : tag.flags[GM_FLAG_FEED_UPM]        ? G_94
                                                      : G_95;
    ...
    g_codes[11] = tag.flags[GM_FLAG_BLEND]        ? G_64
                  : tag.flags[GM_FLAG_EXACT_STOP] ? G_61_1
                                                  : G_61;
    ...
    m_codes[2] = !tag.flags[GM_FLAG_SPINDLE_ON]  ? 5
                 : tag.flags[GM_FLAG_SPINDLE_CW] ? 3
                                                 : 4;
    m_codes[3] = tag.fields[GM_FIELD_TOOLCHANGE];

    m_codes[4] = tag.flags[GM_FLAG_MIST]    ? 7
                 : tag.flags[GM_FLAG_FLOOD] ? -1
                                            : 9;
    m_codes[5] = tag.flags[GM_FLAG_FLOOD] ? 8 : -1;

    // Copied from write_m_codes
    if (tag.flags[GM_FLAG_FEED_OVERRIDE]) {
        if (tag.flags[GM_FLAG_SPEED_OVERRIDE])
            m_codes[6] = 48;
        else
            m_codes[6] = 50;
...
char *
Interp::error_text(int error_code,   //!< code number of error
...
char *
Interp::file_name(char *file_name, //!< string: to copy file name into
                  size_t max_size) //!< maximum number of characters to copy
{
...
char *
Interp::line_text(char *line_text, //!< string: to copy line into
                  size_t max_size) //!< maximum number of characters to copy
{
    size_t n;
    char *the_text;
...
char *
Interp::stack_name(int stack_index,     //!< index into stack of function names
...
int Interp::ini_load(const char *filename)
{
    ...
    if ((inistring = inifile.Find("PARAMETER_FILE", "RS274NGC"))) {
        if (strlen(*inistring) >= sizeof(parameter_file_name)) {
            logDebug("%s:[RS274NGC]PARAMETER_FILE is too long (max len %zu)",
                     filename,
                     sizeof(parameter_file_name) - 1);
        } else {
            strncpy(
                parameter_file_name, *inistring, sizeof(parameter_file_name));
    ...
    CHKS((strlen(parameter_file_name) == 0),
         _("Parameter file name is missing"));
...
FILE *Interp::find_ngc_file(setup_pointer settings,
                            const char *basename,
                            char *foundhere)
{
    FILE *newFP = NULL;
    ...
    snprintf(
        tmpFileName, sizeof(tmpFileName), "%s.ngc", exp_result.we_wordv[0]);
