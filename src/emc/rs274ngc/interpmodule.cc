...
static void setErrorMsg(Interp &interp, const char *s)
{
    ...
    strncpy(settings->stack[settings->stack_index], "Python", STACK_ENTRY_LEN);
...
static int get_task(Interp & /*i*/)
{
    return _task;
};
static const char *get_filename(Interp &i)
{
    return i._setup.filename;
};
static const char *get_linetext(Interp &i)
{
    return i._setup.linetext;
};

// those are exposed here because they look useful for regression testing
static bool __equal(double a, double b)
{
...
int (Interp::*execute_2)(const char *command,
                         int line_number) = &Interp::execute;
...
class InterpreterException : public std::exception
{
  ...
  public:
    InterpreterException(const std::string &error_message,
                         int line_number,
...
static int
wrap_interp_execute_2(Interp &interp, const char *command, int lineno)
{
    setup &_setup = interp._setup;
    block saved_block = _setup.blocks[0];
...
static inline void
set_spindle_mode(Interp &interp, int spindle, SPINDLE_MODE value)
{
...
static inline void set_parameter_g73_peck_clearance(Interp &interp,
                                                    double value)
{
...
static inline void set_parameter_g83_peck_clearance(Interp &interp,
                                                    double value)
{
...
BOOST_PYTHON_MODULE(interpreter)
{
    ...
    scope().attr("__doc__") = "Interpreter introspection\n";
    ...
    export_Internals();
    export_Block();
    bp::class_<InterpreterException> InterpreterExceptionClass(
        "InterpreterException", bp::init<std::string, int, std::string>());
    ...
    bp::class_<Interp, boost::noncopyable>("Interp", bp::no_init)
        ...
        .add_property(
            "arc_not_allowed", &get_arc_not_allowed, &set_arc_not_allowed)
        ...
        .add_property(
            "speed_override", &get_speed_override, &set_speed_override)
        .add_property(
            "toolchange_flag", &get_toolchange_flag, &set_toolchange_flag)
        .add_property("u_current", &get_u_current, &set_u_current)
        .add_property(
            "AA_axis_offset", &get_AA_axis_offset, &set_AA_axis_offset)
        .add_property("AA_current", &get_AA_current, &set_AA_current)
        .add_property(
            "AA_origin_offset", &get_AA_origin_offset, &set_AA_origin_offset)
        .add_property(
            "BB_axis_offset", &get_BB_axis_offset, &set_BB_axis_offset)
        .add_property("BB_current", &get_BB_current, &set_BB_current)
        .add_property(
            "BB_origin_offset", &get_BB_origin_offset, &set_BB_origin_offset)
        .add_property(
            "CC_axis_offset", &get_CC_axis_offset, &set_CC_axis_offset)
        .add_property("CC_current", &get_CC_current, &set_CC_current)
        .add_property(
            "CC_origin_offset", &get_CC_origin_offset, &set_CC_origin_offset)
        .add_property(
            "active_spindle", &get_active_spindle, &set_active_spindle)
        ...
        .add_property("cutter_comp_radius",
                      &get_cutter_comp_radius,
                      &set_cutter_comp_radius)
        ...
        .add_property(
            "origin_offset_x", &get_origin_offset_x, &set_origin_offset_x)
        .add_property(
            "origin_offset_y", &get_origin_offset_y, &set_origin_offset_y)
        .add_property(
            "origin_offset_z", &get_origin_offset_z, &set_origin_offset_z)
        ...
        .add_property(
            "ijk_distance_mode", &get_ijk_distance_mode, &set_ijk_distance_mode)
        ...
        .add_property("random_toolchanger",
                      &get_random_toolchanger,
                      &set_random_toolchanger)
        .add_property("remap_level", &get_remap_level, &set_remap_level)
        .add_property("retract_mode", &get_retract_mode, &set_retract_mode)
        .add_property(
            "selected_pocket", &get_selected_pocket, &set_selected_pocket)
        .add_property("selected_tool", &get_selected_tool, &set_selected_tool)
        .add_property(
            "sequence_number", &get_sequence_number, &set_sequence_number)
        .add_property(
            "speed_feed_mode", &get_speed_feed_mode, &set_speed_feed_mode)
        .add_property("spindle_mode", &get_spindle_mode, &set_spindle_mode)
        .add_property("num_spindles", &get_num_spindles, &set_num_spindles)
        .add_property(
            "spindle_turning", &get_spindle_turning, &set_spindle_turning)
        .add_property("stack_index", &get_stack_index, &set_stack_index)
        .add_property(
            "value_returned", &get_value_returned, &set_value_returned)

        .add_property("current_tool", &get_current_tool, &set_current_tool)
        .add_property("tool_change_at_g30",
                      &get_tool_change_at_g30,
                      &set_tool_change_at_g30)
        .add_property("tool_change_quill_up",
                      &get_tool_change_quill_up,
                      &set_tool_change_quill_up)
        .add_property("tool_change_with_spindle_on",
                      &get_tool_change_with_spindle_on,
                      &set_tool_change_with_spindle_on)
        .add_property("parameter_g73_peck_clearance",
                      &get_parameter_g73_peck_clearance,
                      &set_parameter_g73_peck_clearance)
        .add_property("parameter_g83_peck_clearance",
                      &get_parameter_g83_peck_clearance,
                      &set_parameter_g83_peck_clearance)
        ...
        .add_property(
            "speed",
            bp::make_function(spindle_speed_w(&spindle_speed_wrapper),
                              bp::with_custodian_and_ward_postcall<0, 1>()))
