...
static int iniLoad(const char *filename)
{
    ...
    if (emc_debug & EMC_DEBUG_CONFIG) {
        ...
        rcs_print("%s (%d) emcsvr: machine '%s'  version '%s'\n",
    ...
    while (fabs(etime() - start_time) < 10.0 &&
        ...
        if (NULL == emcCommandChannel) {
            rcs_print_debug(PRINT_NML_CONSTRUCTORS,
                            "emcCommandChannel==NULL, attempt to create\n");
            emcCommandChannel = new RCS_CMD_CHANNEL(
                emcFormat, "emcCommand", "emcsvr", emc_nmlfile);
        }
        if (NULL == emcStatusChannel) {
            rcs_print_debug(PRINT_NML_CONSTRUCTORS,
                            "emcStatusChannel==NULL, attempt to create\n");
            emcStatusChannel = new RCS_STAT_CHANNEL(
                emcFormat, "emcStatus", "emcsvr", emc_nmlfile);
        }
