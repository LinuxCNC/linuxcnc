...
*/
static char user_defined_fmt[MAX_M_DIRS]
                            [EMC_SYSTEM_CMD_LEN]; // ex: "dirname/M1%02d"
...
static void user_defined_add_m_code(int num, double arg1, double arg2)
{
    ...
    snprintf(
        system_cmd->string, sizeof(system_cmd->string), fmt, num, arg1, arg2);
    ...
    if ((inistring = inifile.Find("USER_M_PATH", "RS274NGC"))) {
        ...
        while (dct < MAX_M_DIRS) {
            ...
            if (strlen(nextdir) >= sizeof(mdir[dct])) {
                rcs_print(
                    "[RS274NGC]USER_M_PATH component (%s) too long (max len "
                    "%zu)\n",
                    nextdir,
                    sizeof(mdir[dct]));
                return -1;
    ...
    for (num = 0; num < USER_DEFINED_FUNCTION_NUM; num++) {
        for (dct = 0; dct < dmax; dct++) {
            ...
            if (inifile.TildeExpansion(
                    mdir[dct], expanddir, sizeof(expanddir))) {
                rcs_print(
                    "emcTaskInit: TildeExpansion failed for %s, ignoring\n",
                    mdir[dct]);
            }
            size_t ret =
                ...
                if (buf.st_mode & S_IXUSR) {
                    ...
                    if (ret >= sizeof(user_defined_fmt[0])) {
                        ...
                        if (emc_debug & EMC_DEBUG_CONFIG) {
                            rcs_print(
                                "emcTaskInit: adding user-defined function "
                                "%s\n",
                                path);
                        }
                    ...
                    if (emc_debug & EMC_DEBUG_CONFIG) {
                        rcs_print(
                            "emcTaskInit: user-defined function %s found, but "
                            "not executable, so ignoring\n",
                            path);
                    }
                }
            }
...
int emcTaskPlanInit()
{
    if (!pinterp) {
        ...
        if ((inistring = inifile.Find("INTERPRETER", "TASK"))) {
            ...
            if (!pinterp) {
                fprintf(stderr,
                        "failed to load [TASK]INTERPRETER (%s)\n",
                        *inistring);
                return -1;
            }
        }
...
int emcTaskPlanExecute(const char *command)
{
    ...
    if (command !=
        0) { // Command is 0 if in AUTO mode, non-null if in MDI mode.
