/*
  ...
  16-Aug-2004  FMP took out velocity clamping on STRAIGHT_TRAVERSE, as
  it should not be there, at least not in the same form as for STRAIGHT_FEED.
  16-Jun-2004  FMP added USE_SPINDLE_FORCE(), LOCK_SPINDLE_Z(), STOP(),
  all unimplemented, but discovered missing when writing straight-through 
  canonical file interpreter. Also replaced () with (void) in decls.
  11-May-2004  FMP put velocity clamping from STRAIGHT_FEED into
  STRAIGHT_TRAVERSE, as it was apparently left out.
  28-Apr-2004  FMP put SYSTEM, USER_DEFINED_FUNCTION, SET_MOTION_OUTPUT_BIT
  and related functions. These are the changes FMP made that clobbered MGS's
  previous changes, which PC restored for us. Thanks Paul.
  09-Jan-2004 MGS made a bunch of changes to STRAIGHT_FEED and 
  STRAIGHT_TRAVERSE to fix problems with improper feed rates.
  7-Jan-2004  FMP updated to reflect new 6-axis canonical interface 
  12-Dec-2001  FMP added setting of units to be based on traj->linearUnits,
  instead of hard-coding it here. The value of units, CANON_UNITS_INCHES or
  CANON_UNITS_MM, is guessed from the floating point number in
  traj->linearUnits. If this isn't one of the two well-known values, a
  CANON_ERROR is popped up and units are set to mm.
...
/* Used to indicate whether the current move is linear, angular, or a combination of both. */
...
void SET_FEED_RATE(double rate)
{
  ...
  /* If we are setting the feed rate,
     a velocity message should go out,
...
CANON_MOTION_MODE GET_MOTION_CONTROL_MODE(void)
{
...
void STOP_CUTTER_RADIUS_COMPENSATION(void)
{
  // nothing need be done here
}

void START_SPEED_FEED_SYNCH(void)
{
  // FIXME-- unimplemented
}

void STOP_SPEED_FEED_SYNCH(void)
{
  // FIXME-- unimplemented
}
...
/* Spindle Functions */
void SPINDLE_RETRACT_TRAVERSE(void)
{
  // FIXME-- unimplemented
}
...
void START_SPINDLE_CLOCKWISE(void)
{
...
void START_SPINDLE_COUNTERCLOCKWISE(void)
{
  EMC_SPINDLE_ON emc_spindle_on_msg;
...
void STOP_SPINDLE_TURNING(void)
{
...
void CHANGE_TOOL(int slot)
{
  ...
  if (HAVE_TOOL_CHANGE_POSITION) {
    linear_move_msg.end.tran = TOOL_CHANGE_POSITION.tran; // struct copy
    linear_move_msg.end.a = 0.0;
    linear_move_msg.end.b = 0.0;
    linear_move_msg.end.c = 0.0;
    interp_list.append(linear_move_msg);
    /* FIXME-- orient spindle command goes here. We don't yet have an
       NML message for this. */
  ...
  if (HAVE_TOOL_HOLDER_CLEAR) {
    linear_move_msg.end.tran = TOOL_HOLDER_CLEAR.tran; // struct copy
    linear_move_msg.end.a = 0.0;
    linear_move_msg.end.b = 0.0;
    linear_move_msg.end.c = 0.0;
    interp_list.append(linear_move_msg);
...
void DISABLE_FEED_OVERRIDE(void)
{
  // FIXME-- unimplemented
}

void DISABLE_SPEED_OVERRIDE(void)
{
  // FIXME-- unimplemented
}

void ENABLE_FEED_OVERRIDE(void)
{
  // FIXME-- unimplemented
}

void ENABLE_SPEED_OVERRIDE(void)
{
  // FIXME-- unimplemented
}
...
void SYSTEM(char *s)
{
  ...
  strncpy(system_cmd.string, s, EMC_SYSTEM_CMD_LEN);
  system_cmd.string[EMC_SYSTEM_CMD_LEN - 1] = 0;
...
void OPTIONAL_PROGRAM_STOP(void)
{
...
CANON_VECTOR GET_PROGRAM_ORIGIN(void)
{
  CANON_VECTOR origin;
...
CANON_UNITS GET_LENGTH_UNITS(void)
{
  return lengthUnits;
}

CANON_PLANE GET_PLANE(void)
{
  return activePlane;
}

double GET_TOOL_LENGTH_OFFSET(void)
{
...
void INIT_CANON(void)
{
  ...
  /*
    ...
    traj->linearUnits, which is already set from the .ini file in iniTraj().
    This is a floating point number, in user units per mm. We can compare this
    against known values and set the symbolic values accordingly. If it doesn't
    match, we have an error.
   */
...
CANON_POSITION GET_EXTERNAL_POSITION(void)
{
...
CANON_POSITION GET_EXTERNAL_PROBE_POSITION(void)
{
  CANON_POSITION position;
  EmcPose pos;
...
double GET_EXTERNAL_TRAVERSE_RATE(void)
{
  double traverse;
...
int GET_EXTERNAL_TOOL(void)
{
  return emcStatus->io.tool.toolInSpindle;
}

int GET_EXTERNAL_MIST(void)
{
  return emcStatus->io.coolant.mist;
}

int GET_EXTERNAL_FLOOD(void)
{
  return emcStatus->io.coolant.flood;
}

int GET_EXTERNAL_POCKET(void)
{
  return emcStatus->io.tool.toolPrepped;
}

double GET_EXTERNAL_SPEED(void)
{
  // speed is in RPMs everywhere
  return emcStatus->io.spindle.speed;
}

CANON_DIRECTION GET_EXTERNAL_SPINDLE(void)
{
...
int GET_EXTERNAL_TOOL_MAX(void)
{
...
CANON_MOTION_MODE GET_EXTERNAL_MOTION_CONTROL_MODE(void)
{
  return canonMotionMode;
}
...
int GET_EXTERNAL_TOOL_SLOT(void)
{
  return emcStatus->io.tool.toolInSpindle;
}

CANON_PLANE GET_EXTERNAL_PLANE(void)
{
  return activePlane;
}
