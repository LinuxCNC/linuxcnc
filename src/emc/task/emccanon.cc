...
/* Axis n (0 X to 8 W) is a length or an angle as its [AXIS_<letter>] TYPE
   says, and the move length is measured along the axes axisKindsMeasured()
   picks.  The defaults are the letters' own: A B C angles, XYZ the feed. */
...
/**
 * A straight move from canon.endPoint to the given position: how far each
 * axis goes, a distance below the smallest move motion can take, or on an
 * axis the machine lacks, counting as none; the axes that move; the axes
 * the move is measured along (axisKindsMeasured()) and its length along
 * them.  Sets canon.cartesian_move and canon.angular_move, whether a linear
 * and whether an angular axis moves.
 */
struct StraightSpan {
    double d[9];
    unsigned moving;
...
/**
 * Motion measures a line along X Y Z, else U V W, else A B C, in external
 * units, whatever the axes are.  The rates canon sends are along its own
 * length, span.length; this factor turns them into rates along motion's
 * length, so the move takes the time canon planned.  It is exactly 1 where
 * both measure along the same axes of one kind, which the default kinds
 * always do.
 */
...
// Rates along canon's length, made rates along motion's: see
// motionLengthRatio().
template <class M> static void toMotionLength(M &msg, double ratio)
{
    msg.vel *= ratio;
...
static double getMotionLengthRatio(double x, double y, double z,
                                   double a, double b, double c,
                                   double u, double v, double w)
{
    return motionLengthRatio(getStraightSpan(x, y, z, a, b, c, u, v, w));
}
...
static void flush_segments(void) {
    ...
    toMotionLength(*linearMoveMsg, getMotionLengthRatio(x, y, z, a, b, c, u, v, w));
                    ...
                    unsigned char probe_type)
{
    ...
    toMotionLength(*probeMsg, getMotionLengthRatio(x, y, z, a, b, c, u, v, w));
