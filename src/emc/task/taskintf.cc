/*
  ...
  Bridgeport NML interface functions for motion and IO
...
static unsigned long localMotionHeartbeat;
...
static int sendCommand(RCS_CMD_MSG *msg)
{
  ...
  if (emcIoStatus->echo_serial_number != emcIoCommandSerialNumber ||
    ...
    rcs_print_error("Command to IO level (%s:%s) timed out waiting for last command done. \n",
...
int emcIoInit()
{
  ...
  if (0 != (retval = iniSpindle(EMC_INIFILE)))
  {
    return -1;
  }

  if (0 != (retval = iniTool(EMC_INIFILE))) {
    return -1;
  }

  // send init command to emcio
  return forceCommand(&ioInitMsg);
}
