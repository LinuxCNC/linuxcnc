                            ...
                            int absolute_encoder)
{
    ...
    if (absolute_encoder) {
        ...
        default:
            fprintf(stderr,
                    "Unknown option for absolute_encoder <%d>",
                    absolute_encoder);
            break;
        }
    }
    ...
    if (emc_debug & EMC_DEBUG_CONFIG) {
        rcs_print(
            "%s(%d, %.4f, %.4f, %.4f, %.4f, %.4f, %d, %d, %d, %d, %d) returned "
            "%d\n",
            __FUNCTION__,
            joint,
            home,
...
int emcJointUpdate(EMC_JOINT_STAT stat[], int numJoints)
{
   ...
   happen */
        ...
        stat[joint_num].overrideLimits =
            !!(emcmotStatus.overrideLimitMask); // one
                                                // for
                                                // all
...
{
#ifdef ISNAN_TRAP
    if (std::isnan(home.tran.x) || std::isnan(home.tran.y) ||
        std::isnan(home.tran.z) || std::isnan(home.a) || std::isnan(home.b) ||
        std::isnan(home.c) || std::isnan(home.u) || std::isnan(home.v) ||
        std::isnan(home.w)) {
    ...
    if (emc_debug & EMC_DEBUG_CONFIG) {
        rcs_print(
            "%s(%.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f) returned "
            "%d\n",
            __FUNCTION__,
            home.tran.x,
            home.tran.y,
...
int emcTrajLinearMove(const EmcPose &end,
                      int type,
                      double vel,
                      double ini_maxvel,
                      double acc,
...
{
#ifdef ISNAN_TRAP
    if (std::isnan(end.tran.x) || std::isnan(end.tran.y) ||
        std::isnan(end.tran.z) || std::isnan(end.a) || std::isnan(end.b) ||
        std::isnan(end.c) || std::isnan(end.u) || std::isnan(end.v) ||
        std::isnan(end.w)) {
...
int emcTrajCircularMove(const EmcPose &end,
                        ...
                        double ini_maxjerk)
{
    ...
    if (std::isnan(end.tran.x) || std::isnan(end.tran.y) ||
        std::isnan(end.tran.z) || std::isnan(end.a) || std::isnan(end.b) ||
        std::isnan(end.c) || std::isnan(end.u) || std::isnan(end.v) ||
        std::isnan(end.w) || std::isnan(center.x) || std::isnan(center.y) ||
...
int emcTrajProbe(const EmcPose &pos,
                 int type,
                 double vel,
                 double ini_maxvel,
                 double acc,
...
{
#ifdef ISNAN_TRAP
    if (std::isnan(pos.tran.x) || std::isnan(pos.tran.y) ||
        std::isnan(pos.tran.z) || std::isnan(pos.a) || std::isnan(pos.b) ||
        std::isnan(pos.c) || std::isnan(pos.u) || std::isnan(pos.v) ||
        std::isnan(pos.w)) {
...
int emcTrajRigidTap(const EmcPose &pos,
                    double vel,
                    double ini_maxvel,
                    double acc,
                    double ini_maxjerk,
...
/*! \todo Another #if 0 */
    ...
    if (EMC_DEBUG_MOTION_TIME & emc_debug) {
        if (stat->status == RCS_STATUS::DONE &&
            last_status != RCS_STATUS::DONE && stat->id != last_id_printed) {
            rcs_print("Motion id %d took %f seconds.\n",
                      last_id,
                      etime() - last_id_time);
...
int emcPositionLoad()
{
    ...
    for (int i = 0; i < EMCMOT_MAX_JOINTS; i++) {
        ...
        if (r != 1) {
            fclose(f);
            rcs_print(
                "%s: failed to load joint %d position from %s, ignoring\n",
                __FUNCTION__,
                i,
                *posfile);
            return -1;
    ...
    for (int i = 0; i < EMCMOT_MAX_JOINTS; i++) {
        if (emcJointSetMotorOffset(i, -positions[i]) != 0) {
            rcs_print("%s: failed to set joint %d position (%.6f) from %s, "
                      "ignoring\n",
                      __FUNCTION__,
                      i,
                      positions[i],
...
int emcMotionUpdate(EMC_MOTION_STAT *stat)
{
    ...
    localMotionCommandType =
        emcmotStatus.commandEcho; /*! \todo FIXME-- not NML one! */
