...
int emcErrorBufferOKtoWrite(int space, const char *caller)
{
    ...
    if (etime() >= send_errorchan_timout) {
        if (emc_debug & EMC_DEBUG_TASK_ISSUE) {
            rcs_print("timeout waiting for error channel to drain, caller=`%s' "
                      "request=%d\n",
                      caller,
                      space);
        }
...
static void mdi_execute_hook(void)
{
    ...
    if (interp_list.len() == 0 && emcTaskCommand == 0 &&
        ...
        emcStatus->io.status == RCS_STATUS::DONE && !mdi_execute_wait &&
        !mdi_execute_next) {

        // finished. Check for dequeuing of queued MDI command is done in emcTaskPlan().
        if (emc_debug & EMC_DEBUG_TASK_ISSUE)
            rcs_print(
                "mdi_execute_hook: MDI command '%s' done (remaining: %d)\n",
...
static int emcTaskPlan(void)
{
    ...
    case EMC_TASK_STATE::ESTOP_RESET:
        ...
        case EMC_TASK_MODE::MDI:
            ...
            default:
                emcOperatorError(
                    _("command (%s) cannot be executed until the machine is "
                      "out of E-stop and turned on"),
            ...
            case EMC_TASK_INTERP::IDLE: // ON, AUTO, IDLE
		       ...
		       step */
                    ...
                    if (retval != 0)
                        break;
                    emcTrajPause();
                    if (emcStatus->task.interpState !=
                        EMC_TASK_INTERP::PAUSED) {
                ...
                case EMC_TOOL_LOAD_TOOL_TABLE_TYPE: {
                    // send to IO
                    emcTaskQueueCommand(
                        std::make_unique<EMC_TOOL_LOAD_TOOL_TABLE>(
                            *static_cast<EMC_TOOL_LOAD_TOOL_TABLE *>(
                                emcCommand)));
                    // signify no more reading
                    emcTaskPlanSetWait();
                    // then resynch interpreter
                ...
                default:
                    ...
                    emcOperatorError(
                        _("can't do that (%s) in auto mode with the "
                          "interpreter idle"),
                        emc_symbol_lookup(type));
                    retval = -1;
                    break;
        ...
        case EMC_TASK_MODE::MDI: // ON, MDI
            ...
            case EMC_TASK_PLAN_EXECUTE_TYPE:
                ...
                if ((mdi_execute_queue.len() == 0) &&
                    ...
                    rtapi_strlcpy(
                        cmd->command,
                        static_cast<EMC_TASK_PLAN_EXECUTE *>(emcCommand)
                            ->command,
                        sizeof(cmd->command));
...
static int emcTaskIssueCommand(NMLmsg *cmd)
{
    int retval = 0;
    int execRetval = 0;
    static char remote_tmpfilename
        [LINELEN]; // path to temporary file received from remote process
    ...
    case EMC_OPERATOR_DISPLAY_TYPE:
        retval =
            emcOperatorDisplay("%s", ((EMC_OPERATOR_DISPLAY *)cmd)->display);
        break;
    ...
    case EMC_JOG_STOP_TYPE:
        ...
        retval =
            emcJogStop(jog_stop_msg->joint_or_axis, jog_stop_msg->jjogmode);
        break;
    ...
    case EMC_JOINT_SET_HOMING_PARAMS_TYPE:
        ...
        retval = emcJointSetHomingParams(
            set_homing_params_msg->joint,
    ...
    case EMC_JOINT_SET_FERROR_TYPE:
        ...
        retval =
            emcJointSetFerror(set_ferror_msg->joint, set_ferror_msg->ferror);
        break;
    ...
    case EMC_TRAJ_SET_FO_ENABLE_TYPE:
        retval = emcTrajSetFOEnable(((EMC_TRAJ_SET_FO_ENABLE *)cmd)
                                        ->mode); // feed override enable/disable
        break;

    case EMC_TRAJ_SET_FH_ENABLE_TYPE:
        retval = emcTrajSetFHEnable(
            ((EMC_TRAJ_SET_FH_ENABLE *)cmd)->mode); //feed hold enable/disable
        break;

    case EMC_TRAJ_SET_SO_ENABLE_TYPE:
        retval = emcTrajSetSOEnable(
            ((EMC_TRAJ_SET_SO_ENABLE *)cmd)
                ->mode); //spindle speed override enable/disable
        break;
    ...
    case EMC_TRAJ_SET_ACCELERATION_TYPE:
        ...
        retval =
            emcTrajSetAcceleration(emcTrajSetAccelerationMsg->acceleration);
        break;
    ...
    case EMC_TRAJ_SET_SPINDLESYNC_TYPE:
        ...
        retval =
            emcTrajSetSpindleSync(emcTrajSetSpindlesyncMsg->spindle,
    ...
    case EMC_AUX_INPUT_WAIT_TYPE:
        ...
        if (emcAuxInputWaitMsg->timeout ==
            WAIT_MODE_IMMEDIATE) { //nothing to do, CANON will get the needed value when asked by the interp
            ...
            emcAuxInputWaitType =
                emcAuxInputWaitMsg
                    ->wait_type; // remember what we are waiting for
            emcAuxInputWaitIndex =
                emcAuxInputWaitMsg->index; // remember the input to look at
    ...
    case EMC_SPINDLE_WAIT_ORIENT_COMPLETE_TYPE:
        wait_spindle_orient_complete_msg =
            (EMC_SPINDLE_WAIT_ORIENT_COMPLETE *)cmd;
        taskExecDelayTimeout =
            etime() + wait_spindle_orient_complete_msg->timeout;
        break;
    ...
    case EMC_TASK_SET_MODE_TYPE:
        ...
        if (emcStatus->task.mode == EMC_TASK_MODE::AUTO &&
            ...
            emcOperatorError(
                _("Can't switch mode while mode is AUTO and interpreter is not "
                  "IDLE"));
    ...
    case EMC_TASK_PLAN_OPEN_TYPE:
        ...
        if (open_msg->remote_filesize > 0) {
            ...
            if (open_msg->remote_buffersize == 0) {
                rcs_print_error(
                    "EMC_TASK_PLAN_OPEN received empty chunk from remote "
                    "process.\n");
                retval = -1;
                break;
            }
            ...
            if (received == 0) {
                /* create new tempfile */
                snprintf(remote_tmpfilename,
                         LINELEN,
                         EMC2_TMP_DIR "/%s.XXXXXX",
                         basename(open_msg->file));
                if ((fd = mkstemp(remote_tmpfilename)) < 0) {
                    rcs_print_error("mkstemp(%s) error: %s",
                                    remote_tmpfilename,
                                    strerror(errno));
                    retval = -1;
                    break;
            ...
            size_t bytes_written =
                write(fd, open_msg->remote_buffer, open_msg->remote_buffersize);
            if (bytes_written < open_msg->remote_buffersize) {
                rcs_print_error("fwrite(%s) error: %s",
                                remote_tmpfilename,
                                strerror(errno));
                retval = -1;
                break;
    ...
    case EMC_TASK_PLAN_EXECUTE_TYPE:
        ...
        if (!all_homed() &&
            !no_force_homing) { //!no_force_homing = force homing before MDI
        ...
        if (execute_msg->command[0] != 0) {
            ...
            case INTERP_ERROR:
                ...
                emcAbortCleanup(EMC_ABORT::INTERPRETER_ERROR_MDI,
                                "interpreter error during MDI");
                retval = -1;
                break;
    ...
    case EMC_TASK_PLAN_RUN_TYPE:
        if (!all_homed() &&
            !no_force_homing) { //!no_force_homing = force homing before Auto
...
static int emcTaskExecute(void)
{
            ...
            if (0 == emcTaskCommand) {
                ...
                if (0 != emcTaskCommand) {
                    ...
                    if (emcStatus->motion.traj.queueFull) {
                        ...
                        emcStatus->task.execState =
                            emcTaskCheckPreconditions(emcTaskCommand.get());
                    }
                }
            } else {
                ...
                if (0 != emcTaskIssueCommand(emcTaskCommand.get())) {
                    ...
                    emcStatus->task.execState =
                        emcTaskCheckPostconditions(emcTaskCommand.get());
                    emcTaskEager = 1;
                }
            ...
            case EMCMOT_ORIENT_FAULTED:
                ...
                for (int n = 0; n < emcStatus->motion.traj.spindles; n++) {
                    if (emcStatus->motion.spindle[n].orient_fault)
                        emcOperatorError(
                            "wait for orient complete: FAULTED code=%d",
    ...
    case EMC_TASK_EXEC::WAITING_FOR_SYSTEM_CMD:
        ...
        if (emcSystemCmdPid != pid) {
            ...
            if (emc_debug & EMC_DEBUG_TASK_ISSUE) {
                rcs_print(
                    "emcSystemCmd: error waiting for system command %d, we got "
                    "%d\n",
                    emcSystemCmdPid,
                    pid);
            }
        ...
        if (WIFEXITED(status)) {
            if (0 == WEXITSTATUS(status)) {
                ...
                if (emc_debug & EMC_DEBUG_TASK_ISSUE) {
                    rcs_print(
                        "emcSystemCmd: system command %d exited abnormally "
                        "with value %d\n",
                        emcSystemCmdPid,
                        WEXITSTATUS(status));
                }
...
static int iniLoad(const char *filename)
{
    ...
    if (emc_debug & EMC_DEBUG_CONFIG) {
        ...
        rcs_print("%s (%d) task: machine '%s'  version '%s'\n",
    ...
    saveInt =
        emc_task_interp_max_len; //remember default or previously set value
...
int main(int argc, char *argv[])
{
	      ...
	      and in emcTaskIssueCommand() */
            ...
            if (emcStatus->io.status == RCS_STATUS::ERROR) {
                ...
                if (emc_debug & EMC_DEBUG_RCS) {
                    rcs_print(
                        "io.status=RCS_STATUS::ERROR, fault=%d reason=%d\n",
                        emcStatus->io.fault,
                        emcStatus->io.reason);
                }
                if (emcStatus->io.reason < 0) {
    ...
    rcs_print(
        "task: %u cycles, min=%.6f, max=%.6f, avg=%.6f, %u latency excursions "
        "(> %dx expected cycle time of %.6fs)\n",
