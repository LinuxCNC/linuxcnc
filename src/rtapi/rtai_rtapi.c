...
long long int rtapi_get_time(void)
{
  ...
  /* This will only work if the kernel is compiled for a 586TSC (Pentium-Classic)
     or higher. Should return a true time in nSec or zero if no TSC. */
...
{
  /* RTAI has LOTS of different priorities - RT_LOWEST_PRIORITY
     is 0x3FFFFFFF!  I don't want such ugly numbers, and we only
		   ...
		   unsigned long int stacksize, int uses_fp)
{
  ...
  retval =
    rt_task_init_cpuid(ostask_array[task_id], wrapper, task_id, stacksize,
		       prio, uses_fp, 0 /*signal */ , 0 /*cpu id */ );
  if (retval != 0) {
