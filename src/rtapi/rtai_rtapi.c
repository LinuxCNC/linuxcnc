...
void cleanup_module(void)
{
    ...
    for (n = 1; n <= RTAPI_MAX_MODULES; n++) {
        if (module_array[n].state == REALTIME) {
            rtapi_print_msg(
                RTAPI_MSG_WARN,
                "RTAPI: WARNING: module '%s' (ID: %02d) did not call "
                "rtapi_exit()\n",
                module_array[n].name,
                n);
            module_delete(n);
    ...
    for (n = 1; n <= RTAPI_MAX_FIFOS; n++) {
        if (fifo_array[n].state != UNUSED) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "RTAPI: ERROR: FIFO %02d not deleted\n", n);
        }
    }
    for (n = 1; n <= RTAPI_MAX_SEMS; n++) {
        while (sem_array[n].users > 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "RTAPI: ERROR: semaphore %02d not deleted\n", n);
        }
    }
    ...
    for (n = 1; n <= RTAPI_MAX_TASKS; n++) {
        if (task_array[n].state != EMPTY) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "RTAPI: ERROR: task %02d not deleted\n", n);
            /* probably un-recoverable, but try anyway */
            rtapi_task_pause(n);
            task_delete(n);
        }
...
int rtapi_init(const char *modname)
{
    ...
    if (n > RTAPI_MAX_MODULES) {
        ...
        rtapi_mutex_give(&(rtapi_data->mutex));
        rtapi_print_msg(
            RTAPI_MSG_ERR, "RTAPI: ERROR: reached module limit %d\n", n);
        return -EMFILE;
    }
...
static int module_delete(int module_id)
{
    ...
    for (n = 1; n <= RTAPI_MAX_IRQS; n++) {
        if (irq_array[n].owner == module_id) {
            rtapi_print_msg(
                RTAPI_MSG_WARN,
                "RTAPI: WARNING: module '%s' failed to delete handler for IRQ "
                "%d\n",
                module->name,
                irq_array[n].irq_num);
            irq_delete(irq_array[n].irq_num);
...
long int rtapi_clock_set_period(long int nsecs)
{
    ...
    rtapi_print_msg(
        RTAPI_MSG_DBG,
        "RTAPI: clock_set_period requested: %ld  actual: %ld  counts "
...
static int
rtapi_trap_handler(int vec, int signo, struct pt_regs *regs, void *task)
{
    ...
    /* request to handle traps in the new task */
    {
        ...
        for (v = 0; v < RTAI_NR_TRAPS; v++)
            rt_set_task_trap_handler(
                ostask_array[task_id], v, rtapi_trap_handler);
    }
...
int rtapi_task_start(int task_id, unsigned long int period_nsec)
{
    ...
    /* start the task */
    retval = rt_task_make_periodic(
        ostask_array[task_id], rt_get_time() + period_counts, period_counts);
    if (retval != 0) {
        return -EINVAL;
    }
    /* ok, task is started */
    ...
    if (result != 0) {
        ...
        if (error_printed < 10) {
            ...
            if (result == RTE_TMROVRN) {
                ...
                rtapi_print_msg(
                    error_printed == 0 ? RTAPI_MSG_ERR : RTAPI_MSG_WARN,
                    "RTAPI: ERROR: rt_task_wait_period() returned RTE_UNBLKD "
                    "(%d).\n",
                    result);
            } else
#endif
...
static int sem_delete(int sem_id, int module_id)
{
    ...
    rtapi_data->sem_count--;
    rtapi_print_msg(RTAPI_MSG_DBG,
                    "RTAPI: sem %02d deleted by module %02d\n",
                    sem_id,
                    module_id);
    return 0;
}
...
int rtapi_fifo_new(int key, int module_id, unsigned long int size, char mode)
{
    ...
    if (mode == 'R') {
        ...
        rtapi_print_msg(
            RTAPI_MSG_DBG,
            "RTAPI: fifo %02d created for read by module %02d, key: %d, size: "
            "%ld\n",
            fifo_id,
            module_id,
            key,
        ...
        rtapi_print_msg(
            RTAPI_MSG_DBG,
            "RTAPI: fifo %02d created for write by module %02d, key: %d, size: "
            "%ld\n",
            fifo_id,
            module_id,
            key,
...
static int irq_delete(unsigned int irq_num)
{
    ...
    rtapi_data->irq_count--;
    rtapi_print_msg(
        RTAPI_MSG_DBG, "RTAPI: handler for IRQ %d deleted\n", irq_num);
    return 0;
}
