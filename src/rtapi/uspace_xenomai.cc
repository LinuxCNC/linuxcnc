...
struct XenomaiApp : RtapiApp {
    XenomaiApp() : RtapiApp(SCHED_FIFO) { pthread_once(&key_once, init_key); }
    ...
    int task_start(int task_id, unsigned long period_nsec)
    {
        ...
        CPU_SET(nprocs - 1,
                &cpuset); // assumes processor numbers are contiguous
    ...
    int task_pause(int task_id) { return -ENOSYS; }

    int task_resume(int task_id) { return -ENOSYS; }
    ...
    void wait()
    {
        ...
        rtapi_timespec_advance(task->nextstart,
                               task->nextstart,
