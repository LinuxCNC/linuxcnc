    ...
    char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    for (n = 1; n <= RTAPI_MAX_MODULES; n++) {
        if (module_array[n].state != NO_MODULE) {
            ...
            case REALTIME: state_str = "RT  "; break;
            case USERSPACE: state_str = "USER"; break;
    ...
    char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    for (n = 1; n <= RTAPI_MAX_TASKS; n++) {
        if (task_array[n].state != EMPTY) {
            ...
            case PERIODIC: state_str = "PERIODIC"; break;
            case FREERUN: state_str = "FREE RUN"; break;
            case ENDED: state_str = "ENDED   "; break;
    ...
    char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    for (n = 1; n <= RTAPI_MAX_FIFOS; n++) {
        if (fifo_array[n].state != UNUSED) {
            ...
            case HAS_READER: state_str = "R-"; break;
            case HAS_WRITER: state_str = "-W"; break;
            case HAS_BOTH: state_str = "RW"; break;
            default: state_str = "UNKNOWN "; break;
            }
