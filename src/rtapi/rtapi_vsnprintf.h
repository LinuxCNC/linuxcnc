...
static char *fnumber(char *buf, char *end, double num)
{
    ...
    if (du.ieee.exponent == 0x7ff) {
        ...
        mantissa =
            ((unsigned long long)du.ieee.mantissa0 << 32) + du.ieee.mantissa1;
        if (mantissa == 0) {
            exponent = 0;
        } else {
        ...
        // normal
        exponent = du.ieee.exponent - IEEE754_DOUBLE_BIAS;
        mantissa = ((unsigned long long)du.ieee.mantissa0 << 32) +
                   du.ieee.mantissa1 + (1ull << 52);
    }
                    ...
                    va_list args)
{
    ...
    repeat:
        ...
        if (qualifier == 'l' ||
            (qualifier == 'z' && sizeof(size_t) == sizeof(long))) {
