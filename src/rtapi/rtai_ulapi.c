...
int rtapi_exit(int module_id)
{
    ...
    /* update module data */
    rtapi_print_msg(RTAPI_MSG_DBG,
                    "RTAPI: module %02d exited, name = '%s'\n",
                    module_id,
                    module->name);
...
int rtapi_shmem_new(int key, int module_id, unsigned long int size)
{
    ...
    if ((key == 0) || (key == RTAPI_KEY)) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "RTAPI: ERROR: bad shmem key: %d\n", key);
        return -EINVAL;
    }
    /* get the mutex */
    rtapi_mutex_get(&(rtapi_data->mutex));
    ...
    if ((module_id < 1) || (module_id > RTAPI_MAX_MODULES)) {
        rtapi_mutex_give(&(rtapi_data->mutex));
        rtapi_print_msg(
            RTAPI_MSG_ERR, "RTAPI: ERROR: bad module ID: %d\n", module_id);
        return -EINVAL;
    }
    ...
    if (n > RTAPI_MAX_SHMEMS) {
        ...
        rtapi_mutex_give(&(rtapi_data->mutex));
        rtapi_print_msg(
            RTAPI_MSG_ERR, "RTAPI: ERROR: reached shmem limit %d\n", n);
        return -EMFILE;
    }
    ...
    if (shmem_addr_array[shmem_id] == NULL ||
        ...
        rtapi_mutex_give(&(rtapi_data->mutex));
        rtapi_print_msg(
            RTAPI_MSG_ERR, "RTAPI: ERROR: could not create shmem %d\n", n);
        return -ENOMEM;
    }
...
static void check_memlock_limit(const char *where)
{
    ...
    rtapi_print_msg(
        ...
        (unsigned long)lim.rlim_cur / 1024,
        RECOMMENDED / 1024,
        where);
    return;
}
