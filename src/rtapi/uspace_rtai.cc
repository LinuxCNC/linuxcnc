...
struct RtaiApp : RtapiApp {
    RtaiApp() : RtapiApp(SCHED_FIFO) { pthread_once(&key_once, init_key); }

    RtaiTask *do_task_new() { return new RtaiTask; }
    ...
    int task_start(int task_id, unsigned long period_nsec)
    {
        ...
        if ((ret = pthread_attr_setinheritsched(&attr,
            ...
            return -ret;
        if ((ret = pthread_create(&task->thr,
                                  &attr,
                                  &wrapper,
    ...
    static void *wrapper(void *arg)
    {
        ...
        rt_task_make_periodic_relative_ns(
            task->rt_task, task->period, task->period);
    ...
    long long do_get_time() { return rt_get_cpu_time_ns(); }

    void do_delay(long ns) { rt_sleep(nano2count(ns)); }

    int prio_highest() const { return RT_SCHED_HIGHEST_PRIORITY; }

    int prio_lowest() const { return RT_SCHED_LOWEST_PRIORITY; }
};
