...
static int
do_newinst_cmd(const string &type, const string &name, const string &arg)
{
    ...
    if (!comp) {
        rtapi_print_msg(
            RTAPI_MSG_ERR, "newinst: component %s not found\n", type.c_str());
        return -1;
    }
...
static int do_comp_args(void *module, vector<string> args)
{
    for (unsigned i = 1; i < args.size(); i++) {
        ...
        if (idx == string::npos) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "Invalid parameter `%s'\n", s.c_str());
            return -1;
        }
        ...
        if (!item) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "Unknown parameter `%s'\n", s.c_str());
            return -1;
        }
        char **item_type =
        ...
        int *max_size_ptr =
            DLSYM<int *>(module, "rtapi_info_size_" + param_name);
...
static int master(int fd, const vector<string> &args)
{
    ...
    if ((result = pthread_create(
             &queue_thread, nullptr, &queue_function, nullptr)) != 0) {
        errno = result;
...
static std::string _get_fifo_path()
{
    std::string s;
    if (getenv("RTAPI_FIFO_PATH"))
        ...
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "rtapi_app: RTAPI_FIFO_PATH and HOME are unset.  rtapi fifo "
            "creation is unsafe.");
        return NULL;
    }
...
static const char *get_fifo_path()
{
    ...
    if (getuid() == 0) {
        ...
        if (fallback_uid == 0) {
            ...
            fprintf(stderr,
                    ...
                    "    sudo env RTAPI_UID=`id -u` "
                    "RTAPI_FIFO_PATH=$HOME/.rtapi_fifo gdb " EMC2_BIN_DIR
                    "/rtapi_app\n");
            exit(1);
        }
...
become_master:
    ...
    int result =
        ::bind(fd, (sockaddr *)&addr, len + sizeof(addr.sun_family) + 1);
    ...
    if (result == 0) {
        ...
        for (int i = 0; i < 3 || (t1.tv_sec < 3 + t0.tv_sec); i++) {
            result = connect(
                fd, (sockaddr *)&addr, len + sizeof(addr.sun_family) + 1);
            if (result == 0)
                break;
            if (i == 0)
...
rtapi_task::rtapi_task()
    : magic{}, id{}, owner{}, uses_fp{}, stacksize{}, prio{}, period{},
      nextstart{}, ratio{}, pll_correction{}, pll_correction_limit{}, arg{},
      taskcode{}
...
struct Posix : RtapiApp {
    ...
    void wait();
    struct rtapi_task *do_task_new() { return new PosixTask; }
    ...
    static void init_key(void) { pthread_key_create(&key, NULL); }

    static pthread_once_t lock_once;
    static pthread_mutex_t thread_lock;
    static void init_lock(void) { pthread_mutex_init(&thread_lock, NULL); }
...
static void signal_handler(int sig, siginfo_t * /*si*/, void * /*uctx*/)
{
    ...
    default: // pretty bad
        rtapi_print_msg(
            RTAPI_MSG_ERR, "rtapi_app: caught signal %d - dumping core\n", sig);
...
static int harden_rt()
{
    ...
    if (setrlimit(RLIMIT_RTPRIO, &unlimited) < 0) {
        rtapi_print_msg(
            RTAPI_MSG_WARN, "setrlimit(RTLIMIT_RTPRIO): %s\n", strerror(errno));
        return -errno;
    }
    ...
    if (prctl(PR_SET_DUMPABLE, 1) < 0)
        rtapi_print_msg(
            RTAPI_MSG_WARN,
            "prctl(PR_SET_DUMPABLE) failed: no core dumps will be created - %d "
            "- %s\n",
            errno,
            strerror(errno));
#endif /* __linux__ */
    ...
    if (fd < 0) {
        ...
        if (r != 4) {
            rtapi_print_msg(RTAPI_MSG_WARN,
                            "failed to write to /dev/cpu_dma_latency: %s\n",
                            strerror(errno));
        }
...
int RtapiApp::allocate_task_id()
{
    for (int n = 0; n < MAX_TASKS; n++) {
        ...
        if (__sync_bool_compare_and_swap(
                taskptr, (rtapi_task *)0, TASK_MAGIC_INIT))
            return n;
    }
    return -ENOSPC;
}

int RtapiApp::task_new(void (*taskcode)(void *),
                       void *arg,
                       int prio,
                       int owner,
...
static int find_rt_cpu_number()
{
    ...
    // 2.19 and older.
    for (long i = 0; i < top_probe && i < CPU_SETSIZE; i++)
        CPU_SET(i, &cpuset);
...
int Posix::task_start(int task_id, unsigned long int period_nsec)
{
    ...
    if (period_nsec < (unsigned long)period)
        period_nsec = (unsigned long)period;
    ...
    if (nprocs > 1) {
        ...
        if (rt_cpu_number != -1) {
            ...
            if ((ret = pthread_attr_setaffinity_np(
                     &attr, sizeof(cpuset), &cpuset)) != 0)
                return -ret;
        }
    }
    if ((ret = pthread_create(
             &task->thr, &attr, &wrapper, reinterpret_cast<void *>(task))) != 0)
        return -ret;
...
void Posix::wait()
{
    ...
    struct rtapi_task *task =
        reinterpret_cast<rtapi_task *>(pthread_getspecific(key));
    rtapi_timespec_advance(
        task->nextstart, task->nextstart, task->period + task->pll_correction);
    struct timespec now;
    clock_gettime(RTAPI_CLOCK, &now);
    if (rtapi_timespec_less(task->nextstart, now)) {
        ...
        int res = rtapi_clock_nanosleep(
            RTAPI_CLOCK, TIMER_ABSTIME, &task->nextstart, nullptr, &now);
        if (res < 0)
...
int rtapi_task_new(void (*taskcode)(void *),
                   void *arg,
                   int prio,
                   int owner,
                   unsigned long int stacksize,
