...
| 13 | `TASK_PLAN_EXECUTE` multi-level MDI | **CONFIRMED BUG (real, unlike #10/#11).** `Interp::_execute` (rs274ngc_pre.cc:290) runs a `while(MDImode && call_level)` loop for an MDI o-word sub; when a block inside the sub is a queue-buster (probe, M66, dwell, tool change) it returns `INTERP_EXECUTE_FINISH` **mid-sub** (sets `mdi_interrupt`) and expects the caller to drain the queue and call `execute()` again. gomc's `executeMDI` calls `ExecuteString` once and routes `EXECUTE_FINISH` into its `default` (done) case — the rest of the sub after the queue-buster **never runs**. Hits MDI probing/measurement macros (`o<probe_z> call`). Fix: in `finishMDI`, if `interp.CallLevel() > 0` after the drain, re-run `interp.Execute()` (loop via mdiDoneCmd on repeat `EXECUTE_FINISH`) until the call level unwinds — mirrors C++ / the AUTO path. `CallLevel()==0` at `EXECUTE_FINISH` = a single queue-buster, no continuation. | `commands.go` `executeMDI`/`finishMDI` |
