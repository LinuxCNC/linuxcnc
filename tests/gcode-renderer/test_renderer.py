...
class ToolOffsetRecords(unittest.TestCase):
    """What a G43/G43.1/G49 leaves behind: nothing drawn, so the record is
    the only way back to the offset, and the row it takes effect at addresses
    it."""

    def test_an_offset_is_addressed_by_row_and_not_by_line(self):
        pg = parse(programs.offsets_in_a_loop(), axis_mask=0b111,
                   want_axis_positions=True).program_geometry
        records = pg.tool_offsets
        self.assertEqual(len(records), 2)
        (row_a, off_a), (row_b, off_b) = records
        self.assertLess(row_a, row_b)
        self.assertAlmostEqual(off_a[2], 0.5, places=6)
        self.assertAlmostEqual(off_b[2], 0.75, places=6)

        # One line, two passes: the line column cannot tell them apart.
        self.assertEqual(int(pg.lines[row_a]), int(pg.lines[row_b]))

        # Each row resolves to its own pass's offset, and the tip comes back
        # by adding it to the axes.
        self.assertEqual(pg.tool_offset_at(row_a), off_a)
        self.assertEqual(pg.tool_offset_at(row_b - 1), off_a)
        self.assertEqual(pg.tool_offset_at(row_b), off_b)
        self.assertEqual(pg.tool_offset_at(len(pg) - 1), off_b)
        z = pg.axis_position("Z")
        self.assertAlmostEqual(float(z[row_b - 1]) + off_a[2], 0.0, places=6)
