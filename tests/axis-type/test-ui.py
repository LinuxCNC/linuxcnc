...
# X with A: F along X, the feed axis (G21 on its own line: F comes first)
mdi("G21")
peak, _ = mdi("G1 X10 A35.4 F600")
near("X with A at F600, mm/s", peak, 10.0, 0.1)

# A with V: F along A; motion measures along V, 8 times longer, so it
# runs 8 times faster and the move still takes 1 s
...
# the max velocity slider caps A alone and A with V (along A), not V alone
c.maxvel(2.0)
peak, _ = mdi("G1 V230 F3600")
near("V alone at F3600 under the slider, deg/s", peak, 60.0, 0.6)
peak, _ = mdi("G1 A47.4 F600")
near("A alone at F600 under the slider, mm/s", peak, 2.0, 0.02)
peak, secs = mdi("G1 A49.4 V246 F600")
near("A with V at F600 under the slider, motion rate", peak, 16.0, 0.16)
near("A with V at F600 under the slider, seconds", secs, 1.0, 0.1)
c.maxvel(400.0)
