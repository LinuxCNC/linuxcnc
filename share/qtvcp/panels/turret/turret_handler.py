...
class HandlerClass:
    ...
    def _build_estado(self, page):
        ...
        cgrid.addWidget(self._button("Referenciar", lambda: self._pulse("home")), 0, 0)
        cgrid.addWidget(self._button("Paso (+1)", lambda: self._pulse("step")), 0, 1)
        self.target = QSpinBox()
        self.target.setRange(1, 24)
        self.target.setValue(1)
        cgrid.addWidget(QLabel("Estacion:"), 1, 0)
        cgrid.addWidget(self.target, 1, 1)
        cgrid.addWidget(self._button("Ir", self._goto), 2, 0, 1, 2)
        cgrid.addWidget(self._button("Liberar pinza (sacar)", lambda: self._pulse("release")), 3, 0)
        cgrid.addWidget(self._button("Meter pinza", lambda: self._pulse("lock")), 3, 1)
        cgrid.addWidget(self._button("Reset fallas", lambda: self._pulse("reset")), 4, 0)
        cgrid.addWidget(self._button("Cancelar", lambda: self._pulse("cancel")), 4, 1)
        ...
        cgrid.addWidget(self.test_enable, 5, 0, 1, 2)
