...
- **OPERATOR_DISPLAY / `(DEBUG,)` messages not delivered to clients.** G-code `(DEBUG,msg)` / operator-display messages go to the gomc-server log only; the `gmi` ErrorChannel (`e.poll()`) never receives them, so UIs/tests can't read them. (tests/interp/oword-mdi-sub-update)
