...
- **modcompile doesn't provide the classic comp globals (`comp_id`, implicit `<errno.h>`).** A `.comp`'s C body that references `comp_id` (e.g. for `rtapi_shmem_new/delete`) fails to build under modcompile ("comp_id undeclared"), and `-EINVAL` needs an explicit `#include <errno.h>` (classic halcompile pulled it in). modcompile should expose the same component globals halcompile did. (tests/rtapi-shmem, after the singleton rework)
