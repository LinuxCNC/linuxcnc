...
- **jog/teleop + limit status.** After `teleop_enable(0)`, a joint jog errors "Mode is TELEOP, cannot jog joint"; and `status.limit[0]` reads -1 on a tripped min hard limit where classic expects a bitmask (1). (tests/hard-limits)
