...
- **`motion-logger` not ported to gomc** + **remove the RT-embedded parity trace.** The motmod-replacement command-logger is still NML-based (no cmod/bin). Plan (see `src/emc/motion-logger/GOMC_PORT_SPEC.md`): implement `motion-logger` as an **interceptor/proxy** cmod — registers `motctl`/`motstat` under its own instance name (milltask's `[EMCMOT]EMCMOT`), looks up the real motmod by `mot_instance=`, and logs + forwards every call (real motmod does real motion + real status; no faking). Idiomatic gomc (motmod already looks up kins/tp by instance name). Then rewire `tests/milltask-parity` to use it; then **delete the `#ifdef MILLTASK_PARITY_TRACE` `motcmd_trace()` hook from `src/emc/motion/command.c`** so production RT (motmod) carries no test instrumentation. One clean, upstream-aligned mechanism serves both the milltask-parity comparison and the 6 xfail tests (tests/motion-logger/*, tests/interp/m98m99/12, tests/abort/*crazy-move). Also a useful standalone debug tool.
