...
- **C O-word positional args not collected (`n_args` always 0).** A C-registered O-word (`interp_ext register_oword`) called as `o<name> call [a] [b]` receives `n_args=0`: `interp_python.cc:87` gathers args from the call frame's `#1..#30` via `find_named_param`, but those are not populated at C-O-word dispatch time. So a C O-word can be registered, dispatched, and return a value, but cannot see its call arguments. (tests/interp-ext covers register+dispatch+return; args are documented here.)
